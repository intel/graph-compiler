...
module @gemm attributes {gpu.container_module} {
  gpu.module @kernel {
    gpu.func @load_store_2d_dpas(%a: memref<8x16xf16, 1>, %b: memref<16x16xf16, 1>, %c: memref<8x16xf32, 1>) kernel {
        ...
        %a_tdesc = xegpu.create_nd_tdesc %a_tdesc_memref[0, 0] : memref<8x16xf16> -> !xegpu.tensor_desc<8x16xf16, #xegpu.block_tdesc_attr<memory_space = global>, #xegpu.sg_map<wi_layout = [1, 16], wi_data = [1, 1]>>
        %b_tdesc = xegpu.create_nd_tdesc %b_tdesc_memref[0, 0] : memref<16x16xf16> -> !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<memory_space = global>, #xegpu.sg_map<wi_layout = [1, 16], wi_data = [1, 1]>>
        %c_tdesc = xegpu.create_nd_tdesc %c_tdesc_memref[0, 0] : memref<8x16xf32> -> !xegpu.tensor_desc<8x16xf32, #xegpu.block_tdesc_attr<memory_space = global>, #xegpu.sg_map<wi_layout = [1, 16], wi_data = [1, 1]>>
               
        %a_loaded = xegpu.load_nd %a_tdesc <{l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<uncached>}> : !xegpu.tensor_desc<8x16xf16, #xegpu.block_tdesc_attr<memory_space = global>, #xegpu.sg_map<wi_layout = [1, 16], wi_data = [1, 1]>> -> vector<8x1xf16>
        %b_loaded = xegpu.load_nd %b_tdesc <{l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<uncached>}> : !xegpu.tensor_desc<16x16xf16, #xegpu.block_tdesc_attr<memory_space = global>, #xegpu.sg_map<wi_layout = [1, 16], wi_data = [1, 1]>> -> vector<16x1xf16>
        %c_loaded = xegpu.load_nd %c_tdesc <{l1_hint = #xegpu.cache_hint<cached>, l2_hint = #xegpu.cache_hint<uncached>}> : !xegpu.tensor_desc<8x16xf32, #xegpu.block_tdesc_attr<memory_space = global>, #xegpu.sg_map<wi_layout = [1, 16], wi_data = [1, 1]>> -> vector<8x1xf32>

        %b_loaded_d = vector.shape_cast %b_loaded : vector<16x1xf16> to vector<1x16xf16>
        %d = xegpu.dpas %a_loaded, %b_loaded_d, %c_loaded : vector<8x1xf16>, vector<1x16xf16>, vector<8x1xf32> -> vector<8x1xf32>

        xegpu.store_nd %d, %c_tdesc <{l1_hint = #xegpu.cache_hint<write_back>, l2_hint = #xegpu.cache_hint<uncached>}>: vector<8x1xf32>, !xegpu.tensor_desc<8x16xf32, #xegpu.block_tdesc_attr<memory_space = global>, #xegpu.sg_map<wi_layout = [1, 16], wi_data = [1, 1]>>
        gpu.return 
    }
  }
