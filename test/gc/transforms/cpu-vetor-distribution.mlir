...
func.func @entry(%arg0: tensor<256x256xf32>) -> tensor<256x256xf32> {
    %cst = arith.constant dense<1.000000e+00> : tensor<256x256xf32>
    %cst_0 = arith.constant 0.000000e+00 : f32
    %0 = tensor.empty() : tensor<256x256xf32>
    // %2 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d2, d1)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%arg0, %cst : tensor<256x128xf32>, tensor<128x256xf32>) outs(%0 : tensor<256x256xf32>) {
    // ^bb0(%in: f32, %in_3: f32, %out: f32):
    //   %7 = arith.addf %out, %in : f32
    //   linalg.yield %7 : f32
    // } -> tensor<256x256xf32>
    %filled = linalg.fill ins(%cst_0 : f32) outs(%0 : tensor<256x256xf32>) -> tensor<256x256xf32>
    %2 = linalg.add ins(%arg0, %cst : tensor<256x256xf32>, tensor<256x256xf32>) outs(%filled: tensor<256x256xf32>) -> (tensor<256x256xf32>)
    // %3 = linalg.transpose ins(%2 : tensor<256x256xf32>) outs(%0 : tensor<256x256xf32>) permutation = [1, 0]
