    ...
    if depth == len(affine_from):
        ...
        for i in range(len(block.arguments)):
            ...
            if i + len(op.results) < len(op.regions[0].blocks[0].arguments):
                ...
                # output argument
                block_arg[block_cache.arg[i]] = var[
                    cache.res[i + len(op.results) - len(block.arguments)]
                ][index]
        ...
        for i in range(it[0], it[1], it[2]):
            ...
            generic_loop(
                cache, op, depth + 1, iterspace, affine_from, affine_to, var, loop_var
            )
    ...
    if depth == len(in_shape):
        ...
        for i in range(in_shape[depth]):
            if reduce_axis:
                ...
                reduce_loop(
                    cache, op, depth + 1, in_shape, var, in_idx, out_idx, reduced_axis
                )
...
def ref_reduce(
    ...
    reduce_loop(
        cache, op, 0, in_shape, tensors, [0] * len(in_shape), [0] * len(out_shape), 0
    )
    return
