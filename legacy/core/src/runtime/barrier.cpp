...
static void make_trace(int in_or_out, int count) {
  if (sc_is_trace_enabled()) {
    sc_make_trace_kernel(3, in_or_out, count);
  }
}
static void make_trace_prefetch(int in_or_out, int count) {
  if (sc_is_trace_enabled()) {
    sc_make_trace_kernel(4, in_or_out, count);
  }
}
#else
...
sc_arrive_at_barrier(gc::runtime::barrier_t *b,
                     gc::runtime::barrier_idle_func idle_func,
                     void *idle_args) {
  make_trace(0, 0);
...
extern "C" SC_API void sc_init_barrier(gc::runtime::barrier_t *b,
                                       int num_barriers,
