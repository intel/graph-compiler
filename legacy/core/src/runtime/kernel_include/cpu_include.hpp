...
extern "C" void *sc_aligned_malloc(gc::runtime::stream_t *stream,
                                   size_t sz) noexcept;
extern "C" void sc_aligned_free(gc::runtime::stream_t *stream,
                                void *p) noexcept;
extern "C" void *sc_thread_aligned_malloc(gc::runtime::stream_t *stream,
                                          size_t sz) noexcept;
extern "C" void sc_thread_aligned_free(gc::runtime::stream_t *stream,
                                       void *p) noexcept;
...
template <typename T, typename T2> T sc_reinterpret(T2 v) {
  union {
    T v1;
    T2 v2;
  } val;
...
inline bool sc_isnan(float v) { return std::isnan(v); }

inline bool sc_isnan(uint16_t v) {
  return (v & 0x7F80) == 0x7F80 && (v & 0x007F);
}

template <class T1, class T2> inline T1 sc_saturated_cast(const T2 &v) {
  return static_cast<T1>(v);
}

template <> inline int8_t sc_saturated_cast(const int32_t &v) {
  if (v > 127) {
    return 127;
  } else if (v < -128) {
    return -128;
...
template <> inline uint8_t sc_saturated_cast(const int32_t &v) {
  if (v > 255) {
    return 255;
  } else if (v < 0) {
    return 0;
...
template <> inline int8_t sc_saturated_cast(const float &v) {
  int iv = std::round(v);
  if (iv > 127) {
    return 127;
  } else if (iv < -128) {
...
template <> inline uint8_t sc_saturated_cast(const float &v) {
  int iv = std::round(v);
  if (iv > 255) {
    return 255;
  } else if (iv < 0) {
...
template <class T1, class T2> inline T1 sc_round_and_cast(const T2 &v) {
  return static_cast<T1>(v);
}

template <> inline int32_t sc_round_and_cast(const float &v) {
  return std::roundf(v);
}

inline float sc_gather(float const *a, int const &b) { return a[b]; }

inline float sc_pow(float const &a, float const &b) { return powf(a, b); }

#ifdef __AVX512F__
INLINE vec_f32x16 sc_permutex2var(vec_f32x16 const &a, vec_u32x16 const &idx,
                                  vec_f32x16 const &b) {
  return _mm512_permutex2var_ps(a.v, idx.v, b.v);
}
INLINE vec_f32x8 sc_permutex2var(vec_f32x8 const &a, vec_u32x8 const &idx,
                                 vec_f32x8 const &b) {
  return _mm256_permutex2var_ps(a.v, idx.v, b.v);
}
INLINE vec_f32x4 sc_permutex2var(vec_f32x4 const &a, vec_u32x4 const &idx,
                                 vec_f32x4 const &b) {
  return _mm_permutex2var_ps(a.v, idx.v, b.v);
}
INLINE vec_f32x16 sc_permutex2var(vec_f32x16 const &a, vec_s32x16 const &idx,
                                  vec_f32x16 const &b) {
  return _mm512_permutex2var_ps(a.v, idx.v, b.v);
}
INLINE vec_f32x8 sc_permutex2var(vec_f32x8 const &a, vec_s32x8 const &idx,
                                 vec_f32x8 const &b) {
  return _mm256_permutex2var_ps(a.v, idx.v, b.v);
}
INLINE vec_f32x4 sc_permutex2var(vec_f32x4 const &a, vec_s32x4 const &idx,
                                 vec_f32x4 const &b) {
  return _mm_permutex2var_ps(a.v, idx.v, b.v);
}
#endif
