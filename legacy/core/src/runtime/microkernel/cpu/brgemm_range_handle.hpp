...
struct brg_range_handle_t {
  struct extra_arg_t {
    ...
    extra_arg_t(float beta, int LDA, int LDB, int LDC, int stride_a,
        ...
        : beta(beta), LDA(LDA), LDB(LDB), LDC(LDC), stride_a(stride_a),
          stride_b(stride_b), dtypeA(dtypeA), dtypeB(dtypeB),
          brg_attrs(brg_attrs) {}
  };
  ...
  brg_range_handle_t(int M, int N, int K, int LDA, int LDB, int LDC, float beta,
                     int dtypeA, int dtypeB, const void *brg_attrs,
                     int M_tail_value, int N_tail_value, int K_tail_value);
  brg_range_handle_t(int M, int N, int K, int LDA, int LDB, int LDC,
                     int stride_a, int stride_b, float beta, int dtypeA,
                     int dtypeB, const void *brg_attrs, int M_tail_value,
                     int N_tail_value, int K_tail_value);
  brgemm_kernel_info *get_linear_kernel(int M_real, int N_real,
                                        int K_real) const;
  void brg_list_call(int M_real, int N_real, int K_real, const void **A_list,
                     const void **B_list, void *C, int num, int stride_a,
                     int stride_b, int len, int dtypeA, int dtypeB,
