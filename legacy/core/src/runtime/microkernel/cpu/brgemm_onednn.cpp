...
static int64_t get_brgemm_attr_dispatch_avx(const attrs_setting_t &attrs) {
  for (int i = 0; i < attrs.num_; i++) {
    ...
    if (it.first == attr_key::dispatch_avx) {
      return it.second;
    }
  }
  return 0;
  ...
  brgemm_kernel_info *getInstance(float alpha, float beta, int LDA, int LDB,
    ...
    auto choose_isa_type = [&]() {
      ...
      if (attrs_setting_ptr) {
        int64_t dispatch_avx = get_brgemm_attr_dispatch_avx(*attrs_setting_ptr);
        if (dispatch_avx) {
          SC_MODULE_WARN << "Dispatch to AVX isa: " << fallback_isa
      ...
      int max_rd_block = dnnl_dtypeA == dnnl_bf16 ? 32
                         : (dnnl_dtypeA == dnnl_s8 || dnnl_dtypeA == dnnl_u8)
                             ? 64
                             : -1;
    ...
    if (arg.K == -1 && arg.LDB == -1) {
      ...
      status = brdgmm_desc_init(&desc, isa_type, arg.brg_type, dnnl_dtypeA,
                                dnnl_dtypeB, false, brgemm_row_major, arg.alpha,
                                arg.beta, arg.LDA, arg.LDC, arg.M, arg.N,
                                &stride_info);
    } else {
      status = brgemm_desc_init(&desc, isa_type, arg.brg_type, dnnl_dtypeA,
                                dnnl_dtypeB, false, false, brgemm_row_major,
                                arg.alpha, arg.beta, arg.LDA, arg.LDB, arg.LDC,
                                arg.M, arg.N, arg.K, &stride_info);
    }
    ...
    if (status == dnnl::impl::status::success) {
      ...
      dnnl::impl::graph::gc::runtime::thread_local_buffer_t::tls_buffer()
          .amx_buffer_.cur_palette = nullptr;
...
void brg_range_handle_t::init_func(brgemm_batch_kind_t brg_type, int M, int N,
                                   ...
                                   int dtypeA, int dtypeB,
                                   const void *brg_attrs, int M_tail_value,
  ...
  if (total_size <= linear_cache_capacity) {
    ...
    for (int i = 0; i < total_size; i++) {
      int M_real =
          get_real_value(i / (N_size * K_size), M_tail_value, M_upper_bound);
      int N_real =
          get_real_value(i / K_size % N_size, N_tail_value, N_upper_bound);
      int K_real = get_real_value(i % K_size, K_tail_value, K_upper_bound);
      linear_cache.emplace_back(g_brg_desc_s.getInstance(
          1.f, beta, LDA, LDB, LDC, M_real, N_real, K_real,
          static_cast<int>(stride_a * get_dtype_sizeof(dtypeA)),
          static_cast<int>(stride_b * get_dtype_sizeof(dtypeB)), brg_type,
          dtypeA, dtypeB, brg_attrs, nullptr, nullptr));
    }
  } else {
    extra_args = std::make_shared<extra_arg_t>(
        beta, LDA, LDB, LDC, stride_a, stride_b, dtypeA, dtypeB, brg_attrs);
  }
}
brg_range_handle_t::brg_range_handle_t(int M, int N, int K, int LDA, int LDB,
                                       int LDC, float beta, int dtypeA,
                                       int dtypeB, const void *brg_attrs,
                                       int M_tail_value, int N_tail_value,
                                       int K_tail_value)
    : M_upper_bound(M), N_upper_bound(N), K_upper_bound(K),
      M_tail_value(M_tail_value), N_tail_value(N_tail_value),
...
brg_range_handle_t::brg_range_handle_t(int M, int N, int K, int LDA, int LDB,
                                       ...
                                       float beta, int dtypeA, int dtypeB,
                                       const void *brg_attrs, int M_tail_value,
                                       int N_tail_value, int K_tail_value)
    : M_upper_bound(M), N_upper_bound(N), K_upper_bound(K),
      M_tail_value(M_tail_value), N_tail_value(N_tail_value),
...
brgemm_kernel_info *brg_range_handle_t::get_linear_kernel(int M_real,
                                                          int N_real,
                                                          int K_real) const {
  int M_size = get_range_size(M_tail_value, M_upper_bound);
  int N_size = get_range_size(N_tail_value, N_upper_bound);
  ...
  if (linear_idx < linear_cache.size()) {
    return linear_cache[linear_idx];
  }
  return nullptr;
}
void brg_range_handle_t::brg_list_call(
    int M_real, int N_real, int K_real, const void **A_list,
    const void **B_list, void *C, int num, int stride_a, int stride_b, int len,
    int dtypeA, int dtypeB, const void *top_pad, const void *bottom_pad,
  ...
  if (!brg) {
    assert(extra_args);
    brg = g_brg_desc_s.getInstance(
        1.f, extra_args->beta, extra_args->LDA, extra_args->LDB,
        extra_args->LDC, M_real, N_real, K_real, 0, 0, brgemm_addr, dtypeA,
        dtypeB, extra_args->brg_attrs, nullptr, nullptr);
  }
  dnnl_brgemm_list_call(brg, A_list, B_list, C, num, stride_a, stride_b, len,
                        dtypeA, dtypeB, top_pad, bottom_pad, stream);
}

void brg_range_handle_t::brg_strd_call(
    int M_real, int N_real, int K_real, const void *A, const void *B, void *C,
    int num, const void *top_pad, const void *bottom_pad,
    dnnl::impl::graph::gc::runtime::stream_t *stream) {
  brgemm_kernel_info *brg = get_linear_kernel(M_real, N_real, K_real);
  if (!brg) {
    // default use runtime kernel creation.
    ...
    brg = g_brg_desc_s.getInstance(
        1.f, extra_args->beta, extra_args->LDA, extra_args->LDB,
        extra_args->LDC, M_real, N_real, K_real,
...
static void *get_amx_tile_buf(brgemm_kernel_info *brg_desc,
  ...
  return do_get_amx_tile_buf(brg_desc->palette_, stream, amx_exclusive,
                             need_config_amx);
}
...
SC_API void dnnl_brgemm_call_range(brg_range_handle_t *brg_range_desc,
                                   int M_real, int N_real, int K_real,
                                   const void *A, const void *B, void *C,
                                   int num, const void *top_pad,
                                   const void *bottom_pad,
                                   gc::runtime::stream_t *stream) {
  brg_range_desc->brg_strd_call(M_real, N_real, K_real, A, B, C, num, top_pad,
                                bottom_pad, stream);
}
...
SC_API void dnnl_brgemm_call_postops(brgemm_kernel_info *brg_desc,
  ...
  if (top_pad || bottom_pad) {
    ...
    brgemm_kernel_execute_postops(
        brg_desc->brg_kernel_, num, (void **)A, (void **)B, batch,
        (void *)c_buf, (void *)C,
...
SC_API void dnnl_brgemm_list_call(brgemm_kernel_info *brg_desc,
  ...
  if (bottom_pad) {
    for (int i = 0; i < len; ++i) {
      for (int j = 0; j < num; ++j) {
        batch[i * num + j].vvpad.bottom = ((int *)bottom_pad)[i * num + j];
      }
    }
  }
  bool amx_exclusive = false;
...
SC_API void dnnl_brgemm_list_call_range(
  ...
  brg_range_desc->brg_list_call(M_real, N_real, K_real, A_list, B_list, C, num,
                                stride_a, stride_b, len, dtypeA, dtypeB,
                                top_pad, bottom_pad, stream);
}

SC_API void dnnl_brgemm_list_call_postops(
    brgemm_kernel_info *brg_desc, const void **A_list, const void **B_list,
    void *C, int num, int stride_a, int stride_b, int len, int dtypeA,
...
SC_API int dnnl_brgemm_init_list_update(
  ...
  int ret = dnnl_brgemm_list_update_func(
      ...
      dtypeA, dtypeB, beta, brg_attrs, bd_mask, postops_setting, top_pad,
      bottom_pad, postops_data, c_buf, stream);
  return ret;
}
