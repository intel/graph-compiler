...
static void make_trace(int in_or_out, int count) {
  if (sc_is_trace_enabled()) {
    sc_make_trace_kernel(2, in_or_out, count);
  }
}
...
void thread_manager::thread_pool_state::wait_all() {
  ...
  bool has_idle_func =
      idl_f && (execution_flags & thread_pool_flags::THREAD_POOL_RUN_IDLE_FUNC);
  int count = 0;
...
static void worker_func(thread_manager *ths, int tid) {
  ...
  while (true) {
    bool has_idle_func =
        idl_f && (exec_flags & thread_pool_flags::THREAD_POOL_RUN_IDLE_FUNC);
    int count = 0;
    if (has_idle_func && (st = ths->state.trigger) != current_job_id) {
      count =
          idl_f(&ths->state.trigger, current_job_id, tid, ths->state.idle_args);
    }
...
struct mtp_threadpool_adapter_t {
  ...
  static thread_manager *all_thread_prepare(thread_manager *ths,
  ...
  static void main_thread(thread_manager *ths, thread_manager::main_func_t f,
                          runtime::stream_t *stream, void *mod_data,
                          generic_val *args) {
#if SC_CPU_THREADPOOL == SC_THREAD_POOL_CUSTOM
  ...
  static void single_thread(thread_manager *ths, thread_manager::main_func_t f,
                            runtime::stream_t *stream, void *mod_data,
                            generic_val *args) {
    auto &tls = thread_local_buffer_t::tls_buffer();
...
static void do_dispatch(thread_manager *s, int tid) {
  ...
  bool disable_rolling =
      s->state.execution_flags &
...
void sc_parallel_call_managed(void (*pfunc)(void *, void *, int64_t,
                                            generic_val *),
                              uint64_t execution_flags, void *rtl_ctx,
                              void *module_env, int64_t begin, int64_t end,
  ...
  if (execution_flags &
      dnnl::impl::graph::gc::runtime::thread_pool_flags::THREAD_POOL_EXIT) {
    return;
  }
  stream->state.wait_all();

  if (execution_flags & runtime::thread_pool_flags::THREAD_POOL_RUN_IDLE_FUNC) {
