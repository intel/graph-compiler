...
union dispatch_key {
  ...
  struct meta {
    ...
    static constexpr uint64_t FORMAT_MASK =
        ((static_cast<uint64_t>(1) << FORMAT_BITS) - 1) << FORMAT_BITS_OFFSET;
  ...
  template <uint64_t format_kind, uint64_t... args> struct linear_converter {
    static constexpr int idx = 1 + linear_converter<args...>::idx;
    static uint64_t call(dispatch_key v) {
      if (v.format_kind_ == uint32_t(format_kind)) {
        return idx;
      }
  ...
  template <uint64_t format_kind> struct linear_converter<format_kind> {
...
namespace std {
template <> struct hash<::dnnl::impl::graph::gc::runtime::dispatch_key> {
  std::size_t
  operator()(const ::dnnl::impl::graph::gc::runtime::dispatch_key &in) const {
