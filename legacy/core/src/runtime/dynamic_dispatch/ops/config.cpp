...
void get_managed_matmul_config(
  ...
  auto cal_cost = [&](int i) {
    ...
    if ((K >= 1024 && is_int8 && !is_amx) || (K >= 512 && is_f32 && is_scpi)) {
  ...
  if (is_int8 && N <= 512 && K <= 512) {
      ...
      if (is_int8) {
        if (K >= 4096 && N >= 4096 && M <= 4) {
          ...
          for (auto i = split_idx_init;
            ...
            float new_cost =
                num_threads / possible_splits.at(i) / possible_splits.at(i) -
                split_ratio;
            if (new_cost >= 0 && new_cost < cost && split_ratio != 0.) {
              cost = new_cost;
              split_idx = i;
            }
          }
          ...
          split_idx =
              K >= 4096
                  ? (N < 2 * K
                         ? (N <= K / 2 && possible_splits.size() > 2 ? 2 : 1)
                         : (K >= 4096 ? 1 : 0))
                  : 0;
        }
        ...
        if (K >= 4096) {
          if (M < 16) {
            split_idx = possible_splits.size() > 6 ? (N >= 4 * K ? 2 : 3)
                                                   : (num_threads > 32 ? 1 : 0);
            if (N >= 10 * K) {
              if (M > 16 || num_threads <= 28) {
                split_idx = 1;
              }
            }
          } else {
    ...
    if (K >= 1024 && K >= 2 * N && thread_factors.size() > 3) {
      ...
      N_split_num = N_split_num / K_split_num > 0 ? N_split_num / K_split_num
                                                  : N_split_num;
    ...
    if (is_int8 && !is_amx) {
      ...
      int K_split_num = 1;
      if (K >= 16 * M) {
        K_split_num = thread_factors.size() > 2 ? thread_factors.at(1) : 1;
      }
      N_split_num = num_threads / K_split_num;
    }
  }
  int single_M = utils::divide_and_ceil(utils::divide_and_ceil(M, iim_block),
                                        M_split_num) *
                 iim_block;
  int single_N = utils::divide_and_ceil(utils::divide_and_ceil(N, iin_block),
                                        N_split_num) *
                 iin_block;
  int single_K = K;
  ...
  if (single_K >= single_K_threshold) {
    ...
    int L2_K = utils::divide_and_ceil(
                   utils::divide_and_ceil(single_K, iik_block), K_sub_block) *
               iik_block;
