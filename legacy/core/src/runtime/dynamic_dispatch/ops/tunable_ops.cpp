...
static int check_and_set_matmul_core_impl(
    runtime::op_dispatch_tables_t *op_table, runtime::dispatch_key *data_fmt_st,
  ...
  if (impl_kind_table) {
    uint64_t keys[3] = {static_cast<uint64_t>(M_blk),
                        static_cast<uint64_t>(N_blk),
                        static_cast<uint64_t>(K_blk)};
...
static int check_and_set_managed_matmul_core_impl(
  ...
  const int N = utils::divide_and_ceil(weight_dyn_tsr->dims_[1], N_blk) * N_blk;
  ...
  get_managed_matmul_config(
      runtime::get_runtime_target_machine(), M_split_num, N_split_num,
      M_sub_block, N_sub_block, K_sub_block, im_loop_order, M, N, K, M_blk,
      N_blk, K_blk, sizeofdtypeA, sizeofdtypeC, is_int8, is_f32 || no_vnni_f16,
      /*is_dynamic*/ true);
  uint64_t keys[6] = {
      static_cast<uint64_t>(M_split_num), static_cast<uint64_t>(N_split_num),
      static_cast<uint64_t>(M_sub_block), static_cast<uint64_t>(N_sub_block),
      static_cast<uint64_t>(K_sub_block), static_cast<uint64_t>(im_loop_order)};
  ...
  if (K_split_num > 1) {
    return mmm_impl_kind_t::is_partial;
  }
...
typedef int (*impl_set_func)(runtime::op_dispatch_tables_t *,
                             ...
                             runtime::dynamic_tensor_t *,
                             runtime::dynamic_tensor_t *,
                             runtime::dynamic_tensor_t *, int, int, int, int &);
extern "C" void query_format_matmul_common_process(
    void *table, void *out, void *data, void *weight, void *ori_data,
    void *ori_weight, uint64_t *out_fmt, uint64_t *data_fmt,
    uint64_t *weight_fmt, uint64_t *ori_data_fmt, uint64_t *ori_weight_fmt,
    uint64_t *out_size, void *kernel, int *impl_alg, impl_set_func impl_func,
    bool is_mmm = false) {
  ...
  // update dyn_mask
  out_dyn_tsr->dyn_mask_ = data_dyn_tsr->dyn_mask_ | weight_dyn_tsr->dyn_mask_;
  // M mask
  ...
  if (data_fmt_st->is_plain() || data_fmt_st->ndims() == data_ndims) {
    ...
    if ((!is_mmm && (M % M_blk || K % K_blk || !data_fmt_st->is_plain())) ||
        (is_mmm && data_dyn_tsr->dtype_ == uint32_t(sc_data_etype::S8) &&
         K % K_blk)) {
  ...
  if (weight_fmt_st->is_plain() || weight_fmt_st->ndims() == weight_ndims) {
    ...
    if (N % N_blk || K % K_blk || is_vnni || !weight_fmt_st->is_plain()) {
      ...
      if (is_vnni) {
        weight_fmt_st->set(weight_ndims + 2, K_axis);
      }
  ...
  if (is_mmm) {
    ...
    *(reinterpret_cast<void **>(kernel) +
      static_cast<int>(impl_alg == nullptr)) = func;
  }
...
static int check_and_set_conv_fwd_impl(runtime::op_dispatch_tables_t *op_table,
                                       ...
                                       runtime::dispatch_key *out_fmt_st, int N,
                                       int P, int Q, int K, int C, int k_blk,
                                       bool is_bf16, bool dyn_bs, bool dyn_h,
                                       bool dyn_w, bool is_conv_1x1) {
  // query impl kind here. default return normal impl kind.
  ...
  for (size_t i = 0; i < num_threads_candidates; ++i) {
    h_threads_.push_back(threads_candidates.size() > i &&
                                 P > threads_candidates[i]
  ...
  if (impl_kind_table) {
    ...
    if (N < num_threads) {
      if (K >= 512) {
        oc_threads = *(std::find_if(
            oc_threads_.rbegin(), oc_threads_.rend(), [&](int split) {
              return split == 1 ||
                     (K / k_blk % split == 0 && num_threads % split == 0);
            }));
      }
      ...
      if (N == 1) {
        ...
        for (int i = h_threads_.size() - 1; i >= 0; --i) {
          if (P > (2 ^ (i + 2))) {
            h_threads =
                std::max(h_threads,
                         num_threads % h_threads_[3] == 0 ? h_threads_[3] : 1);
          }
        }
      }
    ...
    if (P % h_threads == 0 && num_threads <= 4) {
      if (Q <= 16)
        im_h_blk = P / h_threads % 4 == 0 ? 4 : P / h_threads % 2 == 0 ? 2 : 1;
      else if (Q <= 32)
        im_h_blk = P / h_threads % 2 == 0 && P % h_threads == 0 ? 2 : 1;
    }

    uint64_t keys[4] = {
        static_cast<uint64_t>(h_threads), static_cast<uint64_t>(oc_threads),
        static_cast<uint64_t>(im_h_blk), static_cast<uint64_t>(im_w_blk)};
...
extern "C" void infer_shape_conv_fwd_op(void *out, void *data, void *weight,
  ...
  int pads_begin[3] = {op_info.pads_begin_d, op_info.pads_begin_h,
                       op_info.pads_begin_w};
  int pads_end[3] = {op_info.pads_end_d, op_info.pads_end_h,
                     op_info.pads_end_w};
  int dilations[3] = {op_info.dilation_d, op_info.dilation_h,
                      op_info.dilation_w};
  ...
  for (int i = 2; i < out_ndims; i++) {
    out_dyn_tsr->dims_[i] = calc_out_shapes(
        data_dims[i], weight_dyn_tsr->dims_[i], pads_begin[i + offset],
        pads_end[i + offset], strides[i + offset], dilations[i + offset]);
  }
  out_dyn_tsr->dims_[1] = OC;
}

extern "C" void query_format_conv_fwd_core_op(
    void *table, void *out, void *data, void *weight, void *ori_data,
    void *ori_weight, uint64_t *out_fmt, uint64_t *data_fmt,
    uint64_t *weight_fmt, uint64_t *ori_data_fmt, uint64_t *ori_weight_fmt,
    uint64_t *out_size, void *kernel, int *impl_alg) {
  // update output shape and mask.
  runtime::dynamic_tensor_t *out_dyn_tsr =
      reinterpret_cast<runtime::dynamic_tensor_t *>(out);
  runtime::dynamic_tensor_t *data_dyn_tsr =
  ...
  dyn_conv_fwd_runtime_info_t info =
      *op_table->op_info_.unchecked_get_as<dyn_conv_fwd_runtime_info_t>();
  ...
  bool has_pad =
      info.pads_begin_h > 0 || info.pads_begin_w > 0 || info.pads_begin_d > 0;
  bool is_f32 = sc_data_etype(data_dyn_tsr->dtype_) == sc_data_etype::F32;
  bool no_vnni_f16 = get_default_context()->machine_.cpu_flags_.fAVX512FP16 &&
                     sc_data_etype(data_dyn_tsr->dtype_) == sc_data_etype::F16;
  auto default_block = get_dyn_conv_default_block(
      is_conv_1x1, utils::get_sizeof_etype(sc_data_etype(data_dyn_tsr->dtype_)),
  ...
  if (weight_ndims == 4) {
    ...
    if (weight_fmt_st->ndims() == weight_ndims) {
      ...
      weight_fmt_st->set(weight_ndims + 1, k_axis);
      if (is_vnni) {
        weight_fmt_st->set(weight_ndims + 2, c_axis);
      }
      weight_fmt_st->is_plain_ = 0;
    } else {
      assert((!is_vnni && weight_fmt_st->ndims() == weight_ndims + 2) ||
  ...
  *out_fmt = reinterpret_cast<uint64_t *>(value)[0];
  ...
  auto *out_fmt_st = reinterpret_cast<runtime::dispatch_key *>(&cp_out_fmt);
  auto &kernel_table = op_table->kernel_table_;
  bool is_bf16_weight = weight_dyn_tsr->dtype_ == uint32_t(sc_data_etype::BF16);
  if (kernel_table) {
    check_and_set_conv_fwd_impl(op_table, data_fmt_st, weight_fmt_st,
                                out_fmt_st, BS, OH, OW, OC, IC, k_block,
                                is_bf16_weight, is_BS_dynamic, is_IH_dynamic,
                                is_IW_dynamic, is_conv_1x1);
    uint64_t keys[3] = {cp_data_fmt, cp_weight_fmt, cp_out_fmt};
    void *func = runtime::run_query_and_wait(op_table->kernel_dispatch_func_,
                                             kernel_table.get(), keys, 3);
    assert(func);
    ...
    *reinterpret_cast<void **>(kernel) = func;
    ...
    assert(impl_alg);
    *impl_alg = check_and_set_conv_fwd_impl(
        op_table, data_fmt_st, weight_fmt_st, out_fmt_st, BS, OH, OW, OC, IC,
        k_block, is_bf16_weight, is_BS_dynamic, is_IH_dynamic, is_IW_dynamic,
        is_conv_1x1);
  }
  // avoid internal status change in multi thread case.
  *data_fmt = cp_data_fmt;
...
extern "C" void
query_format_matmul_core_op(void *table, void *out, void *data, void *weight,
                            void *ori_data, void *ori_weight, uint64_t *out_fmt,
                            uint64_t *data_fmt, uint64_t *weight_fmt,
                            uint64_t *ori_data_fmt, uint64_t *ori_weight_fmt,
                            uint64_t *out_size, void *kernel, int *impl_alg) {
  query_format_matmul_common_process(
      table, out, data, weight, ori_data, ori_weight, out_fmt, data_fmt,
      weight_fmt, ori_data_fmt, ori_weight_fmt, out_size, kernel, impl_alg,
      check_and_set_matmul_core_impl);
}

extern "C" void query_format_managed_matmul_core_op(
    void *table, void *out, void *data, void *weight, void *ori_data,
    void *ori_weight, uint64_t *out_fmt, uint64_t *data_fmt,
    uint64_t *weight_fmt, uint64_t *ori_data_fmt, uint64_t *ori_weight_fmt,
    uint64_t *out_size, void *kernel, int *impl_alg) {
