...
SC_API void infer_shape_pooling_fusible_op(void *out, void *in,
                                           dyn_pooling_runtime_info_t &op_info);
SC_API void infer_shape_reduce_op(void *out, void *in, int *rd_axis,
                                  int num_axis);
SC_API void infer_shape_transpose_op(void *out, void *in, int *tr_axis,
                                     int num_axis);
SC_API void infer_shape_tensor_view_op(void *out, void *in, int64_t *old_axis,
                                       int num_old_axis, int64_t *new_axis,
                                       int num_new_axis);
...
SC_API void query_format_matmul_core_op(void *table, void *out, void *data,
                                        void *weight, uint64_t *out_fmt,
                                        uint64_t *data_fmt,
                                        uint64_t *weight_fmt,
                                        uint64_t *out_size, void *kernel,
                                        int *impl_alg);
SC_API void query_format_managed_matmul_core_op(
    void *table, void *out, void *data, void *weight, uint64_t *out_fmt,
    uint64_t *data_fmt, uint64_t *weight_fmt, uint64_t *out_size, void *kernel,
    int *impl_alg);
...
SC_API void query_combined_fused_op(void *table, uint64_t **combined_keys,
                                    int *combined_algs, int *each_op_num_key,
                                    int op_num, void *kernel);
};
