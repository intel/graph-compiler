...
namespace gc {
namespace runtime {
size_t dyn_dispatch_table_t::compute_linear_index(uint64_t *keys,
...
void *dyn_dispatch_table_t::dispatch(dispatch_table_t *ths, uint64_t *keys,
                                     uint64_t num_keys) {
...
dyn_dispatch_table_t::dyn_dispatch_table_t(
    std::vector<format_arg_t> &&format_args, block_extract_func_t block_to_idx,
    size_t number_of_blocks)
    : block_to_idx_(block_to_idx), number_of_blocks_(number_of_blocks),
...
template <uint64_t idx, uint64_t total_size> struct unrolled_find_t {
...
template <uint64_t idx> struct unrolled_find_t<idx, 1> {
  static uint64_t call(uint32_t *cur, uint32_t v) {
    assert(cur[idx] == v);
    return idx;
  }
...
static void *semi_dispatch(dispatch_table_t *ths, uint64_t *keys,
                           uint64_t num_keys) {
  dyn_dispatch_table_t *table = static_cast<dyn_dispatch_table_t *>(ths);
  auto result_idx =
      semi_dyn_dispatch_t<0, 0, 1, num_candidates...>::call(table, keys);
