...
void *filo_memory_pool_t::alloc(runtime::stream_t *stream, size_t sz) {
  if (unlikely(!buffers_)) {
    buffers_ =
        memory_block_t::make(stream, get_block_size(sz), nullptr, nullptr);
    current_ = buffers_;
  }
  do {
    ...
    if (!current_->next_) {
      current_->next_ =
          memory_block_t::make(stream, get_block_size(sz), current_, nullptr);
    }
...
extern "C" SC_API void *sc_aligned_malloc(stream_t *pstream,
                                          size_t sz) noexcept {
  if (sz == 0) {
    return nullptr;
  }
...
extern "C" SC_API void *sc_thread_aligned_malloc(stream_t *pstream,
                                                 size_t sz) noexcept {
  return runtime::get_tls(pstream).thread_memory_pool_.alloc(pstream, sz);
}

extern "C" SC_API void sc_thread_aligned_free(stream_t *pstream,
                                              void *p) noexcept {
