...
extern "C" void sc_parallel_call_cpu_with_env_impl(
    ...
    void *rtl_ctx, void *module_env, int64_t begin, int64_t end, int64_t step,
    generic_val *args) {
  ...
  int nthr = adjust_num_threads(
      std::min(get_num_threads(), dnnl_get_current_num_threads()), num_jobs);
  if (nthr) {
    dnnl::impl::parallel(nthr, [&](int ithr, int nthr) {
      runtime::thread_local_buffer_t::tls_buffer()
          .additional_->linear_thread_id_ = ithr;
      auto f = [&](int64_t i) {
...
static int get_num_threads() { return get_default_threads(); }
...
static int get_in_parallel() { return 0; }
