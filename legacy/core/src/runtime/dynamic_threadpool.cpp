...
queue::~queue() { delete[] base_; }

void queue::lock() noexcept {
  uint64_t expected = 0;
  while (
      !lock_.compare_exchange_strong(expected, 1, std::memory_order_acq_rel)) {
    expected = 0;
  }
}

static bool try_enqueue_fast_slot(std::atomic<work_item *> &fast_slot,
                                  work_item *item) {
  auto old_slot = fast_slot.load(std::memory_order_relaxed);
  if (!old_slot) {
    // if fast slot is empty
    if (fast_slot.compare_exchange_strong(old_slot, item,
...
bool queue::enqueue(work_item *item) noexcept {
  if (try_enqueue_fast_slot(fast_slot_, item)) {
    return true;
  }
  if (volatile_length() >= size_ - 1) {
    return false;
  }
  lock();
...
bool queue::jump_queue(work_item *item) noexcept {
  ...
  if (unlikely(volatile_length() >= size_ - 1)) {
    return false;
  }
  lock();
  // double check locking
...
work_item *queue::dequeue() noexcept {
  if (auto old_slot = fast_slot_.load(std::memory_order_relaxed)) {
    ...
    auto ret = old_slot;
    if (fast_slot_.compare_exchange_strong(old_slot, nullptr,
                                           std::memory_order_acq_rel)) {
      return ret;
    }
  }
  if (volatile_length() == 0) {
    return nullptr;
  }
  lock();
  // double check locking
...
threadpool_scheduler::threadpool_scheduler(stream_t *stream,
                                           uint64_t queue_size,
...
threadpool_scheduler::~threadpool_scheduler() { aligned_free(queues_); }

static bool find_queue_and_insert_work(threadpool_scheduler *scheduler,
                                       work_item *item, uint64_t &outqid,
                                       queue *&theq) {
...
static void deref_buffer(void *ptr) {
  auto buffer =
      (shared_buffer *)((uint8_t *)ptr - offsetof(shared_buffer, buffer_));
...
static void on_work_item_done(threadpool_scheduler *scheduler,
                              work_item_shared_data *shared_data,
                              uint64_t shared_buffer_count) {
  auto &the_signal =
      scheduler->cur_section_.load(std::memory_order_relaxed)->pending_sink_;
...
static bool submit_single_work_item(threadpool_scheduler *scheduler,
  ...
  if (likely((theq->*insert_method)(item))) {
    return false;
  }
  // insert in an non-empty queue
  if (find_queue_and_insert_work(scheduler, item, qid, theq)) {
    return true;
  }
...
static void do_work(threadpool_scheduler *scheduler, threadpool_section *sect,
                    void *stream, void *module_data, threadlocals_t *tls,
                    work_item *item, queue *local_q) {
#if DYN_THREAD_POOL_PROFILE
  auto timer = utils::create_scoped_timer(true, [](utils::time_duration v) {
    postprocess_us +=
        std::chrono::duration_cast<std::chrono::nanoseconds>(v).count();
  });
#endif
  ...
  // call the workload function
  {
    ...
    for (uint64_t i = 0; i < item->size_; i++) {
      ...
      cur_layer.pfunc_(stream, module_data, iter, shared_data->buffers_.ptr_,
                       sect->args_);
    }
...
static void try_execute_broadcast_work(threadpool_scheduler *sched,
                                       threadpool_section *cur_sect,
                                       uint64_t cur_tid, queue *q) {
  ...
  auto exec = [](threadpool_scheduler *sched, threadpool_section *cur_sect,
                 queue *q, broadcast_work_item *thework, uint64_t work_ver,
                 uint64_t tid) {
    auto cur_bcast_ver =
        q[tid].broadcast_ob_ver_.load(std::memory_order_acquire);
    // wait until cur_bcast_ver reaches the expected value
    for (;;) {
      if (likely(cur_bcast_ver >= work_ver - 1)) {
        break;
      }
      cur_bcast_ver = q[tid].broadcast_ob_ver_.load(std::memory_order_acquire);
    }
    if (cur_bcast_ver >= work_ver) {
      return;
    }
    ...
    if (tid < thework->loop_len_) {
      ...
      thework->layer_.pfunc_(sched->stream_, cur_sect->module_, &tid, nullptr,
                             cur_sect->args_);
    }
  ...
  if (thework) {
    ...
    if (cur_tid == 0) {
      ...
      while (thework->trigger_.pending_.load(std::memory_order_acquire)) {
        for (; min_unattended < sched->num_queues_; min_unattended++) {
          if (q[min_unattended].thr_state_ == queue::thread_state::UNATTENDED) {
            exec(sched, cur_sect, q, thework, work_ver, min_unattended);
          }
        }
        _mm_pause();
      }
    ...
    while (sched->broadcast_work_.load(std::memory_order_acquire) == thework) {
    }
  }
}
    ...
    for (size_t i = 0; i < vec.size(); i++) {
      ...
      for (size_t j = 1; j < v.size(); j++) {
        ...
        printf("tid=%zu,itr=%zu,queued=%u,steal=%u,overflow=%u,"
               ...
               trace.total_us, trace.payload_us / 1000, trace.postprocess_us,
               trace.main_us);
...
void threadpool_scheduler::select_and_run_jobs(uint64_t tid) {
  ...
  auto timer = utils::create_scoped_timer(true, [&](utils::time_duration v) {
    ...
    trace_holder.trace(tid, count, steal_count, overflow, total_us, payload_us,
                       postprocess_us);
  });
#endif
  for (;;) {
    ...
    if (likely(item)) {
      count++;
      tls->work_tid = qid;
      do_work(this, cur_sect, stream_, cur_sect->module_, tls, item, local_q);
      continue;
    }
...
threadpool_scheduler *threadpool_adapter_t::all_thread_prepare(
  ...
  for (uint64_t i = 0; i < main_sched->num_queues_; i++) {
    ...
    main_sched->queues_[i].broadcast_ob_ver_.store(0,
...
void threadpool_adapter_t::main_thread(threadpool_scheduler *sched,
                                       main_func_t f, runtime::stream_t *stream,
...
#if DYN_THREAD_POOL_PROFILE
extern "C" SC_API void sc_dyn_threadpool_print_trace() { trace_holder.clear(); }
#endif
...
extern "C" SC_API void
sc_dyn_threadpool_sched_init(runtime::stream_t *stream, void *module,
                             generic_val *args, uint64_t num_roots,
...
extern "C" SC_API work_item_shared_data *
sc_dyn_threadpool_loop_end(work_item_shared_data *current, uint64_t up_levels) {
  if (!current) {
    current = threadlocals.current_work_data;
  }
  for (uint64_t i = 0; i < up_levels; i++) {
    current = current->parent_;
  }
  auto updated_count =
      current->trigger_.pending_.fetch_sub(1, std::memory_order_acq_rel);
  assert(updated_count >= 1);
  if (unlikely(updated_count == 1)) {
    return current;
  }
  return nullptr;
}
...
static bool submit_bcast_work(threadpool_scheduler *sched,
                              memory_pool::filo_memory_pool_t *pool,
                              closure_t pfunc, uint64_t loop_len,
                              uint64_t flags) {
  ...
  item->trigger_.pending_.store(sched->num_queues_, std::memory_order_relaxed);
...
extern "C" SC_API void sc_dyn_threadpool_create_work_items(
    closure_t pfunc, uint64_t *iter, uint64_t num_iter, uint64_t loop_len,
  ...
  auto shared = threadpool_arena::alloc_vsize<work_item_shared_data, uint64_t>(
  ...
  if (unlikely(!buffers && num_buffers)) {
    buffers = parent->buffers_.ptr_;
  }
  ...
  sched->cur_section_.load(std::memory_order_relaxed)
      ->pending_sink_.fetch_add(std::min(loop_len, num_blocks),
                                std::memory_order_acq_rel);
