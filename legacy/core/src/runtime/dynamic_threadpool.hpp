...
template <typename T> struct arena_vector {
  T *ptr_;
  uint64_t size_;
  ...
  void init(memory_pool::filo_memory_pool_t *pool, stream_t *s, uint64_t size) {
    if (size > 0) {
      ptr_ = reinterpret_cast<T *>(
          threadpool_arena::alloc(pool, s, sizeof(T) * size, alignof(T)));
    }
    size_ = size;
  }
};
...
struct threadpool_scheduler {
  ...
  threadpool_scheduler(stream_t *stream, uint64_t queue_size,
...
struct threadpool_adapter_t {
  ...
  static threadpool_scheduler *all_thread_prepare(threadpool_scheduler *ths,
