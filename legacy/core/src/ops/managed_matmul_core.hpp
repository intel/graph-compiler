...
public:
  ...
  sc_op_ptr do_compensations(sc_graph_t &mgr, const context_ptr &ctx) override;
  ...
  get_s8s8_and_weight_compensation(sc_graph_t &mgr, bool s8s8_compensation);
  ...
  std::vector<int>
  get_impl_dispatch_candidates(const context_ptr &ctx) override;
  dispatch_set_ptr
  get_internal_dispatch_key_set(const context_ptr &ctx) override;
  ...
  std::vector<int>
  query_prefetch(const context_ptr &ctx, bool is_global,
                 const std::vector<tensor_slice> &ins) override;

  void generate_prefetcher_body_for_tensor(
      const context_ptr &ctx, const std::vector<expr> &func_args,
      const std::vector<expr> &ins, const std::vector<int> &indices) override;
  void infer_binding_axis(binding_axis_map &bdax_map) override;
  void pre_infer_binding_axis(binding_axis_map &bdax_map) override;

  void set_config_by_key(const op_dispatch_key_t &key,
                         const context_ptr &ctx) override;
  void set_internal_config_by_key(const impl_op_dispatch_key_t &key,
                                  const context_ptr &ctx) override;
