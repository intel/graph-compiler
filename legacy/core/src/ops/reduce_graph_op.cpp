...
graph_reduce_base_t::graph_reduce_base_t(
  ...
  if (outs.empty()) {
    ...
    if (keep_dims_) {
      ...
      for (size_t i = 0; i < ins[0]->details_.get_plain_dims().size(); i++) {
    ...
    info_.outputs_.emplace_back(
        std::make_shared<graph_tensor>(this, ins[0]->details_.get_format(),
                                       out_dims, ins[0]->details_.dtype_));
  } else {
    info_.outputs_ = outs;
...
void graph_reduce_base_t::query_format(
    context_ptr ctx,
...
void reduce_mean_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  auto reduce_sum_div = graph->make(
      "div", {reduce_sum->get_outputs()[0], reduce_num->get_outputs()[0]}, {},
      {});
  // output
...
void reduce_l2_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  auto l2_res = graph->make("squared_root", reduce_val->get_outputs(), {}, {});
  // cast output
