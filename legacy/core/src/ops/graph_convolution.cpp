...
static void permute_shape_NXC2NCX(sc_dims &shape) {
  ...
  COMPILE_ASSERT(ndims >= 2,
                 "permute_shape_NXC2NCX requires shape.size() >= 2.")
...
static void permute_shape_NCX2NXC(sc_dims &shape) {
  size_t ndims = shape.size();
  COMPILE_ASSERT(ndims >= 2,
                 "permute_shape_NCX2NXC requires shape.size() >= 2.")
...
static void permute_shape_XIO2OIX(sc_dims &shape) {
  size_t ndims = shape.size();
  COMPILE_ASSERT(ndims >= 2,
                 "permute_shape_XIO2OIX requires shape.size() >= 2.")
...
check_and_infer_out_dtype(const sc_data_type_t &src_dtype,
                          const sc_data_type_t &wei_dtype) {
  if (utils::is_one_of(src_dtype, datatypes::u8, datatypes::s8)) {
    ...
    COMPILE_ASSERT(
        wei_dtype == datatypes::f16,
        "wei_dtype expected to be f16 when src_dtype is f16, but got "
            << wei_dtype << ".");
    return datatypes::f16;
  } else {
    COMPILE_ASSERT(src_dtype == datatypes::f32 && wei_dtype == datatypes::f32,
...
sc_dims conv_fwd_op_t::infer_out_dims(
    sc_graph_t &owner_graph, const sc_dims &input_dims,
    const sc_dims &filter_dims, const sc_dims &pads_begin,
    const sc_dims &pads_end, const sc_dims &strides, const sc_dims &dilations,
  ...
  sc_dims output_dims = conv_fwd_core_op_t::infer_out_dims(
      owner_graph, input_dims_copy, filter_dims_copy, pads_begin, pads_end,
      strides, dilations);
  if (data_format == "NXC") {
    permute_shape_NCX2NXC(output_dims);
  }
  return output_dims;
}
...
conv_fwd_op_t::conv_fwd_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  COMPILE_ASSERT(ndims >= 3,
                 "conv ndims should >= 3, but got " << ndims << ".");
  auto data_format = attrs_.get_or_else("data_format", std::string("NXC"));
  auto filter_format = attrs_.get_or_else("weights_format", std::string("XIO"));
  ...
  if (attrs_.has_key("auto_pad")) {
    ...
    if (pad_type == "VALID") {
      ...
      conv_fwd_core_op_t::infer_auto_pad(get_owner_graph(), input_dims_copy,
                                         filter_dims_copy, strides, dilations,
  ...
  auto expected_out_shape =
      infer_out_dims(info_.inputs_[0]->producer_owner_->get_owner_graph(),
                     input_dims, filter_dims, pads_begin, pads_end, strides,
                     dilations, data_format, filter_format);
  auto expected_out_dtype = check_and_infer_out_dtype(
      info_.inputs_[0]->details_.dtype_, info_.inputs_[1]->details_.dtype_);
  if (outs.empty()) {
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(
        this, sc_data_format_t(), expected_out_shape, expected_out_dtype));
  } else {
    // skip check when is dynamic
    if (is_dynamic())
      return;
    COMPILE_ASSERT(info_.outputs_.size() == 1, "convolution expects 1 output.");
...
sc_dims parse_shape_to_NGCX(const sc_dims &shape, const std::string &format,
                            int groups) {
  size_t ndims = shape.size();
  auto NCX_shape = shape;
  if (format == "NXC") {
    permute_shape_NXC2NCX(NCX_shape);
  }
...
sc_dims parse_shape_to_GOIX(const sc_dims &shape, const std::string &format,
                            int groups) {
  size_t ndims = shape.size();
  auto OIX_shape = shape;
  if (format == "XIO") {
    permute_shape_XIO2OIX(OIX_shape);
  }
  auto ret_shape = sc_dims(ndims + 1, 0);
...
void conv_fwd_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  bool is_low_precision_fp =
      utils::is_one_of(input->details_.dtype_, datatypes::bf16, datatypes::f16);
  auto data_format = attrs_.get_or_else("data_format", std::string("NXC"));
  auto filter_format = attrs_.get_or_else("weights_format", std::string("XIO"));
  ...
  if (data_format == "NXC") {
    auto permute_input =
        graph->make("transpose", {input}, {},
                    {{"order", is_3D ? std::vector<int>{0, 4, 1, 2, 3}
                                     : std::vector<int>{0, 3, 1, 2}},
                     {"out_format", is_3D ? sc_data_format_t::NCDHW()
                                          : sc_data_format_t::NCHW()}});
    input = permute_input->get_outputs()[0];
  }
  if (filter_format == "XIO") {
    auto permute_weight =
        graph->make("transpose", {filter}, {},
                    {{"order", is_3D ? std::vector<int>{4, 3, 0, 1, 2}
                                     : std::vector<int>{3, 2, 0, 1}},
                     {"out_format", is_3D ? sc_data_format_t::KCDRS()
                                          : sc_data_format_t::KCRS()}});
    filter = permute_weight->get_outputs()[0];
  }
  if (groups > 1) {
    input =
        (graph->make("tensor_view", {input}, {},
                     {{"shape", parse_shape_to_NGCX(
                                    info_.inputs_[0]->details_.get_plain_dims(),
                                    data_format, groups)},
                      {"cache_input_format", is_3D ? sc_data_format_t::NDHWC()
                                                   : sc_data_format_t::NHWC()},
                      {"format", is_3D ? sc_data_format_t::NDHWGC()
                                       : sc_data_format_t::NHWGC()},
                      {"expand_dim", std::vector<int>{}}}))
            ->get_outputs()[0];
    filter =
        (graph->make("tensor_view", {filter}, {},
                     {{"shape", parse_shape_to_GOIX(
                                    info_.inputs_[1]->details_.get_plain_dims(),
                                    filter_format, groups)},
                      {"cache_input_format", is_3D ? sc_data_format_t::KCDRS()
                                                   : sc_data_format_t::KCRS()},
                      {"format", is_3D ? sc_data_format_t::GKCDRS()
  ...
  if (groups > 1) {
    ...
    conv =
        graph->make("tensor_view", conv->get_outputs(), {},
                    {{"shape", output_shape},
                     {"cache_input_format", is_3D ? sc_data_format_t::NGCDHW()
                                                  : sc_data_format_t::NGCHW()},
                     {"format", is_3D ? sc_data_format_t::NCDHW()
                                      : sc_data_format_t::NCHW()},
                     {"expand_dim", std::vector<int>{}}});
  }
  ...
  if (data_format == "NXC") {
    ...
    conv = graph->make("transpose", conv->get_outputs(), {},
                       {{"order", is_3D ? std::vector<int>{0, 2, 3, 4, 1}
...
void conv_fwd_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}

conv_bwd_data_op_t::conv_bwd_data_op_t(
    const std::vector<graph_tensor_ptr> &ins,
    const std::vector<graph_tensor_ptr> &outs, const any_map_t &attrs) {
  COMPILE_ASSERT(
      ins.size() == 2 || ins.size() == 3,
  ...
  if (outs.empty()) {
    ...
    COMPILE_ASSERT(info_.outputs_.size() == 1,
                   "conv_bwd_data expects 1 output.");
    COMPILE_ASSERT(info_.outputs_[0]->details_.get_plain_dims() == out_shape,
...
void conv_bwd_data_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  bool is_low_precision_fp = utils::is_one_of(inputs[0]->details_.dtype_,
  ...
  bool padding_all_1 = std::all_of(pads_begin.begin(), pads_begin.end(),
                                   [](int x) { return x == 1; });
  bool valid_padding = (attrs_.has_key("auto_pad") &&
  ...
  // insert transpose to make NXC --> NCX and XIO --> OIX
  if (data_format == "NXC") {
    auto permute_output_delta =
        graph->make("transpose", {output_delta}, {},
                    {{"order", is_3D ? std::vector<int>{0, 4, 1, 2, 3}
                                     : std::vector<int>{0, 3, 1, 2}}});
  ...
  if (!is_3D && ctx->use_amx() && is_3x3 && stride_all_1 && valid_padding) {
    ...
    auto permute_channel = graph->make(
        "transpose", {filter}, {}, {{"order", std::vector<int>{1, 0, 2, 3}}});
    filter = permute_channel->get_outputs()[0];
    attrs.set("inverse_filter", true);
    conv = graph->make("conv_fwd_core", {output_delta, filter}, {}, attrs);
  } else {
    conv = graph->make("conv_bwd_data_core", {output_delta, filter}, {}, attrs);
  }
...
void conv_bwd_data_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}
...
void conv_bwd_weight_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  conv = graph->make("conv_bwd_weight_core", {input, output_delta}, {}, attrs);
  ...
  if (filter_format == "XIO") {
    conv = graph->make("transpose", conv->get_outputs(), {},
                       {{"order", is_3D ? std::vector<int>{2, 3, 4, 1, 0}
                                        : std::vector<int>{2, 3, 1, 0}}});
  }
  graph->make_output(conv->get_outputs());
}

void conv_bwd_weight_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}
