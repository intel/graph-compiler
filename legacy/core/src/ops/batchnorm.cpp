...
static sc_dim get_channel_size(const any_map_t &attrs,
...
static void validate_channel_related_input(const graph_tensor_ptr &gt,
                                           sc_dim channel_size,
                                           bool allow_bf16) {
...
batchnorm_inference_op::batchnorm_inference_op(
  ...
  bool allow_bf16 =
      (info_.inputs_[0]->details_.dtype_.type_code_ == sc_data_etype::BF16);
  for (size_t i = 1; i < info_.inputs_.size(); ++i) {
    validate_channel_related_input(info_.inputs_[i], channel_size, allow_bf16);
  }
  if (outs.empty()) {
...
void batchnorm_inference_op::get_graph_impl(
  ...
  // variance+eps
  auto var_eps =
      graph->make("add", {inputs[4], const_op->get_outputs()[0]}, {}, {});
  // sqrt(variance+eps)
  auto sqrt_op =
...
void batchnorm_inference_op::query_format(
    context_ptr ctx,
...
batchnorm_forward_training_op::batchnorm_forward_training_op(
  ...
  for (size_t i = 1; i < info_.inputs_.size(); ++i) {
    validate_channel_related_input(info_.inputs_[i], channel_size, allow_bf16);
    COMPILE_ASSERT(
        info_.inputs_[1]->details_.dtype_ == info_.inputs_[i]->details_.dtype_,
...
void batchnorm_forward_training_op::get_graph_impl(
  ...
  // input
  graph->make_input(inputs);
  int num_dims = static_cast<int>(inputs[0]->details_.get_plain_dims().size());
  ...
  auto chan_size_op = graph->make<constant_op_t>(
      ...
      datatypes::f32, sc_dims{1});
  auto reduce0 =
      graph->make("reduce", {src}, {},
                  {{"rd_axis", rd_axis}, {"rd_op", 0}, {"keep_dims", false}});
  std::shared_ptr<sc_op> new_var;
  auto new_mean = graph->make(
      "div", {reduce0->get_outputs()[0], chan_size_op->get_outputs()[0]}, {},
      {{op_attr_key::break_post_fuse, true}, {op_attr_key::must_div, true}});
  if (use_bnorm_opt) {
    auto src_squared = graph->make("mul", {src, src}, {}, {});
    auto reduce0_squared = graph->make(
        "mul", {reduce0->get_outputs()[0], reduce0->get_outputs()[0]}, {}, {});
    auto reduce0_squared_mul = graph->make(
        "div",
        {reduce0_squared->get_outputs()[0], chan_size_op->get_outputs()[0]}, {},
        {{op_attr_key::must_div, true}});
    auto reduce1 =
        graph->make("reduce", {src_squared->get_outputs()[0]}, {},
                    {{"rd_axis", rd_axis}, {"rd_op", 0}, {"keep_dims", false}});
    auto sub0 = graph->make(
        ...
        {});
    new_var = graph->make(
        "div", {sub0->get_outputs()[0], chan_size_op->get_outputs()[0]}, {},
        {{op_attr_key::break_post_fuse, true}, {op_attr_key::must_div, true}});
  } else {
    auto diff = graph->make("sub", {src_pass3, new_mean->get_outputs()[0]}, {},
                            {{"bc_axis", bc_axis}});
    auto diff_squared = graph->make(
        "mul", {diff->get_outputs()[0], diff->get_outputs()[0]}, {}, {});
    auto reduce1 =
        graph->make("reduce", {diff_squared->get_outputs()[0]}, {},
                    {{"rd_axis", rd_axis}, {"rd_op", 0}, {"keep_dims", false}});
    new_var = graph->make(
  ...
  auto add0 = graph->make(
      "add", {new_var->get_outputs()[0], epsilon->get_outputs()[0]}, {}, {});
  auto sqrt = graph->make("squared_root", {add0->get_outputs()[0]}, {}, {});
  auto div1 =
      graph->make("div", {sub1->get_outputs()[0], sqrt->get_outputs()[0]}, {},
                  {{"bc_axis", bc_axis}});
  // gamma*x_normalized + beta
  auto mul2 = graph->make("mul", {div1->get_outputs()[0], scale}, {},
                          {{"bc_axis", bc_axis}});
  ...
  if (attrs_.get_or_else("momentum", float(1.)) == 1.f) {
    ...
    auto momentum = graph->make<constant_op_t>(
        std::make_shared<static_data_t>(
            std::vector<float>{attrs_.get_or_else("momentum", float(1.))}),
        datatypes::f32, sc_dims{1});
    auto one_sub_momentum = graph->make<constant_op_t>(
        std::make_shared<static_data_t>(
            std::vector<float>{1 - attrs_.get_or_else("momentum", float(1.))}),
        datatypes::f32, sc_dims{1});

    // running_mean = momentum * mean + (1- momentum) * mean_x
    auto mul3 = graph->make("mul", {mean, momentum->get_outputs()[0]}, {}, {});
    auto mul4 = graph->make(
        "mul", {new_mean->get_outputs()[0], one_sub_momentum->get_outputs()[0]},
        {}, {});
    add2 = graph->make("add", {mul3->get_outputs()[0], mul4->get_outputs()[0]},
                       {}, {});

    // running_var = momentum * variance + (1- momentum) * var_x
    auto mul5 =
        graph->make("mul", {variance, momentum->get_outputs()[0]}, {}, {});
    auto mul6 = graph->make(
        "mul", {new_var->get_outputs()[0], one_sub_momentum->get_outputs()[0]},
        {}, {});
    add3 = graph->make("add", {mul5->get_outputs()[0], mul6->get_outputs()[0]},
                       {}, {});
  }
...
void batchnorm_forward_training_op::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}
...
void batchnorm_training_backprop_op_t::get_graph_impl(
  ...
  graph_tensor_ptr src = inputs[0], output_delta = inputs[1], mean = inputs[2],
                   variance = inputs[3], gamma = inputs[4];
  ...
  // var + eps
  auto var_eps =
      graph->make("add", {variance, const_op->get_outputs()[0]}, {}, {});
  // rsqrt(var + eps)
  ...
  auto x_hat = graph->make(
      "div", {x_mu->get_outputs()[0], sqrt_var_eps->get_outputs()[0]}, {},
      {{"bc_axis", bc_axis}});
  // ------ calculate x_hat end ------
  ...
  // x_hat = (x - mu) *  rsqrt(var + eps)
  auto x_hat_pass2 = graph->make(
      "div", {x_mu_pass2->get_outputs()[0], sqrt_var_eps->get_outputs()[0]}, {},
      {{"bc_axis", bc_axis}});
  // ------ duplicate x_mu && x_hat end ------

  // gamma_delta = reducesum(dy * x_hat)
  auto dy_x_hat =
      graph->make("mul", {output_delta, x_hat->get_outputs()[0]}, {}, {});
  auto gamma_delta =
      graph->make("reduce", dy_x_hat->get_outputs(), {},
                  {{"rd_axis", rd_axis}, {"rd_op", 0}, {"keep_dims", false}});
  // gamma * x_hat + beta = y --> beta_delta = reducesum(dy)
  auto beta_delta =
      graph->make("reduce", {output_delta}, {},
                  {{"rd_axis", rd_axis}, {"rd_op", 0}, {"keep_dims", false}});
  // ------ calculate x_delta start ------
  // calculate: (x - mu) * rsqrt(var + eps) * gamma_delta <==> x_hat *
  // gamma_delta
  auto x_hat_gamma_delta = graph->make(
      "mul", {x_hat_pass2->get_outputs()[0], gamma_delta->get_outputs()[0]}, {},
      {{"bc_axis", bc_axis}});
  ...
  auto add = graph->make(
      ...
      {{"bc_axis", bc_axis}});
  //  * (1 / channel_size)
  auto rescale = graph->make(
      "div", {add->get_outputs()[0], chan_size_op->get_outputs()[0]}, {}, {});
  // get subtract results
  auto sub = graph->make("sub", {output_delta_pass2, rescale->get_outputs()[0]},
                         {}, {});
  // final mul: x_delta = gamma * rsqrt(var + eps) * sub_result
  auto div =
      graph->make("div", {gamma, sqrt_var_eps->get_outputs()[0]}, {}, {});
  auto x_delta =
  ...
  graph->make_output({x_delta->get_outputs()[0], gamma_delta->get_outputs()[0],
                      beta_delta->get_outputs()[0]});
}

void batchnorm_training_backprop_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}
