...
void softmax_base_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}

graph_tensor_ptr
softmax_base_t::get_stable_exp_inp(const graph_tensor_ptr &input,
...
softmax_op_t::get_exp_reduce(std::shared_ptr<sc_graph_t> &graph,
  ...
  // exp(x)
  auto fexp =
      graph->make("exp", {fexpinp}, {}, {{"overflow_check", !numeric_stable}});

  // sum(exp(x))
  auto freduce =
      graph->make("reduce", {fexp->get_outputs()[0]}, {},
...
softmax_op_t::get_softmax_result(std::shared_ptr<sc_graph_t> &graph,
                                 const graph_tensor_ptr &input,
                                 const std::vector<int> &axis) {
  ...
  auto res = graph->make(
      "div", {f_exp->get_outputs()[0], f_reduce->get_outputs()[0]}, {}, {});
  return res;
}
...
void softmax_bwd_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  auto f_rd =
      graph->make("reduce", f_sbr->get_outputs(), {},
                  {{"need_mean", false}, {"rd_axis", axis}, {"rd_op", 0}});
