...
static void get_output_shape(sc_dims &outshape, const sc_dims &input_dims,
  ...
  for (int i = 0; i < dim; i++) {
    int shape_v = shape[i];
    if (shape_v == -1) {
      COMPILE_ASSERT(auto_cal_dim == -1, "reshape only support one -1 shape");
      auto_cal_dim = i;
    } else {
      if (special_zero && shape_v == 0) {
        COMPILE_ASSERT(static_cast<size_t>(i) < input_dims.size(),
                       "The special zero at "
                           << i << " dimension is out of range in input shape");
...
static static_data_t *validate_and_get_static_shape(sc_op *ths) {
  COMPILE_ASSERT(ths->get_inputs().size() == 2,
                 "dynamic reshape op takes 2 inputs");
  ...
  COMPILE_ASSERT(
      in1_detail.get_blocking_dims().size() == 1 &&
          utils::is_one_of(in1_detail.dtype_, datatypes::index, datatypes::s32),
      "Expecting 1D and int32/int64 tensor for input 2 of " << ths->op_name_);
  if (!in1->isa<input_op>() && !in1->isa<constant_op_t>()) {
    return nullptr;
  }
  if (ths->is_dynamic()) {
    return nullptr;
  }
  auto ret = in1->attrs_.get_or_else<std::shared_ptr<static_data_t>>("values",
                                                                     nullptr);
  COMPILE_ASSERT(
      ret, "Since dynamic shape is not supported yet, we are expecting the "
...
dynamic_reshape_op::dynamic_reshape_op(
  ...
  if (!ins[0]->is_dynamic()) {
    ...
    COMPILE_ASSERT(dim * sizeof(int32_t) == shape_data->size_,
                   "Bad shape data");
  ...
  if (info_.outputs_.empty()) {
    ...
    COMPILE_ASSERT(info_.outputs_.size() == 1,
                   "Expecting 1 output for reshape");
...
void dynamic_reshape_op::query_format(
    context_ptr ctx,
...
sc_op_ptr dynamic_reshape_op::constant_optimize(sc_graph_t &graph) {
  ...
  auto new_input =
      graph.make("tensor_view", {get_inputs()[0]}, {},
...
static_reshape_op::static_reshape_op(const std::vector<graph_tensor_ptr> &ins,
  ...
  COMPILE_ASSERT(attrs.has_key("shape"),
                 "Static reshape requires shape attributes");
...
void static_reshape_op::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  throw std::runtime_error("Not implemented");
...
sc_op_ptr static_reshape_op::constant_optimize(sc_graph_t &graph) {
  ...
  if (!get_inputs()[0]->details_.get_format().is_any())
    valid_tensor_view =
        trial_tensor_view->dyn_cast<tensor_view_op_t>()->try_penetrate(
            trial_format);
  if (!valid_tensor_view) {
    auto reorder = graph.make(
        "reorder", {get_inputs()[0]}, {},
        {{"internal", true},
         {"out_format", get_inputs()[0]->details_.get_format().to_plain()}});
