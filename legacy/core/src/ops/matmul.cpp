...
matmul_op::matmul_op(const std::vector<graph_tensor_ptr> &ins,
  ...
  COMPILE_ASSERT(
      (ins[0]->details_.get_plain_dims().size() >= 2 &&
  ...
  bool is_int8 =
      utils::is_one_of(ins[0]->details_.dtype_, datatypes::u8, datatypes::s8);
  bool is_bf16 = ins[0]->details_.dtype_ == datatypes::bf16;
  bool is_low_precision_fp = utils::is_one_of(ins[0]->details_.dtype_,
                                              datatypes::bf16, datatypes::f16);
  sc_dims output_shape;
  if (!is_dynamic()) {
    output_shape = {
        merge_vec(matmul_core_op_t::get_batch_dims_with_bc_impl(A_dims, B_dims),
  ...
  if (outs.empty()) {
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(
        this, sc_data_format_t(), output_shape,
...
static void transed_matmul(const std::shared_ptr<sc_graph_t> &graph,
                           ...
                           const graph_tensor_ptr &ins1,
                           graph_tensor_ptr &trans0, graph_tensor_ptr &trans1) {
  if (attrs.get_or_else("transpose_a", false)) {
    ...
    auto out = graph_tensor::make(
        transed_plain_dims, ins0->details_.get_format(), ins0->details_.dtype_);
  ...
  if (attrs.get_or_else("transpose_b", false)) {
    ...
    std::swap(order[transed_plain_dims.size() - 1],
              order[transed_plain_dims.size() - 2]);
    auto out = graph_tensor::make(
        transed_plain_dims, ins1->details_.get_format(), ins1->details_.dtype_);
...
void matmul_op::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  if (!is_dynamic()) {
    ...
    if (trans0_plain_dims.size() > 2 && trans1_plain_dims.size() == 2) {
      sc_dims reshape_dest = {
          math_utils::get_dims_product(
              sc_dims{trans0_plain_dims.begin(), trans0_plain_dims.end() - 1}),
          trans0_plain_dims.back()};
      auto reshape_node = graph->make(
          "tensor_view", {trans0},
          ...
          {{"shape", reshape_dest},
           ...
           {"cache_input_format",
            sc_data_format_t::get_plain_by_dims(trans0_plain_dims.size())}});
    ...
    if (trans0_plain_dims.size() == 2 && trans1_plain_dims.size() > 2) {
      sc_dims reshape_dest = {
          ...
          math_utils::get_dims_product(
              sc_dims{trans1_plain_dims.begin(), trans1_plain_dims.end() - 2}) *
              trans1_plain_dims.back()};
      auto reshape_fmt = sc_data_format_t::NKkn(
          trans1_plain_dims.at(trans1_plain_dims.size() - 2),
          trans1_plain_dims.back());
      sc_op_ptr reshape_node =
          graph->make("tensor_view", {trans1},
  ...
  int M = trans0->details_
              .get_plain_dims()[trans0->details_.get_plain_dims().size() - 2];
  ...
  if (is_dynamic() || trans0->details_.get_plain_dims().size() > 2 ||
    ...
    if (use_mmm) {
      matmul =
          graph->make("managed_matmul_core", {trans0, trans1}, {},
                      {{"transposed_a", transposed_a},
                       {"transposed_b", transposed_b},
      ...
      matmul =
          graph->make("matmul_core", {trans0, trans1}, {},
                      {{attr_keys::output_channel_axis, output_channel_axis},
                       {"transposed_a", transposed_a},
                       {"transposed_b", transposed_b}});
    }
  ...
  if (!is_dynamic()) {
    // Nd*2d cases
    if (trans0_plain_dims.size() > 2 && trans1_plain_dims.size() == 2) {
      sc_dims reshape_dest = {trans0_plain_dims.begin(),
                              trans0_plain_dims.end() - 1};
      reshape_dest.emplace_back(trans1_plain_dims.back());
      matmul = graph->make(
          "tensor_view", {matmul->get_outputs()[0]},
          {graph_tensor::make(reshape_dest, sc_data_format_t(),
    ...
    // 2d*Nd cases
    if (trans0_plain_dims.size() == 2 && trans1_plain_dims.size() > 2) {
      sc_dims reshape_dest = {trans1_plain_dims.begin(),
  ...
  if (info_.inputs_.size() == 3) {
    ...
    // is equal with N.
    if (is_bf16 || is_f16) {
      COMPILE_ASSERT(inputs[2]->details_.dtype_ == inputs[0]->details_.dtype_,
                     "All inputs should have same data type.")
    }
    ...
    for (int axis_id = bias_shape_size - 1; axis_id >= 0; --axis_id) {
      ...
      if (bias_dim != 1) {
        ...
        COMPILE_ASSERT(bias_dim == matmul_output_shape[matmul_output_axis],
                       "Cannot broadcast bias");
        bc_axes.push_back(matmul_output_axis);
      }
    }
    auto bias =
        graph->make("add", {matmul->get_outputs()[0], ins->get_outputs()[2]},
                    {}, {{"bc_axis", bc_axes}});
