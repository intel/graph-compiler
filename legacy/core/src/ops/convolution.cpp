...
sc_data_type_t
conv_fwd_core_op_t::infer_out_dtype(const sc_data_type_t &input_dtype,
...
infer_status_code conv_fwd_core_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                         fslice_map &fsmap) {
  bool is_weight_constant =
      ...
      info_.inputs_[1]->attrs_.get_or_else("constant", const_kind::not_const);
  if (attrs_.has_key("inverse_filter") ||
      !attrs_.get_or_else("image_affinity", true) || !is_weight_constant ||
      is_dynamic()) {
  ...
  COMPILE_ASSERT(groups == 1 ||
                     (inp_plain_dim.size() >= 5 && wei_plain_dim.size() >= 5 &&
                      groups == inp_plain_dim[1] && groups == wei_plain_dim[0]),
                 "Group conv format should be NGCX and GOIX");
  bool is_dw_brdgmm =
      (groups > 1) && (1 == inp_plain_dim[2]) && (1 == wei_plain_dim[1]);
  ...
  auto input_slice_size =
      known_ranges_map[0].empty()
          ? -1
          : get_slice_size(known_ranges_map[0][0],
                           utils::get_sizeof_type(data_dtype));
  auto weight_size = math_utils::get_dims_product(wei_dims) *
                     utils::get_sizeof_type(weight_dtype);
  auto can_fit_in_L2_cache =
      input_slice_size > 0 && (input_slice_size + weight_size < L2_cache_size);
  if (config_data_ && inp_dims[0] % num_threads == 0 &&
      wei_plain_dim.size() == 4 && wei_plain_dim[2] == 1 &&
      wei_plain_dim[3] == 1 && can_fit_in_L2_cache) { // 1x1 NH-wise fusion
    ...
    if (!known_ranges_map[0].empty()) {
      ...
      if (!slice_divisible_by_factor(inp_tmp, {in_p2b_map[2].back()}, tile_p) ||
          !slice_larger_than_bound_on_axis(
              inp_tmp, in_p2b_map[2], tile_p,
              data_dtype == datatypes::f32 ? 1 : 2)) { // dividable on H
        return infer_status_code::RETRY;
      }
      ...
      for (auto plain_ax : plain_axis_required) {
        for (unsigned i = 0; i < in_p2b_map[plain_ax].size(); i++) {
          ...
          inp_slice[ax] = std::make_pair(expr(0), dim2unsigned(inp_dims[ax]));
        }
        for (unsigned i = 0; i < out_p2b_map[plain_ax].size(); i++) {
          auto ax = out_p2b_map[plain_ax][i];
          out_slice[ax] = std::make_pair(expr(0), dim2unsigned(out_dims[ax]));
        }
      }
    }
...
void conv_fwd_core_op_t::infer_out_tensor_details() {
  if (!info_.outputs_[0]->details_.get_plain_dims().empty())
    return;
  ...
  auto &pads_end = attrs_.has_key("pads_end") ? attrs_.get<sc_dims>("pads_end")
                                              : attrs_.get<sc_dims>("paddings");
  ...
  auto expected_out_shape = infer_out_dims(
      get_owner_graph(), indims, weightdims, pads_begin, pads_end,
      attrs_.get<sc_dims>("strides"), get_dilations(attrs_), attrs_);
  if (groups > 1) {
...
sc_dims conv_fwd_core_op_t::infer_out_dims(
    sc_graph_t &owner_graph, const sc_dims &input_dims,
    const sc_dims &weight_dims, const sc_dims &pads_begin,
    const sc_dims &pads_end, const sc_dims &stride, const sc_dims &dilation,
    const any_map_t &attrs) {
  int ndims = input_dims.size();
  const bool is_1d = (ndims == 3);
  const bool is_3d = (ndims == 5);
  sc_dims wei_dims = weight_dims;
  COMPILE_ASSERT(utils::is_one_of(static_cast<int>(input_dims.size()), 3, 4, 5),
  ...
  COMPILE_ASSERT(
      ...
      : is_1d ? utils::is_one_of(static_cast<int>(pads_begin.size()), 1, 1)
      ...
      "3D for 3D conv, but got "
          << pads_begin.size() << "D for in " << (is_3d ? 3 : 2) << "D conv.");
  COMPILE_ASSERT(
      is_3d   ? utils::is_one_of(static_cast<int>(pads_end.size()), 1, 3)
      : is_1d ? utils::is_one_of(static_cast<int>(pads_end.size()), 1, 1)
      ...
      "for 3D conv, but got "
          ...
          << "D conv.");
  COMPILE_ASSERT(
      is_3d   ? utils::is_one_of(static_cast<int>(stride.size()), 1, 3)
      : is_1d ? utils::is_one_of(static_cast<int>(stride.size()), 1, 2)
      ...
      "for 3D conv, but got "
          ...
          << "D conv.");
  COMPILE_ASSERT(
      is_3d   ? utils::is_one_of(static_cast<int>(dilation.size()), 1, 3)
      : is_1d ? utils::is_one_of(static_cast<int>(dilation.size()), 1, 2)
  ...
  if (pads_begin.size() > 1) {
    pads_begin_dims = pads_begin;
  }
  sc_dims pads_end_dims(ndims - 2, pads_end[0]);
  if (pads_end.size() > 1) {
    pads_end_dims = pads_end;
  }
  ...
  for (int i = 2; i < ndims; ++i) {
    if (is_dynamic_dim(input_dims[i]) || is_dynamic_dim(wei_dims[i]) ||
      ...
      out_dims[i] = calc_out_shapes(
          input_dims[i], wei_dims[i], pads_begin_dims[i - 2],
          pads_end_dims[i - 2], stride_dims[i - 2], dilation_dims[i - 2]);
    }
  }
...
void conv_fwd_core_op_t::infer_auto_pad(
    sc_graph_t &owner_graph, const sc_dims &input_dims,
    const sc_dims &weight_dims, const sc_dims &stride, const sc_dims &dilation,
    any_map_t &attrs, bool is_same_upper, bool is_NGCX_layout) {
  int ndims = input_dims.size();
  ...
  for (int i = 2 + is_NGCX_layout; i < ndims; ++i) {
    if (is_dynamic_dim(input_dims[i]) || is_dynamic_dim(weight_dims[i]) ||
      ...
      sc_dim total_pad =
          calc_total_padding(input_dims[i], weight_dims[i], output_dim,
                             stride_dims[i - 2 - is_NGCX_layout],
                             dilation_dims[i - 2 - is_NGCX_layout]);
      if (total_pad % 2 == 0) {
        pads_begin[i - 2 - is_NGCX_layout] = pads_end[i - 2 - is_NGCX_layout] =
            total_pad / 2;
      } else {
        pads_begin[i - 2 - is_NGCX_layout] =
...
void conv_fwd_core_op_t::check_dtypes(const sc_data_type_t &data_dtype,
                                      const sc_data_type_t &weight_dtype,
                                      const sc_data_type_t &out_dtype) {
  if (utils::is_one_of(data_dtype, datatypes::u8, datatypes::s8)) {
    ...
    COMPILE_ASSERT(
        (weight_dtype == datatypes::bf16),
        "weight_dtype expected to be bf16 when data_dtype is bf16, but "
        "got "
            << weight_dtype << ".");
  } else {
    COMPILE_ASSERT(
        ((data_dtype == datatypes::f32) && (weight_dtype == datatypes::f32) &&
         (out_dtype == datatypes::undef || out_dtype == datatypes::f32)),
...
conv_fwd_core_op_t::conv_fwd_core_op_t(
  ...
  if (attrs_.has_key("auto_pad")) {
    ...
    if (pad_type == "VALID") {
      attrs_.set<sc_dims>("pads_begin", sc_dims(ndims_ - 2 - (groups > 1), 0));
      attrs_.set<sc_dims>("pads_end", sc_dims(ndims_ - 2 - (groups > 1), 0));
    } else if (pad_type == "SAME_UPPER" || pad_type == "SAME_LOWER") {
      // output spatial dims are equal to input spatial dims
      infer_auto_pad(get_owner_graph(), indims, weightdims, strides, dilations,
                     attrs_, pad_type == "SAME_UPPER", groups > 1);
    }
  ...
  if (info_.outputs_.empty()) {
    ...
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(
        this, sc_data_format_t(), sc_dims{},
        infer_out_dtype(data_dtype, weight_dtype)));
  } else {
    COMPILE_ASSERT(info_.outputs_.size() == 1, "conv expects 1 output");
    check_dtypes(data_dtype, weight_dtype, info_.outputs_[0]->details_.dtype_);
  }
}
...
bool conv_fwd_core_op_t::use_nested_conv_fwd_generator() {
  ...
  auto is_1x1 = std::all_of(weight_shape.begin() + 2, weight_shape.end(),
                            [](int x) { return x == 1; });
  auto is_int8 = utils::is_one_of(info_.inputs_[0]->details_.dtype_,
                                  datatypes::u8, datatypes::s8);
  const int num_threads = runtime_config_t::get().get_num_threads();
  auto dtype_size = utils::get_sizeof_type(info_.inputs_[1]->details_.dtype_);
  bool os_blocking_with_oc_threads =
      !has_pad && output_shape.back() * dtype_size < 32 &&
      utils::is_one_of(info_.inputs_[0]->details_.dtype_, datatypes::u8,
  ...
  // well tuned
  auto use_nested_conv =
      (ndims_ == 4 && !has_pad && !has_dilation && is_int8 &&
       !(data_shape.back() > 56 && !is_1x1) &&
       !(output_shape.back() <= 7 && !is_1x1) &&
       !(data_shape[1] % 32 != 0 && is_1x1) &&
       num_threads / data_shape[0] <= 4 && !os_blocking_with_oc_threads &&
...
bool conv_fwd_core_op_t::use_conv1d(const context_ptr &ctx) {
  ...
  auto dyn_data_zero_points =
      attrs_.get_or_else(attr_keys::dyn_data_zero_points, graph_tensor_ptr());
  auto dyn_weight_zero_points =
      attrs_.get_or_else(attr_keys::dyn_weight_zero_points, graph_tensor_ptr());
  if (is_dyn_quan && !dyn_data_zero_points && !dyn_weight_zero_points) {
    return false;
  }
  auto data_zero_points =
      attrs_.get_or_else(attr_keys::data_zero_points, std::vector<int>{0});
  auto weight_zero_points =
      attrs_.get_or_else(attr_keys::weight_zero_points, std::vector<int>{0});
  ...
  if ((!weight_zero_points.empty() &&
       !std::all_of(weight_zero_points.begin(), weight_zero_points.end(),
                    [](int i) { return i == 0; })) ||
      s8s8_compensation) {
    return false;
  }
  ...
  bool is_weight_constant =
      ...
      get_inputs()[1]->producer_owner_->attrs_.get_or_else(
          "constant", const_kind::not_const) ||
      get_inputs()[1]->attrs_.get_or_else("constant", const_kind::not_const);
  ...
  auto boundry = 5UL;
  bool has_stride =
      !std::all_of(stride.begin(), stride.end(), [](int x) { return x == 1; });
  auto is_int8 = utils::is_one_of(info_.inputs_[0]->details_.dtype_,
                                  datatypes::u8, datatypes::s8);
  ...
  if ((data_shape[0] == 1 && has_stride && is_int8) ||
      data_shape[1] % 64 != 0 || weight_shape[0] % 64 != 0) {
    return false;
  }
  return true;
}
...
body_generator_ptr conv_fwd_core_op_t::create_generator() {
  ...
  sc_dim groups = attrs_.get_or_else("groups", 1);
  bool is_dw_brdgmm =
      (groups > 1) && (1 == input_plain_dims[2]) && (1 == weight_plain_dims[1]);
...
void conv_fwd_core_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  std::vector<std::vector<sc_data_format_t>> in_formats, out_formats;
  sc_data_format_t data_format, weight_format, out_format;
  sc_data_format_t raw_weight_format = info_.inputs_[1]->details_.get_format();
  bool dynamic = is_dynamic();
  COMPILE_ASSERT(info_.inputs_.size() == 2, "conv expects 2 inputs, but got "
                                                << info_.inputs_.size()
                                                << " inputs.");
  ...
  auto is_1x1 =
      std::all_of(weight_plain_dims.begin() + 2 + (groups > 1),
                  weight_plain_dims.end(), [](int x) { return x == 1; });
  if (!is_1d) {
    channel_last_support =
        is_1x1 || ops::is_amx_dtype(ctx, src_dtype) ||
  ...
  auto default_block =
      get_dyn_conv_default_block(is_1x1, utils::get_sizeof_type(src_dtype),
                                 has_pad, src_dtype == datatypes::f32);
  if (dynamic) {
  ...
  if (use_rl == ops::rl_kind::FULL_LOWERING && !dynamic) {
    ...
    if (is_dw_brdgmm) {
      ...
      if (utils::is_one_of(src_dtype, datatypes::u8, datatypes::s8) &&
        ...
        weight_format =
            is_3d
                ? (groups == 1 ? sc_data_format_t::KCDRSck4c(C_block, K_block)
                               : sc_data_format_t::GKCDRSck4c(C_block, K_block))
            : is_1d
                ? sc_data_format_t::KCSck4c(C_block, K_block)
                : (groups == 1 ? sc_data_format_t::KCRSck4c(C_block, K_block)
                               : sc_data_format_t::GKCRSck4c(C_block, K_block));
      } else if (src_dtype == datatypes::bf16 && wei_dtype == datatypes::bf16) {
        weight_format =
            is_3d
                ? (groups == 1 ? sc_data_format_t::KCDRSck2c(C_block, K_block)
                               : sc_data_format_t::GKCDRSck2c(C_block, K_block))
            : is_1d
                ? sc_data_format_t::KCSck2c(C_block, K_block)
                : (groups == 1 ? sc_data_format_t::KCRSck2c(C_block, K_block)
                               : sc_data_format_t::GKCRSck2c(C_block, K_block));
      } else {
        weight_format =
            is_3d ? (groups == 1 ? sc_data_format_t::KCDRSck(C_block, K_block)
                                 : sc_data_format_t::GKCDRSck(C_block, K_block))
            : is_1d
                ? sc_data_format_t::KCSck(C_block, K_block)
                : (groups == 1 ? sc_data_format_t::KCRSck(C_block, K_block)
                               : sc_data_format_t::GKCRSck(C_block, K_block));
      }
    }
  }
  ...
  std::vector<sc_data_format_t> ret_formats = {data_format, weight_format,
                                               out_format};
  ...
  if (dynamic) {
    ...
    std::vector<std::vector<sc_dim>> var_block = {{}, {C_block, K_block}, {}};
    ...
    dispatch_key_set->get_inner_set().insert(cur_dispatch_key_set.set_.begin(),
                                             cur_dispatch_key_set.set_.end());
  }
  format_to_dense_format_stride_pair(in_formats, out_formats, supported_ins,
                                     supported_outs);
}

void conv_fwd_core_op_t::set_config_by_key(const op_dispatch_key_t &key,
...
std::vector<int>
conv_fwd_core_op_t::get_impl_dispatch_candidates(const context_ptr &ctx) {
...
void conv_fwd_core_op_t::calculate_dynamic_shape_expression() {
  ...
  auto expr_pads_begin = g.dims_to_expr(
      attrs_.has_key("pads_begin")
          ? attrs_.get<sc_dims>("pads_begin")
          : attrs_.get_or_else<sc_dims>("paddings", sc_dims(ndims_ - 2, 0)));

  auto expr_pads_end = g.dims_to_expr(
      attrs_.has_key("pads_end")
          ? attrs_.get<sc_dims>("pads_end")
          : attrs_.get_or_else<sc_dims>("paddings", sc_dims(ndims_ - 2, 0)));
  ...
  if (ndims_ == 5) {
    expr_kernels.insert(expr_kernels.begin(),
                        g.dim_to_expr(weight_dims[ndims_ - 3]));
  }
  ...
  for (int i = 0; i < ndims_ - 2; i++) {
    if (is_dynamic_dim(data_dims[shape_begin_axis + i]) &&
        out_dims[shape_begin_axis + i] != data_dims[shape_begin_axis + i]) {
      auto var_in = g.dim_to_expr(data_dims[shape_begin_axis + i]);
      auto var_out = g.dim_to_expr(out_dims[shape_begin_axis + i]);
      expr_c cal_expr =
          do_cast_and_fold((var_in + expr_pads_begin[i] + expr_pads_end[i] -
...
conv_fwd_core_op_t::get_dynamic_runtime_info() {
  ...
  if (dilations.size() > 1) {
    dilation_dims = dilations;
  }

  auto dyn_info =
      ndims_ == 5
          ? dyn_conv_fwd_runtime_info_t(
                stride[0], stride[1], stride[2], pads_begin[0], pads_begin[1],
                pads_begin[2], pads_end[0], pads_end[1], pads_end[2],
                dilation_dims[0], dilation_dims[1], dilation_dims[2])
          : dyn_conv_fwd_runtime_info_t(stride[0], stride[1], pads_begin[0],
                                        pads_begin[1], pads_end[0], pads_end[1],
                                        dilation_dims[0], dilation_dims[1]);
...
shape_rl_vec conv_fwd_core_op_t::get_shape_relations_impl(
  ...
  shape_rl_vec ret;
  auto is_1x1 =
      std::all_of(weight_plain_dims.begin() + 2, weight_plain_dims.end(),
                  [](int x) { return x == 1; });
  ...
  if (is_dynamic_dim(data_BS)) {
    ret.emplace_back(data_BS, out_BS);
  }
...
static graph_tensor_ptr get_conv_rl_real_weight(const graph_tensor_ptr &wei) {
  ...
  while ((current->producer_owner_->isa<padding_op_t>() ||
         ...
         !current->producer_owner_->attrs_.get_or_else("produce_real_weight",
                                                       false)) {
...
sc_op_ptr conv_fwd_core_op_t::do_compensations(sc_graph_t &mgr,
                                               const context_ptr &ctx) {
  ...
  if (data_com) {
    cur_node = mgr.make(
        "sub", {cur_node->get_outputs()[0], data_com->get_outputs()[0]}, {},
        {});
  }
  if (s8s8_weight_com[0]) {
    cur_node = mgr.make(
        "sub",
        {cur_node->get_outputs()[0], s8s8_weight_com[0]->get_outputs()[0]}, {},
        {{"bc_axis",
          is_group_conv ? std::vector<int>{1, 2} : std::vector<int>{1}}});
  }
  if (s8s8_weight_com[1]) {
    cur_node = mgr.make(
        "sub",
        {cur_node->get_outputs()[0], s8s8_weight_com[1]->get_outputs()[0]}, {},
        {{"bc_axis",
          is_group_conv ? std::vector<int>{1, 2} : std::vector<int>{1}}});
  }
  if (const_com) {
    cur_node = mgr.make(
        "add", {cur_node->get_outputs()[0], const_com->get_outputs()[0]}, {},
        {});
  }
...
sc_op_ptr conv_fwd_core_op_t::get_data_compensation(sc_graph_t &mgr) {
  ...
  auto reduce_node =
      mgr.make("reduce", cast_node->get_outputs(), {},
               {{"rd_axis", rdaxis}, {"rd_op", 0}, {"keep_dims", true}});
  sc_op_ptr ret_node;
  if (is_dyn_quan) {
    if (!dyn_weight_zero_points) {
      return nullptr;
    }
    ...
    ret_node = mgr.make(
        "mul", {reduce_node->get_outputs()[0], dyn_weight_zero_points}, {}, {});
  } else {
    ...
    if (weight_zero_points.size() == 1) {
      ...
      // per channel
      COMPILE_ASSERT(0, "conv_fwd does not support per channel weight zero "
                        "points "
                        "compensation yet");
    }
    auto constant_node = mgr.make("constant", {}, {},
                                  {{"values", weight_zero_points_ptr},
                                   {"dtype", datatypes::s32},
                                   {"plain_dims", const_plain_dims},
                                   {"format", const_format}});
    ret_node = mgr.make(
        "mul", {reduce_node->get_outputs()[0], constant_node->get_outputs()[0]},
        {}, {});
  }
  if (kh > 1 || kw > 1 || kd > 1) {
    std::shared_ptr<static_data_t> kernel_data_ptr =
        std::make_shared<static_data_t>(std::vector<int>(kh * kw * kd, 1));
    ...
    auto constant_node = mgr.make(
        "constant", {}, {},
        {{"values", kernel_data_ptr},
         {"dtype", datatypes::s32},
         {"plain_dims", const_plain_dims},
         {"format", use_rl ? sc_data_format_t()
...
conv_fwd_core_op_t::get_s8s8_and_weight_compensation(sc_graph_t &mgr,
                                                     bool s8s8_compensation) {
  ...
  auto data_zero_points = attrs_.get_or_else(data_zp_key, std::vector<int>{0});
  auto dyn_data_zero_points =
      attrs_.get_or_else(dyn_data_zp_key, graph_tensor_ptr());
  bool weight_compensation =
      (is_dyn_quan && dyn_data_zero_points) ||
      (!data_zero_points.empty() &&
       !(std::all_of(data_zero_points.begin(), data_zero_points.end(),
                     [](int i) { return i == 0; })));
  std::vector<sc_op_ptr> nodes = {nullptr, nullptr};
  if (!s8s8_compensation && !weight_compensation) {
    return nodes;
  }

  auto weight = get_conv_rl_real_weight(info_.inputs_[1]);
  auto cast_node = mgr.make("cast", {weight}, {}, {{"dtype", datatypes::s32}});
  ...
  if (weight_compensation) {
    if (is_dyn_quan) {
      COMPILE_ASSERT(dyn_data_zero_points &&
                     ...
                     "points compensation yet");
      nodes[0] = mgr.make(
          "mul", {reduce_node->get_outputs()[0], dyn_data_zero_points}, {}, {});
    } else {
      ...
      if (data_zero_points.size() == 1) {
        ...
        COMPILE_ASSERT(0, "conv_fwd does not support per channel data zero "
                          "points "
                          "compensation yet");
      }
      auto constant_node = mgr.make("constant", {}, {},
  ...
  if (s8s8_compensation) {
    auto s8_constant_node = mgr.make(
        "constant", {}, {},
        {{"values", std::make_shared<static_data_t>(std::vector<int>{128})},
         {"dtype", datatypes::s32},
         {"plain_dims", sc_dims{1}},
         {"format", sc_data_format_t()}});
    nodes[1] = mgr.make(
        "mul",
        {reduce_node->get_outputs()[0], s8_constant_node->get_outputs()[0]}, {},
        {});
  }
  ...
  if (has_pad && (s8s8_compensation || weight_compensation)) {
    if (s8s8_compensation) {
      attrs_["padding_value"] = 128;
    }
    if (weight_compensation) {
      COMPILE_ASSERT(data_zero_points.size() == 1,
                     "padded conv_fwd currently doesn't support per channel "
                     "zero points");
      attrs_["padding_value"] =
          attrs_.get_or_else("padding_value", 0) + data_zero_points[0];
    }
  }
  return nodes;
}
  ...
  if (is_dyn_quan) {
    COMPILE_ASSERT(
        dyn_data_zero_points->details_.get_plain_dims() == sc_dims{1} &&
            dyn_weight_zero_points->details_.get_plain_dims() == sc_dims{1},
        "conv_fwd does not support per channel data/weight zero "
        "points compensation yet");
    ret_node =
        g.make("mul", {dyn_data_zero_points, dyn_weight_zero_points}, {}, {});
    auto const_reduce =
        g.make("constant", {}, {},
               {{"dtype", datatypes::s32},
                {"values", std::make_shared<static_data_t>(&C, sizeof(int))},
                {"plain_dims", sc_dims{1}}});
    ret_node = g.make(
        "mul", {ret_node->get_outputs()[0], const_reduce->get_outputs()[0]}, {},
        {});
  } else if (!has_pad) {
    COMPILE_ASSERT(data_zero_points.size() == 1 &&
    ...
    ret_node = g.make(
        ...
        {{"values",
          std::make_shared<static_data_t>(std::vector<int>{
              data_zero_points[0] * weight_zero_points[0] * C * kd * kh * kw})},
         {"dtype", datatypes::s32},
         {"plain_dims", sc_dims{1}},
         {"format", sc_data_format_t()}});
  } else {
    ...
    auto constant_data_zps =
        g.make("constant", {}, {},
               {{"values", std::make_shared<static_data_t>(std::vector<int>(
    ...
    auto constant_wei_zps =
        g.make("constant", {}, {},
               {{"values", std::make_shared<static_data_t>(std::vector<int>(
                ...
                {"format", use_rl ? sc_data_format_t()
                                  : info_.inputs_[1]->details_.get_format()}});
    auto cast_data_zps = g.make("cast", {constant_data_zps->get_outputs()[0]},
                                {}, {{"dtype", datatypes::f32}});
    ...
    auto conv_node = g.make(
        "conv_fwd_core",
        {cast_data_zps->get_outputs()[0], cast_wei_zps->get_outputs()[0]}, {},
        conv_attr);
...
bool conv_bwd_data_core_op_t::use_nested_generator() {
  ...
  if (is_1x1_) {
    ...
    auto tmp_kernel = utils::make_unique<gen_nested_conv1x1_backprop_data_t>(
    ...
    int possible_parallel_space = BS * (OS / im_ow_block) * (IC / im_ic_block);
    if (possible_parallel_space < num_threads) {
      return false;
    }
    return true;
  } else {
    ...
    auto tmp_kernel = utils::make_unique<gen_nested_convNxN_backprop_data_t>(
        this, stride, pads_begin,
        graph::extract_detail_from_tensors(get_inputs()),
        graph::extract_detail_from_tensors(get_outputs()));
    int im_ic_block = tmp_kernel->im_ic_block_;
...
body_generator_ptr conv_bwd_data_core_op_t::create_generator() {
  ...
  if (D == 1 && R == 1 && S == 1) {
    if (use_nested_generator()) {
      return utils::make_unique<gen_nested_conv1x1_backprop_data_t>(
          this, stride, pads_begin,
          graph::extract_detail_from_tensors(get_inputs()),
          graph::extract_detail_from_tensors(get_outputs()));
    } else {
      SC_MODULE_WARN << "Fall-back to non-nested conv1x1 backprop data.";
      return utils::make_unique<gen_conv1x1_backprop_data_t>(
          this, stride, pads_begin,
          graph::extract_detail_from_tensors(get_inputs()),
          graph::extract_detail_from_tensors(get_outputs()));
    }
    ...
    if (use_nested_generator()) {
      return utils::make_unique<gen_nested_convNxN_backprop_data_t>(
          this, stride, pads_begin,
          graph::extract_detail_from_tensors(get_inputs()),
          graph::extract_detail_from_tensors(get_outputs()));
    } else {
      SC_MODULE_WARN << "Fall-back to non-nested convNxN backprop data.";
      return utils::make_unique<gen_convNxN_backprop_data>(
          this, stride, pads_begin,
          graph::extract_detail_from_tensors(get_inputs()),
          graph::extract_detail_from_tensors(get_outputs()));
    }
...
void conv_bwd_data_core_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  std::vector<std::vector<sc_data_format_t>> in_formats, out_formats;
  ...
  if (is_bf16) {
    ...
    in_formats.push_back({is_3d
                              ? sc_data_format_t::CKDRSkc(oc_block, ic_block)
...
conv_bwd_weight_core_op_t::conv_bwd_weight_core_op_t(
  ...
  is_1x1_ = std::all_of(weight_shape.begin() + 2, weight_shape.end(),
                        [](int x) { return x == 1; });
  COMPILE_ASSERT(
      in_data_dims[0] == in_fwd_output_dims[0],
  ...
  if (attrs_.has_key("auto_pad")) {
    ...
    if (pad_type == "VALID") {
      ...
      conv_fwd_core_op_t::infer_auto_pad(get_owner_graph(), in_data_dims,
                                         weight_shape, strides, dilations,
  ...
  if (info_.outputs_.empty()) {
    ...
    COMPILE_ASSERT(info_.outputs_[0]->details_.get_plain_dims() == weight_shape,
...
bool conv_bwd_weight_core_op_t::use_nested_generator() {
  ...
  if (!is_1x1_) {
    ...
    int BS = input_shape[0], IC = input_shape[1], OC = delta_shape[1],
        OH = delta_shape[2];
    if (BS % im_bs_block || IC % im_ic_block || OC % im_oc_block || OH % 7) {
      return false;
    }
    // we need to check whether we can fully utilize all threads
    int possible_parallel_space =
        (BS / im_bs_block) * (IC / im_ic_block) * (OC / im_oc_block) * (OH / 7);
    if (possible_parallel_space < num_threads) {
      return false;
    }
    return true;
    ...
    auto tmp_kernel = utils::make_unique<gen_nested_conv1x1_backprop_weight_t>(
        this, stride, pads_begin,
        graph::extract_detail_from_tensors(get_inputs()),
        graph::extract_detail_from_tensors(get_outputs()));
    int im_oc_block = tmp_kernel->im_oc_block_;
...
body_generator_ptr conv_bwd_weight_core_op_t::create_generator() {
  ...
  if (is_1x1_) {
    if (use_nested_generator()) {
      ...
      SC_MODULE_WARN << "Fall-back to non-nested conv1x1 backprop weight.";
      // tested for reduce on ALL
      int block_size = 64;
      if (weight_shape[0] * weight_shape[1] * input_dims[0] /
              (block_size * block_size * block_size) <
          runtime_config_t::get().get_num_threads()) {
        return utils::make_unique<gen_conv1x1_backprop_weight_t>(
            this, stride, pads_begin,
            graph::extract_detail_from_tensors(get_inputs()),
            graph::extract_detail_from_tensors(get_outputs()),
            gen_conv1x1_backprop_weight_t::generator_type_t::REDUCE_ALL2);
      } else {
        return utils::make_unique<gen_conv1x1_backprop_weight_t>(
            this, stride, pads_begin,
            graph::extract_detail_from_tensors(get_inputs()),
    ...
    if (use_nested_generator()) {
      return utils::make_unique<gen_nested_convNXN_bwd_weight_t>(
          this, stride, pads_begin,
          graph::extract_detail_from_tensors(get_inputs()),
          graph::extract_detail_from_tensors(get_outputs()));
    }
    SC_MODULE_WARN << "Fall-back to non-nested convNxN backprop weight.";
    return utils::make_unique<gen_convNxN_backprop_weight>(
        this, stride, pads_begin,
        graph::extract_detail_from_tensors(get_inputs()),
        graph::extract_detail_from_tensors(get_outputs()),
...
void conv_bwd_weight_core_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  std::vector<std::vector<sc_data_format_t>> in_formats, out_formats;
  ...
  if (use_nested_generator()) {
    ...
    auto gen_1x1 = dynamic_cast<gen_nested_conv1x1_backprop_weight_t *>(
        temp_generator.get());
    auto gen_NxN =
        dynamic_cast<gen_nested_convNXN_bwd_weight_t *>(temp_generator.get());
    int im_bs_block = is_1x1_ ? gen_1x1->im_bs_block_ : gen_NxN->im_bs_block_;
    int im_ic_block = is_1x1_ ? gen_1x1->im_ic_block_ : gen_NxN->im_ic_block_;
    int im_oc_block = is_1x1_ ? gen_1x1->im_oc_block_ : gen_NxN->im_oc_block_;
    const bool is_3d = ndims_ == 5;
    in_formats.reserve(get_inputs().size());
    if (!is_1x1_) {
  ...
  if (info_.inputs_[0]->details_.dtype_ == datatypes::bf16) {
    in_formats.push_back(
        {is_3d ? sc_data_format_t(sc_data_format_kind_t(0, 1, 2, 3, 4, 0, 1, 0),
    ...
    // NC(D)HWnc
    in_formats.push_back(
        {is_3d ? sc_data_format_t(sc_data_format_kind_t(0, 1, 2, 3, 4, 0, 1),
  ...
  // NK(D)HWkn
  in_formats.push_back(
      {is_3d ? sc_data_format_t(sc_data_format_kind_t(0, 1, 2, 3, 4, 1, 0),
