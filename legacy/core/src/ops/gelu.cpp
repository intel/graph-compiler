...
gelu_op::gelu_op(const std::vector<graph_tensor_ptr> &ins,
  ...
  if (outs.empty()) {
    ...
    COMPILE_ASSERT(info_.outputs_.size() == 1,
                   "gelu op shall have only 1 output.")
...
gelu_backprop_op::gelu_backprop_op(const std::vector<graph_tensor_ptr> &ins,
  ...
  info_.inputs_ = ins;
  COMPILE_ASSERT(
      gc::graph::check_shape_equal(info_.inputs_[0]->details_.get_plain_dims(),
...
void gelu_op::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  if (attrs_.get_or_else("gelu_type", std::string("erf")) == "tanh") {
    ...
    auto mul1 = graph->make(
        "mul", {mul0->get_outputs()[0], fitting_const->get_outputs()[0]}, {},
        {});
    ...
    auto mul3 = graph->make(
        "mul", {mul2->get_outputs()[0], sqrt_2_over_pi->get_outputs()[0]}, {},
        {});
    ...
    half = graph->make<constant_op_t>(
        std::make_shared<static_data_t>(std::vector<float>{0.5f}),
        datatypes::f32, sc_dims{1});
    auto mul0 = graph->make("mul", {inputs0, one_over_sqrt_2->get_outputs()[0]},
                            {}, {});
    auto erf0 = graph->make("erf", {mul0->get_outputs()[0]}, {}, {});
    auto add0 = graph->make(
        "add", {erf0->get_outputs()[0], one->get_outputs()[0]}, {}, {});
    auto mul1 = graph->make("mul", {add0->get_outputs()[0], inputs0}, {}, {});
    auto mul2 = graph->make(
        "mul", {mul1->get_outputs()[0], half->get_outputs()[0]}, {}, {});
    output_op = mul2;
  }
...
void gelu_backprop_op::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  if (attrs_.get_or_else("gelu_type", std::string("erf")) == "tanh") {
    ...
    // 0.0356774x*x
    auto mul1 = graph->make(
        "mul", {mul0->get_outputs()[0], fitting_const1->get_outputs()[0]}, {},
        {});
    // 0.0356774x*x+0.797885
    auto add0 = graph->make(
        "add", {mul1->get_outputs()[0], fitting_const2->get_outputs()[0]}, {},
        {});
    ...
    auto mul4 = graph->make(
        "mul", {mul0->get_outputs()[0], fitting_const3->get_outputs()[0]}, {},
        {});
    // 0.053561x*x+0.398942
    auto add1 = graph->make(
        "add", {mul4->get_outputs()[0], fitting_const4->get_outputs()[0]}, {},
        {});
    ...
    // -0.5f*x*x
    auto mul1 = graph->make(
        "mul", {mul0->get_outputs()[0], neg_half->get_outputs()[0]}, {}, {});
    auto exp = graph->make("exp", mul1->get_outputs(), {}, {});
    auto mul2 = graph->make(
        "mul", {exp->get_outputs()[0], fitting_const1->get_outputs()[0]}, {},
        {});
    auto mul3 = graph->make("mul", {mul2->get_outputs()[0], inputs0}, {}, {});
    auto mul4 =
        graph->make("mul", {inputs0, fitting_const2->get_outputs()[0]}, {}, {});
    auto erf = graph->make("erf", mul4->get_outputs(), {}, {});
    auto add0 = graph->make(
        "add", {erf->get_outputs()[0], one->get_outputs()[0]}, {}, {});
    auto mul5 = graph->make(
        "mul", {add0->get_outputs()[0], half->get_outputs()[0]}, {}, {});
    auto add1 = graph->make(
        "add", {mul3->get_outputs()[0], mul5->get_outputs()[0]}, {}, {});
    auto mul6 = graph->make("mul", {add1->get_outputs()[0], inputs1}, {}, {});
    output_op = mul6;
  }
  // cast output
  output_op = cast_output_dtype(outputs[0], graph, output_op);
...
void gelu_op::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}

void gelu_backprop_op::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {}
