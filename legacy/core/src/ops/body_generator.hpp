...
struct body_generator_base_t {
   ...
   * others in graph
   * */
  virtual bool is_valid_config(const context_ptr &ctx,
  ...
  virtual config_ptr_vec
  get_dynamic_config_candidates(const context_ptr &ctx) const {
    return config_ptr_vec();
  }

  virtual std::vector<uint64_t>
  convert_config_to_keys(const config_ptr &config) const {
  ...
  body_generator_base_t(sc_op *owner,
      ...
      : owner_(owner), in_tensors_(std::move(in_tensors)),
...
struct body_generator_t : public body_generator_base_t {
  ...
  bool is_valid_config(const context_ptr &ctx,
                       const void *config) const override {
  ...
  func_t get_single_core_func(context_ptr ctx, const void *config,
                              ...
                              const std::vector<expr> &outputs,
                              std::vector<for_loop> &loops) const override {
    return get_single_core_func(ctx, *reinterpret_cast<const TConfig *>(config),
                                fusion, inputs, outputs, loops);
  }

  virtual void schedule_loops(context_ptr ctx, const TConfig &config, stmt body,
                              std::vector<for_loop> &fors) const = 0;

  void schedule_loops(context_ptr ctx, const void *config, stmt body,
                      std::vector<for_loop> &fors) const override {
    schedule_loops(ctx, *reinterpret_cast<const TConfig *>(config), body, fors);
  }
