...
public:
  ...
  round_op_t(const std::vector<graph_tensor_ptr> &ins, const any_map_t &attrs);
...
public:
  ...
  hardswish_op_t(graph_tensor_ptr v, float alpha = 1.f / 6.f, float beta = 0.5f)
      : unary_elementwise_op_impl_t(std::move(v), "hardswish"), alpha_(alpha),
        beta_(beta) {
    alg_kind_ = brgemm::eltwise_hardswish;
  };
  float alpha_;
...
public:
  ...
  hardsigmoid_op_t(graph_tensor_ptr v, float alpha = 1.f, float beta = 0.f)
      : unary_elementwise_op_impl_t(std::move(v), "hardsigmoid"), alpha_(alpha),
        beta_(beta){};
  float alpha_;
  float beta_;
};
