...
binary_backward_op_impl_t::binary_backward_op_impl_t(
  ...
  auto set_out_shape_format = [this, &output_shape]() {
    ...
    info_.outputs_[pos]->details_.dtype_ = info_.inputs_[0]->details_.dtype_;
  };
  if (outs.empty()) {
    // out[0]
    ...
    info_.outputs_ = outs;
    COMPILE_ASSERT(outs.size() == 2, "Binary backward op shall have 2 outpus.");
  }
...
static void compute_binary_backward_vectorized_op(
    ...
    const vectorized_info_t &vx_info, const mask_compute_func_t &compute_lanes,
  ...
  for (unsigned i = 0; i < dst_1->nslice_dims(); i++) {
    ...
    iter_vars.emplace_back(
        range_from_outer_loop(dst_1->get_ranges()[i])
            ? expr(0)
  ...
  if (dense_stride && (!floor.isa<constant>() || floor_int)) {
    ...
    indexing_from_diff_cond(false, false, *dst_1, dst_idx_floor, lanes,
                            indexed_target_floor_1, slice_len,
                            iter_vars.at(vx_info.axis), floor);
    // dst_2
    indexing_from_diff_cond(false, false, *dst_2, dst_idx_floor, lanes,
                            indexed_target_floor_2, slice_len,
                            iter_vars.at(vx_info.axis), floor);
    ...
    for (unsigned j = 0; j < src.size(); j++) {
      indexed_input_floor.emplace_back(indexing_from_diff_cond(
          ...
          input_floor_idx, slice_len, iter_vars.at(vx_info.axis), floor));
    }
    ...
    if (iter_vars.at(vx_info.axis).isa<var>()) {
      cur =
          make_stmt<for_loop_node_t>(iter_vars.at(vx_info.axis), expr(0), floor,
                                     expr(lanes), cur, true, for_type::NORMAL);
  ...
  if (((!tail.isa<constant>() && !is_blocking_shape) || tail_int) ||
    ...
    expr mask;
    if (!use_scalar) {
      mask = last_dim_generate_mask(tail_var, floor, slice_len, lanes, true);
    }
    ...
    cur = make_stmt<for_loop_node_t>(
        ...
        use_scalar ? expr(1) : expr(lanes), cur, true, for_type::NORMAL);
    bind_loop_axis(expand_gt, cur, vx_info.axis, true);
    tcur.emplace_back(cur);
  ...
  for (int i = loop_size - 1; i >= 0; i--) {
    if (i != vx_info.axis) {
      ...
      if (iter_vars.at(i).isa<var>()) {
        ...
        cur = make_stmt<for_loop_node_t>(
            std::move(iter_vars.at(i)), expr(0), dst_1->get_shape().at(i),
            vx_info.axis == i ? lanes : expr(1), std::move(body), true,
            for_type::NORMAL);
...
void binary_backward_op_impl_t::compute_block(
  ...
  auto func = [&](const std::vector<expr> &in,
    ...
    case binary_backward_operator::PRELU_BWD: {
      // src diff
      expr res_out0 =
          builder::make_select(in0 > make_expr<constant_node>(0.f, in0->dtype_),
                               in2, builder::make_mul(in1, in2));
      // weight diff
      expr res_out1 = builder::make_mul(
          builder::make_min(in0, make_expr<constant_node>(0.f, in0->dtype_)),
          in2);
      auto assign_out0 = builder::make_assign_unattached(out[0], res_out0);
      auto assign_out1 = builder::make_assign_unattached(out[1], res_out1);
      return builder::make_stmts_unattached({assign_out0, assign_out1});
    } break;
    default: {
      COMPILE_ASSERT(false, "Unsupport binary backward op "
                            "found.\n");
      return stmt();
    } break;
    }
  ...
  compute_binary_backward_vectorized_op(
      ctx, get_owner_graph(), inputs, dst, info_, vx_info_,
      mask_compute_func_t(func), mask_compute_func_t(func), attrs_,
      get_outputs()[0], wkld, use_mask);
}

void binary_backward_op_impl_t::query_format(
    context_ptr ctx,
...
infer_status_code
binary_backward_op_impl_t::infer_slice_ranges(const context_ptr &ctx,
                                              fslice_map &fsmap) {
  ...
  if (known_ranges_map.size() == get_inputs().size()) {
    auto erase_datamap = [&known_ranges_map, this, &fsmap](int32_t p1,
                                                           int32_t p2) {
      if (known_ranges_map[p1].size() != known_ranges_map[p2].size()) {
        // try to align with smaller one and erase bigger one
        int erase_input_id =
            known_ranges_map[p1].size() < known_ranges_map[p2].size() ? p2 : p1;
...
infer_status_code
binary_backward_op_impl_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                  fslice_map &fsmap) {
  auto &outslice_1 = fsmap.get(get_outputs()[0]);
  auto &outslice_2 = fsmap.get(get_outputs()[1]);
...
shape_rl_vec binary_backward_op_impl_t::get_dynamic_shape_relations() const {
  ...
  auto add_rls_func = [&in0_plain_dims, &in1_plain_dims, &in2_plain_dims,
    ...
    auto condition_add = [&ret, &out_plain_dims](size_t i,
                                                 const sc_dims &in_plain_dims) {
