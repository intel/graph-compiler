...
public:
  ...
  std::vector<int>
  get_non_broadcast_input_index(bool assert_non_empty) const override;
...
public:
  unary_backward_base_t(graph_tensor_ptr lhs, graph_tensor_ptr rhs,
                        elt_operator elt_opt);
...
public:
  abs_bwd_op_t(graph_tensor_ptr lhs, graph_tensor_ptr rhs,
               bool vectorized = false)
      : unary_backward_base_t(std::move(lhs), std::move(rhs),
                              elt_operator::ABS_BWD) {}
...
public:
  clamp_bwd_op_t(graph_tensor_ptr src, graph_tensor_ptr dst)
      : unary_backward_base_t(std::move(src), std::move(dst),
                              elt_operator::CLAMP_BWD) {}
  ...
  clamp_bwd_op_t(const std::vector<graph_tensor_ptr> &ins,
    ...
    assert(attrs.get_or_null<float>("min") && attrs.get_or_null<float>("max"));
