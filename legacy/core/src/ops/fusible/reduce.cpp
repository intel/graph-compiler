...
reduce_op_t::reduce_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  for (unsigned i = 0; i < old_reduce_dims.size(); i++) {
    bool is_reduction = std::find(plain_rd_axis_.begin(), plain_rd_axis_.end(),
                                  static_cast<int>(i)) != plain_rd_axis_.end();
    if (is_reduction) {
      if (keep_dims_) {
        new_reduce_dims.push_back(1);
      }
    } else {
      new_reduce_dims.push_back(old_reduce_dims[i]);
    }
  }
  if (new_reduce_dims.empty())
    new_reduce_dims.push_back(1);
  if (outs.empty()) {
    logical_tensor_t out;
    if (keep_dims_) {
      out = logical_tensor_t(
          get_reduced_format(ins[0]->details_.get_format(), plain_rd_axis_,
...
reduce_op_t::reduce_op_t(graph_tensor_ptr v, const std::vector<int> &rd_axis,
    ...
    : reduce_op_t({std::move(v)}, {},
                  {{"rd_axis", rd_axis},
                   {"rd_op", static_cast<int>(rd_op)},
                   {"keep_dims", keep_dims}}) {}

void reduce_op_t::query_format(
    context_ptr ctx,
  ...
  if (keep_dims_) {
    out_formats.push_back({get_reduced_format(
        in_fmt, plain_rd_axis_,
    ...
    if (!in_fmt.is_blocking()) {
      ...
      if (static_cast<int>(plain_rd_axis_.size()) ==
        ...
        for (int i = 0; i < in_fmt.format_code_.norig_dims(); i++) {
          if (std::all_of(
                  plain_rd_axis_.begin(), plain_rd_axis_.end(),
                  [&](int j) { return j != in_fmt.format_code_.get(i); })) {
            old_code_idx.push_back(in_fmt.format_code_.get(i));
          }
        }
        for (int i = 0; i < static_cast<int>(old_code_idx.size()); i++) {
        ...
        for (int i = in_fmt.format_code_.norig_dims();
          ...
          for (int j = 0; j < static_cast<int>(old_code_idx.size()); j++) {
        ...
        for (int i = 0; i < static_cast<int>(new_code_idx.size()); i++) {
          new_code.set(i, new_code_idx[i]);
        }
        // copy blocks_ to new_format
        if (std::all_of(
                plain_rd_axis_.begin(), plain_rd_axis_.end(), [&](int i) {
                  return !in_fmt.format_code_.collect_blocking_index(i).empty();
                })) {
          int blocks_idx = 0;
          for (int i = in_fmt.format_code_.norig_dims();
               i < in_fmt.format_code_.ndims(); i++) {
            if (std::none_of(
                    plain_rd_axis_.begin(), plain_rd_axis_.end(),
                    [&](int j) { return in_fmt.format_code_.get(i) == j; })) {
              new_format.blocks_[blocks_idx] =
                  in_fmt.blocks_[i - in_fmt.format_code_.norig_dims()];
              blocks_idx++;
            }
          }
        } else {
...
infer_output_slice_range(bool is_reduce_compute, uint64_t vec_step,
  ...
  for (auto &known_ranges : known_ranges_list) {
    ...
    for (size_t i = 0; i < known_ranges.size(); i++) {
      if (real_rd_axis.end() !=
        ...
        if (is_reduce_compute && i == known_ranges.size() - 1) {
          reduce_range.emplace_back(std::pair<expr, expr>{0, vec_step});
        }
      } else {
    ...
    if ((known_ranges.size() == real_rd_axis.size()) && !keep_dims)
      reduce_range.emplace(reduce_range.begin(), std::pair<expr, expr>{0, 1});
...
infer_status_code reduce_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                  fslice_map &fsmap) {
  ...
  auto status =
      update_reduce_op_fsmap(this, get_inputs()[0], fsmap, real_rd_axis);
  if (status == infer_status_code::RETRY)
    return status;
...
infer_status_code reduce_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                      fslice_map &fsmap) {
  ...
  if (in_ranges.empty()) {
    ...
    for (auto &range : out_ranges) {
      ...
      // additional process is needed.
      auto real_dims =
          get_inputs()[0]->details_.get_blocking_dims_expr(get_owner_graph());
      ...
      for (size_t i = 0; i < real_dims.size(); i++) {
        if (real_rd_axis.end() !=
            std::find(real_rd_axis.begin(), real_rd_axis.end(), i)) {
          reduce_range.emplace_back(std::pair<expr, expr>{0, real_dims.at(i)});
          if (keep_dims_)
            idx++;
        } else {
...
void infer_reduce_binding_axis(fusible_op_t *cur, binding_axis_map &bdax_map,
                               const std::vector<int> &plain_rd_axis,
                               bool keep_dims) {
  ...
  if (cur->isa<reduce_impl_op_t>()) {
    if (cur->isa<reduce_compute_op_t>() &&
        !cur->get_inputs()[0]->producer_owner_->isa<reduce_compute_op_t>()) {
      auto user = cur->get_outputs()[0]->uses_[0].second;
      if (user->isa<output_op>())
        return;
  ...
  if (keep_dims) {
    ...
    for (size_t i = 0; i < plain_dims.size(); i++) {
      if (plain_rd_axis.end() != std::find(plain_rd_axis.begin(),
                                           plain_rd_axis.end(),
                                           static_cast<int>(i)))
        continue;
      else
    ...
    for (auto &bd_ax : known_axis_map[0]) {
      std::vector<int> ret;
      for (auto &ax : bd_ax) {
        auto iter = std::find(non_rd_axis.begin(), non_rd_axis.end(), ax);
...
void pre_reduce_binding_axis(fusible_op_t *cur, binding_axis_map &bdax_map,
  ...
  if (cur->isa<reduce_impl_op_t>()) {
    if (cur->isa<reduce_collect_op_t>()) {
      ...
      COMPILE_ASSERT(!producer->get_inputs()[0]
                          ->producer_owner_->isa<reduce_compute_op_t>(),
...
void reduce_op_t::pre_infer_binding_axis(binding_axis_map &bdax_map) {
  pre_reduce_binding_axis(
      this, bdax_map, attrs_.get_or_else("orig_plain_rd_axis", plain_rd_axis_),
      keep_dims_);
}
...
static union_val get_init_val_for_reduce(reduce_operator rd_op,
                                         sc_data_type_t dtype) {
  variant<float, int64_t> init_value;
  bool is_int =
      utils::is_one_of(dtype.type_code_, sc_data_etype::U8, sc_data_etype::U32,
                       sc_data_etype::S8, sc_data_etype::S32);
...
static binary_tir_gen_f get_binary_by_reduce_op(reduce_operator rdop) {
  ...
  case reduce_operator::add:
    return builder::make_add;
    break;
  case reduce_operator::mul:
    return builder::make_mul;
    break;
...
static unary_tir_gen_f get_binary_reduce_by_reduce_op(reduce_operator rdop) {
  switch (rdop) {
  case reduce_operator::add:
    return builder::make_reduce_add;
    break;
  case reduce_operator::mul:
    return builder::make_reduce_mul;
    break;
  case reduce_operator::max:
    return builder::make_reduce_max;
    break;
  case reduce_operator::min:
    return builder::make_reduce_min;
    break;
  }
  return nullptr;
}
...
static void compute_block_reduce(sc_graph_t &graph, const sc_op_info_t &info,
  ...
  bool last_axis_reduce =
      *rd_axis.rbegin() == static_cast<int>(src.at(0)->nslice_dims() - 1);
  ...
  for (unsigned i = 0; i < src.at(0)->nslice_dims(); i++) {
    iter_vars.emplace_back(
        range_from_outer_loop(src.at(0)->get_ranges()[i])
            ? expr(0)
  ...
  if (is_lastdim_meet_require) {
    ...
    if (rd_op == reduce_operator::add) {
      mask_directly =
          last_dim_generate_mask(src_idx.back(), floor, slice_len, lanes);
    } else {
      lanes = 1;
    }
  }
  ...
  expr indexed_target =
      builder::make_indexing(dst.tptr_, dst_idx, !last_axis_reduce ? lanes : 1,
  ...
  stmt body, cur;
  auto reduce_value = builder::make_var(sc_data_type_t(dtype.type_code_, lanes),
  ...
  stmt asnode = make_stmt<assign_node_t>(
      reduce_value,
      make_expr<constant_node>(value, sc_data_type_t(dtype.type_code_, lanes)));
  auto define_reduce =
      make_stmt<define_node_t>(reduce_value, linkage::local, expr());
  stmt padding_middle_asnode = make_stmt<assign_node_t>(
      inter_padding_var,
      make_expr<constant_node>(value, sc_data_type_t(dtype.type_code_, lanes)));
  auto define_padding_reduce =
      make_stmt<define_node_t>(inter_padding_var, linkage::local, expr());
  ...
  for (auto i : new_loop_order) {
    if (i == new_loop_order.front()) {
      ...
      is_padding_ir =
          cond_it != conditions.end() && rd_op != reduce_operator::add;
      ...
      if (is_padding_ir) {
        ...
        cur_list.emplace_back(
            builder::make_assign_unattached(inter_padding_var, indexed_input));

        // calculate mask, upper_bound - cur_index
        auto upper_bound_int =
            builder::make_cast(datatypes::s32, cond_it->second.second);
        auto cur_index_int =
            builder::make_cast(datatypes::s32, cond_it->second.first);
        auto cur_step = builder::make_min(
            builder::make_max(builder::make_sub(upper_bound_int, cur_index_int),
                              0),
            lanes);
        stmt mask_def;
        auto mask = generate_mask_var_by_step(mask_def, cur_step, lanes);
        ...
        cur_list.emplace_back(builder::make_assign_unattached(
            ...
            builder::make_select(
                mask,
                get_binary_by_reduce_op(rd_op)(reduce_value, inter_padding_var),
                reduce_value)));
        cur = builder::make_stmts_unattached(cur_list);
      } else {
        cur = make_stmt<assign_node_t>(
            reduce_value,
            get_binary_by_reduce_op(rd_op)(indexed_input, reduce_value));
      }
    ...
    if (iter_vars.at(i).isa<var>()) {
      ...
      cur = make_stmt<for_loop_node_t>(
          std::move(iter_vars.at(i)), expr(0), src.at(0)->get_shape().at(i),
    ...
    if (i == rd_axis.front()) {
      ...
      std::vector<stmt_c> rd_no_padding_assign{
          define_reduce, asnode, std::move(cur),
          make_stmt<assign_node_t>(
              indexed_target,
              lanes > 1 && last_axis_reduce
                  ? get_binary_reduce_by_reduce_op(rd_op)(reduce_value)
                  : reduce_value)};
      if (is_padding_ir) {
        std::vector<stmt_c> rd_padding_assign{define_padding_reduce,
                                              padding_middle_asnode};
...
void reduce_op_t::compute_block(
  ...
  compute_block_reduce(get_owner_graph(), info_, inputs, *dst[0], rd_op_,
                       ...
                       info_.inputs_[0]->details_.dtype_, attrs_,
                       get_inputs()[0], wkld, is_dynamic());
}

size_t
reduce_op_t::compute_workload(const std::vector<shape_dtype_pair> &ins,
...
bool reduce_op_t::can_split_op() const {
  if (attrs_.get_or_else("temp.no_split_reduce", false))
    return false;
  if (runtime_config_t::get().get_num_threads() == 1) {
    return true;
  }
  auto ax = get_rd_axis();
...
graph_tensor_ptr reduce_op_t::split_op(const context_ptr &ctx,
                                       sc_graph_t &graph, int num_threads) {
  ...
  bool is_bf16 = get_inputs()[0]->details_.dtype_ == datatypes::bf16 &&
                 rd_op_ != reduce_operator::max &&
                 rd_op_ != reduce_operator::min;
  if (is_bf16) {
  ...
  if (last_axis) {
    auto vec_step = vectorize_step(ctx, first_out->details_.dtype_.type_code_);
    auto new_dims = first_out->details_.get_blocking_dims();
    if (num_threads > 1) {
      new_dims.insert(new_dims.begin(), num_threads);
    }
  ...
  auto first =
      graph.make<reduce_compute_op_t>(get_inputs()[0], first_out, rd_ax, rd_op_,
                                      keep_dims_, /*local_mode*/ false);
  ...
  if (is_bf16) {
    ...
    second = graph.make("cast", {second_out}, {out_tsr},
...
void reduce_impl_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
...
infer_status_code
reduce_impl_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                         fslice_map &fsmap) {
...
void reduce_impl_op_t::infer_binding_axis(binding_axis_map &bdax_map) {
  infer_reduce_binding_axis(
      this, bdax_map,
      transform_axis_blocking2plain(get_inputs()[0]->details_, real_rd_axis_),
      keep_dims_);
}
...
reduce_impl_op_t::reduce_impl_op_t(const graph_tensor_ptr &in,
                                   const graph_tensor_ptr &old_out,
                                   const std::vector<int> &rd_axis,
                                   reduce_operator rd_op, bool keep_dims)
...
graph_tensor_ptr reduce_compute_op_t::split_op(const context_ptr &ctx,
                                               sc_graph_t &graph,
                                               int num_threads) {
  assert(can_split_op());
  ...
  auto first =
      graph.make<reduce_compute_op_t>(get_inputs()[0], first_out, real_rd_axis_,
                                      rd_op_, keep_dims_, /*local_mode*/ true);
  if (this->attrs_.has_key(mixed_partition_hint::trial_break)) {
    first->attrs_[mixed_partition_hint::trial_break] =
        this->attrs_[mixed_partition_hint::trial_break];
  }
  auto second = graph.make<reduce_collect_op_t>(
      first_out, second_out, real_rd_axis_, rd_op_, keep_dims_,
      reduce_collect_op_t::COPY);
  get_outputs()[0]->replace_with(second_out);
  remove();
  return second_out;
}
...
sc_op_ptr reduce_compute_op_t::copy(const std::vector<graph_tensor_ptr> &ins,
                                    const std::vector<graph_tensor_ptr> &outs,
                                    sc_graph_t &mgr) {
  auto ret = mgr.make<reduce_compute_op_t>(ins.at(0), outs.at(0), real_rd_axis_,
...
reduce_compute_op_t::reduce_compute_op_t(const graph_tensor_ptr &in,
  ...
  // if no keep dims
  if (!keep_dims_) {
    expected_dims = std::max((size_t)1, (expected_dims - real_rd_axis_.size()));
  }
  ...
  COMPILE_ASSERT(expected_dims == old_out->details_.get_blocking_dims().size(),
...
infer_status_code
reduce_compute_op_t::infer_slice_ranges(const context_ptr &ctx,
                                        fslice_map &fsmap) {
  slice_range_map known_ranges_map = search_known_input_slice(this, fsmap);
  if (known_ranges_map.empty())
    return infer_status_code::RETRY;
  ...
  fsmap.get(get_outputs()[0]) = infer_output_slice_range(
      true, vec_step, known_ranges_list, real_rd_axis, keep_dims_, num_threads);
  return infer_status_code::OK;
}

void reduce_compute_op_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dst,
    const std::vector<const tensor_slice *> &inputs) {
  size_t wkld = compute_fusible_workload(ctx, dst, inputs);
  // set default vectorized information
  ...
  auto func = [&](const std::vector<expr> &in,
    ...
    if (ths->keep_dims_) {
      ...
      if (last_axis_reduce) {
        indexing_nd->idx_.emplace_back(0);
      }
    } else {
      ...
      for (auto itr = indexing_nd->idx_.begin(); itr != indexing_nd->idx_.end();
           ++itr) {
        bool remove = false;
        ...
        if (!remove) {
          new_idx.emplace_back(std::move(*itr));
        }
      }
  ...
  compute_vectorized_op(ctx, get_owner_graph(), inputs, *dst[0], info_,
                        ...
                        mask_compute_func_t(func), attrs_, get_inputs()[0],
                        wkld, false, inputs[0],
                        /*unroll*/ local_mode_);
}
...
void reduce_compute_op_t::set_reduce_buffer(const tensor &buf) {
  ...
  if (local_mode_)
    buf->attr()[attr_keys::must_tensor2var] = true;
}
...
sc_op_ptr reduce_collect_op_t::copy(const std::vector<graph_tensor_ptr> &ins,
                                    const std::vector<graph_tensor_ptr> &outs,
                                    sc_graph_t &mgr) {
  auto ret = mgr.make<reduce_collect_op_t>(ins.at(0), outs.at(0), real_rd_axis_,
                                           rd_op_, keep_dims_, op_);
  ret->copy_dispatch_key_set_from_op(shared_from_this());
  ret->attrs_ = attrs_;
  return ret;
}
...
infer_status_code
reduce_collect_op_t::infer_slice_ranges(const context_ptr &ctx,
                                        fslice_map &fsmap) {
  slice_range_map known_ranges_map = search_known_input_slice(this, fsmap);
  if (known_ranges_map.empty())
    return infer_status_code::RETRY;
...
void reduce_collect_op_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dst,
    const std::vector<const tensor_slice *> &inputs) {
  if (op_ == COPY) {
    vx_info_.axis = dst[0]->get_shape().size() - 1;
    ...
    builder::get_current_builder()->emit(builder::make_stmts_unattached({}));
  }
}
