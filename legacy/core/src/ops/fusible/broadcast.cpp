...
*/
static bool is_auto_broadcast(const sc_dims &input_shape,
                              const sc_dims &output_shape) {
...
broadcast_op_t::broadcast_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  if (outs.empty()) {
    info_.outputs_.emplace_back(
        std::make_shared<graph_tensor>(this, sc_data_format_t(), output_shape_,
...
void broadcast_op_t::query_format(
    context_ptr ctx,
...
std::vector<int> broadcast_op_t::get_bc_axis() const {
  if (plain_bc_axis_ == std::vector<int>{-1})
    return plain_bc_axis_;
  return transform_axis_plain2blocking(info_.outputs_[0], plain_bc_axis_);
}

infer_status_code broadcast_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                     fslice_map &fsmap) {
  ...
  for (size_t i = 0; i < known_ranges_list.size(); i++) {
    ...
    for (size_t j = 0; j < out_dims.size(); j++) {
      if (std::find(blocking_bc_axis.begin(), blocking_bc_axis.end(), j) !=
        ...
        ranges_list[i].emplace_back(expr(0), expr(dim2unsigned(out_dims[j])));
      }
    }
  }
...
infer_status_code broadcast_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                         fslice_map &fsmap) {
...
void broadcast_op_t::compute_block(
  ...
  COMPILE_ASSERT(
      is_auto_broadcast(in_dims, out_dims),
  ...
  for (unsigned i = 0; i < dst_tsl->nslice_dims(); i++) {
    ...
    iter_vars.emplace_back(
        range_from_outer_loop(dst_tsl->get_ranges()[i])
            ? expr(0)
  ...
  if (floor.isa<constant>()) {
    ...
    COMPILE_ASSERT((floor_int + tail_int),
                   "Don't support shape len == 0 cases.");
  }
  ...
  auto func_indexing_input = [&](std::vector<expr> &in_idx, expr &indexed_input,
                                 expr &iter_var, bool use_scalar = false,
                                 bool has_tail = false) {
    if (bc_axis.back() == static_cast<int64_t>(vx_info_.axis)) {
      indexing_from_diff_cond(use_scalar, has_tail, *in_tsl, in_idx, lanes,
                              indexed_input, slice_len, iter_var, floor);
    } else {
      if (use_scalar) {
  ...
  for (int i = static_cast<int>(dst_tsl->get_shape().size() - 1); i >= 0; i--) {
    stmt body;
    ...
    if (static_cast<int>(dst_tsl->get_shape().size()) == vx_info_.axis + 1 &&
      ...
      if (!tail.isa<constant>() || tail_int) {
        ...
        indexing_from_diff_cond(use_scalar, true, *dst_tsl, dst_idx_tail, lanes,
        ...
        bld->push_scope();
        cur = assign(std::vector<expr>{indexed_input_tail}, target_vec_tail);
        cur->attr()[op_traits::workload_computable_t::workload_number] = wkld;
        bld->emit(cur);
      ...
      if (!tcur.empty() && tcur[0].defined()) {
        ...
        cur = make_stmt<for_loop_node_t>(
            std::move(iter_vars.at(i)), expr(0), dst_tsl->get_shape().at(i),
        ...
        indexed_target = builder::make_indexing(dst_tsl->tptr_, dst_idx);
        indexed_input = builder::make_indexing(in_tsl->tptr_, in_idx);
        std::vector<expr::lvalue_proxy_t> target_vec{
            expr::lvalue_proxy_t(indexed_target, false)};
        bld->push_scope();
...
size_t
broadcast_op_t::compute_workload(const std::vector<shape_dtype_pair> &ins,
