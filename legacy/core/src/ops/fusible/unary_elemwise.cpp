...
void unary_elementwise_op_impl_t::compute_block(
  ...
  if (!cur_cpu_flags.fAVX512F && cur_cpu_flags.fAVX2) {
    ...
    if (op_name_ == "cast") {
      const sc_data_etype dst_etype = dst[0]->tptr_->base_->dtype_.type_code_;
...
static infer_status_code infer_unary_slice_ranges(fusible_op_t *cur,
                                                  fslice_map &fsmap) {
...
infer_status_code
unary_elementwise_op_impl_t::infer_slice_ranges(const context_ptr &ctx,
                                                fslice_map &fsmap) {
  return infer_unary_slice_ranges(this, fsmap);
}

static infer_status_code pre_infer_unary_slice_ranges(fusible_op_t *cur,
                                                      fslice_map &fsmap) {
...
infer_status_code
unary_elementwise_op_impl_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                    fslice_map &fsmap) {
  return pre_infer_unary_slice_ranges(this, fsmap);
}

void infer_identical_binding_axis(fusible_op_t *cur,
  ...
  if (!outaxis.empty() && outaxis == known_axis_map[0]) {
    return;
  }
...
void pre_infer_identical_binding_axis(fusible_op_t *cur,
                                      binding_axis_map &bdax_map) {
  auto &outaxis = bdax_map.get(cur->get_outputs()[0]);
...
bool unary_elementwise_op_impl_t::register_brgemm_fusion(
  ...
  return brg_reg.register_op_infos(shared_from_this(),
                                   outputs[0]->get_tensor_ptr());
}

expr relu_op_t::compute_element(expr in) {
  return builder::make_max(make_expr<constant_node>((int64_t)0, in->dtype_),
                           in);
}
...
expr leaky_relu_op_t::compute_element(expr in) {
  ...
  return builder::make_select(
      in > make_expr<constant_node>((int64_t)0, in->dtype_), in, alpha * in);
}
...
expr round_op_t::compute_element(expr in) { return builder::make_round(in); }
...
static expr compute_sigmoid(expr in) {
  ...
  expr sign_mask =
      make_expr<constant_node>(0x80000000UL, sc_data_type_t::u32(lanes));
  // temp vars
  auto f_neg_x = builder::make_var(sc_data_type_t::f32(lanes),
                                   "f_neg_x" + fusion_create_var_idx());
  ...
  // make negative x
  bld->push_assign(
      f_neg_x, builder::make_reinterpret(
                   builder::make_int_xor(builder::make_reinterpret(
                                             in, sc_data_type_t::u32(lanes)),
                                         sign_mask),
  ...
  if (is_bf16) {
    ...
    return builder::make_cast(bf16ty,
                              builder::make_div(f_one, f_one + f_exp_neg_x));
  } else {
    return builder::make_div(f_one, f_one + f_exp_neg_x);
  }
}

expr sigmoid_op_t::compute_element(expr in) { return compute_sigmoid(in); }

expr exp_op_t::compute_element(expr in) {
  auto out = builder::make_exp(in);
  out->attr().set("overflow_check", attrs_.get_or_else("overflow_check", true));
  return out;
}
...
#define DECL_VAR(name, dtype) auto name = builder::make_var( \
  ...
  bld->push_assign(
      abs_a, builder::make_int_and(
                 builder::make_reinterpret(in, sc_data_type_t::u32(lanes)),
                 positive_mask));
  bld->push_assign(
      f_abs_a, builder::make_reinterpret(abs_a, sc_data_type_t::f32(lanes)));
  bld->push_assign(
      sign, builder::make_int_and(
                builder::make_reinterpret(in, sc_data_type_t::u32(lanes)),
                sign_mask));
  bld->push_assign(f_2a, builder::make_mul(f_abs_a, f_two));
  bld->push_assign(f_exp_2a, builder::make_exp(f_2a));
  bld->push_assign(f_tmp,
                   builder::make_div(f_exp_2a - f_one, f_exp_2a + f_one));
  bld->push_assign(
      f_out, builder::make_select(abs_a > uint_saturate_ubound, f_one, f_tmp));
  bld->push_assign(
      f_fin, builder::make_reinterpret(
                 builder::make_int_xor(builder::make_reinterpret(
...
expr square_op_t::compute_element(expr in) { return in * in; }

expr squared_root_op_t::compute_element(expr in) {
  if (reciprocal_) {
    return builder::make_rsqrt(in);
  }
...
cast_op_t::cast_op_t(graph_tensor_ptr v, sc_data_type_t out_dtype,
                     bool saturated)
    : unary_elementwise_op_impl_t(std::move(v), "cast"), dtype_(out_dtype),
...
expr abs_op_t::compute_element(expr in) { return builder::make_abs(in); }
...
expr swish_op_t::compute_element(expr in) {
  ...
  auto bld = builder::get_current_builder();
  auto f_sigmoid_in =
      builder::make_var(dtype, "f_sigmoid_in" + fusion_create_var_idx());
