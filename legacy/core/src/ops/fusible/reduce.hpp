...
struct maybe_split_optimized_t : public virtual op_base_trait_t {
  ...
  virtual graph_tensor_ptr split_op(const context_ptr &ctx, sc_graph_t &graph,
                                    int num_threads) = 0;
};
} // namespace op_traits
...
public:
  ...
  reduce_impl_op_t(const graph_tensor_ptr &in, const graph_tensor_ptr &old_out,
...
public:
  ...
  reduce_compute_op_t(const graph_tensor_ptr &in,
                      ...
                      const std::vector<int> &rd_axis, reduce_operator rd_op,
                      bool keep_dims, bool local_mode);
