...
padding_op_t::padding_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  if (outs.empty()) {
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(
        this, info_.inputs_[0]->details_.get_format(), expected_out_shape,
        info_.inputs_[0]->details_.dtype_));
  } else {
    COMPILE_ASSERT(outs.size() == 1, "padding expects 1 output");
    COMPILE_ASSERT(is_dynamic() ||
                       outs[0]->details_.get_plain_dims() == expected_out_shape,
...
padding_op_t::padding_op_t(graph_tensor_ptr v, sc_dims &pads_begin,
                           sc_dims &pads_end)
    : padding_op_t(
          {std::move(v)}, {},
...
sc_dims padding_op_t::infer_out_dims(const sc_dims &input_dims,
  ...
  if (ndims == 2) {
    ...
    COMPILE_ASSERT(!is_dynamic(),
                   "dynamic padding op does not support 2D cases");
    out_dims[0] += pads_begin[0] + pads_end[0];
  } else {
    out_dims[0] =
        is_dynamic_dim(input_dims[0]) ? dimensions::dynamic_any : input_dims[0];
    for (int i = 2; i < ndims; i++) {
      if (is_dynamic_dim(input_dims[i])) {
        out_dims[i] = dimensions::dynamic_any;
      } else {
        out_dims[i] = input_dims[i] + pads_begin[i - 2] + pads_end[i - 2];
      }
    }
  }
  return out_dims;
...
infer_status_code padding_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                       fslice_map &fsmap) {
  throw std::runtime_error("Not implemented");
}

infer_status_code padding_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                   fslice_map &fsmap) {
  ...
  for (size_t i = 0; i < slice_size; i++) {
    ...
    for (size_t j = 0; j < pads_begin.size(); ++j) {
      ...
      len =
          do_cast_and_fold(len + static_cast<int>(pads_begin[j] + pads_end[j]));
    }
  }
...
void padding_op_t::compute_block(context_ptr ctx,
  ...
  for (size_t i = 0; i < ndims; ++i) {
    iter_vars.emplace_back(
        range_from_outer_loop(src[0]->get_ranges()[i])
            ? expr(0)
    ...
    if (itr != padding_axis.end()) {
      dst_idx.back() =
          dst_idx.back() +
          static_cast<int>(
              pads_begin[std::distance(padding_axis.begin(), itr)]);
    }
  }
  ...
  expr indexed_src =
      builder::make_indexing(src[0]->tptr_, src_idx, can_vectorize ? step : 1);
  expr indexed_dst =
      builder::make_indexing(dst[0]->tptr_, dst_idx, can_vectorize ? step : 1);
  ...
  for (int64_t i = static_cast<int64_t>(ndims) - 1; i >= 0; --i) {
    ...
    cur = make_stmt<for_loop_node_t>(
        std::move(iter_vars[i]), expr(0), src[0]->get_shape()[i],
        i == static_cast<int64_t>(ndims) - 1 && can_vectorize ? expr(step)
                                                              : expr(1),
...
size_t
padding_op_t::compute_workload(const std::vector<shape_dtype_pair> &ins,
...
stmt padding_op_t::get_zero_out_stmt(const tensor &out,
  ...
  auto out_dtype = out->dtype_.is_pointer() ? out->dtype_.get_pointer_element()
                                            : out->dtype_;

  auto range = range_list.empty() ? slice_range{} : range_list[0];
  auto out_tsl =
      range.empty() ? tensor_slice(out) : tensor_slice(out, std::move(range));
  ...
  if (plain_ndims_ == 2) {
    ...
    bld.push_scope();
    if (pt > 0) {
      builtin::brgemm_init(builder::tensor_ptr(out_tptr, {0, 0}), pt, nm, nm,
                           out_dtype, padding_value);
    }
    if (pb > 0) {
      builtin::brgemm_init(builder::tensor_ptr(out_tptr, {km - pb, 0}), pb, nm,
      ...
      _named_for_(lk, pad_k, 0, K) {
        if (ph1_ > 0) {
          auto ptr =
              is_4d_out
                  ? (is_channel_last
                         ? builder::tensor_ptr(out_tptr, {pad_n, 0, 0, 0})
                         : builder::tensor_ptr(out_tptr, {pad_n, pad_k, 0, 0}))
                  : builder::tensor_ptr(out_tptr, {pad_n, pad_k, 0, 0, 0});
          builtin::brgemm_init(ptr, ph1_ * ow, c, c, out_dtype, padding_value);
        }
      }
    }
    _named_for_(ln, pad_n, 0, N, 1, for_type::PARALLEL) {
            ...
            builtin::brgemm_init(
                is_4d_out ? (is_channel_last
                                 ? builder::tensor_ptr(out_tptr,
                                                       {pad_n, p1 + ph1_, 0, 0})
                                 : builder::tensor_ptr(
                                       out_tptr, {pad_n, pad_k, p1 + ph1_, 0}))
                          : builder::tensor_ptr(
                                out_tptr, {pad_n, pad_k, p1 + ph1_, 0, 0}),
          ...
          if (pw2_ > 0) {
            builtin::brgemm_init(
                is_4d_out
                    ? (is_channel_last
                           ? builder::tensor_ptr(
                                 out_tptr, {pad_n, p1 + ph1_, iw + pw1_, 0})
                           : builder::tensor_ptr(out_tptr,
                                                 {
                                                     pad_n,
                                                     pad_k,
                    ...
                    : builder::tensor_ptr(
                          out_tptr, {pad_n, pad_k, p1 + ph1_, iw + pw1_, 0}),
          ...
          builtin::brgemm_init(
              is_4d_out ? (is_channel_last
                               ? builder::tensor_ptr(out_tptr,
                                                     {pad_n, ph1_ + ih, 0, 0})
                               : builder::tensor_ptr(
                                     out_tptr, {pad_n, pad_k, ph1_ + ih, 0}))
                        : builder::tensor_ptr(out_tptr,
