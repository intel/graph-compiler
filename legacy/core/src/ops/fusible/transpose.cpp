...
bool can_be_fast_transpose(
  ...
  bool is_s8u8 =
      dtype.is_etype(sc_data_etype::S8) || dtype.is_etype(sc_data_etype::U8);
  inp_a_axis.clear();
  inp_b_axis.clear();
  out_a_axis.clear();
  out_b_axis.clear();
  ...
  while (inp_idx < input_ndims || out_idx < output_ndims) {
    while (inp_idx < input_ndims &&
           utils::is_one_of(inp_code.get(inp_idx), out_a_idx, inp_b_idx)) {
    ...
    while (out_idx < output_ndims &&
           utils::is_one_of(out_code.get(out_idx), out_a_idx, inp_b_idx)) {
  ...
  if (static_cast<size_t>(input_ndims) - inp_a_axis.size() -
          inp_b_axis.size() !=
  ...
  auto find_another_vec_axis = [&](const sc_data_format_t &format,
                                   int &vec_axis, const tensor_slice &tsl,
                                   const int ori_idx) {
  ...
  if (input_format.is_blocking() && output_format.is_blocking()) {
    ...
    if (block_count_inp > 1 || block_count_out > 1) {
      for (auto iter = input_block_axis.begin(); iter != input_block_axis.end();
           iter++) {
  ...
  auto satisfy_dim_lanes = [&]() {
    ...
    return plain_dims[inp_b_idx] % trans_lanes2 == 0 &&
           ...
           get_expr_as_int(src.shape_[vec_inp_lastdim]) % trans_lanes2 == 0 &&
           get_expr_as_int(dst.shape_[vec_out_lastdim]) % trans_lanes1 == 0;
  };
  ...
  if (is_16bit && (dynamic_no_padding ||
                   (!is_dynamic && !meet_kernel_require(bit16_threshold)))) {
    trans_kernel_used = sc_trans_kernel::NO_TRANS;
    return false;
  } else if (is_s8u8 &&
             (dynamic_no_padding ||
    ...
    trans_kernel_used = sc_trans_kernel::NO_TRANS;
    return false;
  } else if (is_float && (!is_dynamic && !meet_kernel_require(f32_threshold))) {
    trans_kernel_used = sc_trans_kernel::NO_TRANS;
    return false;
  }
...
#define TRANS2D_REG_CALCULATION_BIT16x16(type_bits)                            \
  ...
  for (int iter_i = 0; iter_i < 16; iter_i += 8) {                             \
    for (int iter_j = 0, offset = 0; iter_j < 8; iter_j += 2, offset++) {      \
      TRANS2D_UNPACK_ASSIGN(low, iter_i + 17 + iter_j, iter_i + offset + 1,    \
                            iter_i + offset + 5, 64)                           \
      TRANS2D_UNPACK_ASSIGN(high, iter_i + 18 + iter_j, iter_i + offset + 1,   \
                            iter_i + offset + 5, 64)                           \
    }                                                                          \
  }                                                                            \
  for (int iter_i = 0; iter_i < 8; iter_i += 1) {                              \
    TRANS2D_SHUFFLE_PERMUTE_ASSIGN_F32(permute, iter_i + 1, iter_i + 17,       \
                                       iter_i + 25, 32, type_bits)             \
  }                                                                            \
  for (int iter_i = 8; iter_i < 16; iter_i += 1) {                             \
    TRANS2D_SHUFFLE_PERMUTE_ASSIGN_F32(permute, iter_i + 1, iter_i + 17 - 8,   \
                                       iter_i + 25 - 8, 49, type_bits)         \
  }
...
void compute_fast_transpose(sc_graph_t &graph, const context_ptr &ctx,
                            ...
                            bool is_dynamic, bool dynamic_no_padding,
  ...
  bool direct_use_bit16x16 =
      is_16bit && trans_kernel_used == sc_trans_kernel::BIT16_16X16_TRANS;
  ...
  auto is_shapesize_need_mask = [&](const int64_t inp_a_size,
    ...
    assert(!is_dynamic);
    if ((!is_dynamic && (inp_a_size % inp_a_step || inp_b_size % inp_b_step ||
                         out_a_size % inp_a_step || out_b_size % inp_b_step))) {
      need_mask = true;
    }
  };
  if (is_dynamic && !dynamic_no_padding) {
    need_mask = true;
  }
  if (!is_dynamic) {
    is_shapesize_need_mask(
        input_blocking_dims[inp_a_axis], input_blocking_dims[inp_b_axis],
        output_blocking_dims[out_a_axis], output_blocking_dims[out_b_axis]);
    // tensor slice check
  ...
  if (!is_dynamic && math_utils::get_dims_product(input_blocking_dims) !=
  ...
  auto make_rows_var = [&](std::vector<expr> &rows,
    ...
    for (auto i = 0; i < var_len; i++) {
      rows[i] = builder::make_var(dtype, "row" + std::to_string(i + 1) +
  ...
  auto determine_cur_step = [&](const std::vector<expr> &blocking_dims_expr,
                                ...
                                int in_axis, int in_baxis, bool use_output_loop,
                                int step, sc_data_format_kind_t &format_code,
    ...
    auto slice_condition = [&]() {
      cur_step = builder::make_min(
          builder::make_max(cast_to_s32(blocking_dims_expr[in_baxis]) -
                            ...
                            0),
          step);
      sup_condition = tmp_in_indexes[in_axis] >= blocking_dims_expr[in_axis];
    };
    auto plain_condition = [&]() {
      ...
      cur_step = builder::make_min(
          builder::make_max(cast_to_s32(tmp_plain[input_last_dim]) -
                                cast_to_s32(plain_indexes[input_last_dim]),
                            0),
          step);
      sup_condition =
          plain_indexes[input_other_dim] >= tmp_plain[input_other_dim];
    };
  ...
  auto src_opt_mask = [&](expr &cur_step, expr &sup_condition, const int step) {
    auto src_shape = output_loop ? input_blocking_dims_expr : src.get_shape();
    auto src_mask_can_empty = [&](const int src_mask_axis,
      ...
      bool is_srcshape_multiple_step =
          (get_expr_as_int(src_shape[src_mask_axis]) % mask_step) == 0;
      bool is_dstshape_multiple_step =
          (output_loop &&
           (get_expr_as_int(dst.get_shape()[dst_mask_axis]) % mask_step) == 0 &&
           !is_padding);
  ...
  auto dst_opt_mask = [&](expr &cur_step, expr &sup_condition,
                          const std::vector<expr> &in_indexes, const int i,
                          const int step) {
    auto dst_shape = output_loop ? dst.get_shape() : output_blocking_dims_expr;
    auto dst_mask_can_empty = [&](const int dst_mask_axis,
      ...
      if (is_dynamic)
        return false;
      bool is_dstshape_multiple_step =
          (get_expr_as_int(dst_shape[dst_mask_axis]) % mask_step) == 0;
      bool is_srcshape_multiple_step =
          (!output_loop &&
           (get_expr_as_int(src.get_shape()[src_mask_axis]) % mask_step) == 0 &&
           !is_padding);
  ...
  auto inputloop_split_ir_directly = [&](const std::vector<expr> &in_indexes,
    ...
    for (int i = 0; i < (direct_use_bit16x16 ? lanesx16 : step); i++) {
      ...
      if (need_mask) {
        ...
        determine_cur_step(
            ...
            tmp_in_indexes, tmp_plain_indexes, cur_step, sup_condition, in_axis,
            inp_b_axis, output_loop, step, input_format_code);
        src_opt_mask(cur_step, sup_condition, step);
        if (i == 0 && (lastdim_bound != 0)) {
          inputloop_lastdim_condition =
              cast_to_s32(src.get_shape()[inp_b_axis]) - cast_to_s32(step) -
        ...
        if (inputloop_otheraxis_condition.defined()) {
          mask_floor = generate_mask_var_by_step(mask_def_floor, cur_step, step,
                                                 expr(), true);
      ...
      auto assign = builder::make_assign_unattached(
          rows[i], builder::make_indexing(tmp_in, tmp_in_indexes, step, mask));
      ...
      if (inputloop_otheraxis_condition.defined()) {
        auto assign_floor = builder::make_assign_unattached(
            rows[i],
            builder::make_indexing(tmp_in, tmp_in_indexes, step, mask_floor));
        assign_floor
            ->attr()[op_traits::workload_computable_t::workload_number] = wkld;
      ...
      if (inputloop_lastdim_condition.defined()) {
        ...
        assign_floor_lastdim = builder::make_assign_unattached(
            rows[i], builder::make_indexing(tmp_in, tmp_in_indexes, step));
        assign_floor_lastdim
            ->attr()[op_traits::workload_computable_t::workload_number] = wkld;
    ...
    if (inputloop_otheraxis_condition.defined() ||
        inputloop_lastdim_condition.defined()) {
      cur_list_floor.insert(cur_list_floor.end(), cur_list.begin(),
                            cur_list.end());
      cur_list_lastdim_floor.insert(cur_list_lastdim_floor.end(),
                                    cur_list.begin(), cur_list.end());
      tmp_cur_list.insert(tmp_cur_list.end(), cur_list.begin(), cur_list.end());
      cur_list.clear();
    ...
    for (int i = 0; i < (direct_use_bit16x16 ? lanesx16 : step); i++) {
      ...
      tmp_out_indexes_slice[out_axis] =
          tmp_out_indexes_slice[out_axis] + static_cast<uint64_t>(i);
      if (need_mask) {
        expr cur_step, sup_condition;
        determine_cur_step(
            output_loop ? dst.get_shape() : output_blocking_dims_expr,
            tmp_out_indexes_slice, plain_indexes, cur_step, sup_condition,
            out_axis, out_a_axis, output_loop, step, input_format_code, true);
        ...
        if (inputloop_lastdim_condition.defined()) {
          ...
          mask_floor_lastdim =
              generate_mask_var_by_step(mask_def_floor_lastdim, expr(step),
                                        step, sup_condition_lastdim, true);
          cur_list_lastdim_floor.emplace_back(mask_def_floor_lastdim);
        }
      }
      stmt assign;
      assign = builder::make_assign_unattached(
          builder::make_indexing(output, tmp_out_indexes, step, mask), rows[i]);

      cur_list.emplace_back(assign);
      if (inputloop_otheraxis_condition.defined()) {
        auto assign_floor = builder::make_assign_unattached(
            builder::make_indexing(output, tmp_out_indexes, step, mask_floor),
            rows[i]);
        cur_list_floor.emplace_back(assign_floor);
      }
      if (inputloop_lastdim_condition.defined()) {
  ...
  auto outputloop_split_ir_directly = [&](const std::vector<expr> &in_indexes,
    ...
    for (int i = 0; i < (direct_use_bit16x16 ? lanesx16 : step); i++) {
      ...
      auto assign_floor = builder::make_assign_unattached(
          rows[i],
          builder::make_indexing(tmp_in, tmp_in_indexes, step, mask_floor));
      assign_floor->attr()[op_traits::workload_computable_t::workload_number] =
          wkld;
      cur_list_floor.emplace_back(assign_floor);
    }
    auto tmp_cur_list = cur_list;
  ...
  auto inputloop_split_ir_bit16 = [&](const std::vector<expr> &in_indexes,
    ...
    for (int i = 0; i < step; i++) {
      for (int p = 0; p < 4; p++) {
        ...
        auto in_axis = inp_a_axis;
        tmp_in_indexes[in_axis] =
            tmp_in_indexes[in_axis] + (static_cast<uint64_t>(i) + p * 8);
        auto tmp_plain_indexes = plain_indexes;
        tmp_plain_indexes[input_format_code.get(in_axis)] =
            tmp_plain_indexes[input_format_code.get(in_axis)] +
        ...
        if (need_mask) {
          ...
          mask = generate_mask_var_by_step(mask_def, cur_step, trans_lanesx8,
                                           sup_condition, true);
          cur_list.emplace_back(mask_def);
          if (inputloop_otheraxis_condition.defined()) {
            mask_floor = generate_mask_var_by_step(mask_def_tail, cur_step,
                                                   step, expr(), true);
        ...
        auto assign = builder::make_assign_unattached(
            rows[i],
            p > 0 ? builder::make_select(0xff << (p * step), brct_src, rows[i])
                  : brct_src);
        ...
        if (inputloop_otheraxis_condition.defined()) {
          auto brct_src_floor = builder::make_broadcast(
              ...
              trans_lanes_16bitx8);
          auto assign_floor = builder::make_assign_unattached(
              rows[i], p > 0 ? builder::make_select(0xff << (p * step),
                                                    brct_src_floor, rows[i])
                             : brct_src_floor);
          assign_floor
    ...
    for (int i = 0; i < step; i++) {
      ...
      if (need_mask) {
        ...
        mask = generate_mask_var_by_step(
            mask_def, cur_step, trans_lanes_16bitx8, sup_condition, true);
        cur_list.emplace_back(mask_def);
        if (inputloop_otheraxis_condition.defined()) {
          mask_floor = generate_mask_var_by_step(mask_def_floor, cur_step,
                                                 trans_lanes_16bitx8,
                                                 sup_condition, true);
          cur_list_floor.emplace_back(mask_def_floor);
        }
      ...
      auto assign = builder::make_assign_unattached(
          builder::make_indexing(output, tmp_out_indexes, trans_lanes_16bitx8,
                                 mask),
          rows[i + 8]);
      cur_list.emplace_back(assign);
      if (inputloop_otheraxis_condition.defined()) {
  ...
  auto compute_transpose_direct =
      [&](const std::vector<expr> &in_indexes,
          const std::vector<expr> &out_indexes,
          const std::vector<expr> &plain_indexes,
          const std::vector<expr> &out_indexes_slice) {
        expr mask;
        for (int i = 0; i < (direct_use_bit16x16 ? lanesx16 : step); i++) {
          auto tmp_in_indexes = in_indexes;
          auto in_axis = inp_a_axis;
          ...
          if (need_mask) {
            ...
            determine_cur_step(
                output_loop ? input_blocking_dims_expr : src.get_shape(),
                tmp_in_indexes, tmp_plain_indexes, cur_step, sup_condition,
                in_axis, inp_b_axis, output_loop, step, input_format_code);
            src_opt_mask(cur_step, sup_condition, step);
            stmt mask_def;
            mask = generate_mask_var_by_step(mask_def, cur_step, step,
                                             sup_condition, true);
          ...
          for (int p = 0; p < 4; p++) {
            ...
            if (need_mask) {
              ...
              determine_cur_step(output_loop ? input_blocking_dims_expr
                                             : src.get_shape(),
                                 tmp_in_indexes, tmp_plain_indexes, cur_step,
                                 sup_condition, in_axis, inp_b_axis,
                                 output_loop, trans_lanesx8, input_format_code);
              src_opt_mask(cur_step, sup_condition, trans_lanesx8);
              stmt mask_def;
              mask = generate_mask_var_by_step(
                  mask_def, cur_step, trans_lanesx8, sup_condition, true);
              cur_list.emplace_back(mask_def);
            }
            auto brct_src = builder::make_broadcast(
                builder::make_indexing(tmp_in, tmp_in_indexes, step, mask),
            ...
            auto assign = builder::make_assign_unattached(
                ...
                // tensor_shrink
                p > 0 ? builder::make_select(0xff << (p * step), brct_src,
                                             rows[i])
                      : brct_src);
            assign->attr()[op_traits::workload_computable_t::workload_number] =
                wkld;
  ...
  auto compute_loops = [&](const std::vector<expr> &blocking_dims,
    ...
    for (int i = static_cast<int>(blocking_dims.size()) - 1; i >= 0; i--) {
      if (utils::is_one_of(i, a_axis, b_axis) && iter_vars.at(i).isa<var>()) {
        body = cur.isa<stmts>()
                   ? cur
        ...
        cur = make_stmt<for_loop_node_t>(
            std::move(iter_vars.at(i)), expr(0), (tsr.get_shape()[i]),
            expr(cur_step), std::move(body), true, for_type::NORMAL);
    ...
    for (int i = static_cast<int>(blocking_dims.size()) - 1; i >= 0; i--) {
      if (!utils::is_one_of(i, a_axis, b_axis) && iter_vars.at(i).isa<var>()) {
        body = cur.isa<stmts>()
                   ...
                   : make_stmt<stmts_node_t>(std::vector<stmt>{std::move(cur)});
        cur = make_stmt<for_loop_node_t>(
            std::move(iter_vars.at(i)), expr(0), tsr.get_shape()[i], expr(1),
  ...
  if (!output_loop) {
    ...
    std::vector<expr> tmp_indexes = get_reorder_block2plain_indexes(
        ...
        input_format.format_code_.get(
            static_cast<int>(input_format.format_code_.ndims()) - 1));
    ...
    if (dtype == datatypes::f32 || is_s8u8 || direct_use_bit16x16) {
      if (use_split_loop && !is_dynamic) {
        inputloop_split_ir_directly(loop_indexes, out_indexes, tmp_indexes,
                                    out_indexes);
      } else {
        compute_transpose_direct(loop_indexes, out_indexes, tmp_indexes,
                                 out_indexes);
      }
    } else {
      if (use_split_loop && !is_dynamic) {
        inputloop_split_ir_bit16(loop_indexes, out_indexes, tmp_indexes,
                                 out_indexes);
      } else {
    ...
    if (use_split_loop && !is_dynamic) {
      if (inputloop_otheraxis_condition.defined() &&
          inputloop_lastdim_condition.defined()) {
        auto cur_floor_lastdim =
            builder::make_stmts_unattached(cur_list_lastdim_floor);
        ...
        auto other_axis_meet = builder::make_if_else_unattached(
            inputloop_lastdim_condition, cur_floor, cur_floor_lastdim);
        cur = builder::make_if_else_unattached(inputloop_otheraxis_condition,
                                               cur, other_axis_meet);
      } else if (inputloop_lastdim_condition.defined()) {
        auto cur_floor_lastdim =
            builder::make_stmts_unattached(cur_list_lastdim_floor);
        cur = builder::make_if_else_unattached(inputloop_lastdim_condition, cur,
                                               cur_floor_lastdim);
    ...
    if (use_direct) {
      if (use_split_loop && !is_dynamic) {
        outputloop_split_ir_directly(in_indexes, out_indexes, tmp_indexes,
                                     out_indexes_slice);
      } else {
        compute_transpose_direct(in_indexes, out_indexes, tmp_indexes,
                                 out_indexes);
      }
    } else {
    ...
    if (use_split_loop && !is_dynamic) {
      if (outputloop_axis_condition.defined() && use_direct) {
        auto cur_floor = builder::make_stmts_unattached(cur_list_floor);
        cur = builder::make_if_else_unattached(outputloop_axis_condition, cur,
                                               cur_floor);
      }
      var_define_list.emplace_back(cur);
      cur = builder::make_stmts_unattached(var_define_list);
