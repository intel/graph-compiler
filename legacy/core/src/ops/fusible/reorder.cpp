...
ir_module_ptr inplaced_reorder_get_func(sc_op *op, const context_ptr &ctx) {
  ...
  bld.push_scope();
  bld.push_evaluate(builder::make_write_struct(
      real_outs[0],
...
void reorder_op_t::query_format(
    context_ptr ctx,
  ...
  if (graph.is_dynamic() && !graph.attrs_.get_or_else("insert_reorder", true)) {
...
reorder_op_t::reorder_op_t(graph_tensor_ptr v, sc_data_format_t input_format,
    ...
    : reorder_op_t({std::move(v)}, {},
                   any_map_t{{"out_format", output_format}}) {}
...
slice_range get_block2plain_ranges(const expr &block_num_start,
                                   const expr &block_num_length,
                                   const expr &block_size_start,
  ...
  int block_size_length_int =
      get_const_as_int(folded_block_size_length.checked_as<constant_c>());
  ...
  if (block_size_length_int == blocks) {
    ...
    if (!block_num_length.isa<constant>()) {
    ...
    for (int i = 0; i < block_num_length_int; i++) {
      auto plain_range =
          std::make_pair(do_cast_and_fold((block_num_start + expr(i)) * blocks +
                                          block_size_start),
                         block_size_length);
...
get_plain2block_ranges(const expr &start, const expr &length, int blocks) {
  ...
  if (folded_start.isa<constant>() && get_expr_as_int(folded_start) == 0) {
    if (folded_length.isa<constant>()) {
      ...
      std::pair<expr, expr> block_num_range = std::make_pair(
          0, folder(caster(divide_and_ceil(folded_length, blocks)))
                 .remove_const());
    ...
    if (folded_start->node_type_ == sc_expr_type::mul) {
      auto r = constant_folding::get_operand_from_binary(folded_start).second;
      if (r.isa<constant>()) {
        auto multiple = get_expr_as_int(r);
        if (multiple % blocks == 0) {
          if (ilength >= blocks) {
            auto block_num_range =
                std::make_pair(folded_start / blocks, ilength / blocks);
            auto block_size_range = std::make_pair(0, blocks);
            ret.emplace_back(std::make_pair(std::move(block_num_range),
                                            std::move(block_size_range)));
          }
          if (ilength % blocks != 0) {
            auto block_num_range =
                std::make_pair(folded_start / blocks + ilength / blocks, 1);
            auto block_size_range = std::make_pair(0, ilength % blocks);
            ret.emplace_back(std::make_pair(std::move(block_num_range),
                                            std::move(block_size_range)));
          }
          ...
          for (int i = 0; i < ilength / gcd; i++) {
            auto block_num_range =
                std::make_pair((folded_start + i * gcd) / blocks, 1);
            auto block_size_range =
                std::make_pair((folded_start + i * gcd) % blocks, gcd);
            ret.emplace_back(std::make_pair(std::move(block_num_range),
                                            std::move(block_size_range)));
          }
          if (ilength % gcd != 0) {
            auto block_num_range =
                std::make_pair((folded_start + ilength / gcd) / blocks, 1);
            auto block_size_range = std::make_pair(
                (folded_start + ilength / gcd) % blocks, ilength % gcd);
            ret.emplace_back(std::make_pair(std::move(block_num_range),
                                            std::move(block_size_range)));
          }
        }
  ...
  if (ret.empty()) {
    ...
    for (int i = 0; i < ilength; i++) {
      auto block_num_range = std::make_pair((folded_start + i) / blocks, 1);
      auto block_size_range = std::make_pair((folded_start + i) % blocks, 1);
      ret.emplace_back(std::make_pair(std::move(block_num_range),
                                      std::move(block_size_range)));
    }
  }
...
void infer_stride2stride_reorder(slice_range_list &input_slice_list,
  ...
  for (auto &input_slice : input_slice_list) {
    ...
    infer_stride2plain_reorder(input_slice, input_format, output_format,
                               plain_slice);
    infer_plain2stride_reorder(plain_slice, input_format, output_format,
                               reorder_ranges);
 ...
 * */
void dispatch_reorder_ranges(
    slice_range_list &total_ranges_list, slice_range_list &reorder_ranges_list,
  ...
  for (int i = 0; i < total_size; i++) {
    ...
    for (size_t j = 0; j < acc_size_list.size(); j++) {
      ...
      if (!output_format.is_any() &&
          ...
          index_list[output_format.format_code_.get(j)] != index_list[j]) {
        valid = false;
        break;
      }
      rmd = rmd % size;
...
void infer_stride2block_reorder(slice_range_list &input_slice_list,
  ...
  for (auto &input_slice : input_slice_list) {
    ...
    for (int i = 0; i < out_kind.ndims(); i++) {
      ...
      if (block_slice_dict[plain_pos].empty()) {
        ...
        for (auto &block_range_list : block_slice_dict[plain_pos]) {
          ...
          auto cur_block =
              output_format.blocks_[out_kind.collect_blocking_index(plain_pos)
                                        .at(block_cnt_dict[plain_pos] // NOLINT
                                                .size() -
                                            2)];
          auto cur_block_ranges = get_plain2block_ranges(
              cur_plain_range.first, cur_plain_range.second, cur_block);
    ...
    output_slice_list.insert(output_slice_list.end(),
                             reorder_ranges_list.begin(),
                             reorder_ranges_list.end());
  }
}
  ...
  for (auto &input_slice : input_slice_list) {
    ...
    for (int i = in_kind.ndims() - 1; i >= 0; i--) {
      ...
      if (plain_slice_dict[plain_pos].empty()) {
        plain_slice_dict[plain_pos].emplace_back(slice_range{input_slice[i]});
      } else {
        std::pair<expr, expr> cur_block_num_range = input_slice[i];
        slice_range res;
        for (auto &cur_block_size_range : plain_slice_dict[plain_pos].back()) {
          auto cur_blocks = in_kind.collect_blocking_index(plain_pos);
          int cur_block = input_format.blocks_[cur_blocks.at(
              cur_blocks.size() - plain_slice_dict[plain_pos].size())];
          slice_range cur_plain_ranges_list = get_block2plain_ranges(
              cur_block_num_range.first, cur_block_num_range.second,
              cur_block_size_range.first, cur_block_size_range.second,
              cur_block);
    ...
    for (auto &range : reorder_ranges_list) {
      ...
      infer_plain2stride_reorder(range, output_format.to_plain(), output_format,
                                 stride_range);
...
void infer_reorder_slice(slice_range_list &input_slice_list,
  ...
  if (is_not_blocking(input_format) && is_not_blocking(output_format)) {
    infer_stride2stride_reorder(input_slice_list, input_format, output_format,
                                output_slice_list);
  } else if (is_not_blocking(input_format) && output_format.is_blocking()) {
    infer_stride2block_reorder(input_slice_list, input_format, output_format,
                               output_slice_list);
  } else if (input_format.is_blocking() && is_not_blocking(output_format)) {
    infer_block2stride_reorder(input_slice_list, input_format, output_format,
                               output_slice_list);
...
void find_vectorized_axis(std::vector<expr> const &blocking_dims_expr,
  ...
  for (int i = origin_axis_vectorized; i >= 0; i--) {
    if (!blocking_dims_expr[i].isa<constant>()) {
      break;
    }
...
void find_vectorized_axis(const tensor_slice &tsl,
  ...
  // original logic
  for (int i = origin_axis_vectorized; i >= 0; i--) {
    if (!tsl.get_shape()[i].isa<constant>()) {
      break;
    }
 ...
 * */
int collect_axis_shape_size(sc_dims &blocking_dims,
...
#define SLICE_RAGNE_CHECK_INIT_DATA()                                          \
  ...
  auto dtype = info_.inputs_[0]->details_.dtype_;                              \
  auto blocking_exprs =                                                        \
      get_blocking_shapes_expr(get_owner_graph(), plain_dims_, target_format); \
  auto block_axis =                                                            \
      target_format.format_code_.get(target_format.format_code_.ndims() - 1);  \
  int origin_axis_vectorized = target_format.format_code_.ndims() - 1;         \
  auto toy_inp =                                                               \
      builder::make_tensor(std::string("dummy_inp"), blocking_exprs, dtype);   \
  auto inp_slice = tensor_slice(toy_inp);                                      \
  find_vectorized_axis(inp_slice, target_format, block_axis,                   \
  ...
  bool special_slice_check = (support_optimized_kernel(ctx) || must_recheck);  \
  len_from_last +=                                                             \
      optimized_slice_check ? meet_vnni_reorder_require(ctx) ? 3 : 2 : 1;

// infer reorder slice according input_slice
infer_status_code reorder_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                   fslice_map &fsmap) {
  ...
  for (auto &src_range : input_slice_list) {
    if (!get_inputs()[0]->details_.is_dynamic() &&
        !slice_divisible_on_axis(get_inputs()[0]->details_.get_blocking_dims(),
  ...
  if (special_slice_check &&
      !check_required_slice(get_inputs()[0], input_slice_list, len_from_last)) {
    return infer_status_code::RETRY;
  }
  ...
  if (!check_padding() || is_dynamic()) {
    ...
    infer_padding_reorder_slice(
        input_slice_list, get_inputs()[0]->details_.get_blocking_dims(),
        get_outputs()[0]->details_.get_blocking_dims(), reorder_ranges_list);
  }
...
infer_status_code reorder_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                       fslice_map &fsmap) {
  ...
  if (fsmap.datamap_.size() == 1) {
    if (!use_output_loop() ||
        (special_slice_check &&
         !attrs_.get_or_else(mixed_partition_hint::pre_fuse_begin_op, false) &&
...
std::vector<expr> get_reorder_stride2stride_indexes(
  ...
  COMPILE_ASSERT(
      in_format.format_code_ != format_kinds::any &&
...
std::vector<expr> get_reorder_block2plain_indexes(
    sc_graph_t &graph, const std::vector<expr> &in_indexes,
    const sc_data_format_t &format, const sc_dims &plain_dims, expr &condition,
    expr &last_axis_offset, expr &other_axis_condition, const int target_axis) {
  if (in_indexes.empty()) {
    return std::vector<expr>();
  }
  COMPILE_ASSERT(
      format.format_code_ != format_kinds::any,
  ...
  if (target_axis != TARGET_AXIS_NOT_DEFINE && last_orig_axis != target_axis) {
  ...
  for (int inp_idx = static_cast<int>(num_format_dims + base_out_dim) - 1;
    ...
    if (axis2blocks[orig_axis] == static_cast<int>(blocks.size())) {
      ...
      ret[base_out_dim + orig_axis] =
          ret[base_out_dim + orig_axis] +
          in_indexes[inp_idx] * format.blocks_[blocks[axis2blocks[orig_axis]]];
      if (axis2blocks[orig_axis] == 0) {
        auto new_condition =
            ret[base_out_dim + orig_axis] <
            graph.dim_to_expr(plain_dims[base_out_dim + orig_axis]);
        ...
        if (orig_axis == last_orig_axis) {
          ...
          last_axis_offset =
              last_axis_offset.defined()
                  ? builder::make_min(last_axis_offset_tmp, last_axis_offset)
                  : last_axis_offset_tmp;
        } else {
          other_axis_condition = other_axis_condition && new_condition;
        }
      } else {
        ...
        if (orig_axis == last_orig_axis) {
          last_axis_offset =
              cast_to_s32(format.blocks_[blocks[axis2blocks[orig_axis] - 1]]) -
              cast_to_s32(ret[base_out_dim + orig_axis]);
        } else {
          other_axis_condition = other_axis_condition && new_condition;
        }
...
std::vector<expr>
get_reorder_plain2block_indexes(const std::vector<expr> &in_indexes,
...
static bool check_u8s8(sc_data_type_t dtype) {
  return dtype.is_etype(sc_data_etype::S8) || dtype.is_etype(sc_data_etype::U8);
}
void compute_reorder_stride2stride(
    sc_graph_t &graph, const context_ptr &ctx, const tensor_slice &src,
  ...
  auto output_last_origin_axis =
      output_format.format_code_.get(output_format.format_code_.ndims() - 1);
  auto input_last_origin_axis =
      input_format.format_code_.get(input_format.format_code_.ndims() - 1);
  ...
  const int max_step = static_cast<int>(vectorize_step(ctx, dtype.type_code_));
  if ((!output_loop &&
       src_slice_shape[input_origin_axis_vectorized].isa<constant>()) ||
      (output_loop &&
       dst_slice_shape[output_origin_axis_vectorized].isa<constant>())) {
  ...
  bool can_vectorize =
      ...
      step * utils::get_sizeof_type(dtype) * byte >=
          (uint64_t)(is_u8s8 ? u8s8_min_simd_length : avx_simd_length);
  if (!can_vectorize) {
  ...
  if (!output_loop) {
    no_padding |=
        !is_dynamic &&
        get_expr_as_int(src_slice_shape[input_origin_axis_vectorized]) % step ==
            0;
    ...
    out_indexes = get_reorder_stride2stride_indexes(in_indexes, input_format,
                                                    output_format, plain_dims);
    ...
    if (!no_padding && can_vectorize) {
      ...
      auto cur_step = builder::make_min(
          builder::make_max(builder::make_constant(0), idx_len), step);
    ...
    for (int i = static_cast<int>(plain_dims.size()) - 1; i >= 0; i--) {
      ...
      cur = make_stmt<for_loop_node_t>(
          std::move(iter_vars.at(i)), expr(0), src.get_shape()[i],
          can_vectorize && i == input_origin_axis_vectorized ? expr(step)
                                                             : expr(1),
    ...
    in_indexes = get_reorder_stride2stride_indexes(out_indexes, output_format,
                                                   input_format, plain_dims);
    auto cur = builder::make_stmts_unattached({});
    expr mask;
    stmt mask_def;
    if (!no_padding && can_vectorize) {
      auto idx_len =
          cast_to_s32(dst_slice_shape[output_origin_axis_vectorized]) -
    ...
    for (int i = static_cast<int>(plain_dims.size()) - 1; i >= 0; i--) {
      body = cur.isa<stmts>()
                 ...
                 : make_stmt<stmts_node_t>(std::vector<stmt>{std::move(cur)});
      cur = make_stmt<for_loop_node_t>(
          std::move(iter_vars.at(i)), expr(0), dst.get_shape()[i],
          can_vectorize && i == output_origin_axis_vectorized ? expr(step)
                                                              : expr(1),
          std::move(body), true, for_type::NORMAL);
      bind_loop_axis(expand_gt, cur, i);
      loops.push_back(cur);
...
void compute_reorder_block2stride(
    sc_graph_t &graph, const context_ptr &ctx, const tensor_slice &src,
    tensor_slice &dst, const sc_data_format_t &input_format,
    const sc_data_format_t &output_format, sc_data_type_t dtype,
  ...
  int step =
      std::min(max_step, static_cast<int>(get_expr_as_int(
                             src_slice_shape[input_origin_axis_vectorized])));
  step = utils::get_nearest_vector_step(step);
  ...
  bool no_padding =
      !is_dynamic && sc_data_format_t::get_padded_plain_shapes(
  ...
  for (size_t i = 0; i < input_blocking_dims.size(); i++) {
    iter_vars.emplace_back(
        range_from_outer_loop(src.get_ranges()[i])
            ? expr(0)
            : builder::make_var(datatypes::index, std::string("_fuseiter") +
                                                      fusion_create_idx()));
    in_indexes.emplace_back(iter_vars[i] + src.get_offset()[i]);
    loop_indexes.emplace_back(iter_vars[i]);
  }
  ...
  std::vector<expr> tmp_out_indexes = get_reorder_block2plain_indexes(
      graph, in_indexes, input_format, plain_dims, condition, last_axis_offset,
      other_axis_condition, input_origin_axis_vectorized);
  std::vector<expr> out_indexes = get_reorder_stride2stride_indexes(
      tmp_out_indexes, output_format.to_plain(), output_format, plain_dims);
  ...
  if (!no_padding && can_vectorize) {
    ...
    auto cur_step = builder::make_min(
        builder::make_max(builder::make_constant(0), last_axis_offset), step);
  ...
  stmt body;
  for (int i = static_cast<int>(input_blocking_dims.size()) - 1; i >= 0; i--) {
...
void compute_reorder_stride2block(
    sc_graph_t &graph, const context_ptr &ctx, const tensor_slice &src,
    tensor_slice &dst, const sc_data_format_t &input_format,
    const sc_data_format_t &output_format, sc_data_type_t dtype,
    const sc_dims &plain_dims, bool output_loop, any_map_t &attrs,
  ...
  bool can_vectorize =
      !is_innermost_dim_strided &&
      input_last_origin_axis == output_last_origin_axis &&
      get_expr_as_int(dst_slice_shape[output_origin_axis_vectorized]) % step ==
          0 &&
      is_valid_step(step) &&
      step * utils::get_sizeof_type(dtype) * byte >=
          (uint64_t)(is_u8s8 ? u8s8_min_simd_length : avx_simd_length);
  ...
  if (!output_loop &&
      !is_dynamic_dim(input_blocking_dims[input_origin_axis_vectorized]) &&
      get_expr_as_int(src_slice_shape[input_origin_axis_vectorized]) % step !=
          0) {
    can_vectorize = false;
  }
  ...
  if (!output_loop) {
    ...
    std::vector<expr> out_indexes =
        get_reorder_plain2block_indexes(tmp_out_indexes, output_format);
    ...
    auto assign =
        builder::make_stmts_unattached({builder::make_assign_unattached(
            ...
            // illegal exception in index_flatten
            builder::make_indexing(src.tptr_, loop_indexes, step))});
    assign->attr()[op_traits::workload_computable_t::workload_number] = wkld;
    auto cur = assign;
    stmt body;
    ...
    for (size_t i = 0; i < output_blocking_dims.size(); i++) {
      iter_vars.emplace_back(
          range_from_outer_loop(dst.get_ranges()[i])
              ? expr(0)
              : builder::make_var(datatypes::index, std::string("_fuseiter") +
                                                        fusion_create_idx()));
      out_indexes.emplace_back(iter_vars[i] + dst.get_offset()[i]);
    ...
    std::vector<expr> tmp_in_indexes = get_reorder_block2plain_indexes(
        ...
        last_axis_offset, other_axis_condition, output_origin_axis_vectorized);
    std::vector<expr> in_indexes = get_reorder_stride2stride_indexes(
        tmp_in_indexes, input_format.to_plain(), input_format, plain_dims);
    expr mask;
    stmt mask_def;
    if (!no_padding && can_vectorize) {
      // mask = min(max(0, last_dim_len - last_dim_idx),step)
...
void compute_reorder_block2block(
    sc_graph_t &graph, const context_ptr &ctx, const tensor_slice &src,
    tensor_slice &dst, const sc_data_format_t &input_format,
    const sc_data_format_t &output_format, sc_data_type_t dtype,
  ...
  if (plain_dims.empty()) {
    ...
    for (int64_t i = 0; i < dst.nslice_dims(); i++) {
      dst_blocking_dims.push_back(
          get_const_as_int(dst.get_shape()[i].checked_as<constant>()));
    }
    plain_dims = sc_data_format_t::get_padded_plain_shapes(dst_blocking_dims,
                                                           output_format);
  }
  auto input_blocking_dims =
      sc_data_format_t::get_blocking_shapes(plain_dims, input_format);
  ...
  auto input_block_axis =
      input_format.format_code_.get(input_format.format_code_.ndims() - 1);
  auto output_block_axis =
      output_format.format_code_.get(output_format.format_code_.ndims() - 1);
  int input_origin_axis_vectorized = input_format.format_code_.ndims() - 1;
  int output_origin_axis_vectorized = output_format.format_code_.ndims() - 1;
  find_vectorized_axis(input_blocking_exprs, input_format, input_block_axis,
                       input_origin_axis_vectorized);
  find_vectorized_axis(output_blocking_exprs, output_format, output_block_axis,
                       output_origin_axis_vectorized);
  auto src_slice_shape = src.get_shape();
  auto dst_slice_shape = dst.get_shape();
  bool no_padding =
      !is_dynamic && input_padded_plain_dims == output_padded_plain_dims;
  no_padding |= (is_dynamic && dynamic_no_padding);
  int step = std::min(
  ...
  bool is_u8s8 = check_u8s8(dtype);
  bool can_vectorize =
      !is_innermost_dim_strided && input_block_axis == output_block_axis &&
      get_expr_as_int(dst_slice_shape[output_origin_axis_vectorized]) % step ==
          0 &&
      get_expr_as_int(src_slice_shape[input_origin_axis_vectorized]) % step ==
          0 &&
      ...
      plain_dims[output_block_axis] %
          ...
          0 &&
      is_valid_step(step) &&
      step * utils::get_sizeof_type(dtype) * 8 >=
          (uint64_t)(is_u8s8 ? u8s8_min_simd_length : avx_simd_length);
  ...
  if (!output_loop) {
    ...
    std::vector<expr> tmp_indexes = get_reorder_block2plain_indexes(
        ...
        last_axis_offset, other_axis_condition, input_origin_axis_vectorized);
    std::vector<expr> out_indexes =
    ...
    auto assign =
        builder::make_stmts_unattached({builder::make_assign_unattached(
            ...
            builder::make_indexing(src.tptr_, loop_indexes, step))});
    assign->attr()[op_traits::workload_computable_t::workload_number] = wkld;
    auto cur = no_padding ? assign
                          : builder::make_if_else_unattached(condition, assign,
                                                             stmt());
    stmt body;
    for (int i = static_cast<int>(input_blocking_dims.size()) - 1; i >= 0;
         i--) {
...
void compute_reorder_block(sc_graph_t &graph, const context_ptr &ctx,
                           ...
                           bool output_loop, any_map_t &attrs,
                           const graph_tensor_ptr &expand_gt, size_t wkld = 0UL,
  ...
  if (!is_innermost_dim_strided &&
      can_be_vnni_reorder(ctx, inp_a_axis, inp_b_axis, out_a_axis, out_b_axis,
                          plain_dims, input_format, output_format, src, dst,
                          dtype, is_vnni_reorder, is_dynamic,
                          dynamic_no_padding, vnni_kernel_used)) {
    compute_vnni_reorder(graph, ctx, src, dst, input_format, output_format,
                         dtype, plain_dims, output_loop, attrs, inp_a_axis,
                         inp_b_axis, out_a_axis, out_b_axis, expand_gt, wkld,
                         is_vnni_reorder, is_dynamic, dynamic_no_padding,
                         vnni_kernel_used);
  } else if (!is_innermost_dim_strided &&
             can_be_fast_transpose(
                 graph, ctx, inp_a_axis, inp_b_axis, out_a_axis, out_b_axis,
                 plain_dims, input_format, output_format, src, dst, dtype,
                 is_dynamic, dynamic_no_padding, trans_kernel_used)) {
    compute_fast_transpose(graph, ctx, src, dst, input_format, output_format,
                           dtype, plain_dims, output_loop, attrs, inp_a_axis,
                           inp_b_axis, out_a_axis, out_b_axis, expand_gt, wkld,
                           is_dynamic, dynamic_no_padding, trans_kernel_used);
  } else if (is_not_blocking(input_format) && is_not_blocking(output_format)) {
    compute_reorder_stride2stride(
        graph, ctx, src, dst, input_format, output_format, dtype, plain_dims,
        output_loop, attrs, expand_gt, wkld, is_innermost_dim_strided,
        is_dynamic, dynamic_no_padding);
  } else if (is_not_blocking(input_format) && output_format.is_blocking()) {
    compute_reorder_stride2block(
        graph, ctx, src, dst, input_format, output_format, dtype, plain_dims,
        output_loop, attrs, expand_gt, wkld, is_innermost_dim_strided,
        is_dynamic, dynamic_no_padding);
  } else if (input_format.is_blocking() && is_not_blocking(output_format)) {
    compute_reorder_block2stride(graph, ctx, src, dst, input_format,
                                 output_format, dtype, plain_dims, attrs,
                                 expand_gt, wkld, is_innermost_dim_strided,
                                 is_dynamic, dynamic_no_padding);
  } else if (input_format.is_blocking() && output_format.is_blocking()) {
    compute_reorder_block2block(
        graph, ctx, src, dst, input_format, output_format, dtype, plain_dims,
        output_loop, attrs, expand_gt, wkld, is_innermost_dim_strided,
        is_dynamic, dynamic_no_padding);
  } else {
...
size_t
reorder_op_t::compute_workload(const std::vector<shape_dtype_pair> &ins,
...
bool reorder_op_t::check_padding() const {
  ...
  return (!is_dynamic_block && sc_data_format_t::get_padded_plain_shapes(
                                   input_blocking_dims, input_format) !=
                                   sc_data_format_t::get_padded_plain_shapes(
                                       output_blocking_dims, output_format)) ||
         (is_dynamic_block && !(info_.cur_impl_ & impl_kind_t::no_padding));
}

bool reorder_op_t::use_output_loop() const {
  if (attrs_.get_or_else("use_input_loop", false)) {
    return false;
  }
  if (check_padding()) {
    auto &input_format = get_input_format();
    ...
    if (input_format.is_blocking() && is_not_blocking(output_format))
      ...
      // same?
      return (get_dims_product(get_inputs()[0]->details_.get_blocking_dims()) <
              get_dims_product(get_outputs()[0]->details_.get_blocking_dims()));
    }
    return true;
  }
  if (attrs_.get_or_else(op_attr_key::no_fuse, false)) {
    if (!get_input_format().is_blocking())
      return true;
  }
  if (attrs_.get_or_else(op_attr_key::break_pre_fuse, false))
    return true;
  return false;
}
...
#define INIT_REORDER_OP_INFO()                                                 \
  ...
  auto dtype = info_.inputs_[0]->details_.dtype_;                              \
  auto input_blocking_shapes_expr =                                            \
      get_blocking_shapes_expr(get_owner_graph(), plain_dims_, input_format);  \
  auto output_blocking_shapes_expr =                                           \
      get_blocking_shapes_expr(get_owner_graph(), plain_dims_, output_format); \
  sc_graph_t g;                                                                \
  auto toy_inp_tsr = builder::make_tensor(std::string("dummy_inp"),            \
                                          input_blocking_shapes_expr, dtype);  \
  auto toy_out_tsr = builder::make_tensor(std::string("dummy_out"),            \
                                          output_blocking_shapes_expr, dtype); \
...
bool reorder_op_t::support_optimized_kernel(const context_ptr &ctx) const {
  ...
  return (!is_innermost_dim_strided &&
          can_be_fast_transpose(
              get_owner_graph(), ctx, inp_a_axis, inp_b_axis, out_a_axis,
              out_b_axis, plain_dims_, input_format, output_format, src, dst,
              dtype, is_dynamic(), info_.cur_impl_ & impl_kind_t::no_padding,
              trans_kernel_used)) ||
         can_be_vnni_reorder(
             ctx, inp_a_axis, inp_b_axis, out_a_axis, out_b_axis, plain_dims_,
             input_format, output_format, src, dst, dtype, is_vnni_reorder,
             is_dynamic(), info_.cur_impl_ & impl_kind_t::no_padding,
             vnni_kernel_used);
}
...
void reorder_op_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dst,
    const std::vector<const tensor_slice *> &inputs) {
  bool is_innermost_dim_strided =
      !is_dynamic() && (info_.inputs_[0]->details_.get_strides().back() != 1 ||
                        info_.outputs_[0]->details_.get_strides().back() != 1);
  size_t wkld = compute_fusible_workload(ctx, dst, inputs);
  auto &input_format = info_.inputs_[0]->details_.get_format();
  auto &output_format = info_.outputs_[0]->details_.get_format();
  bool output_loop = use_output_loop();
  compute_reorder_block(
      get_owner_graph(), ctx, *inputs[0], *dst[0], input_format, output_format,
      info_.inputs_[0]->details_.dtype_, plain_dims_, output_loop, attrs_,
      output_loop ? get_outputs()[0] : get_inputs()[0], wkld,
      is_innermost_dim_strided, is_dynamic(), info_.cur_impl_);
}

std::vector<int>
reorder_op_t::get_impl_dispatch_candidates(const context_ptr &ctx) {
