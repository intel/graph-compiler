...
binary_elementwise_op_impl_t::get_inplace_map() {
  ...
  for (size_t i = 0; i < inp.size(); i++) {
    if (inp[i]->details_.get_plain_dims() == out_dim) {
      ret.emplace_back(tensor_inplace_info_t{static_cast<int>(i),
...
infer_status_code infer_binary_slice_ranges(fusible_op_t *cur,
                                            fslice_map &fsmap) {
...
static slice_range_list
infer_broadcast_arg_slice(const slice_range_list &known_range_list,
  ...
  for (size_t i = 0; i < bc_arg_range_list.size(); i++) {
    ...
    for (size_t j = 0; j < known_range.size(); j++) {
      if (bc_axis.end() != std::find(bc_axis.begin(), bc_axis.end(), j)) {
        ...
        if (keep_dims) {
          bc_arg_range_list[i].emplace_back(std::make_pair(expr(0), expr(1)));
        }
      }
    }
...
static slice_range_list
infer_broadcast_slice(const slice_range_list &known_range_list,
                      const std::vector<int> &bc_axis,
                      const std::vector<expr> &bc_dim) {
  ...
  for (size_t i = 0; i < bc_range_list.size(); i++) {
    ...
    for (size_t j = 0; j < bc_dim.size(); j++) {
      if (bc_axis.end() != std::find(bc_axis.begin(), bc_axis.end(), j)) {
        ...
        bc_range_list[i].emplace_back(std::make_pair(expr(0), bc_dim[j]));
      }
    }
  }
...
static sc_dims
infer_binary_elementwise_output_shape(const sc_dims &lhs_shape,
                                      const sc_dims &rhs_shape,
                                      const std::vector<int> &input_bc_axis) {
...
static sc_data_type_t infer_output_dtype(sc_data_type_t a, sc_data_type_t b,
                                         bool is_b_scalar) {
  if (is_b_scalar)
    return a;
...
void binary_elementwise_op_impl_t::set_plain_bc_axis() {
  ...
  if (input_bc_axis.empty()) {
    plain_bc_axis_.emplace_back(
        op_traits::may_broadcast_t::get_auto_broadcast_bc_axis(lhs_shape,
                                                               output_shape));
    plain_bc_axis_.emplace_back(
        op_traits::may_broadcast_t::get_auto_broadcast_bc_axis(rhs_shape,
                                                               output_shape));
  } else {
    ...
    plain_bc_axis_[ref_idx] =
        op_traits::may_broadcast_t::get_auto_broadcast_bc_axis(
            info_.inputs_[ref_idx]->details_.get_plain_dims(), output_shape);
...
binary_elementwise_op_impl_t::binary_elementwise_op_impl_t(
  ...
  COMPILE_ASSERT(ins.size() == 2, "Binary elementwise op shall have 2 inputs.");
  ...
  COMPILE_ASSERT(
      auto_broadcast == "numpy" || ins[0]->details_.get_plain_dims() ==
  ...
  if (outs.empty()) {
    ...
    bool is_b_scalar =
        (info_.inputs_[1 - ref_idx]->details_.get_plain_dims() == sc_dims{1});
  ...
  COMPILE_ASSERT(
      gc::graph::check_shape_equal(info_.outputs_[0]->details_.get_plain_dims(),
                                   output_shape),
...
std::vector<int> binary_elementwise_op_impl_t::get_non_broadcast_input_index(
  ...
  const sc_dims &out_dims = infer_binary_elementwise_output_shape(
      lhs_dims, rhs_dims, attrs_.get_or_else("bc_axis", std::vector<int>{}));
  std::vector<int> ret;
...
int binary_elementwise_op_impl_t::get_ref_input_index(
  ...
  auto non_bc_input_indices = get_non_broadcast_input_index(assert_determined);
...
void binary_elementwise_op_impl_t::query_format(
    context_ptr ctx,
  ...
  if (info_.inputs_[0]->details_.get_plain_dims().size() !=
    ...
    if (layout_input_idx) {
      ...
      auto target_format = infer_broadcast_format(info_.inputs_[1]->details_,
                                                  info_.inputs_[0]->details_);
      in_formats.push_back({target_format});
      in_formats.push_back({in1_format});
      out_formats.push_back({in1_format});
    } else {
      // propagate layout from input 1 to 0.
      auto target_format = infer_broadcast_format(info_.inputs_[0]->details_,
                                                  info_.inputs_[1]->details_);
      in_formats.push_back({in0_format});
      in_formats.push_back({target_format});
...
infer_status_code
binary_elementwise_op_impl_t::infer_slice_ranges(const context_ptr &ctx,
                                                 fslice_map &fsmap) {
  ...
  if (known_ranges_map.size() == get_inputs().size()) {
    ...
    if (known_ranges_map[0].size() != known_ranges_map[1].size()) {
      ...
      int erase_input_id =
          known_ranges_map[0].size() < known_ranges_map[1].size() ? 1 : 0;
  ...
  if (known_ranges_map.size() < get_inputs().size()) {
    ...
    if (bc_input_idx >= 0) {
      bool keep_dims =
          get_inputs()[bc_input_idx]->details_.get_blocking_dims().size() ==
          get_inputs()[1 - bc_input_idx]->details_.get_blocking_dims().size();
      ...
      if (unknown_idx != bc_input_idx) {
        slice_range_list bc_range_list = infer_broadcast_slice(
            ...
            get_inputs()[1 - bc_input_idx]->details_.get_blocking_dims_expr(
                get_owner_graph()));
...
infer_status_code
binary_elementwise_op_impl_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                     fslice_map &fsmap) {
  auto &outslice = fsmap.get(get_outputs()[0]);
  if (outslice.empty()) {
    return infer_status_code::RETRY;
  ...
  for (size_t i = 0; i < get_inputs().size(); i++) {
    ...
    if (inpslice.empty()) {
      if (bc_input_idx == static_cast<int>(i)) {
        auto bc_axis = get_bc_axis();
        inpslice = infer_broadcast_arg_slice(
            outslice, bc_axis,
            get_inputs()[bc_input_idx]->details_.get_blocking_dims().size() ==
...
void binary_elementwise_op_impl_t::infer_binding_axis(
  ...
  if (!outaxis.empty()) {
    ...
    if (outaxis == known_axis_map[ref_idx]) {
      return;
    }
  }

  if (known_axis_map.size() < get_inputs().size()) {
    int unknown_idx = known_axis_map.find(0) != known_axis_map.end() ? 1 : 0;
    // check broadcast
    if (bc_input_idx >= 0) {
      bool keep_dims =
          get_inputs()[bc_input_idx]->details_.get_blocking_dims().size() ==
      ...
      if (keep_dims) {
        ...
        if (unknown_idx != bc_input_idx) {
          ...
          std::transform(
              known_axis.begin(), known_axis.end(), unknown_axis.begin(),
              [&bc_axis](const std::vector<int> &bd_ax) {
                std::vector<int> ret(bd_ax.size());
                std::transform(bd_ax.begin(), bd_ax.end(), ret.begin(),
                               [&bc_axis](const int &ax) {
                                 COMPILE_ASSERT(
                                     ax < static_cast<int64_t>(bc_axis.size()),
                                     "Unexpected ax found: " << ax)
                                 return bc_axis[ax];
                               });
                return ret;
              });
          ...
          for (auto &bd_ax : known_axis) {
            std::vector<int> ret;
            for (auto &ax : bd_ax) {
              auto iter = std::find(bc_axis.begin(), bc_axis.end(), ax);
              if (iter != bc_axis.end()) {
...
std::vector<int> binary_elementwise_op_impl_t::get_bc_axis() const {
  ...
  return transform_axis_plain2blocking(info_.inputs_[1 - bc_input_idx],
                                       plain_bc_axis_[bc_input_idx]);
}
...
bool binary_elementwise_op_impl_t::register_brgemm_fusion(
  ...
  return brg_reg.register_op_infos(
      ...
      inputs[1]->get_tensor_ptr(), inputs[1]->get_shape());
}
...
static stmt select_algorithm(elt_operator elt_op, const expr &in0,
                             const expr &in1, const expr &out,
                             const any_map_t &attrs, const sc_op_info_t &info) {
  ...
  auto assign_maker = [&cur_list](const expr &def_var, const expr &def_val) {
    cur_list.emplace_back(builder::make_assign_unattached(def_var, def_val));
  };
  switch (elt_op) {
  case elt_operator::ADD: {
  ...
  case elt_operator::MIN: {
    return builder::make_assign_unattached(out, builder::make_min(in0, in1));
  } break;
  case elt_operator::MAX: {
    return builder::make_assign_unattached(out, builder::make_max(in0, in1));
  } break;
  case elt_operator::SQD_DIFF: {
    return builder::make_assign_unattached(out, (in0 - in1) * (in0 - in1));
  } break;
  case elt_operator::PRELU: {
    return builder::make_assign_unattached(
        out, builder::make_select(
                 in0 >= make_expr<constant_node>((int64_t)0, in0->dtype_), in0,
                 in0 * in1));
  } break;
    ...
    return builder::make_assign_unattached(
        out, builder::make_select(
                 ...
                 builder::make_select(
                     ...
                     builder::make_sub(
                         make_expr<constant_node>(0.f, in0->dtype_), in1),
                     make_expr<constant_node>(0.f, in0->dtype_))));
  } break;
  case elt_operator::CLAMP_BWD: {
    return builder::make_assign_unattached(
        out, builder::make_select(
                 in0 > make_expr<constant_node>((float)attrs.get<float>("min"),
                                                in0->dtype_),
                 builder::make_select(
                     in0 < make_expr<constant_node>(
                               (float)attrs.get<float>("max"), in0->dtype_),
                     in1, make_expr<constant_node>(0.f, in1->dtype_)),
                 make_expr<constant_node>(0.f, in0->dtype_)));
  } break;
  case elt_operator::ELU_BWD: {
    expr used_inp = builder::make_select(
        in0 > make_expr<constant_node>(0.f, in0->dtype_), in1,
        in1 *
            make_expr<constant_node>((float)attrs.get<float>("alpha"),
                                     in0->dtype_) *
            builder::make_exp(in0));
    expr used_out = builder::make_select(
        in0 > make_expr<constant_node>(0.f, in0->dtype_), in1,
        in1 * (in0 + make_expr<constant_node>((float)attrs.get<float>("alpha"),
                                              in0->dtype_)));
    expr res = attrs.get_or_else<bool>("use_dst", true) ? used_out : used_inp;
  ...
  case elt_operator::HARDSWISH_BWD: {
    ...
    expr cal_expr =
        in1 * (make_expr<constant_node>(2.f, in0->dtype_) *
                   make_expr<constant_node>(alpha, in0->dtype_) * in0 +
               make_expr<constant_node>(beta, in0->dtype_));
    expr res = builder::make_select(
        (test_expr) <= make_expr<constant_node>(0.f, in0->dtype_),
        make_expr<constant_node>(0.f, in0->dtype_),
        builder::make_select((test_expr) >=
                                 make_expr<constant_node>(1.f, in0->dtype_),
                             in1, cal_expr));
    return builder::make_assign_unattached(out, res);
  } break;
  case elt_operator::HARDSIGMOID_BWD: {
    auto alpha = constant_maker<float>(attrs.get<float>("alpha"), in0->dtype_);
    auto beta = constant_maker<float>(attrs.get<float>("beta"), in0->dtype_);
    ...
    expr res = builder::make_select(
        f_var0 < one_f, builder::make_select(f_var0 > zero_f, f_var1, zero_f),
        zero_f);
  ...
  case elt_operator::MISH_BWD: {
    auto min_inp =
        builder::make_min(in0, constant_maker<float>(22.180708f, in0->dtype_));
    ...
    // 4 * e^2x
    auto formular_0 = exp_f2_var * constant_maker<float>(4.f, in0->dtype_);
    auto f_var0 = var_maker("f_var0" + fusion_create_var_idx());
    assign_maker(f_var0, formular_0);
    // e^3x + 4 * e^2x
    auto formular_1 = builder::make_fmadd(var_exp_f, exp_f2_var, f_var0);
    ...
    auto formular_6 =
        builder::make_fmadd(constant_maker(4.f, in0->dtype_), f_var2, f_var5);
    auto f_var6 = var_maker("f_var6" + fusion_create_var_idx());
    assign_maker(f_var6, formular_6);
    // delta = (e^x+1)^2 + 1
    auto formular_7 = var_exp_f + constant_maker<float>(1.f, in0->dtype_);
  ...
  case elt_operator::TANH_BWD: {
    ...
    expr src = builder::make_mul(
        in1, builder::make_sub(make_expr<constant_node>(1.f, in0->dtype_),
                               builder::make_mul(tanh_ret, tanh_ret)));
    expr dst = builder::make_mul(
        in1, builder::make_sub(make_expr<constant_node>(1.f, in0->dtype_),
  ...
  case elt_operator::SOFTPLUS_BWD: {
    ...
    auto make_cast_f32 = [](const expr &inp) {
      return builder::make_cast(sc_data_type_t::f32(inp->dtype_.lanes_), inp);
    };
    auto sigmoid_inp = is_f32 ? in0 : make_cast_f32(in0);
    const auto &sigmoid_ret = sigmoid_compute.compute_element(
        sigmoid_inp *
        constant_maker<float>(beta, sc_data_type_t::f32(in0->dtype_.lanes_)));
    auto inp2 = is_f32 ? in1 : make_cast_f32(in1);
    auto f_val = builder::make_mul(sigmoid_ret, inp2);
    auto res_val = is_f32 ? f_val : builder::make_cast(in0->dtype_, f_val);
    auto res = builder::make_assign_unattached(out, res_val);
    return res;
  } break;
  default: {
    COMPILE_ASSERT(false, "Unsupport elementwise op "
                          "found.\n");
    return stmt();
  } break;
  }
...
void compute_block_broadcast(const context_ptr &ctx, sc_graph_t &graph,
                             ...
                             int bc_input_idx, const std::vector<int> &bc_axis,
                             const vectorized_info_t &vx_info,
                             const mask_compute_func_t &compute,
                             const graph_tensor_ptr &expand_gt,
  ...
  bool keep_dims =
      in_tsl->get_base_dims().size() == in_bc_tsl->get_base_dims().size();
  // add output type check, manual downcast
  sc_data_etype out_etype = dst.tptr_->dtype_.get_pointer_element().as_etype();
  ...
  auto tail_var = builder::make_var(datatypes::index, std::string("_fuseiter") +
                                                          fusion_create_idx());
  ...
  if (use_mask) {
    compute_mask_and_generate_condition(
        graph, src, info.inputs_[0]->details_.get_plain_dims(),
  ...
  stmt cur;
  bool bc_input_cast =
      !in_bc_tsl->tptr_->dtype_.get_pointer_element().is_etype(out_etype);
  ...
  auto func_op_cast = [](sc_data_etype out_etype, expr &indexed_input,
    ...
    if (use_scalar) {
      indexed_input =
          builder::make_cast(sc_data_type_t(out_etype), indexed_input);
    } else {
  ...
  auto func_index_bc_input = [&](std::vector<expr> &in_bc_idx,
    ...
    if (bc_axis.back() == static_cast<int64_t>(vx_info.axis)) {
      indexing_from_diff_cond(use_scalar, has_tail, *in_bc_tsl, in_bc_idx,
                              lanes, indexed_bc_input, slice_len, iter_var,
                              floor);
    }
    // IF last dim is excluded in bc_axis.
    else {
      if (use_scalar) {
        indexed_bc_input = builder::make_indexing(in_bc_tsl->tptr_, in_bc_idx);
      } else {
    ...
    if (static_cast<int>(dst.get_shape().size()) == vx_info.axis + 1 &&
      ...
      if (!floor.isa<constant>() || floor_int) {
        ...
        indexing_from_diff_cond(false, false, dst, dst_idx, lanes,
                                indexed_target, slice_len, iter_vars.at(i),
                                floor);
        indexing_from_diff_cond(false, false, *in_tsl, in_idx, lanes,
                                indexed_input, slice_len, iter_vars.at(i),
                                floor);
        ...
        func_index_bc_input(in_bc_idx, indexed_bc_input, iter_vars.at(i));
        ...
        if (cond_it != conditions.end()) {
          ...
          cur = compute(std::vector<expr>{indexed_input, indexed_bc_input},
        ...
        if (iter_vars.at(i).isa<var>()) {
          cur = make_stmt<for_loop_node_t>(iter_vars.at(i), expr(0), floor,
                                           expr(int(vx_info.lanes)), cur, true,
                                           for_type::NORMAL);
      ...
      if ((!tail.isa<constant>() && !is_blocking_shape) || tail_int) {
        auto res_it = std::find(bc_axis.begin(), bc_axis.end(), vx_info.axis);
        if (res_it != bc_axis.end()) {
          in_bc_idx_tail[keep_dims ? vx_info.axis
                                   : (res_it - bc_axis.begin())] = tail_var;
        }
        expr indexed_bc_input_tail;
        func_index_bc_input(in_bc_idx_tail, indexed_bc_input_tail, tail_var,
                            use_scalar, true);
        expr indexed_target_tail;
        expr indexed_input_tail;
        indexing_from_diff_cond(use_scalar, true, dst, dst_idx_tail, lanes,
                                indexed_target_tail, slice_len, tail_var, floor,
                                true);
        indexing_from_diff_cond(use_scalar, true, *in_tsl, in_idx_tail, lanes,
                                indexed_input_tail, slice_len, tail_var, floor,
                                true);
        if (!in_tsl->tptr_->dtype_.get_pointer_element().is_etype(out_etype)) {
        ...
        bld->push_scope();
        cur = compute(
            std::vector<expr>{indexed_input_tail, indexed_bc_input_tail},
            target_vec_tail);
        cur->attr()[op_traits::workload_computable_t::workload_number] = wkld;
        bld->emit(cur);
        cur = make_stmt<for_loop_node_t>(
            tail_var, expr(floor), do_cast_and_fold(floor + tail),
      ...
      if (!tcur.empty() && tcur[0].defined()) {
        ...
        cur = make_stmt<for_loop_node_t>(
            std::move(iter_vars.at(i)), expr(0), dst.get_shape().at(i), expr(1),
        ...
        if (bc_input_cast) {
          indexed_bc_input = builder::make_cast(
              sc_data_type_t(out_etype, indexed_bc_input->dtype_.lanes_),
              indexed_bc_input);
        }
        std::vector<expr::lvalue_proxy_t> target_vec{
            expr::lvalue_proxy_t(indexed_target, false)};
        ...
        bld->emit(cur);
        cur = make_stmt<for_loop_node_t>(
            iter_vars.at(i), expr(0), dst.get_shape().at(i), expr(1),
...
void binary_elementwise_op_impl_t::compute_block(
  ...
  if (use_broadcast) {
    ...
    compute_block_broadcast(ctx, get_owner_graph(), inputs, *dst[0], info_,
                            ...
                            mask_compute_func_t(func), get_outputs()[0], wkld,
                            use_mask);
  } else {
    compute_vectorized_op(ctx, get_owner_graph(), inputs, *dst[0], info_,
                          vx_info_, mask_compute_func_t(func),
                          mask_compute_func_t(func), attrs_, get_outputs()[0],
                          wkld, use_mask);
  }
}

void unary_backward_base_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dst,
    const std::vector<const tensor_slice *> &inputs) {
  size_t wkld = compute_fusible_workload(ctx, dst, inputs);
  ...
  compute_vectorized_op(ctx, get_owner_graph(), inputs, *dst[0], info_, vx_info,
                        mask_compute_func_t(func), mask_compute_func_t(func),
                        attrs_, get_outputs()[0], wkld, use_mask);
}

unary_backward_base_t::unary_backward_base_t(graph_tensor_ptr lhs,
                                             graph_tensor_ptr rhs,
                                             elt_operator elt_op)
