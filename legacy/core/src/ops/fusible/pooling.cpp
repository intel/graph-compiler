...
static inline any_map_t add_pl_type_and_in_shape(const any_map_t &attrs,
                                                 int pl_type,
                                                 const sc_dims &input_shape) {
  auto ret = attrs;
  ret[pooling_attr_key::pooling_type] = pl_type;
...
get_real_pooling_axis_form_tensor(const graph_tensor_ptr &t,
                                  bool channel_last) {
...
static std::vector<int>
get_ncx_formatcode_vector_form_tensor(const graph_tensor_ptr &t,
                                      bool channal_last) {
  std::vector<int> out;
  ...
  for (int i = 0; i <= sc_data_format_kind_t::MAX_DIMS; i++) {
    ...
    if (n == sc_data_format_kind_t::UNDEF_DIM)
      break;
    if (channal_last) {
      if (n == bs_axis)
        out.push_back(n);
...
std::vector<int> pooling_op_t::get_channel_axis() const {
  std::vector<int> ret;
  auto ncx_fmt_vec =
      get_ncx_formatcode_vector_form_tensor(get_inputs()[0], channel_last_);
...
static void check_format(sc_data_format_t fmt, bool channel_last) {
  ...
  if (n_kernel_dims == 3) {
    pos_name.insert(pos_name.begin(), "d");
  }
  ...
  for (int i = 0; i < n_kernel_dims; i++) {
    COMPILE_ASSERT(
        ...
        pos_name[i] + " axis should not be blocked in fusible pooling!");
  }
}
...
slice_range_list infer_pool_slice_ranges(const graph_tensor_ptr &infered_tensor,
  ...
  for (auto &range_list : in_range_list) {
    ...
    for (unsigned i = 0; i < range_list.size(); i++) {
      if (!(std::find(real_pooling_axis.begin(), real_pooling_axis.end(), i) ==
...
static void check_and_set_pads_begin_and_pads_end(
    any_map_t &attrs, const sc_dims &input_plain_dims, sc_dims &pads_begin,
    sc_dims &pads_end, bool channel_last) {
  ...
  COMPILE_ASSERT(
      auto_pad == auto_pad_options::none ||
  ...
  if (auto_pad == auto_pad_options::none) {
    if (attrs.has_key(pooling_attr_key::paddings)) {
      ...
      if (padding.size() == 1) {
        ...
        COMPILE_ASSERT(false, "padding should have " << n_kernel_dims
                                                     << " or 1 n-dims, but got "
                                                     << padding.size());
      }
    } else {
      ...
      COMPILE_ASSERT(pads_begin.size() == n_kernel_dims,
                     "pads_begin should have " << n_kernel_dims
                                               << "n-dims, but got "
                                               << pads_begin.size());
      COMPILE_ASSERT(pads_end.size() == n_kernel_dims, "pads_end should have "
                                                           << n_kernel_dims
                                                           << "n-dims, but got "
                                                           << pads_end.size());
    }
...
static void check_and_set_kernel_strides_and_pooling_type(
    any_map_t &attrs, uint64_t n_kernel_dims, sc_dims &kernel, sc_dims &stride,
  ...
  pooling_type = pooling_type_t(attrs.get<int>(pooling_attr_key::pooling_type));
}
...
pooling_op_t::pooling_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  check_and_set_kernel_strides_and_pooling_type(attrs_, n_kernel_dims, kernel_,
                                                stride_, pooling_type_);
  ...
  check_and_set_pads_begin_and_pads_end(attrs_,
                                        ...
                                        pads_begin_, pads_end_, channel_last_);
  ...
  COMPILE_ASSERT(rounding_type == rounding_type_options::floor ||
                 ...
                 "rounding type should be floor or ceil, but got"
                     << rounding_type);
  ...
  if (outs.empty()) {
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(
        this, info_.inputs_[0]->details_.get_format(), output_dims,
...
pooling_op_t::pooling_op_t(const std::vector<graph_tensor_ptr> &ins,
                           const std::vector<graph_tensor_ptr> &outs,
                           const pooling_type_t &pl_type,
                           const any_map_t &attrs)
    : pooling_op_t(ins, outs, add_pl_type(attrs, static_cast<int>(pl_type))) {}

infer_status_code pooling_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                   fslice_map &fsmap) {
  ...
  auto real_required_axis =
      get_real_pooling_axis_form_tensor(info_.inputs_[0], channel_last_);
  auto &in_blocked_dims = info_.inputs_[0]->details_.get_blocking_dims();
  for (auto &range_list : known_ranges_map[0]) {
    if (!slice_full_on_axis(in_blocked_dims, range_list, real_required_axis)) {
...
infer_status_code pooling_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                       fslice_map &fsmap) {
...
static void compute_block_pooling(
    const std::vector<const tensor_slice *> &inputs, const tensor_slice &dst,
    pooling_type_t pooling_typ, sc_dims kernel, sc_dims stride,
    const std::vector<expr> &pads_begin, const std::vector<int> &in_fmt_vector,
    const vectorized_info_t &vx_info, sc_data_type_t in_dtype,
    sc_data_type_t out_dtype, any_map_t &attrs,
  ...
  auto in_vectorized_dtype = sc_data_type_t(in_dtype.type_code_, vx_info.lanes);
  ...
  for (unsigned i = 0; i < kernel.size(); i++) {
    kernel_iter_vars.emplace_back(builder::make_var(
        datatypes::index, std::string("_kernel") + fusion_create_idx()));
  }
  ...
  for (unsigned i = 0; i < inputs[0]->nslice_dims(); i++) {
    if (in_fmt_vector[i] < 2) {
      ...
      if (out_h_idx.isa<constant>())
        out_h_idx = iter_vars[i];
  ...
  expr indexed_input = builder::make_indexing(inputs[0]->tptr_,
                                              src_indices.at(0), vx_info.lanes);
  ...
  bool is_int = utils::is_one_of(in_dtype.type_code_, sc_data_etype::U8,
                                 sc_data_etype::U32, sc_data_etype::S8,
                                 sc_data_etype::S32);
  if (pooling_typ == pooling_type_t::avg) {
    init_value = 0.f;
  } else {
    COMPILE_ASSERT(pooling_typ == pooling_type_t::max, "wrong pooling type");
  ...
  expr zero_constant, one_constant, kernel_size_constant, pooling_buf_constant;
  if (in_dtype.type_code_ == sc_data_etype::F32 ||
    ...
    kernel_size_constant =
        make_expr<constant_node>(float(kernel_size), in_vectorized_dtype);
    pooling_buf_constant =
        make_expr<constant_node>(init_value.get<float>(), in_vectorized_dtype);
  ...
  for (int i = kernel_iter_vars.size() - 1; i >= 0; i--) {
    ...
    if (i == int(kernel_iter_vars.size() - 1)) {
      if (pooling_typ == pooling_type_t::avg) {
        if (pool_buf_vectorized_dtype != in_vectorized_dtype) {
          indexed_input =
              builder::make_cast(pool_buf_vectorized_dtype, indexed_input);
        }
        then_stmt = make_stmt<assign_node_t>(
            pooling_buf_var, builder::make_add(indexed_input, pooling_buf_var));
        then_stmt->attr()[op_traits::workload_computable_t::workload_number] =
            wkld;
        if (exclude_pad) {
          ...
          additional_assign
              ...
              wkld;
          then_stmt = make_stmt<stmts_node_t>(
              std::vector<stmt>{std::move(then_stmt), additional_assign});
        }
      } else if (pooling_typ == pooling_type_t::max) {
        then_stmt = make_stmt<assign_node_t>(
            pooling_buf_var, builder::make_max(indexed_input, pooling_buf_var));
        then_stmt->attr()[op_traits::workload_computable_t::workload_number] =
            wkld;
      }
    } else {
    ...
    cur = make_stmt<for_loop_node_t>(std::move(kernel_iter_vars.at(i)), expr(0),
                                     int(kernel[i]), expr(1), std::move(body),
  ...
  if (pooling_typ == pooling_type_t::avg && exclude_pad) {
    stmt define_kernel_size_var =
        make_stmt<define_node_t>(kernel_size_var, linkage::local, expr());
    stmt kernel_size_asnode =
        make_stmt<assign_node_t>(kernel_size_var, kernel_size_constant);
  ...
  for (int i = iter_vars.size() - 1; i >= 0; i--) {
    if (i == int(iter_vars.size() - 1)) {
      if (pooling_typ == pooling_type_t::avg) {
        ...
        if (exclude_pad)
          kernel_size_expr = kernel_size_var;
        expr pooling_result = builder::make_div(
            pooling_buf_var,
            builder::make_cast(pool_buf_vectorized_dtype, kernel_size_expr));
        if (out_vectorized_dtype.type_code_ !=
            pool_buf_vectorized_dtype.type_code_)
          pooling_result =
              builder::make_cast(out_vectorized_dtype, pooling_result);
        target_assign =
            make_stmt<assign_node_t>(indexed_target, pooling_result);
      } else if (pooling_typ == pooling_type_t::max) {
        target_assign =
            make_stmt<assign_node_t>(indexed_target, pooling_buf_var);
      }
    ...
    if (!body.ptr_same(cur))
      add_parent_node(cur, body);
    ...
    for (int j = i; j >= 0; j--) {
      ...
      inner_slice[j].second =
          ((static_cast<int>(j) == vx_info.axis) ? expr(int(vx_info.lanes))
                                                 : expr(1));
    }
    fslice_map fsmap;
    ...
    cur = make_stmt<for_loop_node_t>(
        std::move(iter_vars.at(i)), 0, dst.get_shape()[i],
        (i == int(iter_vars.size() - 1)) ? int(vx_info.lanes) : 1, body, true,
        i == 0 ? for_type::PARALLEL : for_type::NORMAL);
...
void pooling_op_t::compute_block(
  ...
  const int channel_axis =
      channel_last_ ? info_.inputs_[0]->details_.get_plain_dims().size() - 1
                    : 1;
  ...
  auto in_fmt_vector =
      get_ncx_formatcode_vector_form_tensor(info_.inputs_[0], channel_last_);
  std::string auto_pad = attrs_.get_or_else<std::string>(
      pooling_attr_key::auto_pad, auto_pad_options::none);
  auto pads_begin_expr = get_owner_graph().dims_to_expr(pads_begin_);
  if ((is_dynamic() && (auto_pad == auto_pad_options::same_upper ||
                        auto_pad == auto_pad_options::same_lower))) {
    ...
    for (unsigned i = 0; i < inputs[0]->nslice_dims(); i++) {
      ...
      int plan_axis = in_fmt_vector[i];
      if (!is_dynamic_dim(
              info_.inputs_[0]->details_.get_plain_dims()[plan_axis])) {
        continue;
      }
      int pads_stride_index = plan_axis - 2;
      auto total_pad = builder::make_max((dst_shapes[i] -
                                          1) * int(stride_[pads_stride_index]) +
                                             int(kernel_[pads_stride_index]),
                                         src_shapes[i]) -
                       src_shapes[i];
      if (auto_pad == auto_pad_options::same_upper) {
        pads_begin_expr[pads_stride_index] = do_cast_and_fold(total_pad / 2);
      } else {
  ...
  compute_block_pooling(inputs, *dst[0], pooling_type_, kernel_, stride_,
                        pads_begin_expr, in_fmt_vector, vx_info_, in_dtype,
                        out_dtype, attrs_, info_.outputs_[0], wkld);
}

void pooling_op_t::query_format(
    context_ptr ctx,
...
size_t
pooling_op_t::compute_workload(const std::vector<shape_dtype_pair> &ins,
  ...
  for (unsigned i = 0; i < n_pad_dims; i++) {
    if (is_dynamic_dim(in_dims[shape_begin_axis + i])) {
      if (auto_pad == auto_pad_options::same_upper ||
        ...
        if (stride_[i] == 1 &&
            (kernel_[i] - (pads_begin_[i] + pads_end_[i]) == 1)) {
          ret.emplace_back(in_dims[shape_begin_axis + i],
                           out_dims[shape_begin_axis + i]);
        }
      }
...
reflection::shared_general_object_t pooling_op_t::get_dynamic_runtime_info() {
  ...
  auto dyn_info =
      ndims == 5
          ? dyn_pooling_runtime_info_t(
                stride_[0], stride_[1], stride_[2], pads_begin_[0],
                pads_begin_[1], pads_begin_[2], pads_end_[0], pads_end_[1],
                pads_end_[2], kernel_[0], kernel_[1], kernel_[2],
                rounding_type_floor, auto_pads_same)
          : dyn_pooling_runtime_info_t(stride_[0], stride_[1], pads_begin_[0],
                                       pads_begin_[1], pads_end_[0],
                                       pads_end_[1], kernel_[0], kernel_[1],
                                       rounding_type_floor, auto_pads_same);
  ...
  for (unsigned i = 0; i < n_pad_dims; i++) {
    if (is_dynamic_dim(data_dims[shape_begin_axis + i]) &&
      ...
      if (auto_pad == auto_pad_options::same_upper ||
          auto_pad == auto_pad_options::same_lower) {
        cal_expr =
            do_cast_and_fold((var_in + expr_strides[i] - 1) / expr_strides[i]);
      } else {
        cal_expr = do_cast_and_fold(
            (var_in + expr_pads_begin[i] + expr_pads_end[i] - expr_kernels[i]) /
                expr_strides[i] +
            1);
      }
  ...
  for (unsigned i = 0; i < n_pads_dims; i++) {
    ...
    if (is_dynamic_dim(input_dims[shape_begin_axis + i])) {
      ...
      if (rounding_floor) {
        ...
        output_dims[shape_begin_axis + i] =
            utils::divide_and_ceil(input_dims[shape_begin_axis + i] + padding -
                                       kernel_[i],
                                   stride_[i]) +
            1;
      }
...
pooling_avg_op_t::pooling_avg_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  COMPILE_ASSERT(attrs.has_key("exclude_pad"),
                 "avg pooling must have exclude_pad");
}
...
pooling_backprop_op_t::pooling_backprop_op_t(
  ...
  check_and_set_pads_begin_and_pads_end(attrs_, input_plain_shape, pads_begin_,
                                        pads_end_, channel_last_);

  // set outputs
  sc_dims out_delta_dims = input_plain_shape;
  if (outs.empty()) {
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(
        this, info_.inputs_[0]->details_.get_format(), out_delta_dims,
        ins[0]->details_.dtype_));
  } else {
...
pooling_avg_backprop_op_t::pooling_avg_backprop_op_t(
    ...
    : pooling_backprop_op_t(
          ins, outs,
          add_pl_type_and_in_shape(attrs, static_cast<int>(pooling_type_t::avg),
                                   input_shape)) {
  COMPILE_ASSERT(ins.size() == 1, " pooling_avg_backprop_op_t have 1 inputs");
  op_name_ = "pooling_avg_backprop";
  COMPILE_ASSERT(attrs.has_key("exclude_pad"),
...
pooling_max_backprop_op_t::pooling_max_backprop_op_t(
    ...
    : pooling_backprop_op_t(
          ins, outs,
          add_pl_type_and_in_shape(attrs, static_cast<int>(pooling_type_t::max),
                                   ins[0]->details_.get_plain_dims())) {
...
void pooling_backprop_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  std::vector<std::vector<sc_data_format_t>> in_formats, out_formats;
...
infer_status_code
pooling_backprop_op_t::infer_slice_ranges(const context_ptr &ctx,
                                          fslice_map &fsmap) {
  // search known ranges from any input of cur fusbile op
  slice_range_map known_ranges_map = search_known_input_slice(this, fsmap);
...
static void pooling_backward_fill_zero_dst(const tensor_slice &dst,
  ...
  for (unsigned i = 0; i < dst.nslice_dims(); i++) {
    dst_idx.emplace_back(builder::make_var(
  ...
  stmt indelta_zero_asnode = make_stmt<assign_node_t>(
      indexed_in_delta, make_expr<constant_node>(0.f, in_vectorized_dtype));
  // make loops
  stmt body, cur;
  for (int i = dst_idx.size() - 1; i >= 0; i--) {
    if (i == int(dst_idx.size() - 1)) {
      cur = indelta_zero_asnode;
    }
    body = make_stmt<stmts_node_t>(std::vector<stmt>{std::move(cur)});
    cur = make_stmt<for_loop_node_t>(
        std::move(dst_idx.at(i)), 0, dst.get_shape()[i],
...
static void compute_block_pooling_backward_avg(
    const std::vector<const tensor_slice *> &inputs, const tensor_slice &dst,
    sc_dims kernel, sc_dims stride, sc_dims pads_begin,
    const std::vector<int> &dst_fmt_vector, const vectorized_info_t &vx_info,
    sc_data_type_t in_dtype, any_map_t &attrs,
    const graph_tensor_ptr &expand_gt, size_t wkld = 0UL) {
  /***The final IR may look like below:
   * _for_(_fuseiter_i, 0, I, 1)
   *   _for_(_fuseiter_j, 0, J, 1)
  ...
  for (unsigned i = 0; i < inputs[0]->nslice_dims(); i++) {
    iter_vars.emplace_back(
        range_from_outer_loop(inputs[0]->get_ranges()[i])
            ? expr(0)
            : builder::make_var(datatypes::index, std::string("_fuseiter") +
                                                      fusion_create_idx()));
  }
  expr indexed_src_delta =
      builder::make_indexing(inputs[0]->tptr_, iter_vars, vx_info.lanes);
  ...
  for (unsigned i = 0; i < dst.nslice_dims(); i++) {
    if (dst_fmt_vector[i] < 2) {
      ...
      auto window_start = make_expr<intrin_call_node>(
          intrin_type::max,
          std::vector<expr>{
              make_expr<constant_node>(int64_t(0), datatypes::s32),
              int(stride[pads_stride_index]) *
                      make_expr<cast_node>(datatypes::s32, iter_vars[i]) -
                  int(pads_begin[pads_stride_index])},
          any_map_t());
      auto window_end = make_expr<intrin_call_node>(
          intrin_type::min,
          std::vector<expr>{
              make_expr<cast_node>(datatypes::s32, dst.get_shape()[i]),
              int(stride[pads_stride_index]) *
                      make_expr<cast_node>(datatypes::s32, iter_vars[i]) -
                  int(pads_begin[pads_stride_index]) +
          ...
          any_map_t());
      if (multi_first) {
        kernel_size_multi_expr = builder::make_sub(window_end, window_start);
        multi_first = false;
      } else
  ...
  if (in_dtype.type_code_ == sc_data_etype::F32 ||
    ...
    if (exclude_pad) {
      kernel_size_asnode =
          make_stmt<assign_node_t>(kernel_size_var, kernel_size_multi_expr);
    } else {
  ...
  for (int i = kernel_iter_vars.size() - 1; i >= 0; i--) {
    ...
    if (i == int(kernel_iter_vars.size() - 1)) {
      ...
      if (vx_info.lanes > 1) {
        kernel_size = builder::make_broadcast(kernel_size_var, vx_info.lanes);
      }
  ...
  for (int i = iter_vars.size() - 1; i >= 0; i--) {
    ...
    cur = make_stmt<for_loop_node_t>(
        std::move(iter_vars.at(i)), 0, inputs[0]->get_shape()[i],
...
static void compute_block_pooling_backward_max(
  ...
  for (size_t i = 0; i < kernel.size(); i++) {
    max_val_pos[i] =
        builder::make_var(datatypes::index, "max_val_pos_" + pos_name[i]);
    defines_of_max.emplace_back(
        make_stmt<define_node_t>(max_val_pos[i], linkage::local, expr()));
  }
  if (in_dtype.type_code_ == sc_data_etype::F32 ||
      in_dtype.type_code_ == sc_data_etype::BF16) {
    assigns_of_max.emplace_back(make_stmt<assign_node_t>(
        max_val, make_expr<constant_node>(
  ...
  for (unsigned i = 0; i < inputs[1]->nslice_dims(); i++) {
    iter_vars.emplace_back(
        range_from_outer_loop(inputs[1]->get_ranges()[i])
            ? expr(0)
            : builder::make_var(datatypes::index, std::string("_fuseiter") +
                                                      fusion_create_idx()));
  }
  expr indexed_src_delta = builder::make_indexing(inputs[1]->tptr_, iter_vars);
  ...
  for (unsigned i = 0; i < dst.nslice_dims(); i++) {
    if (dst_fmt_vector[i] < 2) {
      ...
      update_max_pos_stmts.emplace_back(
          make_stmt<assign_node_t>(max_val_pos[pads_stride_index], idx));
      max_value_delta_idices.emplace_back(max_val_pos[pads_stride_index]);
    }
  }
  expr indexed_dst_delta = builder::make_indexing(dst.tptr_, dst_delta_idices);
  ...
  for (int i = kernel_iter_vars.size() - 1; i >= 0; i--) {
    ...
    if (i == int(kernel_iter_vars.size() - 1)) {
      ...
      then_stmt = make_stmt<if_else_node_t>(max_val < indexed_cur_tensor_val,
                                            update_max_info, stmt());
    } else {
      then_stmt = cur;
    }
  ...
  for (int i = iter_vars.size() - 1; i >= 0; i--) {
    if (i == int(iter_vars.size() - 1)) {
      ...
      kernel_body.emplace_back(make_stmt<if_else_node_t>(
          has_max,
          make_stmt<assign_node_t>(
              indexed_max_dst_delta,
              builder::make_add(indexed_max_dst_delta, indexed_src_delta)),
          stmt()));
...
void pooling_backprop_op_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dst,
    const std::vector<const tensor_slice *> &inputs) {
  // set up vx_info
  vx_info_.axis = dst[0]->get_shape().size() - 1;
  ...
  for (size_t i = 0; i < info_.inputs_.size(); i++) {
    auto last_axis = info_.inputs_[i]->details_.get_format().format_code_.get(
  ...
  if (pooling_type_ == pooling_type_t::avg) {
    compute_block_pooling_backward_avg(
        inputs, *dst[0], kernel_, stride_, pads_begin_,
        get_ncx_formatcode_vector_form_tensor(info_.inputs_[0], channel_last_),
        vx_info_, in_dtype, attrs_, get_inputs()[0]);
  } else {
    compute_block_pooling_backward_max(
        inputs, *dst[0], kernel_, stride_, pads_begin_,
        get_ncx_formatcode_vector_form_tensor(info_.inputs_[1], channel_last_),
        vx_info_, in_dtype, attrs_, get_inputs()[1]);
  }
}

size_t
pooling_backprop_op_t::compute_workload(const std::vector<shape_dtype_pair> &,
