...
static sc_dims infer_select_output_shape(const sc_dims &cond_shape,
                                         const sc_dims &then_shape,
                                         const sc_dims &else_shape) {
  sc_dims output_shape =
      op_traits::may_broadcast_t::infer_auto_broadcast_output_shape(then_shape,
                                                                    else_shape);
  output_shape = op_traits::may_broadcast_t::infer_auto_broadcast_output_shape(
      output_shape, cond_shape);
  return output_shape;
}

std::vector<int>
select_op_t::get_non_broadcast_input_index(bool assert_non_empty) const {
  ...
  auto output_dims = infer_select_output_shape(cond_dims, then_dims, else_dims);
  std::vector<int> ret;
...
infer_broadcast_arg_slice(slice_range_list known_range_list,
...
static binding_axis
infer_broadcast_axis_binding(binding_axis known_axis_list,
                             const std::vector<int> &bc_axis) {
...
static binding_axis
infer_broadcast_arg_axis_binding(binding_axis known_axis_list,
                                 const std::vector<int> &bc_axis) {
...
int select_op_t::get_ref_input_index(bool assert_determined) const {
  ...
  bool is_cond_non_bc = std::find(non_bc_index.begin(), non_bc_index.end(),
                                  0) != non_bc_index.end();
  bool is_then_non_bc = std::find(non_bc_index.begin(), non_bc_index.end(),
                                  1) != non_bc_index.end();
  bool is_else_non_bc = std::find(non_bc_index.begin(), non_bc_index.end(),
                                  2) != non_bc_index.end();
    ...
    if (is_dynamic()) {
      max_input_idx =
          info_.inputs_[1]->details_.get_format_candidates().size() >=
                  info_.inputs_[2]->details_.get_format_candidates().size()
              ? 1
              : 2;
    } else {
      max_input_idx = 1;
...
void select_op_t::query_format(
  ...
  for (size_t i = 0; i < info_.inputs_.size(); ++i) {
    ...
    COMPILE_ASSERT((input_rank == 1 &&
                    info_.inputs_[i]->details_.get_plain_dims() == sc_dims{1} &&
    ...
    if (static_cast<int>(i) == max_input_idx) {
      ...
      auto target_format = infer_broadcast_format(
          info_.inputs_[max_input_idx]->details_, info_.inputs_[i]->details_);
...
infer_status_code select_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                  fslice_map &fsmap) {
  COMPILE_ASSERT(get_inputs().size() == 3,
                 "Select op is expected to have 3 inputs");
    ...
    if (maxtensor_idx >= 0) {
      if (known_idx[maxtensor_idx] == 1) {
        for (int i = 0; i < 3; i++) {
          if (known_idx[i] == 0) {
            bool keep_dims =
                get_inputs()[i]->details_.get_blocking_dims().size() ==
            ...
            slice_range_list bc_arg_range_list = infer_broadcast_arg_slice(
                known_ranges_map[maxtensor_idx], bc_axis, keep_dims);
        ...
        slice_range_list bc_range_list = infer_broadcast_slice(
            ...
            get_inputs()[maxtensor_idx]->details_.get_blocking_dims_expr(
                get_owner_graph()));
        ...
        if (it != known_idx.end()) {
          ...
          bool keep_dims =
              get_inputs()[remaining_idx]
                  ->details_.get_blocking_dims()
                  .size() ==
              get_inputs()[maxtensor_idx]->details_.get_blocking_dims().size();
    ...
    if (maxtensor_idx >= 0) {
      if (known_idx[maxtensor_idx] == 1) {
        for (int i = 0; i < 3; i++) {
          if (known_idx[i] == 0) {
            ...
            if (keep_dims) {
              ...
              COMPILE_ASSERT(get_inputs()[i]->details_.get_plain_dims() ==
                                 sc_dims{1},
        ...
        bool keep_dims =
            ...
            get_inputs()[maxtensor_idx]->details_.get_blocking_dims().size();
        if (keep_dims) {
          known_axis_map[maxtensor_idx] = known_axis_map[known_tensor_idx];
        } else {
          COMPILE_ASSERT(
              get_inputs()[known_tensor_idx]->details_.get_plain_dims() ==
                  sc_dims{1},
          ...
          auto plain_bc_axis =
              op_traits::may_broadcast_t::get_auto_broadcast_bc_axis(
                  get_inputs()[known_tensor_idx]->details_.get_plain_dims(),
                  get_inputs()[maxtensor_idx]->details_.get_plain_dims());
          if (plain_bc_axis == std::vector<int>{-1}) {
            plain_bc_axis[0] =
                get_inputs()[maxtensor_idx]->details_.get_plain_dims().size() -
                1;
          }
        ...
        if (it != known_idx.end()) {
          ...
          bool keep_dims =
              ...
              get_inputs()[maxtensor_idx]->details_.get_blocking_dims().size();
          if (keep_dims) {
            known_axis_map[remaining_idx] = known_axis_map[maxtensor_idx];
          } else {
            COMPILE_ASSERT(
                get_inputs()[remaining_idx]->details_.get_plain_dims() ==
                    sc_dims{1},
                "Select op's infer binding axis encountered "
                "unaligned input shapes.");
            binding_axis bc_arg_axis_list(known_axis_map[maxtensor_idx].size());
...
std::vector<int> select_op_t::get_bc_axis(const int axis1,
                                          const int axis2) const {
  ...
  int ref_axis = std::find(non_bc_indices.begin(), non_bc_indices.end(),
                           axis1) != non_bc_indices.end()
                     ? axis1
                     : axis2;
  std::vector<int> plain_axis =
      ref_axis == axis1
          ? op_traits::may_broadcast_t::get_auto_broadcast_bc_axis(shape2,
                                                                   shape1)
          : op_traits::may_broadcast_t::get_auto_broadcast_bc_axis(shape1,
                                                                   shape2);
    ...
    for (size_t j = i + 1; j < get_inputs().size(); ++j) {
      ...
      if (dims1.size() == dims2.size()) {
        for (size_t idx = 0; idx < dims1.size(); ++idx) {
          // maybe broadcast
          if ((is_dynamic_dim(dims1[idx]) || is_dynamic_dim(dims2[idx])) &&
  ...
  for (size_t i = 0; i < out_plain_dims.size(); ++i) {
    if (is_dynamic_dim(out_plain_dims[i])) {
      for (size_t j = 0; j < get_inputs().size(); ++j) {
        const auto &in_plain_dims = get_inputs()[j]->details_.get_plain_dims();
...
void compute_block_select(
    const context_ptr &ctx, const std::vector<const tensor_slice *> &src,
  ...
  bool keep_dims_1 =
      in_tsl->get_base_dims().size() == in_bc_tsl_1->get_base_dims().size();
  bool keep_dims_2 =
      in_tsl->get_base_dims().size() == in_bc_tsl_2->get_base_dims().size();
  ...
  for (unsigned i = 0; i < dst.nslice_dims(); i++) {
    ...
    if (std::find(bc_axis_1.begin(), bc_axis_1.end(), i) != bc_axis_1.end()) {
      in_bc_idx_1.emplace_back(iter_vars.back());
    } else if (keep_dims_1) {
      in_bc_idx_1.emplace_back(0);
    }
    if (std::find(bc_axis_2.begin(), bc_axis_2.end(), i) != bc_axis_2.end()) {
  ...
  // For empty bc_axis
  if (in_bc_idx_1.empty())
    in_bc_idx_1 = {0};
  if (in_bc_idx_2.empty())
    in_bc_idx_2 = {0};
    ...
    if (bc_axis.back() == static_cast<int64_t>(vx_info.axis)) {
      indexed_bc_input =
          builder::make_indexing(in_bc_tsl->tptr_, in_bc_idx,
                                 tail_threshold ? 1 : vx_info.lanes, mask);
    }
  ...
  for (int i = static_cast<int>(dst.get_shape().size() - 1); i >= 0; i--) {
    ...
    if (static_cast<int>(dst.get_shape().size()) == vx_info.axis + 1 &&
      ...
      if ((!floor.isa<constant>() || floor_int)) {
        ...
        if (maxtensor_idx == 0) {
          inputs = {indexed_input, indexed_bc_input_1, indexed_bc_input_2};
        } else if (maxtensor_idx == 1) {
          inputs = {indexed_bc_input_1, indexed_input, indexed_bc_input_2};
        } else {
          inputs = {indexed_bc_input_1, indexed_bc_input_2, indexed_input};
        }
        ...
        if (iter_vars.at(i).isa<var>()) {
          cur = make_stmt<for_loop_node_t>(
              iter_vars.at(i), expr(0), expr(floor), expr(int(vx_info.lanes)),
              cur, true, for_type::NORMAL);
      ...
      if ((!tail.isa<constant>() && !is_blocking_shape) || tail_int) {
        auto func_tail_var_pos = [&](std::vector<expr> &in_bc_idx,
          ...
          if (res_it != bc_axis.end()) {
            in_bc_idx[keep_dims ? vx_info.axis : (res_it - bc_axis.begin())] =
                tail_var;
          }
        };
        func_tail_var_pos(in_bc_idx_1_tail, bc_axis_1, keep_dims_1, tail_var);
        func_tail_var_pos(in_bc_idx_2_tail, bc_axis_2, keep_dims_2, tail_var);
        expr mask;
        if (!use_scalar) {
          mask =
              last_dim_generate_mask(tail_var, floor, slice_len, lanes, true);
        }
        ...
        expr indexed_input_tail = builder::make_indexing(
            in_tsl->tptr_, in_idx_tail, use_scalar ? 1 : lanes, mask);
        ...
        if (maxtensor_idx == 0) {
          ...
          inputs_tail = {indexed_bc_input_1_tail, indexed_bc_input_2_tail,
                         indexed_input_tail};
        }
        ...
        bld->emit(cur);
        cur = make_stmt<for_loop_node_t>(
            tail_var, expr(floor), slice_len, use_scalar ? expr(1) : lanes,
      ...
      if (!tcur.empty() && tcur[0].defined()) {
        ...
        expr indexed_bc_input_1 =
            builder::make_indexing(in_bc_tsl_1->tptr_, in_bc_idx_1);
        expr indexed_bc_input_2 =
            builder::make_indexing(in_bc_tsl_2->tptr_, in_bc_idx_2);
...
void select_op_t::compute_block(
  ...
  for (int64_t i = dst[0]->nslice_dims() - 1; i >= 0; --i) {
    int cur_dim =
        get_const_as_int(dst[0]->get_shape()[i].checked_as<constant>());
    if (1 != cur_dim) {
      vx_info_.axis = i;
      break;
    }
  ...
  auto func = [&](const std::vector<expr> &ins,
    ...
    return builder::make_assign_unattached(
        outs[0], builder::make_select(
                     ins[0] > make_expr<constant_node>(static_cast<uint64_t>(0),
                                                       ins[0]->dtype_),
                     ins[1], ins[2]));
    // Here we use "ins[0] >
...
infer_status_code select_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                      fslice_map &fsmap) {
