...
shape_of_tensor_op_t::shape_of_tensor_op_t(
  ...
  shape_type_ =
      padding_shape_etype_t(attrs_.get<int>(attr_keys::padding_shape_type));
  auto shape_size = static_cast<int>(ins[0]->details_.get_plain_dims().size());
...
void shape_of_tensor_op_t::query_format(
    context_ptr ctx,
...
infer_status_code
shape_of_tensor_op_t::infer_slice_ranges(const context_ptr &ctx,
                                         fslice_map &fsmap) {
...
static int get_constant_padded_shape(int shape, padding_shape_etype_t type,
                                     bool is_batch) {
  switch (type) {
...
static expr get_expr_padded_shape(builder::builder_impl_t *bld,
                                  const expr &shape, padding_shape_etype_t type,
                                  bool is_batch) {
  switch (type) {
  case padding_shape_etype_t::without_padding:
  ...
  case padding_shape_etype_t::matmul_padding: {
    ...
    bld->push_var_tensor_def(
        block, linkage::local,
        builtin::call_get_matmul_dyn_cfg_single(
            builder::make_cast(datatypes::s32, shape), is_batch));
    return builder::make_cast(datatypes::s32,
                              divide_and_ceil(shape, block) * block);
    break;
  }
  case padding_shape_etype_t::conv_padding:
    COMPILE_ASSERT(false, "Unimplement of conv padding shape!");
...
void shape_of_tensor_op_t::compute_block(
  ...
  if (shape.isa<constant>()) {
    padded_shape = get_constant_padded_shape(
        static_cast<int>(get_expr_as_int(shape)), shape_type_, is_batch);
  } else {
