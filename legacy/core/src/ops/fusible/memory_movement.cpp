...
static void
check_concat_validity(const std::vector<graph_tensor_ptr> &candidates,
                      unsigned axis) {
  ...
  for (unsigned i = 1; i < candidates.size(); i++) {
    ...
    if (curShape.size() != firstShape.size()) {
      COMPILE_ASSERT(0, "Input shapes are not matched in concat fusion op!\n");
    }
    for (unsigned dim = 0; dim < firstShape.size(); dim++) {
      if (axis == dim && curShape[dim]) {
        continue;
      }
...
static void compute_block_concat(const context_ptr &ctx,
                                 ...
                                 const tensor_slice &dst, int64_t axis,
                                 size_t wkld = 0UL) {
  ...
  for (unsigned j = 0; j < src.size(); j++) {
    ...
    if (is_static) { // static shape case
      ...
      if (tail_int) { // tail part
        ...
        auto mask = last_dim_generate_mask(inner_iter[last_axis - axis][j],
                                           floor, slice_len, vec_lanes, true);
        indexed_dst =
            builder::make_indexing(dst.tptr_, dst_idx[j], vec_lanes, mask);
        indexed_src =
            builder::make_indexing(src[j]->tptr_, src_idx[j], vec_lanes, mask);
        auto assign = make_stmt<assign_node_t>(indexed_dst, indexed_src);
        ...
        tail_part = make_stmt<for_loop_node_t>(
            ...
            src[j]->get_shape()[last_axis], vec_lanes, std::move(tail_part),
      ...
      if (floor_int) { // divisible part
        ...
        indexed_dst = builder::make_indexing(dst.tptr_, dst_idx[j], vec_lanes);
        indexed_src =
            builder::make_indexing(src[j]->tptr_, src_idx[j], vec_lanes);
        auto assign = make_stmt<assign_node_t>(indexed_dst, indexed_src);
        assign->attr()[op_traits::workload_computable_t::workload_number] =
            wkld;
        divisible_part.static_as<stmts>()->seq_.emplace_back(assign);
        divisible_part = make_stmt<for_loop_node_t>(
            inner_iter[last_axis - axis][j], expr(0), floor, vec_lanes,
            std::move(divisible_part), true, for_type::NORMAL);
        cur.static_as<stmts>()->seq_.emplace_back(divisible_part);
      }

      // for other inner axes
      for (int64_t i = static_cast<int64_t>(dst.nslice_dims()) - 2; i >= axis;
           i--) {
        auto body =
            cur.isa<stmts>()
                ? std::move(cur)
        ...
        cur = make_stmt<for_loop_node_t>(
            inner_iter[i - axis][j], expr(0), src[j]->get_shape()[i], expr(1),
...
static bool
check_slice_on_non_concat_axis_equal(const slice_range_map &known_ranges_map,
                                     int64_t axis) {
  ...
  for (size_t i = 1; i < slices.size(); ++i) { // number of known slices
    ...
    for (size_t j = 0; j < slices[0].size(); ++j) { // number of multi-slice
      ...
      for (size_t k = 0; k < slices[0][j].size(); ++k) {
        ...
        if (int64_t(k) != axis && slices[0][j][k].first.isa<constant_c>() &&
          ...
          if (input0_offset != inputi_offset || input0_range != inputi_range) {
            return false;
          }
        }
      }
...
static sc_dims
infer_concat_output_shape(const std::vector<graph_tensor_ptr> &inputs,
                          const int &axis) {
...
concat_op_t::concat_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  if (outs.empty()) {
    ...
    COMPILE_ASSERT(outs.size() == 1,
                   "Only one output is supported for concat op");
...
void concat_op_t::set_format_and_axis() {
  ...
  auto max_buffer = std::max_element(
      info_.inputs_.begin(), info_.inputs_.end(),
  ...
  COMPILE_ASSERT(blocking_axes.size() == 1,
                 "The concat axis should not be blocked");
  axis_ = blocking_axes[0];
}

concat_op_t::concat_op_t(const std::vector<graph_tensor_ptr> &candidates,
                         int axis)
    : concat_op_t(candidates, {}, any_map_t({{"axis", axis}})) {}

void concat_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  set_format_and_axis();
  std::vector<std::vector<sc_data_format_t>> in_formats(info_.inputs_.size(),
                                                        {ori_format_});
  std::vector<std::vector<sc_data_format_t>> out_formats(info_.outputs_.size(),
                                                         {ori_format_});
  ...
  for (size_t i = 0; i < info_.inputs_.size(); ++i) {
    if (info_.inputs_[i]->details_.get_format() == ori_format_) {
      supported_ins[i][0].second = info_.inputs_[i]->details_.get_strides();
    }
  }
}

infer_status_code concat_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                  fslice_map &fsmap) {
  ...
  for (size_t n = 0; n < slice_size; n++) { // multi-slice index
    ...
    if (!slice_full_on_axis(
            info_.inputs_[known_id]->details_.get_blocking_dims(), sr[n],
            {int(axis_)})) {
      return infer_status_code::RETRY;
    }
    ...
    for (size_t i = 0; i < get_inputs().size(); ++i) {
      ...
      sr_i[axis_].first = 0;
      sr_i[axis_].second = int(
          dim2unsigned(info_.inputs_[i]->details_.get_blocking_dims()[axis_]));
      fsmap.get(get_inputs()[i]).at(n) = sr_i;
    }
    slice_range sr_o = sr[n];
    sr_o[axis_].first = 0;
    sr_o[axis_].second = int(
        dim2unsigned(info_.outputs_[0]->details_.get_blocking_dims()[axis_]));
    fsmap.get(get_outputs()[0]).at(n) = sr_o;
  }
  return infer_status_code::OK;
}

infer_status_code concat_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                      fslice_map &fsmap) {
  throw std::runtime_error("Not implemented");
}

void concat_op_t::compute_block(
...
static sc_dims infer_transpose_output_shape(const sc_dims &input_shape,
...
void transpose_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
...
infer_status_code transpose_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                     fslice_map &fsmap) {
  throw std::runtime_error("Not implemented");
}

infer_status_code transpose_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                         fslice_map &fsmap) {
  throw std::runtime_error("Not implemented");
}

void compute_block_transpose(const std::vector<const tensor_slice *> &src,
                             const tensor_slice &dst,
                             const std::vector<int> &axis, size_t wkld) {
  ...
  for (unsigned i = 0; i < src[0]->nslice_dims(); i++) {
    iters[i] = builder::make_var(datatypes::index, std::string("_fuseiter") +
                                                       fusion_create_idx());
    src_idx[i] = iters[i];
  }
  dst_idx = src_idx;
  ...
  for (int64_t i = src[0]->nslice_dims() - 1; i >= 0; i--) {
    auto body = make_stmt<stmts_node_t>(std::vector<stmt>{std::move(cur)});
    cur = make_stmt<for_loop_node_t>(iters[i], expr(0), src[0]->get_shape()[i],
                                     expr(1), std::move(body), true,
                                     for_type::NORMAL);
  }
  bld->emit(cur);
}

void transpose_op_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dst,
    const std::vector<const tensor_slice *> &inputs) {
  throw std::runtime_error("Not implemented");
}
...
size_t
transpose_op_t::compute_workload(const std::vector<shape_dtype_pair> &ins,
...
std::vector<expr> tensor_view_op_t::get_shapes_expr() {
  return info_.outputs_[0]->details_.get_blocking_dims_expr(get_owner_graph());
}
...
tensor_view_op_t::tensor_view_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  COMPILE_ASSERT(
      is_dynamic() || total_shape1 == total_shape2 ||
          (!outs.empty() && total_shape1 == total_shape3),
      "Wrong total size of input shapes, can not do reshape plain dims "
      "from "
          << utils::print_vector(ins[0]->details_.get_plain_dims()) << " to "
          << utils::print_vector(shapes));
  if (outs.empty()) {
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(this));
    info_.outputs_[0]->details_.dtype_ = ins[0]->details_.dtype_;
  ...
  if (is_dynamic() &&
      count_dynamic_dims(get_inputs()[0]->details_.get_plain_dims()) !=
          count_dynamic_dims(get_outputs()[0]->details_.get_plain_dims())) {
...
bool tensor_view_op_t::try_penetrate(
  ...
  if (attrs_.get_or_else("forbid_penetrate", false)) {
    return false;
  }
  ...
  while (short_idx < short_size) {
    ...
    if (inp_short) {
      auto blk_idx_list =
          input_format.format_code_.collect_blocking_index(short_idx);
  ...
  if (can_penetrate) {
    if (!inp_short) {
      ...
      for (size_t out_idx = 0; out_idx < short_size; ++out_idx) {
        ...
        for (size_t inp_idx = 0; inp_idx < long_size; ++inp_idx) {
          if (long_to_short[inp_idx] == out_idx) {
            concat_axis.insert(concat_axis.end(), input_p2b[inp_idx].begin(),
      ...
      new_code.set(
          ...
          input_format.format_code_.get(sc_data_format_kind_t::MAX_DIMS));
      size_t inp_blk_idx = 0;
      while (inp_blk_idx < 4 && blk_idx < 4 &&
             input_format.blocks_[inp_blk_idx] > 0) {
        new_format.blocks_[blk_idx++] = input_format.blocks_[inp_blk_idx++];
      }
      new_output_format = new_format;
      if (!is_dynamic()) {
        if (math_utils::get_dims_product(input_blocking_shapes) !=
            math_utils::get_dims_product(sc_data_format_t::get_blocking_shapes(
                output_plain_shapes, new_output_format))) {
          return false;
        }
      }
      return true;
      ...
      for (int i = 0; i < static_cast<int>(input_format.format_code_.ndims());
           i++) {
        ...
        if (axis_count[input_dim] > 1) {
          ...
          // plain dimension
          for (int idx = 0; idx < static_cast<int>(output_dims.size()); ++idx) {
...
shape_rl_vec tensor_view_op_t::get_dynamic_shape_relations() const {
  auto rl_axis_pair = attrs_.get_or_else("temp.rl_axis_pair",
...
void tensor_view_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  ...
  if (is_dynamic()) {
    ...
    key_set->for_each_key_process([&](const op_dispatch_key_base_t *key) {
      ...
      if (!query_by_dispatch_key &&
          dkey->in_out_formats_[0] == info_.inputs_[0]->details_.get_format()) {
...
slice_range_list
infer_tensor_view_slice(sc_graph_t &graph,
  ...
  for (auto known_ranges : known_ranges_list) {
    ...
    while (!dst_dims.empty() && !src_dims.empty()) {
      if (!slice_expr_equals(dst_dims.back(), src_dims.back()))
        break;
      consistent_tv_slice.insert(consistent_tv_slice.begin(),
                                 known_ranges.back());
    ...
    for (int i = src_dims.size() - 1; i >= 0; i--) {
      ...
      if (continuous_slice_stop) {
        ...
        if (!slice_expr.isa<constant_c>() || get_expr_as_int(slice_expr) != 1)
        ...
        if (!(known_ranges[i].first.isa<constant_c>() &&
          ...
          if (i == static_cast<int>(src_dims.size()) - 1) {
            ...
            if (slice_expr.isa<constant>() && dst_expr.isa<constant>()) {
              ...
              if ((slice_int > dst_int && slice_int % dst_int != 0) ||
                  (dst_int > slice_int && dst_int % slice_int != 0)) {
                return slice_range_list{};
              }
            }
          }
      ...
      flatten_idx = flatten_idx + known_ranges[i].first * acc_src_dim_expr;
    ...
    sc_dim tmp_acc = 1;
    expr tmp_acc_expr = 1;
    for (int64_t i = static_cast<int64_t>(dst_dims.size()) - 1; i >= 0; i--) {
      tmp_acc *=
          !dst_dims[i].isa<constant>() ? dyn_len : get_expr_as_int(dst_dims[i]);
    ...
    for (int64_t i = static_cast<int64_t>(dst_dims.size()) - 1; i >= 0; i--) {
      if (abs(total_len) > abs(acc_dst_dim[i])) {
        reshape_ranges.emplace_back(std::make_pair(expr(0), dst_dims[i]));
      } else {
        if (i == static_cast<int64_t>(dst_dims.size()) - 1) {
          reshape_ranges.emplace_back(
              std::make_pair(flatten_idx, expr(dim2unsigned(total_len))));
        } else {
          if (!complex_case && abs(total_len) == abs(acc_dst_dim[i]) &&
            ...
            reshape_ranges.emplace_back(std::make_pair(
                ...
                expr(std::max(UINT64_C(1),
                              dim2unsigned(total_len / acc_dst_dim[i + 1])))));
          }
        }
      }
    }
...
infer_status_code tensor_view_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                       fslice_map &fsmap) {
  ...
  if (fsmap.get(get_outputs()[0]).empty()) {
    ...
    // src
    auto src_dims = info_.inputs_[0]->details_.get_blocking_dims_expr(graph);
    // dst
    auto dst_dims = info_.outputs_[0]->details_.get_blocking_dims_expr(graph);

    auto tv_slice =
        infer_tensor_view_slice(graph, known_ranges_list, src_dims, dst_dims);
...
infer_status_code
tensor_view_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                         fslice_map &fsmap) {
  if (share_gt_with_op<output_op>(get_inputs()[0])) {
    return infer_status_code::FAIL;
  }
  if (fsmap.get(get_inputs()[0]).empty()) {
    ...
    auto tv_slice =
        infer_tensor_view_slice(graph, known_ranges_list, dst_dims, src_dims);
    if (tv_slice.empty()) {
      return infer_status_code::RETRY;
    }
...
binding_axis infer_tensor_view_binding_axis(
    const binding_axis &src_axis, const sc_dims &src_dims,
    const sc_dims &dst_dims, const std::vector<int> &expand_dims = {},
  ...
  for (auto &bd_ax : src_axis) {
    ...
    for (auto &ax : bd_ax) {
      if (expand_dims.end() !=
        ...
        ret.insert(ret.end(), tv_axis_map[ax].begin(), tv_axis_map[ax].end());
      }
    }
...
void tensor_view_op_t::infer_binding_axis(binding_axis_map &bdax_map) {
  ...
  auto plain_bd_axis = infer_tensor_view_binding_axis(
      known_axis_map[0], src_plain_dims, dst_plain_dims, std::vector<int>{},
      axis_mapping);
...
void tensor_view_op_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dst,
    const std::vector<const tensor_slice *> &inputs) {}
...
infer_status_code reshape_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                       fslice_map &fsmap) {
  throw std::runtime_error("Not implemented");
}

infer_status_code reshape_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                   fslice_map &fsmap) {
  slice_range_map known_ranges_map = search_known_input_slice(this, fsmap);
...
void reshape_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  std::vector<std::vector<sc_data_format_t>> in_formats, out_formats;
...
void reshape_op_t::compute_block(
    context_ptr ctx, const std::vector<tensor_slice *> &dsts,
    const std::vector<const tensor_slice *> &inputs) {
  auto *src = inputs[0];
  auto *dst = dsts[0];
  ...
  // outer nested loop vars
  expr iters = builder::make_var(datatypes::index, std::string("_fuseiter") +
                                                       fusion_create_idx());
  ...
  for (int i = 0; i < (int)src->nslice_dims(); i++) {
    if (i == 0) {
      ...
      src_idx[i] =
          iters % src_accsize[i - 1] / src_accsize[i] + src->get_offset()[i];
    }
  }
  for (int i = 0; i < (int)dst->nslice_dims(); i++) {
    if (i == 0) {
      ...
      dst_idx[i] =
          iters % dst_accsize[i - 1] / dst_accsize[i] + dst->get_offset()[i];
    }
  }
  auto bld = builder::get_current_builder();
  COMPILE_ASSERT(bld, "No active builder is set");
  ...
  auto body =
      builder::make_stmts_unattached(std::vector<stmt_c>{std::move(cur)});
...
infer_status_code split_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                 fslice_map &fsmap) {
  // search known ranges from any input of cur fusbile op
  slice_range_map known_ranges_map = search_known_input_slice(this, fsmap);
  if (known_ranges_map.empty())
      ...
      for (size_t j = 0; j < split_ranges_list.at(n).size(); j++) {
        if (j == dim_) {
          ...
          fsmap.get(get_outputs()[i])
              .at(n)
              .emplace_back(
                  std::make_pair(expr(0), dim2unsigned(shapes_.at(i))));
        } else {
          fsmap.get(get_outputs()[i])
              .at(n)
              .emplace_back(
                  std::make_pair(split_ranges_list.at(n).at(j).first,
...
infer_status_code split_op_t::pre_infer_slice_ranges(const context_ptr &ctx,
                                                     fslice_map &fsmap) {
  throw std::runtime_error("Not implemented");
}
