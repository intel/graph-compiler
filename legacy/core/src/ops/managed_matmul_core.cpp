...
namespace gc {
namespace ops {
static sc_data_type_t
infer_out_dtype(const std::vector<graph_tensor_ptr> &ins) {
...
managed_matmul_core_op_t::managed_matmul_core_op_t(
  ...
  COMPILE_ASSERT(info_.inputs_.size() == 2,
                 "managed_matmul_core expects 2 inputs");
...
std::vector<int>
managed_matmul_core_op_t::query_prefetch(const context_ptr &ctx, bool is_global,
  ...
  if (gen_ptr->is_okay_to_prefetch(
          ctx, *config_data_.get_as<managed_matmul_core_config_t>(),
          is_global)) {
    return {1};
  } else {
    return {};
...
void managed_matmul_core_op_t::generate_prefetcher_body_for_tensor(
  ...
  static_cast<gen_managed_matmul_core_t *>(gen.get())
      ->generate_prefetcher_body_for_tensor(
          ctx, *config_data_.get_as<managed_matmul_core_config_t>(), func_args,
          ins, indices);
}
...
body_generator_ptr managed_matmul_core_op_t::create_generator() {
  ...
  bool is_valid_int8 = (info_.inputs_[0]->details_.dtype_ == datatypes::u8 ||
  ...
  auto mat_gen = utils::make_unique<gen_managed_matmul_core_t>(
  ...
  if (iim_block_ != -1) {
    mat_ptr->iim_block_ = iim_block_;
  }
  if (iin_block_ != -1) {
    mat_ptr->iin_block_ = iin_block_;
  }
  if (iik_block_ != -1) {
    mat_ptr->iik_block_ = iik_block_;
  }
  if (is_dynamic()) {
...
void managed_matmul_core_op_t::query_format(
    context_ptr ctx,
        ...
        for (auto A_isp : is_padding) {   // A is_padding
          for (auto B_isp : is_padding) { // B is_padding
            if (is_dynamic_dim(M)) {
              iim_block = m_b;
              if (treat_as_static) {
                iim_block = iim_block_;
              }
            }
            if (is_dynamic_dim(N)) {
              iin_block = n_b;
              if (treat_as_static) {
                iin_block = iin_block_;
              }
            }
            if (is_dynamic_dim(K)) {
              iik_block = k_b;
              if (treat_as_static) {
                iik_block = iik_block_;
              }
            }
            if (A_dims.size() == 2) {
              if (constant_A ||
                  (!dynamic && A_format.is_blocking() &&
                   p2bmp_a.at(0).size() > 1 && p2bmp_a.at(1).size() > 1) ||
                  A_isp || (!dynamic && M % iim_block) ||
                  (!dynamic && K % iik_block) ||
                  (!dynamic && is_A_vnni_low_fp &&
                   A_format == sc_data_format_t::NK())) {
                ret_A_format = sc_data_format_t::MKmk(iim_block, iik_block);
              } else {
              ...
              COMPILE_ASSERT(0, "managed_matmul_core only supports 2d "
                                "yet");
            }
            if (B_dims.size() == 2) {
              if (utils::is_one_of(B_dtype, datatypes::u8, datatypes::s8)) {
                ret_B_format = sc_data_format_t::NKkn4k(iik_block, iin_block);
              } else if (is_B_vnni_low_fp) {
                // do vnni reorder in template for
                // transposed matmul
                if (!dynamic) {
                  ret_B_format = sc_data_format_t::NKkn2k(iik_block, iin_block);
                }
              } else {
                if (constant_B || B_isp ||
                    (!dynamic && B_format.is_blocking() &&
                     p2bmp_b.at(0).size() > 1 && p2bmp_b.at(1).size() > 1) ||
                    (!is_dynamic_dim(K) && K % iik_block) ||
                    (!is_dynamic_dim(N) && N % iin_block)) {
                  ret_B_format = sc_data_format_t::NKkn(iik_block, iin_block);
                } else {
            ...
            if (M == iim_block && M >= 32 && N % iin_block == 0 && !dynamic) {
              ...
              ret_C_format = sc_data_format_t(
                  sc_data_format_kind_t::get_2dblocking_by_dims(C_dims.size(),
                                                                false),
                  {iim_block, iin_block});
              if (!dynamic) {
...
void managed_matmul_core_op_t::set_config_by_key(const op_dispatch_key_t &key,
...
ir_module_ptr
managed_matmul_core_op_t::get_internal_func(const context_ptr &ctx) {
  assert(is_dynamic());
  ...
  const auto &in_details =
      info_.cur_impl_ == mmm_impl_kind_t::is_partial
          ? graph::extract_detail_from_tensors(get_inputs())
          : info_.internal_info_->parti_in_ltsrs_;
  auto pouts = graph::tensor_detail_to_ir_tensor(get_owner_graph(), "__outs_",
                                                 out_details);
  auto pins = graph::tensor_detail_to_ir_tensor(get_owner_graph(), "__ins_",
                                                in_details);
  ...
  bool status = gen_ptr->generate(ctx, config_data_.data_.get(), nullptr, pins,
                                  pouts, loops);
  assert(status);
...
sc_op_ptr
managed_matmul_core_op_t::copy(const std::vector<graph_tensor_ptr> &ins,
...
sc_op_ptr managed_matmul_core_op_t::do_compensations(sc_graph_t &mgr,
                                                     const context_ptr &ctx) {
...
sc_op_ptr managed_matmul_core_op_t::get_constant_compensation(sc_graph_t &mgr) {
  ...
  if (is_dyn_quan) {
    COMPILE_ASSERT(
        ...
        "points compensation yet");
    ret_node =
        mgr.make("mul", {dyn_data_zero_points, dyn_weight_zero_points}, {}, {});
    auto const_reduce =
        mgr.make("constant", {}, {},
                 {{"dtype", datatypes::s32},
...
std::vector<int>
managed_matmul_core_op_t::get_impl_dispatch_candidates(const context_ptr &ctx) {
  return std::vector<int>{mmm_impl_kind_t::full_k, mmm_impl_kind_t::is_partial};
}
