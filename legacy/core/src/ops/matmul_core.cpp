...
blocking_axis_t get_mm_blocking_axis(const logical_tensor_t &inp,
                                     const logical_tensor_t &wei,
                                     const logical_tensor_t &out) {
  ...
  blocking_axis.A_bs =
      transform_axis_plain2blocking(inp, generate_axis_by_num(A_dim_size - 2));
  blocking_axis.A_m =
      transform_axis_plain2blocking(inp, std::vector<int>{A_dim_size - 2});
  blocking_axis.A_k =
      transform_axis_plain2blocking(inp, std::vector<int>{A_dim_size - 1});
  blocking_axis.B_bs =
      transform_axis_plain2blocking(wei, generate_axis_by_num(B_dim_size - 2));
  blocking_axis.B_k =
      transform_axis_plain2blocking(wei, std::vector<int>{B_dim_size - 2});
  blocking_axis.B_n =
      transform_axis_plain2blocking(wei, std::vector<int>{B_dim_size - 1});
  blocking_axis.C_bs =
      transform_axis_plain2blocking(out, generate_axis_by_num(C_dim_size - 2));
  blocking_axis.C_m =
      transform_axis_plain2blocking(out, std::vector<int>{C_dim_size - 2});
  blocking_axis.C_n =
      transform_axis_plain2blocking(out, std::vector<int>{C_dim_size - 1});
...
matmul_core_op_t::matmul_core_op_t(const std::vector<graph_tensor_ptr> &ins,
  ...
  sc_dims expected_out_shape = {merge_vec(
      batch_dims_, {A_dims[A_dims.size() - 2], B_dims[B_dims.size() - 1]})};

  if (info_.outputs_.empty()) {
    info_.outputs_.emplace_back(std::make_shared<graph_tensor>(
        this, sc_data_format_t(), expected_out_shape, infer_out_dtype(ins)));
  } else {
    COMPILE_ASSERT(info_.outputs_.size() == 1, "matmul_core expects 1 output");
    if (!is_dynamic()) {
...
body_generator_ptr matmul_core_op_t::create_generator() {
  auto mat_gen = utils::make_unique<gen_matmul_core_t>(
      this, graph::extract_detail_from_tensors(get_inputs()),
      graph::extract_detail_from_tensors(get_outputs()));
  return std::move(mat_gen);
}

float matmul_core_op_t::get_gflop() { return create_generator()->get_gflop(); }
...
sc_dims matmul_core_op_t::get_batch_dims_impl(const sc_dims &A_dims,
                                              const sc_dims &B_dims) {
...
sc_dims
matmul_core_op_t::get_batch_dims_with_bc_impl(const sc_dims &A_plain_dims,
                                              const sc_dims &B_plain_dims) {
  ...
  for (int i = dims_diff; i <= num_dims - 3; ++i) {
    if (long_dims[i] == short_dims[i - dims_diff]) {
      ...
      if (long_dims[i] == 1 && short_dims[i - dims_diff] != 1) {
        ...
        COMPILE_ASSERT(false, "Can not broadcast in batch dims properly");
      }
    }
  }
  return C_batch_dims;
...
sc_data_type_t
matmul_core_op_t::infer_out_dtype(const std::vector<graph_tensor_ptr> &ins) {
...
void matmul_core_op_t::query_format(
    context_ptr ctx,
  ...
  if (info_.inputs_[0]->producer_owner_->isa<constant_op_t>() ||
    ...
    for (const auto &input : info_.inputs_[0]->producer_owner_->get_inputs()) {
      auto parent_node = input->producer_owner_;
      constant_A_parents &=
          (parent_node->attrs_.get_or_else("constant", const_kind::not_const) ||
  ...
  if (info_.inputs_[1]->producer_owner_->isa<constant_op_t>() ||
    ...
    for (const auto &input : info_.inputs_[1]->producer_owner_->get_inputs()) {
      auto parent_node = input->producer_owner_;
      constant_B_parents &=
          (parent_node->attrs_.get_or_else("constant", const_kind::not_const) ||
           parent_node->isa<constant_op_t>());
      ...
      for (auto &k_b : blk_candidates) {             // K
        for (auto A_isp : is_padding) {              // A is_padding
          for (auto B_isp : is_padding) {            // B is_padding
            for (auto out_plain : is_output_plain) { // output plain, always
              // false in dynamic
              if (is_dynamic_dim(M_block) || is_dynamic_dim(N_block)) {
                if (is_dynamic_dim(M_block)) {
                  A_m_blk = C_m_blk = m_b;
                  COMPILE_ASSERT(!constant_A, "if M is dynamic, input A should "
                                              "not "
                                              "be constant!");
                } else {
                ...
                if (is_dynamic_dim(N_block)) {
                  B_n_blk = C_n_blk = n_b;
                  COMPILE_ASSERT(!constant_B, "if N is dynamic, input B should "
                                              "not "
                                              "be constant!");
                } else {
              ...
              if (constant_A || block_A || A_isp ||
                  (A_dims.size() > 2 && !A_format.is_plain()) // follow
                  // original
                  // logic
                ...
                // should be blocking
                if (A_dims.size() == 2) {
                  ret_A_format = sc_data_format_t::MKmk(A_m_blk, A_k_blk);
                } else {
                  // regular ND*ND matmul (non-batch
                  // format) whether constant and no
                  // transA
                  auto A_formt_m_blk =
                      transposed_a ? A_format.blocks_[1] : A_format.blocks_[0];
                  auto A_formt_k_blk =
                      transposed_a ? A_format.blocks_[0] : A_format.blocks_[1];
                  if (A_formt_m_blk != A_m_blk || A_formt_k_blk != A_k_blk ||
              ...
              if (utils::is_one_of(B_dtype, datatypes::u8, datatypes::s8)) {
                if (B_dims.size() == 2) {
                  ret_B_format = sc_data_format_t::NKkn4k(B_k_blk, B_n_blk);
                } else {
                  ret_B_format = sc_data_format_t(
                      sc_data_format_kind_t::get_2dblocking_by_dims(
                          B_dims.size(), true, true),
                      {B_k_blk, B_n_blk, 4});
                }
              } else if (is_B_vnni_low_fp) {
                if (B_dims.size() == 2) {
                  ret_B_format = sc_data_format_t::NKkn2k(B_k_blk, B_n_blk);
                } else {
                  ret_B_format = sc_data_format_t(
                      sc_data_format_kind_t::get_2dblocking_by_dims(
                          B_dims.size(), true, true),
                ...
                auto B_format_kind =
                    sc_data_format_kind_t::get_2dblocking_by_dims(B_dims.size(),
                                                                  true);
                if (constant_B || block_B || B_isp ||
                    (B_dims.size() > 2 && !B_format.is_plain()) ||
                    (!dynamic && B_format.is_blocking()) ||
                    (!is_dynamic_dim(K) && K % K_block) ||
                    (!is_dynamic_dim(N) && N % N_block)) {
                  // should be blocking
                  if (B_dims.size() == 2) {
                    ret_B_format = sc_data_format_t::NKkn(B_k_blk, B_n_blk);
                  } else {
                    // regular ND*ND matmul (non-batch
                    // format) whether constant and no
                    // transA
                    auto B_formt_k_blk = transposed_b ? B_format.blocks_[1]
                                                      : B_format.blocks_[0];
                    auto B_formt_n_blk = transposed_b ? B_format.blocks_[0]
                                                      : B_format.blocks_[1];
                    if (B_formt_k_blk != B_k_blk || B_formt_n_blk != B_n_blk ||
                        B_format.format_code_.get(B_dims.size() - 1) ==
                            static_cast<int>(B_dims.size() - 2)) {
                      ret_B_format =
                          sc_data_format_t(B_format_kind, {B_k_blk, B_n_blk});
                    } else {
              ...
              if (((!constant_A && !constant_B && !dynamic &&
                ...
                if (C_dims.size() == 2) {
                  ret_C_format = sc_data_format_t::MKmk(C_m_blk, C_n_blk);
                } else {
                  // regular ND*ND matmul (non-batch
                  // format)
              ...
              std::vector<std::vector<sc_dim>> var_block = {
                  {A_m_blk, A_k_blk}, {B_k_blk, B_n_blk}, {C_m_blk, C_n_blk}};
            ...
            if (!is_dynamic_dim(K) && !is_dynamic_dim(N)) {
              break;
            }
          }
          // break is A padding loop if it is static
          if (!is_dynamic_dim(M) && !is_dynamic_dim(K)) {
            break;
          }
        }
...
std::vector<int>
matmul_core_op_t::get_impl_dispatch_candidates(const context_ptr &ctx) {
  return {};
}

sc_op_ptr matmul_core_op_t::do_compensations(sc_graph_t &mgr,
                                             const context_ptr &ctx) {
...
sc_op_ptr matmul_core_op_t::get_data_compensation(sc_graph_t &mgr) {
  ...
  if (is_dyn_quan && !dyn_weight_zero_points) {
    return nullptr;
  }
  ...
  if (is_dyn_quan) {
    ...
    if (weight_zero_points.size() == 1) {
      ...
      // per channel
      COMPILE_ASSERT(0, "matmul_core does not support per channel weight zero "
                        "points "
                        "compensation yet");
  ...
  if (data->details_.get_plain_dims().size() < batch_dims_.size() + 2) {
    sc_dims unsqueeze_shape(
        batch_dims_.size() + 2 - data->details_.get_plain_dims().size(), 1);
    ...
    auto reshape_node = mgr.make(
        "tensor_view", mul_node->get_outputs(),
...
matmul_core_op_t::get_s8s8_and_weight_compensation(sc_graph_t &mgr,
  ...
  if (weight_compensation) {
    if (is_dyn_quan) {
      ...
      if (data_zero_points.size() == 1) {
        ...
        // per channel
        COMPILE_ASSERT(0, "matmul_core does not support per channel data zero "
                          "points "
                          "compensation yet");
        auto data = info_.inputs_[0];
    ...
    if (weight->details_.get_plain_dims().size() < batch_dims_.size() + 2) {
      sc_dims unsqueeze_shape(
          batch_dims_.size() + 2 - weight->details_.get_plain_dims().size(), 1);
      sc_dims reshape_dest =
          merge_vec(unsqueeze_shape, weight->details_.get_plain_dims());
      reshape_dest.at(reshape_dest.size() - 2) = 1;
      auto reshape_fmt = info_.outputs_[0]->details_.get_format();
      nodes[0] = mgr.make(
          "tensor_view", nodes[0]->get_outputs(),
          {graph_tensor::make(reshape_dest, sc_data_format_t(),
  ...
  if (s8s8_compensation) {
    ...
    if (weight->details_.get_plain_dims().size() < batch_dims_.size() + 2) {
      ...
      auto reshape_fmt = info_.outputs_[0]->details_.get_format();
      nodes[1] = mgr.make(
          "tensor_view", nodes[1]->get_outputs(),
          {graph_tensor::make(reshape_dest, sc_data_format_t(),
...
sc_op_ptr matmul_core_op_t::get_constant_compensation(sc_graph_t &mgr) {
  ...
  if (is_dynamic_dim(K_orig)) {
    ...
    int shape_idx =
        static_cast<int>(weight->details_.get_plain_dims().size() - 2);
    auto reduce_shape =
        mgr.make("shape_of_tensor", {weight}, {},
                 {{"shape_idx", shape_idx},
                  {attr_keys::padding_shape_type,
                   static_cast<int>(padding_shape_etype_t::matmul_padding)}});
    ret_node = mgr.make(
        "mul", {ret_node->get_outputs()[0], reduce_shape->get_outputs()[0]}, {},
        {});
  } else {
    if (is_dyn_quan) {
      COMPILE_ASSERT(
          ...
          "points compensation yet");
      ret_node = mgr.make("mul", {dyn_data_zero_points, dyn_weight_zero_points},
                          {}, {});
      auto const_reduce = mgr.make(
          "constant", {}, {},
          {{"dtype", datatypes::s32},
           {"values", std::make_shared<static_data_t>(&K, sizeof(int))},
           {"plain_dims", sc_dims{1}},
           {"format", sc_data_format_t()},
           {"temp.val/var", 1},
      ...
      ret_node = mgr.make(
          ...
          {{"values", std::make_shared<static_data_t>(std::vector<int>{
           ...
           {"plain_dims", sc_dims{1}},
           {"format", sc_data_format_t()},
           {"temp.val/var", data_zero_points[0] * weight_zero_points[0]},
           {"temp.var", attrs_["temp.padded_A_K"]}});
    }
  }
  return ret_node;
...
infer_status_code matmul_core_op_t::infer_slice_ranges(const context_ptr &ctx,
                                                       fslice_map &fsmap) {
  ...
  auto inp_dims_expr = get_inputs()[0]->details_.get_blocking_dims_expr(graph);
  auto wei_dims_expr = get_inputs()[1]->details_.get_blocking_dims_expr(graph);
  auto out_dims_expr = get_outputs()[0]->details_.get_blocking_dims_expr(graph);

  auto batch_dims_size = batch_dims_.size();

  slice_range inp_slice, wei_slice, out_slice;
  blocking_axis_t blocking_axis =
      get_mm_blocking_axis(get_inputs()[0]->details_, get_inputs()[1]->details_,
                           get_outputs()[0]->details_);
  if (!known_ranges_map[0].empty()) {
    ...
    std::vector<int> required_axis =
        M_axis_fuse ? std::vector<int>{blocking_axis.A_m.begin() + 1,
  ...
  if (!known_ranges_map[0].empty() && known_ranges_map[1].empty()) {
    ...
    for (int64_t i = static_cast<int64_t>(wei_size) - 1; i >= 0; i--) {
      if (std::find(blocking_axis.B_bs.begin(), blocking_axis.B_bs.end(), i) !=
          blocking_axis.B_bs.end()) {
        int bs_idx_inp =
            blocking_axis.A_bs[blocking_axis.A_bs.size() - 1 - bs_cnt];
        ...
        if (inp_dims[bs_idx_inp] < wei_dims[i]) {
          ...
          COMPILE_ASSERT(wei_dims[i] == 1, "broadcast weight is expected")
  ...
  if (known_ranges_map[0].empty() && !known_ranges_map[1].empty()) {
    ...
    for (int64_t i = static_cast<int64_t>(inp_size) - 1; i >= 0; i--) {
      if (std::find(blocking_axis.A_bs.begin(), blocking_axis.A_bs.end(), i) !=
          blocking_axis.A_bs.end()) {
        int bs_idx_wei =
            blocking_axis.B_bs[blocking_axis.B_bs.size() - 1 - bs_cnt];
        // explicit broadcast semantic
        if (wei_dims[bs_idx_wei] < inp_dims[i]) {
          ...
          COMPILE_ASSERT(inp_dims[i] == 1, "broadcast input is expected")
    ...
    for (auto &bd_axis : inp_axis) {
      ...
      for (auto &ax : bd_axis) {
        ...
        if (distance_from_right == 0) {
          ...
          ret_w.emplace_back(wei_plain_dims.size() - 1 - distance_from_right);
        }
        // bind output axis
        if (distance_from_right == 1) {
          ret_o.emplace_back(out_plain_dims.size() - 2);
        } else if (distance_from_right > 1) {
          ret_o.emplace_back(out_plain_dims.size() - 1 - distance_from_right);
        }
      }
    ...
    for (auto &bd_axis : wei_axis) {
      ...
      for (auto &ax : bd_axis) {
        ...
        if (distance_from_right == 1) {
          ...
          ret_i.emplace_back(inp_plain_dims.size() - 1 - distance_from_right);
        }
        // bind output axis
        if (distance_from_right == 0) {
  ...
  for (size_t i = 0; i < cur->get_inputs().size(); i++) {
    ...
    if (inpaxis.empty()) {
      ...
      for (auto &bd_axis : outaxis) {
        std::vector<int> ret;
        for (auto &ax : bd_axis) {
          COMPILE_ASSERT(ax < static_cast<int64_t>(out_plain_dims.size()),
                         "matmul core output binded axis could not exceed "
                         "plain dims size: "
                             << out_plain_dims.size() << ", but got " << ax)
          int distance_from_right =
              static_cast<int64_t>(out_plain_dims.size()) - 1 - ax;
          // bind input/weight axis
          if (distance_from_right == (1 - static_cast<int64_t>(i))) {
            ret.emplace_back(in_plain_dims.size() - 1 - distance_from_right);
          } else if (distance_from_right > 1 &&
                     distance_from_right <
                         static_cast<int64_t>(in_plain_dims.size())) {
            ret.emplace_back(in_plain_dims.size() - 1 - distance_from_right);
          }
        }
