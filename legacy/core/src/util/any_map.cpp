...
reflection::general_ref_t any_to_general_ref(const any_t &v) {
  ...
  COMPILE_ASSERT(ty, "Cannot find the type in reflection for any_t: "
                         << v.type_code()->name());
...
void any_vtable_t::set_rtti_to_vtable_map(const std::type_info *typeinfo,
                                          any_vtable_t *table) {
...
void any_t::create_buffer(const any_detail::any_vtable_t *vt) {
  if (vt) {
    if (vt->size_ > INLINE_BUFFER_SIZE)
      data_.ptr_ = new char[vt->size_];
  }
  vtable_ = vt;
}
...
any_t any_t::make_by_type(const reflection::type *ty) {
  any_t ret;
  if (ty->base_ == reflection::basic_type::t_string && ty->array_depth_ == 0) {
    ret = std::string();
  } else {
    ...
    if (ty->meta_) {
      ty->meta_->constructor_(ret.get_raw());
    }
  }
  return ret;
}
...
void any_t::copy_from(const void *data, const any_detail::any_vtable_t *vt) {
  ...
  if (switch_buffer_to_type(vt)) {
    if (vtable_)
      vtable_->copy_assigner_(get_raw(), (void *)data);
  } else {
    if (vtable_)
      vtable_->copy_ctor_(get_raw(), (void *)data);
  }
}
...
void any_t::move_from(void *data, const any_detail::any_vtable_t *vt) {
  ...
  if (switch_buffer_to_type(vt)) {
    if (vtable_)
      vtable_->move_assigner_(get_raw(), data);
  } else {
    if (vtable_)
      vtable_->move_ctor_(get_raw(), data);
  }
}
...
void any_t::move_from_any(any_t &&v) {
  ...
  if (v.vtable_) {
    ...
    if (vtable_->size_ > INLINE_BUFFER_SIZE) {
      ...
      // instead
      vtable_->move_ctor_(&data_.inlined_buffer_, &v.data_.inlined_buffer_);
    }
  }
}
...
any_t &any_map_t::get_any(const std::string &v) {
  ...
  COMPILE_ASSERT(itr != impl_.end(),
                 "Cannot find the key " << v << " in the map");
  return itr->second;
}
