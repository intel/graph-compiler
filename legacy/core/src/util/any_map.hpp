...
struct any_vtable_t {
  ...
  any_vtable_t(size_t size, const std::type_info &typeinfo,
      ...
      : size_(size), typeinfo_(typeinfo), destructor_(destructor),
        move_assigner_(move_assigner), move_ctor_(move_ctor),
        copy_assigner_(copy_assigner), copy_ctor_(copy_ctor) {
    set_rtti_to_vtable_map(&typeinfo_, this);
  }

  SC_API static void set_rtti_to_vtable_map(const std::type_info *typeinfo,
                                            any_vtable_t *);
  SC_API static any_vtable_t *
  get_vtable_by_rtti(const std::type_info *typeinfo);
};

template <typename T> struct registry;

// if is_move_assignable, use move
template <bool movable, typename T> struct move_assign_impl_t {
...
template <typename T> struct move_assign_impl_t<false, T> {
  static constexpr void (*call)(void *ths, void *other) = nullptr;
};

// if is_move_constructible, use move
template <bool movable, typename T> struct move_constru_impl_t {
  static inline void call(void *ths, void *other) {
    T *oth = reinterpret_cast<T *>(other);
...
template <typename T> struct move_constru_impl_t<false, T> {
  static constexpr void (*call)(void *ths, void *other) = nullptr;
};

// if is_copy_assignable, use copy
template <bool copyable, typename T> struct copy_assign_impl_t {
...
template <typename T> struct copy_assign_impl_t<false, T> {
  static constexpr void (*call)(void *ths, const void *other) = nullptr;
};

// if is_copy_constructible, use copy
template <bool copyable, typename T> struct copy_constru_impl_t {
  static inline void call(void *ths, const void *other) {
    const T *oth = reinterpret_cast<const T *>(other);
    new (ths) T(*oth);
  }
...
template <typename T> struct copy_constru_impl_t<false, T> {
  static constexpr void (*call)(void *ths, const void *other) = nullptr;
};

template <typename T> struct destructor_impl_t {
...
template <typename T, std::size_t N> struct destructor_impl_t<T[N]> {
...
template <typename T>
SC_API any_vtable_t registry<T>::vtable(
    sizeof(T), typeid(T), destructor_impl_t<T>::destructor,
...
template <typename T> struct assign_impl;
...
private:
  ...
  template <class T> friend struct any_detail::assign_impl;
...
public:
  ...
  template <typename T, typename T1> bool match(T defaults, T1 func1) const {
    using MatchedT = any_detail::first_argument<T1>;
    if (isa<MatchedT>()) {
      func1(get<MatchedT>());
      return true;
  ...
  template <typename T> any_t(T &&v) { *this = std::forward<T>(v); }
  ...
  template <typename T> any_t &operator=(T &&v) {
  ...
  template <typename T> void set(const T &v) { *this = v; }

  template <typename T> static any_detail::any_vtable_t *get_vtable() {
    return any_detail::registry<typename std::decay<T>::type>::get_vtable();
  }

  template <typename T> T *get_or_null() {
    if (!isa<T>()) {
      return nullptr;
    }
  ...
  template <typename T> const T *get_or_null() const {
    return const_cast<any_t *>(this)->get_or_null<T>();
  }

  template <typename T> T &get() {
    COMPILE_ASSERT(isa<T>(), "Incorrect type for any_t::get, this = "
  ...
  template <typename T> bool isa() const { return vtable_ == get_vtable<T>(); }
...
template <> struct assign_impl<any_t> {
...
public:
  any_map_t() = default;
  any_map_t(const std::unordered_map<std::string, any_t> &impl) : impl_(impl) {}
  any_map_t(std::initializer_list<std::pair<const std::string, any_t>> init)
      : impl_(init) {}

  size_t size() const { return impl_.size(); };

  const std::unordered_map<std::string, any_t> &as_map() const { return impl_; }
  ...
  template <typename T> T &get(const std::string &v) {
    return get_any(v).get<T>();
  };

  template <typename T> const T &get(const std::string &v) const {
    return get_any(v).get<T>();
  };
  ...
  template <typename T> T *get_or_null(const std::string &k) {
    if (!has_key(k)) {
      return nullptr;
    }
    return &get_any(k).get<T>();
  ...
  // gets the pointer of a value. if key is not found, returns nullptr
  template <typename T> const T *get_or_null(const std::string &k) const {
    if (!has_key(k)) {
      return nullptr;
    }
    return &get_any(k).get<T>();
  ...
  any_t &operator[](const std::string &v) {
    if (!has_key(v)) {
      impl_.insert(std::make_pair(v, any_t()));
    }
    return get_any(v);
  }
  /**
  ...
  template <typename T>
  inline static const T fetch_or_else(const any_map_t *v, const std::string &k,
                                      const T &defaultv) {
    if (!v) {
      return defaultv;
    }
  ...
  template <typename T>
  inline static const T *fetch_or_null(const any_map_t *v,
                                       const std::string &k) {
    if (!v) {
      return nullptr;
    }
