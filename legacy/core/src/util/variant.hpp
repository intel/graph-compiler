...
using std::size_t;
template <size_t arg1, size_t... args> struct const_max;
...
template <typename T, typename... Args> struct helper<T, Args...> {
  ...
  template <typename T2, bool is_same> struct handle_get_id {
    static constexpr size_t call() { return id; }
    static constexpr bool has_type() { return true; }
  };

  template <typename T2> struct handle_get_id<T2, false> {
    static constexpr size_t call() { return inner::template get_id<T2>(); }
    static constexpr bool has_type() { return inner::template has_type<T2>(); }
  };

  template <typename T2> static constexpr size_t get_id() {
    return handle_get_id<T2, std::is_same<T2, T>::value>::call();
  }

  template <typename T2> static constexpr bool has_type() {
  ...
  template <typename T2, bool is_T2_parent_of_T> struct handle_as {
  ...
  template <typename T2> struct handle_as<T2, false> {
  ...
  template <typename T2> static T2 *as(size_t curid, void *data) {
  ...
  template <typename T2, bool is_T_convertible_to_T2> struct handle_cast {
    // case when is_T2_parent_of_T=true
  ...
  template <typename T2> struct handle_cast<T2, false> {
    // case when is_T2_parent_of_T=false
    static T2 call(size_t curid, const void *data) {
      return inner::template cast<T2>(curid, data);
    }
  ...
  template <typename T2> static T2 cast(size_t curid, const void *data) {
    return handle_cast<T2, std::is_convertible<T, T2>::value>::call(curid,
                                                                    data);
  }
};
...
template <> struct helper<> {
  ...
  template <typename T2> static constexpr bool has_type() { return false; }
...
template <typename T, typename variantT> struct copy_or_move_handler {
...
template <typename variantT> struct copy_or_move_handler<variantT, variantT> {
...
private:
  ...
  using copy_or_move_handler =
      variant_impl::copy_or_move_handler<typename std::decay<T>::type, variant>;
...
public:
  ...
  template <typename T> bool isa() const { return id_ == get_id_of_type<T>(); }

  // get the type index of the stored value. returns 0 if it is empty.
  size_t get_id() const { return id_; }

  // get the type index of a type in candidates
  template <typename T> static constexpr size_t get_id_of_type() {
  ...
  template <typename T> const T *as_or_null() const {
  ...
  // stored type. Otherwise, return null. Note that if the stored type is X,
  // it returns a pointer to X
  template <typename T> T *as_or_null() {
