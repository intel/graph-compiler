...
template <typename T> struct pointer_like_optional_impl {
...
template <typename T> struct extract_nested_optional {};

template <typename T> struct extract_nested_optional<optional<T>> {
  using internal_t = T;
};

template <typename T, typename Func> struct extract_mapper_func_ret {
  using type = typename std::decay<decltype(std::declval<Func>()(
      std::declval<T>()))>::type;
};
...
template <typename TOptional, bool copyable> struct handle_copyable {
...
template <typename TOptional> struct handle_copyable<TOptional, false> {
  handle_copyable() = default;
...
struct SC_EMPTY_BASE_OPTIMIZE optional
    : protected optional_impl::optional_base<T>,
      protected optional_impl::handle_copyable<
          optional<T>, std::is_copy_constructible<T>::value> {
private:
  ...
  template <
      ...
      typename Dummy = typename std::enable_if<
          copyable && std::is_same<optionalT, optional>::value, int>::type>
...
public:
  ...
  optional<Ret> map(Func &&f) const {
    if (has_value()) {
      return optional<Ret>(f(*get_impl()));
    }
    return none_opt{};
  }
  ...
  template <typename Func,
            typename Ret = typename optional_impl::extract_nested_optional<
                typename optional_impl::mapper_func_ret_t<T, Func>>::internal_t>
   ...
   * @return optional<T>
   */
  template <typename Func> optional or_else(Func &&f) && {
    if (has_value()) {
   ...
   * @return optional<T>
   */
  template <typename Func> optional filter(Func &&f) && {
    if (has_value() && f(*get_impl())) {
      return std::move(*this);
    }
    return none_opt{};
  }
...
template <typename T> optional<typename std::decay<T>::type> some_opt(T &&v) {
