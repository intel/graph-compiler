...
template <typename T, typename Dummy> struct type_registry {
public:
...
#define SC_REFLECT_REG_TYPE(ty)                                                \
  template <> struct type_registry<ty, int> {                                  \
    static constexpr unsigned depth = 0;                                       \
...
template <typename T> struct type_registry<std::vector<T>> {
  static_assert(!std::is_same<T, bool>::value,
                "Reflection do not support std::vector<bool>");
  SC_ARRAY_LIKE_FIELDS(T)
};

template <typename T, std::size_t N> struct type_registry<std::array<T, N>> {
  SC_ARRAY_LIKE_FIELDS(T)
};

template <typename T, std::size_t N> struct type_registry<T[N]> {
  SC_ARRAY_LIKE_FIELDS(T)
};
...
template <typename T> struct field : public field_base_t {
  ...
  using copier =
      typename any_detail::copy_assign_impl_t<std::is_copy_assignable<T>::value,
                                              T>;
...
template <typename T> struct type_name_trait_t<T, false> {
...
template <typename T> struct container_impl;

template <typename T> struct container_impl<std::vector<T>> {
...
template <typename T, std::size_t N> struct container_impl<std::array<T, N>> {
...
struct container_impl<T[N]> : container_impl<std::array<T, N>> {
  ...
  using impl_t::size;
  static void name(std::stringstream &ss) { ss << "na[" << N; } // native array
};

// create vector_metadata for container<T>
template <typename VecT>
vector_metadata create_vector_meta_impl(const std::type_info *rtti,
                                        vector_metadata *ptr) {
  vector_metadata v;
  using T =
      typename std::remove_reference<decltype(std::declval<VecT>()[0])>::type;
  ...
  ss << ']';
  v.name_ = ss.str();
  set_rtti_map_to_type(
      rtti, reflection::type{type_registry<T>::base, v.array_depth_, ptr},
      &v.name_);
  return v;
}
...
vector_metadata create_vector_meta(vector_metadata *ptr) {
  static auto shared =
      std::shared_ptr<vector_metadata>(ptr, dummy_class_metadata_deleter);
...
#define SC_VECTOR_TYPE_DEF2(...)                                               \
  type type_registry<__VA_ARGS__>::type_ = {                                   \
      type_registry<__VA_ARGS__>::base, type_registry<__VA_ARGS__>::depth,     \
      type_registry<__VA_ARGS__>::metadata()}

template <typename T> SC_VECTOR_TYPE_DEF(std::vector<T>);
template <typename T> SC_VECTOR_TYPE_DEF2(std::vector<T>);

template <typename T, std::size_t N> SC_VECTOR_TYPE_DEF(std::array<T, N>);
template <typename T, std::size_t N> SC_VECTOR_TYPE_DEF2(std::array<T, N>);

template <typename T, std::size_t N> SC_VECTOR_TYPE_DEF(T[N]);
template <typename T, std::size_t N> SC_VECTOR_TYPE_DEF2(T[N]);
...
type type_registry<T, Dummy>::type_ = {type_registry<T, Dummy>::base,
                                       type_registry<T, Dummy>::depth,
                                       type_registry<T, Dummy>::metadata()};
...
template <typename T> struct class_builder_t {
  ...
  class_builder_t(class_metadata *ptr, const char *name) {
    ...
    static auto meta_shared_ptr =
        std::shared_ptr<class_metadata>(ptr, dummy_class_metadata_deleter);
   ...
   * */
  template <void (T::*method)()> class_builder_t &init() {
   ...
   * */
  template <typename FT> class_builder_t &field(const char *name, FT T::*ptr) {
    ...
    metadata_.fields_.emplace_back(
        utils::make_unique<reflection::field<FT>>(name, std::move(field_m)));
...
#define SC_CLASS_WITH_NAME(IDENTIFIER, ...)                                    \
  ...
  static_assert(                                                               \
      std::is_same<                                                            \
          __namespace_checker_##IDENTIFIER,                                    \
          ::dnnl::impl::graph::gc::__namespace_checker_##IDENTIFIER>::value,   \
      "SC_CLASS macro should be used in sc namespace!");                       \
  namespace reflection {                                                       \
  template <>                                                                  \
  SC_INTERNAL_API class_metadata *type_registry<__VA_ARGS__, int>::metadata(); \
  void *__reflection_init_##IDENTIFIER =                                       \
      &type_registry<__VA_ARGS__, int>::type_;                                 \
  template <> class_metadata *type_registry<__VA_ARGS__, int>::metadata() {    \
    using TClass = __VA_ARGS__;                                                \
    static class_metadata meta = class_builder_t<TClass>(&meta, #__VA_ARGS__)
...
#define SC_REG_ENUM(NAME)                                                      \
  namespace reflection {                                                       \
  template <> struct type_name_trait_t<NAME, true> {                           \
    static constexpr const char *name(const vector_metadata &v) {              \
      static_assert(std::is_enum<NAME>::value, #NAME " should be an enum");    \
      return #NAME;                                                            \
    }                                                                          \
  };                                                                           \
  }
