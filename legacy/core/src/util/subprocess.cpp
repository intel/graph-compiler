...
bool create_process_and_await(const std::string &program,
                              ...
                              int &exit_code, const std::string *rstdin,
                              std::string *rstdout, std::string *rstderr) {
  // fix-me: (win32)
...
bool create_process(const std::string &program,
                    ...
                    ofdstream_t *rstdin, ifdstream_t *rstdout,
                    ifdstream_t *rstderr) {
  // fix-me: (win32)
...
bool create_process_and_await(const std::string &program,
  ...
  if (!child_fd_actions_helper.success) {
    return false;
  }
  posix_spawn_file_actions_t &child_fd_actions =
...
#define ADD_ACTION(action, ...)                                                \
  if (auto retv = posix_spawn_file_actions_add##action(&child_fd_actions,      \
                                                       __VA_ARGS__)) {         \
  ...
  if (auto retv = posix_spawnp(&pid, program.c_str(), &child_fd_actions, &attr,
                               opt_char, environ)) {
    perror("posix_spawnp");
    return false;
  }
  ...
  if (rstdout) {
    read_all_from_fd(stdoutpipefd[0], *rstdout);
  }
  if (rstderr) {
    read_all_from_fd(stderrpipefd[0], *rstderr);
  }
