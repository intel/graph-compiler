...
inline size_t get_index(const std::vector<T> vec, const T &element) {
  auto pos = std::find_if(vec.begin(), vec.end(), [&](const T &other) -> bool {
...
template <typename T> constexpr bool is_one_of(T value, T last) {
  return value == last;
}
...
template <typename TDst, typename TSrc> struct bind_assigner_t {
...
template <typename T> void args_to_vector(std::vector<T> &v) {}
 ...
 * */
SC_INTERNAL_API bool create_process_and_await(
    const std::string &program, const std::vector<std::string> &args,
    int &exit_code, const std::string *rstdin = nullptr,
    std::string *rstdout = nullptr, std::string *rstderr = nullptr);
 ...
 * @return true if creating process succeeded
 * */
SC_INTERNAL_API bool
create_process(const std::string &program, const std::vector<std::string> &args,
               uintptr_t &outhandle, ofdstream_t *rstdin = nullptr,
               ifdstream_t *rstdout = nullptr, ifdstream_t *rstderr = nullptr);
 ...
 * */
inline std::vector<int> get_blocks(const int X, int threshold = 8,
                                   int floor = 1024) {
  std::vector<int> blocks;
  for (auto i = 1; i <= (int)sqrt((double)X); ++i) {
    if (X % i == 0) {
      // add a judgement here to prune search space for efficient search
      if (i >= threshold && i <= floor) {
        blocks.push_back(i);
      }
      auto div_x = X / i;
