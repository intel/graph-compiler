...
std::shared_ptr<jit_module>
cfake_jit::make_jit_module(const_ir_module_ptr module, bool generate_wrapper) {
  // fix-me: (win32)
  throw std::runtime_error("make_jit_module().");
}
...
static std::string get_include_path_() {
  std::string path;
  char home_path[512];
  if (utils::getenv(env_names[env_key::SC_C_INCLUDE], home_path,
                    sizeof(home_path)) != 0) {
    path = home_path;
  } else {
#ifdef SC_HOME
...
std::shared_ptr<jit_module>
cfake_jit::make_jit_module(const std::string &inpath,
  ...
  if (options_group.empty() || (options_group == "default")) {
    ...
    if (debug_info_) {
      discretionary_options.emplace_back("-g");
    }
  } else if (options_group == "xbyak-dev") {
    discretionary_options = std::vector<std::string>{
        "-O3", "-march=native",
  ...
  int exit_status;
  bool success = utils::create_process_and_await(command, option, exit_status);
  ...
  if (success) {
    ...
    auto init_func =
        reinterpret_cast<init_func_t>(dlsym(compiled_module, "__sc_init__"));
    if (init_func) {
      init_func(nullptr, globals.data_.data_);
    }
  } else {
...
statics_table_t
cfake_jit::codegen_to_cpp(std::ostream &os, const_ir_module_ptr &new_mod,
                          const const_ir_module_ptr &module,
                          bool generate_wrapper,
                          thread_pool_mode_t &out_managed_thread_pool,
...
cfake_jit::make_jit_module(const_ir_module_ptr module, bool generate_wrapper) {
  ...
  c_generator_optional_out_t optional_dump{&dump_main_f, &dump_header_f,
                                           &dump_data_f};
  ...
  if (!compiler_config.dump_gen_code_.empty()) {
    ...
    dump_path_base += module->attr_.get_or_else(
        ir_module_t::attr_key_t::NAME, "/cfake_jit_module-" + unique_name);
...
void *cfake_jit_module_code_t::get_function(const std::string &name,
                                            void *&wrapper) {
...
template <typename T, typename TF> constexpr uintptr_t myoffsetof(TF T::*fld) {
...
static cpu_flags_t do_get_compiler_flags() {
  ...
  bool success =
      utils::create_process_and_await(cfake_jit::get_compiler_command(), option,
                                      exit_status, &rstdin, &rstdout);
