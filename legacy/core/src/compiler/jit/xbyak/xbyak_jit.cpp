...
sequential_module_pass_t
get_xbyak_precodegen_passes(const context_ptr &ctx,
  ...
  ret.emplace_back(
      module_function_pass_t::make<x86_intrinsics_lowering_t>(ctx->machine_));
...
xbyak_jit::make_jit_module(const_ir_module_ptr ir_mod, bool generate_wrapper) {
...
xbyak_jit_module_code::xbyak_jit_module_code(
    ...
    : jit_module_code(managed_thread_pool), jit_output_(std::move(jit_output)) {
}
...
void *xbyak_jit_module_code::get_function(const std::string &name,
                                          void *&wrapper) {
