...
// stack_push imm
int64_t location_manager::stack_push(const uint64_t &imm,
                                     x86_64::cpu_data_type dtype) {
  ...
  default:
    COMPILE_ASSERT(false, "Invalid stack push imm: " << dtype);
  }

  sf_model_.push_anonymous_object(dtype, slot_size,
                                  "stack push imm: " + std::to_string(imm));
  return get_stack_top_rbp_offset();
}

// stack_push reg
int64_t location_manager::stack_push(const Xbyak::Reg &reg,
                                     x86_64::cpu_data_type dtype) {
  const size_t slot_size = get_data_slot_size(dtype);
...
// stack_push addr
int64_t location_manager::stack_push(const Xbyak::Address &addr,
                                     x86_64::cpu_data_type dtype) {
  const size_t slot_size = get_data_slot_size(dtype);
  ...
  default:
    COMPILE_ASSERT(false, "Invalid stack push addr: " << dtype);
  }
...
// stack_pop reg
int64_t location_manager::stack_pop(const Xbyak::Reg &reg,
                                    x86_64::cpu_data_type dtype) {
  const size_t slot_size = get_data_slot_size(dtype);
...
// stack_pop addr
int64_t location_manager::stack_pop(const Xbyak::Address &addr,
                                    x86_64::cpu_data_type dtype) {
  const size_t slot_size = get_data_slot_size(dtype);
  ...
  default:
    COMPILE_ASSERT(false, "Invalid stack pop addr: " << dtype);
  }
...
void location_manager::stack_padding(const size_t &padding_bytes_needed,
...
size_t location_manager::stack_tensor_define(x86_64::cpu_data_type cpu_dtype,
  ...
  sf_model_.push_named_tensor_buffer_object(name, cpu_dtype, num_elem,
                                            slot_size, comment);
  return slot_size;
}
...
void location_manager::handle_func_params(
    const std::vector<expr> &func_params,
  ...
  for (size_t i = 0; i < func_params.size(); ++i) {
    ...
    const cpu_data_type_table::row &r = get_cpu_data_type_row(ir_expr->dtype_);
    ...
    case abi_value_location::tag_type::STACK: {
      ...
      int64_t rbp_offset = initial_loc.get_rsp_offset() + saved_rbp_slot_size;
      auto param_slot = stack_frame_model::caller_param_slot(
          name, r.abi_stack_slot_size_, cpu_dtype, rbp_offset,
          "caller-created parameter slot");
      sf_model_.add_caller_param_slot(param_slot);
      // Get location on stack
      auto location = expr_location::make_stack_var(rbp_offset, cpu_dtype);
...
void location_manager::align_call_stack(
  ...
  const size_t current_rsp_mod16 =
      (initial_rsp_mod16 + saved_rbp_bytes + get_stack_current_size()) % 16;
...
void location_manager::push_caller_saved(
  ...
  for (auto &v : caller_saved) {
    ...
    local_location_map_[v] =
        expr_location::make_stack_var(rbp_offset, get_cpu_data_type(v->dtype_));
...
void location_manager::handle_definition(const expr_c &v) {
  if (GET_VIRTUAL_REG(v).allocated()) {
    allocate_free_reg(v);
  }
}
...
void location_manager::handle_spilled_definition(
  ...
  for (auto &v : defined_spill) {
    if (expr_location_map_.find(v) != expr_location_map_.end()) {
      ...
      // v is a var
      slot_size +=
          stack_var_define(data_type, name, "allocate on stack: " + name);
      auto offset = get_stack_top_rbp_offset();
      expr_location_map_[v] = expr_location::make_stack_var(offset, data_type);
      ...
      assert(num_elem <= 128);
      // v is a tensor
      slot_size += stack_tensor_define(elem_dtype, num_elem, name,
                                       "allocate on stack: " + name);
      auto offset = get_stack_top_rbp_offset();
...
void location_manager::conclude_local_scope() { local_location_map_.clear(); }
  ...
  auto gen_callee_save = [&](const virt_reg_type &reg_type) {
    ...
    for (auto i : target_callee_save) {
      if (register_usage.find(i) != register_usage.end()) {
        ...
        auto loc = expr_location::make_reg(reg, callee_save_cpu_type(reg_type));
...
void location_manager::expire(stmt_index_t current_index) {
  ...
  for (auto &loc_kv : expr_location_map_) {
    ...
    if (last_ll.end_ <= current_index) {
      to_be_expired.push_back(v);
    }
  }
  // release location
...
operand location_manager::get_operand(const expr_location &location) {
  ...
  case expr_location::type::stack_var: {
    auto addr =
        get_offset_address(location.get_stack_var(), location.get_data_type());
    return operand(operand::type::addr, wrap_op_ptr(addr));
  } break;
  case expr_location::type::stack_tensor: {
    auto addr = get_offset_address(location.get_stack_tensor(),
                                   location.get_data_type());
    return operand(operand::type::addr, wrap_op_ptr(addr));
  } break;
  case expr_location::type::simd_constant: {
...
operand location_manager::get_operand_sib(const expr_c &base,
                                          const expr_c &indx,
                                          const expr_c &disp) {
  ...
  return operand(
      gen_.ptr[loc_base.get_reg() + loc_indx.get_reg() + loc_disp.get_imm()]);
}
...
size_t location_manager::get_data_slot_size(x86_64::cpu_data_type data_type) {
  const size_t slot_size =
      get_cpu_data_types().lookup(data_type).local_value_stack_slot_size_;
  return slot_size;
}

size_t location_manager::get_tensor_slot_size(x86_64::cpu_data_type data_type,
  ...
  if (const size_t excess = buffer_size % 8) {
    buffer_size += (8 - excess);
  }
...
const Xbyak::AddressFrame *
location_manager::get_address_frame(const cpu_data_type cpu_dtype) {
  switch (cpu_dtype) {
  // integer 8-bit/ 1-byte
  case cpu_data_type::uint_8:
    return &(gen_.byte);
...
location_manager::encode_simd_constant() {
  std::function<int8_t(union_val)> select_s8 = [](union_val u) -> int8_t {
    return (int8_t)u.s64;
  };
  std::function<uint8_t(union_val)> select_u8 = [](union_val u) -> uint8_t {
    return (uint8_t)u.u64;
  };
  std::function<uint16_t(union_val)> select_u16 = [](union_val u) -> uint16_t {
    return (uint16_t)u.u64;
  };
  std::function<uint32_t(union_val)> select_u32 = [](union_val u) -> uint32_t {
    return (uint32_t)u.u64;
  };
  std::function<int32_t(union_val)> select_s32 = [](union_val u) -> int32_t {
    return (int32_t)u.s64;
  };
  std::function<float(union_val)> select_f32 = [](union_val u) -> float {
    return (float)u.f32;
  };
  std::function<uint16_t(union_val)> select_bf16 = [](union_val u) -> uint16_t {
    return bf16_t(u.f32).storage_;
  };
  std::function<uint16_t(union_val)> select_f16 = [](union_val u) -> uint16_t {
    return fp16_t(u.f32).storage_;
  };
  uint8_t buffer[64];
  for (auto &c : simd_constant_vec_) {
    ...
    case sc_data_etype::BF16: {
      encode_simd_to_buffer((uint16_t *)buffer, lanes, v->value_, select_bf16);
    } break;
    case sc_data_etype::U8: {
      encode_simd_to_buffer((uint8_t *)buffer, lanes, v->value_, select_u8);
    } break;
    case sc_data_etype::S8: {
      encode_simd_to_buffer((int8_t *)buffer, lanes, v->value_, select_s8);
    } break;
    case sc_data_etype::U16: {
      encode_simd_to_buffer((uint16_t *)buffer, lanes, v->value_, select_u16);
    } break;
    case sc_data_etype::U32: {
      encode_simd_to_buffer((uint32_t *)buffer, lanes, v->value_, select_u32);
    } break;
    case sc_data_etype::S32: {
      encode_simd_to_buffer((int32_t *)buffer, lanes, v->value_, select_s32);
    } break;
    case sc_data_etype::F32: {
      encode_simd_to_buffer((float *)buffer, lanes, v->value_, select_f32);
    } break;
    case sc_data_etype::F16: {
      encode_simd_to_buffer((uint16_t *)buffer, lanes, v->value_, select_f16);
    } break;
    default:
...
template <typename T>
void location_manager::encode_simd_to_buffer(
    T *buffer, uint32_t lanes, const std::vector<union_val> &value,
...
expr_location location_manager::get_location(const constant_c &c) {
  ...
  if (GET_VIRTUAL_REG(v).spilled()) {
    ...
    if (simd_iter == simd_constant_map_.end()) {
      simd_iter = simd_constant_map_.insert(
          simd_iter, std::make_pair<expr_c, Xbyak::Label>(v, Xbyak::Label()));
    ...
    loc_iter = expr_location_map_.insert(
        loc_iter, std::make_pair<expr_c, expr_location>(
    ...
    if (v->dtype_ == datatypes::bf16) {
      ...
      return expr_location::make_imm(bf16_t(v->value_[0].f32).storage_,
                                     data_type);
    } else {
...
void location_manager::load_location_to_reg(const Xbyak::Reg &reg,
...
void location_manager::load_reg_value_to_reg(const Xbyak::Reg &reg,
  ...
  default: {
    COMPILE_ASSERT(false, "Invalid: load_reg_value_to_reg " << data_type);
  }
  }
}
...
Xbyak::Address location_manager::get_address(const Xbyak::RegExp &exp,
                                             x86_64::cpu_data_type cpu_dtype) {
  const auto xaf = get_address_frame(cpu_dtype);
  return (*xaf)[exp];
}

Xbyak::Address location_manager::get_address(const Xbyak::RegRip &rxp,
                                             x86_64::cpu_data_type cpu_dtype) {
  const auto xaf = get_address_frame(cpu_dtype);
  return (*xaf)[rxp];
}
...
Xbyak::Address
location_manager::get_offset_address(const int64_t &offset,
                                     x86_64::cpu_data_type cpu_dtype) {
  return get_address(get_rbp_offset(offset), cpu_dtype);
}

Xbyak::Address
location_manager::get_offset_address(const Xbyak::Label &label,
                                     x86_64::cpu_data_type cpu_dtype) {
...
expr_location location_manager::convert_virtual_reg(const expr_c &v) {
  ...
  COMPILE_ASSERT(index != virt_reg_const::invalid,
                 "convert_virtual_reg failed");
