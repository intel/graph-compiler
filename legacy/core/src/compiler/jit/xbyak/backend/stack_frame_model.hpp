...
public:
  ...
  struct slot : public stack_item {
    slot(size_t stack_size_before_item, size_t slot_size,
        ...
        : stack_item(stack_size_before_item, debug_comment),
          slot_size_(slot_size), val_type_(val_type) {
      assert(slot_size >=
             x86_64::get_cpu_data_types().lookup(val_type).size_in_bytes_);
    }
  ...
  struct named_tensor_buffer_slot : public named_slot {
    named_tensor_buffer_slot(std::string name, size_t stack_size_before_item,
                             size_t slot_size,
                             x86_64::cpu_data_type element_type,
                             size_t num_elements, std::string debug_comment)
  ...
  void push_named_tensor_buffer_object(const std::string &name,
                                       ...
                                       size_t num_elements, size_t num_bytes,
                                       const std::string &debug_comment = "");
  ...
  bool try_get_named_object_rbp_offset(const std::string &name,
                                       int64_t &rbp_offset) const;
