...
xbyak_lowering_viewer::code_comment::code_comment(const Xbyak::Label &label,
                                                  std::string comment)
  ...
  for (auto &line : debug_lines_) {
    ...
    const source_pos *src =
        some_opt(line.ir_node_)
            .map([](const node_base *p) { return p->attr_.get(); })
            .map([](const any_map_t *v) {
              return v->get_or_null<source_pos>("source_pos");
            })
            .get_or_else(nullptr);
    if (src) {
      lines.emplace_back(debug_line{p, src->line_, src->pos_});
    }
  }
...
xbyak_lowering_viewer::xbyak_lowering_viewer(
    const xbyak_jit &xje, const ir_module_t &ir_mod,
    const x86_64::target_profile_t &profile)
    : xje_(xje), p_ir_mod_(&ir_mod), profile_(profile),
      cpu_flags_(profile.target_machine_.cpu_flags_),
      sf_model_(
          utils::compiler_configs_t::get().xbyak_jit_log_stack_frame_model_),
      logging_ind_(4), asm_listing_ind_(4) {
  ...
  for (const func_t &f : ir_mod.get_contents()) {
    ...
    if (f->name_.find("_should_inline_") != string::npos) {
      continue;
    }
    dispatch(f);
  }
...
xbyak_lowering_viewer::cached_func_register_usage(const func_t &v) {
  ...
  const auto &register_usage =
      v->attr_->get<std::set<virt_reg_index_t>>(attr_keys::register_usage);
  return register_usage;
}

const std::vector<expr_c> &
xbyak_lowering_viewer::cached_func_global_spilled(const func_t &v) {
...
void xbyak_lowering_viewer::handle_func_resolve(
    const std::string &name, const execute_func_label &label_f,
    const execute_func_addr &addr_f) {
  ...
  if (ir_sym_table_iter != func_name_to_entry_label_.end()) {
    ...
    COMPILE_ASSERT(false, "Address/label lookup failed for function name \""
                              << name << "\"");
  }
}

void xbyak_lowering_viewer::handle_local_definition(const expr_c &v,
                                                    const expr_c &v_init) {
  location_manager_->handle_definition(v);
  if (v_init.defined()) {
    handle_operations(v, v_init);
  }
}
...
void xbyak_lowering_viewer::handle_operations(const expr_c &lhs,
                                              const expr_c &rhs) {
...
void xbyak_lowering_viewer::handle_xbyak_intrin(const expr_c &lhs,
                                                const xbyak_intrin_c &rhs) {
  ...
  default: {
    ...
    case xbyak_intrin_isa::x86: {
      handle_x86_intrisic(lhs, rhs->args_, intrin, rhs->modifier_);
    } break;
    case xbyak_intrin_isa::avx: {
      handle_avx_intrisic(lhs, rhs->args_, intrin, rhs->modifier_);
    } break;
    default: {
...
void xbyak_lowering_viewer::handle_x86_intrisic(
    const expr_c &dst, array_ref<expr> args, const xbyak_intrin_type &intrin,
  ...
  case xbyak_intrin_type::mulhl: {
    ...
    default:
      COMPILE_ASSERT(false, "x86 mulhl type error");
    }
  } break;
...
void xbyak_lowering_viewer::handle_avx_intrisic(
    const expr_c &dst, array_ref<expr> args, const xbyak_intrin_type &intrin,
    const xbyak_intrin_modifier &modifier) {
  // Get low level data type
  const cpu_data_type cpu_dtype = get_cpu_data_type(dst->dtype_);
  ...
  case xbyak_intrin_type::mask_mov: {
    ...
    auto op_dst = GET_OPERAND(dst);
    auto op_src = GET_OPERAND(args[0]);
    handle_avx_mask_mov(op_dst, op_src, op_cond, cpu_dtype,
                        modifier.zero_mask_);
  } break;
...
void xbyak_lowering_viewer::handle_assign(const expr_c &lhs,
                                          const expr_c &rhs) {
...
void xbyak_lowering_viewer::handle_tensorptr(const expr_c &lhs,
                                             const tensorptr_c &rhs) {
...
void xbyak_lowering_viewer::handle_func_addr(const expr_c &lhs,
                                             const func_addr_c &rhs) {
  auto cpu_dtype = get_cpu_data_type(lhs->dtype_);
...
void xbyak_lowering_viewer::handle_cast(const expr_c &lhs, const cast_c &v) {
  ...
  if (location_manager_->is_stack_tensor(v->in_)) {
    ...
    handle_x86_mov(op_out, op_in);
              ...
              out_dtype == datatypes::s8 || out_dtype == datatypes::u8 ||
              out_dtype == datatypes::u16) &&
             (in_dtype == datatypes::generic || in_dtype == datatypes::index)) {
    handle_x86_mov(op_out, op_in);
    ...
    COMPILE_ASSERT(false, FUNC_INFO << "Invalid type: " << out_dtype << " <- "
                                    << in_dtype << ". v=" << v);
  }
}

void xbyak_lowering_viewer::handle_saturated_cast(const expr_c &dst,
                                                  const expr_c &src) {
  ...
  if (dst_dtype == sc_data_type_t::s8(16) &&
    ...
    COMPILE_ASSERT(false, FUNC_INFO << "Invalid type: " << dst_dtype << " <- "
                                    << src_dtype);
  }
}

void xbyak_lowering_viewer::handle_round_and_cast(const expr_c &dst,
                                                  const expr_c &src) {
  const sc_data_type_t src_dtype = src->dtype_;
  const sc_data_type_t dst_dtype = dst->dtype_;
...
void xbyak_lowering_viewer::handle_reinterpret(const expr_c &lhs,
                                               const expr_c &rhs) {
  ...
  case 4: { // 32-bit
    ...
    } else if (dtype_dst == datatypes::f32 || dtype_src == datatypes::f32) {
      // float <-> int
      XBYAK_GEN(vmovd, AVX_XMR32_XMR32, op_dst, op_src);
    } else {
      // int <-> int
  ...
  default:
    COMPILE_ASSERT(false, FUNC_INFO << "Invalid type: " << dtype_dst << " <- "
                                    << dtype_src);
  }
}
...
void xbyak_lowering_viewer::handle_x86_mov(const operand &op_dst,
...
void xbyak_lowering_viewer::handle_x86_cmp(const operand &op_lhs,
                                           const operand &op_rhs) {
  XBYAK_GEN(cmp, X86_RM_RMI, op_lhs, op_rhs);
}

void xbyak_lowering_viewer::handle_x86_set(
    const operand &op_dst, const xbyak_condition &code,
  ...
  case cpu_data_type::uint_64: {
    ...
    case xbyak_condition::gt:
      ...
      break;
    default:
      COMPILE_ASSERT(false, FUNC_INFO << "Invalid condition: " << code);
    }
  } break;
  case cpu_data_type::sint_8:
  case cpu_data_type::sint_32: {
...
void xbyak_lowering_viewer::handle_x86_cmov(
    const operand &op_dst, const operand &op_src, const xbyak_condition &code,
    const x86_64::cpu_data_type &cpu_dtype) {
  const auto get_cmov_op = [](const operand &op) {
    // cmov instructions do not support 8-bit reg as operand
    if (op.is_reg(8)) {
      return operand(op.get_reg32());
    }
    return op;
  };
...
void xbyak_lowering_viewer::handle_avx_movq(const operand &op_dst,
                                            const operand &op_src) {
  if (op_dst == op_src) {
    return;
  }
...
void xbyak_lowering_viewer::handle_avx_movss(const operand &op_dst,
                                             const operand &op_src) {
  if (op_dst == op_src) {
    return;
  }
...
void xbyak_lowering_viewer::handle_avx_movsh(const operand &op_dst,
                                             const operand &op_src) {
  if (op_dst == op_src) {
    return;
  }
...
void xbyak_lowering_viewer::handle_avx_movps(const operand &op_dst,
                                             const operand &op_src) {
  if (op_dst == op_src) {
    return;
    ...
    COMPILE_ASSERT(false, FUNC_INFO << "Invalid operand: " << op_dst << ", "
                                    << op_src);
  }
}

void xbyak_lowering_viewer::handle_avx512_kmov(
...
void xbyak_lowering_viewer::handle_avx_add(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_16_x32:
  case cpu_data_type::float_16_x16:
...
void xbyak_lowering_viewer::handle_avx_sub(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_16_x32:
...
void xbyak_lowering_viewer::handle_avx_mul(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_16_x32:
...
void xbyak_lowering_viewer::handle_avx_mulhl(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::sint_32_x16: {
...
void xbyak_lowering_viewer::handle_avx_div(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_16: {
...
void xbyak_lowering_viewer::handle_avx_bit_or(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  auto gen_avx_bit_or = [&]() {
    switch (cpu_dtype) {
...
void xbyak_lowering_viewer::handle_avx_bit_and(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
...
void xbyak_lowering_viewer::handle_avx_bit_xor(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
...
void xbyak_lowering_viewer::handle_avx_min(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::uint_8_x8:
...
void xbyak_lowering_viewer::handle_avx_max(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_16_x32:
...
void xbyak_lowering_viewer::handle_avx_abs(
...
void xbyak_lowering_viewer::handle_avx_shr(
    const operand &op_dst, const operand &op_lhs, const operand &op_sft,
...
void xbyak_lowering_viewer::handle_avx_shl(
    const operand &op_dst, const operand &op_lhs, const operand &op_sft,
    const x86_64::cpu_data_type &cpu_dtype, bool variable) {
  switch (cpu_dtype) {
  case cpu_data_type::uint_64_x8: {
...
void xbyak_lowering_viewer::handle_avx_sar(
    const operand &op_dst, const operand &op_lhs, const operand &op_sft,
    const x86_64::cpu_data_type &cpu_dtype, bool variable) {
  switch (cpu_dtype) {
  case cpu_data_type::uint_32_x16:
...
void xbyak_lowering_viewer::handle_avx_round(
    const operand &op_lhs, const operand &op_rhs,
...
void xbyak_lowering_viewer::handle_avx_sqrt(
    const operand &op_dst, const operand &op_src,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_32_x16: {
...
void xbyak_lowering_viewer::handle_avx_rsqrt(
    const operand &op_dst, const operand &op_src,
    const x86_64::cpu_data_type &cpu_dtype) {
...
void xbyak_lowering_viewer::handle_avx_fmadd(
    const operand &op_dst, const operand &op_mul, const operand &op_add,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_32_x16: {
    assert(cpu_flags_.fAVX512F);
...
void xbyak_lowering_viewer::handle_avx_fnmadd(
    const operand &op_dst, const operand &op_mul, const operand &op_add,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_32_x16: {
...
void xbyak_lowering_viewer::handle_avx_pshuffle(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::uint_8_x8:
...
void xbyak_lowering_viewer::handle_avx_shuffle(const operand &op_dst,
                                               const operand &op_lhs,
                                               const operand &op_rhs,
                                               const operand &op_imm,
  ...
  auto gen_avx_shuffle = [&]() {
    ...
    default:
      COMPILE_ASSERT(false, FUNC_INFO << "Invalid type_bits: " << type_bits);
    }
  };
  ...
  auto gen_avx512_shuffle = [&]() {
    ...
    case 128: {
      XBYAK_GEN(vshuff32x4, AVX_Y_Y_XM_I, op_dst, op_lhs, op_rhs, op_imm);
    } break;
    default:
      COMPILE_ASSERT(false, FUNC_INFO << "Invalid type_bits: " << type_bits);
    }
...
void xbyak_lowering_viewer::handle_avx_permute(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
    const operand &op_imm, const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::uint_16_x16:
  case cpu_data_type::float_32_x8: {
...
void xbyak_lowering_viewer::handle_avx_gather(
    const operand &op_dst, const operand &op_ptr, const operand &op_idx,
    const operand &mask, const x86_64::cpu_data_type &cpu_dtype) {
  auto op_addr =
      operand(gen_->ptr[op_ptr.get_reg() + op_idx.get_xmm() * sizeof(float)]);
...
void xbyak_lowering_viewer::handle_avx_insert(const operand &op_dst,
  ...
  case 128: {
    if (simd_level_ == simd_level::avx512) {
      ...
      XBYAK_GEN(vinsertf128, AVX_Y_Y_YM_I, op_dst, op_dst, op_b, op_imm);
    }
  } break;
  case 256: {
    assert(cpu_flags_.fAVX512DQ);
  ...
  default:
    COMPILE_ASSERT(false, FUNC_INFO << "Invalid elem_bits: " << elem_bits);
  };
}
...
void xbyak_lowering_viewer::handle_avx_extract_low(
    const operand &op_dst, const operand &op_src,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_32_x16:
...
void xbyak_lowering_viewer::handle_avx_extract_high(
    const operand &op_dst, const operand &op_src,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_32_x16: {
  ...
  case cpu_data_type::float_32_x8: {
    ...
    default:
      COMPILE_ASSERT(false, FUNC_INFO << "No simd support");
    }
  } break;
  case cpu_data_type::sint_32_x8: {
    switch (simd_level_) {
...
void xbyak_lowering_viewer::handle_avx_permutex2var(
    const operand &op_dst, const operand &op_idx, const operand &op_src,
    const x86_64::cpu_data_type &cpu_dtype) {
  assert(cpu_flags_.fAVX512F);
  switch (cpu_dtype) {
  case cpu_data_type::float_32_x16:
...
void xbyak_lowering_viewer::handle_avx_permutexvar(
    const operand &op_dst, const operand &op_idx, const operand &op_src,
...
void xbyak_lowering_viewer::handle_avx_broadcast(
    const operand &op_dst, const operand &op_src,
    const x86_64::cpu_data_type &cpu_dtype,
  ...
  case cpu_data_type::uint_16_x8: {
    ...
    if (op_src.is_addr()) {
      ...
      case cpu_data_type::uint_16_x32: {
        ...
        XBYAK_GEN(vshufi32x4, AVX_Y_Y_XM_I, op_dst, op_src_z, op_src_z,
                  operand(INT64_C(0x0)));
      } break;
      case cpu_data_type::uint_16_x16: {
        auto op_src_y = operand(to_ymm(op_src.get_reg()));
        XBYAK_GEN(vshufi32x4, AVX_Y_Y_XM_I, op_dst, op_src_y, op_src_y,
                  operand(INT64_C(0x0)));
      } break;
      default:
...
void xbyak_lowering_viewer::handle_avx_blend(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
  ...
  auto gen_avx_blend = [&]() {
    ...
    case cpu_data_type::float_32_x8:
    case cpu_data_type::float_32_x4: {
      XBYAK_GEN(vblendvps, AVX_X_X_XM_X, op_dst, op_lhs, op_rhs, op_cond);
    } break;
    case cpu_data_type::uint_32_x8:
    case cpu_data_type::sint_32_x8: {
      XBYAK_GEN(vblendvps, AVX_X_X_XM_X, op_dst, op_lhs, op_rhs, op_cond);
    ...
    case cpu_data_type::uint_8_x8:
    case cpu_data_type::sint_8_x8: {
      XBYAK_GEN(vpblendvb, AVX_X_X_XM_X, op_dst, op_lhs, op_rhs, op_cond);
    } break;
    case cpu_data_type::uint_16_x16:
    case cpu_data_type::uint_16_x8: {
...
void xbyak_lowering_viewer::handle_avx_mask_mov(
    const operand &op_dst, const operand &op_src, const operand &op_cond,
...
void xbyak_lowering_viewer::handle_avx_cmov(
    const operand &op_dst, const operand &op_src, const xbyak_condition &code,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_32:
  case cpu_data_type::float_16: {
    ...
    default:
      COMPILE_ASSERT(false, "Invalid condition: " << code);
    }
    handle_avx_movss(op_dst, op_src);
    // end
    gen_->L(l_end_cmov);
...
void xbyak_lowering_viewer::handle_avx_cmp_set(
    const operand &op_dst, const operand &op_lhs, const operand &op_rhs,
  ...
  auto op_imm = [](const xbyak_condition &code) {
    switch (code) {
    case xbyak_condition::eq:
      return operand(INT64_C(0x0));
    case xbyak_condition::lt:
      return operand(INT64_C(0x1));
    case xbyak_condition::le:
      return operand(INT64_C(0x2));
    case xbyak_condition::ne:
      return operand(INT64_C(0x4));
    case xbyak_condition::ge:
      return operand(INT64_C(0x5));
    case xbyak_condition::gt:
      return operand(INT64_C(0x6));
    default:
      COMPILE_ASSERT(false, FUNC_INFO << "Invalid condition: " << code);
    }
    return operand();
  ...
  auto gen_avx_cmp_set = [&]() {
    ...
    case cpu_data_type::uint_32_x4: {
      ...
      default:
        COMPILE_ASSERT(false, "No avx_cmp for: " << code);
      }
    } break;
    case cpu_data_type::sint_32_x8:
    case cpu_data_type::sint_32_x4: {
...
void xbyak_lowering_viewer::handle_avx_mov_mask(
    const operand &op_dst, const operand &op_src,
    const x86_64::cpu_data_type &cpu_dtype) {
  switch (cpu_dtype) {
  case cpu_data_type::float_32_x8:
...
void xbyak_lowering_viewer::handle_pre_call(const stmts_c &v) {
  ...
  auto callee_iface =
      v->attr_->get<abi_function_interface::ptr>(attr_keys::abi_interface);
...
void xbyak_lowering_viewer::view(stmts_c v) {
  if (TRANSFORMED_CALL(v)) {
    ...
    handle_pre_call(v);
    ir_viewer_t::view(v);
    COMPILE_ASSERT(conserved == location_manager_->get_conserved_stack_size(),
...
void xbyak_lowering_viewer::view(returns_c v) {
  if (v->value_.defined()) {
    ...
    default:
      COMPILE_ASSERT(false, "Unsupported type: " << cpu_dtype);
    }
  }
...
void xbyak_lowering_viewer::view(for_loop_c v) {
  ...
  const cpu_data_type loop_var_cpu_dtype = get_cpu_data_type(loop_var->dtype_);
