...
#define LOG_FUNC_EXIT_WITH_TEXT(S1, ...)                                       \
  if (logging_enabled_) {                                                      \
    cout << __PRETTY_FUNCTION__ << " : PRE-EXIT: " << S1 __VA_ARGS__ << endl;  \
  }
  ...
  while (!stack_.empty()) {
    ...
    if (const slot *s = dynamic_cast<const slot *>(top)) {
      ...
      LOG_MODIFIER_FUNC_EXIT_WITH_TEXT(
          "scope_size=" << scope_size << " stack-size=" << get_size());
      return scope_size;
    }
  }
...
void stack_frame_model::push_named_tensor_buffer_object(
  ...
  const size_t old_stack_size = get_size();
  stack_.push_back(utils::make_unique<named_tensor_buffer_slot>(
      name, old_stack_size, num_bytes, val_type, num_elements, debug_comment));

  LOG_MODIFIER_FUNC_EXIT_WITH_TEXT("stack-size=" << get_size());
}

void stack_frame_model::push_anonymous_object(
  ...
  const size_t old_stack_size = get_size();
  stack_.push_back(utils::make_unique<anonymous_slot>(old_stack_size, num_bytes,
                                                      val_type, debug_comment));
  LOG_MODIFIER_FUNC_EXIT_WITH_TEXT("stack-size=" << get_size());
}
...
void stack_frame_model::shrink(size_t num_bytes) {
  ...
  const size_t old_stack_size = get_size();
  COMPILE_ASSERT(num_bytes <= get_size(), "stack_frame_model size is "
                                              << old_stack_size
...
void stack_frame_model::shrink_to_size(size_t new_size) {
  ...
  LOG_MODIFIER_FUNC_EXIT_WITH_TEXT(
      "(old)current_size=" << current_size << " (new)size=" << get_size());
}
...
void stack_frame_model::assert_unused_name(const std::string &name) {
  COMPILE_ASSERT(
      name_to_caller_param_slot_.find(name) == name_to_caller_param_slot_.end(),
      "A caller-parameter slot with name \"" << name << "\" already exists.");
}
...
int64_t
stack_frame_model::get_named_object_rbp_offset(const std::string &name) const {
  int64_t offset;
...
const stack_frame_model::named_slot *
stack_frame_model::try_get_named_slot(const std::string &name) const {
  const auto iter2 = name_to_caller_param_slot_.find(name);
  if (iter2 != name_to_caller_param_slot_.end()) {
    return &(iter2->second);
  }
...
bool stack_frame_model::try_get_named_object_rbp_offset(const std::string &name,
