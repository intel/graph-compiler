...
private:
  ...
  dump_code_comments(std::ostream &os);
  ...
  const std::set<virt_reg_index_t> &cached_func_register_usage(const func_t &v);
  ...
  void handle_func_resolve(const std::string &name,
                           const execute_func_label &label_f,
  ...
  void handle_x86_sign_ext(const operand &op_rdx,
                           const x86_64::cpu_data_type &cpu_dtype);
  void handle_x86_div(const operand &op_div,
                      const x86_64::cpu_data_type &cpu_dtype);
