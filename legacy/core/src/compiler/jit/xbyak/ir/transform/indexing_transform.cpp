...
public:
  ...
  expr_c visit(indexing_c v) override {
    ...
    if (is_var_or_const(idx)) {
      ...
      return vv;
    } else if (idx.isa<add>() && is_var_or_const(idx.static_as<add>()->r_)) {
      ...
      return builder::make_indexing(new_ptr, new_idx, vv->dtype_.lanes_,
                                    vv->mask_);
    } else {
      auto scale = builder::make_constant({elem_size}, idx_type);
  ...
  expr_c visit(tensorptr_c v) override {
    ...
    if (idx_zero.has_value()) {
      ...
      return builder::make_cast(
          v->dtype_, builder::make_cast(datatypes::index, ptr) + offset);
    }
    return v;
  }
};
