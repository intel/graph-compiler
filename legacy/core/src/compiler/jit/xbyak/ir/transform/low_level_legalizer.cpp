...
public:
  ...
  stmt_c visit(assign_c v) override {
    ...
    if (value.isa<tensor>()) {
      ...
      return make_stmt<assign_node_t>(var.remove_const(),
                                      builder::make_cast(var->dtype_, value));
    }
    return vv;
  }
  ...
  expr_c visit(intrin_call_c v) override {
    ...
    case intrin_type::rsqrt: {
      if (dst_dtype == datatypes::f32) {
        return builder::make_constant(1.f) / builder::make_sqrt(vv->args_[0]);
      }
    } break;
    case intrin_type::reduce_add: {
      if (can_cast_to_f32) {
        return transform_reduce(vv->args_[0], dst_dtype,
                                &builder::make_reduce_add);
      }
    } break;
    ...
    case intrin_type::reduce_min: {
      if (can_cast_to_f32) {
        return transform_reduce(vv->args_[0], dst_dtype,
                                &builder::make_reduce_min);
      }
    } break;
    case intrin_type::reduce_max: {
      if (can_cast_to_f32) {
        return transform_reduce(vv->args_[0], dst_dtype,
                                &builder::make_reduce_max);
      }
    } break;
  ...
  expr_c transform_fabs(const expr &src, const union_val &val) {
    return builder::make_int_and(src,
                                 builder::make_constant({val}, src->dtype_));
  }
  ...
  expr_c transform_f32_cast(sc_data_type_t dst_dtype, const expr &src) {
    ...
    const auto is_f16_scalar_cast = [](const sc_data_type_t &type_dst,
                                       const sc_data_type_t &type_src) {
      return (type_dst.type_code_ == sc_data_etype::F16) &&
             (utils::is_one_of(type_src.type_code_, sc_data_etype::U16,
                               sc_data_etype::S8, sc_data_etype::U8)) &&
             type_dst.lanes_ == 1;
    };
    const auto convert_s32_transform = [](const sc_data_type_t &dst_dtype,
                                          const sc_data_type_t &src_dtype,
                                          const expr &src) {
      return builder::make_cast(
          dst_dtype,
          builder::make_cast(sc_data_type_t::s32(src_dtype.lanes_), src));
    };
