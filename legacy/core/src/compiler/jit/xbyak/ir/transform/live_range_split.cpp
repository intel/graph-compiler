...
public:
  ...
  void view(call_c v) override {
    ...
    if (!cur_loop_iter_.empty()) {
      split_analysis_data_t::get(cur_loop_iter_.back()).cross_call_ = true;
    }
  }
  ...
  stmt_c visit(for_loop_c v) override {
    ...
    if (data.used_vars_.empty()) {
      ...
      for (const auto &x : data.used_vars_) {
        ...
        new_v->name_ += std::string("_spilt_") + std::to_string(var_counter_++);
        var_replace_map_[x] = new_v;
        seq.emplace_back(
            builder::make_var_tensor_def_unattached(new_v, linkage::local, x));
      }
      // dispatch loop
      ...
      for (const auto &x : data.modified_vars_) {
        seq.emplace_back(
            builder::make_assign_unattached(x, var_replace_map_[x]));
      }
