...
public:
  ...
  use_count_analysis_viewer_t()
      : uc_ana_{
            temp_data_addresser<use_count_t, size_t, &use_count_t::val_>()} {}
...
public:
  ...
  expr_c visit(intrin_call_c v) override {
    ...
    auto combine_to_broadcast = [](const expr &e, int dst_lanes) {
      ...
      return owner.cast<define>()
          ...
          .map([dst_lanes](const indexing &idxn) -> expr_c {
            if (idxn->mask_.defined()) {
              return expr();
            }
            auto src_lanes = builder::make_constant((int)idxn->dtype_.lanes_);
    ...
    case intrin_type::broadcast: {
      ...
      return combine_to_broadcast(v->args_[0], v->dtype_.lanes_).get_or_else(v);
    } break;
    default:
      break;
    }
