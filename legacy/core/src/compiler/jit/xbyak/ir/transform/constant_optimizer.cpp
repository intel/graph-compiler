...
public:
  ...
  expr_c visit(div_c v) override {
    ...
    if (v->r_.isa<constant>() && v->r_->dtype_.is_etype(sc_data_etype::S32)) {
    ...
    if (vv->r_.isa<constant>()) {
      ...
      bool transform_pow_2 =
          utils::is_one_of(vv->l_->dtype_, datatypes::u8, datatypes::u16,
  ...
  expr_c visit(cmp_c v) override {
    auto make_swap_cmp = [&](sc_expr_type t, const expr_c &lhs,
      ...
      case sc_etype::cmp_eq:
        return builder::make_cmp_eq(rhs, lhs);
      case sc_etype::cmp_lt:
        return builder::make_cmp_gt(rhs, lhs);
      case sc_etype::cmp_le:
        return builder::make_cmp_ge(rhs, lhs);
      case sc_etype::cmp_ne:
        return builder::make_cmp_ne(rhs, lhs);
      case sc_etype::cmp_ge:
        return builder::make_cmp_le(rhs, lhs);
      case sc_etype::cmp_gt:
        return builder::make_cmp_lt(rhs, lhs);
      default:
    ...
    if (v->l_.isa<constant>()) {
      return make_swap_cmp(v->node_type_, v->l_, ir_visitor_t::dispatch(v->r_));
    } else {
      return ir_visitor_t::visit(std::move(v));
    }
  }
  ...
  bool is_power_of_2(const constant &v) {
    const auto dtype = v->dtype_;
    if (utils::is_one_of(dtype, datatypes::u8, datatypes::u16, datatypes::u32,
                         datatypes::index)) {
      uint64_t val = v->value_[0].u64;
  ...
  expr single_lane_constant(const constant_c &v) {
    ...
    uint64_t val = v->value_[0].u64;
    return builder::make_constant({val},
                                  sc_data_type_t(v->dtype_.type_code_, 1));
  }
};
