...
void prepare_virtual_reg(reg_allocator_t *allocator, virtual_reg_t *virt_reg,
  ...
  if (!virt_reg->live_range_.empty()) {
    allocator->enqueue(virt_reg);
  }
}
...
public:
  ...
  expr_c visit(call_c v) override {
    ...
    for (size_t i = 0; i < args.size(); ++i) {
      ...
      if (initial_loc.get_type() == abi_value_location::tag_type::REGISTER) {
...
private:
  ...
  void set_func_return_hint(const expr &v,
                            const abi_function_interface::ptr &func_abi) {
    auto &slots_map = allocator_->slots_map();
    auto &virt_reg = GET_VIRTUAL_REG(v);
    auto ret_reg = func_abi->return_val_loc_.get_register();
    virt_reg.set_hint(virt_reg_hint::weak, slots_map.get_reg_index(ret_reg));
  }
};
...
public:
  ...
  stmt_c visit(stmts_c v) override {
    ...
    for (auto &s : v->seq_) {
      ...
      if (GET_STMT_DATA(ss).optimized_out_) {
        continue;
      }
      // insert before
  ...
  stmt_c visit(assign_c v) override {
    ...
    auto ret =
        xbyak_visitor_t::visit(std::move(v)).remove_const().static_as<assign>();
    if (dst_is_mem_) {
...
protected:
  ...
  stmt_c resolve_spill(define_c v) {
    auto vv =
        xbyak_visitor_t::visit(std::move(v)).remove_const().static_as<define>();
  ...
  expr_c resolve_spill(xbyak_intrin_c x) {
    ...
    case xbyak_intrin_format::directed_end_mem: {
      ...
      if (cpu_flags_.fAVX512F && imm_last) {
        n = n - 1;
      }
      if (spilled_args_sum(v->args_, n) > 0) {
        return resolve();
      }
    } break;
...
private:
  ...
  stmt loop_var_load(expr &old_expr, stmt_index_t index) {
    ...
    auto new_var =
        new_temp_var(old_var, "load_" + old_var->name_, index_load, index);
  ...
  expr insert_load(expr old_expr, stmt_index_t index) {
    ...
    auto get_new_expr = [&]() {
      if (old_expr.isa<tensor>()) {
        ...
        auto new_ptr =
            new_temp_tensor(old_ptr, "load_tensor_", index_load, index);
        return new_ptr;
      } else {
        auto old_var = old_expr.static_as<var>();
        auto new_var = new_temp_var(old_var, "load_var_", index_load, index);
        return new_var;
      }
    };
    auto new_expr = get_new_expr();
    auto new_load = new_temp_assign(new_expr, std::move(old_expr), index_load);
  ...
  stmt insert_store(assign vv, stmt_index_t index) {
    ...
    if (vv->var_.isa<tensor>()) {
      ...
      auto new_ptr =
          new_temp_tensor(old_ptr, "store_tensor_", index, index_store);
      ...
      auto new_var = new_temp_var(old_var, "store_var_", index, index_store);
  ...
  stmt insert_load_store(assign vv, stmt_index_t index) {
    ...
    if (vv->var_.isa<tensor>()) {
      auto old_ptr = vv->var_.static_as<tensor>();
      auto new_ptr =
          new_temp_tensor(old_ptr, "store_tensor_", index_load, index_store);
      ...
      auto old_var = vv->var_.static_as<var>();
      auto new_var =
          new_temp_var(old_var, "store_var_", index_load, index_store);
  ...
  expr new_temp_var(const expr &old_var, const std::string &prefix,
    ...
    // new var
    auto new_var = builder::make_var(old_var->dtype_,
                                     prefix + std::to_string(temp_index_++));
...
public:
  ...
  void set_register_usage() {
    func_t func = std::const_pointer_cast<func_base>(func_);
    func->attr().set(attr_keys::register_usage, slots_array().utilized_slots());
  }
...
func_c register_allocation_t::operator()(func_c v) {
  if (v->name_.find("_should_inline_") != std::string::npos) {
    return v;
  }
