...
public:
               ...
               v->init_.cast<intrin_call>()
                   .filter([](const intrin_call &v) {
                     return v->type_ == intrin_type::reinterpret &&
                            v->args_.at(0)->dtype_ == datatypes::index;
                   })
                   .has_value()) {
  ...
  void x86_intrinsics_transform(const expr &dst, const expr &val) {
    ...
    case sc_expr_type::mul: {
      ...
      transform(dst, {bin->l_, bin->r_},
                dst->dtype_, //
                transform_x86_mul(), transform_intrin(xbyak_intrin_type::mul));
    } break;
  ...
  void transform_intrin_call(const expr &dst, const intrin_call &intrin) {
    ...
    case intrin_type::abs: {
      transform(dst, {intrin->args_[0]},
                dst->dtype_, //
                transform_x86_abs(), transform_intrin(xbyak_intrin_type::abs));
    } break;
    ...
    case intrin_type::shr: {
      auto is_uint = (CATE_UINT == get_etype_category(dst->dtype_.type_code_));
      auto sft_type = is_uint ? xbyak_intrin_type::shr : xbyak_intrin_type::sar;
      transform(dst, {intrin->args_[0], intrin->args_[1]},
                dst->dtype_, //
                transform_x86_shift(sft_type), transform_intrin(sft_type));
    } break;
    ...
    case intrin_type::fmadd: {
      transform(dst, {intrin->args_[0], intrin->args_[1], intrin->args_[2]},
                dst->dtype_, //
    ...
    case intrin_type::unpack_low: {
      ...
      transform(
          dst,
          {intrin->args_[0], intrin->args_[1], builder::make_constant(imm)},
          dst->dtype_, //
    ...
    case intrin_type::insert: {
      ...
      auto elem_bits = utils::get_sizeof_type(intrin->args_[1]->dtype_) * 8;
      bool need_to_reg32 = elem_bits < 32;
      auto insert_val =
          builder::make_var(sc_data_type_t::u32(1), "insert_val_");
      if (need_to_reg32) {
        // need reg32
        add_defination(insert_val, linkage::local);
        add_assignment(insert_val,
                       builder::make_cast(datatypes::u32, intrin->args_[1]));
      }
      transform(
          dst,
          {intrin->args_[0], need_to_reg32 ? insert_val : intrin->args_[1],
           builder::make_constant(imm), builder::make_constant(elem_bits)},
          dst->dtype_, //
    ...
    case intrin_type::permutexvar: {
      const int elem_bits = intrin->intrin_attrs_->get_or_else("lanes", 1) *
  ...
  void transform_low_level_intrin(const expr &dst,
                                  const low_level_intrin &intrin) {
    ...
    case x86_intrin_type::avx_compare: {
      ...
      add_assignment(dst, make_xbyak_intrin(
                              dst->dtype_, {intrin->args_[0], intrin->args_[1]},
  ...
  void transform_cast(const expr &dst, const cast &src) {
    ...
    if (dst_dtype == sc_data_type_t::bf16(1) &&
      ...
      transform_intrin(
          bias,
          {builder::make_constant({UINT64_C(32767)}, sc_data_type_t::u32(1))},
  ...
  void transform_select(const expr &dst, const select &sel) {
    ...
    if (cond->dtype_ == datatypes::boolean) {
      ...
      if (isa == xbyak_intrin_isa::x86) {
        ...
        add_assignment(rax, make_xbyak_intrin(
                                dst_dtype, {rhs}, xbyak_intrin_type::cmov, isa,
        ...
        add_assignment(
            dst,
            make_xbyak_intrin(dst_dtype, {sel->r_}, xbyak_intrin_type::cmov,
                              isa, xbyak_intrin_modifier(xbyak_condition::eq)));
      }
    } else {
  ...
  void transform_cmp_set(const expr &dst, const cmp &src, sc_expr_type t) {
    ...
    if (cmp_dtype == datatypes::f32) {
      ...
      add_assignment(dst, make_xbyak_intrin(dst->dtype_, {xmm0},
  ...
  using transform_func = std::function<void(const expr &, array_ref<expr>,
                                            sc_data_type_t, xbyak_intrin_isa)>;
  ...
  transform_func transform_assign() {
    return [this](const expr &dst, array_ref<expr> src, sc_data_type_t dtype,
                  xbyak_intrin_isa isa) { add_assignment(dst, src[0]); };
  }

  transform_func transform_gather() {
    return
        [this](const expr &dst, array_ref<expr> src, sc_data_type_t dtype,
               xbyak_intrin_isa isa) { transform_gather(dst, src[0], src[1]); };
  }
  ...
  transform_func transform_5a_to_4a(xbyak_intrin_type intrin) {
    return [this, intrin](const expr &dst, array_ref<expr> src,
                          sc_data_type_t dtype, xbyak_intrin_isa isa) {
      transform_5a_to_4a(dst, src[0], src[1], src[2], src[3], intrin, isa);
    };
  }
  ...
  transform_func transform_x86_abs() {
    return [this](const expr &dst, array_ref<expr> src, sc_data_type_t dtype,
                  xbyak_intrin_isa isa) { transform_x86_abs(dst, src[0]); };
  }
  ...
  void transform_intrin(const expr &v, array_ref<expr> args,
    ...
    add_assignment(v, make_xbyak_intrin(v->dtype_, args.as_vector(), intrin,
                                        isa, xbyak_intrin_modifier(v->dtype_)));
  }

  void transform_intrin_eval(array_ref<expr> args, //
                             xbyak_intrin_type intrin, xbyak_intrin_isa isa) {
    add_evaluate(
        make_xbyak_intrin(datatypes::void_t, args.as_vector(), intrin, isa));
  }
  ...
  void transform_intrin_eval_attr(const expr &v, array_ref<expr> args, //
                                  ...
                                  xbyak_intrin_isa isa) {
    add_evaluate(
        copy_attr(*v, make_xbyak_intrin(datatypes::void_t, args.as_vector(),
                                        intrin, isa)));
  }
  ...
  void transform_4a_to_3a(const expr &dst, const expr &a, const expr &b,
                          const expr &c, xbyak_intrin_type intrin,
                          xbyak_intrin_isa isa) {
  ...
  void transform_x86_min_max(const expr &dst, const expr &lhs, const expr &rhs,
                             xbyak_intrin_type intrin) {
    // dst = lhs
    add_assignment(dst, lhs);
    // dst = min/max(rhs)
  ...
  void transform_x86_abs(const expr &dst, const expr &src) {
    ...
    // neg(rax)
    add_assignment(rax, make_xbyak_intrin(dtype, {}, xbyak_intrin_type::neg,
                                          xbyak_intrin_isa::x86));
    // if(SF != OF) rax = src
    add_assignment(
        rax, make_xbyak_intrin(dtype, {src}, xbyak_intrin_type::cmov,
                               xbyak_intrin_isa::x86,
  ...
  void transform_x86_mod_div(const expr &dst, const expr &lhs, const expr &rhs,
    ...
    if (rhs.isa<constant>() && rhs->dtype_ == datatypes::index) {
      ...
      if (mult.compensate_) {
        ...
        add_assignment(rdx, make_xbyak_intrin(dtype, {rax, lhs},
        ...
        add_assignment(rax, make_xbyak_intrin(
                                dtype, {builder::make_constant(mult.sft_pre_)},
                                xbyak_intrin_type::shr, xbyak_intrin_isa::x86));
        add_assignment(rdx,
        ...
        if (mult.sft_post_ > 0) {
          add_assignment(
              rdx,
              make_xbyak_intrin(dtype, {builder::make_constant(mult.sft_post_)},
                                xbyak_intrin_type::shr, xbyak_intrin_isa::x86));
        }
      } else {
        // %rdx = lhs
        ...
        if (mult.sft_pre_ > 0) {
          add_assignment(
              rdx,
              make_xbyak_intrin(dtype, {builder::make_constant(mult.sft_pre_)},
                                xbyak_intrin_type::shr, xbyak_intrin_isa::x86));
        }
        add_assignment(rax, builder::make_constant(mult.magic_));
        add_assignment(rdx, make_xbyak_intrin(dtype, {rax, rdx},
                                              xbyak_intrin_type::mulhl,
                                              xbyak_intrin_isa::x86));
        if (mult.sft_post_ > 0) {
          add_assignment(
      ...
      if (intrin == xbyak_intrin_type::mod) {
        ...
        add_assignment(
            rdx,
            make_xbyak_intrin(dtype, {rdx, builder::make_constant(divisor)},
                              xbyak_intrin_type::muli, xbyak_intrin_isa::x86));
        add_assignment(dst, lhs);
        add_assignment(dst,
                       make_xbyak_intrin(dtype, {rdx}, xbyak_intrin_type::sub,
                                         xbyak_intrin_isa::x86));
      ...
      auto sign_ext_rdx = make_stmt<assign_node_t>(
          rdx,
          make_xbyak_intrin(dst->dtype_, {rax}, xbyak_intrin_type::sign_ext));
      ...
      add_assignment(dst, make_xbyak_intrin(dst->dtype_, {tmp, rax, rdx},
  ...
  void transform_avx_div(const expr &dst, const expr &lhs, const expr &rhs,
    ...
    if (rhs.isa<constant>() && rhs->dtype_.is_etype(sc_data_etype::S32)) {
      ...
      const auto transform_mulh_s32 = [this, dtype](const expr &dst,
        ...
        add_assignment(
            magic,
            make_xbyak_intrin(dtype, {magic_c}, xbyak_intrin_type::broadcast,
                              xbyak_intrin_isa::avx,
                              xbyak_intrin_modifier(datatypes::s32)));
        add_assignment(hi1, make_xbyak_intrin(dtype, {lhs, magic},
                                              xbyak_intrin_type::mulhl,
                                              xbyak_intrin_isa::avx));
        add_assignment(hi1, make_xbyak_intrin(
                                dtype, {hi1, builder::make_constant(32)},
                                xbyak_intrin_type::shr, xbyak_intrin_isa::avx,
                                xbyak_intrin_modifier(dtype_64)));
        add_assignment(hi2, make_xbyak_intrin(
                                dtype, {lhs, builder::make_constant(32)},
                                xbyak_intrin_type::shr, xbyak_intrin_isa::avx,
                                xbyak_intrin_modifier(dtype_64)));
        add_assignment(hi2, make_xbyak_intrin(dtype, {hi2, magic},
                                              xbyak_intrin_type::mulhl,
                                              xbyak_intrin_isa::avx));
        add_assignment(hi2, make_xbyak_intrin(dtype, {hi2, mask_c},
                                              xbyak_intrin_type::bit_and,
                                              xbyak_intrin_isa::avx));
        add_assignment(dst, make_xbyak_intrin(dtype, {hi1, hi2},
      ...
      if (mult.power_of_2_) {
        ...
        // dst = sar(dst, [l])
        // dst = d < 0 ? -dst : dst
        add_assignment(dst, make_xbyak_intrin(dtype, {lhs, sft1},
                                              xbyak_intrin_type::sar,
                                              xbyak_intrin_isa::avx,
                                              xbyak_intrin_modifier(dtype)));
        add_assignment(dst, make_xbyak_intrin(dtype, {dst, sft2},
                                              xbyak_intrin_type::shr,
                                              xbyak_intrin_isa::avx,
                                              xbyak_intrin_modifier(dtype)));
        add_assignment(dst, make_xbyak_intrin(dtype, {dst, lhs},
                                              xbyak_intrin_type::add,
                                              xbyak_intrin_isa::avx));
        add_assignment(dst, make_xbyak_intrin(dtype, {dst, sft3},
                                              xbyak_intrin_type::sar,
                                              xbyak_intrin_isa::avx,
                                              xbyak_intrin_modifier(dtype)));
        if (mult.negative_) {
          ...
          add_assignment(simd_zero,
                         make_xbyak_intrin(dtype, {simd_zero, simd_zero},
                                           ...
                                           xbyak_intrin_isa::avx));
          add_assignment(dst, make_xbyak_intrin(dtype, {simd_zero, dst},
        ...
        add_assignment(dst, make_xbyak_intrin(dtype, {dst, lhs},
                                              xbyak_intrin_type::add,
                                              xbyak_intrin_isa::avx));
        add_assignment(dst, make_xbyak_intrin(dtype, {dst, sft},
                                              xbyak_intrin_type::sar,
                                              xbyak_intrin_isa::avx,
                                              xbyak_intrin_modifier(dtype)));
        add_assignment(xsign, make_xbyak_intrin(dtype, {lhs, bit},
                                                xbyak_intrin_type::sar,
                                                xbyak_intrin_isa::avx,
                                                xbyak_intrin_modifier(dtype)));
        if (mult.negative_) {
          add_assignment(dst, make_xbyak_intrin(dtype, {xsign, dst},
                                                xbyak_intrin_type::sub,
                                                xbyak_intrin_isa::avx));
        } else {
          add_assignment(dst, make_xbyak_intrin(dtype, {dst, xsign},
                                                xbyak_intrin_type::sub,
                                                xbyak_intrin_isa::avx));
        }
      } else {
      ...
      add_assignment(dst, make_xbyak_intrin(dst->dtype_, {lhs, rhs},
                                            xbyak_intrin_type::div,
                                            xbyak_intrin_isa::avx));
    }
  }
  ...
  void transform_simd_reduce_seq(const expr &dst, const expr &src,
    ...
    while (lanes > 1) {
      ...
      // hig, low
      auto extract_hig =
          builder::make_var(dtype, "extract_hig_" + std::to_string(lanes));
      auto extract_low =
          builder::make_var(dtype, "extract_low_" + std::to_string(lanes));
      add_defination(extract_hig, linkage::local);
      add_defination(extract_low, linkage::local);
      // hig = ext_h(src)
      add_assignment(extract_hig,
                     make_xbyak_intrin(
                         dtype, {current_src}, xbyak_intrin_type::extract_high,
                         isa, xbyak_intrin_modifier(current_src->dtype_)));
      // low = ext_l(src)
      add_assignment(extract_low,
                     make_xbyak_intrin(
                         dtype, {current_src}, xbyak_intrin_type::extract_low,
                         isa, xbyak_intrin_modifier(current_src->dtype_)));
      // hig = op(low, hig)
      if (lanes == 1) {
        if (convert_x86_operation(dtype)) {
          ...
          add_assignment(dst, make_xbyak_intrin(dst->dtype_, {extract_hig},
                                                intrin, xbyak_intrin_isa::x86));
        } else {
          add_assignment(dst, make_xbyak_intrin(dst->dtype_,
                                                {extract_low, extract_hig},
                                                intrin, isa));
        }
      } else {
        add_assignment(extract_low,
                       make_xbyak_intrin(extract_low->dtype_,
                                         {extract_low, extract_hig}, intrin,
                                         isa));
      }
      // src = hig
  ...
  void transform_assign(const expr &dst, const expr &src,
    ...
    if (maybe_masked.isa<indexing>()) {
      ...
      if (mask.defined()) {
        ...
        add_assignment(dst, make_xbyak_intrin(
                                dst->dtype_, {src}, xbyak_intrin_type::mask_mov,
                                xbyak_intrin_isa::avx,
  ...
  void transform_gather(const expr &dst, const expr &src, const expr &idx) {
    ...
    add_assignment(xmm2, make_xbyak_intrin(dst->dtype_, {src, xmm1},
                                           xbyak_intrin_type::gather,
                                           xbyak_intrin_isa::avx,
  ...
  void transform_extract(const expr &dst, const expr &src, const expr &imm,
    ...
    add_assignment(dst, make_xbyak_intrin(dst->dtype_, {src, imm, elem_bits},
                                          xbyak_intrin_type::extract,
                                          xbyak_intrin_isa::avx));
  }

  void transform_broadcast(const expr &dst, const expr &src,
                           const sc_data_type_t &src_type) {
    // transform broadcast with type hint
    add_assignment(dst, make_xbyak_intrin(dst->dtype_, {src},
  ...
  void transform_avx_mask_cast(const expr &dst, const expr &src) {
    ...
    const auto is_scalar_mask = [](const expr &v) {
      ...
      return (type_lane == 1 && utils::is_one_of((int)type_size, 1, 2, 4, 8)) ||
    ...
    if (is_scalar_mask(src)) {
      ...
      case 1:
        cast_mask8_avx2(dst, tmp, dtype);
        break;
      case 2:
        cast_mask16_avx2(dst, tmp, dtype);
        break;
      case 4:
        cast_mask32_avx2(dst, tmp, dtype);
        break;
      default:
      ...
      if (src->dtype_.is_etype(sc_data_etype::BF16) ||
        ...
        add_assignment(dst,
                       make_xbyak_intrin(
                           dst->dtype_, {dst, builder::make_constant(0x5555)},
                           xbyak_intrin_type::bmi_pext, xbyak_intrin_isa::x86));
      }
    } else {
  ...
  expr cast_when_mask(expr cond, sc_data_type_t dtype) {
    auto lanes = dtype.lanes_;
    if (cpu_flags_.fAVX512F && cond->dtype_ != sc_data_type_t::boolean(lanes)) {
      auto tmp = load_when_imm(cond, "__msk_tmp_var");
      auto mask = builder::make_var(sc_data_type_t::boolean(lanes), "__mmask");
  ...
  void cast_mask8_avx2(const expr &mask, const expr &src,
    ...
    const std::vector<union_val> val1 = {
        UINT64_C(0x00000000), UINT64_C(0x00000000), UINT64_C(0x01010101),
        UINT64_C(0x01010101), UINT64_C(0x02020202), UINT64_C(0x02020202),
    ...
    const std::vector<union_val> val2 = {
        UINT64_C(0xf7fbfdfe), UINT64_C(0x7fbfdfef), UINT64_C(0xf7fbfdfe),
        UINT64_C(0x7fbfdfef), UINT64_C(0xf7fbfdfe), UINT64_C(0x7fbfdfef),
    ...
    //
    add_assignment(xmm0, make_xbyak_intrin(movd_type, {src}, //
                                           xbyak_intrin_type::movd,
                                           xbyak_intrin_isa::avx));
    add_assignment(ymm0,
    ...
    add_assignment(ymm0, make_xbyak_intrin(mask_type, {ymm0, table1}, //
                                           xbyak_intrin_type::pshuffle,
                                           xbyak_intrin_isa::avx,
                                           xbyak_intrin_modifier(mask_type)));
    add_assignment(ymm0, make_xbyak_intrin(mask_type, {ymm0, table2}, //
                                           xbyak_intrin_type::bit_or,
                                           xbyak_intrin_isa::avx));
    add_assignment(ymm1,
                   make_xbyak_intrin(
  ...
  void cast_mask16_avx2(const expr &mask, const expr &src,
                        const sc_data_type_t &dtype) {
    // vmovd        xmm0, src
    ...
    add_assignment(ymm0,
                   make_xbyak_intrin(
                       mask_type, {ymm0, table1}, //
                       xbyak_intrin_type::cmp_set, xbyak_intrin_isa::avx,
                       xbyak_intrin_modifier(xbyak_condition::eq, mask_type)));
    add_assignment(mask, ymm0);
  }

  void cast_mask32_avx2(const expr &mask, const expr &src,
                        const sc_data_type_t &dtype) {
    // vmovd        xmm0, src
    // vpbroadcastd ymm0, xmm0
    // vpand        ymm0, ymm0, table1
