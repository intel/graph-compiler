...
public:
  ...
  stmt_c process_scope_indexing(const expr &value, const expr &var,
                                const stmt_c &v) {
    ...
    if ((value.defined() && value->node_type_ != sc_expr_type::indexing) ||
      ...
      if (!idx_transform_stmt.empty()) {
        ...
        if (v.isa<define_c>()) {
          ...
          cur_list.emplace_back(builder::make_assign_unattached(var, vv));
        }
        idx_var_map.clear();
  ...
  expr_c visit(indexing_c v) override {
    ...
    if (vv.defined() && is_base_type_less_than_32bit(vv->dtype_) &&
      ...
      auto nested_assign = builder::make_assign_unattached(nested_idx_var, vv);
