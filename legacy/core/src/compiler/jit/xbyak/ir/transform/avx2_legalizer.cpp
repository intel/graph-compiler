...
public:
  ...
  expr_c visit(cast_c v) override {
    ...
    auto is_32bit_avx2_cast = [&](const sc_data_etype &cmp_dst_type,
      ...
      return src_dtype.lanes_ > 1 && (dst_dtype.type_code_ == cmp_dst_type) &&
             (src_dtype.type_code_ == cmp_src_type);
    };
    auto s32_smid_cast = [&](bool is_u16 = false) {
      expr cast_u16 =
          builder::make_cast(sc_data_type_t::u16(src_dtype.lanes_ * 2), src);
      expr permutexvar_expr = builder::make_permutexvar(
          builder::make_constant(UINT64_C(0b11011000)), cast_u16, 4);
      expr extract_u16_expr =
          builder::make_extract(permutexvar_expr, 0, src_dtype.lanes_);
      if (is_u16) {
        return extract_u16_expr;
      }
    ...
    if ((is_32bit_avx2_cast(sc_data_etype::U8, sc_data_etype::S32) ||
         ...
         is_32bit_avx2_cast(sc_data_etype::U8, sc_data_etype::U32))) {
      return s32_smid_cast();
    } else if ((is_32bit_avx2_cast(sc_data_etype::U16, sc_data_etype::U32) ||
                is_32bit_avx2_cast(sc_data_etype::U16, sc_data_etype::S32))) {
  ...
  expr_c visit(select_c v) override {
    ...
    if (dtype != vv->cond_->dtype_) {
      return builder::make_select(avx_mask_cast(vv->cond_, dtype), vv->l_,
                                  vv->r_);
    } else {
      return vv;
    }
...
#define DEFINE_OFFSET_ITER_VAR()                                               \
  ...
  cur_list.emplace_back(builder::make_var_tensor_def_unattached(               \
      offset_var, linkage::local,                                              \
      builder::make_constant({0UL}, datatypes::index)));                       \
  ...
  expr iter_var = builder::make_var(datatypes::s32,                            \
                                    "iter_var" + std::to_string(var_index++)); \
  cur_list.emplace_back(                                                       \
      builder::make_var_tensor_def_unattached(iter_var, linkage::local, 1));   \
  auto mask = builder::make_var(datatypes::s32,                                \
                                "mask_var" + std::to_string(var_index++));     \
  cur_list.emplace_back(builder::make_var_tensor_def_unattached(               \
      mask, linkage::local, builder::make_cast(datatypes::s32, vv->mask_)));
...
#define INSERT_EXTRACT_DATA_TO_VAR(target_var, data, elembits, imm)            \
  ...
  cur_list.emplace_back(                                                       \
      builder::make_if_else_unattached(mask >= iter_var, then_block, stmt())); \
...
#define INSERT_EXTRACT_DATA_u8s8(var_name, data)                               \
  ...
  stmt_c insert_byte_by_byte(const expr &value, const expr &var) {
    ...
    if (value.defined() && (value->node_type_ == sc_expr_type::indexing)) {
      ...
      if (vv->mask_.defined() && is_base_type_less_than_32bit(dtype)) {
        ...
        sc_data_type_t insert_type = is_bf16 ? sc_data_type_t::bf16(type_lanes)
                                     : is_s8 ? sc_data_type_t::s8(type_lanes)
                                             : sc_data_type_t::u8(type_lanes);

        // original defined var
        cur_list.emplace_back(builder::make_var_tensor_def_unattached(
            var, linkage::local, builder::make_constant({0UL}, dtype)));
        ...
        auto make_insert_var = [&](expr &var) {
          ...
          cur_list.emplace_back(builder::make_var_tensor_def_unattached(
              var, linkage::local, builder::make_constant({0UL}, insert_type)));
        };

        make_insert_var(insert_var_1);
        if (need_2_xmm) {
          make_insert_var(insert_var_2);
        }
  ...
  stmt_c extract_byte_by_byte(const expr &value, const expr &var) {
    ...
    if (var.defined() && (var->node_type_ == sc_expr_type::indexing)) {
      ...
      if (vv->mask_.defined() && is_base_type_less_than_32bit(dtype)) {
        ...
        bool need_2_xmm = is_bf16 ? lanes > 8 : lanes > 16;
        const int type_lanes = need_2_xmm ? lanes / 2 : lanes;
        sc_data_type_t extract_type = is_bf16 ? sc_data_type_t::bf16(type_lanes)
                                      : is_s8 ? sc_data_type_t::s8(type_lanes)
                                              : sc_data_type_t::u8(type_lanes);
        ...
        auto make_extract_var = [&](expr &var, const int imm) {
          ...
          cur_list.emplace_back(builder::make_assign_unattached(
              var, builder::make_extract(value, imm, type_lanes)));
        };
        ...
        if (need_2_xmm) {
          ...
          extract_var_1 = builder::make_var(
              extract_type, "extract_var" + std::to_string(var_index++));
          cur_list.emplace_back(
              builder::make_var_tensor_def_unattached(extract_var_1));
          cur_list.emplace_back(
              builder::make_assign_unattached(extract_var_1, value));
        }
  ...
  stmt_c visit(assign_c v) override {
    ...
    } else if (value.defined() && value->node_type_ == sc_expr_type::indexing) {
      auto res = insert_byte_by_byte(value, var);
      return res.defined() ? res : assign;
    }
    return assign;
...
protected:
  ...
  expr avx_mask_cast(const expr &v, sc_data_type_t dtype) {
    ...
    if (nested.has_value()) {
      ...
      return builder::make_x86_intrin(x86_intrin_type::avx_mask_cast, {v},
                                      {{"dtype", dtype}});
    } else {
      return v;
    }
  ...
  expr avx_uint_cmp(const cmp_c &v) {
    ...
    const auto &r = v->r_;
    switch (t) {
    case sc_etype::cmp_eq:
      return transform_uint_eq(l, r);
    case sc_etype::cmp_lt:
      return transform_uint_lt(l, r);
    case sc_etype::cmp_le:
      return transform_uint_le(l, r);
    case sc_etype::cmp_ne:
      return transform_uint_ne(l, r);
    case sc_etype::cmp_ge:
      return transform_uint_ge(l, r);
    case sc_etype::cmp_gt:
      return transform_uint_gt(l, r);
    default:
      COMPILE_ASSERT(false, "Invalid compare type: " << t);
    }
    return v.remove_const();
  }
  ...
  expr avx_sint_cmp(const cmp_c &v) {
    ...
    switch (t) {
    case sc_etype::cmp_eq:
      return transform_sint_eq(l, r);
    case sc_etype::cmp_lt:
      return transform_sint_lt(l, r);
    case sc_etype::cmp_le:
      return transform_sint_le(l, r);
    case sc_etype::cmp_ne:
      return transform_sint_ne(l, r);
    case sc_etype::cmp_ge:
      return transform_sint_ge(l, r);
    case sc_etype::cmp_gt:
      return transform_sint_gt(l, r);
    default:
      COMPILE_ASSERT(false, "Invalid compare type: " << t);
    }
    return v.remove_const();
...
func_c avx2_legalizer_t::operator()(func_c v) {
  ...
  if (target_machine_.cpu_flags_.fAVX512F) {
    return v;
  }
