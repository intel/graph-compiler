...
public:
  ...
  expr extract_call(expr v) {
    auto call_var = builder::make_var(v->dtype_, "tmp_call_var_" +
                                                     std::to_string(index_++));
    stmt_seq_.emplace_back(
        make_stmt<define_node_t>(call_var, linkage::local, std::move(v)));
    return call_var;
  }
};
  ...
  expr_c dispatch(expr_c v) override {
    if (v.isa<call>()) {
      ...
      for (size_t i = 0; i < old_args.size(); i++) {
        ...
        auto arg_placeholder = builder::make_var(
            arg->dtype_,
            "call_arg_" + std::to_string(index_) + "_" + std::to_string(i + 1));
        // Create wrapper for actural arg value
        auto arg_wrapper =
            make_xbyak_intrin(arg->dtype_, {arg}, xbyak_intrin_type::call_arg);
        ...
        if (initial_loc.get_type() == abi_value_location::tag_type::REGISTER) {
          ...
          if (arg_reg.isREG() && arg_reg.getIdx() == Xbyak::Operand::RDX) {
            is_reg_rdx = true;
          }
        }
      ...
      index_++;
      return copy_attr(*vv,
                       make_expr<call_node>(vv->func_, std::move(new_args)));
    } else {
