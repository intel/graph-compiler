...
static handler_table register_handlers() {
  ...
  REGISTER_INTRIN("SATURATED_CAST", x86, saturated_cast, directed_dst_mem,
                  _2A_);
  REGISTER_INTRIN("SATURATED_CAST", avx, saturated_cast, directed_dst_mem,
                  _2A_);
  REGISTER_INTRIN("ROUND_AND_CAST", x86, round_and_cast, directed_dst_reg,
                  _2A_);
  REGISTER_INTRIN("ROUND_AND_CAST", avx, round_and_cast, directed_dst_reg,
                  _2A_);
  ...
  REGISTER_INTRIN("AVX_UNPACK_HIGH", avx, unpack_high, directed_end_mem, _4A_);

  REGISTER_INTRIN("AVX_EXTRACT_LOW", avx, extract_low, directed_dst_mem, _2A_);
  REGISTER_INTRIN("AVX_EXTRACT_HIGH", avx, extract_high, directed_all_reg,
                  _2A_);

  REGISTER_INTRIN("AVX_PERMUTEX2VAR", avx, permutex2var, compound_end_mem,
                  _3A_);
  REGISTER_INTRIN("AVX_PERMUTEXVAR", avx, permutexvar, directed_end_mem, _4A_);
...
xbyak_intrinsic_handler_t &get_xbyak_intrin_handler(xbyak_intrin_isa isa,
                                                    int64_t intrin) {
  auto &handler = xbyak_handlers[TO_INDEX(isa)][TO_INDEX(intrin)];
  COMPILE_ASSERT(handler, "Invalid isa-intrin code: " << TO_INDEX(isa) << " - "
                                                      << TO_INDEX(intrin));
  return *handler;
}
...
xbyak_condition get_xbyak_condition(sc_expr_type t) {
  ...
  case sc_expr_type::cmp_eq:
    return xbyak_condition::eq;
  case sc_expr_type::cmp_lt:
    return xbyak_condition::lt;
  case sc_expr_type::cmp_le:
    return xbyak_condition::le;
  case sc_expr_type::cmp_ne:
    return xbyak_condition::ne;
  case sc_expr_type::cmp_ge:
    return xbyak_condition::ge;
  case sc_expr_type::cmp_gt:
    return xbyak_condition::gt;
  default:
...
expr xbyak_intrin_node::remake() const {
  expr node = make_xbyak_intrin(this->dtype_, this->args_,
                                static_cast<xbyak_intrin_type>(this->type_),
                                this->isa_, this->modifier_);
...
xbyak_intrin_node::xbyak_intrin_node(const std::vector<expr> &args,
    ...
    : low_level_intrin_node(low_level_intrin_kind::x86_xbyak,
      ...
      modifier_(std::move(modifier)), isa_(isa) {
...
expr make_xbyak_intrin(sc_data_type_t dtype, const std::vector<expr> &values,
                       ...
                       xbyak_intrin_modifier modifier) {
  expr node =
      make_expr<xbyak_intrin_node>(values, intrin, isa, std::move(modifier));
  node->dtype_ = dtype;
  return node;
}
...
expr make_physical_reg(sc_data_type_t dtype, const Xbyak::Reg &reg,
  ...
  expr node =
      builder::make_var(dtype, std::string("%") + reg.toString() + post);
