...
private:
  ...
  void update_liveness(array_ref<expr> ref, stmt_index_t index) {
    for (auto &v : ref) {
      ...
      auto def_loop_depth_ =
          expr_data.def_scope_ ? GET_STMT_DATA(expr_data.def_scope_).loop_depth_
                               : 0;
      auto cur_loop_depth_ =
          current_scope() ? GET_STMT_DATA(current_scope()).loop_depth_ : 0;
  ...
  void update_dependency_liveness(const expr &v, stmt_index_t index) {
    ...
    case sc_expr_type::call: {
      ...
      if (func) {
        update_liveness({expr(func)}, index);
      }
