...
public:
  ...
  virtual_reg_t *encompassing(const live_range_t &range) {
    ...
    auto query_func = [&](virtual_reg_t *vreg) {
      ...
      if (live_range.encompasses(range)) {
        virtual_regs.insert(vreg);
      }
    };
    query(range.start_, range.end_, query_func);
    if (virtual_regs.empty()) {
      return nullptr;
    }
...
public:
  ...
  spill_weight_t interfered_weights(virtual_reg_t *virt_reg,
                                    virt_reg_index_t index) {
    assert(virt_reg);
    assert(index >= 0 && index < virtual_slots_sum_);
  ...
  std::set<virtual_reg_t *> interfered_regs(virtual_reg_t *virt_reg,
                                            virt_reg_index_t index) {
    assert(virt_reg);
    assert(index >= 0 && index < virtual_slots_sum_);
  ...
  virtual_reg_t *encompassing_reg(const live_range_t &range,
                                  virt_reg_index_t index) {
    assert(!range.empty());
    assert(index >= 0 && index < virtual_slots_sum_);
...
public:
  ...
  virtual_slots_map_t(const x86_64::target_profile_t &profile,
    ...
    // ========================================
    auto get_allocatable_regs = [&](const virt_reg_type &reg_type,
      ...
      for (size_t i = 0; i < alloc_regs.size(); i++) {
        ...
        allocatable_regs_name_.push_back(alloc_regs[i].toString());
    ...
    get_allocatable_fp_vex_regs(virt_reg_type::fp_vex_reg,
                                profile.alloc_xmm_vex_regs_);
    ...
    // Allocatable gp regs to virtual indexes
    get_callee_saved_regs(virt_reg_type::gp_reg, profile.callee_saved_gp_regs_);
    // Allocatable fp regs to virtual indexes
    if (!fp_regs_volatile) {
      get_callee_saved_regs(virt_reg_type::fp_reg,
