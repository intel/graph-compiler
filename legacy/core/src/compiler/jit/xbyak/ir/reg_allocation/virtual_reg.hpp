...
struct virtual_reg_t {
  ...
  void add_weight(spill_weight_t weight) {
    spill_weight_ =
        std::min(spill_weight_const::infinity - 1, spill_weight_ + weight);
  ...
  friend std::ostream &operator<<(std::ostream &os, const virtual_reg_t &m) {
    ...
    case virt_reg_stat::allocated: {
      os << ": %" << type_enum_str[static_cast<int>(m.type_)] << m.index_;
    } break;
    default:
      break;
    }
...
inline virt_reg_type get_virt_reg_type(const sc_data_type_t &t, bool is_avx512,
                                       bool force_fp_vex = false) {
