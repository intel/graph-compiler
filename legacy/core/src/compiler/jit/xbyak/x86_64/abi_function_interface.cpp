...
static abi_function_interface
compute_iface_system_v(const target_profile_t &profile,
  ...
  for (size_t param_idx = 0; param_idx < param_types.size(); ++param_idx) {
    ...
    case abi_value_kind::INTEGER:
      if (next_arg_gp_idx < profile.func_arg_gp_regs_.size()) {
        const Xbyak::Reg64 reg =
            to_reg64(profile.func_arg_gp_regs_[next_arg_gp_idx++]);
    ...
    case abi_value_kind::SSE:
      if (next_arg_xmm_idx < profile.func_arg_xmm_regs_.size()) {
        const Xbyak::Xmm reg =
            to_xmm(profile.func_arg_xmm_regs_[next_arg_xmm_idx++]);
        loc.set_to_register(reg);
      } else {
        stack_param_indices.push_back(param_idx);
      }
    ...
    case abi_value_kind::SSEUPx15_SSE:
      COMPILE_ASSERT(false, "Not yet supported: callers/callees with parameter "
                            "type f32x16");
      break;
    }
  }
  ...
  for (const size_t param_idx : stack_param_indices) {
    ...
    iface.initial_rsp_alignment_ =
        std::max(iface.initial_rsp_alignment_, r.abi_precall_stack_alignment_);
  ...
  if (ret_cpu_dtype != cpu_data_type::void_t) {
    ...
    switch (r.abi_initial_val_kind_) {
    case abi_value_kind::INTEGER:
      iface.return_val_loc_.set_to_register(profile.func_return_gp_reg_);
      break;

    case abi_value_kind::SSE:
      iface.return_val_loc_.set_to_register(profile.func_return_xmm_reg_);
      break;
...
static abi_function_interface
compute_iface_microsoft(const target_profile_t &profile,
                        const std::vector<cpu_data_type> &param_types,
                        const cpu_data_type ret_cpu_dtype) {
  ...
  for (size_t param_idx = 0; param_idx < param_types.size(); ++param_idx) {
    ...
    case abi_value_kind::INTEGER:
      if (param_idx < profile.func_arg_gp_regs_.size()) {
        const Xbyak::Reg64 reg = to_reg64(profile.func_arg_gp_regs_[param_idx]);
        loc.set_to_register(reg);
      } else {
        stack_param_indices.push_back(param_idx);
      }
    ...
    case abi_value_kind::SSE:
      if (param_idx < profile.func_arg_xmm_regs_.size()) {
        const Xbyak::Xmm reg = to_xmm(profile.func_arg_xmm_regs_[param_idx]);
        loc.set_to_register(reg);
      } else {
        stack_param_indices.push_back(param_idx);
      }
...
static abi_function_interface
compute_iface(const target_profile_t &profile,
              const std::vector<cpu_data_type> &param_types,
              const cpu_data_type ret_cpu_dtype) {
...
abi_function_interface::get_stack_params_descending_idx() const {
  ...
  for (int idx = int(param_locs_.size() - 1); idx >= 0; --idx) {
    if (param_locs_[idx].get_type() == abi_value_location::tag_type::STACK) {
      v.push_back(idx);
    }
  }
...
abi_function_interface::get_register_params_ascending_idx() const {
  ...
  for (size_t idx = 0; idx < param_locs_.size(); ++idx) {
    if (param_locs_[idx].get_type() == abi_value_location::tag_type::REGISTER) {
      v.push_back(idx);
    }
  }
...
abi_function_interface::ptr
abi_function_interface::make_interface(const target_profile_t &profile,
