...
struct vtune_debug_info_mgr_t : debug_info_mgr {
  vtune_debug_info_mgr_t(void *base, size_t size, const std::string &src_path,
    ...
    for (size_t i = 0; i < funcs.size(); i++) {
      ...
      jmethod.method_size =
          (unsigned int)((uint8_t *)funcs[i].end_ - (uint8_t *)funcs[i].start_);
...
std::unique_ptr<debug_info_mgr>
create_vtune_debug_info(void *base, size_t size, const std::string &src_path,
                        const std::vector<func_symbol> &funcs,
                        const std::vector<debug_line> &lines) {
  return utils::make_unique<vtune_debug_info_mgr_t>(base, size, src_path, funcs,
                                                    lines);
}
