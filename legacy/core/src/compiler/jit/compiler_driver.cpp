...
SC_API std::shared_ptr<jit_function_t> compiler_driver(
  ...
  graph_driver(graph, ctx, in_cfg, nullptr, 0, 0, 0, nullptr, nullptr, nullptr,
               /*allow_cache*/ true);
  if (auto code = graph.attrs_.get_or_else<std::shared_ptr<jit_module_code>>(
          "graph_code_cache", nullptr)) {
    auto static_tbl =
        prepare_static_table_for_cached_code(*code->graph_cache_handle_, graph);
  ...
  if (auto graph_key =
          graph.attrs_
              .get_or_else<std::shared_ptr<prehashed_graph_for_code_share_t>>(
