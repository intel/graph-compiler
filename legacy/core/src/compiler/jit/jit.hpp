...
public:
   ...
   * @param args the arguments
   */
  virtual void call_generic(runtime::stream_t *stream,
                            generic_val *args) const = 0;
  ...
  template <typename... Args> void call_default(Args... args) const {
...
public:
  ...
  void postprocess(const const_ir_module_ptr &ir_mod, statics_table_t &globals);
...
struct SC_INTERNAL_API jit_module
  ...
  virtual std::shared_ptr<jit_function_t> get_function(const std::string &name);
  /// This method only exists to help with debugging.
  virtual std::vector<std::string> get_temp_filenames() const {
...
public:
  ...
  static std::shared_ptr<jit_function_t>
  make(const std::shared_ptr<jit_module> &module, void *funcptr, void *wrapper,
...
public:
  ...
  virtual std::shared_ptr<jit_module>
  make_jit_module(const_ir_module_ptr module, bool generate_wrapper) = 0;
   ...
   * */
  std::shared_ptr<jit_function_t> get_entry_func(const ir_module_ptr &m,
                                                 bool generic = true);
  ...
  // machine
  static void set_target_machine(jit_kind kind, scflags_t &sc_flags,
