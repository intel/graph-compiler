...
static void *resolve_llvm_symbol(llvm::ExecutionEngine *engine,
...
llvm_jit::make_jit_module(const_ir_module_ptr module, bool generate_wrapper) {
  ...
  auto engine =
      llvm::EngineBuilder(std::move(llvmmod))
          .setErrorStr(&err)
          .setOptLevel(llvm_opt)
          .setEngineKind(llvm::EngineKind::JIT)
          .setMCJITMemoryManager(utils::make_unique<sc_llvm_jit_resolver>())
          .create(tm);
  ...
  auto init_func =
      reinterpret_cast<init_func_t>(resolve_llvm_symbol(engine, "__sc_init__"));
  if (init_func) {
    init_func(nullptr, attr_table.data_.data_);
  }
  ...
  if (copied_ir_module) {
    ...
    auto gen =
        create_c_generator(of, context_, generate_wrapper, &optional_dump);
...
llvm_jit_module_code::llvm_jit_module_code(
    ...
    : jit_module_code(managed_thread_pool), listeners_(std::move(listeners)),
      llvm_ctx_(std::move(llvm_ctx)), engine_(std::move(engine)),
...
void *llvm_jit_module_code::get_function(const std::string &name,
                                         void *&wrapper) {
