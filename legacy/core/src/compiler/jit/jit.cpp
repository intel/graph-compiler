...
std::shared_ptr<jit_function_t>
jit_engine_t::get_entry_func(const ir_module_ptr &ir_mod, bool generic) {
  ...
  if (jm->code_) {
    jit_func->inplace_pairs_ = jm->code_->inplace_pairs_;
  }
  return jit_func;
}
...
#if SC_CFAKE_JIT_ENABLED
  case jit_kind::cfake:
    return utils::make_unique<cfake_jit>(ctx);
#endif
#if defined(SC_LLVM_BACKEND)
  case jit_kind::llvm:
    return utils::make_unique<llvm_jit>(ctx);
#endif
#if SC_BUILTIN_JIT_ENABLED
  case jit_kind::xbyak:
    return utils::make_unique<xbyak_jit>(ctx);
#endif
  default:
...
void jit_engine_t::set_target_machine(jit_kind kind, scflags_t &sc_flags,
                                      runtime::target_machine_t &tm) {
  switch (kind) {
  case jit_kind::cfake:
...
#else
  {
    ...
    if (flags.fAVX512AMXBF16 && flags.fAVX512AMXTILE && flags.fAVX512AMXINT8) {
...
void jit_module_code::postprocess(const const_ir_module_ptr &ir_mod,
  ...
  if (ir_mod->get_entry_func()) {
    ...
    inplace_pairs_ = any_map_t::fetch_or_else(
        ir_mod->get_entry_func()->attr_.get(), function_attrs::inplace_hint,
...
void jit_module_code::update_op_dispatch_table(
  ...
  for (auto &kv : compiler_tables) {
    ...
    if (!compiler_format_table.empty()) {
      uint32_t format_num_keys = compiler_format_table.begin()->first.size();
      size_t format_capacity =
          math_utils::nearest_power_of_2(compiler_format_table.size()) *
          capacity_coefficient;
      runtime_table->format_table_ =
          utils::make_unique<runtime::hash_dispatch_table_t>(format_num_keys,
                                                             format_capacity);
    }
    if (!compiler_impl_kind_table.empty()) {
      uint32_t impl_num_keys = compiler_impl_kind_table.begin()->first.size();
      size_t impl_capacity =
          math_utils::nearest_power_of_2(compiler_impl_kind_table.size()) *
          capacity_coefficient;
      runtime_table->impl_kind_table_ =
          utils::make_unique<runtime::hash_dispatch_table_t>(impl_num_keys,
                                                             impl_capacity);
    }
    if (!compiler_kernel_table.empty()) {
      uint32_t kernel_num_keys = compiler_kernel_table.begin()->first.size();
      size_t kernel_capacity =
          math_utils::nearest_power_of_2(compiler_kernel_table.size()) *
          capacity_coefficient;
      // Currently we use hash dispatch table for kernel dispatch
      runtime_table->kernel_table_ =
          utils::make_unique<runtime::hash_dispatch_table_t>(kernel_num_keys,
                                                             kernel_capacity);
    ...
    for (auto &format_kv : compiler_format_table) {
      runtime_table->set_format_table_keys(
          reinterpret_cast<uint64_t *>(
              const_cast<runtime::dispatch_key *>(format_kv.first.data())),
          format_kv.first.size(),
          reinterpret_cast<uint64_t *>(
              const_cast<runtime::dispatch_key *>(format_kv.second.data())),
    ...
    for (auto &impl_kv : compiler_impl_kind_table) {
      runtime_table->set_impl_kind_table_keys(
          const_cast<uint64_t *>(impl_kv.first.data()), impl_kv.first.size(),
          impl_kv.second);
    }
    // initialize kernel table
    for (auto &kernel_kv : compiler_kernel_table) {
      void *kernel_addr =
          kernel_kv.second.already_compiled()
              ? get_address_of_symbol(kernel_kv.second.name_or_postfix_)
              : nullptr;
      runtime_table->kernel_table_->set(
          reinterpret_cast<uint64_t *>(
              const_cast<runtime::dispatch_key *>(kernel_kv.first.data())),
...
void jit_module_code::update_runtime_data(const const_ir_module_ptr &ir_mod,
                                          statics_table_t &globals) {
  ...
  brg_handles_.insert(brg_handles_.end(), brg_handles.begin(),
                      brg_handles.end());
}
...
std::shared_ptr<jit_function_t>
jit_module::get_function(const std::string &name) {
  void *wrapper = nullptr;
  auto func = code_->get_function(name, wrapper);
  if (func || wrapper) {
    if (runtime_config_t::get().execution_verbose_) {
      return general_jit_function_t::make(shared_from_this(), func, wrapper,
                                          name, code_->managed_thread_pool_);
    } else {
      return general_jit_function_t::make(shared_from_this(), func, wrapper,
                                          std::string(),
                                          code_->managed_thread_pool_);
    }
  } else {
    return nullptr;
  }
...
template <bool execution_verbose> struct jit_timer_t {
  jit_timer_t(const general_jit_function_t *) {}
};

template <> struct jit_timer_t<true> {
  struct callback_t {
    const general_jit_function_t *ths;
    void operator()(utils::time_duration dur) const {
      using namespace std::chrono;
      double duration =
          static_cast<double>(duration_cast<nanoseconds>(dur).count()) / 1e6;
      printf("Entry point: %s@%zu. Time elapsed: %lf ms\n", ths->fname_.c_str(),
             ths->module_->code_->module_id_, duration);
    }
  };
...
template <thread_pool_mode_t thread_pool_init> struct thread_pool_caller_t {
...
template <> struct thread_pool_caller_t<thread_pool_mode_t::MANAGED> {
  static void call(functype f, runtime::stream_t *stream, void *module_data,
                   generic_val *args) {
    runtime::thread_manager::cur_mgr.run_main_function(f, stream, module_data,
                                                       args);
  }
};

template <> struct thread_pool_caller_t<thread_pool_mode_t::DYNAMIC> {
  static void call(functype f, runtime::stream_t *stream, void *module_data,
                   generic_val *args) {
...
class injected_general_jit_function_t : public general_jit_function_t {
  void call_generic(runtime::stream_t *stream,
  ...
  void call_generic(runtime::stream_t *stream, void *module_data,
    ...
    before_kernel_run();
    thread_pool_caller_t<thread_pool_init>::call(f, stream, module_data, args);
    after_kernel_run()
  }
...
void general_jit_function_t::call_generic(runtime::stream_t *stream,
                                          generic_val *args) const {
  general_jit_function_t::call_generic(stream, module_->globals_.data_.data_,
                                       args);
}
...
std::shared_ptr<jit_function_t> general_jit_function_t::make(
  ...
  case thread_pool_mode_t::MANAGED:
    if (runtime_cfg.execution_verbose_) {
      return std::shared_ptr<general_jit_function_t>(
          new injected_general_jit_function_t<thread_pool_mode_t::MANAGED,
                                              true>(module, funcptr, wrapper,
                                                    name));
    } else {
      return std::shared_ptr<general_jit_function_t>(
          new injected_general_jit_function_t<thread_pool_mode_t::MANAGED,
                                              false>(module, funcptr, wrapper,
                                                     name));
    }
    break;
  ...
  case thread_pool_mode_t::DYNAMIC:
    if (runtime_cfg.execution_verbose_) {
      return std::shared_ptr<general_jit_function_t>(
          new injected_general_jit_function_t<thread_pool_mode_t::DYNAMIC,
                                              true>(module, funcptr, wrapper,
                                                    name));
    } else {
      return std::shared_ptr<general_jit_function_t>(
