...
protected:
  ...
  void trinary_func_codegen_c(const std::vector<expr> &args,
                              const char *funcname);
  void binary_func_codegen_c(const std::vector<expr> &args,
                             const char *funcname);
...
extern const_ir_module_ptr preprocess_module_and_make_decl(
    ...
    std::ostream &source, c_generator_optional_out_t *optionalout = nullptr);
...
extern void write_cpp_generic_wrapper(std::ostream *source_, const func_c &f,
                                      bool is_parallel);
} // namespace gc
} // namespace graph
} // namespace impl
