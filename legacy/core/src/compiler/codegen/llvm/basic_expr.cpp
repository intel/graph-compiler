...
void codegen_llvm_vis_t::generate_bin_op(const expr_c &v, const expr_c &l,
                                         const expr_c &r) {
...
#define HANDLE_BIN_OP2(scname, intname, fpname, case_other)                    \
  case sc_expr_type::scname:                                                   \
    ...
    default:                                                                   \
      COMPILE_ASSERT(false, "Cannot generate binary op for this type: " << v); \
    }                                                                          \
    break
...
#define HANDLE_BIN_SIGNED_OP2(scname, intname1, intname2, fpname)              \
  case sc_expr_type::scname:                                                   \
    switch (cate) {                                                            \
    case CATE_INT:                                                             \
      current_val_ = builder_.Create##intname1##S##intname2(lhs, rhs);         \
      break;                                                                   \
    case CATE_UINT:                                                            \
      current_val_ = builder_.Create##intname1##U##intname2(lhs, rhs);         \
      break;                                                                   \
    case CATE_FLOAT:                                                           \
      current_val_ = builder_.Create##fpname(lhs, rhs);                        \
      break;                                                                   \
...
void codegen_llvm_vis_t::view(binary_c v) { generate_bin_op(v, v->l_, v->r_); }
void codegen_llvm_vis_t::view(cmp_c v) { generate_bin_op(v, v->l_, v->r_); }

void codegen_llvm_vis_t::view(logic_c v) { generate_bin_op(v, v->l_, v->r_); }
...
void codegen_llvm_vis_t::view(constant_c v) {
  ...
  case CATE_FLOAT: {
    for (auto &val : v->value_) {
      if (is_bf16) {
        ...
        vals.push_back(ConstantInt::get(llvm_base_type, val_u64, false));
      } else if (is_f16) {
        uint16_t val_u16 = fp16_t(val.f32).storage_;
        vals.push_back(ConstantFP::get(
            llvm_base_type, APFloat(APFloat::IEEEhalf(), APInt(16, val_u16))));
      } else {
        vals.push_back(ConstantFP::get(llvm_base_type, APFloat(val.f32)));
      }
    }
  } break;
  case CATE_UINT:
  case CATE_INT: {
    ...
    for (auto &val : v->value_) {
      vals.push_back(ConstantInt::get(llvm_base_type, val.u64, is_signed));
    }
  } break;
  default:
    ...
    for (auto &val : v->value_) {
      vals.push_back(ConstantExpr::getIntToPtr(builder_.getInt64(val.u64),
                                               llvm_base_type));
    }
    break;
  }
  if (vals.size() != v->dtype_.lanes_) {
    COMPILE_ASSERT(vals.size() == 1, "Bad constant node. Expecting 1 value");
    // broadcast value
    current_val_ = builder_.CreateVectorSplat(v->dtype_.lanes_, vals.front());
  } else {
    if (vals.size() != 1) {
...
void codegen_llvm_vis_t::view(var_c v) {
  ...
  if (is_lvalue_mode_) {
    ...
    if (is_special_params) {
      ...
      current_val_ =
          builder_.CreateLoad(get_type(v->dtype_), ret, v->name_ + "_v");
    }
  }
}
...
void codegen_llvm_vis_t::view(cast_c v) {
  ...
  auto check_cate = [&v]() {
    COMPILE_ASSERT(v->dtype_ == datatypes::generic, "Unexpected outtype " << v);
  };
  ...
  if (v->in_->dtype_.is_etype(sc_data_etype::F32) &&
    ...
    case 1: {
      ...
      vec = builder_.CreateIntrinsic(
          ...
          {vec, UndefValue::get(get_llvm_bf16_native_type(8)),
           /*mask*/
           builder_.CreateVectorSplat(4, builder_.getInt1(true))});
    ...
    case 4:
      ...
      current_val_ = builder_.CreateShuffleVector(current_val_, current_val_,
                                                  ArrayRef<int>({0, 1, 2, 3}));
#else
      current_val_ = builder_.CreateShuffleVector(current_val_, {0, 1, 2, 3});
#endif
      break;
    case 8:
      current_val_ = builder_.CreateIntrinsic(
  ...
  case CATE_FLOAT: {
    ...
    case CATE_OTHER: {
      check_cate();
      auto bits = utils::get_sizeof_etype(v->in_->dtype_.type_code_) * 8;
      auto ret = builder_.CreateBitCast(in_v, IntegerType::get(context_, bits));
      current_val_ = builder_.CreateZExtOrBitCast(ret, builder_.getInt64Ty());
    } break;
    }
  } break;
  case CATE_INT: {
    ...
    case CATE_OTHER: {
      check_cate();
      current_val_ = builder_.CreateZExtOrBitCast(in_v, builder_.getInt64Ty());
    } break;
    }
  } break;
  case CATE_UINT: {
    ...
    case CATE_OTHER: {
      if (v->dtype_.is_pointer()) {
        ...
        check_cate();
        current_val_ =
            builder_.CreateZExtOrBitCast(in_v, builder_.getInt64Ty());
      }
    } break;
    }
  } break;
  case CATE_OTHER:
    if (v->in_->dtype_ == datatypes::generic) {
      auto bits = module_->getDataLayout().getTypeAllocSizeInBits(outtype);
      auto ret =
          builder_.CreateTruncOrBitCast(in_v, IntegerType::get(context_, bits));
      switch (cate_out) {
      case CATE_OTHER:
        COMPILE_ASSERT(v->dtype_.is_pointer(), "Unexpected out type " << v);
      ...
      COMPILE_ASSERT(v->in_->dtype_.is_pointer(), "Unexpected in type " << v);
      if (v->dtype_.is_pointer()) {
        // pointer to pointer
        current_val_ = builder_.CreatePointerCast(in_v, outtype);
      } else {
        // pointer to generic val or uint64
        if (v->dtype_ != datatypes::index) {
          check_cate();
        }
        current_val_ = builder_.CreatePtrToInt(in_v, builder_.getInt64Ty());
      }
    }
    break;
  }
...
void codegen_llvm_vis_t::view(indexing_c v) {
  ...
  auto ptr =
      builder_.CreateGEP(element_type, base, generate_expr(v->idx_.front()));
  ...
  if (is_lvalue_mode) {
    ...
    bool is_volatile =
        (v->ptr_->attr_ && v->ptr_->attr_->get_or_else("volatile", false));
    if (v->mask_.defined()) {
      Value *mask;
      ...
      if (v->dtype_.lanes_ != bit_len) {
        COMPILE_ASSERT(v->dtype_.lanes_ == 4,
                       "Currently only 8bit -> 4bit is supported, but get "
                           << v->dtype_ << " lanes = " << v->dtype_.lanes_);
      ...
      current_val_ =
          set_alias(builder_.CreateMaskedLoad(get_type(v->dtype_), ptr,
                                              SC_LLVM_ALIGN(1), mask, zero),
                    v->ptr_);
#elif SC_LLVM_BACKEND >= 11
      current_val_ = set_alias(
          builder_.CreateMaskedLoad(ptr, SC_LLVM_ALIGN(1), mask, zero),
          v->ptr_);
#else
      current_val_ =
          set_alias(builder_.CreateMaskedLoad(ptr, 1, mask, zero), v->ptr_);
#endif
    } else {
      if (v->dtype_.lanes_ > 1) {
        current_val_ =
            set_alias(builder_.CreateAlignedLoad(get_type(v->dtype_), ptr,
                                                 SC_LLVM_ALIGN(1), is_volatile),
                      v->ptr_);
      } else {
        current_val_ = set_alias(
            builder_.CreateLoad(get_type(v->dtype_), ptr, is_volatile),
            v->ptr_);
      }
    }
  }
...
void codegen_llvm_vis_t::view(func_addr_c v) {
  current_val_ = builder_.CreatePointerCast(get_or_create_func(v->func_),
