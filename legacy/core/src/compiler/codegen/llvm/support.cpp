...
static void print_helper(Value *v) { v->print(llvm::errs()); }
...
#define WHEN_LLVM_VER_GE(V, VALUE)                                             \
  if (SC_LLVM_BACKEND >= (V)) {                                                \
    return VALUE;                                                              \
  }
#define WHEN_LLVM_VER_RANGE(V1, V2, VALUE)                                     \
  if (SC_LLVM_BACKEND >= (V1) && SC_LLVM_BACKEND <= (V2)) {                    \
    return VALUE;                                                              \
  }
...
get_llvm_target_machine(LLVM_CodeGenOptLevel optlevel) {
  ...
  auto host_cpu = inferred_cpu_name ? inferred_cpu_name : sys::getHostCPUName();
  ...
  auto tm = (TargetMachine *)LLVMCreateTargetMachine(
      (LLVMTargetRef)target, target_triple.c_str(), host_cpu.data(),
      features.c_str(), LLVMCodeGenLevelDefault, LLVMRelocStatic,
      LLVMCodeModelJITDefault);
#else
  auto tm = target->createTargetMachine(target_triple, host_cpu, features, opt,
                                        reloc_model, the_none, optlevel, true);
#endif
...
codegen_llvm_vis_t::codegen_llvm_vis_t(const context_ptr &ctx,
    ...
    : ctx_(ctx), context_(context), builder_(context_),
  ...
  if (ctx->flags_.debug_info_) {
    dbg_cu_ = dbuilder_->createCompileUnit(
        dwarf::DW_LANG_C, dbuilder_->createFile(source_file_name, source_dir),
...
DISubroutineType *codegen_llvm_vis_t::create_func_dtype(const func_c &v) {
  ...
  return dbuilder_->createSubroutineType(dbuilder_->getOrCreateTypeArray(tys));
}

Function *codegen_llvm_vis_t::get_or_create_func(const func_c &v) {
  auto itr = name_to_func_.find(v->name_);
  if (itr != name_to_func_.end()) {
    return itr->second;
  }
  auto FT = create_func_type(v);
  bool is_private =
      v->attr_ && v->attr_->get_or_else(function_attrs::private_, false);
  Function *F = Function::Create(
      FT, is_private ? Function::InternalLinkage : Function::ExternalLinkage,
      v->name_, module_.get());
...
std::pair<Type *, DIType *>
codegen_llvm_vis_t::do_get_type(sc_data_type_t dtype) {
  Type *ty = nullptr;
  DIType *dty = nullptr;
  if (dtype.is_etype_pointer() && dtype.type_code_ != sc_data_etype::POINTER) {
  ...
  case sc_data_etype::BF16:
    ...
    dty = dbuilder_->createBasicType("bf16", 16, dwarf::DW_ATE_unsigned);
    break;
  ...
  case sc_data_etype::POINTER:
    ...
    dty = dbuilder_->createBasicType("pointer", 64, dwarf::DW_ATE_address);
    break;
  ...
  if (dtype.lanes_ > 1) {
    ...
    llvm::DINodeArray subscriptarray = dbuilder_->getOrCreateArray(subscript);
    dty = dbuilder_->createVectorType(utils::get_sizeof_type(dtype) * 8, 8, dty,
                                      subscriptarray);
  }
  return {ty, dty};
}
...
std::pair<Type *, DIType *>
codegen_llvm_vis_t::get_type_both(sc_data_type_t dtype) {
...
Value *codegen_llvm_vis_t::define_var(const expr_c &e, Value *initv) {
  auto ptr = builder_.CreateAlloca(get_type(e->dtype_), nullptr,
                                   e.checked_as<var>()->name_);
  if (initv)
    builder_.CreateStore(initv, ptr);
  var_ptr_in_func_.insert(std::make_pair(e, ptr));
  return ptr;
}

void codegen_llvm_vis_t::set_dbg_info_for_func_arg(
    llvm::Value *v, DISubprogram *SP, DIFile *dunit, sc_data_type_t type,
...
void codegen_llvm_vis_t::emit_location(const node_base *p) {
  ...
  if (!p) {
    return builder_.SetCurrentDebugLocation(DebugLoc());
  }
  if (p->attr_) {
    if (auto loc = p->attr_->get_or_null<source_pos>("source_pos")) {
      ...
      builder_.SetCurrentDebugLocation(
          DILocation::get(Scope->getContext(), loc->line_, loc->pos_, Scope));
    }
  }
}
...
void codegen_llvm_vis_t::prepare_alias_metadata(
  ...
  for (auto &clique : cliques_sorted) {
    MDNode *scope = MDB.createAnonymousAliasScope(alias_domain,
                                                  std::to_string(clique->id_));
...
func_c codegen_llvm_vis_t::dispatch(func_c v) {
  ...
  if (ctx_->flags_.debug_info_) {
    ...
    ScopeLine = LineNo;
    dunit =
        dbuilder_->createFile(dbg_cu_->getFilename(), dbg_cu_->getDirectory());

    SP = dbuilder_->createFunction(
        dunit, v->name_, StringRef(), dunit, LineNo, create_func_dtype(v),
  ...
  if (has_alias) {
    ...
    for (size_t i = 0; i < v->params_.size(); i++) {
      if (v->params_[i].isa<tensor>()) {
        auto ainfo = alias_info::get_or_create_alias_info(*v->params_[i]);
  ...
  bool is_low_level =
      v->attr_ && v->attr_->get_or_else(function_attrs::low_level, false);
    ...
    if (p.isa<var>()) {
      ...
      if (is_low_level) {
        ...
        case 0:
          ...
          set_dbg_info_for_func_arg(arg, SP, dunit, p->dtype_, varnode->name_,
                                    i + 1, LineNo, true);
          break;
        case 1:
      ...
      var_ptr_in_func_.insert(std::make_pair(p, arg));
      set_dbg_info_for_func_arg(arg, SP, dunit, tnode->dtype_, tnode->name_,
                                i + 1, LineNo, true);
    }
  }
  dispatch(v->body_);
  ...
  if (ctx_->flags_.debug_info_) {
    dbg_scopes_.pop_back();
  }
  return v;
}

Instruction *codegen_llvm_vis_t::set_alias(Instruction *inst,
                                           const expr_c &tsr) {
  if (tsr.isa<indexing>()) {
...
void codegen_llvm_vis_t::set_dbg_info_for_local_var(const source_pos *pos,
  ...
  // Create a debug descriptor for the variable.
  DILocalVariable *D = dbuilder_->createAutoVariable(
      dbg_scopes_.back(), name, dbg_cu_->getFile(), pos->line_, dbgtype, true);

  dbuilder_->insertDeclare(llvm_value, D, dbuilder_->createExpression(),
                           DILocation::get(dbg_scopes_.back()->getContext(),
                                           pos->line_, pos->pos_,
                                           dbg_scopes_.back()),
                           builder_.GetInsertBlock());
}
...
void codegen_llvm_vis_t::set_dbg_info_for_local_var(const define_node_t *v,
  ...
  if (pos) {
    set_dbg_info_for_local_var(pos, v->var_->dtype_, name, llvm_value,
                               need_ref);
  }
}
