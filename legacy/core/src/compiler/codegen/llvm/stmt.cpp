...
void codegen_llvm_vis_t::generate_codeblock(const stmt_c &v,
                                            BasicBlock *current,
                                            BasicBlock *cont) {
...
void codegen_llvm_vis_t::view(assign_c v) {
  ...
  if (v->var_->dtype_.lanes_ > 1) {
    ptr = builder_.CreatePointerCast(ptr,
                                     get_type(v->var_->dtype_)->getPointerTo());
  }
  ...
  if (v->value_->dtype_.lanes_ > 1 && v->var_.isa<indexing>()) {
    ...
    if (v->var_.static_as<indexing>()->mask_.defined()) {
      ...
      set_alias(builder_.CreateMaskedStore(val, ptr, SC_LLVM_ALIGN(1), mask),
                v->var_);
#else
...
void codegen_llvm_vis_t::view(if_else_c v) {
  ...
  if (fb != cb) {
    generate_codeblock(v->else_case_, fb, cb);
  }
...
void codegen_llvm_vis_t::view(define_c v) {
  ...
  if (v->var_.isa<var>()) {
    ...
    if (thevar->attr_ &&
      ...
      if (auto absptr = offset.get_or_null<void *>()) {
        ptr = builder_.CreateIntToPtr(
            ...
            get_type(thevar->dtype_)->getPointerTo(), thevar->name_);
      } else {
        ...
        ptr = builder_.CreatePointerCast(
            ptr, get_type(thevar->dtype_)->getPointerTo(), thevar->name_);
      }
    ...
    if (need_align) {
      ptr->setAlignment(SC_LLVM_ALIGN(64));
    }
...
void codegen_llvm_vis_t::view(for_loop_c v) {
  ...
  if (ctx_->flags_.debug_info_) {
    ...
    if (pos) {
      set_dbg_info_for_local_var(
          pos, v->var_->dtype_, v->var_.checked_as<var>()->name_, itr_v, false);
    }
  }
  ...
  }
  {
    ...
    if (body->empty() || !llvm::isa<llvm::ReturnInst>(body->back())) {
      ...
      Value *itr_value = builder_.CreateLoad(get_type(v->var_->dtype_), itr_v);
