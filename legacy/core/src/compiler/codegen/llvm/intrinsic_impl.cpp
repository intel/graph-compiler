...
Value *codegen_llvm_vis_t::call_binary_llvm_normal(const intrin_call_c &v,
                                                   llvm_binary_func op) {
  assert(v->args_.size() == 2);
  auto inval1 = generate_expr(v->args_[0]);
  auto inval2 = generate_expr(v->args_[1]);
...
Value *codegen_llvm_vis_t::make_int_min_max(const intrin_call_c &v, bool ismin,
                                            type_category cate) {
  assert(v->args_.size() == 2);
...
Value *codegen_llvm_vis_t::do_lower_saturated_cast(const intrin_call_c &v) {
  ...
  auto pmovus_db_512 = [ths, out_llvm_ty](Value *v, bool issigned) {
    ...
    return ths->builder_.CreateIntrinsic(
        id, {},
        {v, UndefValue::get(out_llvm_ty), ths->builder_.getInt16(0xffff)});
  };
  ...
  COMPILE_ASSERT(false, "lowered saturated_cast cannot handle: "
                            << v << '(' << intype << "->" << v->dtype_ << ')');
}
