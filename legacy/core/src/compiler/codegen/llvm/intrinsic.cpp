...
void codegen_llvm_vis_t::view(intrin_call_c v) {
  ...
  case intrin_type::reinterpret: {
    ...
    if (outty->isPointerTy()) {
      ...
      bool is_dest_int = dst_cate == CATE_INT || dst_cate == CATE_UINT;
      COMPILE_ASSERT(is_dest_int, "Expecting pointer to int for reinterpret");
  ...
  case intrin_type::abs: {
    ...
    switch (cate) {
    case CATE_FLOAT:
      current_val_ = builder_.CreateUnaryIntrinsic(Intrinsic::fabs, inval);
      break;
    case CATE_INT: {
      auto znode = make_expr<constant_node>(0UL, v->args_[0]->dtype_);
  ...
  case intrin_type::sqrt: {
    current_val_ = call_unary_llvm_intrin(v, cate, Intrinsic::sqrt, true);
  } break;
  case intrin_type::rsqrt: {
    ...
    if (ctx_->machine_.cpu_flags_.fAVX512F && v->dtype_.lanes_ > 1) {
      ...
      case 4:
        current_val_ =
            builder_.CreateIntrinsic(Intrinsic::x86_avx512_rsqrt14_ps_128, {},
                                     {inval, inval, builder_.getInt8(0xff)});

        break;
      case 8:
        current_val_ =
            builder_.CreateIntrinsic(Intrinsic::x86_avx512_rsqrt14_ps_256, {},
                                     {inval, inval, builder_.getInt8(0xff)});
      ...
      case 16:
        ...
        current_val_ =
            builder_.CreateIntrinsic(Intrinsic::x86_avx512_rsqrt14_ps_512, {},
                                     {inval, inval, builder_.getInt16(0xffff)});
        break;
      default:
        COMPILE_ASSERT(false, "LLVM backend has not yet support "
                              "rsqrt with lanes = "
                                  << v->dtype_.lanes_);
        break;
      }
      ...
      current_val_ = call_unary_llvm_intrin(v, cate, Intrinsic::sqrt, true);
      Value *ones = ConstantFP::get(builder_.getFloatTy(), APFloat(1.0f));
  ...
  case intrin_type::int_and: {
    current_val_ = call_binary_llvm_normal(v, &llvm::IRBuilder<>::CreateAnd);
  } break;
  case intrin_type::int_or: {
    current_val_ = call_binary_llvm_normal(v, &llvm::IRBuilder<>::CreateOr);
  } break;
  case intrin_type::int_xor: {
    current_val_ = call_binary_llvm_normal(v, &llvm::IRBuilder<>::CreateXor);
  } break;
  case intrin_type::round: {
    current_val_ = call_unary_llvm_intrin(v, cate, Intrinsic::nearbyint, true);
  } break;
  case intrin_type::ceil: {
    current_val_ = call_unary_llvm_intrin(v, cate, Intrinsic::ceil, true);
  } break;
  case intrin_type::floor: {
    current_val_ = call_unary_llvm_intrin(v, cate, Intrinsic::floor, true);
  } break;
  case intrin_type::log: {
    current_val_ = call_unary_llvm_intrin(v, cate, Intrinsic::log, true);
  } break;
  case intrin_type::max: {
    if (cate == CATE_FLOAT) {
      current_val_ = call_binary_llvm_intrin(v, cate, Intrinsic::maxnum, true);
    } else {
  ...
  case intrin_type::min: {
    if (cate == CATE_FLOAT) {
      current_val_ = call_binary_llvm_intrin(v, cate, Intrinsic::minnum, true);
    } else {
  ...
  case intrin_type::shr: {
    ...
    if (cate == CATE_INT) {
      ...
      COMPILE_ASSERT(cate == CATE_UINT, "Bad type. Expecting int: " << v);
    ...
    if (lanes != 1) {
      if (in_lanes != 1) {
        while (in_lanes < lanes) {
          ...
          inval1 = builder_.CreateShuffleVector(inval1, inval1, array);
          in_lanes = in_lanes << 1;
        }
        current_val_ = inval1;
      } else {
  ...
  case intrin_type::fnmadd: {
    ...
    auto ret = builder_.CreateIntrinsic(Intrinsic::fma, {get_type(v->dtype_)},
...
#define LLVM_INTRINSIC_EXP LLVM_INTRINSIC_EXP_V2
    ...
    if ((v->type_ == intrin_type::reduce_add ||
      ...
      auto start_val =
          v->type_ == intrin_type::reduce_mul ? APFloat(1.0f) : APFloat(0.0f);
...
#else
          cur_intrinsic,
          {get_type(v->dtype_), get_type(v->dtype_), inval->getType()},
#endif
          {ConstantFP::get(
               get_type(v->dtype_),
               is_fp16 ? APFloat(APFloat::IEEEhalf(), APInt(16, fp16_start_val))
                       : start_val),
           inval});
  ...
  case intrin_type::round_and_cast: {
    ...
    COMPILE_ASSERT((v->dtype_.type_code_ == sc_data_etype::S32 ||
                    (v->dtype_.type_code_ == sc_data_etype::U32 &&
                     v->dtype_.lanes_ > 1)) &&
                       v->args_[0]->dtype_.type_code_ == sc_data_etype::F32,
    ...
    case 1: {
      current_val_ = builder_.CreateFPToSI(
          builder_.CreateUnaryIntrinsic(Intrinsic::roundeven, inval1),
          builder_.getInt32Ty());
    } break;
    case 4: {
      if (is_u32) {
        current_val_ = builder_.CreateIntrinsic(
            ...
            {inval1, UndefValue::get(get_type(v->dtype_)),
             /*mask*/ builder_.getInt8(0xff), builder_.getInt32(0x04)});
      } else {
        current_val_ =
            builder_.CreateIntrinsic(Intrinsic::x86_sse2_cvtps2dq, {}, inval1);
      }
    } break;
    case 8: {
      if (is_u32) {
    ...
    case 16: {
      ...
      current_val_ = builder_.CreateIntrinsic(
          is_u32 ? Intrinsic::x86_avx512_mask_cvtps2udq_512
  ...
  case intrin_type::permutex2var: {
    ...
    case sc_data_etype::F32:
      ...
      case 4:
        current_val_ =
            builder_.CreateIntrinsic(Intrinsic::x86_avx512_vpermi2var_ps_128,
                                     {}, {inval1, inval2, inval3});
        break;
      case 8:
        current_val_ =
            builder_.CreateIntrinsic(Intrinsic::x86_avx512_vpermi2var_ps_256,
                                     {}, {inval1, inval2, inval3});
        break;
      case 16:
        current_val_ =
            builder_.CreateIntrinsic(Intrinsic::x86_avx512_vpermi2var_ps_512,
                                     {}, {inval1, inval2, inval3});
        break;
      default:
        COMPILE_ASSERT(false, "Unimplement lanes for permute2var: "
                                  << v->args_[0]->dtype_.lanes_);
      }
      break;
    case sc_data_etype::U8:
      current_val_ =
          builder_.CreateIntrinsic(Intrinsic::x86_avx512_vpermi2var_qi_128, {},
                                   {inval1, inval2, inval3});
      break;
    default:
      COMPILE_ASSERT(false, "Unimplement datatype for permute2var");
    }
  } break;
  case intrin_type::permutexvar: {
    ...
    auto elem_bits =
        utils::get_sizeof_etype(v->args_[1]->dtype_.type_code_) * 8 * lanes;
    switch (elem_bits) {
    case 8: {
      current_val_ = builder_.CreateIntrinsic(
          Intrinsic::x86_avx512_permvar_qi_512, {}, {inval2, inval1});
    } break;
    case 16: {
      current_val_ = builder_.CreateIntrinsic(
          Intrinsic::x86_avx512_permvar_hi_512, {}, {inval2, inval1});
    } break;
    case 64: {
      ...
      if (v->args_[0].isa<constant_c>()) {
        ...
        bool is_avx512 = utils::get_sizeof_type(v->args_[1]->dtype_) * 8 == 512;
        auto target_type = is_avx512 ? get_type(sc_data_type_t::index(8))
        ...
        auto ret = builder_.CreateShuffleVector(tmp1, indices, "perm");
        current_val_ = builder_.CreateBitCast(ret, outtype);
      } else {
        COMPILE_ASSERT(false, "Currently we do not need "
                              "_mm512_permutexvar_epi64 (__m512i idx, "
                              "__m512i a)");
      }
    } break;
    default: {
      COMPILE_ASSERT(false, "Currently, we don't support "
                            "\"" << v->args_[1]->dtype_
                                 << "\"");
    } break;
    }
  ...
  case intrin_type::unpack_low: {
    ...
    case 128: {
      ...
      case 8: {
        ...
        lo_array.resize(16);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
      ...
      case 16: {
        ...
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array =
            std::vector<shuffle_idx_t>{4, 8 + 4, 5, 8 + 5, 6, 8 + 6, 7, 8 + 7};
        lo_array.resize(8);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
      ...
      case 64: {
        auto target_type = get_type(sc_data_type_t::index(2));
        tmp1 = builder_.CreateBitCast(inval1, target_type);
        tmp2 = builder_.CreateBitCast(inval2, target_type);
      ...
      default: {
        COMPILE_ASSERT(false, "Only support 8, 16, 32 and 64 bits");
      };
      };
    } break;
    case 256: {
      ...
      case 8: {
        ...
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array = std::vector<shuffle_idx_t>{
            8,  32 + 8,  9,  32 + 9,  10, 32 + 10, 11, 32 + 11,
            12, 32 + 12, 13, 32 + 13, 14, 32 + 14, 15, 32 + 15,
            24, 32 + 24, 25, 32 + 25, 26, 32 + 26, 27, 32 + 27,
            28, 32 + 28, 29, 32 + 29, 30, 32 + 30, 31, 32 + 31};
        lo_array.resize(32);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
                       [](shuffle_idx_t x) { return x - 8; });
      } break;
      case 16: {
        auto target_type = get_type(sc_data_type_t::u16(16));
        tmp1 = builder_.CreateBitCast(inval1, target_type);
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array = std::vector<shuffle_idx_t>{
      ...
      case 32: {
        ...
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array =
            std::vector<shuffle_idx_t>{2, 8 + 2, 3, 8 + 3, 6, 8 + 6, 7, 8 + 7};
        lo_array.resize(8);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
                       [](shuffle_idx_t x) { return x - 2; });
      } break;
      case 64: {
        auto target_type = get_type(sc_data_type_t::index(4));
        tmp1 = builder_.CreateBitCast(inval1, target_type);
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array = std::vector<shuffle_idx_t>{1, 4 + 1, 3, 4 + 3};
        lo_array.resize(4);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
                       [](shuffle_idx_t x) { return x - 1; });
      } break;
    ...
    case 512: {
      ...
      case 8: {
        ...
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array = std::vector<shuffle_idx_t>{
            8,  64 + 8,  9,  64 + 9,  10, 64 + 10, 11, 64 + 11, 12, 64 + 12,
            13, 64 + 13, 14, 64 + 14, 15, 64 + 15, 24, 64 + 24, 25, 64 + 25,
            26, 64 + 26, 27, 64 + 27, 28, 64 + 28, 29, 64 + 29, 30, 64 + 30,
            31, 64 + 31, 40, 64 + 40, 41, 64 + 41, 42, 64 + 42, 43, 64 + 43,
            44, 64 + 44, 45, 64 + 45, 46, 64 + 46, 47, 64 + 47, 56, 64 + 56,
            57, 64 + 57, 58, 64 + 58, 59, 64 + 59, 60, 64 + 60, 61, 64 + 61,
            62, 64 + 62, 63, 64 + 63};
        lo_array.resize(64);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
                       [](shuffle_idx_t x) { return x - 8; });
      ...
      case 16: {
        auto target_type = get_type(sc_data_type_t::u16(32));
        tmp1 = builder_.CreateBitCast(inval1, target_type);
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array = std::vector<shuffle_idx_t>{
            4,  32 + 4,  5,  32 + 5,  6,  32 + 6,  7,  32 + 7,
            12, 32 + 12, 13, 32 + 13, 14, 32 + 14, 15, 32 + 15,
            20, 32 + 20, 21, 32 + 21, 22, 32 + 22, 23, 32 + 23,
            28, 32 + 28, 29, 32 + 29, 30, 32 + 30, 31, 32 + 31};
        lo_array.resize(32);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
                       [](shuffle_idx_t x) { return x - 4; });
      ...
      case 32: {
        auto target_type = get_type(sc_data_type_t::u32(16));
        tmp1 = builder_.CreateBitCast(inval1, target_type);
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array = std::vector<shuffle_idx_t>{
            2,     18,     3,     19,     2 + 4,  18 + 4,  3 + 4,  19 + 4,
      ...
      case 64: {
        auto target_type = get_type(sc_data_type_t::index(8));
        tmp1 = builder_.CreateBitCast(inval1, target_type);
        tmp2 = builder_.CreateBitCast(inval2, target_type);
        hi_array = std::vector<shuffle_idx_t>{1,     9,     1 + 2, 9 + 2,
                                              1 + 4, 9 + 4, 1 + 6, 9 + 6};
        lo_array.resize(8);
        std::transform(hi_array.begin(), hi_array.end(), lo_array.begin(),
                       [](shuffle_idx_t x) { return x - 1; });
    ...
    ArrayRef<shuffle_idx_t> arr =
        v->type_ == intrin_type::unpack_high ? hi_array : lo_array;
  ...
  case intrin_type::shuffle: {
    ...
    case 32: {
      ...
      unsigned num_lanes =
          utils::get_sizeof_type(v->args_[0]->dtype_) * 8 / 128;
      ...
      current_val_ =
          builder_.CreateShuffleVector(inval1, inval2, indices, "shuf32");
    } break;
    case 128: {
      unsigned num_elts = v->args_[0]->dtype_.lanes_;
      unsigned num_lanes =
          utils::get_sizeof_type(v->args_[0]->dtype_) * 8 == 512 ? 4 : 2;
      unsigned num_lanes_elts = num_elts / num_lanes;
      ...
      current_val_ =
          builder_.CreateShuffleVector(inval1, inval2, indices, "shuf128");
    } break;
    default: {
      COMPILE_ASSERT(false, "Curerntly only support type_bits 32 and 128, "
                            "but get "
                                << type_bits);
    } break;
    };
  ...
  case intrin_type::permute: {
    ...
    for (unsigned l = 0; l != 2; ++l) {
      ...
      if (imm8 & (1 << ((l * 4) + 3)))
        outops[l] = llvm::ConstantAggregateZero::get(val0->getType());
    ...
    current_val_ = builder_.CreateShuffleVector(
        outops[0], outops[1], ArrayRef<shuffle_idx_t>(indices, numelts),
        "vperm");
  } break;
  case intrin_type::prefetch: {
    assert(v->args_.size() == 1);
    auto locality = v->intrin_attrs_->get<int>("locality");
    assert(locality <= 3 && locality >= 0 && "bad locality for prefetch");
    auto inval1 = generate_expr(v->args_[0]);
    current_val_ = builder_.CreateIntrinsic(
        Intrinsic::prefetch,
        ...
        {},
#endif
        {builder_.CreatePointerCast(inval1, builder_.getInt8PtrTy()),
  ...
  case intrin_type::gather: {
    ...
    auto inval1 = builder_.CreatePointerCast(generate_expr(v->args_[0]),
                                             builder_.getInt8PtrTy());
    auto inval2 = generate_expr(v->args_[1]);
    auto full_mask = generate_expr(make_expr<constant_node>(
        std::vector<union_val>(v->dtype_.lanes_, UINT64_C(0xffffffff)),
        v->dtype_));
    auto dummy_vec = generate_expr(make_expr<constant_node>(
        std::vector<union_val>(v->dtype_.lanes_, UINT64_C(0)), v->dtype_));
    switch (v->dtype_.lanes_) {
    case 1: {
      auto indexing = make_expr<indexing_node>(
          v->args_[0], std::vector<expr>{v->args_[1]}, expr());
    ...
    case 4:
      current_val_ = builder_.CreateIntrinsic(
          ...
          {dummy_vec, inval1, inval2, full_mask, builder_.getInt8(4)});
      break;
    case 8:
      current_val_ = builder_.CreateIntrinsic(
    ...
    case 16: {
      ...
      current_val_ = builder_.CreateIntrinsic(
          ...
          {dummy_vec, inval1, inval2, builder_.getInt16(0xffff),
           builder_.getInt32(4)});
      break;
    }
    default:
  ...
  case intrin_type::insert: {
    ...
    const unsigned elem_bits = utils::get_sizeof_type(v->args_[1]->dtype_) * 8;
    ...
    if (ctx_->machine_.cpu_flags_.fAVX512F && elem_bits >= 128) {
      ...
      for (unsigned i = 0; i != dst_num_elts; ++i) {
        indices[i] =
            (i >= src_num_elts) ? src_num_elts + (i % src_num_elts) : i;
      }
      ...
      current_val_ = builder_.CreateShuffleVector(val0, op1, indices, "insert");
    } else {
  ...
  case intrin_type::extract: {
    ...
    if (ctx_->machine_.cpu_flags_.fAVX512F && elem_bits >= 128) {
      ...
      for (unsigned i = 0; i != dst_num_elts; ++i)
        indices[i] = i + index;
      current_val_ = builder_.CreateShuffleVector(val0, indices, "extract");
    } else {
      assert(elem_bits <= 128);
