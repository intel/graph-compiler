...
void codegen_c_vis::print_type(sc_data_type_t dtype) {
  if (dtype.lanes_ == 1) {
    ...
    switch (dtype) {
    case sc_data_type_t::bf16(4):
      *os << "vec_u16x4";
      break;
    case sc_data_type_t::bf16(8):
      *os << "vec_u16x8";
      break;
    case sc_data_type_t::bf16(16):
      *os << "vec_u16x16";
      break;
    case sc_data_type_t::bf16(32):
      *os << "vec_u16x32";
      break;

    case sc_data_type_t::f16(4):
      *os << "vec_f16x4";
      break;
    case sc_data_type_t::f16(8):
      *os << "vec_f16x8";
      break;
    case sc_data_type_t::f16(16):
      *os << "vec_f16x16";
      break;
    case sc_data_type_t::f16(32):
      *os << "vec_f16x32";
      break;

    case sc_data_type_t::f32(4):
      *os << "vec_f32x4";
      break;
    case sc_data_type_t::f32(8):
      *os << "vec_f32x8";
      break;
    case sc_data_type_t::f32(16):
      *os << "vec_f32x16";
      break;

    case sc_data_type_t::s32(4):
      *os << "vec_s32x4";
      break;
    case sc_data_type_t::s32(8):
      *os << "vec_s32x8";
      break;
    case sc_data_type_t::s32(16):
      *os << "vec_s32x16";
      break;

    case sc_data_type_t::u32(4):
      *os << "vec_u32x4";
      break;
    case sc_data_type_t::u32(8):
      *os << "vec_u32x8";
      break;
    case sc_data_type_t::u32(16):
      *os << "vec_u32x16";
      break;

    case sc_data_type_t::index(2):
      *os << "vec_u64x2";
      break;
    case sc_data_type_t::index(4):
      *os << "vec_u64x4";
      break;
    case sc_data_type_t::index(8):
      *os << "vec_u64x8";
      break;

    case sc_data_type_t::u16(8):
      *os << "vec_u16x8";
      break;
    case sc_data_type_t::u16(16):
      *os << "vec_u16x16";
      break;
    case sc_data_type_t::u16(32):
      *os << "vec_u16x32";
      break;

    case sc_data_type_t::s8(8):
      *os << "vec_s8x8";
      break;
    case sc_data_type_t::s8(16):
      *os << "vec_s8x16";
      break;
    case sc_data_type_t::s8(32):
      *os << "vec_s8x32";
      break;
    case sc_data_type_t::s8(64):
      *os << "vec_s8x64";
      break;

    case sc_data_type_t::u8(8):
      *os << "vec_u8x8";
      break;
    case sc_data_type_t::u8(16):
      *os << "vec_u8x16";
      break;
    case sc_data_type_t::u8(32):
      *os << "vec_u8x32";
      break;
    case sc_data_type_t::u8(64):
      *os << "vec_u8x64";
      break;

    case sc_data_type_t::boolean(4):
      *os << "uint8_t";
      break;
    case sc_data_type_t::boolean(8):
      *os << "uint8_t";
      break;
    case sc_data_type_t::boolean(16):
      *os << "uint16_t";
      break;
    case sc_data_type_t::boolean(32):
      *os << "uint32_t";
      break;
    case sc_data_type_t::boolean(64):
      *os << "uint64_t";
      break;

    default:
      COMPILE_ASSERT(0, "Cannot generate vector type for C++: " << dtype);
      break;
    }
  }
}
...
static const std::string &get_func_name(const func_c &v) {
  if (v->attr_) {
    if (auto pname =
            v->attr_->get_or_null<std::string>("temp.replace_func_name")) {
      return *pname;
    }
  }
  return v->name_;
...
func_c codegen_c_vis::dispatch(func_c v) {
  if (utils::string_startswith(v->name_, "_should_inline_")) {
    return v;
  }
  if (prototype_only) {
    print_func_comments(v, *os);
  }
...
void codegen_c_vis::view(var_c v) { *os << v->name_; }
...
void codegen_c_vis::trinary_func_codegen_c(const std::vector<expr> &args,
                                           const char *funcname) {
...
void codegen_c_vis::binary_func_codegen_c(const std::vector<expr> &args,
                                          const char *funcname) {
...
void codegen_c_vis::unary_func_codegen_c(const expr &arg,
                                         const char *funcname) {
  auto &os = *this->os;
  os << funcname << '(';
  dispatch(arg);
...
static const char *prefetch_names[] = {"_MM_HINT_T0", "_MM_HINT_T1",
                                       "_MM_HINT_T2", "_MM_HINT_NTA"};

void codegen_c_vis::view(intrin_call_c v) {
  switch (v->type_) {
  case intrin_type::min:
    binary_func_codegen_c(v->args_, "sc_min");
    break;
  case intrin_type::max:
    binary_func_codegen_c(v->args_, "sc_max");
    break;
  ...
  case intrin_type::prefetch: {
    *os << "_mm_prefetch(";
    ...
    COMPILE_ASSERT(locality <= 3 && locality >= 0, "bad locality for prefetch");
...
void codegen_c_vis::view(add_c v) { print_binary(v, " + "); }
void codegen_c_vis::view(sub_c v) { print_binary(v, " - "); }
void codegen_c_vis::view(mul_c v) { print_binary(v, " * "); }
void codegen_c_vis::view(div_c v) { print_binary(v, " / "); }
void codegen_c_vis::view(mod_c v) { print_binary(v, " % "); }

void codegen_c_vis::view(cmp_eq_c v) { print_binary(v, " == "); }
void codegen_c_vis::view(cmp_lt_c v) { print_binary(v, " < "); }
void codegen_c_vis::view(cmp_le_c v) { print_binary(v, " <= "); }
void codegen_c_vis::view(cmp_gt_c v) { print_binary(v, " > "); }
void codegen_c_vis::view(cmp_ge_c v) { print_binary(v, " >= "); }
void codegen_c_vis::view(cmp_ne_c v) { print_binary(v, " != "); }

void codegen_c_vis::view(logic_and_c v) { print_binary(v, " && "); }
void codegen_c_vis::view(logic_or_c v) { print_binary(v, " || "); }
...
void codegen_c_vis::view(call_c v) {
  ...
  if (!v->para_attr_.empty()) {
    ...
    *os << v->args_[0] << ')';
    ...
    if (the_func) {
      if (!is_offline_ && default_external_symbol_resolve(the_func->name_)) {
        // if set gcc attributes, call func wrapper instead
        if (!the_func->attr_ ||
            !the_func->attr_->get_or_else(function_attrs::pure, false)) {
...
void codegen_c_vis::view(tensor_c v) { *os << v->name_; }
...
void codegen_c_vis::view(define_c v) {
  ...
  if (v->var_.isa<var>()) {
    ...
    if (v->var_->attr_ &&
      ...
      *os << '&' << ' ' << thevar->name_;
      auto &offset = v->var_->attr_->get_any(attr_keys::module_global_offset);
      *os << " = *(";
      print_type(thevar->dtype_);
...
static bool is_func_static(const func_c &f, bool is_offline) {
  ...
  if (is_offline) {
    ...
    if (is_main_func_wrapper(f)) {
      return false;
    }
    if (f->name_ == "__sc_init__" || f->name_ == "memset") {
      return false;
    }
...
void write_cpp_generic_wrapper(std::ostream *source_, const func_c &f,
                               bool is_parallel) {
...
static func_c do_generate_c(func_c f, std::ostream &source,
                            const std::vector<define> &globals,
                            bool gen_wrapper, bool is_static, bool is_offline) {
...
void c_generator_pass_t::operator()(func_t f) {
  f = pre_passes_(ir_module_t::from_entry_func(context_, f))->get_entry_func();
...
c_generator_pass_t::c_generator_pass_t(std::ostream &source,
  ...
  if (optional_out_) {
    prepare_include(optional_out_->offline_source_);
  }
}

const_ir_module_ptr
preprocess_module_and_make_decl(const const_ir_module_ptr &mod,
                                module_pass_t &pre_passes, std::ostream &source,
...
static void generate_dumped_source(const const_ir_module_ptr &mod,
  ...
  for (auto &f : mod->get_contents()) {
    if (f->name_ == "__sc_init__") {
      ...
      f->attr()["comments"] = std::vector<std::string>{
...
extern uint8_t )"
  ...
  for (auto &f : mod->get_contents()) {
    if (f->name_ == skip_func_name) {
      continue;
    }
    bool is_static_func = is_func_static(f, true);
    if (!is_static_func) {
      write_cpp_prototype(&header_src, f, true);
    }
...
const_ir_module_ptr c_generator_pass_t::operator()(const_ir_module_ptr mod) {
  ...
  mod =
      preprocess_module_and_make_decl(mod, pre_passes_, source_, optional_out_);
...
c_generator_pass_t
create_c_generator(std::ostream &os, const context_ptr &ctx, bool gen_wrapper,
                   c_generator_optional_out_t *optional_out) {
