...
static void check_within(int &val, int lo, int hi, int defaultv,
                         const char *prompt) {
  if (val < lo || val > hi) {
...
static void reset_cpu_flags_by_dnnl_envs(runtime::target_machine_t &tm) {
  ...
  if (!max_isa.empty()) {
    ...
    do {
      // amx fp16
      if (max_isa == "AVX512_CORE_AMX_FP16") {
        break;
      }
      tm.cpu_flags_.fAVX512AMXFP16 = false;
      // amx
      if (max_isa == "AVX512_CORE_AMX") {
        break;
      }
      ...
      // avx512 fp16
      if (max_isa == "AVX512_CORE_FP16") {
        break;
      }
      tm.cpu_flags_.fAVX512FP16 = false;
      // avx512 bf16
      if (max_isa == "AVX512_CORE_BF16") {
        break;
      }
      tm.cpu_flags_.fAVX512BF16 = false;
      // avx512 vnni
      if (max_isa == "AVX512_CORE_VNNI") {
        break;
      }
      tm.cpu_flags_.fAVX512VNNI = false;
      // avx512 core
      if (max_isa == "AVX512_CORE") {
        break;
      }
      ...
      // avx vnni 2
      if (max_isa == "AVX2_VNNI_2") {
        break;
      }
      // avx vnni
      if (max_isa == "AVX2_VNNI") {
        break;
      }
      // avx2
      if (max_isa == "AVX2") {
...
template <typename T> static void parse_value(const char *name, T &v) {
...
context_ptr make_context_from_env() {
  ...
  check_within(opt_level, 0, 3, 3, "Bad optimization level in SC_OPT_LEVEL: ");
...
bool context_t::use_amx() const { return machine_.use_amx(); }

context_t::context_t(const scflags_t &flags,
                     runtime::target_machine_t &&machine,
                     runtime::engine_t *engine)
    : engine_(engine ? engine : runtime::get_default_stream()->engine_),
      flags_(flags), machine_(std::move(machine)) {}
