...
ostream &operator<<(ostream &os, const stmt_c &s) { return os << s.get(); }
...
#undef HANDLE_CASE
  default:
    os << "(unrecognized for_type value)";
    break;
  }
  return os;
}
...
#undef HANDLE_CASE
  default:
    os << "(unrecognized sc_stmt_type value)";
    break;
  }
  return os;
}
...
bool stmts_node_t::equals(stmt_c v, ir_comparer &ctx) const {
  ...
  for (size_t i = 0; i < seq_.size(); i++) {
    if (!seq_.at(i)->equals(other->seq_.at(i), ctx)) {
      return false;
    }
  }
  return true;
...
stmt if_else_node_t::remake() const {
  return copy_attr(
      *this, make_stmt<if_else_node_t>(condition_, then_case_, else_case_));
}
...
stmt for_loop_node_t::remake() const {
  return copy_attr(*this, make_stmt<for_loop_node_t>(
                              var_, iter_begin_, iter_end_, step_, body_,
                              incremental_, kind_, num_threads_));
}
...
bool for_loop_node_t::equals(stmt_c v, ir_comparer &ctx) const {
  ...
  return ctx.set_result(node_ptr_from_this(), v,
         ...
         step_->equals(other->step_, ctx) && body_->equals(other->body_, ctx);
}
