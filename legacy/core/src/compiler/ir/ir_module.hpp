...
public:
  ...
  struct attr_key_t {
    ...
    static constexpr const char *MODULE_DATA_BUFFERS = "MODULE_DATA_BUFFERS";
    ...
    static constexpr const char *MANAGED_THREAD_POOL = "MANAGED_THREAD_POOL";
  ...
  context_ptr ctx_;
  any_map_t attr_;
  ir_module_t(context_ptr ctx) : ctx_(std::move(ctx)) { entry_func_idx_ = -1; }
  ...
  void set_entry_func_idx(int entry_func_idx) {
    COMPILE_ASSERT(
        entry_func_idx == -1 ||
            (entry_func_idx >= 0 && size_t(entry_func_idx) < contents_.size()),
  ...
  const dispatch_table_map_t &get_op_table_map() const { return op_table_map_; }
  ...
  copy_and_remove_funcs(const std::vector<bool> &mask) const;
   ...
   * */
  static std::shared_ptr<ir_module_t>
  from_entry_func(context_ptr ctx, const std::vector<func_t> &f);

  /**
   * @see from_entry_func overloaded function
   * */
  static std::shared_ptr<ir_module_t> from_entry_func(context_ptr ctx,
                                                      func_t f);
