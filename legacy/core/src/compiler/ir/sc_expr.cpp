...
any_map_t &node_base::attr() {
  if (!attr_) {
    attr_ = utils::make_unique<any_map_t>();
  }
  return *attr_;
}
...
#undef HANDLE_CASE
  default:
    os << "(unrecognized sc_expr_type value)";
    break;
  }
  return os;
}
...
#undef HANDLE_CASE
  default:
    os << "(unrecognized intrin_type value)";
    break;
  }
  return os;
}
...
ostream &operator<<(ostream &os, x86_intrin_type::x86_intrin_type_t val) {
  switch (val) {
#define HANDLE_CASE(X)                                                         \
  case x86_intrin_type::X:                                                     \
    os << "x86_intrin_type::" #X;                                              \
    break;
    HANDLE_CASE(avx_broadcast_idx)
    HANDLE_CASE(NUM_INTRINSICS)
#undef HANDLE_CASE
  default:
    os << "(unrecognized x86_intrin_type value)";
    break;
  }
  return os;
}
...
std::ostream &operator<<(std::ostream &os, linkage val) {
  switch (val) {
  case linkage::public_global:
    os << "linkage::public_global";
    break;
  case linkage::private_global:
    os << "linkage::private_global";
    break;
  case linkage::static_local:
    os << "linkage::static_local";
    break;
  case linkage::local:
    os << "linkage::local";
    break;
...
expr::lvalue_proxy_t::operator expr() const { return get(); }

expr::lvalue_proxy_t::operator expr_c() const { return get(); }
...
expr::lvalue_proxy_t
expr::lvalue_proxy_t::operator[](const std::vector<expr> &index) const {
...
expr::lvalue_proxy_t expr::lvalue_proxy_t::operator[](expr index) const {
  return expr::lvalue_proxy_t(builder::make_indexing(*this, std::move(index)),
                              true);
}

expr::lvalue_proxy_t
expr::lvalue_proxy_t::operator[](const span_t &index) const {
...
expr::lvalue_proxy_t expr::operator[](const span_t &index) const {
  ...
  return expr::lvalue_proxy_t(builder::make_indexing(*this, idx, index.length_,
                                                     index.mask_, index.rows_),
                              true);
}
...
ostream &operator<<(ostream &os, const expr_c &e) { return os << e.get(); }
...
#define ASCAST_OR_RETURN(v, other)                                             \
  using self = node_ptr<typename std::remove_reference<decltype(*this)>::type, \
                        expr_base>;                                            \
  if (!(v).isa<self>()) {                                                      \
...
bool constant_node::equals(expr_c v, ir_comparer &ctx) const {
  ...
  sc_data_etype etype =
      dtype_.is_etype_pointer() ? sc_data_etype::POINTER : dtype_.type_code_;
  ...
  auto is_values_equals = [&is_float_category](
                              const std::vector<union_val> &v1,
                              const std::vector<union_val> &v2) {
    assert(v1.size() == 1);
    return std::all_of(
        v2.begin(), v2.end(), [&v1, &is_float_category](const union_val &x) {
          return is_float_category ? x.f32 == v1[0].f32 : x.u64 == v1[0].u64;
        });
  };
...
bool var_node::equals(expr_c v, ir_comparer &ctx) const {
  if (ctx.cmp_var_ref_) {
    if (ctx.get_expr_mapping(node_ptr_from_this(), v)) {
      return true;
    }
    RETURN(v.get() == this);
  }
...
bool binary_node::equals(expr_c v, ir_comparer &ctx) const {
  ...
  if (!ret && ctx.cmp_commutative_ &&
      (node_type_ == sc_expr_type::add || node_type_ == sc_expr_type::mul)) {
...
expr indexing_node::remake() const {
  return copy_attr(*this, builder::make_indexing(ptr_, idx_, dtype_.lanes_,
                                                 mask_, dtype_.rows_));
}
...
call_node::call_node(const std::shared_ptr<node_base> &func,
    ...
    : expr_base(get_func_type(func.get()), sc_expr_type::call), func_(func),
      args_(args), para_attr_(std::move(para_attr)) {}

call_node::call_node(const expr &func, const std::vector<expr> &args)
    : expr_base(func->attr().get<func_t>("prototype")->ret_type_,
                sc_expr_type::call),
      func_(func.impl), args_(args) {}

call_node::call_node(const func_t &func, const std::vector<expr> &args,
                     std::vector<parallel_attr_t> &&para_attr)
    : expr_base(func->ret_type_, sc_expr_type::call), func_(func), args_(args),
...
bool call_node::equals(expr_c v, ir_comparer &ctx) const {
  ...
  if (typeid(ths_func) != typeid(other_func)) {
    return false;
  }
  if (auto e = dynamic_cast<expr_base *>(func_.get())) {
    // if is expr, compare the callee by equals()
    if (!e->equals(expr_c(std::static_pointer_cast<expr_base>(other->func_)),
                   ctx)) {
      return false;
    }
  } else {
    if (ctx.cmp_callee_) {
      ...
      assert(f);
      if (!f->equals(std::static_pointer_cast<func_base>(other->func_), ctx))
        return false;
    } else {
      if (!ctx.set_result(shared, v, func_ == other->func_))
        return false;
    }
  }
  if (para_attr_.size() != other->para_attr_.size()) {
    RETURN(false);
  }
...
tensor_node::tensor_node(sc_data_type_t dtype, const std::string &name,
    ...
    : expr_base(sc_data_type_t::pointerof(dtype.type_code_),
                sc_expr_type::tensor),
      elem_dtype_(dtype), dims_(dims), name_(name),
      address_space_(address_space), init_value_(init_value),
      strides_(strides) {
  if (strides_.empty()) {
    strides_ = dims_to_dense_stride(dims_);
  }
}
...
void tensor_node::to_string_full(ir_printer_t &printer) {
  ...
  if (address_space_ != address_space::automatic) {
    ...
    case address_space::device:
      os << " device";
      break;
    case address_space::host:
      os << " host";
      break;
    default:
      assert(0);
...
std::shared_ptr<static_data_t>
tensor_node::make_tensor_initializer(union_val val) {
  union_val theval;
  theval.u64 = 0;
  theval = val;
...
expr tensorptr_node::remake() const {
  return copy_attr(*this, make_expr<tensorptr_node>(base_, shape_, is_slice_));
}

bool tensorptr_node::equals(expr_c v, ir_comparer &ctx) const {
  ASCAST_OR_RETURN(v, other);
  if (other->is_slice_ != is_slice_) {
    RETURN(false);
  }
  if (!ctx.expr_arr_equals(shape_, other->shape_)) {
    RETURN(false);
  }
...
intrin_call_node::intrin_call_node(intrin_type intrin,
    ...
    : expr_base(sc_expr_type::intrin_call), type_(intrin), args_(args),
...
expr intrin_call_node::remake() const {
  return copy_attr(*this,
                   make_expr<intrin_call_node>(type_, args_, *intrin_attrs_));
}
  ...
  if (type_ == intrin_type::brgemm || type_ == intrin_type::list_brgemm) {
    ...
    auto &other_extra_args =
        other->intrin_attrs_->get<brgemm_args::extra_args_t>(
            intrin_attr::brgemm_extras);
    if (extra_args != other_extra_args) {
      RETURN(false);
    }
  }
  RETURN(ctx.expr_arr_equals(args_, other->args_));
...
low_level_intrin_node::low_level_intrin_node(low_level_intrin_kind kind,
                                             ...
                                             const std::vector<expr> &args,
                                             const any_map_t &attrs)
    : expr_base(sc_expr_type::low_level_intrin), kind_(kind), type_(type),
      args_(args), intrin_attrs_(utils::make_unique<any_map_t>(attrs)) {}

expr low_level_intrin_node::remake() const {
  auto ret =
      make_expr<low_level_intrin_node>(kind_, type_, args_, *intrin_attrs_);
  ret->dtype_ = dtype_;
  return copy_attr(*this, std::move(ret));
}
...
const std::string &get_node_name(const expr &e) {
  ...
  COMPILE_ASSERT(false, "Not an expr_base subclass that has a 'name_' member.");
}
