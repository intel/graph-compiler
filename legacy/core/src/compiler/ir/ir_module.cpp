...
public:
  ...
  expr visit_impl(func_addr c) override {
    auto ret = ir_inplace_visitor_t::visit_impl(std::move(c)).as<func_addr>();
    func_t callee = std::dynamic_pointer_cast<func_base>(ret->func_);
    if (!callee) {
      return ret;
    }
  ...
  func_t dispatch_impl(func_t f) override {
    if (f->decl_) {
      if (f->decl_->name_ != f->name_) {
        f->decl_->name_ = f->name_;
      }
    }
...
std::shared_ptr<ir_module_t> ir_module_t::from_entry_func(context_ptr ctx,
                                                          func_t funct) {
  auto ret =
      from_entry_func(std::move(ctx), std::vector<func_t>{std::move(funct)});
...
std::shared_ptr<ir_module_t>
ir_module_t::from_entry_func(context_ptr ctx,
                             const std::vector<func_t> &funcs) {
...
func_t ir_module_t::get_func(const std::string &name) const {
  ...
  if (itr != symbols_.end()) {
    return contents_.at(itr->second);
  }
  return func_t();
}
...
ir_module_ptr
ir_module_t::copy_and_remove_funcs(const std::vector<bool> &mask) const {
  ...
  for (size_t old_id = 0; old_id < ret->contents_.size(); old_id++) {
    if (mask.at(old_id)) {
      ...
      if ((int)old_id == entry_func) {
        ret->entry_func_idx_ = new_id;
      }
    }
  }
...
var ir_module_t::make_global_var(sc_data_type_t dtype, const std::string &name,
  ...
  auto def =
      builder::make_var_tensor_def_unattached(ret, linkage, std::move(init))
          .static_as<define>();
...
tensor ir_module_t::make_global_tensor(sc_data_type_t dtype,
  ...
  auto def =
      builder::make_var_tensor_def_unattached(ret, linkage).static_as<define>();
  add_global_var(std::move(def));
  return ret;
}
...
tensor ir_module_t::make_global_stensor(sc_data_type_t dtype,
                                        ...
                                        linkage linkage, stmt *out_def_node) {
  tensor ret =
      builder::make_stensor(name, dims, strides, dtype).static_as<tensor>();
  auto def =
      builder::make_var_tensor_def_unattached(ret, linkage).static_as<define>();
  if (out_def_node) {
    *out_def_node = def;
