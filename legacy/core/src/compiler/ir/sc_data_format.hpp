...
public:
  ...
  constexpr sc_data_format_kind_t(Args... args)
      : storage_(make_storage<0>(set_ith_int(0xffffffffffffffff, MAX_DIMS, 0),
                                 args...)) {
  ...
  static sc_data_format_kind_t
  get_2dblocking_by_dims(size_t ndims, bool is_weight = false,
                         bool is_vnni_format = false);
};

namespace format_kinds {
#define SC_DEF_FMT(name, ...) constexpr sc_data_format_kind_t name{__VA_ARGS__};
...
struct SC_API sc_data_format_t {
  ...
  constexpr sc_data_format_t(sc_data_format_kind_t format_code,
  ...
  static sc_dims get_blocking_shapes(const sc_dims &plain_shapes,
  ...
  static sc_dims get_padded_plain_shapes(const sc_dims &real_shapes,
                                         const sc_data_format_t &format);
...
struct sc_data_format_cmper_t {
  bool operator()(const sc_data_format_t &key0,
                  const sc_data_format_t &key1) const;
};

SC_INTERNAL_API std::ostream &operator<<(std::ostream &os,
                                         const sc_data_format_t &in);
...
namespace std {
template <> struct hash<dnnl::impl::graph::gc::sc_data_format_t> {
  std::size_t
  operator()(const dnnl::impl::graph::gc::sc_data_format_t &k) const;
};
} // namespace std
