...
static const char *tir_state_names[] = {"CONST_FOLDED", "IR_SIMPLIFIED",
                                        "FUNC_LININED", "SSA_STAGE"};
...
void validate_pass_order(
    ...
    bool gen_wrapper) {
  uint64_t state = 0;
  std::vector<bool> visited(cast_to_int(tir_pass::pass_id::MAX_ID_PLUS_1),
                            false);
  for (size_t i = 0; i < passes.size(); i++) {
    ...
    for (int sid = 0; sid < tir_pass::state::NUM_STATES; sid++) {
      ...
      if ((dep.required_state_ & mask) && ((state & mask) == 0)) {
        COMPILE_ASSERT(
            false, "The pass " << get_pass_name(pass) << " at index " << i
                               << " requires the state " << tir_state_names[sid]
                               << ", but this requirement is not satisfied");
      }
