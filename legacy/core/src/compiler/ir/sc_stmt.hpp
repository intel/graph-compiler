...
public:
  ...
  assign_node_t(expr var, expr value)
      : stmt_base_t(sc_stmt_type::assign), var_(std::move(var)),
...
public:
  ...
  if_else_node_t(expr condition, stmt then_case, stmt else_case)
      : stmt_base_t(sc_stmt_type::if_else), condition_(std::move(condition)),
        then_case_(std::move(then_case)), else_case_(std::move(else_case)){};
  stmt remake() const override;
  bool equals(stmt_c other, ir_comparer &ctx) const override;
};
...
public:
  ...
  define_node_t(expr var, linkage linkage, expr init)
      : stmt_base_t(type_code_), var_(std::move(var)), init_(std::move(init)),
        linkage_(linkage) {}
};
...
using node_ptr_map =
    std::unordered_map<std::shared_ptr<node_base>, std::shared_ptr<node_base>>;
...
public:
  ...
  ptr_type split_on_num_threads(int64_t num_groups,
                                node_ptr_map *node_remap = nullptr);
  ...
  for_loop_node_t(expr var, expr iter_begin, expr iter_end, expr step,
                  ...
                  int num_threads = 0)
      : stmt_base_t(sc_stmt_type::for_loop), var_(std::move(var)),
        iter_begin_(std::move(iter_begin)), iter_end_(std::move(iter_end)),
        step_(std::move(step)), body_(std::move(body)),
        incremental_(incremental), kind_(kind), num_threads_(num_threads) {}
};
