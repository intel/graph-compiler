...
SC_DECL_PASS_INFO(ssa_transform,
                  SC_PASS_DEPENDS_ON(module_globals_resolver,
                                     local_tensor_lowering_cpu, closurizer_cpu,
                                     buffer_scheduler),
...
struct var_cmper_t {
  bool operator()(const expr_c &l, const expr_c &r) const {
    if (l->node_type_ != r->node_type_) {
      return static_cast<int>(l->node_type_) < static_cast<int>(r->node_type_);
    }
...
struct ssa_scope_t {
  ...
  ssa_scope_t(int for_depth, kind kind) : kind_(kind), for_depth_(for_depth) {}
};
...
public:
  ...
  ssa_scope_t &push_scope(ssa_scope_t::kind k) {
    ...
    if (k == ssa_scope_t::kind::for_loop) {
      for_depth++;
    }
  ...
  ssa_var_status_t *insert_local_var(const expr_c &old_var, const expr &new_val,
                                     size_t scope_idx) {
    auto itr = scopes_[scope_idx].vars_.insert(std::make_pair(
        old_var, ssa_var_status_t{new_val, scope_idx, std::vector<expr>()}));
    return &itr.first->second;
  }

  // add an old var definition to scopes, returns the new var
  ssa_var_status_t *insert_local_var(const expr_c &old_var,
                                     const expr &new_val) {
  ...
  ssa_var_status_t *get_local_var_for_update(const expr_c &old_var) {
    if (is_old_var_global(old_var.get())) {
      return get_local_var(old_var);
    }
    auto &back = scopes_.back();
    auto varitr = back.vars_.find(old_var);
    if (varitr != back.vars_.end()) {
      return &varitr->second;
    }
  ...
  void set_var_as_global(const expr_c &old_var) {
    if (!old_var->ssa_data_) {
      old_var.remove_const()->ssa_data_ = utils::make_unique<ssa_data_t>();
    }
  ...
  func_c dispatch(func_c f) override {
    ...
    pop_scope();
    return copy_attr(*f, builder::make_func(f->name_, new_params,
                                            body.remove_const(), f->ret_type_));
  }
  ...
  expr_c visit(var_c v) override {
    ...
    if (!ret->current_value->ssa_data_->is_global_) {
      ...
      if (cur_scope.for_depth_ > scopes_[parent_scope_id].for_depth_) {
        ...
        if (cur_scope.for_depth_ != scopes_[parent_scope_id].for_depth_ + 1) {
          ...
          for (size_t i = parent_scope_id + 1; i < scopes_.size() - 1; i++) {
            ...
            auto phi =
                make_expr<ssa_phi_node>(std::vector<expr>{cur_val}, false);
            auto new_ssa_def = add_ssa_def_to_parent_scope(phi, &the_scope);
            cur_val = new_ssa_def->var_;
            rename_temp_var_with_version(cur_val.checked_as<var>(), v);
            insert_local_var(v, cur_val, i)->for_loop_phi.emplace_back(cur_val);
          }
        }
        ...
        if (insert_scope == &scopes_.back()) {
          ...
          phi = add_ssa_def_to_parent_scope(phi_expr, insert_scope)->var_;
        }
        ...
        insert_local_var(v, phi, scope_idx)->for_loop_phi.emplace_back(phi);
  ...
  stmt_c visit(define_c v) override {
    ...
    if (type == GLOBAL_VAR) {
      newvar->ssa_data_->is_global_ = true;
    }
    expr_c init;
    if (v->init_.defined()) {
      need_flatten_ = false;
    ...
    return copy_attr(
        *v, builder::make_var_tensor_def_unattached(newvar, v->linkage_, init));
  }
  ...
  void rename_temp_var_with_version(const var &newv, const var_c &old_var) {
    if (newv->ssa_data_->is_local()) {
      newv->name_ = old_var->name_ + "_" + std::to_string(var_version_idx++);
    }
  }
  ...
  stmt_c visit(assign_c v) override {
    if (v->var_.isa<var>()) {
      ...
      if (!var_info->current_value.defined() ||
        ...
        if (!v->value_.isa<var>() ||
            get_local_var(v->value_)->defined_scope_idx == scopes_.size() - 1) {
          ...
          rename_temp_var_with_version(cur_value, v->var_.static_as<var>());
          return stmt_c();
        }
        // make a copy for RHS
        auto ret = make_def(rhs);
        auto newvar = ret->var_;
        rename_temp_var_with_version(newvar.checked_as<var>(),
                                     v->var_.static_as<var>());
        ...
        // if is global var
        return copy_attr(
            *v, builder::make_assign_unattached(var_info->current_value, rhs));
      }
    } else {
      ...
      return copy_attr(
          *v, builder::make_assign_unattached(lhs, dispatch(v->value_)));
    }
  }

  expr resolve_single_phi(const expr &val, expr *out_last_level_var) {
    if (val.isa<var>()) {
      if (out_last_level_var) {
        *out_last_level_var = val;
      }
    ...
    if (val.isa<ssa_phi>()) {
      ...
      if (val_phi->values_.size() == 1) {
        return resolve_single_phi(val_phi->values_[0], out_last_level_var);
      }
    }
    return val;
  }

  bool is_same_with_parent_var(const expr &val, const expr &parent_val,
                               expr *out_same) {
    expr last_level_var;
    auto v1 = resolve_single_phi(val, nullptr);
    auto v2 =
        resolve_single_phi(parent_val, out_same ? &last_level_var : nullptr);
    if (v1.ptr_same(v2)) {
      // if the variable is unchanged in loop
      if (out_same) {
        if (!v1.isa<var>() && !v1.isa<tensor>() && !v1.isa<constant>()) {
    ...
    for (auto &kv : scope.vars_) {
      ...
      if (parent_var) {
        ...
        for (auto &phi : kv.second.for_loop_phi) {
          if (is_same_with_parent_var(phi, kv.second.current_value, nullptr)) {
            continue;
          }
          // if the variable is changed in loop, we need to update the
          // phi node input
          auto thephi =
              phi->ssa_data_->get_value_of_var().checked_as<ssa_phi>();
        ...
        auto new_var =
            make_expr<ssa_phi_node>(std::vector<expr>{parent_var->current_value,
        ...
        rename_temp_var_with_version(cur_v.checked_as<var>(),
                                     kv.first.checked_as<var>());
      }
    }
    return copy_attr(*v, builder::make_for_loop_unattached(
                             thevar, begin, end, step, body, v->incremental_,
  ...
  stmt_c visit(if_else_c v) override {
      ...
      for (auto &kv : updated_vars) {
        ...
        if (kv.second.size() == 1) {
          ...
          if (scopes_[parent_var->defined_scope_idx].for_depth_ !=
            ...
            // value
            current_value = visit(kv.first.static_as<var_c>()).remove_const();
          }
        ...
        if (kv.second.size() == 2) {
          ...
          expr same_var;
          if (is_same_with_parent_var(kv.second[0], kv.second[1], &same_var)) {
            get_local_var_for_update(kv.first)->current_value = same_var;
        ...
        if (kv.second.size() == 1) {
          get_local_var_for_update(kv.first)->current_value = kv.second[0];
          continue;
        }
      ...
      for (auto &kv : then_scope.vars_) {
        ...
        if (parent_var) {
          ...
          auto new_phi = make_expr<ssa_phi_node>(
              std::vector<expr>{current_value, kv.second.current_value}, false);
          auto new_var = add_def_after_current_stmt(new_phi);
    ...
    return copy_attr(
        *v, builder::make_if_else_unattached(cond, then_block, else_block));
  }
};
