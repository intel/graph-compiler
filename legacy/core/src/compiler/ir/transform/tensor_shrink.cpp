...
static bool is_reshaped_and_should_shrink(const expr &e) {
  return e.cast<tensorptr>()
      .filter(
          [](const tensorptr &v) { return !v->is_slice_ && should_shrink(v); })
      .map([](const tensorptr &v) { return v->base_; })
...
bool check_brgemm_LDX(const expr &buffer, expr &LDX_e) {
  ...
  if (is_tensor_and_should_shrink(tsr)) {
    ...
    if (LDX_e->attr_->get_or_else("stride_w", 1) > 1) {
      auto N_axis = LDX_e->attr_->get_or_else("N_axis", std::vector<size_t>{});
      // plain
      if (N_axis.size() == 1) {
    ...
    COMPILE_ASSERT(0, "Unexpected LDX found: "
                          << LDX << " for corresponding tensor dims: "
...
public:
  ...
  stmt_c visit(define_c v) override {
    expr var = v->var_;
    if (var->attr_ &&
        var->attr_->has_key(concat_optim_attr_keys::pass_memory_offset)) {
    ...
    if (is_tensor_and_should_shrink(v->var_)) {
      ...
      bool no_init =
          !tsr->init_value_ ||
          tsr->init_value_ == tensor_node::get_zero_tensor_initializer() ||
      ...
      COMPILE_ASSERT(no_init && !v->init_.defined() &&
                     ...
                     "re-scheduled. And it should be a local tensor: "
                         << v);
      auto &shrink_info = v->var_->attr_->get<tensor_shrinker_t::shrink_info_t>(
          tensor_shrinker_attrs::should_shrink);
      COMPILE_ASSERT(shrink_info.shape_.size() == tsr->dims_.size(),
                     "Bad shape for shrinking the tensor: "
                         << v << ", target shape = "
                         << utils::print_vector(shrink_info.shape_));
      auto replacer = copy_attr(
          *tsr, builder::make_tensor(tsr->name_ + "_shr", shrink_info.shape_,
                                     tsr->elem_dtype_, tsr->address_space_,
                                     tsr->init_value_));
      replacer->attr_->remove(tensor_shrinker_attrs::should_shrink);
      replace_map[tsr] = replacer;
      auto ret = builder::make_var_tensor_def_unattached(replacer, v->linkage_);
  ...
  expr_c visit(indexing_c v) override {
    if (is_tensor_and_should_shrink(v->ptr_)) {
      ...
      return builder::make_indexing(itr->second, new_idx, v->dtype_.lanes_,
      ...
      if (new_cur_idx.size() == new_cld_idx.size()) {
        ...
        for (size_t i = 0; i < new_cld_idx.size(); i++) {
          if (new_cld_idx[i].isa<sub>()) {
            ...
            if (rhs.ptr_same(new_cur_idx[i].remove_const()) ||
                (rhs.isa<constant>() && new_cur_idx[i].isa<constant>() &&
        ...
        if (idx_changed) {
          ...
          new_ptr = copy_attr(
              *new_ptr, builder::tensor_ptr(tptr->base_->ptr_, new_cld_idx,
                                            tptr->shape_, tptr->is_slice_));
        }
      }
      if (changed) {
        return copy_attr(*v, builder::make_indexing(
                                 new_ptr.remove_const(), new_cur_idx,
  ...
  expr_c visit(tensorptr_c v) override {
    ...
    if (is_reshaped_and_should_shrink(v.remove_const())) {
      ...
      auto &shrink_info = tptr->attr_->get<tensor_shrinker_t::shrink_info_t>(
          tensor_shrinker_attrs::should_shrink);
    ...
    // transform reshaped tensorptr's idx
      ...
      // below
      auto &shrink_info =
          v->base_->ptr_->attr_->get<tensor_shrinker_t::shrink_info_t>(
              tensor_shrinker_attrs::should_shrink);
      auto tptr = ir_visitor_t::visit(v).checked_as<tensorptr>();
      auto inner_tptr = tptr->base_->ptr_;
      std::vector<expr> newidx;
      bool changed =
          ir_visitor_t::dispatch_expr_vector(tptr->base_->idx_, newidx);
  ...
  stmt_c visit(stmts_c s) override {
    if (s->attr_ && s->attr_->has_key(temp_shrink_tag)) {
      COMPILE_ASSERT(s->seq_.empty(),
                     "Shrink definition placeholder not empty");
