...
public:
  ...
  expr get_barrier_for_current_for() {
    ...
    for (int64_t i = info_.size() - 2; i >= 0; i--) {
      ...
      if (i != 0) {
        idx = info_[i - 1].group_id_ * (num_barrier * barrier_size) + idx;
      }
    }
    ...
    info_.back().barriers_ = builder::make_tensor(
        make_name("_barrier"), {num_barrier * barrier_size}, datatypes::u8);
    top_level_parallel_seq_->emplace_back(
        builder::make_var_tensor_def_unattached(info_.back().barriers_));
    top_level_parallel_seq_->emplace_back(
        builder::make_evaluate_unattached(builtin::get_init_barrier_func()(
            info_.back().barriers_, num_barrier,
  ...
  void gen_call_to_barrier(std::vector<stmt> *cur_insert_point,
    ...
    auto the_call =
        builtin::get_barrier_arrive_func()(b, get_ir_null(), get_ir_null());
    cur_insert_point->emplace_back(builder::make_evaluate_unattached(the_call));
  ...
  bool is_trace_call(const stmt &v) {
    return v.cast<evaluate>()
        ...
        .filter([](func_base *f) {
          return f->attr_ &&
                 f->attr_->get_or_else(function_attrs::is_trace_func, false);
        })
        .has_value();
  }
...
#pragma omp parallel for num_threads(64)
  ...
  */
  void transform_loop(const for_loop_c &v, int num_threads_parent_group,
    ...
    if (!divisible) {
      ...
      seq.emplace_back(builder::make_if_else_unattached(
          gid1 <
              make_expr<constant_node>(uint64_t(num_threads), datatypes::index),
    ...
    builtin::generate_balance211(
        ...
        [&](const char *v) { return make_name(v); }, &begin, nullptr, &end,
        cur_insert_point);
    if (need_pre_barrier) {
      gen_call_to_barrier(cur_insert_point, -1);
    }
    ...
    for (size_t i = 0; i < old_body.size(); i++) {
      if (old_body[i].isa<for_loop>() &&
          old_body[i].static_as<for_loop>()->kind_ == for_type::PARALLEL) {
        ...
        auto body = dispatch(old_body[i]).remove_const().checked_as<stmts>();
        ...
        local_need_pre_barrier = false;
                     ...
                     .filter([](const define &v) {
                       ...
                       bool is_allowed =
                           ...
                           (v->init_.isa<select>() &&
                            utils::is_one_of(v->init_->dtype_, datatypes::u32,
                                             datatypes::u16, datatypes::u8));
                       return !is_allowed;
                     })
                     .has_value()) {
        ...
        if (old_body[i].static_as<define>()->init_.defined()) {
          single_thread_body = stmts();
          new_body->seq_.emplace_back(dispatch(old_body[i]).remove_const());
        } else {
        ...
        bool is_set_idle_func_call =
            dispatched.isa<evaluate>() &&
            dispatched.static_as<evaluate>()->value_.isa<intrin_call>() &&
            dispatched.static_as<evaluate>()
                    ->value_.static_as<intrin_call>()
                    ->type_ == intrin_type::set_thread_idle_func;
        if (is_set_idle_func_call) {
          dispatched.static_as<evaluate_c>()
              ->value_.static_as<intrin_call>()
              ->attr()["post_barrier_id"] = for_count_;
        }
        if (threads_per_group > 1 && !is_set_idle_func_call) {
          if (dispatched.isa<stmts>() &&
            ...
            if (!single_thread_body.defined()) {
              single_thread_body = make_stmt<stmts_node_t>(std::vector<stmt>{});
              new_body->seq_.emplace_back(builder::make_if_else_unattached(
                  tid1 == UINT64_C(0), single_thread_body, stmt()));
            }
  ...
  stmt_c visit(for_loop_c v) override {
    COMPILE_ASSERT(v->num_threads_ >= 0 && v->num_threads_ <= runtime_threads_,
                   "Bad thread count: " << v);
    if (v->kind_ == for_type::PARALLEL) {
      COMPILE_ASSERT(!cannot_parallel_,
                     ...
                     "directly nested in parent parallel-for. "
                         << v);
      if (info_.empty()) {
        if (v->num_threads_ == 0 || v->num_threads_ == runtime_threads_) {
        ...
        auto tid0 = builder::make_var(datatypes::index, make_name("tid"));
        count_++;
        ...
        auto for_lv1 = builder::make_for_loop_unattached(
            tid0, UINT64_C(0), uint64_t(num_threads), UINT64_C(1), body_lv1,
            true, for_type::PARALLEL);
        transform_loop(v, num_threads, body_lv1->seq_, tid0, false, false);
        body_lv0->seq_.emplace_back(for_lv1);
        if (v->attr_ &&
            v->attr_->get_or_else(stmt_attr_key::no_post_barrier, false)) {
        ...
        auto body_lv1 = make_stmt<stmts_node_t>(std::vector<stmt>{});
        transform_loop(
            v, parent_info.threads_per_group_, body_lv1->seq_,
            parent_info.thread_id_, need_pre_barrier_,
            need_post_barrier_ &&
                !(v->attr_ && v->attr_->get_or_else(
  ...
  expr_c visit(intrin_call_c v) override {
    if (v->type_ == intrin_type::get_group_id) {
      ...
      COMPILE_ASSERT(level_id < info_.size(), "Level of group out of range");
