...
struct ssa_cmper_t {
  bool operator()(const define_c &v, const define_c &v2) const {
    ...
    if (v->var_->dtype_ != v2->var_->dtype_) {
      return false;
    }
...
class value_numbering_mutator_t : public ssa_visitor_t {
  using ssa_value_set = std::unordered_set<define_c, ssa_hasher_t, ssa_cmper_t>;
  ...
  stmt_c visit(define_c v) override {
    ...
    if (v->init_.defined()) {
      ...
      if (!new_init.ptr_same(v->init_)) {
        new_def = builder::make_var_tensor_def_unattached(v->var_, v->linkage_,
                                                          new_init);
      }
    }
    if (!v->var_.isa<var>()) {
    ...
    if (!new_def.ptr_same(v)) {
      new_def->temp_data() = v->temp_data();
    }
    bool replaced = false;
    ...
    if (itr != var_set_.end()) {
      ...
      if (!replaced && !old_vn_data.ref_by_phi_) {
        ...
        auto shared_parent = old_info.find_shared_parent(
            get_vn_result(new_def).parent_info_, addresser_, false, true);
        if (shared_parent) {
          if (shared_parent->node_type_ == sc_stmt_type::if_else) {
            shared_parent =
                get_vn_result(shared_parent).parent_info_.get_raw_parent();
          }
          ...
          auto olditr = std::find_if(
              seq_remove.begin(), seq_remove.end(),
              [&old_def](const stmt_c &v) { return v.get() == old_def.get(); });
          ...
          auto scope_itr = std::find_if(
              scopes_.begin(), scopes_.end(), [shared_parent](scope_t &scope) {
          ...
          auto &scope = *scope_itr;
          {
            ...
            get_vn_result(shared_parent)
                .parent_info_.is_parent_of(old_info, addresser_, false, true,
                                           &second_level);
            ...
            // before
            auto insert_before =
                get_vn_result(second_level->get_raw_cur_node()).new_object_;
            if (!insert_before) {
