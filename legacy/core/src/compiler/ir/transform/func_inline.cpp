...
public:
  ...
  func_body_copier_t(std::unordered_map<expr_c, expr> &replace_map,
                     expr ret_var)
  ...
  expr copy(const expr_c &v) override {
    ...
    if (!utils::is_one_of(v->node_type_, sc_expr_type::var,
                          sc_expr_type::tensor) ||
    ...
    if (ret.isa<tensor>() || ret.isa<tensorptr>()) {
      if (ret->attr_ &&
          ret->attr_->has_key(tensor_shrinker_attrs::should_shrink)) {
        COMPILE_ASSERT(attr_copied, "The shrink info should be copied: " << v);
        auto &shrink_info = ret->attr_->get<tensor_shrinker_t::shrink_info_t>(
  ...
  void view(returns_c v) override {
    ...
    if (v->value_.defined()) {
      ...
      returned_stmt_ = make_stmt<assign_node_t>(ret_var_, std::move(newval));
    } else {
...
public:
  ...
  func_inliner_impl_t(bool need_index_flatten,
    ...
    if (auto f = dynamic_cast<func_base *>(callee)) {
      if (modu_) {
        if (auto real_f = modu_->get_func(f->name_)) {
          return any_map_t::fetch_or_else(real_f->attr_.get(), "inline_level",
                                          -1) == 2;
        }
      }
    }
  ...
  expr_c visit(call_c v) override {
    ...
    if (!is_parallel_call &&
        (any_map_t::fetch_or_else(ret->attr_.get(), "inline_level", -1) == 2 ||
  ...
  expr_c inline_at(call_c site, const const_ir_module_ptr &modu = nullptr) {
    ...
    if (modu) {
      ...
      for (size_t i = 0; i < module_vars.size(); i++) {
        rmap.insert(std::make_pair(module_vars[i]->var_, module_vars[i]->var_));
      }
    }
    ...
    if (the_only_stmt.defined() && the_only_stmt.isa<returns>()) {
      ...
      if (site->dtype_ != datatypes::void_t) {
        ...
        outvar = builder::make_var(site->dtype_,
                                   "_retval" + std::to_string(count++));
        itr = ins_point->base.insert(
                  itr, builder::make_var_tensor_def_unattached(outvar)) +
              1;
      }
...
expr_c func_inliner_t::inline_at(call_c c, std::vector<stmt> &seq,
                                 size_t insert_idx,
                                 const const_ir_module_ptr &modu) {
