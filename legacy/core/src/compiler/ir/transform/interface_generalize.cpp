...
const_ir_module_ptr
interface_generalizer_t::operator()(const_ir_module_ptr in) {
  auto ret = in->copy();
      ...
      _function_(datatypes::void_t, wrapper_func,
        ...
        for (uint64_t idx = 0; idx < f->params_.size(); idx++) {
          ...
          if (param->dtype_ == datatypes::generic) {
            ...
            fargs.emplace_back(builder::make_cast(param->dtype_, args[idx]));
          }
        }
      ...
      if (f->attr_) {
        ...
        if (auto comments =
          ...
          std::vector<std::string> new_comments = {
              comments->at(0), "@param args The array of arguments. It should "
