  ...
  void view(call_c v) override {
    ...
    if (funct) {
      ...
      if (funcdef) {
        ...
        if (is_inserted) {
          new_found_->emplace_back(funcdef);
        }
      }
    }
  }
...
const_ir_module_ptr dead_func_eliminate_t::operator()(const_ir_module_ptr f) {
  ...
  std::unordered_set<std::shared_ptr<func_base>> met{gc_roots.begin(),
                                                     gc_roots.end()};
  ...
  if (met.size() == f->get_contents().size()) {
    return f;
  }
  ...
  for (auto &func : f->get_contents()) {
    auto keep =
        met.count(func) != 0 ||
        (func->attr_ && func->attr_->get_or_else(attr_keys::keep_func, false));
    mask.push_back(keep);
    if (!keep) {
      SC_MODULE_INFO << "removing func " << func->name_;
    }
  }
