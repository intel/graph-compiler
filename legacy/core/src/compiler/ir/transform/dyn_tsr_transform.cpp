...
public:
  ...
  expr create_dyn_tsr_from_tensor(const expr &in,
                                  std::vector<stmt> &def_stmts) {
    ...
    if (placeholder.defined() &&
      ...
      shape_tsr =
          placeholder->attr().get<expr>("temp.dyn_shape_of_placeholder");
      ...
      dyn_mask = builder::make_read_struct(placeholder, dyn_tsr_struct_t::name,
      ...
      dyn_mask =
          builder::make_var(datatypes::u8, std::string("dyn_mask_") + name);
    ...
    def_stmts.push_back(builder::make_evaluate_unattached(
        builder::make_write_struct(dyn_tsr, in.remove_const(),
                                   dyn_tsr_struct_t::name,
                                   dyn_tsr_struct_t::fields::data_ptr)));
    def_stmts.push_back(builder::make_evaluate_unattached(
        builder::make_write_struct(dyn_tsr, shape_tsr, dyn_tsr_struct_t::name,
    ...
    def_stmts.push_back(builder::make_evaluate_unattached(
        builder::make_write_struct(dyn_tsr, dtype, dyn_tsr_struct_t::name,
                                   dyn_tsr_struct_t::fields::dtype)));
    def_stmts.push_back(builder::make_evaluate_unattached(
        builder::make_write_struct(dyn_tsr, dyn_mask, dyn_tsr_struct_t::name,
                                   dyn_tsr_struct_t::fields::dyn_mask)));
    return dyn_tsr;
  }
  ...
  expr_c visit(intrin_call_c v) override {
    ...
    case intrin_type::write_struct: {
      if (v->intrin_attrs_->get<std::string>(intrin_attr::struct_name) ==
        ...
        if (changed) {
          return copy_attr(*v, make_expr<intrin_call_node>(v->type_, new_args,
                                                           *v->intrin_attrs_));
        }
      }
      return v;
  ...
  expr_c visit(call_c v) override {
    ...
    // call kernel
    bool trans_all_params =
        (the_func && the_func->attr_ &&
         the_func->attr_->get_or_else(attr_keys::always_trans, false)) ||
        (proto_func->attr_ &&
         proto_func->attr_->get_or_else(attr_keys::always_trans, false));
    for (size_t i = 0; i < v->args_.size(); i++) {
      ...
      if (it != tsr_replace_map_.back().end()) {
        ...
        should_trans =
            should_trans || trans_all_params ||
            tsr_arg->attr().get_or_else(attr_keys::always_trans, false);
        if (should_trans) {
          auto dyn_tsr = create_dyn_tsr_from_tensor(tsr_arg, def_stmts_);
          tsr_replace_map_.back().insert(std::make_pair(arg, dyn_tsr));
          new_args[i] = dyn_tsr;
          changed = true;
        }
  ...
  stmt_c visit(define_c v) override {
    ...
    if (ret->var_.isa<tensor>()) {
      auto plain_dims = any_map_t::fetch_or_else(
          ret->var_->attr_.get(), attr_keys::plain_dims, std::vector<expr>());
      for (size_t i = 0; i < plain_dims.size(); i++) {
        auto &d = plain_dims[i];
        if (!d.isa<constant>()) {
          assert(d.isa<var>());
  ...
  func_c dispatch(func_c v) override {
    ...
    bool trans_all_params =
        v->attr_ && v->attr_->get_or_else(attr_keys::always_trans, false);
    for (auto &p : v->params_) {
      if (p.isa<tensor>()) {
        auto old_tsr = p.static_as<tensor>();
        std::vector<std::pair<size_t, expr>> shape_vars;
        bool cur_const = true;
        auto plain_dims = old_tsr->attr().get_or_else<std::vector<expr>>(
            attr_keys::plain_dims, std::vector<expr>());
        for (size_t i = 0; i < plain_dims.size(); i++) {
          auto &d = plain_dims[i];
          if (!d.isa<constant>()) {
            assert(d.isa<var>());
            if (var_defined_in_func_.find(d) == var_defined_in_func_.end()) {
        ...
        if (trans_all_params ||
            p->attr().get_or_else(attr_keys::always_trans, false)) {
          cur_const = false;
        }
        if (!cur_const) {
          expr dyn_tsr;
          ...
          if (it != tsr_replace_map_.back().end()) {
            ...
            dyn_tsr = builder::make_tensor(std::string("dyn_") + old_tsr->name_,
                                           {sizeof(runtime::dynamic_tensor_t)},
                                           datatypes::u8);
            tsr_replace_map_.back().insert(std::make_pair(old_tsr, dyn_tsr));
          }
          ...
          if (!shape_vars.empty()) {
            // shape tensor define
            auto dyn_shape_tsr =
                builder::make_tensor(std::string("dyn_shape_") + old_tsr->name_,
                                     {plain_dims.size()}, datatypes::index);
            dyn_shape_tsr->attr().set(attr_keys::no_tensor2var, true);
            auto def = builder::make_var_tensor_def_unattached(
                dyn_shape_tsr, linkage::local,
                builder::make_read_struct(dyn_tsr, dyn_tsr_struct_t::name,
                                          dyn_tsr_struct_t::fields::dim_ptr));
            def_stmts_.push_back(def);
            for (auto &var_pair : shape_vars) {
              def_stmts_.push_back(builder::make_var_tensor_def_unattached(
                  var_pair.second, linkage::local,
                  builder::make_indexing(dyn_shape_tsr, var_pair.first)));
            }
          }
    ...
    if (changed) {
      if (!all_const) {
        if (body.isa<stmts>()) {
          ...
          body = copy_attr(*body, make_stmt<stmts_node_t>(std::move(seq)));
        } else {
      ...
      auto new_func =
          copy_attr(*v, builder::make_func(v->name_, new_params,
                                           body.remove_const(), v->ret_type_));
      // save map from symbol to decl.
      func_decl_replace_map_.insert(std::make_pair(v->name_, new_func->decl_));
...
public:
  ...
  expr_c visit(call_c v) override {
    ...
    if (the_func) {
      ...
      if (it != func_decl_replace_map_.end()) {
        return copy_attr(
            *v, builder::make_call(
                    std::const_pointer_cast<func_base>(it->second), v->args_));
      }
    } else {
      func_t proto_func = v->get_prototype();
  ...
  func_decl_replace_impl_t(std::unordered_map<std::string, func_t> &replace_map)
...
const_ir_module_ptr
dyn_tensor_transformer_t::operator()(const_ir_module_ptr f) {
  tensor_transform_impl_t trans_pass;
