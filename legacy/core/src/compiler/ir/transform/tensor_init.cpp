...
public:
  ...
  void insert_tensor_zero_init(std::vector<stmt_c> &seq, const expr &e) {
    ...
    if (tsr.defined() && tsr->init_value_ &&
        (!tsr->attr_ || !tsr->attr_->has_key(attr_keys::shared_const))) {
      union_val val;
      if (tsr->init_value_ == tensor_node::get_zero_tensor_initializer()) {
        val.u64 = 0;
      } else {
      ...
      if (dim.isa<constant>()) {
        ...
        auto step = ctx_->get_max_vector_lanes(tsr->elem_dtype_.type_code_);
        ...
        if (len >= step) {
          ...
          stmt assign = builder::make_assign_unattached(
              ...
              make_expr<constant_node>(
                  val, sc_data_type_t{tsr->elem_dtype_.type_code_, step}));
          set_attr(assign, tsr);
          stmt body = builder::make_stmts_unattached({assign});
          body = builder::make_for_loop_unattached(
              itr, UINT64_C(0), static_cast<uint64_t>(len - remainder),
        ...
        if (remainder != 0) {
          ...
          stmt assign = builder::make_assign_unattached(
              e[{itr}], make_expr<constant_node>(val, tsr->elem_dtype_));
          set_attr(assign, tsr);
          stmt body = builder::make_stmts_unattached({assign});
          body = builder::make_for_loop_unattached(
              itr, static_cast<uint64_t>(len - remainder),
              static_cast<uint64_t>(len), static_cast<uint64_t>(1), body, true,
              for_type::NORMAL);
          seq.emplace_back(body);
        } else {
          ...
          if (len / step <= 8 || must_tensor2var) {
            seq.back().remove_const()->attr()[stmt_attr_key::unroll_loop] = 0;
          }
        }
      } else {
        expr itr = builder::make_var(datatypes::index, "itr");
        ...
        stmt body = builder::make_stmts_unattached({assign});
        body = builder::make_for_loop_unattached(
            itr, static_cast<uint64_t>(0), dim, static_cast<uint64_t>(1), body,
            true, can_parallel ? for_type::PARALLEL : for_type::NORMAL);
        seq.emplace_back(body);
      }
    }
  }
  ...
  func_c dispatch(func_c f) override {
    no_parallel_ =
        f->attr_ && f->attr_->get_or_else(function_attrs::no_parallel, false);
