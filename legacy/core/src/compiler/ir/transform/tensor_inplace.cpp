...
SC_DECL_PASS_INFO(tensor_inplace,
                  SC_PASS_DEPENDS_ON(constant_folder, index_flattener,
                                     validator, auto_caster),
...
static void filter_and_sync_inplace_hint(const func_base *callee,
                                         const func_t &f) {
  if (!callee->attr_) {
    return;
  }
  auto hint_in_def =
      callee->attr_->get_or_null<inplace_hint_t>(function_attrs::inplace_hint);
  if (!hint_in_def) {
    return;
  }
...
static void get_inplace_args_from_called_funcs(
    const func_c &f, // caller
  ...
  for (const auto &st : f->body_.checked_as<stmts>()->seq_) {
    if (!st.isa<evaluate_c>()) {
      continue;
    }
    const auto &callee = st.static_as<evaluate_c>()->value_;
    if (!callee.isa<call_c>()) {
      continue;
    }
    const auto &func =
        std::dynamic_pointer_cast<func_base>(callee.static_as<call_c>()->func_);
    if (!func || !func->attr_) {
      continue;
    }
    ...
    auto hint =
        func->attr_->get_or_null<inplace_hint_t>(function_attrs::inplace_hint);
    if (hint) {
      ...
      for (const auto &pair : *hint) {
        ...
        if (ticks.find(out_arg) != ticks.end() && ticks.at(out_arg).is_arg_ &&
            out_arg->attr_ && out_arg->attr_->has_key("write_buffer")) {
          ...
          for (const auto &in_candidate : pair.second) {
            ...
            if (ticks.find(in_arg) != ticks.end() && ticks.at(in_arg).is_arg_ &&
                in_arg->attr_ && in_arg->attr_->has_key("read_buffer") &&
                !(in_arg->attr_->has_key("write_buffer"))) {
              // this func in arg is also graph's in arg
              if (inplaced_in_args.count(in_arg)) {
                continue;
              }
              ...
              SC_MODULE_INFO << "Inplace hint: #" << pair.first
                             ...
                             << " arg: " << in_arg;
              inplace_pairs.emplace_back(std::make_pair(out_arg, in_arg));
              break;
            }
          }
        }
      ...
      for (const auto &arg : args) {
        if (ticks.find(arg) != ticks.end() && ticks.at(arg).is_arg_ &&
          ...
          COMPILE_ASSERT(
              std::any_of(f_args.begin(), f_args.end(),
                          [&arg](const expr &e) { return e.ptr_same(arg); }),
              "Arg is not output arg of caller");
          for (const auto &f_arg : f_args) {
            if (std::none_of(args.begin(), args.end(), [&f_arg](const expr &e) {
                  return e.ptr_same(f_arg);
                })) {
              // f_arg is an arg of f but not an arg of func
              if (inplaced_in_args.count(f_arg)) {
                continue;
              }
              inplaced_in_args.insert(f_arg);
              inplace_pairs.emplace_back(std::make_pair(arg, f_arg));
    ...
    while (titr != last_read_tensor.end()) {
      ...
      if (in_tsr_tick.create_ <= out_tsr_tick.first_access_ &&
        ...
        if (out_tsr_tick.last_read_ != TICK_NOT_EXIST) {
          auto lower =
              in_tsr_tick.writes_.lower_bound(out_tsr_tick.first_access_);
          auto upper = in_tsr_tick.writes_.upper_bound(out_tsr_tick.last_read_);
          ...
          if (lower != upper) {
            ...
            SC_MODULE_INFO << "Write after read: Failed " << out_tsr->name_
                           << "->" << in_tsr->name_ << "@" << *lower;
            ++titr;
            continue;
          }
        ...
        int64_t in_tsr_size =
            get_const_as_int(in_tsr->dims_[0].static_as<constant_c>());
        ...
        if (std::any_of(inplace_pairs.begin(), inplace_pairs.end(),
          ...
          SC_MODULE_INFO << "Inplace result: " << out_arg << " use " << in_arg;
        }
...
const_ir_module_ptr tensor_inplace_t::operator()(const_ir_module_ptr f) {
  ...
  for (auto &entry_f : ret->get_contents()) {
    ...
    if (entry_f->attr_) {
      top_level = entry_f->attr_->get_or_null<bool>(function_attrs::top_level);
    }
    ...
    for (auto &funct : finder.funcs_) {
      if (!funct->body_.defined()) {
        ...
        if (func_def && func_def->attr_) {
          if (auto hint = func_def->attr_->get_or_null<inplace_hint_t>(
    ...
    for (auto &funct : finder.funcs_) {
      ...
      if (func_def) {
        ...
        if (func_def != funct) {
          for (size_t arg_id = 0; arg_id < funct->params_.size(); arg_id++) {
            auto &arg_in_decl = funct->params_[arg_id];
            auto &arg_in_def = func_def->params_.at(arg_id);
            if (auto alias_id = alias_info::get_alias_info(*arg_in_decl)) {
              // sync the alias info
              arg_in_def->attr()[attr_keys::pointer_alias] =
                  arg_in_decl->attr_->get_any(attr_keys::pointer_alias);
            }
          }
        }
      }
