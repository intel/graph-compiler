  ...
  stmt_c visit(stmts_c v) override {
    ...
    for (int i = 0; i < seqsize; i++) {
      ...
      if (loop.defined()) {
        ...
        if (merge_recursive ||
            (loop->attr_ && loop->attr_->has_key(stmt_attr_key::merge_loop))) {
          ...
          for (int j = i + 1; j < seqsize; j++) {
            ...
            if (nextloop.defined() &&
                (merge_recursive ||
                 (nextloop->attr_ &&
                  nextloop->attr_->has_key(stmt_attr_key::merge_loop)))) {
              if (!writable_loop.defined()) {
                writable_loop = loop->remake().static_as<for_loop>();
              }
              int merged_loops = writable_loop->merge_all(
                  stmt(), ir_cper(nextloop).static_as<for_loop>());
              ...
              if (newseq[j].isa<define>()) {
                move_to_newseq(std::move(newseq[j]), j, expr_insert_pos++);
                // skip the merged loop in the for(int i...)
                nexti++;
                // set changed flag
                changed = true;
        ...
        if (writable_loop.defined()) {
          ...
          writable_loop->body_ = dispatch(writable_loop->body_).remove_const();
