...
SC_DECL_PASS_INFO(index2var,
                  SC_PASS_DEPENDS_ON(constant_folder, ir_simplifier, validator,
                                     ...
                                     tensor_inplace),
                  SC_PASS_REQUIRE_STATE(FUNC_INLINED),
  ...
  void view(call_c v) override {
    bool is_pure_no_args =
        some_opt(dynamic_cast<func_base *>(v->func_.get()))
            .filter([](func_base *f) { return f->params_.empty(); })
            .map([](func_base *f) { return f->attr_.get(); })
            .filter([](any_map_t *m) {
              return m->get_or_else(function_attrs::pure, false);
            })
            .has_value();
    if (is_pure_no_args) {
      ir_viewer_t::view(v);
...
public:
  ...
  var_state_analysis_result_t *get_or_create_var_state(const expr &v) {
    if (auto ret = v->temp_data().get_or_null<var_state_analysis_result_t>()) {
      return ret;
    }
  ...
  void view(define_c v) override {
    if (v->var_.isa<var>() && v->init_.defined()) {
      ...
      var_state->is_complex_ =
          !var_dependency_finder_t::find(var_state->depending_on_, {v->init_});
    }
  }
  ...
  void view(stmts_c v) override {
    ...
    for (auto &s : v->seq_) {
      auto &result =
          s->get_temp_data().get<written_tensor_analysis_result_t>().written_;
...
static bool check_index_no_overlap(ir_comparer &cmper, const expr_c &v1,
    ...
    auto ret =
        v.cast<add>()
            ...
            .filter([](const constant &v) {
              return utils::is_one_of(get_type_category_nothrow(v->dtype_),
                                      CATE_INT, CATE_UINT);
            })
  ...
  if (v1_rhs_const == 0 && v2_rhs_const == 0) {
    return false;
  }
...
class indexing2var_impl_t : public ir_visitor_t {
  ...
  struct tensor_cache_t {
    ...
    tensor_cache_t(tensor_c tsr, std::vector<expr_c> &&idx, var_c var,
        ...
        : tsr_(std::move(tsr)), idx_(std::move(idx)), var_(std::move(var)),
          lanes_(lanes), rows_(rows), mask_(std::move(mask)) {}
      ...
      if (v->dtype_.lanes_ == lanes_ && v->dtype_.rows_ == rows_) {
        ...
        if (v->idx_.size() == 1) {
          if (!cmp.compare(v->idx_[0], idx_[0])) {
            ...
            if (!check_index_no_overlap(cmp, v->idx_[0], idx_[0], lanes_)) {
  ...
  void invalidate_internal(tensor_cache_ptr c, // NOLINT
                           std::vector<stmt> *writeback_point,
                           scope_info_t *scope) {
    if (c->is_valid()) {
      ...
      if (c->last_write_.defined()) {
        ...
        writeback_point->emplace_back(builder::make_assign_unattached(
            builder::make_indexing(c->tsr_, c->idx_, c->lanes_, c->mask_,
                                   c->rows_),
            c->var_));
      }
  ...
  bool invalidate_if_exist(const expr_c &arg,
    ...
    for (auto itr = itrs.first; itr != itrs.second; ++itr) {
      invalidate_internal(itr->second, writeback_point, &scope_info_.back());
    }
  ...
  bool is_var_dependent_only_on_loop_var(const expr_c &itr, int recur_count) {
    ...
    if (loop_vars_.count(itr) == 0) {
      ...
      if (auto var_state =
              itr->get_temp_data().get_or_null<var_state_analysis_result_t>()) {
        if (var_state->assignments_ == 1 && !var_state->is_complex_) {
          for (auto v : var_state->depending_on_) {
            ...
            if (!is_var_dependent_only_on_loop_var(expr_c(ptr),
                                                   recur_count + 1)) {
              return false;
            }
          }
  ...
  expr_c make_cache(indexing_c v, const std::vector<expr> &old_idx,
    ...
    if (v->mask_.defined()) {
      is_valid = is_valid && var_dependency_finder_t::find(vars, &(v->mask_));
    }
    if (!is_valid) {
      out_cache = nullptr;
      return std::move(v);
    ...
    var vcache =
        builder::make_var(v->dtype_, "__cached_" + std::to_string(var_cnt_++))
            .static_as<var>();
    ...
    out_cache = std::make_shared<tensor_cache_t>(
        tsr, std::vector<expr_c>(v->idx_.begin(), v->idx_.end()), vcache,
    ...
    if (only_loop_dependent) {
      ...
      for (auto itr = scope_info_.rbegin(); itr != scope_info_.rend(); ++itr) {
        auto &sinfo = *itr;
        if (!sinfo.loop_) {
          break;
        }
        for_loop_node_t *nested_loop =
            sinfo.loop_->body_.cast<stmts>()
                .filter([](const stmts &v) { return v->seq_.size() == 1UL; })
                .map([](const stmts &v) {
        ...
        lifted_scope = &sinfo;
        if (sinfo.loop_->kind_ != for_type::NORMAL) {
          break;
        }
        ...
        if (itr != scope_info_.rbegin() && !nested_loop) {
          break;
        }
      }
  ...
  bool invalidate_alias_group(const expr_c &tsr, bool invalidate_self) {
    auto analysis_result =
        tsr->get_temp_data().get_or_null<tensor_usage_analysis_result_t>();
    bool ret = false;
    ...
    if (invalidate_self) {
      ret |= invalidate_if_exist(tsr);
    }
    return ret;
  }

  expr_c visit_indexing(const indexing_c &v, bool is_read,
    ...
    if (tsr->attr_ && tsr->attr_->get_or_else(attr_keys::no_index2var, false)) {
      return ret;
    }
    if (!is_read) {
    ...
    for (auto itr = its.first; itr != its.second; ++itr) {
      ...
      if (cmp_result == compare_result::EQ) {
        ...
        SC_MODULE_INFO << "Evict parent scope cache in child scope: " << ret;
  ...
  stmt_c visit(stmts_c v) override {
    ...
    scope_info_.emplace_back(scope_info_t{
        parent_loop,
        insertion_point_,
        v->get_temp_data().get<written_tensor_analysis_result_t>().written_,
        {},
        {},
        {}});
    ...
    for (auto &v : outstanding_cache) {
      ...
      if (v->is_valid()) {
          ...
          for (auto &s : seq) {
            if (s.cast<define>()
                    .filter([&v](const define &def) {
                      return def->var_.get() == v->tsr_.get();
                    })
                    .has_value()) {
              return true;
            }
        ...
        if (v->may_lift_to_) {
          SC_MODULE_INFO << "Evict " << v->tsr_ << " num_flushes="
        ...
        */
        if (v->may_lift_to_ && scope_info_.back().num_flushes_[v->tsr_] == 0 &&
            !check_is_tensor_defined_in_current_scope()) {
          utils::weakptr_hashset_t<expr_base> &dep_vars = v->dependencies_;
          ...
          for (auto itr = seq.begin(); itr != seq.end();) {
            ...
            bool assign_dep =
                (*itr)
                    ...
                    .filter([&dep_vars](var_node *var) {
                      return dep_vars.has(var->shared_from_this());
                    })
                    .get_or_else(nullptr);
            ...
            if (assign_dep || def_var) {
              v->may_lift_to_->insert_point_->emplace_back(std::move(*itr));
              itr = seq.erase(itr);
              continue;
            }
    ...
    if (changed) {
      return builder::make_stmts_unattached(seq);
    }
    return std::move(v);
  }
  ...
  stmt_c visit(assign_c v) override {
    if (v->var_.isa<indexing_c>()) {
      ...
      tensor_cache_ptr out_cache;
      auto lhs =
          visit_indexing(v->var_.static_as<indexing_c>(), false, out_cache);
      ...
      if (out_cache) {
        ...
        if (mask.defined()) {
          if (mask.isa<constant>()) {
            ...
            rhs = builder::make_select(
                mask, rhs, builder::make_constant({UINT64_C(0)}, rhs->dtype_));
          }
        }
      ...
      if (its.first != its.second) {
        for (auto it = its.first; it != its.second; ++it) {
          if (it->second->is_valid()) {
            SC_MODULE_INFO << "Evict due to change of index = " << v->var_
...
public:
  ...
  indexing2var_impl_t(std::unordered_map<alias_info::tensor_alias_identity_t *,
                                         expr_c> &alias_map)
