...
SC_DECL_PASS_INFO(bf16_fp16_eliminator,
                  SC_PASS_DEPENDS_ON(constant_folder, bf16_fp16_legalizer,
                                     index2var),
                  SC_PASS_REQUIRE_STATE(), SC_PASS_REQUIRE_NOT_STATE(),
                  SC_PASS_SET_STATE(), SC_PASS_UNSET_STATE());

sc_data_type_t get_etype(const sc_data_type_t &dtype,
                         const uint16_t lanes = 1) {
  return dtype.is_etype(sc_data_etype::BF16) ? sc_data_type_t::bf16(lanes)
                                             : sc_data_type_t::f16(lanes);
}

static bool check_ref_more_than_one(std::unordered_map<expr_c, int> &m,
                                    const expr &a) {
  ...
  if (v->var_.isa<var>()) {
    if (v->var_->dtype_.is_etype(sc_data_etype::BF16)) {
      return v->var_->dtype_.lanes_ <=
             ...
             any_map_t::fetch_or_else(v->var_->attr_.get(),
                                      "can_promote_to_f32", true);
...
bool can_binary_handled_by_hw(const expr &L, const expr &R,
...
std::tuple<expr_c, expr_c>
bf16_fp16_promote_impl_t::docast(const expr &orig_a, const expr &orig_b,
  ...
  *is_low_precision_fp = false;
  if (utils::is_one_of(a->dtype_.type_code_, sc_data_etype::BF16,
                       sc_data_etype::F16)) {
    COMPILE_ASSERT(utils::is_one_of(b->dtype_.type_code_, a->dtype_.type_code_,
                                    sc_data_etype::F32),
    ...
    *is_low_precision_fp = true;
                              ...
                              sc_data_etype::F16)) {
    COMPILE_ASSERT(utils::is_one_of(a->dtype_.type_code_, b->dtype_.type_code_,
                                    sc_data_etype::F32),
...
expr_c bf16_fp16_promote_impl_t::visit(binary_c v) {
  ...
  if (can_binary_handled_by_hw(v->l_, v->r_, ctx_))
    return v;
  ...
  if (changed) {
    if (is_low_precision_fp) {
      return copy_attr(
          *v, builder::make_cast(get_etype(v->l_->dtype_, v->l_->dtype_.lanes_),
...
expr_c bf16_fp16_promote_impl_t::visit(select_c v) {
  ...
  if (utils::is_one_of(v->l_->dtype_.type_code_, sc_data_etype::BF16,
                       sc_data_etype::F16) &&
      v->l_->dtype_.lanes_ > ctx_->get_max_vector_lanes(sc_data_etype::F32)) {
    return v;
  }
  expr_c a, b;
  bool is_low_precision_fp = false;
...
expr_c bf16_fp16_promote_impl_t::visit(intrin_call_c v) {
  ...
  case intrin_type::isnan:
    for (size_t i = 0; i < v->args_.size(); i++) {
      ...
      if (utils::is_one_of(in->dtype_.type_code_, sc_data_etype::BF16,
                           sc_data_etype::F16)) {
        in = builder::make_cast(sc_data_type_t::f32(in->dtype_.lanes_), in);
        is_low_precision_fp = true;
      }
    ...
    if (is_low_precision_fp) {
      for (size_t i = 0; i < args.size(); i++) {
        COMPILE_ASSERT(!utils::is_one_of(args[i]->dtype_.type_code_,
                                         sc_data_etype::BF16,
                                         sc_data_etype::F16),
  ...
  case intrin_type::reinterpret:
    break;
  default:
    COMPILE_ASSERT(false, "Unsupport BF16 / FP16 intrin type: " << v->type_);
  }
  ...
  if (changed) {
    if (is_low_precision_fp) {
      if (utils::is_one_of(v->type_, intrin_type::reduce_add,
        ...
        return copy_attr(
            *v, builder::make_cast(get_etype(raw_dtype),
                                   builder::remake_intrin_call(v, args)));
      } else {
        return copy_attr(
            *v, builder::make_cast(get_etype(raw_dtype, raw_dtype.lanes_),
                                   builder::remake_intrin_call(v, args)));
      }
    } else {
...
expr_c bf16_fp16_cast_elimination_impl_t::visit(var_c v) {
  ...
  if (it != cvt_map_.end()) {
    return builder::make_cast(get_etype(v->dtype_, v->dtype_.lanes_),
                              it->second);
  }
  return v;
}
...
stmt_c bf16_fp16_cast_elimination_impl_t::visit(define_c v) {
  ...
  if (define_can_promote(ctx_, v) &&
    ...
    if (v->init_.defined()) {
      if (v->linkage_ == linkage::local) {
        init = builder::make_cast(sc_data_type_t::f32(v->init_->dtype_.lanes_),
                                  v->init_);
      }
      init = dispatch(init);
    ...
    if (changed) {
      return copy_attr(
          *v, builder::make_var_tensor_def_unattached(newv, v->linkage_, init));
    }
    return v;
  }
...
stmt_c bf16_fp16_cast_elimination_impl_t::visit(assign_c v) {
  ...
  if (varit != cvt_map_.end()) {
    ...
    if (val.isa<cast_c>()) {
      ...
      val = builder::make_cast(sc_data_type_t::f32(val->dtype_.lanes_), val);
    }
    changed = true;
