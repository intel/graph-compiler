...
static std::unordered_map<sc_data_type_t, int> promotion_priority = {
    {datatypes::f32, 100}, {datatypes::index, 90}, {datatypes::u32, 81},
    {datatypes::s32, 80},  {datatypes::u16, 70},   {datatypes::u8, 61},
    {datatypes::s8, 60},
};

int get_casting_priority(sc_data_type_t dty) {
  auto itr = promotion_priority.find(dty);
  if (itr != promotion_priority.end()) {
    return itr->second;
  }
  return -1;
}
...
static void do_promote(expr_c &v1, expr_c &v2) {
  ...
  if (v1->dtype_.type_code_ == sc_data_etype::F32) {
    if (try_cast_to_fp32(v2))
      return;
  } else if (v2->dtype_.type_code_ == sc_data_etype::F32) {
    if (try_cast_to_fp32(v1))
      return;
  }
  COMPILE_ASSERT(false, "Cannot promote types: "
                            << v1->dtype_ << " and " << v2->dtype_
                            << ". The expr are: " << v1 << " and " << v2);
}

static bool cast_to(expr_c &v, sc_data_type_t ty,
                    const std::function<void()> &raiseit) {
  if (v->dtype_ == ty) {
    return false;
  }
...
bool cast_to(expr_c &v, sc_data_type_t ty, expr_c containing) {
  auto raiseit = [&v, ty, &containing]() {
    COMPILE_ASSERT(false, "Cannot cast from " << v->dtype_ << " to " << ty
                                              << ". expr = " << containing);
  };
...
static expr_c cast_intrin_binary(intrin_call_c oldnode,
                                 std::vector<expr> &newargs, bool changed) {
  auto &l = newargs[0];
  auto &r = newargs[1];
  ...
  if (changed) {
    auto ret =
        copy_attr(*oldnode, builder::remake_intrin_call(oldnode, newargs));
    ret->dtype_ = dtype;
    return std::move(ret);
  } else {
  ...
  expr_c visit(intrin_call_c v) override {
    ...
    case intrin_type::brgemm:
      COMPILE_ASSERT(
          ...
          "or larger than "
              << brgemm_args::NUM_FULL_ARGS_STRIDE << ", but got "
              << newargs.size() << ".");
    ...
    case intrin_type::list_brgemm:
      COMPILE_ASSERT(v->check_brgemm_arg_size(brgemm_args::NUM_FULL_ARGS_LIST),
                     "Wrong number of arguments for list brgemm, expected "
                     "equal or larger then "
                         << brgemm_args::NUM_FULL_ARGS_LIST << ", but got "
                         << newargs.size() << ".");
