...
static inline loop_phi_coalesce_data_t *
get_or_create_coalesce_data(const expr_c &v, const expr &c = expr()) {
  if (!v->get_temp_data().isa<loop_phi_coalesce_data_t>()) {
...
static inline bool merge_interfered_data(const expr_c &var,
                                         const ssa_phi_c &phi) {
...
static inline void merge_coalesced_var(const expr_c &thevar,
                                       const ssa_phi_c &phi,
                                       const stmt &toplevel) {
  assert(phi->values_.size() == 2);
  ...
  void process_define(const expr &var, const expr &init = expr()) {
    ...
    if (init.defined() && init.isa<ssa_phi>()) {
      ...
      if (phi->is_loop_phi_) {
        ...
        if (get_coalesce_data(phi_val_0) && get_coalesce_data(phi_val_1)) {
        ...
        if (phi->values_.size() == 2) {
          ...
          if (v0_data && !interfered) {
            v0_data->check_ = v0_check;
          }
          if (v1_data && !interfered) {
            v1_data->check_ = true;
          }
        }
      }
  ...
  void view(var_c v) override {
    ...
    if (owner.defined()) {
      if (auto dessa_data =
        ...
        owner->temp_data().get<dessa_analysis_data_t>().uses_.emplace_back(
            current_);
      }
    }
...
struct ssa_mutator_t : public ir_visitor_t {
  ...
  expr_c visit(var_c v) override {
    ...
    if (coalesced.defined() && !preserved) {
      return coalesced;
    }
    return v;
  }
  ...
  stmt_c visit(stmts_c v) override {
    ...
    for (auto &s : v->seq_) {
      ...
      auto &s_dessa_data = s->get_temp_data().get<dessa_analysis_data_t>();
      if (!s_dessa_data.inserted_after_.empty()) {
        std::transform(
            s_dessa_data.inserted_after_.begin(),
            s_dessa_data.inserted_after_.end(), std::back_inserter(ret_vec),
            [this](stmt in) {
              return ir_visitor_t::dispatch(std::move(in)).remove_const();
            });
        changed = true;
      }
    }
...
static const stmt_base_t *find_parent_stmts(const stmt_base_t *other) {
  const stmt_base_t *cur = other;
  while (cur) {
    if (cur->node_type_ == sc_stmt_type::stmts) {
      return cur;
    }
    cur = get_parent(cur);
  }
...
static const stmt_base_t *find_parent_for(const stmt_base_t *other) {
  const stmt_base_t *cur = other;
  while (cur) {
    if (cur->node_type_ == sc_stmt_type::for_loop) {
      return cur;
    }
    cur = get_parent(cur);
  }
...
static const stmt_base_t *find_shared_parent(const stmt_base_t *lhs,
                                             const stmt_base_t *other) {
  auto cur = lhs;
  while (cur) {
    if (is_parent_of(cur, other)) {
      return cur;
    }
    cur = get_parent(cur);
  }
...
static void coalesce_var_for_phi(const expr &coalesced, const expr &var_for_phi,
  ...
  if (!coalesced_loop_var.defined()) {
    ...
    data.inserted_after_.insert(
        data.inserted_after_.begin(),
...
static bool promote_scope_for_non_phi_var(const define_c &def,
                                          const expr &value) {
    ...
    if (value->dtype_ == datatypes::void_t) {
      ...
      shared_parent->temp_data()
          ...
          .inserted_within_.emplace_back(
              builder::make_var_tensor_def_unattached(def->var_,
                                                      def->linkage_));
      curdata.inserted_after_.insert(
          curdata.inserted_after_.begin(),
...
static void insert_value_copy_for_phi(const ssa_phi &phi,
                                      const expr &var_for_phi,
                                      const stmt_base_t *cur,
                                      const stmt &toplevel) {
  dessa_analysis_data_t &data = cur->temp_data().get<dessa_analysis_data_t>();
  const stmt_base_t *shared_parent = cur;
  const expr &the_target_var =
      data.shadow_phi_var_.defined() ? data.shadow_phi_var_ : var_for_phi;
  ...
  shared_parent =
      find_shared_parent(shared_parent, find_shared_parent_for_uses(cur));
  ...
  parent_data.inserted_within_.emplace_back(
      builder::make_var_tensor_def_unattached(var_for_phi, linkage::local,
                                              init_val_for_param));
  if (data.shadow_phi_var_.defined()) {
    parent_data.inserted_within_.emplace_back(
        builder::make_var_tensor_def_unattached(
            data.shadow_phi_var_, linkage::local, init_val_for_param));
  }
  data.should_remove_ = true; // remove the phi node
}

static void process_phi(const ssa_phi &phi, const stmt_base_t *cur,
                        const stmt &toplevel) {
  dessa_analysis_data_t &data = cur->temp_data().get<dessa_analysis_data_t>();
  auto def = cur->node_ptr_from_this().checked_as<define>();
    ...
    if (is_loop_phi) {
      ...
      if (!merge_interfered_data(thevar, phi)) {
        ...
        data.shadow_phi_var_ =
            builder::make_var(thevar->dtype_, thevar->name_ + "_shadow");
        auto cur_for_body =
        ...
        cur_for_body->temp_data()
            .get<dessa_analysis_data_t>()
            .inserted_within_.emplace_back(
                builder::make_assign_unattached(thevar, data.shadow_phi_var_));
      }
    }
