...
public:
  ...
  stmt_c visit(stmts_c v) override {
    ...
    stmts writable = (ret.ptr_same(v) ? ret->remake() : ret).static_as<stmts>();
  ...
  stmt_c visit(for_loop_c v) override {
    ...
    if (!is_inner_loop_.empty()) {
      is_inner_loop_.back() = false;
    }
    ...
    if (vv->kind_ == for_type::NORMAL && is_most_inner &&
        begin.isa<constant>() && end.isa<constant>() && step.isa<constant>()) {
