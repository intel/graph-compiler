...
static stmt split_parallel_loop(const for_loop &v, size_t wkld,
                                int runtime_num_threads) {
  ...
  if (wkld <
      (unsigned)runtime_num_threads * memory_access_threshold_per_thread) {
    runtime_num_threads =
        utils::divide_and_ceil(wkld, memory_access_threshold_per_thread);
  }
  if (need_split_parallel) {
    ...
    auto thread_for = builder::make_for_loop_unattached(
        tid, UINT64_C(0), uint64_t(runtime_num_threads), UINT64_C(1), seq, true,
        for_type::PARALLEL, 0);
    expr start, end;
    builtin::generate_balance211(runtime_num_threads, v->iter_begin_,
                                 v->iter_end_, v->step_, tid, nullptr, &start,
                                 nullptr, &end, &seq->seq_);
    v->iter_begin_ = start;
    v->iter_end_ = end;
    v->num_threads_ = 0;
    v->kind_ = for_type::NORMAL;
...
public:
  ...
  stmt_c visit(stmts_c v) override {
    ...
    if (changed) {
      ...
      if (record_workload) {
        stmt_workload_map[newv] = total_wkld;
      }
      return std::move(newv);
    }
    if (record_workload) {
      stmt_workload_map[v] = total_wkld;
    }
    return std::move(v);
  }
  ...
  stmt_c visit(for_loop_c v) override {
    ...
    if (begin.isa<constant>() && end.isa<constant>()) {
      ...
      total_wkld = memory_access_threshold_per_thread * runtime_num_threads;
    }
    cur_workload = total_wkld;
    ...
    if (changed) {
      ...
      if (is_normal_parallel) {
        if (!is_complex_pfor_) {
          newv = split_parallel_loop(newv.checked_as<for_loop>(), total_wkld,
                                     runtime_num_threads);
        }
