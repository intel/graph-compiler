...
struct loop_analysis_viewer_t : public ssa_viewer_t {
  ...
  std::unordered_map<alias_info::tensor_alias_identity_t *, expr_c> alias_map_;
  ...
  void view(for_loop_c v) override {
    ...
    cur_tensor_volatile_map.insert(volatile_ptrs.begin(), volatile_ptrs.end());
    // end of loop scope
...
static std::vector<stmt_c> *
find_stmt_in_map(std::unordered_map<expr_c, std::vector<stmt_c>> &m,
                 const stmt_c &st) {
  for (auto &it : m) {
...
template <class T> static stmt get_owner(T &data) {
  assert(data.defined());
...
struct var_exist_analysis_t : public ssa_viewer_t {
  ...
  void view(var_c v) override {
    if (cur_loop_var_need_find.defined()) {
      if (v.ptr_same(cur_loop_var_need_find)) {
        has_loop_var = true;
      }
    }
  }
...
struct var_property_set_t : public ssa_viewer_t {
  ...
  void view(var_c v) override {
    if (cur_property_var_.defined()) {
      if (v.ptr_same(cur_property_var_)) {
        find_property_var_ = true;
      }
    }
  }
  void process_vars_property(expr &var, expr &value,
                             licm_analysis_data_t *v_licm) {
    if (var.defined() && visited_set_.count(var)) {
      return;
    }
    if (value.defined()) {
      ...
      if (find_property_var_) {
        ...
        if (property_var_owner.defined()) {
          auto property_var_prop = property_var_owner->temp_data()
                                       .get_or_null<licm_analysis_data_t>();
          licm_analysis_data_t *cur_licm = v_licm;
          if (!visited_set_.count(var) && cur_licm != nullptr) {
            cur_licm->dep_vars_.insert(property_var_prop->dep_vars_.begin(),
...
static void
set_loop_var_stmt_dep_var(std::vector<expr_c> &cur_loop_vars_,
      ...
      if (x_owner.defined()) {
        ...
        for (size_t i = 0; i < phi_values.size(); i++) {
          ...
          if (cur_phi_owner.defined()) {
            auto &st_data =
                cur_phi_owner->temp_data_->get<licm_analysis_data_t>();
            ...
            if (loop_depth < get_owner_depth(cur_phi_owner)) {
              ...
              auto parent_stmt = cur_phi_owner->temp_data()
...
struct non_loop_phi_volatile_set_t : public ssa_viewer_t {
  ...
  non_loop_phi_volatile_set_t(const std::vector<expr_c> &loop_vars_,
                              ...
                              std::unordered_set<expr_c> &modified_vars)
      : cur_loop_vars_(loop_vars_), phi_values_(values_phi),
        changed_vars_(modified_vars) {}

  void view(var_c v) override {
    if (visited_set_.count(v)) {
      return;
    }
    auto owner = v->ssa_data_->get_owner();
    auto is_loopvar =
        std::find_if(cur_loop_vars_.begin(), cur_loop_vars_.end(),
    ...
    if (is_loopvar == cur_loop_vars_.end() && owner.defined()) {
      set_loop_var_stmt_dep_var(cur_loop_vars_, phi_values_, owner,
                                changed_vars_);
...
static void process_with_non_loop_phi(std::vector<expr> &phi_values,
  ...
  for (size_t i = 1; i < phi_values.size(); i++) {
    ...
    if (owner.defined()) {
      ...
      auto cur_owner_scope = owner->temp_data_->get<licm_analysis_data_t>()
                                 .parent_->temp_data()
                                 .get<licm_analysis_data_t>()
                                 .parent_->node_ptr_from_this()
                                 .remove_const();
      ...
      if (get_licm_data_volatile(cur_owner_scope)) {
        ...
        if (cur_owner.defined()) {
          ...
          // be in same scope
          if (cur_depth + 1 == get_owner_depth(owner) && cur_owner.defined()) {
      ...
      non_loop_phi_volatile_set_t vlt_set(cur_loop_vars_, phi_values,
                                          modified_vars_set);
      vlt_set.dispatch(cur_owner_scope);
      // need to find more accurate owner that have loop var
      set_loop_var_stmt_dep_var(cur_loop_vars_, phi_values, cur_owner_scope,
                                modified_vars_set);
    }
  }
  // propagation
  ...
  while (!modified_vars.empty()) {
    ...
    if (cur_owner.defined()) {
      auto parent_stmt = cur_owner->temp_data()
                             ...
                             .remove_const();
      if (parent_stmt.defined()) {
        var_property_set_t property_set(modified_vars_set, modified_vars);
...
struct licm_analysis_viewer_t : public ssa_viewer_t {
  ...
  expr_c dispatch(expr_c s) override {
    if (!expr_can_hoist(s) && current_ != nullptr && !cur_loop_vars_.empty()) {
  ...
  void view(for_loop_c v) override {
    ...
    v_data.dep_vars_.insert(body_data.dep_vars_.begin(),
                            body_data.dep_vars_.end());
    v_data.dep_tensors_.insert(body_data.dep_tensors_.begin(),
                               body_data.dep_tensors_.end());
    --if_for_depth_count_;
  }
  void view(stmts_c v) override {
    ssa_viewer_t::view(v);
    ...
    for (auto &st : v->seq_) {
      // if st is garbage
      if (!st->get_temp_data().isa<licm_analysis_data_t>()) {
        continue;
      }
      auto &st_data = st->temp_data().get<licm_analysis_data_t>();
      v_data.dep_vars_.insert(st_data.dep_vars_.begin(),
                              st_data.dep_vars_.end());
      v_data.dep_tensors_.insert(st_data.dep_tensors_.begin(),
  ...
  void view(if_else_c v) override {
    ...
    ssa_viewer_t::view(v);
    auto &st_data = v->temp_data().get<licm_analysis_data_t>();
    auto &then_data = v->then_case_->temp_data().get<licm_analysis_data_t>();
    st_data.volatile_ |= then_data.volatile_;
    st_data.dep_vars_.insert(then_data.dep_vars_.begin(),
                             then_data.dep_vars_.end());
    st_data.dep_tensors_.insert(then_data.dep_tensors_.begin(),
                                then_data.dep_tensors_.end());
    if (v->else_case_.defined()) {
      auto &else_data = v->else_case_->temp_data().get<licm_analysis_data_t>();
      st_data.volatile_ |= else_data.volatile_;
      st_data.dep_vars_.insert(else_data.dep_vars_.begin(),
                               else_data.dep_vars_.end());
    ...
    for (auto &var : vars_in_if_scope_) {
      ...
      var_data.dep_vars_.insert(st_data.dep_vars_.begin(),
                                st_data.dep_vars_.end());
  ...
  void view(var_c v) override {
    ...
    st_data.dep_vars_.insert(var_data.dep_vars_.begin(),
                             var_data.dep_vars_.end());
    st_data.dep_tensors_.insert(var_data.dep_tensors_.begin(),
                                var_data.dep_tensors_.end());
    ...
    if (owner.defined() &&
      ...
      if (owner.isa<for_loop_c>()) {
        st_data.dep_vars_.insert(v);
      }
    } else if (!is_param) {
  ...
  void view(ssa_phi_c v) override {
    ...
    if (v->is_loop_phi_) {
      ...
      process_with_non_loop_phi(v.remove_const()->values_, v, cur_loop_vars_);
    }
  }
};
...
struct licm_hoist_prepare_viewer_t : public ssa_viewer_t {
  ...
  void register_loop_invariant_stmt() {
    ...
    if (current_->node_type_ == sc_stmt_type::stmts) {
      return;
    }
    ...
    for (; it != cur_loop_vars_.rend(); it++) {
      ...
      // cannot promote
      bool volatile_by_tensor =
          unordered_intersects(st_data.dep_tensors_, tensor_volatile_map_[*it]);
      // If any dep_vars_ is the loop var, cannot promote
      bool volatile_by_var = //
          st_data.dep_vars_.end() !=
          std::find_if(st_data.dep_vars_.begin(), st_data.dep_vars_.end(),
  ...
  stmt_c visit(stmts_c v) override {
    ...
    for (auto &s : v->seq_) {
      ...
      if (stmt_to_remove_.find(s) != stmt_to_remove_.end()) {
        assert(s->temp_data().get<licm_analysis_data_t>().volatile_ == false);
        changed = true;
        continue;
      }
...
func_c loop_invariant_code_motion_t::operator()(func_c f) {
  ...
  licm_hoist_prepare_viewer_t hoist_prepared(
      loop_analyzer.call_volatile_map_, loop_analyzer.tensor_volatile_map_);
