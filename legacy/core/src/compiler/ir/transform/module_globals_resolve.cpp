...
public:
  ...
  expr_c visit(call_c v) override {
    ...
    if (itr == map->end()) {
      ...
      } else if (v->func_->attr_ && v->func_->attr_->get_or_else(
                                        "is_brgemm_func_with_stream", false)) {
        COMPILE_ASSERT(!v->args_.empty() &&
                           v->args_.back()->dtype_ == datatypes::pointer,
  ...
  expr_c visit(constant_c v) override {
    if (v->dtype_ == datatypes::pointer && v->value_[0].u64 == 0) {
      if (any_map_t::fetch_or_else(v->attr_.get(), "auto_fill_stream", false)) {
        return current_rtl_ctx;
      }
      if (any_map_t::fetch_or_else(v->attr_.get(), "auto_fill_module_data",
                                   false)) {
        return builder::make_reinterpret(current_base, datatypes::pointer);
      }
    }
    return v;
  }
  ...
  expr_c get_or_create_new_var(const expr_c &old) {
    ...
    if (itr != global_symbol_replace_map_.end()) {
      return itr->second;
    }
    auto ret = old->remake();
    ...
    if (absolute_base_ && ret.isa<var>()) {
      ...
      void *ptr = reinterpret_cast<void *>(attr.get<size_t>() + absolute_base_);
      attr = ptr;
    }
      ...
      if (auto buf = any_map_t::fetch_or_else<
        ...
        return copy_attr(*v,
                         builder::make_var_tensor_def_unattached(
                             newvar, linkage::local,
                             builder::tensor_ptr(shared_const_base_tsr_.at(idx),
                                                 {buf->offset_})));
      }
    }
    return ir_visitor_t::visit(v);
  ...
  stmt_c visit(stmts_c v) override {
    if (is_top_level) {
      ...
      if (global_symbol_local_def_.empty()) {
        return ret;
      }
      ...
      for (auto &v : global_symbol_local_def_) {
        expr init;
        if (v.isa<tensor>()) {
          auto anyoffset = v->attr().get_any(attr_keys::module_global_offset);
          if (auto poffset = anyoffset.get_or_null<size_t>()) {
            auto offset = *poffset;
            if (absolute_base_) {
              init =
                  make_expr<constant_node>((uint64_t)(absolute_base_ + offset),
                                           datatypes::s8.get_pointerof());
            } else {
              init = builder::tensor_ptr(current_base, {offset});
            }
          } else {
            init = make_expr<constant_node>((uint64_t)(anyoffset.get<void *>()),
                                            datatypes::s8.get_pointerof());
          }
        }
        seq.emplace_back(
            builder::make_var_tensor_def_unattached(v, linkage::local, init));
      }
      ...
      if (shared_const_base_tsr_.empty()) {
        ...
        if (!shared_const_base_inserted) {
          for (auto &tsr : shared_const_base_tsr_) {
            seq.insert(
                seq.begin() + old_seq_size,
                builder::make_var_tensor_def_unattached(tsr, linkage::local));
          }
        }
      }
...
static size_t update_allocated_size(const tensor &tsr, size_t size,
  ...
  if (size >= 64) {
    allocated_size = align_to_64(allocated_size);
  }
 ...
 * */
static std::vector<void *> process_shared_const_tensors(
    const ir_module_ptr &m,
  ...
  if (bases) {
    ...
    for (auto &base : *bases) {
      out_base_tsr.emplace_back(builder::make_tensor(
          "__shared_const_base_" + std::to_string(collected_base.size()),
      ...
      void *handle =
          (base->is_lazy_) ? base.get() : base->get_buffer_if_not_lazy();
  ...
  for (auto &s : seq) {
    ...
    if (auto const_graph_tsr =
            s.cast<define_c>()
                .flat_map(
                    [](const define_c &v) { return v->var_.cast<tensor>(); })
                .map([](const tensor &v) {
                  return any_map_t::fetch_or_else<
                      std::shared_ptr<cached_const_graph_tensor>>(
                      v->attr_.get(), attr_keys::shared_const, nullptr);
                })
                .get_or_else(nullptr)) {
      ...
      void *handle =
          (base->is_lazy_) ? base.get() : base->get_buffer_if_not_lazy();
      COMPILE_ASSERT(bases, "Expecting SHARED_CONST_BASES attr in the module");
      ...
      if (!out_base_tsr[idx]->attr().has_key(attr_keys::shared_const)) {
        out_base_tsr[idx]->attr()[attr_keys::shared_const] = const_graph_tsr;
      }
    }
  }
  if (!collected_base.empty()) {
    auto tsr = m->make_global_tensor(datatypes::index, shared_const_handle_name,
                                     {uint64_t(collected_base.size())});
  }
  return collected_base;
}

const_ir_module_ptr
module_globals_resolver_t::operator()(const_ir_module_ptr m) {
  auto ret = std::make_shared<ir_module_t>(*m);
  std::vector<std::shared_ptr<cached_const_graph_tensor>> out_graph_tsr;
  std::vector<expr> shared_const_base_tsr;
  auto base_cache_tsr =
      process_shared_const_tensors(ret, out_graph_tsr, shared_const_base_tsr);
  ...
  for (auto &def : ret->get_module_vars()) {
    if (def->var_.isa<var>()) {
      ...
      allocated_size = utils::divide_and_ceil(allocated_size, size) * size;
        ...
        COMPILE_ASSERT(tsr->init_value_->size_ == size,
                       "The size of the global tensor ("
                           ...
                           << ") does not match its "
                              "initializer ("
                           << size << "): " << def);
        allocated_size = update_allocated_size(tsr, size, allocated_size);
      }
    }
  }
  ...
  for (auto &def : ret->get_module_vars()) {
    ...
    if (def->var_.isa<var>()) {
      sym_table->add(def->var_.static_as<var>()->name_,
                     anyoffset.get<size_t>());
    } else {
      auto tsr = def->var_.checked_as<tensor>();
      if (auto offset = anyoffset.get_or_null<size_t>()) {
        sym_table->add(tsr->name_, *offset);
        if (tsr->init_value_) {
          memcpy(reinterpret_cast<char *>(sym_table->data_.data_) + *offset,
  ...
  bool is_static_global =
      m->attr_.get_or_else(ir_module_t::attr_key_t::STATIC_GLOBALS, false);
  ...
  for (auto &f : funcs) {
    ...
    auto retf = builder::make_func(
        f->name_, params, builder::make_stmts_unattached({}), f->ret_type_);
    if (f->attr_) {
  ...
  for (unsigned i = 0; i < funcs.size(); i++) {
    if (!funcs[i]->body_.defined()) {
      continue;
    }
    if (funcs[i]->attr_ &&
        funcs[i]->attr_->get_or_else(function_attrs::low_level, false)) {
      continue;
    }
    if (any_map_t::fetch_or_else(funcs[i]->attr_.get(), "device_func", false)) {
      continue;
    }
    ...
    if ((int)i == ret->get_entry_func_idx()) {
      ...
      auto handle_tsr = ret->get_var_def_from_symbol(shared_const_handle_name);
