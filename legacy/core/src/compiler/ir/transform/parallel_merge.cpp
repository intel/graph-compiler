         ...
         s.cast<define>()
             .filter([](const define &v) {
               if (v->var_.isa<tensor>()) {
                 if (!v->init_.defined() || v->init_.isa<tensorptr>()) {
                   return true;
                 }
               }
               return false;
  ...
  for (auto &s : f->body_.checked_as<stmts>()->seq_) {
    if (auto func = get_callee(s)) {
      ...
      continue;
                   ...
                   .filter([&num_pfor, &num_threads](const for_loop &v) {
                     ...
                     auto v2 =
                         get_const_as_int(v->iter_end_.static_as<constant>());
                     if (v2 > runtime_config_t::get().get_num_threads()) {
                       return false;
                     }
                     auto v3 = get_const_as_int(v->step_.static_as<constant>());
                     if (v3 != 1) {
                       return false;
                     }
                     num_threads = v2;
...
static func_base *get_func_to_merge(const ir_module_t &mod, const stmt &s,
                                    ...
                                    const std::string *&out_expected_next_op) {
...
static void sort_funcs(std::vector<func_call_numthreads_t> &funcs,
...
static func_t merge_funcs(std::vector<func_call_numthreads_t> &funcs) {
  ...
  for (auto &f : funcs) {
    ...
    all_private = all_private &&
                  any_map_t::fetch_or_else(f.f_->attr_.get(),
  ...
  for (auto &kv : funcs) {
    ...
    for (auto &s : seq) {
      if (auto callee = get_callee(s)) {
        ...
        */
        ...
        if (num_threads <= remaining_threads && num_threads != total_threads) {
        ...
        if (num_threads != total_threads) {
          ...
          retseq.emplace_back(builder::make_if_else_unattached(
              loop_iter >= tid_offset && loop_iter < tid_offset + num_threads,
              dispath_body, stmt()));
        }
        ...
        target_body->insert(target_body->end(), copied.begin(), copied.end());
    ...
    for (auto &p : f->params_) {
      retargs.emplace_back(utils::find_map_value(replace_map, p)
                               .map([](expr *v) { return *v; })
  ...
  auto forbody = builder::make_for_loop_unattached(
      loop_iter, UINT64_C(0),
...
const_ir_module_ptr parallel_merge_t::operator()(const_ir_module_ptr f) {
    ...
    if (auto callee =
      ...
      for (size_t j = i + 1; j < old_seq.size(); j++) {
        ...
        if (!sj.defined()) {
          ...
          bool ok_to_merge = callee2 && callee2->name_ == *next_expected_func &&
                             is_func_ok_to_merge(callee2, num_threads);
          if (ok_to_merge) {
            to_merge.emplace_back(
                num_threads, callee2->shared_from_this(),
                sj.static_as<evaluate>()->value_.static_as<call>());
            old_seq[j] = stmt();
            insert_point = j;

            next_expected_func = any_map_t::fetch_or_null<std::string>(
                sj->attr_.get(), attr_keys::no_post_barrier);
            if (next_expected_func) {
              continue;
            }
          }
      ...
      if (to_merge.size() > 1UL) {
        if (!ret) {
          ret = std::make_shared<ir_module_t>(*f);
        }
        old_seq[i] = stmt();
        ...
        for (auto &ca : to_merge) {
          args.insert(args.end(), ca.c_->args_.begin(), ca.c_->args_.end());
        }
  ...
  auto newmain =
      copy_attr(*mainf, builder::make_func(mainf->name_, mainf->params_,
                                           builder::make_stmts_unattached(seq),
                                           mainf->ret_type_));
