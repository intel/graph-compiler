...
func_t get_dyn_threadpool_loop_end_func() {
  static func_t f = builder::_decl_func(
      "sc_dyn_threadpool_loop_end", datatypes::index,
      {_arg_("scope", datatypes::index), _arg_("size", datatypes::index)});
  return f;
}

func_t get_dyn_threadpool_submit_func() {
  static func_t f = builder::_decl_func(
      "sc_dyn_threadpool_create_work_items", datatypes::index,
      {_arg_("pfunc", datatypes::pointer), _arg_("iter", datatypes::pointer),
       _arg_("iter_num", datatypes::index), _arg_("loop_len", datatypes::index),
       _arg_("num_blocks", datatypes::index),
       _arg_("tid_hint", datatypes::index),
       _arg_("buffer_num", datatypes::index),
       _arg_("buffers", datatypes::pointer), _arg_("flags", datatypes::index)});
  return f;
}

func_t get_dyn_threadpool_run_func() {
  static func_t f =
      builder::_decl_func("sc_dyn_threadpool_run", datatypes::void_t, {});
  return f;
}

func_t get_dyn_threadpool_destroy_func() {
  static func_t f = builder::_decl_func("sc_dyn_threadpool_sched_destroy",
                                        datatypes::void_t, {});
  return f;
}
...
func_t get_dyn_threadpool_init_func() {
  static func_t f =
      builder::_decl_func("sc_dyn_threadpool_sched_init", datatypes::void_t,
...
struct dyn_parallel_analysis_result_t {
  ...
  std::vector<std::shared_ptr<parallel_for_scope_t>> pfor_chain_{
      std::make_shared<parallel_for_scope_t>(for_loop_c(), 0, true, 0, nullptr,
                                             expr_c())};
};
...
public:
  ...
  void assert_value_is(const expr &v, int64_t val) const {
    COMPILE_ASSERT_POS(
        v.isa<constant>() && get_const_as_int(v.static_as<constant_c>()) == val,
        "Expecting " << val << " for the boundary of the loop, got " << v);
  }

  dyn_parallel_analysis_impl_t(
      ...
      : main_thread_defined_(main_thread_defined), globals_(globals),
        result_(result), num_threads_{num_threads} {}
  ...
  void view(tensor_c v) override {
    if (!try_capture_in_parallel(v)) {
      ...
      if (!defined_in_main) {
        COMPILE_ASSERT_POS(globals_.count(v), "Use of undefined tensor " << v);
      }
    }
    ir_viewer_t::view(std::move(v));
  }
  ...
  void view(var_c v) override {
    ...
    if (!result_.pfor_chain_.back()->defined_.count(v)) {
      auto defined_in_main = try_capture_main_thread(v);
      if (!defined_in_main) {
        COMPILE_ASSERT_POS(globals_.count(v),
  ...
  void view(for_loop_c v) override {
    ...
    if (v->kind_ == for_type::NORMAL) {
      ...
      COMPILE_ASSERT_POS(!chain_data &&
                             !result_.pfor_chain_.back()->in_if_scope_,
      ...
      auto ptr = std::make_shared<parallel_for_scope_t>(
          v, current_scope->nested_level_ + 1, true, thread_id_step,
...
static int transform_scopes_to_funcs(
    const std::string &name, const stmts_c &body,
    const dyn_parallel_analysis_result_t &result, int idx_in_result,
    const std::unordered_set<expr_c> &globals, std::vector<func_t> &out_funcs) {
  ...
  auto prepare_new_var_tensor = [&replace_map, &globals, &result,
                                 &current_scope]() {
  ...
  auto prepare_new_out_func = [&replace_map, &cpyer, &idx_in_result, &name,
    ...
    auto ret = builder::make_func(name + "_0_closure_N" +
                                      std::to_string(idx_in_result),
                                  new_args, new_body, datatypes::void_t);
  ...
  for (auto &s : body->seq_) {
    ...
    auto captured_def =
        s.cast<define>()
            .flat_map([](const define &v) { return v->var_.cast<tensor>(); })
            .filter([&](const tensor &v) {
              return (uint64_t)idx_in_result + 1 < result.pfor_chain_.size() &&
                     result.pfor_chain_[idx_in_result + 1]->captured_.count(v);
            });
    ...
    // "s" is a parallel for
    idx_in_result =
        transform_scopes_to_funcs(name, loop->body_.as<stmts>(), result,
                                  idx_in_result + 1, globals, out_funcs) +
        1;
...
static void finalize_body_funcs(const dyn_parallel_analysis_result_t &result,
  ...
  for (size_t i = 0; i < body_funcs.size(); i++) {
    ...
    auto func = builder::make_func(
        body_funcs[i]->name_ + "_wrapper",
        {builder::make_tensor("itr", {depth}, datatypes::index),
         builder::make_tensor("buffers", {num_buffers}, datatypes::generic),
         builder::make_tensor("args", {num_args}, datatypes::generic)},
  ...
  auto find_parent_scope =
      [&body_funcs, &result](const for_loop_c &v, size_t idx_in_body_funcs,
                             size_t &out_idx_in_body_funcs) {
        uint64_t diff = 0;
  ...
  for (size_t i = 0; i < body_funcs.size(); i++) {
    ...
    */
    ...
    while (cur_tail_scope->is_end_) {
      ...
      has_barrier = true;
      auto scope_ret = builder::make_var(
          datatypes::index, "__scope_handle_" + std::to_string(scope_count));
      scope_count++;
      uint64_t new_scope = 0;
      auto scope_diff = find_parent_scope(cur_tail_scope->loop_,
                                          last_parent_scope_idx, new_scope);
      last_parent_scope_idx = new_scope;
      cur_body->emplace_back(builder::make_var_tensor_def_unattached(
          scope_ret, linkage::local,
          get_dyn_threadpool_loop_end_func()(current_scope_handle,
                                             scope_diff)));
      auto if_body = builder::make_stmts_unattached({}).static_as<stmts>();
      ...
      cur_tail_scope_idx++;
      if (cur_tail_scope_idx >= result.pfor_chain_.size()) {
        break;
      }
    ...
    if (next_scope_info->is_start_) {
      ...
      num_threads =
          std::max((uint64_t)next_scope_info->loop_->num_threads_, UINT64_C(1));
      ...
      if (nextfunc->body_.static_as<stmts>()->seq_.empty()) {
        ...
        for (auto &itr : next_scope_info->iters_) {
          args.emplace_back(*utils::find_map_value(replace, itr).get());
        }
        ...
        cur_body->emplace_back(builder::make_evaluate_unattached(thecall));
        assert(utils::is_uninitialized_weakptr(next_scope_info->inlined_to_));
    ...
    if (num_shared_buffers) {
      ...
      if (fast_path) {
        ...
        buffers = builder::make_tensor("_shared_buf", {num_shared_buffers},
                                       datatypes::generic);
        ...
        for (size_t idx = 0; idx < next_buffers.size(); idx++) {
          ...
          COMPILE_ASSERT(itr != replace.end(),
                         ...
                         "buffer");
          cur_body->emplace_back(builder::make_assign_unattached(
              buffers[idx],
              builder::make_cast(datatypes::generic, itr->second)));
        }
    ...
    body_funcs[i]->temp_data_ = nullptr;
    cur_body->emplace_back(
        builder::make_evaluate_unattached(get_dyn_threadpool_submit_func()(
            builder::make_func_addr(out_funcs[next_func_idx]->decl_),
            get_ir_null(), num_iter, loop_len, num_threads,
            /*fix-me: (yijie) outer_loop_hash*/ UINT64_C(0), num_shared_buffers,
            buffers,
            /*flags*/ tid_step | runtime::dynamic_threadpool::work_item_flags::
                                     bind_last_level)));
  }
}
...
static void generate_wrappers(const dyn_parallel_analysis_result_t &result,
                              std::vector<func_t> &body_funcs,
                              std::vector<func_t> &wrapper_funcs) {
...
public:
  ...
  dyn_parallel_transform_impl_t(bool always_transform, const ir_module_ptr &mod,
                                int num_threads)
      : always_transform_(always_transform), mod_(mod),
  ...
  stmt transform_loop(for_loop_c v) {
    ...
    dyn_parallel_analysis_result_t result;
    dyn_parallel_analysis_impl_t ana{(uint64_t)num_threads_, defined_, globals_,
                                     result};
    ...
    transform_scopes_to_funcs(cur_func_->name_, v->body_.as<stmts>(), result, 1,
                              globals_, body_funcs);
    ...
    auto captures = builder::make_tensor(
        ...
        {(uint64_t)result.ordered_captured_.size()}, datatypes::generic);
    capture_count_++;
    ...
    retstmts->seq_.emplace_back(
        builder::make_evaluate_unattached(get_dyn_threadpool_init_func()(
            null_for_stream, null_for_module_data, captures, UINT64_C(1),
    ...
    retstmts->seq_.emplace_back(
        builder::make_evaluate_unattached(get_dyn_threadpool_run_func()()));
    retstmts->seq_.emplace_back(
        builder::make_evaluate_unattached(get_dyn_threadpool_destroy_func()()));
  ...
  func_c dispatch(func_c f) override {
    ...
    defined_.emplace_back(
        std::unordered_set<expr_c>(f->params_.begin(), f->params_.end()));
...
const_ir_module_ptr
dynamic_parallel_transform_t::operator()(const_ir_module_ptr f) {
  ...
  for (size_t i = 0; i < sz; i++) {
    if (threads == 1) {
      ...
      contents[i] =
          std::const_pointer_cast<func_base>(impl.dispatch(contents[i]));
    }
  }
