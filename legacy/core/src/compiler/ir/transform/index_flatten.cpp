...
static bool process_indexing(ir_visitor_t *ths,
                             const std::vector<expr> &old_dims,
                             const std::vector<expr> &old_strides,
                             const std::vector<expr> &idx,
                             std::vector<expr_c> &newidx, bool is_2d) {
  ...
  if (idx.size() == 1) {
    ...
    if (newidx.size() == 2 && is_2d) {
      newidx = std::vector<expr_c>{
          builder::make_constant({UINT64_C(0)}, datatypes::s32), newidx[0],
          newidx[1]};
      return true;
    }
    ...
    if (is_2d) {
      ...
      newidx = std::vector<expr_c>{std::move(flattened), std::move(idx0),
                                   std::move(idx1)};
    } else {
...
static bool process_indexing(ir_visitor_t *ths, tensor_c &tsr,
  ...
  COMPILE_ASSERT(old->dims_.size() == idx.size(),
                 "Unmatched dimensions of indexing: tsr= "
                     << tsr << utils::print_vector(old->dims_)
                     << ", indexing on" << utils::print_vector(idx));
  changed |=
      process_indexing(ths, old->dims_, old->strides_, idx, newidx, is_2d);
  return changed;
}
...
static const std::vector<expr> *get_base_shape(const expr_c &ex) {
  if (ex.isa<tensor>()) {
    return &ex.static_as<tensor>()->dims_;
  }
...
static std::vector<expr> get_base_stride(const expr_c &ex) {
  if (ex.isa<tensor>()) {
    return ex.static_as<tensor>()->strides_;
  }
  COMPILE_ASSERT(ex.isa<tensorptr>(), "Expecting tensorptr, got: " << ex);
  auto tptr = ex.static_as<tensorptr>();
  if (tptr->is_slice_) {
...
/* process indexing on tptr(slice=false) with base = tptr(slice=true)
   ...
   flatten(C[s,c]) = flatten(C) + (s / (W-1) % (H-1) * W *
                        C + s % (W-1) * C + c)
*/
static void process_slice_reshape_indexing(
    ir_visitor_t *ths, const std::vector<expr> &old_dims,
    const std::vector<expr> &slice_shape,
  ...
  if (is_shape_all_const) {
    ...
    auto slice_dense_stride = get_expr_to_dims(get_dense_stride(slice_shape));
    ...
    for (auto i = (int)slice_const_shape.size() - 1; i >= 0; i--) {
      while (acc_shape % slice_const_shape[i] != 0 && acc_idx >= 0) {
        flatten_remain = idx[acc_idx] * stride[acc_idx] + flatten_remain;
        acc_shape *= real_shape[acc_idx];
        acc_idx--;
      }
      if (acc_shape == slice_const_shape[i]) {
        flattened = flatten_remain / expr((uint64_t)slice_dense_stride[i]) *
                        parent_stride[i] +
                    flattened;
        flatten_remain = 0UL;
        acc_shape = 1;
        ...
        flattened = flatten_remain / expr((uint64_t)slice_dense_stride[i]) %
                        (uint64_t)slice_const_shape[i] * parent_stride[i] +
                    flattened;
    ...
    for (auto i = (int)slice_shape.size() - 1; i >= 0; i--) {
      flattened =
          (flatten_remain % slice_shape[i]) * parent_stride[i] + flattened;
...
public:
  ...
  expr_c visit(indexing_c v) override {
    ...
    if (v->ptr_.isa<tensor>()) {
      ...
      if (is_2d) {
        ...
        if (changed) {
          return copy_attr(*v,
                           builder::make_indexing(ptr, newidx, v->dtype_.lanes_,
                                                  v->mask_, v->dtype_.rows_));
        } else {
          return std::move(v);
        }
      }
      ...
      if (!oldptr->is_slice_ && oldptr->base_->ptr_.isa<tensorptr>() &&
        ...
        auto old_parent_ptr = oldptr->base_->ptr_.static_as<tensorptr_c>();
        ...
        COMPILE_ASSERT(shape->size() == v->idx_.size(),
                       "Unmatched dimensions of indexing: "
                           << v << ", shape:" << utils::print_vector(*shape));
    ...
    if (v->dims_.size() == 1 && v->strides_[0].isa<constant>() &&
      ...
      for (size_t i = 0; i < v->dims_.size(); ++i) {
        range = builder::make_add(
            builder::make_mul(v->dims_[i] - 1, v->strides_[i]), range);
      }
      // here stride is {1} since all strided info are compressed into
      // dims
      auto ret =
          copy_attr(*v, builder::make_stensor(
                            v->name_, {range}, std::vector<expr>{UINT64_C(1)},
  ...
  expr_c visit(tensorptr_c v) override {
    ...
    if (base.ptr_same(v->base_) && v->shape_.empty()) {
      return v;
    }
    return copy_attr(*v, make_expr<tensorptr_node>(
