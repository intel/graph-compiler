...
int64_t get_const_as_int(const constant_c &c) {
  ...
  case CATE_INT:
    return c->value_[0].s64;
    break;
  case CATE_UINT:
    return c->value_[0].u64;
    break;
  default:
...
static void get_constant_from_for_loop(for_loop_node_t *_loop, int64_t &min,
  ...
  if (check_step) {
    COMPILE_ASSERT(step == 1, "for-loop-transforms only support step=1: "
                                  << _loop->node_ptr_from_this());
  }
...
public:
  ...
  void transform_shrink_info(const expr &v) {
    if (v->attr_ && v->attr_->has_key(tensor_shrinker_attrs::should_shrink)) {
...
bool for_loop_node_t::isvalid() const { return var_.defined(); }

for_loop make_const_for(var v, int64_t min, int64_t max, stmt &&body_) {
  auto ret = make_stmt<for_loop_node_t>(
      std::move(v), make_expr<constant_node>(min, v->dtype_),
      make_expr<constant_node>(max, v->dtype_),
      make_expr<constant_node>(int64_t(1), v->dtype_), std::move(body_), true,
      for_type::NORMAL);
  // set parent node
...
void for_loop_node_t::unroll(uint64_t factor, const stmt &parent) {
  ...
  if (is_const) {
    ...
    if (factor == 0) {
      factor = utils::divide_and_ceil(loop_len, step);
    }
    has_remainder =
        loop_len % (factor * step) != 0 || loop_len < ((int64_t)factor * step);
  }
  // make new variables
  ...
  if (!remove) {
    ...
    replace_value = make_expr<constant_node>(
        get_const_as_int(iter_begin_.static_as<constant>()), oldvar->dtype_);
  }
  ...
  if (!remove) {
    ...
    if (has_remainder) {
      ...
      // var
      expr begins = is_const ? (c_newloop_len * (step * factor) + min)
                             : (newloop_len * (step_ * factor) + iter_begin_);
      auto remainder = builder::make_for_loop_unattached(
          oldvar, begins, iter_end_, step_, body_, incremental_,
      ...
      auto if_check = builder::make_if_else_unattached(
          newvar < newloop_len, std::move(seq), std::move(seq_remainder));
      seq = make_stmt<stmts_node_t>(std::vector<stmt>{std::move(if_check)});
    }
    ...
    pseq.insert(pseq.begin() + loop_idx, seq->seq_.begin(), seq->seq_.end());
  }
}
...
static for_loop build_inner_for(for_loop_node_t *ths, int64_t min,
  ...
  expr remapped =
      ths->var_ * make_expr<constant_node>(block, ths->var_->dtype_) + vin;
  if (min != 0) {
...
for_loop for_loop_node_t::split(int64_t block, node_ptr_map *node_remap) {
  ...
  int64_t loop_len = max - min;
  COMPILE_ASSERT(
      loop_len % block == 0 && loop_len >= block,
      "The loop length "
          << loop_len
...
for_loop for_loop_node_t::split_on_num_threads(int64_t num_groups,
                                               node_ptr_map *node_remap) {
  COMPILE_ASSERT(isvalid(), "Transforming an invalid for-loop");
  int64_t min, max, step;
  get_constant_from_for_loop(this, min, max, step);
  COMPILE_ASSERT(min == 0 && step == 1, "Only support begin is 0 and step is 1")
...
for_loop get_inner_for_loop(const for_loop_node_t *f) {
  ...
  if (cur->body_.isa<stmts>()) {
    ...
    if (stmtlist->seq_.size() == 1 && stmtlist->seq_.at(0).isa<for_loop>()) {
...
for_loop for_loop_node_t::fuse(const for_loop &ax, node_ptr_map *node_remap) {
  ...
  auto get_expr_from_for_loop = [](for_loop_node_t *_loop, expr &min, expr &max,
                                   expr &step) {
  ...
  // make new variables
  var vout = make_expr<var_node>(
      var_->dtype_, std::string("fused_0") + var1->name_ + "__" + var2->name_ +
                        "_" + std::to_string(fuse_count++));
...
void for_loop_node_t::reorder(stmt parent, std::vector<for_loop> &&ax) {
  ...
  for (unsigned i = 0; i < ax.size(); i++) {
    COMPILE_ASSERT(cur.defined(), "Bad number of axises to reorder. Got "
                                      << ax.size()
                                      << " to reorder, but only have " << i
                                      << " nested for-loops");
    COMPILE_ASSERT(cur->isvalid(), "Transforming an invalid for-loop");
    inner_body = cur->body_;
    COMPILE_ASSERT(
        std::find_if(ax.begin(), ax.end(),
                     [cur](for_loop &v) { return v.ptr_same(cur); }) !=
            ax.end(),
        "Cannot find axis " << cur->var_ << " in the given axises to reorder");
...
static void find_ths_and_ax_then_remove(for_loop_node_t *ths,
                                        const stmt &parent,
                                        for_loop_node_t *ax) {
...
static void do_merge(for_loop_node_t *ths, const stmt &parent,
                     const for_loop &ax) {
...
for_loop for_loop_node_t::merge(stmt parent, for_loop ax, unsigned num_inner) {
  ...
  for (unsigned i = 0; i < num_inner; i++) {
    COMPILE_ASSERT(ths && axis.defined(),
                   "Merging " << num_inner << " inner loops, but have only "
                              << i << " loops in the IR");
...
int for_loop_node_t::merge_all(stmt parent, for_loop ax) {
  ...
  for (;;) {
    ...
    if (!is_loop_range_same(ths, axis.get())) {
      break;
    }
    // check binding axis
...
void for_loop_node_t::parallel_merge(const stmt &parent, const for_loop &ax) {
  ...
  body2->seq_.insert(
      ...
      builder::make_var_tensor_def_unattached(
          ax->var_, linkage::local, var_ - iter_end_ + ax->iter_begin_));
  auto if_else =
...
std::vector<for_loop> collect_loops(const stmt &body) {
  ...
  if (body.isa<stmts>()) {
    ...
    if (body.isa<for_loop>()) {
      ret.push_back(body.static_as<for_loop>());
    }
  }
  return ret;
}
...
static size_t collect_all_loops_helper(std::vector<for_loop> &ret,
                                       const stmt &body) {
  size_t collected = 0;
  if (body.isa<stmts>()) {
    for (auto &smt : body.static_as<stmts>()->seq_) {
...
struct loop_normalize_visitor_t : public ir_consistent_visitor_t {
  ...
  stmt_c visit(for_loop_c v) override {
    ...
    if (!is_normalized) {
      ...
      auto new_end =
          divide_and_ceil((end - begin).remove_const(), step.remove_const());
      auto new_step = builder::make_constant({UINT64_C(1)}, step->dtype_);
      auto new_itr = copy_attr(
          *var, builder::make_var(var->dtype_, var.static_as<var_c>()->name_));
      ...
      return copy_attr(*v, builder::make_for_loop_unattached(
                               new_itr, new_beg, new_end, new_step, body,
    ...
    if (changed) {
      stmt_c newv =
          copy_attr(*v, builder::make_for_loop_unattached(
