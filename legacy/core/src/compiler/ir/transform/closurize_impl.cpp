...
closurize_impl_t::closurize_impl_t(const std::vector<define> &globals,
                                   ir_module_ptr modu)
    : modu_(std::move(modu)) {
  for (auto &g : globals) {
...
expr_c closurize_impl_t::create_or_get_tensor_or_var(expr_c v) {
  expr_c newv;
  if (defined_set_.find(v) != defined_set_.end()) {
    return v;
  }
  if (globals_set_.find(v) != globals_set_.end()) {
    return v;
  }
...
stmt_c closurize_impl_t::visit(define_c v) {
  if (in_parallel_for && (v->var_.isa<var>() || v->var_.isa<tensor>())) {
    ...
    if (v->var_.isa<tensor>()) {
      ...
      if (!r.ptr_same(v->var_)) {
        captures_set_.insert({v->var_, r});
      }
    }
...
expr_c closurize_impl_t::visit(tensor_c v) {
  if (in_parallel_for) {
    return create_or_get_tensor_or_var(v);
  }
  return v;
}
