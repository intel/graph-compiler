...
public:
  ...
  stmt_c visit(define_c v) override {
    ...
    if (!cur.empty()) {
      if (var0.isa<var>()) {
        ...
        new_var = ir_visitor_t::visit(var0.static_as<tensor>()).remove_const();
    ...
    if (changed) {
      return copy_attr(*ret, builder::make_var_tensor_def_unattached(
  ...
  stmt_c visit(stmts_c v) override {
    bool parent_is_stmts = (cur.size() > 1 && cur[cur.size() - 2].isa<stmts>());
    // if the current seq is empty and parent is stmt, return null
    if (v->seq_.empty() && parent_is_stmts) {
      return stmt_c();
    }
    ...
    pnewseq = parent_seq;
    bool preserve =
        v->attr_ && v->attr_->get_or_else(attr_keys::preserve_stmts, false);
...
public:
  ...
  stmt_c visit(if_else_c v) override {
    ...
    if (v->else_case_.defined()) {
      ...
      else_is_empty_stmts =
          else_case.isa<stmts>() && else_case.checked_as<stmts>()->seq_.empty();
    }
    ...
    if (condition.isa<constant>() && get_expr_as_int(condition) == 0) {
      // similar to always FALSE
      return else_case.defined() ? else_case
     ...
     * }
     * */
    if (then_case.isa<stmts>() && then_case.static_as<stmts>()->seq_.empty()) {
      if (!else_case.defined() || else_is_empty_stmts) {
        return copy_attr(*v, make_stmt<stmts_node_t>(std::vector<stmt>{}));
      } else {
        return copy_attr(
            *v, builder::make_if_else_unattached(
                    constant_folder_t()(!condition), else_case, stmt()));
      }
    }
    /** simplify empty else
     * if(conditon){
    ...
    if (else_is_empty_stmts) {
      return copy_attr(
          *v, builder::make_if_else_unattached(condition, then_case, stmt()));
    }
    ...
    if (changed_) {
      return copy_attr(*v, builder::make_if_else_unattached(
                               condition, then_case, else_case));
    }
    return std::move(v);
  }

  stmt_c dispatch_loop_body_replace_loop_var(const expr_c &var,
                                             const expr_c &begin,
                                             const stmt &body) {
  ...
  stmt_c visit(for_loop_c v) override {
    // eliminate for(...){}
    if (v->body_.isa<stmts>() && v->body_.static_as<stmts>()->seq_.empty()) {
      return copy_attr(*v, make_stmt<stmts_node_t>(std::vector<stmt>{}));
    }
    ...
    if (v->attr_) {
      int64_t loop_len = v->attr_->get_or_else("loop_len_hint", INT64_C(-1));
      if (loop_len >= 0) {
        if (loop_len == 0) {
          return copy_attr(*v, make_stmt<stmts_node_t>(std::vector<stmt>{}));
        } else if (loop_len == 1) {
          return dispatch_loop_body_replace_loop_var(var, begin, v->body_);
        }
      }
    }

    is_loop_merge |=
        (v->attr_ && v->attr_->get_or_else(stmt_attr_key::merge_loop, false));

    if (begin.isa<constant>() && end.isa<constant>() && step.isa<constant>()) {
      // begin > end
      ...
      if ((get_expr_as_int(begin) + get_expr_as_int(step)) >=
          get_expr_as_int(end)) {
        auto body = dispatch_loop_body_replace_loop_var(var, begin, v->body_);
        is_loop_merge = cached_loop_merge;
        return body;
      }
    }
    ...
    if (changed || (is_loop_merge &&
                    (!v->attr_ ||
                     is_loop_merge != v->attr_->get_or_else(
                                          stmt_attr_key::merge_loop, false)))) {
      auto loop = copy_attr(*v, make_stmt<for_loop_node_t>(
                                    var.remove_const(), begin.remove_const(),
                                    end.remove_const(), step.remove_const(),
                                    body.remove_const(), v->incremental_,
