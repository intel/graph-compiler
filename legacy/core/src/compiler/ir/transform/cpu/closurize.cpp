...
SC_DECL_PASS_INFO(closurizer_cpu,
                  SC_PASS_DEPENDS_ON(nested_parallel_flattener,
                                     parallel_workload_dispatcher, validator,
                                     trace_inserter, parallel_merge,
                                     tensor_init),
...
func_t get_parallel_call_with_env_func(bool managed) {
  static func_t f = builder::_decl_func(
      "sc_parallel_call_cpu_with_env", datatypes::void_t,
      {_arg_("func", datatypes::pointer), _arg_("flags", datatypes::index),
       _arg_("stream", datatypes::pointer),
       _arg_("env", datatypes::s8.get_pointerof()),
       _arg_("begin", datatypes::index), _arg_("end", datatypes::index),
       _arg_("step", datatypes::index),
       _arg_("args", datatypes::generic.get_pointerof())});
  static func_t f_managed = builder::_decl_func(
      "sc_parallel_call_managed", datatypes::void_t,
      {_arg_("func", datatypes::pointer), _arg_("flags", datatypes::index),
       _arg_("stream", datatypes::pointer),
       _arg_("env", datatypes::s8.get_pointerof()),
       _arg_("begin", datatypes::index), _arg_("end", datatypes::index),
...
public:
  ...
  static expr_c find_base_tensor(const expr_c &v,
                                 const tensor_def_collector_t::result_t &scope,
                                 bool &out_is_arg) {
    expr_c cur = v;
    for (;;) {
  ...
  void link_call_args(const call &thecall, const func_t &callee) {
    for (size_t i = 0; i < callee->params_.size(); i++) {
      ...
      if (param.isa<tensor>()) {
        result_[param] = tensor_def_info_t{get_base_tensor_of(arg), false};
      }
    }
  }
};
...
class closurize_cpu_impl_t : public closurize_impl_t {
  ...
  func_t make_closure_func(
    ...
    COMPILE_ASSERT(para_attr.size() == 1,
                   "CPU does not support grouped parallel");
    func_t closure = builder::make_func(name, params, body, datatypes::void_t);
    ...
    expr args = builder::make_tensor("args", {(int)params.size() - 1},
                                     datatypes::generic);
    func_t ret =
        builder::make_func(name + "_0wrapper", std::vector<expr>{itervar, args},
                           seq, datatypes::void_t);
    ...
    for (uint64_t idx = 1; idx < params.size(); idx++) {
      ...
      if (param->dtype_ == datatypes::generic) {
        ...
        fargs.emplace_back(builder::make_cast(param->dtype_, args[idx - 1]));
      }
    }
  ...
  stmt make_parallel_call(
      func_t target, std::vector<expr> &captures,
    ...
    auto argsbuf = builder::make_tensor(argname, {captures.size() - 1},
                                        datatypes::generic);
    seq->seq_.emplace_back(builder::make_var_tensor_def_unattached(argsbuf));
    ...
    auto ret_call = builder::make_call(
        ...
        std::vector<expr_c>{
            builder::make_func_addr(std::move(target)),
...
public:
  ...
  optional<std::vector<expr_c>>
  collect_captured_base_tensors(const std::vector<expr> &captured) {
  ...
  func_c dispatch(func_c f) override {
    ...
    if (!out_calls.empty() && f->attr_ &&
        f->attr_->get_or_else(function_attrs::has_idle_func, false)) {
    ...
    if (f == the_last_op_ && !out_calls.empty()) {
      ...
      for (auto itr = seq.rbegin(); itr != seq.rend(); ++itr) {
        ...
        if (s.cast<returns>()
                .filter(
                    [](const returns &v) { return v->value_.isa<constant>(); })
                .has_value()) {
          continue;
        }
        ...
        if (s.cast<for_loop>()
          ...
          for (auto &base : captured_base.get()) {
            base.remove_const()->attr()[attr_keys::runtime_stack_alloc] = true;
          }
          auto &the_flag = get_last_parallel_call_flag();
          ...
          auto closure_arg =
              out_calls.back()->args_.at(parallel_call_args::ARGS).as<tensor>();
        ...
        SC_MODULE_WARN << "Cannot optimize the last barrier in function "
                       << f->name_
  ...
  closurize_cpu_impl_t(
      const ir_module_ptr &m, bool use_managed_thread_pool,
      ...
      : closurize_impl_t(m->get_module_vars(), m),
        ...
        the_last_op_{the_last_op}, main_tensor_def_info_{&main_tensor_def_info},
  ...
  stmt_c visit(for_loop_c v) override {
    ...
    if (ret->kind_ == for_type::PARALLEL) {
      if (ret.get() != v.get()) {
        ...
        auto retnode = copy_attr(*v, builder::make_for_loop_unattached(
                                         ret->var_, ret->iter_begin_,
                                         ret->iter_end_, ret->step_, ret->body_,
                                         ret->incremental_, for_type::NORMAL));
...
const_ir_module_ptr closurizer_cpu_t::operator()(const_ir_module_ptr inmod) {
  float gflop = inmod->attr_.get_or_else(ir_module_t::attr_key_t::GFLOP, 0.0f);
  auto tp_mod = inmod->attr_.get_or_else(
      ir_module_t::attr_key_t::MANAGED_THREAD_POOL, thread_pool_mode_t::DIRECT);
  ...
  if (!single_core_ && use_managed_thread_pool) {
    ...
    if (main_entry_func && main_entry_func->attr_ &&
        main_entry_func->attr_->get_or_else(function_attrs::is_main, false)) {
      ...
      for (auto itr = seq.rbegin(); itr != seq.rend(); ++itr) {
        ...
        if (f.has_value()) {
          ...
          if (the_last_op) {
            ...
            auto last_call = s.static_as<evaluate>()->value_.static_as<call>();
        ...
        // else, there are some complex statements after the last
        // parallel-for, do not optimize
        SC_MODULE_WARN << "Cannot optimize the last barrier in main function "
  ...
  for (unsigned i = 0; i < sz; i++) {
    auto f = std::const_pointer_cast<func_base>(the_pass->dispatch(funcs[i]));
    funcs[i] = std::move(f);
  }
