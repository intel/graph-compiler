...
namespace std {
template <> struct hash<dnnl::impl::graph::gc::brgemm::postop_setting_t> {
  std::size_t
  operator()(const dnnl::impl::graph::gc::brgemm::postop_setting_t &k) const {
    size_t ret = 0;
...
struct hash<std::pair<dnnl::impl::graph::gc::sc_brgemm_bd_mask_t, int>> {
  std::size_t
  operator()(const std::pair<dnnl::impl::graph::gc::sc_brgemm_bd_mask_t, int>
                 &bdmask_pair) const {
    size_t ret = 0;
...
static bool check_arg_range(const std::vector<expr> &args,
                            std::vector<expr_c> &cached_args, int start,
                            int end) {
...
static expr
get_brgemm_attrs_arg(const ir_module_ptr &mod, const sc_brgemm_attrs_t &attrs,
  ...
  int c = 0;
  for (auto &it : attrs) {
    if (it.first < brgemm::attr_key::nkeys) {
      setting->map_[c++] = it;
    }
  }
  ...
  mod->add_global_var(
      builder::make_var_tensor_def_unattached(tsr, linkage::private_global)
          .static_as<define>());
  cache[key] = tsr;
  return tsr;
}

static std::pair<expr, expr> get_brgemm_bd_mask_arg(
    const ir_module_ptr &mod, const sc_brgemm_bd_mask_t &bd_mask,
    const expr &bd_mask_idx, const expr &bd_mask_len, const int bd_mask_set_num,
  ...
  if (sz == 0) {
    return std::pair<expr, expr>(get_ir_null(), get_ir_null());
  }
  ...
  if (full_cached.find(bd_mask) == full_cached.end()) {
    ...
    expr full_bd_mask_ =
        builder::make_tensor("__brgemm_full_bd_mask", {tsr_sz}, datatypes::u8,
                             address_space::automatic, init);
  ...
  expr bd_mask_arr =
      mod->make_global_tensor(datatypes::pointer, "__brgemm_bd_mask_arr",
                              {bd_mask_set_num}, linkage::private_global);
...
static expr get_brgemm_postops_setting_arg(
    const ir_module_ptr &mod, const sc_brgemm_postops_setting_t &postops,
    std::unordered_map<sc_brgemm_postops_setting_t, expr> &cache) {
  if (postops.empty()) {
    return get_ir_null();
  }
  if (cache.find(postops) != cache.end()) {
    return cache[postops];
  }
  ...
  auto init = std::make_shared<static_data_t>(data.data(), tsr_sz);
  expr tsr =
      builder::make_tensor("__brgemm_postops_setting", {tsr_sz}, datatypes::u8,
                           address_space::automatic, init);
  mod->add_global_var(
      builder::make_var_tensor_def_unattached(tsr, linkage::private_global)
          .static_as<define>());
...
// has post ops
static expr get_brgemm_postops_data_arg(std::vector<stmt_c> &ret,
                                        const std::vector<expr> &ops_data) {
  assert(ops_data.size() == brgemm::postops_data_init_func_nargs);
  auto &in_bufs = ops_data;
  auto postop_data = builder::make_tensor("__brgemm_postops_data",
                                          {postops_data_size}, datatypes::u8);

  expr bin_ptr = in_bufs[2];
  if (!in_bufs[2]->equals(get_ir_null())) {
    bin_ptr = builder::make_tensor("__binary_rhs_ptr", {1}, datatypes::pointer);
    ret.emplace_back(builder::make_var_tensor_def_unattached(bin_ptr));
    ret.back().remove_const()->attr().set(attr_keys::tsr_dont_buf_sched, true);
    ret.emplace_back(
        builder::make_assign_unattached(bin_ptr[UINT64_C(0)], in_bufs[2]));
  }
  ret.emplace_back(builder::make_var_tensor_def_unattached(postop_data));
  ret.emplace_back(builder::make_evaluate_unattached(builder::make_call(
      builtin::get_brgemm_postops_data_init_func(),
      {postop_data, in_bufs[0], in_bufs[1], bin_ptr, in_bufs[3], in_bufs[4],
       in_bufs[5], in_bufs[6], in_bufs[7], in_bufs[8], in_bufs[9], in_bufs[10],
       in_bufs[11], in_bufs[12], in_bufs[13]})));
...
static expr brgemm_init_kernel_cache(brgemm_mode mode,
  ...
  if (mode == brgemm_mode::stride) {
    ...
    const int expected_cache_extra_start = brgemm_args::NUM_FULL_ARGS_STRIDE;
    const int expected_cache_extra_args =
        brgemm_args::extra_args_offset::cache_nargs;
    // +1 for context pointer
    assert(args.size() == brgemm_args::NUM_FULL_ARGS_STRIDE +
    ...
    if (!check_arg_range(args, cached_args, expected_cache_extra_start,
                         expected_cache_extra_start +
                             expected_cache_extra_args)) {
      return expr();
    }
    return get_brgemm_creator_and_call_func(mode, backend, has_postop)
        .first(
            args[brgemm_args::M], args[brgemm_args::N], args[brgemm_args::K],
            args[brgemm_args::LDA], args[brgemm_args::LDB],
            args[brgemm_args::LDC], args[brgemm_args::STRIDE_A],
            args[brgemm_args::STRIDE_B], beta,
            args[brgemm_args::NUM_FULL_ARGS_STRIDE +
                 brgemm_args::extra_args_offset::dtypeA], // dtypeA
            args[brgemm_args::NUM_FULL_ARGS_STRIDE +
                 brgemm_args::extra_args_offset::dtypeB], // dtypeB
            args[brgemm_args::NUM_FULL_ARGS_STRIDE +
                 brgemm_args::extra_args_offset::brg_attrs], // attrs
            args[brgemm_args::NUM_FULL_ARGS_STRIDE +
                 brgemm_args::extra_args_offset::bd_mask], // bd_mask
            args[brgemm_args::NUM_FULL_ARGS_STRIDE +
                 brgemm_args::extra_args_offset::postops_setting]); // postops
                                                                    // set
  } else {
    ...
    const int expected_cache_extra_args =
        brgemm_args::extra_args_offset::cache_nargs;
    // +1 for context pointer
    assert(args.size() == brgemm_args::NUM_FULL_ARGS_LIST +
                              brgemm_args::extra_args_offset::nargs + 1);
    if (!check_arg_range(args, cached_args, brgemm_args::M,
                         expected_cache_org_args)) {
      return expr();
...
static expr brgemm_run(brgemm_mode mode, scflags_t::brgemm_backend_t backend,
  ...
  if (mode == brgemm_mode::stride) {
    ...
    auto run_func =
        get_brgemm_creator_and_call_func(mode, backend, has_postop).second;
    if (has_postop) {
    ...
    const int expected_num_args =
        brgemm_args::NUM_FULL_ARGS_LIST + brgemm_args::extra_args_offset::nargs;
    assert(args.size() == expected_num_args + 1);
    auto run_func =
        get_brgemm_creator_and_call_func(mode, backend, has_postop).second;
    if (has_postop) {
...
public:
  ...
  std::unordered_map<std::pair<sc_brgemm_bd_mask_t, int>, std::pair<expr, expr>>
      cur_bd_mask_cache_;
  ...
  typedef expr (*init_func_t)(brgemm_mode mode,
                              scflags_t::brgemm_backend_t backend,
                              const std::vector<expr> &args,
                              std::vector<expr_c> &cached_args, float beta,
                              bool has_postop);
  using run_func_t = expr (*)(brgemm_mode, scflags_t::brgemm_backend_t,
                              const expr &, const std::vector<expr> &,
                              bool has_postop);

  expr_c optimize_range_kernel_call(
      brgemm_mode mode, scflags_t::brgemm_backend_t backend, expr_c v,
    ...
    if ((M_iter == brg_attrs.end() && !args[brgemm_args::M].isa<constant>()) ||
        (N_iter == brg_attrs.end() && !args[brgemm_args::N].isa<constant>()) ||
        (K_iter == brg_attrs.end() && !args[brgemm_args::K].isa<constant>())) {
      SC_MODULE_INFO << "Cannot optimize the range kernel call: " << v;
      return v;
    }
    ...
    auto get_tail_value = [](const sc_brgemm_attrs_t &m,
    ...
    int M_upper_bound = static_cast<int>(
        is_M_range ? M_iter->second : get_expr_as_int(args[brgemm_args::M]));
    int N_upper_bound = static_cast<int>(
        is_N_range ? N_iter->second : get_expr_as_int(args[brgemm_args::N]));
    int K_upper_bound = static_cast<int>(
        is_K_range ? K_iter->second : get_expr_as_int(args[brgemm_args::K]));
    const int expected_cache_org_args = brgemm_args::NUM_BASIC_ARGS_STRIDE;
    const int expected_cache_extra_start =
        mode == brgemm_mode::stride ? brgemm_args::NUM_FULL_ARGS_STRIDE
    ...
    int stride_a =
        static_cast<int>(get_expr_as_int(args[brgemm_args::STRIDE_A]));
    int stride_b =
        static_cast<int>(get_expr_as_int(args[brgemm_args::STRIDE_B]));
    ...
    auto handle = (mode == brgemm_mode::stride
                       ? std::make_shared<brg_range_handle_t>(
                             M_upper_bound, N_upper_bound, K_upper_bound, LDA,
                             LDB, LDC, stride_a, stride_b, beta, dtypeA, dtypeB,
                             attr_ptr, M_tail_value, N_tail_value, K_tail_value)
                       : std::make_shared<brg_range_handle_t>(
                             M_upper_bound, N_upper_bound, K_upper_bound, LDA,
                             LDB, LDC, beta, dtypeA, dtypeB, attr_ptr,
                             M_tail_value, N_tail_value, K_tail_value));
    ...
    auto handlev =
        mod_->make_global_var(datatypes::pointer, "__sc_range_kernel_cache",
  ...
  expr_c optimize_kernel_call(brgemm_mode mode,
                              ...
                              const std::vector<expr> &args,
                              const std::string &name, init_func_t init_func,
                              run_func_t run_func, float beta, bool has_postop,
                              bool use_bdmask) {
    std::vector<expr_c> cached_args;
    ...
    if (use_bdmask) {
      ...
      expr cachev =
          init_func(mode, backend, args, cached_args, beta, has_postop);
      if (!cachev.defined()) {
      ...
      if (first_itr != kernel_cache.end()) {
        ...
        if (second_itr != entry.end()) {
          ...
          return run_func(mode, backend, second_itr->second[bd_mask_idx], args,
                          has_postop);
        }
      }

      expr sc_kernel_cache_arr = mod_->make_global_tensor(
          datatypes::pointer, "__sc_kernel_cache_arr", {brg_bdmask_set_num_},
      ...
      for (int i = 0; i < brg_bdmask_set_num_; ++i) {
        ...
        expr cachev = init_func(mode, backend, brg_args, cached_args_iter, beta,
                                has_postop);
        ...
        kernel_cache_assignment.emplace_back(builder::make_assign_unattached(
            builder::make_indexing(sc_kernel_cache_arr, {i}), cachev));
      }
      ...
      if (first_itr != kernel_cache.end()) {
        ...
        if (second_itr != entry.end()) {
          ...
          return run_func(mode, backend, second_itr->second, args, has_postop);
        }
      }
      // Make kernel pointer global var. will be auto-renamed
  ...
  expr_c visit(intrin_call_c v) override {
    ...
    sc_brgemm_postops_setting_t &brg_postops_setting = extras->postops_setting_;
    ...
    COMPILE_ASSERT(brg_postops_data.size() ==
                       brgemm::postops_data_init_func_nargs,
    ...
    validate_brgemm_attrs(brg_attrs, bd_mask, brg_bdmask_set_num_, use_bdmask);
    ...
    expr brg_attrs_arg =
        get_brgemm_attrs_arg(mod_, brg_attrs, attrs_cache_, try_range_optimize);
    ...
    if (use_bdmask) {
      ...
      auto bd_mask_arg = get_brgemm_bd_mask_arg(
          mod_, bd_mask, bd_mask_idx, bd_mask_len, brg_bdmask_set_num_,
          cur_bd_mask_cache_, full_bd_mask_cache_, brg_bd_mask_arr_assignment_);
    ...
    if (brg_c_buf->equals(get_ir_null())) {
      if (!brg_postops_setting.empty()) {
        brg_c_buf = builder::make_tensor(
            "__brgemm_c_buf",
            {opt_args[brgemm_args::M] * opt_args[brgemm_args::N]},
            infer_out_dtype(dtypeA));
        ret.emplace_back(builder::make_var_tensor_def_unattached(brg_c_buf));
      }
    }
    ...
    std::vector<expr> no_opt_args(opt_args.begin(),
    ...
    scflags_t::brgemm_backend_t backend = mod_->ctx_->flags_.brgemm_backend_;
    ...
    if (!optimized) {
      ...
      if (try_range_optimize) {
        auto ret = optimize_range_kernel_call(
            mode, backend, v, opt_args, f->name_, range_brgemm_run, brg_attrs,
            extras->cpu_.init_ ? 0.0f : 1.0f, !brg_postops_setting.empty(),
            use_bdmask);
        if (!ret.ptr_same(v)) {
          return ret;
        }
      ...
      auto ret = optimize_kernel_call(mode, backend, v, opt_args, f->name_,
                                      brgemm_init_kernel_cache, brgemm_run,
  ...
  stmt_c visit(stmts_c v) override {
    ...
    for (auto &st : v->seq_) {
      ...
      if (new_st.isa<evaluate>() &&
        ...
        seq.insert(seq.end(), brg_postop_init_.begin(), brg_postop_init_.end());
        changed = true;
      }
      ...
      if (pvar->var_.isa<var>()) {
        ...
        if (name.find("kernel_cache") != std::string::npos) {
          initf->body_.checked_as<stmts>()->seq_.emplace_back(
              builder::make_assign_unattached(pvar->var_, pvar->init_));
        }
      }
    }
  } else {
