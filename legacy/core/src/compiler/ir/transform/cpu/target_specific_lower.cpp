...
static std::vector<expr> make_args_by_intrinsic(const intrin_call_c &node) {
  ...
  for (auto &v : node->args_) {
    ret.emplace_back(
        builder::make_var(v->dtype_, "_intrin_v" + std::to_string(idx)));
    idx += 1;
  }
  return ret;
}
...
static expr_c create_cast_bf16_to_f32(const context_ptr &ctx, const cast_c &v) {
  ...
  auto &in = v->in_;
  const auto count =
      make_expr<constant_node>(16UL, sc_data_type_t::u32(in->dtype_.lanes_));

  auto uint32_v = builder::make_cast(
      sc_data_type_t::u32(in->dtype_.lanes_),
      builder::make_reinterpret(in, sc_data_type_t::u16(in->dtype_.lanes_)));
  auto tmpf32 = copy_attr(
      *v, builder::make_reinterpret(uint32_v << count,
                                    sc_data_type_t::f32(v->dtype_.lanes_)));
...
static expr_c create_cast_f32_to_bf16(const context_ptr &ctx, const cast_c &v) {
  ...
  if (ctx->flags_.bf16_fast_trunc_) {
    auto uint32_v =
        builder::make_reinterpret(in, sc_data_type_t::u32(in->dtype_.lanes_));
    return copy_attr(
        *v, builder::make_reinterpret(
                builder::make_cast(sc_data_type_t::u16(in->dtype_.lanes_),
                                   uint32_v >> count),
    ...
    auto rounding_bias =
        builder::make_int_and(
            ...
            builder::make_constant(
                std::vector<union_val>(in->dtype_.lanes_, (int64_t)1),
                sc_data_type_t::u32(in->dtype_.lanes_))) +
        builder::make_constant(
            std::vector<union_val>(in->dtype_.lanes_, (int64_t)0x7FFF),
            sc_data_type_t::u32(in->dtype_.lanes_));
...
static expr_c create_cast_fp16_to_f32(const context_ptr &ctx, const cast_c &v) {
  if (ctx->machine_.device_type_ == runtime::target_machine_t::type::cpu &&
    ...
    auto uint32_v = builder::make_cast(
        u32_dtype, builder::make_reinterpret(in, sc_data_type_t::u16(lanes)));
    ...
    auto m = (builder::make_int_and(uint32_v,
                                    gen_vec_const_u32(lanes, UINT64_C(0X03FF))))
             << gen_vec_const_u32(lanes, UINT64_C(13));
    // const uint32_t v = as_uint((float)m) >> 23;
    auto l =
        (builder::make_reinterpret(
            builder::make_cast(sc_data_type_t::f32(lanes), m), u32_dtype)) >>
        gen_vec_const_u32(lanes, UINT64_C(23));

    // (x & 0x8000) << 16
    auto cond1 = (builder::make_int_and(
                     uint32_v, gen_vec_const_u32(lanes, UINT64_C(0x8000))))
                 << gen_vec_const_u32(lanes, UINT64_C(16));
    // (e != 0) *  ((e + 112) << 23 | m)
    auto tmp_cond_2 =
        builder::make_int_or((e + gen_vec_const_u32(lanes, UINT64_C(112)))
                                 << gen_vec_const_u32(lanes, UINT64_C(23)),
                             m);
    auto cond2 =
        builder::make_select(e != gen_vec_const_u32(lanes, UINT64_C(0)),
                             tmp_cond_2, gen_vec_const_u32(lanes, UINT64_C(0)));
    ...
    */
    ...
    auto tmpf32 = copy_attr(
        *v, builder::make_reinterpret(f32_v, sc_data_type_t::f32(lanes)));
    if (!v->dtype_.is_etype(sc_data_etype::F32)) {
      tmpf32 = builder::make_cast(v->dtype_, tmpf32);
    }
    return tmpf32;
  ...
  if (ctx->machine_.device_type_ == runtime::target_machine_t::type::cpu &&
    ...
    auto tmp_cond_2 = builder::make_int_and(
        (e - gen_vec_const_u32(lanes, UINT64_C(112)))
            << gen_vec_const_u32(lanes, UINT64_C(10)),
        builder::make_constant(
            std::vector<union_val>(in->dtype_.lanes_, (int64_t)0x7C00),
            sc_data_type_t::u32(in->dtype_.lanes_)));
    ...
    // 1)
    auto tmp_cond_3 = (((gen_vec_const_u32(lanes, UINT64_C(0x007FF000)) + m) >>
    ...
    auto cond3 = builder::make_select(
        ...
        builder::make_select(e > gen_vec_const_u32(lanes, UINT64_C(101)),
                             tmp_cond_3, gen_vec_const_u32(lanes, UINT64_C(0))),
        gen_vec_const_u32(lanes, UINT64_C(0)));
    // (e > 143) * 0x7FFF
    auto cond4 =
        builder::make_select(e > gen_vec_const_u32(lanes, UINT64_C(143)),
    ...
    */
    auto fp16_v = builder::make_int_or(
        builder::make_int_or(builder::make_int_or(cond1, cond2), cond3), cond4);
    return copy_attr(
        *v,
        builder::make_reinterpret(
            builder::make_cast(sc_data_type_t::u16(in->dtype_.lanes_), fp16_v),
...
static func_t create_isnan_func(const ir_module_ptr &mod,
                                const intrin_call_c &node,
                                target_specific_lower_cpu_impl_t *visitor) {
  auto type = node->dtype_;
  uint32_t elements = type.lanes_;
  auto ZERO =
      make_expr<constant_node>(INT64_C(0), sc_data_type_t::s32(elements));
  auto exponent_bits = make_expr<constant_node>(INT64_C(0x7F800000),
                                                sc_data_type_t::s32(elements));
  auto rm_sign_bits = make_expr<constant_node>(INT64_C(0x7FFFFFFF),
                                               sc_data_type_t::s32(elements));
...
static func_t create_exp_func(const ir_module_ptr &mod,
  ...
  auto ONE_i =
      make_expr<constant_node>(INT64_C(1), sc_data_type_t::s32(elements));
  auto ty_epi_32 = sc_data_type_t::s32(elements);
  auto overflow_x = gen_vec_const(elements, 88.72283935f);
  auto underflow_x = gen_vec_const(elements, -87.33654785f);
  auto ret_infinity =
      make_expr<constant_node>(std::numeric_limits<float>::infinity(), type);
  ...
  _function_(type, the_exp_func, make_args_by_intrinsic(node)) {
    ...
    k_float = builder::make_floor(
        a_ * one_over_ln2 + half_float); // k_float = floor(x / ln2 + 0.5f)
    ...
    auto const_23 =
        make_expr<constant_node>(INT64_C(23), sc_data_type_t::s32(elements));
...
static func_t
create_read_struct_func(const ir_module_ptr &mod, const intrin_call_c &node,
                        target_specific_lower_cpu_impl_t *visitor) {
  builder::ir_builder_t builder;
  auto name = node->intrin_attrs_->get<std::string>(intrin_attr::struct_name);
  auto field = node->intrin_attrs_->get<int>(intrin_attr::struct_field);
  ...
  _function_(dtype, read_struct, {node->args_[0]}) {
    ...
    builder::get_current_builder()->push_var_tensor_def(
        util_tsr, linkage::local,
        builder::tensor_ptr(node->args_[0], {offset}));
...
static func_t
create_write_struct_func(const ir_module_ptr &mod, const intrin_call_c &node,
                         target_specific_lower_cpu_impl_t *visitor) {
  builder::ir_builder_t builder;
  auto name = node->intrin_attrs_->get<std::string>(intrin_attr::struct_name);
  auto field = node->intrin_attrs_->get<int>(intrin_attr::struct_field);
  ...
  _function_(datatypes::void_t, write_struct, {node->args_[0]},
    ...
    expr util_tsr = builder::make_tensor(
        "util_tsr", {1}, dtype.is_pointer() ? datatypes::pointer : dtype);
    util_tsr->attr().set(attr_keys::tsr_dont_buf_sched, true);
    util_tsr->attr().set(attr_keys::no_dead_write, true);
    util_tsr->attr().set(attr_keys::no_tensor2var, true);
    size_t offset = get_field_offset(name, field);
    builder::get_current_builder()->push_var_tensor_def(
        util_tsr, linkage::local, builder::tensor_ptr(dyn_tsr, {offset}));
...
using intrin_func_creator = func_t (*)(const ir_module_ptr &mod,
                                       const intrin_call_c &node,
                                       target_specific_lower_cpu_impl_t *);
...
public:
  ...
  std::vector<expr_c> visit_need_def_args(const std::vector<expr> &args) {
    ...
    for (auto &arg : args) {
      if (!arg.isa<var>() && !arg.isa<tensor>() && !arg.isa<indexing>() &&
        ...
        expr_c cache_var = builder::make_var(
            arg->dtype_, "_arg_cache_" + std::to_string(var_cnt_++));
  ...
  expr_c do_lower_saturated_cast(const intrin_call_c &v) {
    ...
    if (mod_->ctx_->machine_.cpu_flags_.fAVX512F) {
      ...
      if (v->dtype_ == sc_data_type_t::s8(16)) {
        ...
        if (intype == sc_data_type_t::s32(16)) {
          auto zero =
              make_expr<constant_node>(UINT64_C(0), sc_data_type_t::s32(16));
          return builder::make_saturated_cast(builder::make_max(inval1, zero),
                                              v->dtype_);
        } else if (intype == sc_data_type_t::f32(16)) {
          return u8s8_saturate_cast();
        }
        ...
        return builder::make_round_and_cast(inval1, sc_data_type_t::s32(16));
      }
    }
    auto cast_s32_u8s8 = [intype, outtype](const expr_c &v) {
      int64_t max_val = outtype.type_code_ == sc_data_etype::U8 ? 255 : 127;
      int64_t min_val = outtype.type_code_ == sc_data_etype::U8 ? 0 : -128;
      expr val255 =
          make_expr<constant_node>(max_val, sc_data_type_t::s32(intype.lanes_));
      expr val0 =
          make_expr<constant_node>(min_val, sc_data_type_t::s32(intype.lanes_));
      // ret = min(v, 255)
  ...
  expr_c visit(call_c v) override {
    ...
    if (v->func_ == builtin::get_barrier_arrive_func()) {
      ...
      if (bar_id >= 0 && itr != barrier_idle_func_args_.end()) {
        ...
        return copy_attr(
            *v, builtin::get_barrier_arrive_func()(
                    ret2->args_.at(0), itr->second.first, itr->second.second));
      }
    }
    return ret;
  }
  expr_c visit(intrin_call_c v) override {
    ...
    if (v->type_ == intrin_type::set_thread_idle_func) {
        ...
        for (auto &seq : cur_scope_->seq_) {
          if (seq.isa<evaluate>()) {
            ...
            if (val.isa<call>()) {
              ...
              if (call_node->func_ == builtin::get_barrier_arrive_func()) {
      ...
      if (v->args_.size() == 2UL) {
        ...
        for (size_t i = 1; i < v->args_.size(); i++) {
          insert_before.emplace_back(builder::make_assign_unattached(
              args_pack[i - 1],
              builder::make_cast(datatypes::generic, v->args_[i])));
        }
      ...
      return builtin::get_set_idle_func_managed_func()(v->args_[0], args_pack);
    } else if (v->type_ == intrin_type::get_group_thread_id) {
      int64_t level_id = get_const_as_int(v->args_[0].checked_as<constant_c>());
    ...
    case intrin_type::read_struct:
      COMPILE_ASSERT(v->args_[0]->dtype_.get_pointer_element() == datatypes::u8,
    ...
    if (lower_func) {
      ...
      if (f) {
        ...
        const char *prompt = "Bad signature found for intrinsic implementation "
                             "function";
        ...
        for (size_t i = 0; i < f->params_.size(); i++) {
          COMPILE_ASSERT(f->params_[i]->dtype_ == v->args_[i]->dtype_ ||
                             (f->params_[i]->dtype_ == datatypes::pointer &&
                              v->args_[i]->dtype_.is_pointer()),
                         prompt << f);
        }
      } else {
...
static func_t create_log_func(const ir_module_ptr &mod,
  ...
  auto inf_f = gen_vec_const(elements, std::numeric_limits<float>::infinity());
  auto qnan_f =
  ...
  if (!global_log_consts_1.defined()) {
    log_table_tsr_1 = builder::make_tensor(
        log_const_table_name_1, {32}, datatypes::f32, address_space::automatic,
        std::make_shared<static_data_t>(sc_log_const_int_vals_1,
                                        sizeof(sc_log_const_int_vals_1)));
    log_table_tsr_2 = builder::make_tensor(
        log_const_table_name_2, {32}, datatypes::f32, address_space::automatic,
        std::make_shared<static_data_t>(sc_log_const_int_vals_2,
                                        sizeof(sc_log_const_int_vals_2)));
    log_table_tsr_1->attr().set(attr_keys::static_global,
                                (void *)sc_log_const_int_vals_1);
    log_table_tsr_2->attr().set(attr_keys::static_global,
                                (void *)sc_log_const_int_vals_2);
  ...
  _function_(type, the_log_func, make_args_by_intrinsic(node)) {
    ...
    aux1_int = ((inval_int >>
                 gen_vec_const_int(elements, mantissa_bits - approx_bits)) &
    ...
    aux2_int =
        builder::make_int_xor(aux2_int, gen_vec_const_int(elements, 0x0000007f))
        << gen_vec_const_int(elements, 23);
    inval_int =
        (inval_int & gen_vec_const_int(elements, 0x007fffff)) | aux2_int;
    _var_init_(aux2_f, type, builder::make_gather(log_table_tsr_1, aux1_int));
    aux2_f = aux2_f * builder::make_reinterpret(inval_int, type) - ONE_f;
    _var_init_(poly_f, type, gen_vec_const(elements, 0.199984118f));
    poly_f = builder::make_fmadd(poly_f, aux2_f,
                                 gen_vec_const(elements, -0.250035613f));
    poly_f = builder::make_fmadd(poly_f, aux2_f,
                                 gen_vec_const(elements, 0.333333343f));
    poly_f =
        builder::make_fmadd(poly_f, aux2_f, gen_vec_const(elements, -0.5f));
...
const_ir_module_ptr
target_specific_lowering_cpu_t::operator()(const_ir_module_ptr m) {
  auto ret = m->copy();
...
static func_t create_erf_func(const ir_module_ptr &mod,
  ...
  if (mod->ctx_->machine_.cpu_flags_.fAVX512F && elements >= 4) {
    std::vector<std::string> erf_const_table_names = {
        "erf_const_table_0", "erf_const_table_1", "erf_const_table_2",
        "erf_const_table_3", "erf_const_table_4", "erf_const_table_5"};
    ...
    if (!global_erf_const_0.defined()) {
      for (int i = 0; i < 6; i++) {
        erf_table_tsr_list[i] = builder::make_tensor(
            ...
            std::make_shared<static_data_t>(sc_erf_const_int_vals[i],
                                            sizeof(uint32_t) * 32));
      ...
      for (int i = 1; i < 6; i++) {
        erf_table_tsr_list[i] =
            mod->get_var_def_from_symbol(erf_const_table_names[i])->var_;
      }
    }
    _function_(type, the_erf_func, make_args_by_intrinsic(node)) {
      ...
      _var_init_(pol, inval->dtype_,
                 builder::make_indexing(erf_table_tsr_list[5], {0}, elements));
      pol = builder::make_permutex2var(
          pol, indices,
          builder::make_indexing(erf_table_tsr_list[5], {16}, elements));
      _var_(tmp, inval->dtype_);
      for (int i = 4; i >= 0; i--) {
        tmp = builder::make_indexing(erf_table_tsr_list[i], {0}, elements);
        tmp = builder::make_permutex2var(
            tmp, indices,
            builder::make_indexing(erf_table_tsr_list[i], {16}, elements));
        pol = builder::make_fmadd(pol, src_pos, tmp);
      }
      auto tmp2 = in_s32 & sign_mask;
      pol = builder::make_reinterpret(
          builder::make_reinterpret(pol, ty_epi_32) ^ tmp2, inval->dtype_);
      _return_(pol);
    }
    ...
    expr const_a1 =
        make_expr<constant_node>(0.254829592f, sc_data_type_t::f32(elements));
    expr const_a2 =
        make_expr<constant_node>(-0.284496736f, sc_data_type_t::f32(elements));
    expr const_a3 =
        make_expr<constant_node>(1.421413741f, sc_data_type_t::f32(elements));
    expr const_a4 =
        make_expr<constant_node>(-1.453152027f, sc_data_type_t::f32(elements));
    expr const_a5 =
        make_expr<constant_node>(1.061405429f, sc_data_type_t::f32(elements));
    expr const_p =
        make_expr<constant_node>(0.3275911f, sc_data_type_t::f32(elements));
    _function_(type, the_erf_func, make_args_by_intrinsic(node)) {
      assert(node->args_.size() == 1);
      // to avoid underflow
      _bind_(inval);
      _var_init_(temp, sc_data_type_t::f32(elements), builder::make_abs(inval));
      ...
      _var_init_(
          t, sc_data_type_t::f32(elements),
          builder::make_div(ONE_f, builder::make_fmadd(const_p, temp, ONE_f)));
      _var_init_(result, sc_data_type_t::f32(elements), const_a5);
      _var_init_(
          sign, sc_data_type_t::s32(elements),
          builder::make_int_and(
              builder::make_reinterpret(inval, sc_data_type_t::s32(elements)),
              sign_mask));
      ...
      result = builder::make_reinterpret(
          builder::make_int_xor(
              sign,
              builder::make_reinterpret(result, sc_data_type_t::s32(elements))),
