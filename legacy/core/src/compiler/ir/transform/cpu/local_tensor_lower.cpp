...
SC_DECL_PASS_INFO(local_tensor_lowering_cpu,
                  SC_PASS_DEPENDS_ON(constant_folder, buffer_scheduler,
                                     tensor_init, module_globals_resolver,
...
func_t get_acquire_const_cache_func() {
  static func_t f_global = set_noalias_function(builder::_decl_func(
      "sc_acquire_const_cache", datatypes::pointer,
      {_arg_("stream", datatypes::pointer), _arg_("cacheptr", datatypes::index),
...
func_t get_release_const_cache_func() {
  static func_t f_global = builder::_decl_func(
      "sc_release_const_cache", datatypes::void_t,
      {_arg_("stream", datatypes::pointer), _arg_("cacheptr", datatypes::index),
...
func_t get_cpu_temp_malloc_func(bool is_thread_local) {
  static func_t f_global = set_noalias_function(builder::_decl_func(
      "sc_aligned_malloc", datatypes::pointer,
      {_arg_("stream", datatypes::pointer), _arg_("size", datatypes::index)}));
  static func_t f_local = set_noalias_function(builder::_decl_func(
      "sc_thread_aligned_malloc", datatypes::pointer,
      {_arg_("stream", datatypes::pointer), _arg_("size", datatypes::index)}));
...
func_t get_cpu_temp_free_func(bool is_thread_local) {
  static func_t f_global = builder::_decl_func(
      "sc_aligned_free", datatypes::void_t,
      {_arg_("stream", datatypes::pointer), _arg_("ptr", datatypes::pointer)});
  static func_t f_local = builder::_decl_func(
      "sc_thread_aligned_free", datatypes::void_t,
      {_arg_("stream", datatypes::pointer), _arg_("ptr", datatypes::pointer)});
  return is_thread_local ? f_local : f_global;
}
...
public:
  ...
  stmt_c visit(define_c v) override {
    ...
    if (v->linkage_ == linkage::local &&
        v->var_.static_as<tensor>()->name_ == shared_const_handle_name) {
      ...
      if (v->init_.isa<tensorptr>()) {
        ...
        if (attr && attr->get_or_else("hoisted", false)) {
          ...
          hoisted_tensor_map_[tsr] = tptr->base_->ptr_;
                   ...
                   attr->get_or_else(attr_keys::can_be_scheduled, false)) {
          auto base = tptr->base_->ptr_;
          COMPILE_ASSERT(tptr->base_->idx_.size() == 1 &&
                             tptr->base_->idx_[0].isa<constant>(),
                         "Expecting 1D constant sized tensor");
          auto offset =
              get_const_as_int(tptr->base_->idx_[0].static_as<constant>());
          ...
          scheduled_tensor_position_.insert(std::make_pair(
              tsr, tensor_base_offset_t{base, offset, offset + tsr_size - 1}));
    ...
    COMPILE_ASSERT(tsr->dims_.size() == 1,
                   "tensor_lower_impl needs 1D tensors");
    ...
    auto shared_cached_buffer =
        any_map_t::fetch_or_else<std::shared_ptr<cached_const_graph_tensor>>(
    ...
    if (is_const && sz <= threshold_ &&
        !shared_cached_buffer && /*and the tensor is not marked
                                    runtime_stack_alloc*/
        !(tsr->attr_ &&
          tsr->attr_->get_or_else(attr_keys::runtime_stack_alloc, false))) {
    ...
    if (shared_cached_buffer) {
      ...
      if (shared_cached_buffer->buf_base_->is_lazy_) {
        COMPILE_ASSERT(tsr_is_init.defined(), "Expecting __is_init defined");
        ...
        initv = builder::make_reinterpret(
            shared_const_handles[uint64_t(*handle_idx)], datatypes::pointer);
      }
    } else {
      bool thread_loca =
          is_in_parallel_ ||
          any_map_t::fetch_or_else(tsr->attr_.get(), "is_thread_buffer", false);
    ...
    return copy_attr(
        *v, builder::make_var_tensor_def_unattached(tsr, v->linkage_, initv));
  }
    ...
    if (!current_scope.empty()) {
      ...
      for (auto itr = current_scope.rbegin(); itr != current_scope.rend();
        ...
        if (auto cached_buffer = any_map_t::fetch_or_else<
                ...
                (*itr)->attr_.get(), attr_keys::shared_const, nullptr)) {
          auto handle_idx = any_map_t::fetch_or_null<size_t>(
              (*itr)->attr_.get(), attr_keys::shared_const_base_idx);
          COMPILE_ASSERT(handle_idx, "Expecting attr shared_const_base_idx");
          COMPILE_ASSERT(shared_const_handles.defined(),
                         "Expecting shared_const_handles defined");
          if (cached_buffer->buf_base_->is_lazy_) {
            the_call = builder::make_evaluate_unattached(builder::make_call(
                get_release_const_cache_func(),
                {cur_rtl_ctx_, shared_const_handles[uint64_t(*handle_idx)],
                 *itr}));
          } else {
          ...
          bool thread_loca = is_in_parallel_ || any_map_t::fetch_or_else(
                                                    (*itr)->attr_.get(),
                                                    "is_thread_buffer", false);
          the_call = builder::make_evaluate_unattached(builder::make_call(
              get_cpu_temp_free_func(thread_loca), {cur_rtl_ctx_, *itr}));
        }
        if ((*itr)->attr_ &&
            (*itr)->attr_->get_or_else("temp.may_inplace", false)) {
...
mark_alias_for_scheduled_tensors(
  ...
  if (scheduled_tensor_position.empty())
    return {};
  ...
  std::sort(sorted_base_tsr_to_traces.begin(), sorted_base_tsr_to_traces.end(),
          ...
          if (auto aid_base = alias_info::get_alias_info(*hoisted)) {
            ...
            for (auto &aset : aid_base->alias_cliques_) {
              if (!aset->set_.has(aid_tsr)) {
                to_add_hoisted_alias.emplace_back(aid_tsr.get(), aset.get());
              }
            }
          }
