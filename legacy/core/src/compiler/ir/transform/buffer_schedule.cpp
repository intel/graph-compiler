...
SC_DECL_PASS_INFO(buffer_scheduler,
                  SC_PASS_DEPENDS_ON(tensor_shrinker, tensor_inplace,
                                     tensor2var, tensor_init, index_flattener,
...
public:
  ...
  stmt_c visit(for_loop_c v) override {
    ...
    if (changed) {
      return copy_attr(*v, builder::make_for_loop_unattached(
...
static bool is_parallel_for(const stmt_c &v) {
  if (v.isa<for_loop_c>()) {
    ...
    if (loop->kind_ == for_type::PARALLEL) {
      return true;
    }
    if (v->attr_ &&
        v->attr_->get_or_else(attr_keys::buf_sched_top_scope, false)) {
      return true;
    }
  }
  return false;
...
public:
    ...
    if (itr != out_.end()) {
      ...
      if (t->attr_ && t->attr_->has_key(attr_keys::hint_first_access_tick)) {
        ...
        if (hint_first_access_tick == HINT_IN_LOOP ||
          ...
          if (itr->second.first_access_ >= for_start_tick) {
            itr->second.first_access_ = for_start_tick + hint_first_access_tick;
          }
  ...
  void leave_complex_scope(const stmt_c &cur) override {
    ...
    if (for_depth_ == 0 || parallel_depth_ == for_depth_) {
      ...
      for (auto &itr : ticks_in_scope_.back()) {
        update_complex_scope_ticks(itr.first, for_start_ticks_.back(), tick_,
                                   itr.second);
      }
    ...
    if (is_parallel_for(cur)) {
      in_parallel_for_ = false;
    }
    if (parallel_depth_ == for_depth_ + 1) {
      parallel_depth_ = -1;
    }
  }
  ...
  stmt_c visit(assign_c v) override {
    ...
    if (v->var_.isa<indexing>()) {
      ...
      if (tsr->attr_ && tsr->attr_->get_or_else("list_brgemm_arg", false)) {
        ...
        if (itr != out_.end()) {
          if (!itr->second.list_brgemm_tensors_) {
            itr->second.list_brgemm_tensors_ =
                utils::make_unique<std::unordered_set<expr_c>>();
          }
          auto list_brg_real_tsr = get_base_tensor_of(v->value_);
          if (list_brg_real_tsr.defined()) {
            itr->second.list_brgemm_tensors_->insert(list_brg_real_tsr);
          }
        }
      }
    }
  ...
  stmt_c visit(define_c v) override {
    if (v->var_.isa<tensor>()) {
      ...
      if (v->linkage_ == linkage::local &&
          (!v->attr_ ||
           !v->attr_->get_or_else(attr_keys::tsr_dont_buf_sched, false))) {
        bool flag = false;
        if (!v->init_.defined()) {
          ...
          const auto &tsr = v->init_.static_as<tensorptr>()->base_->ptr_;
          if (tsr->attr_ &&
              tsr->attr_->get_or_else(attr_keys::can_be_scheduled, false)) {
            flag = true;
        ...
        if (flag) {
          ...
          if (v->init_.defined()) {
            out_[v->var_].already_scheduled_base_ = already_scheduled_base;
          }
  ...
  expr_c visit(indexing_c v) override {
    ...
    if (indexing_parent_ != TENSOR_PTR) {
      good_tensor_ = true;
    }
  ...
  expr_c visit(call_c v) override {
    ...
    for (unsigned i = 0; i < v->args_.size(); i++) {
      ...
      if (tsr.defined()) {
        bool has_read = funcp->attr_ && funcp->attr_->has_key("read_buffer");
        if (has_read) {
          set_read_tick(tsr, tick_);
        }
        bool has_write = funcp->attr_ && funcp->attr_->has_key("write_buffer");
        if (has_write) {
          set_write_tick(tsr, tick_);
        }
    ...
    if (inplace_hint) {
      for (auto &kv : *inplace_hint) {
        ...
        // already created
        if (tick_ != itr->second.first_access_) {
          continue;
        }
        ...
        for (auto idx : can_inplace_tsrs) {
          ...
          itr->second.inplace_reuse_.emplace_back(inplace_tsr, idx.kind_);
        }
        ...
        if (tsr.defined()) {
          ...
          case brgemm_args::B:
            ...
            if (v->type_ == intrin_type::list_brgemm) {
              auto itr = out_.find(tsr);
              if (itr != out_.end() && itr->second.list_brgemm_tensors_) {
                for (auto &linked_tsr : *itr->second.list_brgemm_tensors_) {
  ...
  void leave_complex_scope(const stmt_c &v) override {
    ...
    if (for_depth_ == 0) {
      for (auto &itr : removed_in_for) {
        ...
        if (writes.find(tick_) != writes.end()) {
          writes.erase(tick_);
        }
      }
    ...
    if (v->var_.isa<indexing>()) {
      ...
      if (itr != out_.end()) {
        ...
        bool should_remove =
            ...
            last_read != COMPLICATED_ACCESS && tick_ > itr->second.last_read_;
        if (should_remove) {
...
static bool
check_if_tensor_valid(const std::pair<const expr_c, tensor_tick_info_t> &itr,
                      bool &need_remove) {
  ...
  if (!itr.first.checked_as<tensor_c>()->dims_[0].isa<constant>()) {
    ...
    SC_MODULE_INFO << "The tensor " << itr.first << " has non-constant shape";
    return false;
  }
  SC_MODULE_INFO << "tsr: " << itr.first << ", LRT=" << itr.second.last_read_
                 << ", LWT="
                 << (itr.second.writes_.empty() ? TICK_NOT_EXIST
...
static void
schedule_tensors(std::unordered_map<expr_c, tensor_tick_info_t> &ticks,
                 std::vector<expr_c> &defined_tensors,
  ...
  for (auto &tsr : defined_tensors) {
    ...
    int64_t cur_tsr_size =
        get_const_as_int(cur_tensor->dims_[0].static_as<constant_c>());
    ...
    while (titr != last_read_tensor.end()) {
      ...
      if (new_info.create_ <= cur_info.first_access_ &&
        ...
        if (cur_info.last_read_ != TICK_NOT_EXIST) {
          auto lower = new_info.writes_.lower_bound(cur_info.first_access_);
          auto upper = new_info.writes_.upper_bound(cur_info.last_read_);
          ...
          if (lower != upper) {
            ...
            SC_MODULE_INFO << "Write after read: Failed " << cur_tensor->name_
        ...
        if (new_info.is_arg_ && !cur_info.writes_.empty() &&
            *cur_info.writes_.rbegin() >= new_info.first_access_) {
          SC_MODULE_INFO << "Write final buf: Failed " << cur_tensor->name_
                         << "->" << new_tensor->name_;
          ++titr;
          continue;
        }
        ...
        // shape
        int64_t new_tsr_size =
            get_const_as_int(new_tensor->dims_[0].static_as<constant_c>());
        ...
        if (cur_tsr_size > new_tsr_size) {
          ...
          if (new_info.is_arg_) {
            SC_MODULE_INFO << cur_tensor->name_ << " cannot reuse "
                           << new_tensor->name_ << ": cannot extend";
            ++titr;
            continue;
          }
        ...
        new_info.writes_.insert(cur_info.writes_.begin(),
                                cur_info.writes_.end());
...
static const tensor_node *get_inplace_target_if_single_inplace(
  ...
  return some_opt(tsr->attr_.get())
      ...
      .filter([](std::vector<temp_tensor_inplace_info_t> *v) {
        return v->size() == 1UL && (*v)[0].kind_ == inplace_kind::ZERO_OFFSET;
      })
      .flat_map(
          [&identity_to_tensor](std::vector<temp_tensor_inplace_info_t> *v) {
            return utils::find_map_value(identity_to_tensor,
                                         (*v)[0].to_reuse_.get());
          })
      .map([](const expr_c *v) {
      ...
      .filter([base](const tensor_node *v) {
        return base->elem_dtype_ == v->elem_dtype_ &&
               get_const_as_int(base->dims_.at(0).static_as<constant>()) >=
                   get_const_as_int(v->dims_.at(0).static_as<constant>());
      })
      .get_or_else(nullptr);
}
...
static std::vector<size_t> schedule_tensor_memory_planner(
  ...
  std::sort(
      ...
      [](const std::pair<const expr_c, tensor_tick_info_t> *x,
        ...
        if (x->second.real_first_access_ != y->second.real_first_access_) {
          return x->second.real_first_access_ < y->second.real_first_access_;
        }
  ...
  for (auto &itr_ptr : sorted_ticks) {
    ...
    if (itr.second.is_arg_) {
      ...
      if (alias_data) {
        arg_identities.emplace_back(alias_data);
      }
    }
  }
  for (auto &itr_ptr : sorted_ticks) {
        ...
        if (alias_data) {
          for (auto &tid : arg_identities) {
            if (tid != alias_data && alias_data->is_alias_of(tid)) {
              SC_MODULE_INFO << "Cannot in-place reuse output buffer" << tsr
      ...
      while (inplace_parent) {
        if (auto pinfo = utils::find_map_value(
          ...
          if (!check_if_tensor_valid(itr, need_remove)) {
            ...
            SC_MODULE_INFO << "In-place reusing output buffer" << inplace_parent
                           << " -> " << tsr;
            pinfo->is_arg_ = true;
            replace_map_in_output[inplace_parent->node_ptr_from_this()] =
  ...
  for (auto &itr_ptr : sorted_ticks) {
    ...
    traces[itr.second.scope_].insert(std::make_pair(
        last_access * 2 + 1,
    ...
    if (inplace) {
      ...
      for (auto &inp_tsr : itr.second.inplace_reuse_) {
        ...
        if (ticks[inp_tsr.first].get_last_access() ==
            itr.second.first_access_) {
          inplace_tsrs.insert(
              std::make_pair((uintptr_t)inp_tsr.first.get(), inp_tsr.second));
        }
      }
      if (tsr->attr_) {
        if (auto inplace_hints =
                tsr->attr_
                    ->get_or_null<std::vector<temp_tensor_inplace_info_t>>(
                        attr_keys::tensor_inplace_hint)) {
          for (auto &hint : *inplace_hints) {
            auto hint_itr = identity_to_tensor.find(hint.to_reuse_.get());
            if (hint_itr == identity_to_tensor.end()) {
              SC_MODULE_WARN << "Bad temp tensor in-place identity:" << tsr;
            } else {
              auto &victim = hint_itr->second;
              auto lastuse = ticks[victim].get_last_access();

              inplace_tsrs.insert(
                  std::make_pair((uintptr_t)victim.get(), hint.kind_));
            }
          }
        }
      }
  ...
  for (size_t i = 0; i < traces.size(); i++) {
    ...
    std::unordered_map<uintptr_t, std::vector<uintptr_t>> out_inplace_result;
    total_list[i] = memory_optim::schedule_memory_allocations(
        in_trace,
        /*512-bit alignment*/ 64, hot_first, inplace_map, out,
        out_inplace_result);
    for (auto &kv : out) {
      auto p = reinterpret_cast<expr_base *>(kv.first)->node_ptr_from_this();
    ...
    for (auto &kv : out_inplace_result) {
      ...
      if (callsite) {
        ...
        for (size_t pidx = 0; pidx < callsite->tensors_passed_.size(); pidx++) {
          if (callsite->tensors_passed_[pidx].ptr_same(p)) {
            auto cur_arg = callsite->func_->params_.at(pidx);
            auto cur_aliasinfo = alias_info::get_or_create_alias_info(*cur_arg);
            if (!arg.defined()) {
              arg = cur_arg;
            }
            SC_MODULE_INFO << "inplace-func " << callsite->func_->name_
                           << " Add arg1:" << cur_arg;
        ...
        for (size_t pidx = 0; pidx < callsite->tensors_passed_.size(); pidx++) {
          for (auto inplace_id : kv.second) {
            ...
            if (callsite->tensors_passed_[pidx].get() == inp) {
              auto cur_arg = callsite->func_->params_.at(pidx);
              auto cur_aliasinfo =
                  alias_info::get_or_create_alias_info(*cur_arg);
              SC_MODULE_INFO << "inplace-func " << callsite->func_->name_
                             << " Add arg2:" << cur_arg;
              cur_aliasinfo->add_to_clique(aliasinfo);
            }
  ...
  stmt_c visit(define_c v) override {
    if (v->var_.isa<tensor>()) {
      ...
      if (itr != extend_map_.end()) {
        ...
        return copy_attr(*v, builder::make_var_tensor_def_unattached(
...
public:
  ...
  buffer_replacer_memory_planner_t(
      ...
      : replace_map_(replace_map),
        ...
        stmts_to_scope_id_(stmts_to_scope_id), total_list_(total_list),
  ...
  stmt_c visit(stmts_c v) override {
    ...
    if (is_top_level) {
      ...
      if (total_list_[cur_scope]) {
        auto rescheduled_tsr =
            builder::make_tensor("__rescheduled_" + std::to_string(cur_scope),
    ...
    if (is_top_level) {
      ...
      if (total_list_[cur_scope]) {
        ...
        newret.insert(newret.begin(), builder::make_var_tensor_def_unattached(
  ...
  func_c dispatch(func_c v) override {
    ...
    if (ret != v) {
      std::const_pointer_cast<func_base>(ret)
          ->attr()[attr_keys::already_buf_sched] = true;
    }
    return ret;
  }
};
...
static T1 run(const context_ptr &ctx, T1 f, const func_c &toplevel,
  ...
  if (type == attr_keys::BUF_SCHED_NONE) {
    return f;
  }
  ...
  if (toplevel) {
    finder.parse_func_params(toplevel->params_);
  }
  finder.dispatch(f);
  ...
  if (aggresive) {
    ...
    auto total_list = schedule_tensor_memory_planner(
        tick_out, finder.identity_to_tensor_, defined, replacer_list,
    ...
    buffer_replacer_memory_planner_t rep{replacer_list, replacer_list_output,
                                         finder.stmts_to_scope_id_, total_list};
    ...
    if (replacer.empty() && extender.empty()) {
      return f;
    }
