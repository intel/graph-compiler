  ...
  stmt_c visit(for_loop_c v) override {
    ...
    auto for_range_new_end = [&](int64_t new_end) -> stmt_c {
      ...
      if (end.defined() && (get_const_as_int(end) > new_end)) {
        auto iter_end =
            builder::make_constant({new_end}, vv->iter_end_->dtype_);
        return copy_attr(
            *vv, make_stmt<for_loop_node_t>(vv->var_, vv->iter_begin_, iter_end,
                                            vv->step_, std::move(new_body),
                                            vv->incremental_, vv->kind_,
                                            vv->num_threads_));
      }
      return vv;
    };
    auto for_range_new_beg = [&](int64_t new_beg) -> stmt_c {
      ...
      if (begn.defined() && step.defined() &&
        ...
        auto iter_beg = builder::make_constant({n * old_stp + old_beg},
                                               vv->iter_begin_->dtype_);
        return copy_attr(*vv, make_stmt<for_loop_node_t>(
                                  vv->var_, iter_beg, vv->iter_end_, vv->step_,
                                  std::move(new_body), vv->incremental_,
                                  vv->kind_, vv->num_threads_));
      }
      return vv;
    };
    ...
    auto cond =
        vv->body_.cast<stmts>()
            .filter([vv](const stmts &v) {
              return vv->kind_ == for_type::NORMAL && v->seq_.size() == 1;
            })
            .map([](const stmts &v) { return v->seq_.back().as<if_else>(); })
            .filter([&new_body](const if_else &v) {
              new_body = v->then_case_;
              return !v->else_case_.defined();
            })
            .map([](const if_else &v) { return v->condition_.dyn_as<cmp>(); })
            .filter([vv](const cmp &v) {
              return v->l_.ptr_same(vv->var_) && v->r_.isa<constant>();
            });
    if (cond.has_value()) {
      ...
      case sc_expr_type::cmp_lt:
        return for_range_new_end(n);
      case sc_expr_type::cmp_le:
        return for_range_new_end(n + 1);
      case sc_expr_type::cmp_gt:
        return for_range_new_beg(n + 1);
      case sc_expr_type::cmp_ge:
        return for_range_new_beg(n);
      default:
        break;
      }
    }
