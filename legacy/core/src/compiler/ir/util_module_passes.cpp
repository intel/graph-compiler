...
const_ir_module_ptr
sequential_module_pass_t::operator()(const_ir_module_ptr f) {
  ...
  for (auto &p : passes_) {
    auto timer = utils::create_scoped_timer(
        need_print_time, [&p](utils::time_duration dur) {
          auto diff = std::chrono::duration_cast<std::chrono::microseconds>(dur)
                          .count();
          std::string mod_name =
              std::string("pass.time.") + get_pass_name(p.get());
          SC_MODULE_INFO2(mod_name.c_str()) << "The pass took " << diff << "us";
        });
    f = (*p)(f);
    if (need_result) {
      std::string mod_name =
...
const_ir_module_ptr dispatch_module_on_visitor(ir_visitor_t *pass,
                                               const const_ir_module_ptr &f) {
