...
template <typename Base, typename Derived> struct is_base_of_t {
...
template <typename T, typename Base> class node_ptr;

namespace optional_impl {
template <typename T, typename Base> struct optional_base<node_ptr<T, Base>> {
 ...
 * */
template <typename T, typename Base> class node_ptr_impl_t {
public:
  template <typename _Arg>
  using _assignable =
      typename std::enable_if<std::is_convertible<_Arg *, T *>::value,
                              Base>::type;
  static_assert(is_base_of_t<Base, T>::value, "T should be a subclass of Base");
  ...
  explicit node_ptr_impl_t(impl_ptr &&impl) noexcept : impl(std::move(impl)) {}
  ...
  template <typename T2>
  node_ptr_impl_t &
  operator=(const node_ptr<T2, _assignable<T2>> &other) noexcept {
    impl = other.impl;
    return *this;
  }
   ...
   *      a.isa<binary>() == false
   * */
  template <typename T2> bool isa() const noexcept {
   ...
   * */
  template <typename T2> bool instanceof () const noexcept { // NOLINT
   ...
   * */
  template <typename T2> node_ptr<typename T2::type, Base> as() const noexcept {
    if (isa<T2>()) {
      return static_as<T2>();
    } else {
   ...
   * @return The casted node_ptr of T2.
   * */
  template <typename T2> node_ptr<typename T2::type, Base> checked_as() const {
...
public:
  ...
  struct SC_INTERNAL_API lvalue_proxy_t {
     ...
     * */
    template <typename T2> node_ptr<typename T2::type, expr_base> static_as() {
...
struct span_t {
  ...
  span_t(std::vector<expr> index, uint16_t length, expr mask = expr(),
         uint16_t rows = 0)
      : index_(std::move(index)), length_(length), rows_(rows),
        mask_(std::move(mask)) {}

  span_t(span_t &&other)
      : index_(std::move(other.index_)), length_(other.length_),
        rows_(other.rows_), mask_(std::move(other.mask_)) {}
};
 ...
 * */
template <typename Base> struct visitable_base_t {
...
public:
  logic_node(sc_expr_type expr_type, expr l, expr r)
      : expr_base(datatypes::boolean, expr_type), l_(std::move(l)),
        r_(std::move(r)){};
  // the left hand side expr
  expr l_;
  // the right hand side expr
...
public:
  ...
  select_node(expr cond, const expr &l, const expr &r)
      : expr_base(l->dtype_, sc_expr_type::select), cond_(std::move(cond)),
        l_(l), r_(r) {
...
public:
   ...
   * */
  template <class... Types> expr operator()(Types... args) {
    return expr(
        std::make_shared<call_node>(*this, std::vector<expr>({expr(args)...})));
  }
  /**
...
public:
  ...
  static std::shared_ptr<static_data_t> make_tensor_initializer(union_val val);
};
...
public:
  ...
  indexing_node(const expr &ptr, const std::vector<expr> &idx, expr mask)
      : expr_base(sc_data_type_t(
        ...
        ptr_(ptr), idx_(idx), mask_(std::move(mask)) {}
  indexing_node(const expr &ptr, const std::vector<expr> &idx, uint16_t lanes,
                uint16_t rows, expr mask)
      : expr_base(
            sc_data_type_t(etypes::get_pointer_element(ptr->dtype_.type_code_),
                           lanes, rows),
            sc_expr_type::indexing),
        ptr_(ptr), idx_(idx), mask_(std::move(mask)) {}
  expr ptr_;
...
public:
  ...
  tensorptr_node(indexing base, const std::vector<expr> &shape, bool is_slice)
      : expr_base(sc_data_type_t::pointerof(base->dtype_.type_code_),
        ...
        base_(std::move(base)), shape_(shape), is_slice_(is_slice) {}
  indexing base_;
...
template <> struct hash<dnnl::impl::graph::gc::expr_c> {
  std::size_t operator()(const dnnl::impl::graph::gc::expr_c &k) const {
    return hash<dnnl::impl::graph::gc::expr::impl_ptr>()(k.impl);
  }
};

template <> struct equal_to<dnnl::impl::graph::gc::expr_c> {
