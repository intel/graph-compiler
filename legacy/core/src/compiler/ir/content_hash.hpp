...
namespace graph {
namespace gc {
template <typename T> struct content_hash_t {};

template <> struct content_hash_t<constant_c> {
  std::size_t operator()(const constant_c &k) const;
};

template <> struct content_hash_t<expr> {
  std::size_t operator()(const expr &k) const;
};

template <> struct content_hash_t<expr_c> {
  std::size_t operator()(const expr_c &k) const;
};

template <typename T> struct content_hash_t<std::vector<T>> {
...
template <typename T> struct content_equals_t {};

template <> struct content_equals_t<expr_c> {
...
template <typename T> struct content_equals_t<std::vector<T>> {
