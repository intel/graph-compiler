...
void print_index(expr v) {
  static const func_t print_index_f =
      make_func("print_index", {make_var(datatypes::index, "v")}, stmt(),
                datatypes::void_t);
  _evaluate_call_(print_index_f, std::move(v));
}

void print_int(expr v) {
  static const func_t print_int_f = make_func(
      "print_int", {make_var(datatypes::s32, "v")}, stmt(), datatypes::void_t);
  _evaluate_call_(print_int_f, std::move(v));
}

void print_float(expr v) {
  static const func_t print_float_f =
      make_func("print_float", {make_var(datatypes::f32, "v")}, stmt(),
                datatypes::void_t);
  _evaluate_call_(print_float_f, std::move(v));
}

void print_str(expr v) {
  static const func_t print_str_f =
      make_func("print_str",
...
void print_str(const char *v) { print_str(std::string(v)); }
...
expr make_trace(expr func_name, expr in_or_out, expr arg) {
  static func_t make_trace_f = mark_trace_func(make_func(
      "sc_make_trace",
      {make_var(datatypes::s32, "func_name"),
       make_var(datatypes::s32, "in_or_out"), make_var(datatypes::s32, "arg")},
      stmt(), datatypes::void_t));
...
func_t get_brgemm_init_func() {
  static func_t brgemm_func = _decl_func(
      "dnnl_brgemm_init", datatypes::s32,
      {_arg_("C", datatypes::pointer), _arg_("M", datatypes::s32),
       _arg_("N", datatypes::s32), _arg_("LDC", datatypes::s32),
       _arg_("dtypeC", datatypes::s32), _arg_("value", datatypes::f32)});
  return brgemm_func;
}
...
static std::pair<func_t, func_t>
declare_brgemm_kernel_creator(scflags_t::brgemm_backend_t backend,
                              brgemm_mode mode, bool has_postop) {
  std::stringstream ss;
  ...
  if (mode == brgemm_mode::stride) {
    ...
    static func_t creator = _decl_func(
        ss.str() + "_func", datatypes::pointer,
        {_arg_("M", datatypes::s32), _arg_("N", datatypes::s32),
         ...
         _arg_("stride_a", datatypes::s32), _arg_("stride_b", datatypes::s32),
         _arg_("beta", datatypes::f32), _arg_("dtypeA", datatypes::s32),
    ...
    auto caller_args =
        std::vector<std::vector<expr>>{_arg_("func", datatypes::pointer),
    ...
    if (has_postop) {
      caller_args.insert(caller_args.end() - 1, post_args.begin(),
                         post_args.end());
    }
...
// returns the kernel creator and kernel caller pair
static func_t declare_brgemm_update_funcs(scflags_t::brgemm_backend_t backend,
                                          brgemm_mode mode, bool init) {
  std::stringstream ss;
  if (mode == brgemm_mode::stride) {
    ...
    auto update_args = std::vector<std::vector<expr>>{
        ...
        _arg_("C", datatypes::pointer),
        _arg_("num", datatypes::s32),
        _arg_("M", datatypes::s32),
        _arg_("N", datatypes::s32),
...
func_t get_brgemm_call_range_func(brgemm_mode mode) {
  if (mode == brgemm_mode::stride) {
    ...
    static func_t update = mark_brgemm_func(_decl_func(
        "dnnl_brgemm_call_range", datatypes::s32, std::move(update_args)));
    return update;
  } else {
    auto update_args =
        std::vector<std::vector<expr>>{_arg_("func", datatypes::pointer),
                                       ...
                                       _arg_("bottom_pad", datatypes::pointer),
                                       _arg_("stream", datatypes::pointer)};
    static func_t brgemm_func = mark_brgemm_func(_decl_func(
        "dnnl_brgemm_list_call_range", datatypes::s32, std::move(update_args)));
    return brgemm_func;
  }
}
...
evaluate brgemm_init_update(
    ...
    const expr &bd_mask_idx, const int &bd_mask_set_num, const expr &top_pad,
    const expr &bottom_pad, const sc_brgemm_postops_setting_t &postops_set,
  ...
  return builder::get_current_builder()
      ->brgemm(A, B, C, num, M, N, K, LDA, LDB, LDC, stride_a, stride_b,
               ...
               {brgemm_args::cpu_t{true}, dtypeA, dtypeB,
                infer_output_dtype(dtypeA), brg_attrs, bd_mask, bd_mask_set_num,
                postops_set})
      .checked_as<evaluate>();
}
...
evaluate brgemm_init_update_allow_fusion(
    ...
    const expr &bottom_pad, const sc_brgemm_postops_setting_t &postops_set,
    const std::vector<expr> &postops_data, const expr &c_buf) {
  auto ret = brgemm_init_update(A, B, C, num, M, N, K, LDA, LDB, LDC, stride_a,
                                stride_b, dtypeA, dtypeB, brg_attrs, bd_mask,
                                bd_mask_idx, bd_mask_set_num, top_pad,
                                bottom_pad, postops_set, postops_data, c_buf)
                 .checked_as<evaluate>();
...
evaluate brgemm_list_update(
  ...
  return builder::get_current_builder()
      ->list_brgemm(
          A, B, C, num, M, N, K, LDA, LDB, LDC, stride_a, stride_b, len,
          postops_data, c_buf, bd_mask_idx, top_pad, bottom_pad,
          brgemm_args::extra_args_t(brgemm_args::cpu_t{false}, dtypeA, dtypeB,
                                    infer_output_dtype(dtypeA), brg_attrs,
...
func_t get_mem_set_func() {
  static func_t memzerofunc =
      _decl_func("memset", datatypes::pointer,
...
func_t get_brgemm_postops_data_init_func() {
  static func_t data_init_func = _decl_func(
      "dnnl_brgemm_postops_data_init", datatypes::void_t,
      {_arg_("dnnl_data", datatypes::pointer),
       _arg_("bias", datatypes::pointer), _arg_("scales", datatypes::pointer),
...
expr call_matmul_core_query_format(
    ...
    const expr &impl) {
  static func_t matmul_core_query_f =
      make_func("query_format_matmul_core_op",
                ...
                stmt(), datatypes::void_t);
  return matmul_core_query_f(tb, out0, in0, in1, ori_in0, ori_in1, out_format0,
                             in_format0, in_format1, ori_in_format0,
                             ori_in_format1, out_size, kernel, impl);
}
...
expr call_managed_matmul_core_query_format(
    ...
    const expr &ori_in_format1, const expr &out_size, const expr &kernel,
    const expr &impl) {
  static func_t mmm_query_f =
      make_func("query_format_managed_matmul_core_op",
                {make_var(datatypes::pointer, "op_table"),
                 make_var(datatypes::pointer, "out"),
                 make_var(datatypes::pointer, "inp0"),
                 make_var(datatypes::pointer, "inp1"),
  ...
  return mmm_query_f(tb, out0, in0, in1, ori_in0, ori_in1, out_format0,
                     in_format0, in_format1, ori_in_format0, ori_in_format1,
  ...
  static func_t conv_fwd_core_query_f =
      make_func("query_format_conv_fwd_core_op",
                {make_var(datatypes::pointer, "op_table"),
                 ...
                 make_var(datatypes::pointer, "impl")},
                stmt(), datatypes::void_t);
  return conv_fwd_core_query_f(
      tb, out0, in0, in1, ori_in0, ori_in1, out_format0, in_format0, in_format1,
      ori_in_format0, ori_in_format1, out_size, kernel, impl);
}

expr call_unary_fusible_op_query_format(
    const expr &tb, const expr &out0, const expr &in0, const expr &out_format0,
    const expr &in_format0, const expr &out_size, const expr &kernel) {
  static func_t unary_query_f = make_func(
      "query_format_unary_fusible_op",
      {make_var(datatypes::pointer, "op_table"),
       make_var(datatypes::pointer, "out"), make_var(datatypes::pointer, "inp"),
       make_var(datatypes::pointer, "out_fmt"),
       make_var(datatypes::pointer, "inp_fmt"),
       make_var(datatypes::pointer, "out_size"),
       make_var(datatypes::pointer, "kernel")},
      stmt(), datatypes::void_t);
  return unary_query_f(tb, out0, in0, out_format0, in_format0, out_size,
                       kernel);
}
...
expr call_padding_op_query_format(const expr &tb, const expr &out0,
                                  ...
                                  const expr &kernel) {
  static func_t padding_query_f = make_func(
      "query_format_padding_op",
      {make_var(datatypes::pointer, "op_table"),
       make_var(datatypes::pointer, "out"), make_var(datatypes::pointer, "inp"),
       make_var(datatypes::pointer, "out_fmt"),
       make_var(datatypes::pointer, "inp_fmt"),
       ...
       make_var(datatypes::pointer, "kernel")},
      stmt(), datatypes::void_t);
  return padding_query_f(tb, out0, in0, out_format0, in_format0, out_size,
                         kernel);
}
...
expr call_pooling_op_query_format(const expr &tb, const expr &out0,
                                  ...
                                  const expr &in_format0, const expr &out_size,
                                  const expr &kernel) {
  static func_t pooling_query_f = make_func(
      "query_format_pooling_op",
      {make_var(datatypes::pointer, "op_table"),
       make_var(datatypes::pointer, "out"), make_var(datatypes::pointer, "inp"),
       make_var(datatypes::pointer, "out_fmt"),
       make_var(datatypes::pointer, "inp_fmt"),
       ...
       make_var(datatypes::pointer, "kernel")},
      stmt(), datatypes::void_t);
  return pooling_query_f(tb, out0, in0, out_format0, in_format0, out_size,
                         kernel);
}
...
expr call_binary_fusible_op_query_format(
  ...
  static func_t binary_query_f =
      make_func("query_format_binary_fusible_op",
                {make_var(datatypes::pointer, "op_table"),
                 make_var(datatypes::pointer, "out"),
                 make_var(datatypes::pointer, "inp0"),
                 make_var(datatypes::pointer, "inp1"),
                 ...
                 make_var(datatypes::pointer, "kernel")},
                stmt(), datatypes::void_t);
  return binary_query_f(tb, out0, in0, in1, out_format0, in_format0, in_format1,
                        out_size, kernel);
}
...
expr call_reorder_op_query_format(const expr &tb, const expr &out0,
                                  const expr &in0, const expr &out_format0,
                                  const expr &in_format0, const expr &out_size,
                                  const expr &kernel, const expr &impl) {
  static func_t reorder_query_f = make_func(
      "query_format_reorder_op",
      {make_var(datatypes::pointer, "op_table"),
       make_var(datatypes::pointer, "out"), make_var(datatypes::pointer, "inp"),
       make_var(datatypes::pointer, "out_fmt"),
       make_var(datatypes::pointer, "inp_fmt"),
       ...
       make_var(datatypes::pointer, "impl")},
      stmt(), datatypes::void_t);
  return reorder_query_f(tb, out0, in0, out_format0, in_format0, out_size,
                         kernel, impl);
}
...
expr call_select_op_query_format(const expr &tb, const expr &out0,
                                 ...
                                 const expr &kernel) {
  static func_t select_query_f =
      make_func("query_format_select_op",
                {make_var(datatypes::pointer, "op_table"),
                 make_var(datatypes::pointer, "out"),
                 make_var(datatypes::pointer, "inp0"),
                 make_var(datatypes::pointer, "inp1"),
...
expr call_fused_op_query_combined(const expr &tb, const expr &combined_keys,
  ...
  static func_t fused_query_f =
      make_func("query_combined_fused_op",
                {make_var(datatypes::pointer, "op_table"),
                 ...
                 make_var(datatypes::pointer, "kernel")},
                stmt(), datatypes::void_t);
  return fused_query_f(tb, combined_keys, combined_algs, each_op_num_key,
                       op_num, kernel);
}

expr call_cal_blocking_dims(const expr &placeholder, const expr &format) {
  static func_t cal_blocking_f =
      make_func("calculate_blocking_dims",
...
expr call_get_matmul_dyn_cfg_single(const expr &in, const expr &is_batch) {
  static func_t matmul_dyn_cfg_f =
      make_func("get_matmul_dyn_cfg_single",
...
func_t get_thread_id_func() {
  static func_t func =
      set_pure_function(_decl_func("sc_get_thread_id", datatypes::s32, {}));
  return func;
}

func_t get_is_in_parallel_func() {
  static func_t func =
      set_pure_function(_decl_func("sc_is_in_parallel", datatypes::s32, {}));
  return func;
}
...
func_t get_init_barrier_func() {
  static func_t func =
      _decl_func("sc_init_barrier", datatypes::void_t,
...
func_t get_set_idle_func_managed_func() {
  static func_t f =
      builder::make_func("sc_set_idle_func_managed",
...
func_t get_tls_amx_buffer_func() {
  static func_t f =
      set_pure_function(_decl_func("sc_get_tls_amx_buffer", datatypes::pointer,
                                   {_arg_("stream", datatypes::pointer)}));
  return f;
}
...
static expr generate_balance211_job_id_base(const expr &gid, const expr &num_A,
                                            const expr &val_A,
                                            const expr &val_B) {
  return builder::make_select(gid <= num_A, gid * val_A,
                              num_A * val_A + (gid - num_A) * val_B);
}

uint64_t generate_balance211(
    int num_threads, const expr &start_e, const expr &end_e, const expr &step_e,
    const expr &gid, const std::function<std::string(const char *)> &namer,
    expr *out_start, expr *out_len, expr *out_end, std::vector<stmt> *out_seq) {
  ...
  if (start_e.isa<constant>() && end_e.isa<constant>() &&
    ...
    if (num_split > 1UL) {
      ...
      expr id_in_split = def_var("id_in_split", gid % num_thread_each_split);
      expr is_large_work = id_in_split < num_thread_larger_work_per_split;
      expr cur_jobs = builder::make_select(is_large_work, my_jobs, my_jobs_2);
      // for gid=5, base_job_id = 11
      expr base_job_id = (gid / num_thread_each_split) * (num_jobs / num_split);
      // for gid=5, job_id_offset = 3
      expr job_id_offset;
      if (num_thread_larger_work_per_split == num_thread_each_split - 1) {
        job_id_offset = id_in_split * my_jobs;
      } else {
        job_id_offset = generate_balance211_job_id_base(
            id_in_split, num_thread_larger_work_per_split, my_jobs, my_jobs_2);
      }
    ...
    my_jobs_e =
        def_var("my_jobs", (num_jobs + (num_threads - 1)) / num_threads);
    // assert(my_jobs > 0);
  ...
  expr my_begin = start_e + generate_balance211_job_id_base(
