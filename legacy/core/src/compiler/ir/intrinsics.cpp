...
namespace graph {
namespace gc {
void intrinsic_handler_t::to_string(const intrin_call_c &v,
                                    ir_printer_t *printer) const {
  printer->os_ << name_ << '(';
  if (!v->args_.empty()) {
...
struct reinterpret_handler_t : public intrinsic_handler_t {
  void on_initialize(intrin_call_node &node) override {
    assert(node.args_.size() == 1);
    node.dtype_ =
        node.intrin_attrs_->get<sc_data_type_t>(intrin_attr::out_dtype);
  }
...
struct extract_handler_t : public intrinsic_handler_t {
  void on_initialize(intrin_call_node &node) override {
    ...
    if (node.args_.size() == 1) {
      ...
      node.dtype_ =
          sc_data_type_t(node.args_[0]->dtype_.type_code_, lanes, rows);
    }
  }
    ...
    COMPILE_ASSERT(node.dtype_.lanes_ == node.args_[0]->dtype_.lanes_ &&
                       (node.dtype_.type_code_ == sc_data_etype::S32 ||
                        (node.dtype_.type_code_ == sc_data_etype::U32 &&
                         node.dtype_.lanes_ > 1)) &&
                       node.args_[0]->dtype_.type_code_ == sc_data_etype::F32,
...
sc_data_type_t get_dtype_from_struct_and_field(const std::string &in,
                                               int field) {
...
struct avx_broadcast_idx_handler_t : public x86_intrinsic_handler_t {
  void on_initialize(low_level_intrin_node &node) override {
    ...
    node.dtype_ = sc_data_type_t(
        etypes::get_pointer_element(node.args_[0]->dtype_.type_code_), lanes);
  }
...
static std::unique_ptr<intrinsic_handler_t> handlers[] = {
    ...
    utils::make_unique<brgemm_handler_t>(brgemm_args::NUM_FULL_ARGS_STRIDE,
                                         "brgemm"),
    utils::make_unique<brgemm_handler_t>(brgemm_args::NUM_FULL_ARGS_LIST,
                                         "list_brgemm"),
};
...
const sc_data_type_t dyn_tsr_struct_t::dtypes[5] = {
    datatypes::pointer, datatypes::pointer, datatypes::s32, datatypes::u32,
    datatypes::u8};
