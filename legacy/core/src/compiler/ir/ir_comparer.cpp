...
void ir_comparer::reset() {
  same_ = true;
  if (diff)
    diff = utils::make_unique<ir_comparer_diff_t>();
  var_mapping_.clear();
}
ir_comparer::ir_comparer(bool needs_diff, bool cmp_names, bool cmp_var_ref,
                         bool cmp_callee, bool cmp_commutative)
    : cmp_names_(cmp_names), cmp_callee_(cmp_callee), cmp_var_ref_(cmp_var_ref),
      cmp_commutative_(cmp_commutative), same_(true) {
  if (needs_diff)
    diff = utils::make_unique<ir_comparer_diff_t>();
}
...
bool ir_comparer::expr_arr_equals(const std::vector<expr> &a,
                                  const std::vector<expr> &b) {
  if (a.size() != b.size()) {
    return false;
  }
  for (size_t i = 0; i < a.size(); i++) {
    if (!a.at(i)->equals(b.at(i), *this)) {
      return false;
    }
  }
  return true;
...
bool ir_comparer::check_or_set_expr_mapping(const expr_c &l, const expr_c &r) {
  ...
  if (f != var_mapping_.end()) {
    return f->second == r.get();
  }
  ...
  if (cmper.same_) {
    ...
    if (cmper.diff) {
      if (cmper.diff->first_diff_expr_.first.defined() ||
        ...
        os << "diff expr = " << cmper.diff->first_diff_expr_.first << " v.s. "
           << cmper.diff->first_diff_expr_.second;
      } else if (cmper.diff->first_diff_func_.first ||
                 cmper.diff->first_diff_func_.second) {
        os << "diff func = " << cmper.diff->first_diff_func_.first << " v.s. "
           << cmper.diff->first_diff_func_.second;
      } else if (cmper.diff->first_diff_stmt_.first.defined() ||
                 cmper.diff->first_diff_stmt_.second.defined()) {
        os << "diff stmt = " << cmper.diff->first_diff_stmt_.first << " v.s. "
           << cmper.diff->first_diff_stmt_.second;
      }
    }
  }
  os << '\n';
