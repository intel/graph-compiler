...
SC_DECL_PASS_INFO(validator,
                  SC_PASS_DEPENDS_ON(dyn_tensor_transformer,
                                     interface_generalizer, tensor_shrinker,
                                     index_flattener, auto_caster),
...
public:
  ...
  void add_def(const expr_base *v, const stmt_c &s) {
    ...
    COMPILE_ASSERT_POS(
        !is_var_defined(v),
        "The variable/tensor "
            << v << " is already defined. The second definition is: " << s);
  ...
  void validate_binary_op(const expr_c &v, const expr_c &l,
                          const expr_c &r) const;
  void check_indexing(const std::vector<expr> &idxvec, const expr_c &v,
                      unsigned expected_dims) const;
  void check_var_tensor_def(const define_c &v, bool allow_tensor_view,
...
func_c validate_impl_t::dispatch(func_c v) {
  ...
  allow_tensor_view_ =
      ...
      (cur_func_->attr_->get_or_else("allow_tensor_view", false) ||
       cur_func_->attr_->get_or_else(function_attrs::low_level, false));
...
void validate_impl_t::validate_type(sc_data_type_t dtype,
                                    const expr_c &e) const {
...
void validate_impl_t::validate_binary_op(const expr_c &v, const expr_c &l,
                                         const expr_c &r) const {
  validate_type(v);
  COMPILE_ASSERT_POS(l->dtype_ == r->dtype_,
                     "The types of LHS and RHS should be the same: "
                         << l->dtype_ << " v.s. " << r->dtype_
                         << ", expr = " << v);
...
static bool validate_brgemm_dtype(sc_data_type_t dtype_A,
                                  sc_data_type_t dtype_B,
                                  sc_data_type_t dtype_C, bool has_postop) {
  if ((dtype_A == datatypes::f32 && dtype_B == datatypes::f32 &&
      ...
      (utils::is_one_of(dtype_A, datatypes::u8, datatypes::s8) &&
       ...
       (has_postop ? utils::is_one_of(dtype_C, datatypes::s32, datatypes::f32,
                                      datatypes::u8, datatypes::s8)
                   : dtype_C == datatypes::s32)) ||
      (dtype_A == datatypes::bf16 && dtype_B == datatypes::bf16 &&
       (has_postop ? utils::is_one_of(dtype_C, datatypes::f32, datatypes::bf16)
                   : dtype_C == datatypes::f32)) ||
      (dtype_A == datatypes::f16 && dtype_B == datatypes::f16 &&
       (has_postop ? utils::is_one_of(dtype_C, datatypes::f32, datatypes::f16)
                   : dtype_C == datatypes::f32))) {
    return true;
  }
  return false;
...
void validate_impl_t::check_indexing(const std::vector<expr> &idxvec,
                                     ...
                                     unsigned expected_dims) const {
  COMPILE_ASSERT_POS(
      idxvec.size() == expected_dims,
      "Indexing node should have the same dimemsion of the tensor "
      "element, expecting "
          << expected_dims << ", got " << idxvec.size() << ". expr = " << v);
  int cnt = 0;
  ...
  for (auto &idx : idxvec) {
    ...
    COMPILE_ASSERT_POS(idx->dtype_ != datatypes::boolean &&
                       ...
                       "The "
                           << cnt << "-th index of the indexing expr has type "
...
void validate_impl_t::view(indexing_c v) {
  ...
  if (v->ptr_.isa<tensorptr>()) {
    ...
    COMPILE_ASSERT_POS(
        v->ptr_.isa<tensor>(),
        "Indexing node is expecting a tensor/tensorptr as the ptr: " << v);
    tensor tnode = v->ptr_.static_as<tensor>();
    COMPILE_ASSERT_POS(v->dtype_.type_code_ == tnode->elem_dtype_.type_code_,
                       "Indexing node should have the same type of the tensor "
                       "element, "
                       "got "
                           << v->dtype_ << " and " << tnode->elem_dtype_
                           << ". expr = " << v);
...
void validate_impl_t::view(call_c v) {
  ...
  for (size_t i = 0; i < v->args_.size(); i++) {
    ...
    COMPILE_ASSERT_POS(ty1 == ty2, "Unmatched types for parameter "
                                       << i + 1 << " : given " << ty1
                                       << ", expecting " << ty2
                                       << ". Expr = " << v);
  }
...
void validate_impl_t::view(intrin_call_c v) {
  ...
  case intrin_type::reduce_min:
    validate_type(v);
    COMPILE_ASSERT_POS(v->args_.size() == 1, "reduce expects 1 argument, "
                                             "got "
                                                 << v);
    break;
  ...
  case intrin_type::permute:
    ...
    COMPILE_ASSERT_POS(v->args_.size() == 2,
                       "Shuffle/permute intrinsics take two parameters. Got "
                           << v);
    COMPILE_ASSERT_POS(
        v->args_[0]->dtype_ == v->args_[1]->dtype_ &&
            (v->intrin_attrs_->get_or_else("shuffle_imm", -1) != -1 ||
             v->intrin_attrs_->get_or_else("permute_imm", -1) != -1),
        "The types of the first two args should be the same and "
        "the third arg should be valid imm: "
            << v->args_[0]->dtype_ << " v.s. " << v->args_[1]->dtype_
            << ", expr = " << v);
    break;
  ...
  case intrin_type::insert:
    validate_type(v);
    if (v->args_.size() == 2) {
      COMPILE_ASSERT_POS(v->intrin_attrs_->get_or_null<int>("insert_imm") !=
                             nullptr,
  ...
  case intrin_type::gather:
    ...
    COMPILE_ASSERT_POS(
        v->args_[0]->dtype_.is_pointer() &&
            v->args_[0]->dtype_.get_pointer_element().is_etype(
                sc_data_etype::F32),
        "The first type of gather should be a pointer to f32. Got " << v);
  ...
  case intrin_type::brgemm: {
    ...
    if (extras.is_cpu_) {
      COMPILE_ASSERT_POS(validate_brgemm_dtype(
                             extras.dtype_A_, extras.dtype_B_, extras.dtype_C_,
    ...
    for (unsigned i = brgemm_args::C + 1; i < brgemm_args::NUM_FULL_ARGS_STRIDE;
         i++) {
      COMPILE_ASSERT_POS(v->args_[i]->dtype_ == brgemm_args::arg_types[i],
                         "The " << i
                                << "-th argument of brgemm does not match. "
                                   "Expecting"
  ...
  case intrin_type::list_brgemm: {
    ...
    for (unsigned i = 0; i < brgemm_args::NUM_FULL_ARGS_LIST; i++) {
      COMPILE_ASSERT_POS(v->args_[i]->dtype_ == brgemm_args::list_arg_types[i],
                         "The "
                             << i
...
void validate_impl_t::view(tensor_c v) {
  ...
  COMPILE_ASSERT_POS(
      v->dims_.size() == v->strides_.size(),
      "Expecting dims and strides having same length, but got dims "
      "length: "
          << v->dims_.size() << " and strides length: " << v->strides_.size());
  int cnt = 0;
  sc_data_type_t idxtype = v->dims_.front()->dtype_;
  for (auto &idx : v->dims_) {
    cnt += 1;
    COMPILE_ASSERT_POS(
        idx->dtype_ == datatypes::index || idx->dtype_ == datatypes::s32,
        "The " << cnt << "-th index of the tensor has type " << idx->dtype_
...
void validate_impl_t::check_var_tensor_def(const define_c &v,
                                           bool allow_tensor_view,
                                           bool is_global) const {
  ...
  if (v->init_.defined()) {
    if (!allow_tensor_view) {
      if (v->var_.isa<var>()) {
        ...
        COMPILE_ASSERT_POS(
            v->init_.isa<intrin_call>() &&
                utils::is_one_of(v->init_.static_as<intrin_call>()->type_,
  ...
  if (!is_global && v->var_.isa<tensor>()) {
    ...
    COMPILE_ASSERT_POS(
        init == nullptr || init == tensor_node::get_zero_tensor_initializer() ||
            init->size_ == sizeof(union_val) ||
            (v->var_->attr_ &&
             v->var_->attr_->has_key(attr_keys::shared_const)),
...
void validate_impl_t::view(if_else_c v) {
  ...
  COMPILE_ASSERT_POS(
      v->condition_->dtype_ == datatypes::boolean,
...
void validate_impl_t::view(for_loop_c v) {
  ...
  if (cur_func_) {
    defined_vars_.back().erase(v->var_.get());
  }
