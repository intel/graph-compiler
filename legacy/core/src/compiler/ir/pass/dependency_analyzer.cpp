...
public:
  ...
  const stmt_set *get_last_update_for_var(const var_c &v) {
    ...
    for (auto itr = state_stack.rbegin(); itr != state_stack.rend(); ++itr) {
  ...
  void add_to_dependency(stmt_base_t *depending, stmt_base_t *dependee) {
    get_dep_info(depending).depends_on_.insert(dependee->shared_from_this());
    get_dep_info(dependee).depended_by_.insert(depending->shared_from_this());
  }
  ...
  bool indexing_definitely_not_same(const indexing_c &v1,
                                    const indexing_c &v2) {
    assert(v1->idx_.size() == v2->idx_.size());
    for (unsigned i = 0; i < v1->idx_.size(); i++) {
      if (!v1->idx_[i].isa<constant>()) {
        return false;
      }
      if (!v2->idx_[i].isa<constant>()) {
        return false;
      }
      if (v1->idx_[i]->dtype_ != v2->idx_[i]->dtype_) {
        return false;
      }
    ...
    auto overlap =
        std::max(INT64_C(0), std::min(end1, end2) - std::max(idx1, idx2));
    return overlap == 0;
  }
  ...
  void add_tensor_depends_on_top_level_access(const tensor_c &tsr,
    ...
    if (itr != tsr_accesses.end()) {
      for (auto &access : itr->second) {
        ...
        if (v.defined()) {
          if (indexing_definitely_not_same(access.checked_as<indexing_c>(),
                                           v)) {
  ...
  void add_tensor_depends_on_loop_access(const tensor_c &tsr,
                                         const indexing_c &v) {
    ...
    if (itr != tsr_accesses_in_loop.end()) {
      for (auto &access : itr->second) {
        ...
        if (v.defined()) {
          ...
          } else if (indexing_same(access.checked_as<indexing_c>(), v)) {
            ...
            */
            if (loop_depth > 0 &&
                (def_loop_depth < loop_depth - 1 ||
                 !get_dep_info(v.get()).depends_on_.has(cur_loop_.impl))) {
              add_to_dependency(other_owner, cur_stmt.remove_const().get());
            }
          } else {
  ...
  void view(tensorptr_c v) override {
    ...
    v->base_->ptr_.checked_as<tensor_c>()
        .remove_const()
        ->attr()[attr_directly_accessed] = true;
  }
  ...
  void view(indexing_c v) override {
    ...
    if (loop_depth > 0) {
      tsr_accesses_in_loop[tsr].insert(v);
    }
