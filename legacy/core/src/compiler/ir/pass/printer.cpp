...
void ir_printer_t::view(constant_c v) {
  ...
  for (unsigned i = 0; i < v->value_.size(); i++) {
    ...
    case sc_data_etype::F32: {
      if (v->value_[i].f32 - static_cast<int>(v->value_[i].f32) == 0) {
...
void ir_printer_t::view(func_addr_c v) { os_ << '&' << v->func_->name_; }
...
public:
  ...
  ir_track_pos_printer_t(std::ostream &os) : ir_printer_t(theos_), theos_(os) {}
};
