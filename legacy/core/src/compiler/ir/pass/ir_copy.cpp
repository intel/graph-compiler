...
ir_copier_impl_t::ir_copier_impl_t(
    ...
    std::unordered_map<stmt_c, stmt> *stmt_replace_map, bool create_var_tensor)
...
void ir_copier_impl_t::view(indexing_c v) {
  ...
  returned_expr_ =
      builder::make_indexing(ptr, idx, v->dtype_.lanes_, mask, v->dtype_.rows_);
}

void ir_copier_impl_t::view(tensorptr_c v) {
  auto ret = make_expr<tensorptr_node>(copy(v->base_).checked_as<indexing>(),
                                       v->shape_, v->is_slice_);
...
void ir_copier_impl_t::view(call_c v) {
  ...
  returned_expr_ = make_expr<call_node>(
      callee, args, std::vector<call_node::parallel_attr_t>{v->para_attr_});
}
...
void ir_copier_impl_t::update_shrink_info(const expr_c &v, const expr &ret) {
  if (v->attr_) {
    if (auto shrink_info =
      ...
      ret->attr()[tensor_shrinker_attrs::should_shrink] = std::move(new_shrink);
    }
  }
}
...
void ir_copier_impl_t::view(tensor_c v) {
  ...
  returned_expr_ =
      builder::make_stensor(v->name_, args, strides, v->elem_dtype_,
...
void ir_copier_impl_t::view(stmts_c v) {
  ...
  if (v->attr_ &&
      v->attr_->has_key(tensor_shrinker_attrs::tensor_for_placerholder)) {
    auto tsr = v->attr_
                   ->get<std::weak_ptr<expr_base>>(
    ...
    auto &shrink_info = tsr_copy->attr_->get<tensor_shrinker_t::shrink_info_t>(
        tensor_shrinker_attrs::should_shrink);
...
void ir_copier_impl_t::view(if_else_c v) {
  returned_stmt_ = make_stmt<if_else_node_t>(
      copy(v->condition_), copy(v->then_case_),
...
void ir_copier_impl_t::view(returns_c v) {
  returned_stmt_ =
      make_stmt<returns_node_t>(v->value_.defined() ? copy(v->value_) : expr());
}
...
void ir_copier_impl_t::view(define_c v) {
  ...
  if (the_var.isa<tensor>()) {
    update_shrink_info(v->var_, the_var);
  }
}

void ir_copier_impl_t::view(for_loop_c v) {
  returned_stmt_ = make_stmt<for_loop_node_t>(
      copy(v->var_), copy(v->iter_begin_), copy(v->iter_end_), copy(v->step_),
      copy(v->body_), v->incremental_, v->kind_, v->num_threads_);
}

stmt_c ir_copier_impl_t::dispatch(stmt_c s) {
  if (find_and_return(s)) {
    return s;
  }
  ir_viewer_t::dispatch(s);
  if (stmt_replace_map_) {
    (*stmt_replace_map_)[s] = returned_stmt_;
  }
  return s;
}
...
func_c ir_copier_impl_t::dispatch(func_c v) {
  ...
  for (auto &i : v->params_) {
    ...
    if (new_param.isa<tensor>()) {
      update_shrink_info(i, new_param);
    }
