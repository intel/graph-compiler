...
struct extra_args_t {
  ...
  extra_args_t(const cpu_t &g, sc_data_type_t dtypeA,
      ...
      : is_cpu_(true), dtype_A_(dtypeA),
        dtype_B_(dtypeB == datatypes::undef ? dtypeA : dtypeB),
        dtype_C_(dtypeC == datatypes::undef ? dtypeA : dtypeC),
        brg_attrs_(brg_attrs), bd_mask_(bd_mask),
        bd_mask_set_num_(bd_mask_set_num), postops_setting_(brg_postops),
        cpu_(g) {}
  bool operator==(const extra_args_t &other) const {
    return is_cpu_ == other.is_cpu_ && dtype_A_ == other.dtype_A_ &&
           ...
           postops_setting_ == other.postops_setting_ && cpu_ == other.cpu_;
  }
  bool operator!=(const extra_args_t &other) const { return !(*this == other); }
};
