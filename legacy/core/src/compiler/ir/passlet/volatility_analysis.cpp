...
namespace gc {
namespace passlet {
static volatility_result_t::state_t
merge_state(volatility_result_t::state_t a, volatility_result_t::state_t b) {
  ...
  if (def->var_.isa<var>() && def->init_.defined() &&
    ...
    if (is_loop_phi || !expr_can_hoist(def->init_.get())) {
      ...
      auto callback = [ths, &is_volatile](array_ref<expr> v) {
        for (auto &val : v) {
          if (is_volatile == volatility_result_t::YES) {
            break;
          }
          ...
          if (val->ssa_data_->is_param_) {
            continue;
          }
          if (val.isa<constant>() || val.isa<tensor>()) {
            continue;
          }
          ...
          if (owner.isa<for_loop>()) {
            ...
            auto parent_is_volatile =
                ths->get_result(val->ssa_data_->get_owner().get())
                    ->is_volatile_;
            is_volatile = merge_state(is_volatile, parent_is_volatile);
          }
        }
      };
      if (def->init_.isa<var>() && def->init_->ssa_data_->is_global_) {
        ...
        get_direct_dependency_of_expr(def->init_->node_ptr_from_this(),
                                      callback);
      }
    }
