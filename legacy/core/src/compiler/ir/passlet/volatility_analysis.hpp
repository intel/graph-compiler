...
inline bool is_tensor_read_only(const expr_base *s) {
  return (s->node_type_ == sc_expr_type::tensor) &&
         any_map_t::fetch_or_else(s->attr_.get(), attr_keys::read_only_tensor,
                                  false);
}
...
inline bool non_volatile_expr(const expr_base *s) {
  ...
  default:
    if (dynamic_cast<const binary_node *>(s)) {
      return true;
    }
    if (dynamic_cast<const logic_node *>(s)) {
      return true;
    }
    if (dynamic_cast<const cmp_node *>(s)) {
      return true;
    }
    return false;
    break;
