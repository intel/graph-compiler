...
void structural_analysis_t::view(const stmt_c &v, pass_phase phase) {
  ...
  cur->parent_ =
      cur_parent_.back() ? cur_parent_.back()->shared_from_this() : nullptr;
...
bool structural_result_t::is_parent_of(
    const structural_result_t &other, const typed_addresser_t &addresser,
    bool allow_across_for, bool allow_across_if,
  ...
  if (out_second_level_parent) {
    *out_second_level_parent = nullptr;
  }
  for (;;) {
    if (cur_info == this) {
      return true;
    ...
    if (out_second_level_parent) {
      *out_second_level_parent = cur_info;
    }
    if (!allow_across_for) {
    ...
    if (!allow_across_if) {
      if (cur->node_type_ == sc_stmt_type::if_else) {
        return false;
      }
    }
...
const stmt_base_t *structural_result_t::find_shared_parent(
  ...
  for (;;) {
    if (cur_info->is_parent_of(other, addresser, allow_across_for,
                               allow_across_if)) {
      return cur;
    }
    if (!allow_across_for) {
