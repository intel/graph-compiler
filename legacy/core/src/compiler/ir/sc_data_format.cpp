...
sc_data_format_kind_t::sc_data_format_kind_t(
  ...
  uint64_t res =
      set_ith_int(0xffffffffffffffff, sc_data_format_kind_t::MAX_DIMS, 0);
...
sc_data_format_kind_t::get_2dblocking_by_dims(size_t ndims, bool is_weight,
                                              bool is_vnni_format) {
  COMPILE_ASSERT(ndims <= sc_data_format_kind_t::MAX_DIMS,
                 "storage size should be less than MAX_DIMS");
  uint64_t res =
...
runtime::dispatch_key sc_data_format_t::to_runtime() const {
  COMPILE_ASSERT(
      format_code_.ndims() < runtime::dispatch_key::meta::MAX_DIMS,
  ...
  return runtime::dispatch_key(static_cast<uint64_t>(format_code_), blocks_[0],
                               blocks_[1], impl_kind_t::normal,
...
static void
get_block_nums_for_axises(const sc_data_format_t &format,
...
void get_blocking_shapes_impl(const sc_dims &plain_shapes,
                              ...
                              size_t base_out_dim, size_t num_format_dims,
                              size_t num_out_dims,
...
sc_dims sc_data_format_t::get_blocking_shapes(const sc_dims &plain_shapes,
                                              const sc_data_format_t &format) {
  if (plain_shapes.empty()) {
    return sc_dims();
  }
  ...
  get_blocking_shapes_impl(plain_shapes, format, base_out_dim, num_format_dims,
                           num_out_dims, callback);
  return ret;
}
...
sc_dims
sc_data_format_t::get_padded_plain_shapes(const sc_dims &real_shapes,
                                          const sc_data_format_t &format) {
  if (real_shapes.empty()) {
    return sc_dims();
  }
  if (format.format_code_ == format_kinds::any) {
    return real_shapes;
...
sc_dims
sc_data_format_t::get_reordered_shapes(const sc_dims &input_shapes,
...
bool sc_data_format_cmper_t::operator()(const sc_data_format_t &fmt0,
                                        const sc_data_format_t &fmt1) const {
...
bool is_dynamic_blocking(const sc_dims &shapes,
                         const sc_data_format_t &format) {
  auto &code = format.format_code_;
  for (size_t i = 0; i < shapes.size(); i++) {
    if (is_dynamic_dim(shapes[i]) && !code.collect_blocking_index(i).empty()) {
      return true;
    }
  }
  return false;
