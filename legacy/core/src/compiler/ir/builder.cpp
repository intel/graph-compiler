...
expr copy_attr(const expr_base &ths, expr &&newexpr) {
  if (ths.attr_) {
    merge_attrs(newexpr->attr_, ths.attr_);
  }
...
stmt copy_attr(const stmt_base_t &ths, stmt &&newstmt) {
  if (ths.attr_) {
    merge_attrs(newstmt->attr_, ths.attr_);
  }
...
func_t copy_attr(const func_base &ths, func_t &&newfunc) {
  if (ths.attr_) {
    merge_attrs(newfunc->attr_, ths.attr_);
  }
...
stmt get_common_parent_node(const stmt &node1, const stmt &node2) {
  ...
  if (!stru_res1 || !stru_res2)
    return stmt();
  return stmt{
      const_cast<stmt_base_t *>(
          stru_res1->find_shared_parent(
              *stru_res2,
              [](passlet::passlet_t *ths,
                 const node_base *v) -> passlet::structural_result_t * {
                if (!v->attr_)
                  return nullptr;
                return v->attr_->get_or_null<passlet::structural_result_t>(
                    "builder.parent_node");
...
builder_impl_t *get_current_builder() { return cur_builder; }

void set_current_builder(builder_impl_t *b) { cur_builder = b; }

expr make_constant(float val) { return make_expr<constant_node>(val); };

expr make_constant(int32_t val) {
  return make_expr<constant_node>(static_cast<int64_t>(val));
};

expr make_constant(uint64_t val) { return make_expr<constant_node>(val); };
...
func_t make_func(const std::string &name, const std::vector<expr_c> &params,
  ...
  return make_func(name, vector_remove_const(params), body.remove_const(),
                   ret_type);
}
...
expr builder_impl_t::make_str(const std::string &str) {
  ...
  expr ret =
      make_tensor("_str_0_" + std::to_string(++counter),
...
expr tensor_ptr(const expr &tens, const std::vector<expr> &idx,
  ...
  COMPILE_ASSERT(
      tens.isa<tensor>() || tens.isa<tensorptr>(),
  ...
  return make_expr<tensorptr_node>(make_expr<indexing_node>(tens, idx, expr()),
                                   *real_shape, is_slice);
}
...
expr tensor_ptr(const expr &tensor, std::initializer_list<expr> idx,
  ...
  return tensor_ptr(tensor, std::vector<expr>(idx), std::vector<expr>(shape),
                    is_slice);
}
      ...
      return copy_attr(
          *original,
          make_expr<intrin_call_node>(
              orig_type, std::vector<expr>{l.remove_const(), r.remove_const()},
              *original.static_as<intrin_call>()->intrin_attrs_));
    default:
      assert(0 && "Bad op for remake_binary");
    }
  }
  switch (original->node_type_) {
  case sc_expr_type::add:
    return copy_attr(*original, make_add(l, r));
  case sc_expr_type::sub:
    return copy_attr(*original, make_sub(l, r));
  case sc_expr_type::mul:
    return copy_attr(*original, make_mul(l, r));
  case sc_expr_type::div:
    return copy_attr(*original, make_div(l, r));
  case sc_expr_type::mod:
    return copy_attr(*original, make_mod(l, r));
...
#define GEN_BINARY(name)                                                       \
  expr make_##name(const expr_c &left, const expr_c &right) {                  \
    return make_expr<name##_node>(left.remove_const(), right.remove_const());  \
  };
...
expr make_reinterpret(const expr_c &v, sc_data_type_t dtype) {
  ...
  auto ret = make_expr<intrin_call_node>(
      intrin_type::reinterpret, std::vector<expr>{v.remove_const()}, attr);
  return ret;
}

expr make_isnan(const expr_c &v) {
  auto ret = make_expr<intrin_call_node>(
      intrin_type::isnan, std::vector<expr>{v.remove_const()}, any_map_t());
...
expr make_saturated_cast(const expr_c &v, sc_data_type_t dtype) {
  ...
  attr[intrin_attr::out_dtype] = dtype;
  auto ret = make_expr<intrin_call_node>(
      intrin_type::saturated_cast, std::vector<expr>{v.remove_const()}, attr);
  return ret;
}

expr make_round_and_cast(const expr_c &v, sc_data_type_t dtype) {
  return make_expr<intrin_call_node>(
      intrin_type::round_and_cast, std::vector<expr>{v.remove_const()},
...
expr make_abs(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::abs, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_round(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::round, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_floor(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::floor, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_ceil(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::ceil, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_exp(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::exp, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_log(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::log, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_erf(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::erf, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_sqrt(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::sqrt, std::vector<expr>{v.remove_const()}, any_map_t());
}

expr make_rsqrt(const expr_c &v) {
  return make_expr<intrin_call_node>(
      intrin_type::rsqrt, std::vector<expr>{v.remove_const()}, any_map_t());
}
...
expr make_permutex2var(const expr_c &v_a, const expr_c &v_b,
                       const expr_c &v_c) {
...
expr make_extract(const expr_c &v_a, const int imm, const int lanes) {
  return make_expr<intrin_call_node>(
      intrin_type::extract, std::vector<expr>{v_a.remove_const()},
      any_map_t{{"extract_imm", imm}, {"lanes", lanes}});
}

static expr flatten_2d_index(const expr_c &v_a, const expr_c &row,
                             const expr_c &col) {
...
expr make_extract(const expr_c &v_a, const expr_c &row, const expr_c &col,
  ...
  return make_expr<intrin_call_node>(
      intrin_type::extract,
      std::vector<expr>{v_a.remove_const(), flatten_2d_index(v_a, row, col)},
...
expr make_read_struct(const expr_c &in, const std::string &struct_name,
  ...
  return make_expr<intrin_call_node>(
      intrin_type::read_struct, std::vector<expr>{in.remove_const()},
...
expr make_get_group_id(uint64_t par_for_level_id) {
  return make_expr<intrin_call_node>(intrin_type::get_group_id,
                                     std::vector<expr>{par_for_level_id},
                                     any_map_t());
}
...
expr make_select(const expr_c &cond, const expr_c &l, const expr_c &r) {
  return make_expr<select_node>(cond.remove_const(), l.remove_const(),
                                r.remove_const());
}
...
expr make_indexing(const expr_c &ptr, const std::vector<expr_c> &idx,
  ...
  return make_expr<indexing_node>(ptr.remove_const(), vector_remove_const(idx),
                                  length, rows, mask.remove_const());
}

expr make_indexing(const expr &ptr, std::initializer_list<expr> idx,
                   uint16_t length, const expr &mask, uint16_t rows) {
  return make_expr<indexing_node>(ptr, std::vector<expr>(idx), length, rows,
                                  mask);
}
...
expr make_tensor(const std::string &name, const std::vector<expr_c> &dims,
  ...
  return make_expr<tensor_node>(dtype, name, vector_remove_const(dims),
                                addrspace, init_value,
                                vector_remove_const(strides));
}
...
stmt builder_impl_t::basic_block_t::get() { return std::move(body); }
...
void builder_impl_t::push_scope() { scopes.emplace_back(); }
...
stmt make_var_tensor_def_unattached(const expr_c &var, linkage l,
                                    const expr_c &init) {
...
stmt make_for_loop_unattached(const expr_c &var, const expr_c &iter_begin,
                              ...
                              for_type kind, int num_threads) {
  return make_stmt<for_loop_node_t>(
      var.remove_const(), iter_begin.remove_const(), iter_end.remove_const(),
      step.remove_const(), body.remove_const(), incremental, kind, num_threads);
}
...
stmt builder_impl_t::push_if_else(const expr &condition_,
  ...
  if (else_case_.defined())
    add_parent_node(else_case_, ret);
  emit(ret);
  return ret;
}
...
stmt builder_impl_t::push_var_tensor_def(const expr &var, linkage l,
                                         const expr &init) {
...
stmt builder_impl_t::push_for_loop(const expr &var_, const expr &iter_begin_,
                                   const expr &iter_end_, const expr &step_,
                                   const stmt &body_, bool incremental_,
                                   for_type kind, int num_threads) {
...
stmt builder_impl_t::brgemm(
    const expr_c &x, const expr_c &w, const expr_c &y, const expr_c &blocks,
    const expr_c &M, const expr_c &N, const expr_c &K, const expr_c &ldx,
    const expr_c &ldw, const expr_c &ldy, const expr_c &x_block_stride,
    const expr_c &w_block_stride, const std::vector<expr> &postops_data,
    const expr_c &c_buf, const expr_c &bd_mask_idx, const expr_c &top_pad,
    const expr_c &bottom_pad, const brgemm_args::extra_args_t &extras) {
  auto args = std::vector<expr>{x.remove_const(),
                                w.remove_const(),
                                y.remove_const(),
                                blocks.remove_const(),
                                M.remove_const(),
  ...
  return push_evaluate(make_expr<intrin_call_node>(
      intrin_type::brgemm, args,
      any_map_t{{intrin_attr::brgemm_extras, extras}}));
}

stmt builder_impl_t::list_brgemm(
    const expr_c &x, const expr_c &w, const expr_c &y, const expr_c &blocks,
    const expr_c &M, const expr_c &N, const expr_c &K, const expr_c &ldx,
    const expr_c &ldw, const expr_c &ldy, const expr_c &x_block_stride,
    const expr_c &w_block_stride, const expr_c &len,
    const std::vector<expr> &postops_data, const expr_c &c_buf,
    const expr_c &bd_mask_idx, const expr_c &top_pad, const expr_c &bottom_pad,
