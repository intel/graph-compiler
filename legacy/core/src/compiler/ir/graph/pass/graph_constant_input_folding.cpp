...
size_t
graph_weak_ptr_hasher::operator()(const std::weak_ptr<sc_graph_t> &v) const {
  auto g = v.lock();
  if (!g) {
    return 0;
  }
...
bool graph_weak_ptr_cmper::operator()(
    const std::weak_ptr<sc_graph_t> &v1,
...
struct shared_global_data_allocator_t {
  ...
  size_t needed_allocation_size(
      const std::vector<std::shared_ptr<cached_const_graph_tensor>> &cache,
  ...
  alloc(const std::vector<std::shared_ptr<cached_const_graph_tensor>> &cache,
        const std::vector<void *> &existing_data, runtime::engine_t *engine) {
    size_t ret_lazy = 0, ret_compile_time_const = 0;
    ret_lazy =
        needed_allocation_size(cache, existing_data, ret_compile_time_const);
    if (ret_lazy + ret_compile_time_const) {
      ...
      if (ret_lazy) {
        ...
        auto base =
            engine->vtable_->alloc_and_register_tensor_cache(engine, ret_lazy);
        size_t offset = 0;
      ...
      if (ret_compile_time_const) {
        ...
        std::shared_ptr<void> baseptr = std::shared_ptr<void>{
            engine->vtable_->persistent_alloc(engine, ret_compile_time_const),
            [engine](void *p) {
        ...
        for (size_t idx = 0; idx < cache.size(); idx++) {
          ...
          if (!v->buf_base_) {
            v->buf_base_ = base;
            v->offset_ = offset;
            memcpy((char *)baseptr.get() + offset, existing_data[idx],
                   v->size_);
            offset += utils::divide_and_ceil(v->size_, 64) * 64;
            SC_MODULE_INFO << "Alloc buffer for " << v
                           << ", offset=" << v->offset_;
...
struct const_graph_tensor_cache {
  ...
  add_tensor(const std::shared_ptr<sc_graph_t> &dep_graph, size_t buf_size,
    ...
    auto ret = std::make_shared<cached_const_graph_tensor>(dep_graph, buf_size,
                                                           get_cache());
    // insert into graph map
    auto graph_iter = from_dep_graph_.insert(std::make_pair(
        dep_graph,
...
SC_INTERNAL_API void graph_constant_input_folding_impl(sc_graph_t &mgr,
    ...
    if (node->isa<constant_op_t>() || cur_const_state) {
      ...
      if (!all_constant_inputs) {
        for (const auto &input : node->get_inputs()) {
          ...
          if (parent_node->attrs_.get_or_else(
                  "constant", const_kind::not_const) != const_kind::not_const) {
            parent_node->attrs_.set("constant", const_kind::global_const);
            ...
            if (parent_node->isa<tensor_view_op_t>()) {
              ...
              tv_in->attrs_.set("constant", const_kind::global_const);
        ...
        if (!node->isa<output_op>()) {
          ...
          for (auto &out : node->get_outputs()) {
            for (auto &cld_node : out->uses_) {
              cld_node.second->attrs_.set("constant", const_kind::local_const);
            }
          }
        }
      }
  ...
  if (share_constants && !edge_ops.empty() && ctx->flags_.const_share_) {
    ...
    for (auto op : edge_ops) {
      ...
      if (is_all_tensor_view && op->isa<tensor_view_op_t>()) {
        continue;
      }
      // the selector for graph visitor to visit sub graph for the
      // current op. Try to normalize the visiting order by hash of
      // the ops
      auto selector = [&op_mask, &hash_cache](op_visitor_t *v) -> sc_op_ptr {
        std::list<sc_op_ptr>::iterator theitr;
        size_t max_hash = 0;
        bool found = false;
        for (auto itr = v->to_visit_.begin(); itr != v->to_visit_.end();) {
          auto &cur = *itr;
      ...
      op_visitor_t visitor{
          selector, op_visitor_t::create_DAG_updater(mgr.ops_.size()), false};
      std::vector<size_t> depending_ops;
      visitor.to_visit_ = std::move(depending_inputs);
      visitor.visit([&depending_ops](op_visitor_t *v, const sc_op_ptr &p) {
      ...
      for (size_t i = 0; i < op->get_outputs().size(); i++) {
        ...
        auto buf_size = op->get_outputs()[i]->details_.get_blocking_byte_size();
        if (buf_size <= 8UL) {
          // skip scalars, don't register it in the cache manager
          auto cached_tsr = std::make_shared<cached_const_graph_tensor>(
        ...
        for (auto &depop_id : depending_ops) {
          ...
          if (theop->isa<input_op>()) {
            newops[depop_id] = g->make_input(
                copy_logical_tsr(theop->get_outputs()), theop->attrs_);
            newops[depop_id]->attrs_["constant_input_id"] =
                theop->attrs_["temp.tensor_id"];
          } else {
            auto copyable = theop->dyn_cast<op_traits::copyable_t>();
            COMPILE_ASSERT(copyable, "The const cache ops must be copyable");
            std::vector<graph_tensor_ptr> newins;
            auto &oldins = theop->get_inputs();
            for (size_t in_idx = 0; in_idx < oldins.size(); in_idx++) {
              ...
              auto idx =
                  std::find(oldowner->get_outputs().begin(),
                            oldowner->get_outputs().end(), oldins[in_idx]) -
                  oldowner->get_outputs().begin();
              newins.emplace_back(mapped_op->get_outputs()[idx]);
            }
            newops[depop_id] = copyable->copy(
                newins, copy_logical_tsr(theop->get_outputs()), *g);
          }
        }
        auto &lastop = newops[op->logical_op_id_];
        g->make_output({lastop->get_outputs()[i]});
        results.emplace_back(get_cache()->add_tensor(
            g, op->get_outputs()[i]->details_.get_blocking_byte_size(),
            ctx->engine_));
        ...
        if (auto sc_stream_temp =
          ...
          (*sc_stream_temp.stream_)
              ...
              << ", uid=" << (void *)results.back()->id_iter_->first
              << ", size=" << results.back()->size_ << ", dep =\n";
      ...
      cached_op->attrs_[op_attr_key::const_input_cache] = std::move(results);
    }
    get_cache()->alloca_.alloc(caches, existing_data_vec, ctx->engine_);
  }
}

SC_INTERNAL_API void graph_constant_input_folding(sc_graph_t &mgr,
                                                  const context_ptr &ctx) {
