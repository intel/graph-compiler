...
bool compare_graph(
  ...
  if (lhs_rhs_input_mapping.empty()) {
    ...
    if (inops_lhs.size() != inops_rhs.size()) {
      return false;
    }
...
#define DO_RETURN                                                              \
  ...
  visitor.visit_graph(lhs, [&](op_visitor_t *visitor, const sc_op_ptr &lnode) {
    sc_op_ptr rnode;
    ...
    for (size_t i = 0; i < lhs_outputs.size(); i++) {
      ...
      if (lhs_uses.size() != rhs_uses.size()) {
        DO_RETURN;
      }
      // 4. op mapping
      for (size_t u = 0; u < lhs_uses.size(); u++) {
        ...
        if (lhs_use.first != rhs_use.first) {
          DO_RETURN;
        }
        ...
        // false
        if (mapped_id != -1 && mapped_id != rhs_use.second->logical_op_id_) {
          DO_RETURN;
        }
