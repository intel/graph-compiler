...
static void print_shapes(const std::string &name,
                         runtime::dynamic_tensor_t *in) {
  std::stringstream ss;
...
SC_API void dynamic_infer_shape_by_graph(sc_graph_t &graph,
  ...
  if (graph.empty() || !graph.is_dynamic()) {
    return;
  }
  ...
  auto get_or_create_dyn_tsr = [&ltsr_dtsr, &inner_dyn_tsr,
                                &inner_shape_tsr](const graph_tensor_ptr &in) {
  ...
  vis.visit_graph(graph, [&get_or_create_dyn_tsr](op_visitor_t *vis,
    ...
    if (node->isa<input_op>() || node->isa<output_op>()) {
      ...
      auto dyn_conv_info =
          data->ndims_ == 4
              ? dyn_conv_fwd_runtime_info_t(stride[0], stride[1], pads_begin[0],
                                            pads_begin[1], pads_end[0],
                                            pads_end[1], dilation_dims[0],
                                            dilation_dims[1])
              : dyn_conv_fwd_runtime_info_t(
                    stride[0], stride[1], stride[2], pads_begin[0],
                    pads_begin[1], pads_begin[2], pads_end[0], pads_end[1],
                    pads_end[2], dilation_dims[0], dilation_dims[1],
                    dilation_dims[2]);
      ...
      print_shapes(node->op_name_, out);
    } else if (node->isa<reduce_op_t>() || node->isa<reduce_mean_op_t>()) {
      auto *in = get_or_create_dyn_tsr(node->get_inputs()[0]);
      auto *out = get_or_create_dyn_tsr(node->get_outputs()[0]);
      ...
      auto input_plain_dims = node->get_inputs()[0]->details_.get_plain_dims();
      ...
      std::string auto_pad = pooling_op->attrs_.get_or_else<std::string>(
      ...
      auto dyn_info =
          data->ndims_ == 5
              ? dyn_pooling_runtime_info_t(
                    pooling_op->stride_[0], pooling_op->stride_[1],
                    pooling_op->stride_[2], pooling_op->pads_begin_[0],
                    pooling_op->pads_begin_[1], pooling_op->pads_begin_[2],
                    pooling_op->pads_end_[0], pooling_op->pads_end_[1],
                    pooling_op->pads_end_[2], pooling_op->kernel_[0],
                    pooling_op->kernel_[1], pooling_op->kernel_[2],
                    rounding_type_floor, auto_pads_same)
              : dyn_pooling_runtime_info_t(
                    pooling_op->stride_[0], pooling_op->stride_[1],
                    pooling_op->pads_begin_[0], pooling_op->pads_begin_[1],
                    pooling_op->pads_end_[0], pooling_op->pads_end_[1],
                    pooling_op->kernel_[0], pooling_op->kernel_[1],
                    rounding_type_floor, auto_pads_same);
