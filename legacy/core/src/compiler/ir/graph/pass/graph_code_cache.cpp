...
struct prehashed_graph_for_code_share_t {
  ...
  static bool attr_filter(const sc_op *op, const std::string &attr) {
    if (attr == op_attr_key::const_input_cache) {
      return false;
    }
    ...
    if (ignorable_keys.find(attr) != ignorable_keys.end()) {
      return false;
    }
    return true;
  }
  ...
  prehashed_graph_for_code_share_t(const context_ptr &ctx, sc_graph_t &&g,
                                   ...
                                   size_t ghash)
      : ctx_(ctx), g_(std::move(g)), base_tensor_id_(base_tensor_id),
        offset_(offset) {
    hash_ = ghash;
  ...
  bool operator==(const prehashed_graph_for_code_share_t &other) const {
    ...
    if (base_tensor_id_ != other.base_tensor_id_) {
      return false;
    }
...
struct graph_code_cache_handle {
  ...
  graph_code_cache_handle(const std::weak_ptr<jit_module_code> &code,
      ...
      : code_(code), module_data_template_(std::move(module_data)),
        deletion_flag_(std::make_shared<bool>(false)), mgr_(mgr) {}
...
prepare_static_table_for_cached_code(graph_code_cache_handle &v,
  ...
  if (auto bases = orig_graph.attrs_.get_or_null<
          ...
          "shared_const_bases")) {
    if (!bases->empty()) {
      void **phandles = (void **)ret.get_or_null(shared_const_handle_name);
      ...
      ret.shared_tensors_ =
          orig_graph.attrs_
              .get<std::vector<std::shared_ptr<cached_const_graph_tensor>>>(
...
struct graph_code_cache_manager {
  ...
  std::shared_ptr<jit_module_code>
  internal_query(const prehashed_graph_for_code_share_t &v) {
    ...
    if (itr != map_.end()) {
      ...
      auto ret =
          some_opt(itr->second.v_.lock())
              .map([](const std::shared_ptr<graph_code_cache_handle> &v) {
                return v->code_.lock();
              })
              .get_or_else(nullptr);
      if (!ret) {
  ...
  std::shared_ptr<jit_module_code>
  query(const prehashed_graph_for_code_share_t &v) {
    std::lock_guard<std::mutex> guard{lock_};
    return internal_query(v);
  }

  std::shared_ptr<graph_code_cache_handle>
  insert(prehashed_graph_for_code_share_t &&v, const jit_module &m) {
    std::lock_guard<std::mutex> guard{lock_};
    if (internal_query(v)) {
    ...
    auto graph_iter = map_.insert(std::make_pair(
        std::move(v), graph_code_cache_value_t{value, value->deletion_flag_}));
    assert(graph_iter.second);
    value->iter_ = graph_iter.first;
    m.code_->graph_cache_handle_ = value;
    SC_MODULE_INFO << "Putting into code cache, " << graph_iter.first->first;
...
graph_code_cache_handle::~graph_code_cache_handle() { mgr_->remove(*this); }
...
SC_INTERNAL_API void graph_code_cache(sc_graph_t &mgr, const context_ptr &ctx) {
  ...
  op_visitor_t::dfs_topology_sort(mgr.ops_.size())
      .visit_graph(mgr, [&](op_visitor_t *, const sc_op_ptr &op) {
        if (auto buffer =
                op->attrs_.get_or_null<
                    std::vector<std::shared_ptr<cached_const_graph_tensor>>>(
                    op_attr_key::const_input_cache)) {
          for (auto &buf : *buffer) {
            shared_tsr.emplace_back(buf);
            auto itr = std::find(bases.begin(), bases.end(), buf->buf_base_);
            if (itr != bases.end()) {
              base_ids.push_back(static_cast<size_t>(itr - bases.begin()));
            } else {
  ...
  auto ghash = mgr.hash_contents(prehashed_graph_for_code_share_t::attr_filter);
  prehashed_graph_for_code_share_t key{
      ctx, std::reference_wrapper<const sc_graph_t>(mgr), base_ids, offsets,
      ghash};
  ...
  for (auto &op : new_graph.ops_) {
    ...
    for (auto itr = attrs.begin(); itr != attrs.end();) {
      if (utils::string_startswith(itr->first, "temp.") ||
          !prehashed_graph_for_code_share_t::attr_filter(op.get(),
                                                         itr->first)) {
        itr = attrs.erase(itr);
        continue;
      }
