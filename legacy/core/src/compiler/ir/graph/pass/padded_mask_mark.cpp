...
const std::unordered_set<std::string> ops_need_mask = {"exp", "select", "add",
                                                       "sub", "div",    "max"};
static int need_op_mask(const sc_op_ptr &op) {
  int pre_mask_type =
      op->attrs_.get_or_else("temp.mask_type", (int)mask_etype::no_need_mask);
  if (ops_need_mask.find(op->op_name_) != ops_need_mask.end()) {
    ...
    // f(0, 0) = 0
    if (utils::is_one_of(op->op_name_, std::string("add"), std::string("sub"),
                         std::string("max"))) {
...
static void mark_mask_attr(const sc_op_ptr &op) {
  ...
  if (op->get_outputs()[0]->details_.get_format().is_blocking() && mask_type) {
    for (auto &use : uses) {
      ...
      if (use.second->isa<movement_op_t>()) {
        if (use.second->isa<reorder_op_t>() &&
            use.second->get_inputs()[0]->details_.get_format().is_blocking() &&
            use.second->get_outputs()[0]->details_.get_format().is_plain()) {
          continue;
        }
        if (use.second->isa<broadcast_op_t>()) {
          continue;
        }
        op->attrs_.set(op_attr_key::use_padded_mask, true);
        return;
