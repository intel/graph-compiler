...
using graph_weak_ptr_map =
    std::unordered_map<std::weak_ptr<sc_graph_t>,
                       flaged_cached_const_graph_tensor_t,
                       graph_weak_ptr_hasher, graph_weak_ptr_cmper>;

using tensor_id_map =
    std::unordered_map<uint64_t, std::weak_ptr<cached_const_graph_tensor>>;
...
struct cached_const_graph_tensor {
  ...
  cached_const_graph_tensor(
      const std::shared_ptr<sc_graph_t> &dep, size_t buf_size,
