...
void op_dispatch_key_t::set_op_dispatch_key(const sc_op_ptr &node,
...
op_dispatch_key_t::convert_to_runtime_format_vec() const {
  ...
  for (size_t i = 0; i < in_out_formats_.size(); i++) {
    ...
    outs[i] = runtime::dispatch_key(
        static_cast<uint64_t>(in_out_formats_[i].format_code_), block0, block1,
        impl_, in_out_formats_[i].is_plain());
  }
  return outs;
}

void impl_op_dispatch_key_t::set_op_dispatch_key(const sc_op_ptr &node,
                                                 const context_ptr &ctx) const {
...
bool dispatch_key_cmper_t::operator()(const op_dispatch_key_t &key0,
                                      const op_dispatch_key_t &key1) const {
  if (key0.impl_ != key1.impl_) {
    return key0.impl_ < key1.impl_;
  }
  ...
  for (size_t i = 0; i < key0.in_out_formats_.size(); i++) {
    ...
    if (key0.in_out_formats_[i].blocks_ != key1.in_out_formats_[i].blocks_) {
      return key0.in_out_formats_[i].blocks_ < key1.in_out_formats_[i].blocks_;
    }
  }
...
bool impl_dispatch_key_cmper_t::operator()(
    const impl_op_dispatch_key_t &key0,
...
combined_dispatch_key_set_t::combined_dispatch_key_set_t(
  ...
  if (!dispatch_sets.empty()) {
    internal_construct(dispatch_sets);
  }
}
...
void recursive_construct(combined_dispatch_key_set_t::inner_set_t &set,
                         ...
                         size_t cur_op_idx, size_t len_key,
  ...
  for (auto it = inner_set.begin(); it != inner_set.end(); it++) {
    ...
    recursive_construct(set, cur_combined_key, op_link_relations, dispatch_sets,
                        inputs, cur_op_idx + 1, len_key, linked_reorder_impl);
    cur_combined_key.keys_.pop_back();
  }
}

static bool
validate_dispatch_key_sets(const std::vector<dispatch_set_ptr> &dispatch_sets,
                           const std::vector<sc_op_ptr> &inputs) {
      ...
      auto get_impl_kind_num = [](const dispatch_set_ptr &dispatch_set) {
        ...
        for (auto &it : set) {
          if (it.impl_ == impl_kind_t::normal) {
            ...
            throw std::runtime_error("Wrong impl kind for reorder.");
          }
          if (has_normal && has_padding) {
            return 2;
          }
        }
        if (has_normal || has_padding) {
...
void combined_dispatch_key_set_t::internal_construct(
  ...
  if (!inputs.empty()) {
    op_link_relations =
        get_op_layout_link_relationships(inputs, dispatch_sets, modified_inp);
  ...
  if (!is_specific && set_.size() > DISPATCH_KEY_MAX_THRESHOLD) {
    SC_MODULE_WARN << "Number of dispatch key set " << set_.size()
                   << " has exceeded threshold " << DISPATCH_KEY_MAX_THRESHOLD;
  }
}
...
impl_dispatch_key_set_t::get_inner_set() {
  throw std::runtime_error("Impl disaptch key set can not get its inner set.");
}
...
std::vector<int> get_default_impl_dispatch_candidates() {
  static std::vector<int> default_impl_candidates = {impl_kind_t::normal,
                                                     impl_kind_t::no_padding};
  return default_impl_candidates;
}

std::vector<int> get_dynamic_impl_dispatch_candidates(tunable_op_t *op,
