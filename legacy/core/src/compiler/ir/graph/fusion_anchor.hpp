...
struct anchor_content_cmper {
  bool operator()(const anchor_content_t &v, const anchor_content_t &v2) const {
...
public:
  ...
  fusion_anchor_t(stmts pos, const fslice_map &fsmap,
      ...
      : is_input_anchor_(is_input_anchor), anchor_position_(std::move(pos)),
        fsmap_(std::move(fsmap)) {
    if (parent) {
      attach_parent_anchor(parent, nullptr, true);
    }
  };
  ...
  void append_contents(std::vector<anchor_content_t> contents, size_t num_id) {
  ...
  template <typename T> T *stc_cast() { return static_cast<T *>(this); }
  ...
  bool is_parent_for(const fusion_anchor_t *cur) const {
    if (!cur)
      return false;
    if (is_input_anchor_ != cur->is_input_anchor_)
      return false;
    while (cur->parent_) {
      cur = cur->parent_.get();
      if (cur == this)
  ...
  bool is_sibling_for(const fusion_anchor_t *other) const {
    if (is_input_anchor_ != other->is_input_anchor_)
      return false;
    if (is_parent_for(other))
      return false;
    ...
    while (parent_loop.defined()) {
      if (this_loop.ptr_same(parent_loop)) {
        return true;
      }
  ...
  bool is_cousin_for(const fusion_anchor_t *cur) const {
    ...
    return !(this->is_parent_for(cur) || cur->is_parent_for(this) ||
             this->is_sibling_for(cur) || cur->is_sibling_for(this)) &&
           (cur->get_root() == this->get_root() ||
            get_common_parent_node(anchor_position_, cur->anchor_position_)
                .defined());
  }
  ...
  virtual bool validate_output_for_op(const sc_op *op) {
    ...
    return !op->is_dynamic() ||
           std::all_of(op->get_outputs().begin(), op->get_outputs().end(),
...
public:
  iter_fusion_anchor_t(expr iter_var, stmts pos, const fslice_map &fsmap,
    ...
    COMPILE_ASSERT(!fsmap.empty(),
                   "iterated fusion anchor init slice not found")
    iter_size_ = fsmap.datamap_.begin()->second.size();
    COMPILE_ASSERT(
        std::all_of(fsmap.datamap_.begin(), fsmap.datamap_.end(),
  ...
  bool validate_output_for_op(const sc_op *op) override {
    ...
    return std::all_of(op->get_outputs().begin(), op->get_outputs().end(),
                       [&ths](const graph_tensor_ptr &out) {
                         return ths->get_iter_size() ==
                                ths->fsmap_.get(out).size();
                       });
  }
};
...
public:
  ...
  grouped_fusion_anchor_t(stmts pos, const fslice_map &fsmap,
    ...
    COMPILE_ASSERT(pos->seq_.size() == group_size_,
                   "grouped anchor position "
                       << pos->seq_.size() << " should be equal to group size "
                       << group_size_)
    group_cnt_ = 0;
  }
