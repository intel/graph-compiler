...
for_loop get_next_inner_loop(const for_loop &cur_loop) {
  if (cur_loop->body_.isa<for_loop>()) {
    ...
    return cur_loop->body_.checked_as<stmts>()->seq_[0].checked_as<for_loop>();
  }
  return for_loop();
}
...
move_reduce_axis_to_inner(const context_ptr &ctx,
                          const std::vector<int> &in_axis, sc_graph_t &graph,
                          const graph_tensor_ptr &base_gt) {
  if (graph.is_dynamic() || !axis_can_be_sort(graph)) {
    return in_axis;
  }
 ...
 * */
static std::vector<int>
move_pooling_axis_to_outer(const context_ptr &ctx,
                           const std::vector<int> &in_axis, sc_graph_t &graph,
                           const graph_tensor_ptr &base_gt) {
  if (graph.is_dynamic() && !axis_can_be_sort(graph)) {
    return in_axis;
  ...
  if (!is_optimized_sub_graph(graph))
    return in_axis;
  auto run_threads = runtime_config_t::get().get_num_threads();
  // auto skip
  if (run_threads == 1)
...
static std::vector<int> continuous_access_satisfaction(
    const context_ptr &ctx, const std::vector<int> &in_axis, sc_graph_t &graph,
    const graph_tensor_ptr &base_gt) {
  ...
  while (fill_up_dim > 0 && cur_load_size * dtype_size < cache_line_size) {
    fill_up_dim--;
    cur_load_size =
        cur_load_size * (base_dims[fill_up_dim].isa<constant>()
                             ? get_expr_as_int(base_dims[fill_up_dim])
                             : 1);
  }
...
bool anchor_loop_generator_t::create_outer_loop_anchor(
  ...
  if (numdims > 1) {
    ...
    if (need_split_loop) {
      cur_tsr_slice[max_ax] = std::make_pair(loop_vars[max_ax] * lanes, lanes);
    }
    for (size_t i = 0; i < numdims - 1; i++) {
      ...
      auto loop =
          bld->push_for_loop(loop_vars[upper_loop_num], 0,
    ...
    if (base_dims[0].isa<constant>()) {
      auto only_dim = get_const_as_int(base_dims[0].static_as<constant>());
      auto lanes = vectorize_step(ctx, base_gt_->details_.dtype_.type_code_);
      if ((only_dim % lanes == 0) && (only_dim > lanes)) {
        ...
        auto body = bld->pop_scope();
        auto loop =
            bld->push_for_loop(loop_vars[0], 0, base_dims[0], expr((int)lanes),
...
bool anchor_loop_generator_t::create_inner_loop_anchor(
    fusion_anchor_mgr_t *fmgr, const fusion_anchor_ptr &parent_fanchor) const {
  COMPILE_ASSERT(parent_fanchor, "parent anchor must be set")
  // do not support multi-slice
  if (parent_fanchor->fsmap_.get(base_gt_).size() != 1)
    return false;
  auto bld = builder::get_current_builder();
  std::vector<expr> loop_vars;
  ...
  for (size_t i = 0; i < range.size(); i++) {
    if (is_valid_constant_range(range[i]) && (i < valid_range_end)) {
      bld->push_scope();
      loop_vars.emplace_back(builder::make_var(
          datatypes::index, std::string("__inner_itr_") + std::to_string(i)));
  ...
  for (int64_t i = static_cast<int64_t>(inner_anchor_num) - 1; i >= 0; i--) {
    ...
    fmgr->create_fusion_anchor(slice_map{{base_gt_.get(), {inner_slice}}},
                               parent_fanchor);
    auto body = bld->pop_scope();
    auto loop = bld->push_for_loop(loop_vars[i], 0, range[loop_num].second, 1,
                                   body, true, for_type::NORMAL);
    if (i == 0) {
