...
static bool require_fusion_level(const context_ptr &ctx,
                                 fusion_opt_level at_least_lvl) {
...
void tensor_detail_to_ir_tensor(sc_graph_t &graph, const std::string &name,
  ...
  bool is_cost_model_enabled = buf_alloc->get_binded_mxp()->cost_->is_enabled();
  if (!buf_alloc->g2b_map_.haskey(gt)) {
    ...
    if (!graph.is_dynamic() && is_cost_model_enabled) {
      auto dim_prod =
          get_dims_product(get_expr_to_dims(tsr.checked_as<tensor>()->dims_));
      ...
      buf_alloc->mem_trace_.emplace_back(
          memory_optim::memory_alloc_trace_t{(uintptr_t)tsr.get(), (size_t)0});
    }
  } else if (!graph.is_dynamic() && is_cost_model_enabled) {
    auto tsr = get_real_tensor(buf_alloc->g2b_map_.get(gt));
    // update last use trace
    auto last_trace = std::remove_if(
        buf_alloc->mem_trace_.begin(), buf_alloc->mem_trace_.end(),
...
void tensor_detail_to_ir_tensor(sc_graph_t &graph,
  ...
  for (size_t i = 0; i < tsrs.size(); i++) {
    tensor_detail_to_ir_tensor(graph, name_prefix + std::to_string(i), tsrs[i],
                               buf_alloc);
  }
}
} // namespace graph
...
void commit_graph_to_func(sc_graph_t &g, const func_t &func,
  ...
  visitor.visit_graph(g, [&parti](op_visitor_t *visitor, const sc_op_ptr &op) {
    if (op->isa<input_op>() || op->isa<output_op>())
      return;
    if (parti->empty())
      parti->buf_alloc_.allocate_buffer(op.get());
    parti->add(op);
  });
  parti->transform_to_mixed_op();
}

void mxp_buffer_allocator::set_buffer_inplace_hint(const expr &target_buf,
                                                   const expr &inplace_buf) {
  // skip dynamic cases
  // Buffer inplace currently does not support dynamic buffers
  if (binded_mxp_->get_host_graph().is_dynamic()) {
    return;
  }
  COMPILE_ASSERT(target_buf.defined() && inplace_buf.defined(),
                 "Both buffer should be defined")
  // skip same buffer
  if (inplace_buf.ptr_same(target_buf))
    return;
...
void mxp_buffer_allocator::allocate_buffer(sc_op *op) {
  ...
  graph::tensor_detail_to_ir_tensor(
      graph, op->op_name_ + "_" + std::to_string(op->logical_op_id_) + "_ins_",
      op->get_inputs(), this);
  ...
  if (auto collc_op = op->dyn_cast<reduce_collect_op_t>()) {
    if (collc_op->is_place_holder_op()) {
      ...
      COMPILE_ASSERT(
          op->get_inputs()[0]->producer_owner_->isa<reduce_compute_op_t>(),
          "reduce collect op is expected to follow reduce compute "
          "op, but got "
              << op->get_inputs()[0]->producer_owner_->op_name_)
      // no code generated
      g2b_map_.get(op->get_outputs()[0]) = g2b_map_.get(op->get_inputs()[0]);
    }
  }

  // allocate output buffer
  graph::tensor_detail_to_ir_tensor(
      graph, op->op_name_ + "_" + std::to_string(op->logical_op_id_) + "_outs_",
      op->get_outputs(), this);

  // reorder op
  if (auto reo_op = op->dyn_cast<reorder_op_t>()) {
    if (reo_op->check_padding()) {
      op->get_outputs()[0]->attrs_.set(mixed_partition_hint::no_inplace, true);
    }
  }
  ...
  for (auto &out : op->get_outputs()) {
    ...
    for (auto &inp : op->get_inputs()) {
      if (inp->attrs_.get_or_else(mixed_partition_hint::no_inplace, false)) {
        continue;
      }
  ...
  if (op->isa<padding_op_t>() && op->get_inputs()[0]->uses_.size() == 1 &&
    ...
    if (old_input.isa<tensor>() ||
        (old_input.isa<tensorptr>() && ins->uses_.size() == 1 &&
         utils::is_one_of(old_input.static_as<tensorptr>()->base_->dtype_,
      ...
      if (old_input.isa<tensorptr>()) {
        ...
        if (tv_inp->attrs_.get_or_else(mixed_partition_hint::no_inplace, false))
          return;
        if (producer->share_gt_with_op<tensor_view_op_t>(tv_inp) ||
            producer->share_gt_with_op<tunable_op_t>(tv_inp))
          return;
      }
      op->attrs_.set<bool>(mixed_partition_hint::inplace_optimized_op, true);
      auto pad_op = op->dyn_cast<padding_op_t>();
      auto new_input = builder::tensor_ptr(
          g2b_map_.get(out), pad_op->get_padding_offsets_exprs(),
          pad_op->get_inputs()[0]->details_.get_blocking_dims_expr(graph),
          true);
      if (old_input.isa<tensorptr>()) {
        auto parent_tsr = get_real_tensor(old_input);
        auto shape = parent_tsr->dims_;
        new_input = builder::tensor_ptr(
            new_input,
            std::vector<expr>(new_input.static_as<tensorptr>()->shape_.size(),
                              0),
            shape, false);
...
mxp_buffer_allocator::get_real_mem_trace(
  ...
  std::transform(shrink_trace.begin(), shrink_trace.end(), shrink_trace.begin(),
...
size_t get_buffer_usage(
  ...
  return schedule_memory_allocations(
      mem_trace, /*alignment*/ 64,
      ctx->flags_.buffer_schedule_ == attr_keys::BUF_SCHED_HOT, inplace_map,
      out_schedule, out_inplace_selection);
}

size_t mxp_buffer_allocator::get_real_buffer_usage() const {
  return get_buffer_usage(binded_mxp_->ctx_, get_real_mem_trace(),
                          inplace_map_);
}

void mxp_buffer_allocator::replace_buffer(const expr &old_buffer,
                                          const expr &new_buffer) {
  // assert new buffer
  COMPILE_ASSERT(
      b2g_map_.find(new_buffer) == b2g_map_.end(),
  ...
  if (new_tsr_already_in_trace) {
    ...
    COMPILE_ASSERT(cnt == 4, "Unexpected buffer occurs time in trace: " << cnt)
  }
...
mxp_buffer_allocator::get_buffer(sc_op *op) const {
  ...
  std::transform(op->get_inputs().begin(), op->get_inputs().end(),
                 inputs.begin(), [&](const graph_tensor_ptr &gt) {
                   COMPILE_ASSERT(g2b_map_.haskey(gt),
                                  "please allocate buffer first")
...
static cmp_res cmp_op_anchor(sc_op *op, fusion_anchor_ptr cur_anchor,
                             fusion_anchor_ptr new_anchor) {
  ...
  if (res == cmp_res::unknown) {
    cmper(op->get_outputs());
  }
...
void mxp_buffer_allocator::update_input_buffer_info(sc_op *op) {
  ...
  auto update_inp_tensor_info = [&](const graph_tensor_ptr &inp) {
    ...
    if (b2g_map_.find(buf) == b2g_map_.end())
      b2g_map_[buf] = inp;
    auto tsr = get_real_tensor(buf);
    ...
    if (tsr2anch_map_.find(tsr) != tsr2anch_map_.end()) {
      ...
      // auto skip
      if (cur_anchor_map == real_anchor_map)
        return;
      ...
      if (cur_anchor_map->is_cousin_for(real_anchor_map)) {
        ...
        tsr2anch_map_[tsr] = real_anchor_map->get_root()->shared_from_this();
      } else {
        ...
        if (res == cmp_res::l_less_r) {
          ...
          need_overwrite = real_anchor_map->is_parent_for(cur_anchor_map) ||
...
void mxp_buffer_allocator::update_output_buffer_info(sc_op *op) {
  ...
  auto update_out_tensor_info = [&](const graph_tensor_ptr &out) {
    ...
    if (tsr2anch_map_.find(tsr) != tsr2anch_map_.end()) {
      ...
      if (!commited_anchor_map->is_input_anchor())
        return;
      COMPILE_ASSERT(b2g_map_.find(buf) != b2g_map_.end(),
                     "base tensor should be visited")
      // auto skip
      if (tsr2anch_map_[tsr] == commited_anchor_map)
        return;
      ...
      for (auto &sub_anchor : sub_of_commited_anchor_map) {
        if (!sub_anchor->fsmap_.hasvalue(out))
          continue;
        if (!min_anchor_map)
          ...
          if (cmp_slice_range(min_slice, cur_slice) == cmp_res::l_larger_r) {
            min_anchor_map = sub_anchor;
          }
        }
      }
  ...
  for (auto &pair : g2b_map_.datamap_) {
    ...
    if (auto padding = op->dyn_cast<padding_op_t>()) {
      ...
      if (pair.second->attr().has_key(mixed_partition_hint::cut_buffer)) {
        ...
        COMPILE_ASSERT(tsr2anch_map_.find(pad_tsr) != tsr2anch_map_.end(),
      ...
      if (!op->attrs_.has_key(
        ...
        op->attrs_[mixed_partition_hint::tensor_already_initialized] = true;
      }
    }
  }
}
  ...
  for (auto &op : binded_mxp_->committed_ops_) {
    ...
    for (auto &input_tsr : concat->get_inputs()) {
      if (input_tsr->attrs_.has_key(
        ...
        buf->attr()[concat_optim_attr_keys::pass_memory_offset] = offset;

        auto &final_tsr = input_tsr->attrs_.get<graph_tensor_ptr>(
            concat_optim_attr_keys::graph_memory_offset_to);
        COMPILE_ASSERT(binded_mxp_->buf_alloc_.g2b_map_.haskey(final_tsr),
                       "No buffer allocated for concat outputs")
        auto &out_buffer = binded_mxp_->buf_alloc_.g2b_map_.get(final_tsr);
        buf->attr()[concat_optim_attr_keys::pass_memory_offset_to] = out_buffer;
        SC_MODULE_INFO << "Buffer: " << buf
...
inline bool check_last_use_for_gt(const graph_tensor *cur_gt,
                                  const graph_tensor *last_gt,
                                  const mxp_buffer_allocator *alloc) {
  return std::all_of(
      cur_gt->uses_.begin(), cur_gt->uses_.end(),
      [&alloc, &last_gt](const std::pair<int, sc_op_weak_ptr_t> &user) {
        return alloc->get_binded_mxp()->dep_m_->lookup(
                   user.second.get(), last_gt->producer_owner_) == 1;
      });
}
...
static void
collect_inplace_info(graph_tensor *cur_gt, graph_tensor *ref_gt,
  ...
  if (EXPLORE_INPLACE_MAX_STEP == (step++))
    return;
  // skip repeated
  if (inplace_set.find(cur_gt) != inplace_set.end())
    return;
  // return when producer is not elementwise op
  if (!is_elementwise_producer(cur_gt)) {
    return;
  }
  ...
  if ((visited_set.find(cur_buf) == visited_set.end()) &&
      ...
      (cur_gt->details_.get_strides() == ref_gt->details_.get_strides()) &&
  ...
  for (auto &inp : elem_op->get_inputs()) {
    collect_inplace_info(inp.get(), ref_gt, inplace_set, visited_set, valid_set,
                         alloc, step);
  }
}
...
void mxp_buffer_allocator::query_inplace() {
  ...
  for (auto &tsr2anch : tsr2anch_map_) {
    ...
    if (tsr->attr().has_key(mixed_partition_hint::cut_buffer))
      continue;
    ...
    for (auto &commit_op : binded_mxp_->committed_ops_) {
      if (commit_op.get() == ref_gt->producer_owner_) {
        ...
        for (auto &out : commit_op->get_outputs()) {
          if (inplace_gt_set.find(out.get()) != inplace_gt_set.end()) {
  ...
  for (auto &buf2shr : b2g_map_) {
    ...
    if (!buf.ptr_same(tsr) &&
        (buf->attr().has_key(tensor_shrinker_attrs::should_shrink) ^
         tsr->attr().has_key(tensor_shrinker_attrs::should_shrink))) {
    ...
    if (cut_buffer_set.find(buf1) != cut_buffer_set.end()) {
      ...
      if ((shrink_info1.empty() ^ shrink_info2.empty()) ||
          (!shrink_info1.empty() && !shrink_info2.empty() &&
           cmp_slice_range({shrink_info1}, {shrink_info2}) != cmp_res::equal)) {
...
inline bool check_tsr_len_under_resigter_size(size_t tsr_len, uint16_t simd_len,
                                              uint16_t max_register_tol = 16) {
...
bool mxp_buffer_allocator::validate_tsr2var() const {
  for (auto &tsr2def : tsr2anch_map_) {
    ...
    if (!tsr->attr().has_key(attr_keys::must_tensor2var))
      continue;
    auto shrinked_info = get_shrinked_info(tsr);
    if (shrinked_info.empty())
      continue;
    ...
    auto tsr_simd_len = vectorize_step(
        binded_mxp_->ctx_, tsr.checked_as<tensor>()->elem_dtype_.type_code_);
...
fusion_anchor_ptr
mxp_buffer_allocator::get_real_anchor_for_buffer(const expr &buffer) const {
  ...
  if (b2g_map_.find(tsr)->second->producer_owner_->isa<tunable_op_t>() &&
    ...
    auto raw =
        parent_loop->attr()
            .get<std::weak_ptr<stmt_base_t>>(stmt_attr_key::reduce_root_loop)
            .lock();
    COMPILE_ASSERT(raw, "reduce_root_loop weak ptr invalidated");
    anch =
        binded_mxp_->get_anchor_inside_loop(stmt(raw).checked_as<for_loop>());
    COMPILE_ASSERT(anch,
...
slice_range mxp_buffer_allocator::get_shrinked_info(const expr &buffer) const {
  ...
  if (range_list.size() != 1) {
    if (range_list.empty() || !(anchor->isa<iter_fusion_anchor_t>() ||
...
void mxp_buffer_allocator::declare_tensor() const {
  ...
  auto declare_tensor_ = [&](const expr &tsr) {
    ...
    for (; def_pos < ss.size(); def_pos++) {
      ...
      if (!ss[def_pos]
               .cast<define>()
               .filter([](const define &d) { return d->var_.isa<var>(); })
               .has_value())
        break;
    }
  ...
  std::sort(
      define_list.begin(), define_list.end(), [](const expr &a, const expr &b) {
        COMPILE_ASSERT(a.isa<tensor>() && b.isa<tensor>(),
                       "tensor node is expected, but got " << a << " and " << b)
        return a.static_as<tensor>()->name_ < b.static_as<tensor>()->name_;
      });
...
std::vector<memory_optim::memory_alloc_trace_t> merge_mem_trace(
  ...
  for (auto &trace : mem_trace2) {
    ...
    auto buf_id =
        is_replaced ? (uintptr_t)(trace_itr->second.get()) : trace.buffer_id_;
    if (trace.size_ > 0) {
      auto last_use =
    ...
    if (is_replaced) {
      ret.emplace_back(memory_optim::memory_alloc_trace_t{buf_id, trace.size_});
    } else {
...
merge_inplace_map(const memory_optim::inplace_info_map &inplace_map1,
  ...
  for (auto &inplace_pair : inplace_map2) {
    ...
    ret[inplace_pair.first] = std::vector<std::pair<uintptr_t, inplace_kind>>{
        {iter != buffer_map.end() ? (uintptr_t)iter->second.get() : info.first,
         info.second}};
  }
  return ret;
}
...
mxp_mem_info merge_real_mem_info(const mxp_buffer_allocator &alloc1,
  ...
  return std::make_pair(
      ...
      merge_inplace_map(alloc1.inplace_map_, alloc2.inplace_map_, buffer_map));
}

void mxp_buffer_allocator::merge(
    mxp_buffer_allocator &other, std::unordered_map<expr, expr> &buffer_map,
  ...
  for (auto &other_g2b : other.g2b_map_.datamap_) {
    ...
    if (g2b_map_.haskey(other_gt)) {
      ...
      if (other.tsr2anch_map_.find(get_real_tensor(buffer)) !=
        ...
        auto other_anchor = other.tsr2anch_map_[get_real_tensor(buffer)];
        tsr2anch_map_[get_real_tensor(buffer)] =
            (other_anchor == common_other_buffer_anchor) ? common_buffer_anchor
                                                         : other_anchor;
      }
    }
  }
...
void search_op_anchor_in_parti(sc_op *op, mixed_parti_t *parti) {
  ...
  for (auto &fanchor : parti->fanchors_) {
    ...
    if (!parti->empty() &&
        op->attrs_.get_or_else(mixed_partition_hint::pre_fuse_begin_op,
                               false)) {
    ...
    if (parti->empty() &&
        std::any_of(op->get_outputs().begin(), op->get_outputs().end(),
  ...
  for (auto &op_a : A_ops) {
    ...
    for (auto &op_b : B_ops) {
      for (auto &inp : op_b->get_inputs()) {
        if (gt_set.find(inp) != gt_set.end())
          return true;
      }
      for (auto &out : op_b->get_outputs()) {
        if (gt_set.find(out) != gt_set.end())
          return true;
      }
    }
  }
      ...
      if (append_parti->is_parti_inp(inp) &&
        ...
        for (auto &op_in_set : target_parti->ops) {
          auto result = target_parti->dep_m_->lookup(
              op_in_set->logical_op_id_, inp->producer_owner_->logical_op_id_);
          if (result == 1) {
            return true;
          }
        }
...
static stmts find_last_parallel_for(const stmts &scope, int64_t &out_index) {
  ...
  if (!outer_body.empty()) {
    for (int64_t i = outer_body.size() - 1; i >= 0; i--) {
      auto &s = outer_body[i];
      if (s.isa<stmts>()) {
        auto ret = find_last_parallel_for(s.static_as<stmts>(), out_index);
        if (ret.defined()) {
          return ret;
        }
      }
      if (s.cast<for_loop>()
              .filter([](const for_loop &v) {
                return v->kind_ == for_type::PARALLEL &&
                       (!v->attr_ ||
                        !v->attr_->get_or_else("dont_prefetch", false));
              })
              .has_value()) {
        out_index = i;
        return scope;
...
static int check_parti_loop_axis_binding(mixed_parti_t *A, mixed_parti_t *B,
  ...
  if (check_parti_forked(A, B))
    return check_loop_size;
  return check_loop_binding_axis(A->get_outer_loops(), B->get_outer_loops(),
                                 check_loop_size);
}

static void merge_parti_impl(mixed_parti_t *pa_to_merge,
                             mixed_parti_t *parti_be_merged,
                             size_t merged_loop_size,
  ...
  auto outer_loops_merged_append = outer_loops_be_merged[merged_loop_size - 1];
  auto max_to_merge_anchor_map =
      pa_to_merge->get_anchor_inside_loop(outer_loops_merged_target);
  auto max_be_merged_anchor_map =
      parti_be_merged->get_anchor_inside_loop(outer_loops_merged_append);
  /* * * * * * * * * * * * * * * * *
   * Step 1: Merge func_
   * * * * * * * * * * * * * * * * */
  COMPILE_ASSERT(max_to_merge_anchor_map,
                 "max-to-merge fusion anchor not found")
  ...
  for (auto &be_merged_anchor_map : parti_be_merged->fanchors_) {
    // skip outer anchor
    if (std::any_of(
            outer_loops_be_merged.begin(),
            outer_loops_be_merged.begin() + merged_loop_size,
            [&be_merged_anchor_map, &parti_be_merged](const for_loop &loop) {
              return parti_be_merged->get_anchor_inside_loop(
                         loop, be_merged_anchor_map->is_input_anchor()) ==
                     be_merged_anchor_map;
            })) {
      continue;
    }
    be_merged_anchor_map->attach_parent_anchor(max_to_merge_anchor_map,
                                               max_be_merged_anchor_map);
  ...
  for (size_t i = 0; i < merged_loop_size; i++) {
    ...
    node_remap[outer_loops_be_merged[i].impl] = outer_loops_to_merge[i].impl;
    auto be_merged_anchor_map =
             parti_be_merged->get_anchor_inside_loop(outer_loops_be_merged[i]),
         to_merge_anchor_map =
   ...
   * * * * * * * * * * * * * * * * */
  pa_to_merge->buf_alloc_.merge(
      parti_be_merged->buf_alloc_, buffer_map,
  ...
  for (auto &op_anchor_pair : parti_be_merged->op_anchor_map_) {
    ...
    pa_to_merge->op_anchor_map_[op_anchor_pair.first] = op_anchor_pair.second;
  }
  ...
  pa_to_merge->fusion_partition_t::merge(
      static_cast<fusion_partition_t *>(parti_be_merged)->shared_from_this());
...
static size_t get_great_common_loop_size(const std::vector<for_loop> &loop_A,
  ...
  for (; merged_loop_size < gcs; merged_loop_size++) {
    ...
    if (!to_merge->iter_end_.isa<constant_c>() &&
        slice_expr_equals(to_merge->iter_end_, be_merged->iter_end_)) {
...
static bool
try_merge_mixed_parti_parallel_inners(mixed_parti_t *pa_to_merge,
                                      mixed_parti_t *parti_be_merged) {
  pa_to_merge = pa_to_merge->get_root(),
  parti_be_merged = parti_be_merged->get_root();
  ...
  auto merged_loop_size =
      get_great_common_loop_size(outer_loops_to_merge, outer_loops_be_merged);
  if (!merged_loop_size)
    return false; // no outer loop can be merged

  // validate axis binding
  merged_loop_size = check_parti_loop_axis_binding(parti_be_merged, pa_to_merge,
                                                   merged_loop_size);
  ...
  if (pa_to_merge->ctx_->flags_.prefetch_) {
    ...
    if (auto second_prefetch =
      ...
      auto last_for_parent = find_last_parallel_for(
          outer_loops_to_merge[merged_loop_size - 1]->body_.static_as<stmts>(),
      ...
      auto prefetch_idx =
          second_prefetch->query_prefetch(pa_to_merge->ctx_, false, in_slice);
      ...
      if (prefetch_insertion_point != -1 && !prefetch_idx.empty()) {
        ...
        second_prefetch->generate_prefetcher_and_set_idle(
            pa_to_merge->ctx_, false, in_slice, prefetch_idx, out_seq);
  ...
  if (!max_to_merge_anchor_map) {
    ...
    outer_loops_merged_target->body_.checked_as<stmts>()->seq_.emplace_back(s);
...
static bool try_merge_mixed_parti_parallel(mixed_parti_t *A, mixed_parti_t *B) {
  ...
  if (A == B)
    return false;
  if (!A->func_.get() || !B->func_.get())
    return false;
  if (!check_parti_connectionship(A, B))
    return false;
  if (check_parti_ring_risk(A, B))
    return false;

  auto dep = check_parti_dep(A, B);
  COMPILE_ASSERT(dep != parti_dep::inter_dep,
                 "inter-dependency is not expected");
  ...
  if (outer_loops_target.empty() || outer_loops_append.empty())
    return false;
      ...
      if (target_end == outermost_loop_target->num_threads_ &&
        ...
        if (outermost_loop_target->num_threads_ >
          ...
          int64_t num_groups = outermost_loop_target->num_threads_ /
                               outermost_loop_append->num_threads_;
          target_parti->try_split_outermost_loop_on_num_threads(num_groups);
          return try_merge_mixed_parti_parallel_inners(target_parti,
                                                       append_parti);
          ...
          int64_t num_groups = outermost_loop_append->num_threads_ /
                               outermost_loop_target->num_threads_;
          append_parti->try_split_outermost_loop_on_num_threads(num_groups);
          return try_merge_mixed_parti_parallel_inners(target_parti,
                                                       append_parti);
        } else {
          return false;
...
static bool try_merge_mixed_parti_horizontally(mixed_parti_t *A,
                                               mixed_parti_t *B) {
  A = A->get_root(), B = B->get_root();
  if (A == B)
    return false;
  if (!A->func_.get() || !B->func_.get())
    return false;
  if (!A->contain_tunable_op() || !B->contain_tunable_op())
    return false;
  if (!check_parti_connectionship(A, B))
    return false;
  if (check_parti_dep(A, B) != parti_dep::no_dep)
    return false;

  auto outer_loops_A = A->get_outer_loops(),
       outer_loops_B = B->get_outer_loops();
  if (outer_loops_A.empty() || outer_loops_B.empty())
    return false;
  // skip parallel merge
  if (outer_loops_A[0]->num_threads_ > 0 || outer_loops_B[0]->num_threads_ > 0)
    return false;
  // check cost model
   ...
   * Step 5: Merge op_anchor_map_
   * * * * * * * * * * * * * * * * */
  A->op_anchor_map_.insert(B->op_anchor_map_.begin(), B->op_anchor_map_.end());
...
static sc_dim get_loops_range(const for_loop &loop) {
  ...
  return get_expr_as_int(loop->iter_end_) - get_expr_as_int(loop->iter_begin_);
}
...
static void try_align_parti_outer_loops(mixed_parti_t *A, mixed_parti_t *B) {
  ...
  if (outer_loops_A.empty() || outer_loops_B.empty())
    return;
  if (outer_loops_A.size() == outer_loops_B.size())
    return;
...
static bool
try_merge_brgemm_and_preop_parti(mixed_parti_t *A, mixed_parti_t *B,
                                 const sc_op_ptr &joint_op = nullptr) {
  if (!require_fusion_level(A->ctx_, fusion_opt_level::lv2))
    return false;
  A = A->get_root(), B = B->get_root();
  if (A == B)
    return false;
  if (!A->func_.get() || !B->func_.get())
    return false;
  if (!joint_op && !check_parti_connectionship(A, B))
    return false;
  if (check_parti_ring_risk(A, B))
    return false;
  auto dep_flag = check_parti_dep(A, B);
  if (dep_flag == parti_dep::inter_dep)
    return false;
  ...
  for (auto &brgemm_parti_anchor : brgemm_parti->fanchors_) {
        ...
        if (preop_parti->contains(pre_fuse_last_op)) {
          ...
          for (auto &op : preop_parti->committed_ops_) {
            if (op.get() == pre_fuse_last_op ||
                preop_parti->dep_m_->lookup(op.get(), pre_fuse_last_op) == 1) {
          ...
          for (auto op_iter = pre_infer_ops.rbegin();
            ...
            auto status =
                (*op_iter)->dyn_cast<fusible_op_t>()->pre_infer_slice_ranges(
                    brgemm_parti->ctx_, tmp_fsmap);
    ...
    // move fsmap
    brgemm_parti_anchor->fsmap_.datamap_.insert(tmp_fsmap.datamap_.begin(),
                                                tmp_fsmap.datamap_.end());
  }
...
static bool try_merge_mixed_parti_vertically(mixed_parti_t *A, mixed_parti_t *B,
                                             const sc_op_ptr &joint_op,
                                             bool keep_outerloop_size = false) {
  A = A->get_root(), B = B->get_root();
  if (A == B)
    return false;
  if (!A->func_.get() || !B->func_.get())
  ...
  if (!joint_op && !check_parti_connectionship(A, B))
    return false;
  if (!joint_op && check_parti_ring_risk(A, B))
    return false;
  auto dep_flag = check_parti_dep(A, B);
  ...
  if (A->contain_input_anchor() && B->contain_input_anchor())
    return false;
  ...
  if (!merged_loop_size || (keep_outerloop_size &&
                            (merged_loop_size != outer_loops_to_merge.size() ||
                             merged_loop_size != outer_loops_be_merged.size())))
    return false;
  ...
  if (auto max_to_merge_anchor_map = pa_to_merge->get_anchor_inside_loop(
    ...
    if (joint_op &&
        std::any_of(
            joint_op->get_inputs().begin(), joint_op->get_inputs().end(),
            [&max_to_merge_anchor_map,
             &pa_to_merge](const graph_tensor_ptr &inp) {
              return pa_to_merge->contains(inp->producer_owner_) &&
                     max_to_merge_anchor_map->blocked_gt_set_.find(inp) !=
                         max_to_merge_anchor_map->blocked_gt_set_.end();
            }))
      return false;
  } else {
    return false;
...
static bool try_merge_mixed_parti_vertically(mixed_parti_t *A,
                                             mixed_parti_t *B) {
  A = A->get_root(), B = B->get_root();
  if (A == B)
    return false;
  ...
  if (A->is_single_op_parti() || B->is_single_op_parti())
    return false;
...
static bool try_merge_mixed_parti_with_joint_op(const mixed_parti_t::ptr &A,
  ...
  if (joint_op->isa<tunable_op_t>() ||
      (A->contain_tunable_op() &&
       (!B->contain_tunable_op() || (B->count_op_with_type<tunable_op_t>() >
                                     A->count_op_with_type<tunable_op_t>())))) {
    default_lhs = B.get();
    default_rhs = A.get();
  } else {
    default_lhs = A.get();
  ...
  return try_merge_brgemm_and_preop_parti(default_lhs, default_rhs, joint_op) ||
         try_merge_mixed_parti_vertically(default_lhs, default_rhs, joint_op);
}
...
mixed_parti_t::mixed_parti_t(const context_ptr &ctx, const sc_op_ptr &op,
  ...
  auto &graph = op->get_owner_graph();
  if (graph.is_dynamic()) {
    cost_ = std::make_shared<dynamic_fusion_cost_model_t>(
        this, graph.attrs_.get_or_else("temp.dynamic_fusion_policy",
  ...
  if (!op->isa<constant_op_t>() && !is_reshape_op(op.get())) {
    SC_MODULE_INFO << "================  create new partition: " << op->op_name_
                   << "_" << op->logical_op_id_ << " ================";
...
bool mixed_parti_t::is_ok_to_add(sc_op *op) {
  if (merged_to) {
    return get_root()->is_ok_to_add(op);
  }
  if (empty()) {
    return false;
  }
  ...
  if (!op->isa<op_traits::mixed_partition_acceptable>())
    return false;
...
bool mixed_parti_t::ready_for_op(sc_op *op) const {
  if (merged_to) {
    return get_root()->ready_for_op(op);
  }
  return op_anchor_map_.find(op) != op_anchor_map_.end();
}

void mixed_parti_t::set_anchor_for_op(sc_op *op,
                                      const fusion_anchor_ptr &fanchor_map) {
  if (merged_to) {
...
fusion_anchor_ptr mixed_parti_t::lookup_anchor_map(sc_op *op,
                                                   bool throw_assert) const {
  if (merged_to) {
    return get_root()->lookup_anchor_map(op);
  }
  auto iter = op_anchor_map_.find(op);
  auto ret = (iter != op_anchor_map_.end()) ? iter->second : nullptr;
  if (throw_assert) {
    COMPILE_ASSERT(ret, "No dispatched fusion anchor map found for "
                            << op->op_name_
...
fusion_anchor_ptr mixed_parti_t::lookup_anchor_map(const stmts &ss) const {
  if (merged_to) {
    return get_root()->lookup_anchor_map(ss);
  }
...
std::vector<fusion_anchor_ptr> mixed_parti_t::lookup_sub_anchor_map(
  ...
  if (merged_to) {
    return get_root()->lookup_sub_anchor_map(parent_fanchor);
  }
  std::vector<fusion_anchor_ptr> subs;
  for (auto &fanc : fanchors_) {
    if (fanc->parent_ == parent_fanchor) {
      subs.emplace_back(fanc);
    }
  }
  return subs;
}
...
std::vector<for_loop> mixed_parti_t::get_outer_loops(fusion_anchor_ptr fanchor,
                                                     bool skip_def_node) const {
  if (merged_to) {
    return get_root()->get_outer_loops(fanchor);
  }
 ...
 * */
fusion_anchor_ptr
mixed_parti_t::get_anchor_inside_loop(const for_loop &loop,
                                      bool input_anchor) const {
  ...
  if (body.isa<stmts>()) {
    ...
    for (auto s : ss->seq_) {
      ...
      if (s.isa<if_else>()) {
        ...
        if (!if_node->then_case_.defined() || if_node->else_case_.defined())
          continue;
      ...
      if (anchor_map) {
        // check anchor type
        if (input_anchor != anchor_map->is_input_anchor())
          continue;
        return anchor_map;
      }
    }
...
for_loop mixed_parti_t::get_next_inner_loop_with_anchor(
    const for_loop &cur_loop, const fusion_anchor_ptr &target_fanchor) const {
...
void mixed_parti_t::try_split_outermost_loop_on_num_threads(
  ...
  if (origin_large_anchor) {
    ...
    std::unordered_map<expr_c, expr_c> vmap = {
        {split_inner_loop->var_,
         make_expr<constant_node>(UINT64_C(0),
                                  split_inner_loop->var_->dtype_)}};
      ...
      for (auto &srange : fspair.second) {
        for (auto &r : srange) {
          ...
          if (!new_v.ptr_same(r.first)) {
            ...
            // set new range
            r.second =
                dim2unsigned(get_expr_as_int(r.second) *
                             (get_expr_as_int(split_inner_loop->iter_end_) -
                              get_expr_as_int(split_inner_loop->iter_begin_)));
          }
        }
      }
    }
...
void mixed_parti_t::try_split_outermost_loop(int64_t block) const {
  ...
  if (outermost_loop_range % block != 0)
    return;
...
bool mixed_parti_t::is_parti_inp(const graph_tensor *gt) const {
  if (merged_to) {
    return get_root()->is_parti_inp(gt);
  }
  auto ths = this;
...
bool mixed_parti_t::is_parti_out(const graph_tensor *gt) const {
  if (merged_to) {
    return get_root()->is_parti_out(gt);
  }
  auto ths = this;
...
bool mixed_parti_t::is_const_parti() const {
  return (get_ops_size() == 1 &&
          get_ith_op(0)->attrs_.get_or_else(
              "constant", const_kind::not_const) != const_kind::not_const);
}
...
bool mixed_parti_t::contain_elemwise_op_only() const {
  if (merged_to) {
    return get_root()->contain_elemwise_op_only();
  }
  for (auto &op : ops) {
...
bool mixed_parti_t::is_single_op_parti() const {
  if (merged_to) {
    return get_root()->is_single_op_parti();
  }
  return ops.size() == 1;
}
...
bool mixed_parti_t::is_optimized() const {
  if (merged_to) {
    return get_root()->is_optimized();
  }
  if (committed_ops_.empty())
    return false;
...
bool mixed_parti_t::is_small_workload() const {
  if (merged_to) {
    return get_root()->is_small_workload();
  }
  // skip partition owning more than one ops
  if (committed_ops_.size() != 1)
    return false;
  // get sinlge op
  auto single_op = committed_ops_[0].get();
  // skip tunable op
  if (single_op->isa<tunable_op_t>())
    return false;
...
static bool check_repartition(const mixed_parti_t::ptr &parti) {
  ...
  if (parti && parti->get_ops_size() == 1 &&
    ...
    parti->committed_ops_[0]->attrs_[mixed_partition_hint::trial_break] = false;
    return true;
  }
  if (!parti || parti->get_ops_size() < 2)
    return false;
  // check tensorview in edge of partition
  bool repartition = false;
  // check buffer is tensorptr
  auto check_parti_out_tptr = [&repartition,
                               &parti](const graph_tensor_ptr &out) {
    auto producer = out->producer_owner_;
    COMPILE_ASSERT(parti->buf_alloc_.g2b_map_.haskey(out),
                   "No buffer allocated found for output "
                   "of "
                       << producer->op_name_ << producer->logical_op_id_)
  ...
  for (auto &op : parti->ops) {
    ...
    // partition
    bool is_terminate_op = std::all_of(
        op->get_outputs().begin(), op->get_outputs().end(),
        [&](const graph_tensor_ptr &gt) {
          return std::all_of(
              gt->uses_.begin(), gt->uses_.end(),
              [&parti](const std::pair<int, sc_op_weak_ptr_t> &user) {
      ...
      for (const auto &prod_input : op->get_inputs()) {
        for (const auto &preced_output :
          ...
          if (parti->is_parti_out(preced_output) &&
              !op->attrs_.has_key(mixed_partition_hint::trial_break)) {
            op->attrs_[op_attr_key::break_pre_fuse] = true;
...
bool mixed_parti_t::contain_input_anchor() const {
  if (merged_to) {
    return get_root()->contain_input_anchor();
  }
...
static mixed_parti_t::ptr try_execute_pre_op_fusion(
    const context_ptr &ctx, const sc_op_ptr &op,
...
static mixed_parti_t::ptr try_execute_post_op_fusion(
    const context_ptr &ctx, const sc_op_ptr &op,
    std::vector<mixed_parti_t::ptr> &avaliable_input_parti) {
  mixed_parti_t::ptr parent_partition = nullptr;
  ...
  for (auto &inp_parti : avaliable_input_parti) {
    if (!parent_partition) {
      ...
      op->dyn_cast<op_traits::mixed_partition_acceptable>()->search_anchor(
  ...
  for (auto &inp_parti : avaliable_input_parti) {
    if (inp_parti->is_ok_to_add(op.get())) {
      if (parent_partition) {
        try_merge_mixed_parti_with_joint_op(parent_partition, inp_parti, op);
        ...
        if (op->attrs_.get_or_else(mixed_partition_hint::no_gather_op, false))
          break;
      }
    }
  }
      ...
      if (!op->attrs_.get_or_else(op_attr_key::break_pre_fuse, false)) {
        ...
        for (auto &in : sorted_inputs) {
          ...
          if (!in->producer_owner_->attrs_.get_or_else(
              ...
              !in->producer_owner_->attrs_.get_or_else(op_attr_key::no_fuse,
                                                       false) &&
        ...
        // try pre op fusion
        parent_partition =
            try_execute_pre_op_fusion(ctx, op, avaliable_input_parti);
        if (!parent_partition) {
          // try post op fusion
          parent_partition =
              try_execute_post_op_fusion(ctx, op, avaliable_input_parti);
        }
      } else {
        SC_MODULE_INFO << op->op_name_ << "_" << op->logical_op_id_
...
bool mixed_parti_t::can_optimize_outer_loop(bool allow_tensorview) const {
  ...
  bool for_reduce =
          ...
          [&](const sc_op_ptr &op) {
            if (op->isa<movement_op_t>()) {
              if (op->isa<tensor_view_op_t>() && allow_tensorview) {
                ...
                return op->attrs_.get_or_else(op_attr_key::no_fuse, false);
              } else {
                return false;
              }
            } else {
              return is_elementwise_op(op.get()) || op->isa<reduce_op_t>() ||
      ...
      std::any_of(ops.begin(), ops.end(), [&](const sc_op_ptr &op) {
        ...
        int cur = (op->get_inputs()[0]->details_.get_blocking_dims().size() -
                   rd_axis.size());
        /** E.g
  ...
  bool for_pooling =
      contain_op_with_type<pooling_op_t>() &&
...
static bool try_optimize_reduce(
    mixed_parti_t *parti, sc_graph_t &sub_graph,
  ...
  for (auto &op : sub_graph.ops_) {
    if (auto red_op = op->dyn_cast<op_traits::maybe_split_optimized_t>()) {
      ...
      if (op->isa<reduce_op_t>() && !nested_parallel_found) {
        ...
        for (auto itr = outer_loops.begin(); itr != outer_loops.end(); itr++) {
  ...
  for (auto &red_op : split_reduce_set) {
    ...
    if (rd_type != reduce_operator::add) {
      ...
      auto padded_dims =
          sc_data_format_t::get_padded_plain_shapes(blocking_dims, fmt);
      // currently, do not support split with padding
      if (plain_dims != padded_dims)
        continue;
    }
    // pre-check
    if (op->isa<reduce_compute_op_t>()) {
      ...
      auto slice_info = parti->buf_alloc_.get_shrinked_info(
          parti->buf_alloc_.g2b_map_.get(orig_iter->second->get_outputs()[0]));
      if (slice_info.empty())
        continue;
      sc_dim prod =
          get_dims_product(get_expr_to_dims(get_slice_shape(slice_info)));
      auto tsr_simd_len =
          vectorize_step(ctx, op->get_inputs()[0]->details_.dtype_.type_code_);
      if (!check_tsr_len_under_resigter_size(prod, tsr_simd_len))
        continue;
    }
    // Do split
...
static bool try_optimize_outer_loop(
    mixed_parti_t *parti, sc_graph_t &sub_graph,
    const std::unordered_map<sc_op_ptr, sc_op_ptr> &graph2orig_ops) {
  // prepare stage
...
static bool try_optimize_fusion_anchor(
    mixed_parti_t *parti, sc_graph_t &sub_graph,
    const std::unordered_map<sc_op_ptr, sc_op_ptr> &graph2orig_ops) {
  // auto skip
  if (parti->committed_ops_.size() < 2)
    return false;
  auto &ctx = parti->ctx_;
  ...
  auto elem_op_with_last_dim_undividable =
    ...
    if (!compute_shape.back().isa<constant>())
      return false;
  ...
  auto query_op_on_graph = [&graph2orig_ops](const sc_op_ptr &op) {
    ...
    COMPILE_ASSERT(iter != graph2orig_ops.end(), "Could not find mapping op")
    return iter->first;
  };
  ...
  for (auto &op : parti->committed_ops_) {
    ...
    if (target_ops.empty() && elem_op_with_last_dim_undividable(op)) {
      ...
      auto first_op = graph2orig_ops.find(target_ops.front())->second.get();
...
bool try_optimize_parti(
    mixed_parti_t *parti, sc_graph_t &sub_graph,
    const std::unordered_map<sc_op_ptr, sc_op_ptr> &graph2orig_ops) {
  // if `opt-level == 0`, auto skip
  if (parti->ctx_->flags_.opt_level_ == sc_opt_level::lv0)
    return false;
  if (sub_graph.is_dynamic()) {
    return false;
  }
  // skip already optimized parti
  if (parti->is_optimized())
    return false;
  ...
  need_optimize |= try_optimize_fusion_anchor(parti, sub_graph, graph2orig_ops);
...
static std::string get_graph_name(const sc_graph_t &graph) {
  std::string ret;
  for (auto &op : graph.ops_) {
    if (op->isa<input_op>() || op->isa<output_op>() || op->isa<constant_op_t>())
      continue;
    if (!ret.empty())
      ret += "_";
    ret += op->op_name_;
...
static std::string get_parti_prefix(const mixed_parti_t &parti) {
  ...
  if (parti.ops.size() > 1) {
    ...
    if (!outer_loops.empty()) {
      prefix = "outerloop_" + print_loops_range(outer_loops) + "_" + prefix;
    }
  }
  return prefix;
}
...
std::shared_ptr<mixed_fuse_op_t> mixed_parti_t::transform_to_mixed_op() {
  ...
  visitor.visit_graph(g, [&](op_visitor_t *visitor, const sc_op_ptr &op) {
    ...
    for (auto &in : op->get_inputs()) {
      ...
      if (is_parti_inp(in) && input_tsr_set.find(in) == input_tsr_set.end()) {
        ...
        if (in->producer_owner_->attrs_.has_key("constant")) {
          new_input_op->attrs_.set(
              "constant", in->producer_owner_->attrs_.get<int>("constant"));
        }
    ...
    for (auto &out : op->get_outputs()) {
      ...
      if (is_parti_out(out) ||
          (ctx_->flags_.concat_optimization_ && op->isa<concat_op_t>() &&
           op->attrs_.get_or_else(concat_optim_attr_keys::is_final_concat,
                                  false))) {
        ...
        if (out_buffer->attr().has_key(attr_keys::tensor_inplace_hint)) {
  ...
  if (try_optimize_parti(this, sub_graph, graph2orig_ops)) {
    ...
    for (auto &op : sub_graph.ops_) {
      ...
      if (auto mx_op = op->dyn_cast<mixed_fuse_op_t>()) {
        COMPILE_ASSERT(
            mx_op->parti_list_.size() == 1,
            "Unexpected partition size found: " << mx_op->parti_list_.size())
        ...
        if (!is_reshape_op(op.get()))
          non_mixed_op_exist = true;
    ...
    if (!fall_back) {
      ...
      return std::make_shared<mixed_fuse_op_t>(mx_op_name, parti_list, modu,
                                               copied_opt_grpah,
                                               /*ins*/ fused_op_in,
                                               /*outs*/
  ...
  if (dyn_inter_) {
    ...
    auto reset_args = [&buffer_args](std::vector<expr> &target_args,
      ...
      target_args.insert(target_args.end(), extra_args.begin(),
                         extra_args.end());
    };
    ...
    dyn_inter_->inter_func_->decl_->params_ = dyn_inter_->inter_func_->params_;
  ...
  auto fused_op = std::make_shared<mixed_fuse_op_t>(
      op_name,
      std::vector<mixed_parti_t::ptr>{
          std::static_pointer_cast<mixed_parti_t>(shared_from_this())},
...
crossover_dispatcher(const std::vector<mixed_parti_t::ptr> &parti_vec,
  ...
  default:
    COMPILE_ASSERT(0, "Unknown partition merge kind found")
  }
...
static void crossover_partition(std::vector<mixed_parti_t::ptr> &op_2_partition,
  ...
  std::vector<mixed_parti_t::ptr> parti_vec = collect_parti_set(op_2_partition);
  for (auto &al : algs) {
    al(parti_vec);
  }
  for (auto &parti : op_2_partition) {
...
static expr merge_fusion_condition_by_parti_list(
  ...
  for (auto &parti : parti_set) {
    if (parti) {
      ret = ret || parti->get_fusion_policy_condition();
    }
  }
  return ret;
}
...
void do_mixed_partition(const context_ptr &ctx, sc_graph_t &graph) {
  ...
  if (require_fusion_level(ctx, fusion_opt_level::lv3)) {
    std::vector<crossover_alg> algs = {horizontal_crossover, parallel_crossover,
                                       vertical_crossover};
