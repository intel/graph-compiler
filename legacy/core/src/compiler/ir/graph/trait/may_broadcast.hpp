...
struct may_broadcast_t : public virtual op_base_trait_t {
  ...
  get_non_broadcast_input_index(bool assert_non_empty) const = 0;
  ...
  get_auto_broadcast_bc_axis(const sc_dims &input_shape,
   ...
   * */
  static bool broadcastable_shape_equal(const sc_dims &shape1,
                                        const sc_dims &shape2);

  static sc_dims infer_auto_broadcast_output_shape(const sc_dims &lhs,
                                                   const sc_dims &rhs);

  static sc_data_format_t
  infer_broadcast_format(const logical_tensor_t &target_lt,
