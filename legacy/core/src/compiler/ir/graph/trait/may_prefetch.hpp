...
struct may_prefetch_t : public virtual op_base_trait_t {
  ...
  query_prefetch(const context_ptr &ctx, bool is_global,
                 const std::vector<tensor_slice> &ins) = 0;
   ...
   */
  virtual void generate_prefetcher_body_for_slice(
   ...
   * @param indices the input indices selected by query_prefetch
   */
  virtual void generate_prefetcher_body_for_tensor(
      const context_ptr &ctx, const std::vector<expr> &func_args,
      const std::vector<expr> &ins, const std::vector<int> &indices) = 0;
   ...
   */
  virtual func_t
  generate_prefetcher_and_set_idle(const context_ptr &ctx, bool is_global,
