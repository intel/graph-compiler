...
struct configurable_t : public virtual op_base_trait_t {
  ...
  virtual void
  set_config(const reflection::shared_general_object_t &config) = 0;
  virtual reflection::shared_general_object_t
  get_default_config(context_ptr ctx) = 0;
  ...
  get_dynamic_config_candidates(const context_ptr &ctx) = 0;
  ...
  virtual impl_kind_map
  convert_config_candidates_to_impl_map(const config_ptr_vec &configs) = 0;
};
