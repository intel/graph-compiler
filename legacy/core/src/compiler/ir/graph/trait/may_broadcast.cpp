...
std::vector<int>
may_broadcast_t::get_auto_broadcast_bc_axis(const sc_dims &input_shape,
  ...
  if (bc_axis.empty()) {
    bc_axis.emplace_back(-1);
  }
  return bc_axis;
}

bool may_broadcast_t::broadcastable_shape_equal(const sc_dims &shape1,
                                                const sc_dims &shape2) {
...
sc_dims may_broadcast_t::infer_auto_broadcast_output_shape(const sc_dims &lhs,
                                                           const sc_dims &rhs) {
...
sc_data_format_t
may_broadcast_t::infer_broadcast_format(const logical_tensor_t &target_lt,
                                        const logical_tensor_t &bc_lt) {
  COMPILE_ASSERT(bc_lt.get_plain_dims().size() ==
                     target_lt.get_plain_dims().size(),
  ...
  int target_batch_dim =
      target_lt.get_plain_dims().size() - target_lt_format_code.norig_dims();
