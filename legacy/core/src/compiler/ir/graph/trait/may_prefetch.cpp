...
void may_prefetch_t::generate_prefetcher_body_for_slice(
...
func_t may_prefetch_t::generate_prefetcher_and_set_idle(
  ...
  if (num_standard_args + 1 == vis.args_.size()) {
    ...
    general_args = builder::make_tensor("__args", {vis.args_.size() - 1},
                                        datatypes::generic);
    ...
    for (size_t i = num_standard_args; i < vis.args_.size(); i++) {
      ...
      func_body->seq_.emplace_back(builder::make_var_tensor_def_unattached(
          arg, linkage::local,
  ...
  builder.push_var_tensor_def(
      realtid, linkage::local,
      builder::make_select(tid_in_arg < 0, builtin::get_thread_id_func()(),
                           tid_in_arg));
  ...
  func_name += "_prefetch";
  auto retfunc =
      builder::make_func(func_name, func_args, func_body, datatypes::index);
  ...
  out_set_idle_code.emplace_back(
      builder::make_evaluate_unattached(make_expr<intrin_call_node>(
          intrin_type::set_thread_idle_func, vis.old_args_, any_map_t{})));
  return retfunc;
}
} // namespace op_traits
} // namespace gc
