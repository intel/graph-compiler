...
public:
  ...
  basic_graph_pass_t(pass_func func, const std::string &name,
                     ...
                     sc_opt_level opt_level = sc_opt_level::lv3,
                     bool enabled = true)
      : name_(name), requires_(required), func_(func), type_(type),
        opt_level_(opt_level), enabled_(enabled) {}
};

using basic_graph_pass_ptr = std::shared_ptr<basic_graph_pass_t>;

basic_graph_pass_ptr
create_graph_pass(const std::string &name, pass_func func,
                  const std::vector<std::string> &required, pass_type type,
                  sc_opt_level opt_level = sc_opt_level::lv3,
                  bool enabled = true);
 ...
 * dynamic_infer_shape
 * */
SC_API sc_graph_t dynamic_shape_infer_preprocess(
    const sc_graph_t &graph, const context_ptr &ctx = get_default_context());
 ...
 * */
SC_API void
graph_driver(sc_graph_t &graph, const context_ptr &ctx = get_default_context(),
             const graph_config *in_cfg = nullptr,
             graph_config *out_cfg = nullptr, int tuner_batch = 0,
             int repeat = 0, int64_t timeout = 0,
