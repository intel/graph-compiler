...
template <typename valT> valT &gt_map_t<valT>::get(graph_tensor *v) {
...
template <typename valT> valT &gt_map_t<valT>::get(const graph_tensor_ptr &v) {
  return get(v.get());
}

template <typename valT> bool gt_map_t<valT>::haskey(graph_tensor *v) const {
...
sc_op_ptr
op_traits::auto_copyable_t::copy(const std::vector<graph_tensor_ptr> &ins,
...
copy_logical_tsr(const std::vector<graph_tensor_ptr> &v) {
...
static std::vector<expr> *get_tensor_dims(const expr &tsr) {
  if (tsr.isa<tensor>()) {
    ...
    COMPILE_ASSERT(
        tsr.isa<tensorptr>(),
        "tensor_slice only accepts a tensor or tensorptr, got: " << tsr);
...
bool tensor_slice::is_const() const {
  return std::all_of(shape_.begin(), shape_.end(), [](const expr &e) {
    return do_cast_and_fold(e).isa<constant>();
  });
}
...
tensor_slice::tensor_slice(const expr &tsr) {
  if (tsr.isa<tensor>()) {
    auto t = tsr.static_as<tensor>();
    tptr_ = builder::tensor_ptr(tsr, std::vector<expr>(t->dims_.size(), 0), {},
                                true)
                .static_as<tensorptr>();
    shape_ = t->dims_;
  } else {
...
tensor_slice::tensor_slice(const expr &tsr, slice_range &&range) {
  ...
  if (dims->size() != range.size())
    COMPILE_ASSERT(
        dims->size() == 1 &&
            get_const_as_int((*dims)[0].checked_as<constant>()) == 1,
        "Unmatched range found. Tensor: "
            << (tsr.isa<tensor>() ? tsr.static_as<tensor>()->name_ : "")
            << " have dims: " << utils::print_vector(*dims)
            << " but got slice range: " << utils::print_pair_vector(range));
  tptr_ =
      builder::tensor_ptr(tsr,
...
graph_tensor::graph_tensor(sc_op *owner, const sc_data_format_t &format,
                           const sc_dims &plain_shape,
                           const sc_data_type_t &type, const sc_dims &stride)
    : details_(format, plain_shape, type, stride), producer_owner_(owner) {}

const sc_dims &logical_tensor_t::get_blocking_dims() const { return dims_; }

std::vector<expr>
logical_tensor_t::get_blocking_dims_expr(sc_graph_t &g) const {
  return get_blocking_shapes_expr(g, plain_dims_, format_);
}

static bool check_stride_validity(const bool is_dynamic, const sc_dims &dims,
                                  const sc_dims &strides) {
  return strides.size() == dims.size() &&
         (is_dynamic || std::is_sorted(strides.begin(), strides.end(),
                                       std::greater<sc_dim>()));
}
...
void logical_tensor_t::set_format_and_stride(const sc_data_format_t &newv,
                                             const sc_dims &strides) {
  format_ = newv;
  strides_ = strides;
  internal_update();
...
void logical_tensor_t::remove_format_candidate(const sc_data_format_t &v) {
  ...
  if (it != format_candidates_.end()) {
    format_candidates_.erase(it);
  }
}
...
bool sc_op::has_graph_output() const {
  for (const auto &output : info_.outputs_) {
    for (const auto &use_node : output->uses_) {
      if (use_node.second->isa<output_op>()) {
        return true;
      }
    }
  }
...
sc_graph_t::sc_graph_t(sc_graph_t &&other)
    : ops_(std::move(other.ops_)), attrs_(std::move(other.attrs_)),
...
void sc_graph_t::reset_op_ids() {
  for (auto it = ops_.begin(); it != ops_.end();) {
    if ((*it)->is_removed_ ||
        ((*it)->get_inputs().empty() && (*it)->get_outputs().empty())) {
      it = ops_.erase(it);
    } else {
      ++it;
    }
...
void sc_graph_t::resort_op_ids(
  ...
  std::sort(ops_.begin(), ops_.end(),
            [&op_id_map](const sc_op_ptr &A, const sc_op_ptr &B) {
              ...
              COMPILE_ASSERT(A_iter != op_id_map.end() &&
                                 B_iter != op_id_map.end(),
...
expr sc_graph_t::dim_to_expr(const sc_dim &v) {
  ...
  if (is_dynamic_dim(v)) {
    ...
    auto it = m.find(v);
    if (it == m.end()) {
      auto dyn_var = make_expr<var_node>(datatypes::index,
                                         dynamic_prefix + std::to_string(-v));
...
std::shared_ptr<sc_op> sc_graph_t::make(
    const std::string &op_name, const std::vector<graph_tensor_ptr> &inputs,
  ...
  for (auto &ins : inputs) {
    if (!ins->producer_owner_) {
      auto in_ret =
          std::make_shared<input_op>(std::vector<graph_tensor_ptr>{ins});
...
std::shared_ptr<sc_op>
sc_graph_t::make_output(const std::vector<graph_tensor_ptr> &inputs,
                        const any_map_t &attrs) {
...
std::shared_ptr<sc_op>
sc_graph_t::make_input(const std::vector<graph_tensor_ptr> &inputs,
                       const any_map_t &attrs) {
  ...
  ret->logical_op_id_ = ops_.size();
  if (ret->is_dynamic()) {
    ret->initialize_dynamic_placeholder();
  }
  ops_.emplace_back(ret);
  return ret;
}
...
bool sc_op::compare_contents(
    const sc_op *other, // NOLINT
    const std::function<bool(const sc_op *, const std::string &)> &filter)
    const {
  if (op_name_ != other->op_name_) {
    return false;
  }
  int numattrs = 0, othernumattrs = 0;
  auto &othermap = other->attrs_.as_map();
  for (auto &kv : attrs_.as_map()) {
    if (utils::string_startswith(kv.first, "temp.")) {
      continue;
    }
    if (filter && !filter(this, kv.first)) {
      continue;
    }
    numattrs++;
    auto otherkv = othermap.find(kv.first);
    if (otherkv == othermap.end()) {
      return false;
    }
    if (kv.second.cmp(otherkv->second) != 0) {
      return false;
    }
  }
...
size_t sc_op::standard_hash_contents(
    const sc_op *p,
    const std::function<bool(const sc_op *, const std::string &)> &filter) {
  size_t seed = 0;
...
float sc_op::get_gflop() { return 0.0f; }
...
expr tensor_detail_to_ir_tensor(sc_graph_t &graph, const std::string &name,
  ...
  auto tsr =
      builder::make_stensor(name, blocking_exprs, tsrd.get_strides_expr(graph),
                            tsrd.dtype_, address_space::automatic, nullptr);
  tsr->attr().set(attr_keys::plain_dims,
                  graph.dims_to_expr(tsrd.get_plain_dims()));
  if (graph.is_dynamic()) {
...
void check_logical_tensor_shape_dtype_identical(const logical_tensor_t &lhs,
  ...
  COMPILE_ASSERT(check_shape_equal(lhs.get_plain_dims(), rhs.get_plain_dims()),
