...
static expr make_global_string(const ir_module_ptr &mod, const std::string &v,
                               int &counter) {
      ...
      if (ltensor->uses_[i].second->isa<tensor_view_op_t>() ||
        ...
        for (auto &cld : next_ltensor->uses_) {
          if (cld.second->isa<output_op>()) {
            ...
            auto cur_linked_out = get_linked_output_tsr(next_ltensor);
            if (cur_linked_out) {
              return cur_linked_out;
            }
          }
        }
      }
...
static bool has_output_uses(const graph_tensor_ptr &ltensor) {
  if (!ltensor->uses_.empty()) {
    for (size_t i = 0; i < ltensor->uses_.size(); i++) {
      if (ltensor->uses_[i].second->isa<output_op>()) {
        return true;
      }
    }
  }
...
struct lowering_visitor_state_t {
  ...
  lowering_visitor_state_t(sc_graph_t &g)
      : topo_sorter_{op_visitor_t::create_DAG_updater(g.ops_.size())},
        op_exec_tick_(g.ops_.size()), op_visited_(g.ops_.size()) {
    max_tensor_size_ = 0;
  ...
  size_t &get_tensor_pending_refcount(const graph_tensor_ptr &p) {
    ...
    if (itr == tensor_pending_refcount_.end()) {
      auto ret =
          tensor_pending_refcount_.insert(std::make_pair(p, p->uses_.size()));
  ...
  int get_op_distance_to_visited_set(sc_op *op, std::vector<int> &d) {
    ...
    for (auto &v : op->get_inputs()) {
      int cur_d = get_op_distance_to_visited_set(v->producer_owner_, d) + 1;
  ...
  float evaluate_op_score(sc_op *op, std::vector<int> &distance_to_visited) {
    ...
    for (auto &in : op->get_inputs()) {
      ...
      if (!in->producer_owner_->isa<input_op>()) {
        ...
        auto tick_diff = cur_tick_ - op_exec_tick_[owner->logical_op_id_];
    ...
    for (auto &out : op->get_outputs()) {
      ...
      if (out->uses_.size() == 1UL && out->uses_[0].second->isa<output_op>()) {
        continue;
      }
      int distance = 1;
      for (auto &use : out->uses_) {
        distance =
            std::max(distance, get_op_distance_to_visited_set(
                                   use.second.get(), distance_to_visited));
      }
      float cur_tsr =
          (distance - 1) * distance_factor +
          (is_dynamic_ ? 1.f
  ...
  op_visitor_t::selector_func get_selector() {
    ...
    return [ths](op_visitor_t *vis) -> sc_op_ptr {
      ...
      if (ths->input_op_itr != vis->to_visit_.end()) {
        ...
        SC_MODULE_INFO << "Scheduling const/input: iter " << ths->cur_tick_
                       << ", Op " << ret->op_name_ << "_"
                       << ret->logical_op_id_;
        return ret;
      }
      ...
      for (auto itr = vis->to_visit_.rbegin(); itr != vis->to_visit_.rend();
           ++itr) {
        auto &op = *itr;
        assert(!op->isa<input_op>() && !op->isa<constant_op_t>());
        float cur_score = ths->evaluate_op_score(op.get(), distance);
        SC_MODULE_INFO << "Scheduling score: iter " << ths->cur_tick_ << ", Op "
      ...
      auto ret = *to_remove;
      SC_MODULE_INFO << "Scheduling selects: iter " << ths->cur_tick_ << ", Op "
                     << ret->op_name_ << "_" << ret->logical_op_id_;
...
std::string get_tensor_name(graph_tensor *t, sc_op *linked_output) {
  ...
  if (t->producer_owner_->get_outputs().size() == 1UL) {
    tensor_name =
        t->producer_owner_->attrs_.get_or_else("temp.name", tensor_name);
  }
  if (tensor_name.empty() && linked_output &&
      linked_output->get_inputs().size() == 1UL) {
    tensor_name = linked_output->attrs_.get_or_else("temp.name", tensor_name);
  }
...
static void add_def_comments(const stmt &def_node, graph_tensor *t) {
  ...
  if (auto old_comments =
          def_node->attr().get_or_null<std::vector<std::string>>("comments")) {
...
expr get_or_create_tensor(general_lower_params_t &gp, const graph_tensor_ptr &t,
  ...
  if (itr == gp.ltsr_rtsr.end()) {
    ...
    if (type == info_etype_t::real_tensor && itr->second.tensor_.defined()) {
  ...
  if (tensor_name.empty()) {
    tensor_name = std::string("buffer_") + std::to_string(itr->second.count_);
  }
  if (type == info_etype_t::real_tensor) {
    bool is_size_dynamic =
        !is_arg && gp.is_graph_dynamic &&
        (t->details_.get_format_candidates().size() > 1 ||
         can_op_query_output(t->producer_owner_->shared_from_this()));
    expr dyn_tsr_size;
    if (is_size_dynamic) {
    ...
    tensor_name += "_format";
    tsr = builder::make_tensor(tensor_name, {UINT64_C(1)}, datatypes::index);
  ...
  if (type == info_etype_t ::real_tensor) {
      ...
      if (const_type != const_kind::not_const) {
        if (const_type == const_kind::global_const) {
          ...
          if (auto cache =
            ...
            folded_name =
                "folded_const_" + std::to_string(gp.global_tensor_counter++);
            tsr = copy_attr(*tsr,
                            gp.ret_mod->make_global_stensor(
                                tsr.checked_as<tensor>()->elem_dtype_,
                                folded_name, tsr.checked_as<tensor>()->dims_,
    ...
    if (def_node.defined()) {
      add_def_comments(def_node, t.get());
    }
  } else if (type == info_etype_t::placeholder) {
    // placeholder
    ...
    if (!itr->second.tensor_.defined()) {
      ...
      gp.func_body->seq_.emplace_back(
          builder::make_evaluate_unattached(builder::make_write_struct(
              tsr, builder::make_constant({dyn_mask_int}, datatypes::u8),
    ...
    if (const_type == const_kind::not_const) {
      gp.func_body->seq_.emplace_back(
          builder::make_var_tensor_def_unattached(tsr));
      gp.func_body->seq_.back()->attr().set(attr_keys::tsr_dont_buf_sched,
                                            true);
    }
  }
  return tsr;
...
expr create_op_query_func(const context_ptr &ctx, general_lower_params_t &gp,
  ...
  for (auto &ltensor : node->get_inputs()) {
    ...
    plhd_ins.emplace_back(get_or_create_tensor(gp, ltensor, false, const_type,
                                               info_etype_t::placeholder));
    fmt_ins.emplace_back(get_or_create_tensor(gp, ltensor, false, const_type,
                                              info_etype_t::format));
  }
  // input before reorder
  if (node->isa<tunable_op_t>() || (node->isa<mixed_fuse_op_t>() &&
    ...
    for (auto i : query_idxs) {
      ...
      auto const_type_before =
          node_before->attrs_.get_or_else("constant", const_kind::not_const);
      ...
      plhd_ins.emplace_back(get_or_create_tensor(
          gp, ltensor, false, const_type_before, info_etype_t::placeholder));
      fmt_ins.emplace_back(get_or_create_tensor(
          gp, ltensor, false, const_type_before, info_etype_t::format));
    }
  }
  auto const_type = node->attrs_.get_or_else("constant", const_kind::not_const);
  for (auto &ltensor : node->get_outputs()) {
    expr plhd, fmt, size;
    if (node->isa<input_op>()) {
      // use real tensor instead of placeholder.
      plhd = get_or_create_tensor(gp, ltensor, true, const_type,
                                  info_etype_t::real_tensor);
      fmt = get_or_create_tensor(gp, ltensor, true, const_type,
                                 info_etype_t::format);
    } else if (node->isa<constant_op_t>()) {
      plhd = get_or_create_tensor(gp, ltensor, false, const_kind::global_const,
                                  info_etype_t::real_tensor);
      fmt = get_or_create_tensor(gp, ltensor, false, const_kind::global_const,
                                 info_etype_t::format);
    } else {
      ...
      fmt = get_or_create_tensor(gp, ltensor, false, const_type,
                                 info_etype_t::format);
      size = get_or_create_tensor(gp, ltensor, false, const_type,
                                  info_etype_t::out_size);
    }
  ...
  if (!op_dispatch_kernel[node->logical_op_id_].defined()) {
    ...
    auto func_name =
        node->op_name_ + "__" + std::to_string(node->logical_op_id_) + "_ptr";
    ...
    dyn_ker_ptr = builder::make_tensor(
        func_name, {static_cast<uint64_t>(1 + internal_func_num)},
        datatypes::index);
    std::vector<expr> query_func_args;
    query_func_args.emplace_back(table_var);
    query_func_args.insert(query_func_args.end(), plhd_outs.begin(),
                           plhd_outs.end());
    query_func_args.insert(query_func_args.end(), plhd_ins.begin(),
                           plhd_ins.end());
    query_func_args.insert(query_func_args.end(), fmt_outs.begin(),
                           fmt_outs.end());
    query_func_args.insert(query_func_args.end(), fmt_ins.begin(),
                           fmt_ins.end());
    query_func_args.insert(query_func_args.end(), size_outs.begin(),
                           size_outs.end());
    ...
    if (node->isa<mixed_fuse_op_t>()) {
      ...
      assert(table_ptr);
      query_call =
          builder::make_call(query_mod->get_entry_func(), query_func_args);
    } else {
...
std::pair<expr, expr> get_reshape_tptr(general_lower_params_t &gp,
                                       const graph_tensor_ptr &old_tsr,
                                       const graph_tensor_ptr &new_tsr,
                                       int const_type, op_kinds kind) {
  auto base_tsr = get_or_create_tensor(gp, old_tsr, kind == kinput, const_type);
  size_t ndims;
...
void create_op_tensors(general_lower_params_t &gp, std::vector<expr> &ins,
                       std::vector<expr> &outs, const sc_op_ptr &node,
                       op_kinds kind) {
  int const_type = node->attrs_.get_or_else("constant", const_kind::not_const);
  for (auto &ltensor : node->get_inputs()) {
  ...
  for (auto &ltensor : node->get_outputs()) {
    if (kind == kconstant) {
      ...
      auto out_tsr_pair = get_reshape_tptr(gp, node->get_inputs()[0], ltensor,
                                           const_type, kind);
      ...
      if (it != gp.ltsr_rtsr.end() && it->second.tensor_.defined()) {
        ...
        tv_replace_map.insert(
            std::make_pair(it->second.tensor_, out_tsr_pair.second));
        tv_tsr_replacer_t cpy(tv_replace_map, false);
        gp.func_body =
            cpy.dispatch(gp.func_body).remove_const().checked_as<stmts>();
        gp.init_body =
            cpy.dispatch(gp.init_body).remove_const().checked_as<stmts>();
      }
      ...
      if (auto out_tsr = get_linked_output_tsr(ltensor)) {
        gp.ltsr_rtsr[ltensor].tensor_ =
            get_reshape_tptr(gp, out_tsr, ltensor, const_type, kind).second;
        outs.emplace_back(gp.ltsr_rtsr[ltensor].tensor_);
      } else {
        outs.emplace_back(
            get_or_create_tensor(gp, ltensor, kind == kinput, const_type));
      }
    }
  }
}
...
static void
dynamic_reshape_var_assignment(general_lower_params_t &gp,
                               std::unordered_set<expr> &dynamic_var_set,
                               const sc_op_ptr &node) {
  auto &graph = gp.graph;
  ...
  for (size_t i = 0; i < out_plain_dims.size(); i++) {
    ...
    if (!is_dynamic_dim(dyn_dim)) {
      continue;
    }
    auto var = graph.dim_to_expr(dyn_dim);
    if (dynamic_var_set.find(var) == dynamic_var_set.end()) {
      gp.func_body->seq_.emplace_back(builder::make_var_tensor_def_unattached(
...
static func_t
insert_prefetch(const context_ptr &ctx,
                const std::vector<std::pair<sc_op_ptr, stmt>> &op_execution_log,
                sc_op *node, const std::vector<expr> &input_tsr,
                ir_module_t &mod, std::vector<stmt> &outbody) {
  if (!ctx->flags_.prefetch_) {
    return func_t();
  }
  if (op_execution_log.empty()) {
    return func_t();
  }
    ...
    if (op_execution_log[i].second.defined()) {
      ...
      if (is_good_for_insert) {
        auto func = op_execution_log[i]
                        ...
                        ->func_;
        if (func->attr_ &&
            func->attr_->get_or_else(function_attrs::has_idle_func, false)) {
  ...
  for (auto in_idx : can_prefetch_index) {
    ...
    if (producer_idx <= pos) {
      if (!input_tsr.at(in_idx).isa<tensor>()) {
        SC_MODULE_WARN << "Cannot prefetch tensor_view" << input_tsr.at(in_idx);
      } else {
  ...
  if (filtered_prefetch_index.empty()) {
    return func_t();
  }
  std::vector<stmt> out;
...
static void add_func_doc_string(const func_t &f) {
  ...
  for (auto &p : f->params_) {
    std::stringstream ss;
    ss << "@param " << p << ' ' << p->attr().get<std::string>("temp.comments");
...
stmt_base_t *has_no_dep_with_prev(
    const sc_op_ptr &node, const op_dep_matrix_t &dep_mat,
    const std::vector<std::pair<sc_op_ptr, stmt>> &op_execution_log) {
...
ir_module_ptr lower_graph(context_ptr ctx, sc_graph_t &graph,
  ...
  op_visitor_t vis{visiter_state.get_selector(), visiter_state.get_updater(),
                   true};
  ...
  auto graph_name =
      graph.attrs_.get_or_else<std::string>("temp.name", default_graph_name);
  // todo: use graph-id to generate name
  auto func =
      builder::make_func(graph_name, params, func_body, datatypes::void_t);
  ...
  // func
  std::vector<expr> shared_consts;
  general_lower_params_t gp{ret_mod,
                            ltsr_rtsr,
                            graph,
                            func_body,
                            init_body,
                            tensor_counter,
                            global_tensor_counter,
                            is_graph_dynamic,
                            /*num_lazy_init_const_tsr*/ 0,
                            /*num_inited_shared_const_tsr_*/ 0,
                            external_dyn_vars,
                            shared_consts};
  ...
  expr extra_internal_func_arg = builder::make_tensor(
      "extra_internal_funcs", {total_num_of_internal_funcs}, datatypes::index);
  extra_internal_func_arg->attr()["temp.comments"] =
      std::string("Extra tensor arg contains pointer to internal funcs");
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    ...
    if (is_graph_dynamic && can_op_be_queried(node)) {
      auto create_op_query_func_wrapper =
          std::bind(create_op_query_func, ctx, std::ref(gp),
                    std::ref(op_dispatch_kernel), std::placeholders::_1);
      lower_query_function(query_visited, node, create_op_query_func_wrapper);
    }
    ...
    default: {
      ...
      if (is_graph_dynamic && can_op_be_dispatched(node)) {
        ...
        if (node->need_dynamic_internal_query()) {
          node->info_.internal_info_->parti_in_ltsrs_ =
              graph::extract_detail_from_tensors(node->get_inputs());
          node->info_.internal_info_->parti_out_ltsrs_ =
              graph::extract_detail_from_tensors(node->get_outputs());
          create_internal_dispatch_funcs_by_node(ctx, ret_mod, table_name, node,
                                                 use_managed_tp);
          ...
          exprargs.emplace_back(builder::tensor_ptr(ker_tsr, {1}));
        }
          ...
          if (!inp_hint.empty()) {
            ...
            for (auto &kv : inp_hint) {
              ...
              for (auto &info : kv.second) {
                ...
                int new_idx = input_id + node->get_outputs().size();
                out_hint.back().second.emplace_back(
                    tensor_inplace_info_t{new_idx, info.kind_});
              }
            }
            func->attr()[function_attrs::inplace_hint] = std::move(out_hint);
          }
        }
        ret_mod->merge(*mod);
        ...
        if (need_extra_internal_func_arg &&
          ...
          int cur_internal_funcs = get_num_of_internal_funcs(node);
          exprargs.emplace_back(
              builder::tensor_ptr(extra_internal_func_arg, {cur_internal_idx}));
        ...
        if (node->isa<concat_op_t>()) {
          kernel_call->attr()[concat_optim_attr_keys::is_standalone_concat] =
              true;
        }
        if (mark_as_main && const_type == const_kind::not_const) {
          insert_prefetch(ctx, op_execution_log, node.get(), ins, *ret_mod,
                          func_body->seq_);
        }
      }
      stmts_node_t *target_body = (const_type != const_kind::not_const)
                                      ? init_body.get()
                                      : func_body.get();
      target_body->seq_.emplace_back(
      ...
      if (const_type == const_kind::not_const) {
        ...
        if (auto marked_stmt =
                has_no_dep_with_prev(node, dep_mat, op_execution_log)) {
          marked_stmt->attr()[attr_keys::no_post_barrier] = callee_name;
        }
        op_execution_log.emplace_back(node, target_body->seq_.back());
        num_ops++;
      }
    }
    }
  ...
  if (!args.empty()) {
    ...
    COMPILE_ASSERT(new_param.size() == params.size(),
                   ...
                   "tensors, new_param.size="
                       << new_param.size() << ", param.size()=" << params.size()
                       << ".");
  ...
  if (!init_body->seq_.empty()) {
    ...
    if (gp.num_lazy_init_shared_const_tsr_ == 0) {
      ...
      is_init_var = ret_mod->make_global_var(
          datatypes::boolean, "is_init", linkage::private_global,
      ...
      to_insert.back()->attr()[attr_keys::is_shared_const_init_stmt] = true;
      to_insert.emplace_back(
    ...
    func_t init_func = builder::make_func(
        "__init_const_globals", params_for_init, init_body, datatypes::void_t);
    ...
    stmt const_init = builder::make_if_else_unattached(
        ...
        builder::make_stmts_unattached({builder::make_evaluate_unattached(
            builder::make_call(init_func, params_for_init))}),
        stmts());
    ...
    if (gp.num_inited_shared_const_tsr_ + gp.num_lazy_init_shared_const_tsr_ ==
      ...
      // shared consts
      for (auto itr = func_body->seq_.begin() + to_insert.size();; itr++) {
        if (itr == func_body->seq_.end() ||
            !(*itr)
                 .cast<define_c>()
                 .map([](const define_c &v) { return v->var_.as<tensor>(); })
                 .filter([](const tensor &v) {
                   return v->attr_ &&
                          v->attr_->has_key(attr_keys::shared_const);
                 })
                 .has_value()) {
  ...
  if (mark_as_main)
    func->attr()[function_attrs::is_main] = true;
    ...
    for (auto &table : ret_mod->get_op_table_map()) {
      for (auto &kv : table.second->kernel_table_) {
        if (kv.second.already_compiled()) {
          if (auto f = ret_mod->get_func(kv.second.name_or_postfix_)) {
