...
void op_visitor_t::assert_all_ops_visited(const sc_graph_t &mgr,
                                          size_t concerned_size) {
#ifndef NDEBUG
  ...
  for (size_t i = 0; i < concerned_size; i++) {
    const auto &op = mgr.ops_[i];
    COMPILE_ASSERT(
        op->is_removed_ || has_visited(op->logical_op_id_),
...
void op_sorting_visitor_t::visit_by_rules(
    sc_graph_t &graph, const std::function<void(sc_op_ptr)> &f,
...
void op_sorting_visitor_t::visit_by_rules(
  ...
  op_visitor_t::dfs_topology_sort(op_size).visit_graph(
      graph, [&op_seq, &dep_matrix](op_visitor_t *vis, const sc_op_ptr &cur) {
...
op_sorting_visitor_t::sort_by_rules(sc_graph_t &graph,
                                    std::vector<sort_rule> rules) {
...
op_sorting_visitor_t::rule_func op_sorting_visitor_t::create_same_kind_rule() {
  return [](std::vector<sc_op_ptr> &op_seq, const op_dep_matrix_t &dep_matrix) {
    // op kind taxonomy
    auto get_op_kind = [](const sc_op_ptr &cur) {
      if (cur->isa<unary_elementwise_op_t>()) {
        return op_kind::elementwise;
      } else if (auto belemop = cur->dyn_cast<binary_elementwise_op_t>()) {
...
op_visitor_t::updater_func
op_visitor_t::create_DAG_updater(size_t total_hint,
                                 const user_sort_func &sorter) {
  struct count_t {
    int count = -1;
  };
  // the count of pending depending logical tensors for each node
  std::vector<count_t> pending_count;
  return [pending_count, total_hint, sorter](op_visitor_t *v,
                                             const sc_op_ptr &cur) mutable {
    v->set_visited(cur->logical_op_id_);
    for (auto &lt : cur->get_outputs()) {
...
op_visitor_t::updater_func
op_visitor_t::create_DAG_updater_post(size_t total_hint) {
  struct count_t {
    int count = -1;
  };
  // the count of pending depending logical tensors for each node
  std::vector<count_t> pending_count;
  return [pending_count, total_hint](op_visitor_t *v,
                                     const sc_op_ptr &cur) mutable {
    v->set_visited(cur->logical_op_id_);
    for (auto &lt : cur->get_inputs()) {
      ...
      if (pending_count[id].count == -1) {
        ...
        // count
        size_t num_count = 0;
        for (const graph_tensor_ptr &out : lt->producer_owner_->get_outputs()) {
      ...
      // all dependencies resolved, we can visit it now
      if (pending_count[id].count == 0) {
        v->to_visit_.emplace_back(lt->producer_owner_->shared_from_this());
      }
    }
  };
}
...
static std::vector<int> usr_speculative_sorter(const graph_tensor_ptr &gt) {
  ...
  auto sort_by_prority = [&priority_index_list, &visit_index]() {
    std::sort(
        priority_index_list.begin(), priority_index_list.end(),
...
op_visitor_t::updater_func
op_visitor_t::create_DAG_updater_speculative(size_t total_hint) {
...
sc_op_ptr op_visitor_t::pop_back_selector(op_visitor_t *v) {
  ...
  if (v->has_visited(ret->logical_op_id_)) {
    return nullptr;
  }
  return ret;
}
...
op_visitor_t op_visitor_t::dfs_topology_sort(size_t total_nodes_hint) {
  return op_visitor_t(pop_back_selector, create_DAG_updater(total_nodes_hint),
                      true);
}

op_visitor_t
op_visitor_t::dfs_topology_speculative_sort(size_t total_nodes_hint) {
...
op_visitor_t op_visitor_t::bfs_topology_sort(size_t total_nodes_hint) {
  return op_visitor_t(dequeue_selector, create_DAG_updater(total_nodes_hint),
                      true);
}
...
op_visitor_t
op_visitor_t::dfs_topology_sort_unchecked(size_t total_nodes_hint) {
  return op_visitor_t(pop_back_selector, create_DAG_updater(total_nodes_hint),
                      false);
}

void op_visitor_t::post_visit_graph(const sc_graph_t &mgr,
                                    const visitor_func &f) {
  for (auto &v : mgr.ops_) {
