...
graph_identity get_identity_by_topology(const sc_graph_t &g) {
  ...
  vis.visit_graph(g,
                  [&seed, &identity](op_visitor_t *vis, const sc_op_ptr &op) {
...
void query_binding_axis(sc_graph_t &g) {
  ...
  if (cache && cache->identity_ == g_identity) {
    return;
  }
  ...
  auto query_based_on_gt = [&gax_map](const graph_tensor_ptr &gt) {
    ...
    if (gax_map.find(key) != gax_map.end())
      return;
    // skip any format
    if (gt->details_.get_format().is_any())
      return;
    // set initial axis
    ...
    if (auto cur =
            producer_op->dyn_cast<op_traits::mixed_partition_acceptable>()) {
...
void bind_loop_axis(const graph_tensor_ptr &gt, const for_loop &loop,
  ...
  // set attr
  loop->attr()[binding_axis_attr::loop_hint] = loop_binding_axis_hint{
      global_ptr, (uintptr_t)gt.get(),
      is_block ? transform_axis_blocking2plain(gt->details_, axis) : axis};
}

void bind_loop_axis(const graph_tensor_ptr &gt, const stmt &loop, int axis,
                    bool is_block) {
  COMPILE_ASSERT(loop.isa<for_loop>(),
                 "loop node is expected, but got " << loop)
  bind_loop_axis(gt, loop.static_as<for_loop>(), std::vector<int>{axis},
                 is_block);
}

void bind_loop_axis(const graph_tensor_ptr &gt,
                    const std::vector<for_loop> &loops,
                    const std::vector<int> &axis, bool is_block) {
...
std::vector<int>
transform_binding_axis_with_indices(const binding_axis &axis,
...
bool get_aligned_binding_axis(const loop_binding_axis_hint &hint_a,
                              const loop_binding_axis_hint &hint_b,
                              std::vector<int> &axis_a,
  ...
  // same graph
  if (hint_a.global_map_ptr_ != hint_b.global_map_ptr_) {
    return false;
  }
  ...
  if (global_map.find(hint_b.key_) == global_map.end()) {
    return false;
  }
  auto &map_b = global_map[hint_b.key_];

  // query binding axis of A from hint B
  if (map_b.find(hint_a.key_) == map_b.end()) {
    return false;
  }
...
bool check_loop_binding_axis(const for_loop_node_t *loop_a,
                             const for_loop_node_t *loop_b) {
...
int check_loop_binding_axis(const std::vector<for_loop> &loop_a,
                            ...
                            int64_t check_loop_size) {
  size_t gcs = std::min(loop_a.size(), loop_b.size());
  if (check_loop_size >= 0) {
    gcs = std::min(gcs, (size_t)check_loop_size);
  }
...
bool check_loop_has_axis(const for_loop &loop, const graph_tensor_ptr &gt,
  ...
  if (global_map.find(hint->key_) == global_map.end())
    return false;
  ...
  // query binding axis
  if (self_map.find(target_key) == self_map.end())
    return false;
  auto &target_binding_axis = self_map[target_key];
  // get target_axis based on self_axis from view of target graph tensor
  std::vector<int> target_axis =
      transform_binding_axis_with_indices(target_binding_axis, self_axis);
...
void copy_binding_axis_hint(const for_loop_node_t *ths,
                            for_loop_node_t *other) {
...
void fuse_binding_axis_hint(const for_loop_node_t *ths,
                            const for_loop_node_t *other) {
  ...
  if (!ths_hint_ptr || !other_hint_ptr)
    return;

  loop_binding_axis_hint hint_ths = *ths_hint_ptr, hint_other = *other_hint_ptr;
  ...
  } else if (get_aligned_binding_axis(hint_other, hint_ths, axis_other,
                                      axis_ths)) {
    auto merged_axis = merge_axis(axis_ths, axis_other);
