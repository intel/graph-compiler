...
public:
  ...
  bool operator==(const logical_tensor_t &other) const {
    return format_ == other.format_ && dtype_ == other.dtype_ &&
           plain_dims_ == other.plain_dims_ && strides_ == other.strides_;
  }
  ...
  void set_format_and_stride(const sc_data_format_t &newv,
  ...
  compute_dense_stride_expr(sc_graph_t &graph, const std::vector<expr> &dims);
  size_t hash() const;

private:
  template <typename T, typename Dummy> friend struct reflection::type_registry;
...
namespace std {
template <> struct hash<dnnl::impl::graph::gc::logical_tensor_t> {
  std::size_t
  operator()(const dnnl::impl::graph::gc::logical_tensor_t &k) const;
};
} // namespace std
#endif
