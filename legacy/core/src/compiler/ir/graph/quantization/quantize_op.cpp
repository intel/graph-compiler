...
static void common_query_function(
    sc_op *node, const sc_data_format_t &out_format,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  std::vector<std::vector<sc_data_format_t>> in_formats, out_formats;
  out_formats.push_back({out_format});
  node->format_to_dense_format_stride_pair(in_formats, out_formats,
...
quantize_infos_t get_quantize_info_from_attrs(const any_map_t &attrs) {
  ...
  infos.scales_ = attrs.get_or_else(attr_keys::scales, std::vector<float>{1.f});
  infos.zero_points_ =
         ...
         (infos.asymmetric_ ||
          (!infos.asymmetric_ &&
           (infos.zero_points_.empty() ||
            (infos.zero_points_.size() == 1 && infos.zero_points_[0] == 0)))));
  return infos;
}

static void
check_dynamic_quantization_schema(const quantize_infos_t &qinfos,
  ...
  if (zps) {
    COMPILE_ASSERT(
        utils::is_one_of(zps->details_.dtype_.type_code_, sc_data_etype::U8,
  ...
  if (qinfos.per_channel_) {
    ...
    if (zps) {
      COMPILE_ASSERT(gc::graph::check_shape_equal(
                         zps->details_.get_plain_dims(), sc_dims{channel_size}),
...
void quantize_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  auto quantize_const_scales = graph->make("constant", {}, {},
                                           {{"values", scales_ptr},
                                            ...
                                            {"all_positive", true}});
  auto div_scale = graph->make(
      "mul", {inputs[0], quantize_const_scales->get_outputs()[0]}, {}, {});
  ...
  if (!zeropoints.empty()) {
    ...
    if (!zp_all_zero) {
      ...
      auto quantize_const_zeropoints = graph->make(
          "constant", {}, {},
          {{"values", zeropoints_ptr},
           {"dtype", datatypes::f32},
           {"plain_dims", sc_dims{static_cast<sc_dim>(zeropoints_f32.size())}},
           {"format", sc_data_format_t()}});
...
void quantize_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
...
void dequantize_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  if (scales.size() > 1) {
    scales_plain_dims.resize(inputs[0]->details_.get_plain_dims().size(), 1);
    scales_plain_dims[qinfos.channel_axis_] = static_cast<int>(scales.size());
  }
  ...
  if (inputs[0]->details_.dtype_.type_code_ != sc_data_etype::F32) {
    f32_cast =
        graph->make("cast", ins->get_outputs(), {}, {{"dtype", qinfos.dtype_}});
  }

  bool all_zero =
      std::all_of(qinfos.zero_points_.begin(), qinfos.zero_points_.end(),
                  [](int x) { return x == 0; });
  if (!all_zero) {
    std::vector<float> zero_points(qinfos.zero_points_.begin(),
                                   qinfos.zero_points_.end());
    auto const_zero_points = graph->make(
        "constant", {}, {},
        {{"values", std::make_shared<static_data_t>(zero_points)},
         {"dtype", datatypes::f32},
         {"plain_dims", sc_dims{static_cast<sc_dim>(zero_points.size())}},
         {"format", sc_data_format_t()}});
    f32_cast = graph->make(
        "sub",
        {f32_cast->get_outputs()[0], const_zero_points->get_outputs()[0]}, {},
        {});
  }
  auto mul_scale = graph->make(
      "mul", {f32_cast->get_outputs()[0], const_scales->get_outputs()[0]}, {},
      {});
  graph->make_output(mul_scale->get_outputs());
}

void dequantize_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  common_query_function(this, info_.inputs_[0]->details_.get_format(),
...
void dynamic_quantize_op_t::get_graph_impl(std::shared_ptr<sc_graph_t> &graph) {
  ...
  if (inputs.size() == 3) {
    auto zp_cast =
        graph->make("cast", {inputs[2]}, {}, {{"dtype", datatypes::f32}});
    div_scale = graph->make(
        "add", {div_scale->get_outputs()[0], zp_cast->get_outputs()[0]}, {},
        {{"bc_axis", bc_axis}});
  }
  auto int8_cast = graph->make("cast", div_scale->get_outputs(), {},
                               {{"dtype", qinfos.dtype_}, {"saturated", true}});
...
void dynamic_quantize_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  common_query_function(this, info_.inputs_[0]->details_.get_format(),
...
void dynamic_dequantize_op_t::get_graph_impl(
  ...
  if (inputs.size() == 3) {
    auto zp_cast =
        graph->make("cast", {inputs[2]}, {}, {{"dtype", datatypes::f32}});
    f32_cast = graph->make(
        "sub", {f32_cast->get_outputs()[0], zp_cast->get_outputs()[0]}, {},
        {{"bc_axis", bc_axis}});
  }
...
void dynamic_dequantize_op_t::query_format(
    context_ptr ctx,
    std::vector<std::vector<format_stride_pair>> &supported_ins,
    std::vector<std::vector<format_stride_pair>> &supported_outs) {
  common_query_function(this, info_.inputs_[0]->details_.get_format(),
