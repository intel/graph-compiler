...
inline uint16_t vectorize_step(const context_ptr &ctx, sc_data_etype detype) {
  ...
  if (!ctx->machine_.cpu_flags_.fAVX512F && avx2_lanes_require_dtype(detype)) {
...
struct mask_compute_func_t {
  ...
  stmt operator()(const std::vector<expr> &in,
                  ...
                  const expr &cur_idx = expr(),
                  const expr &upper_bound = expr(), uint32_t lanes = 16) const;
  std::function<stmt(const std::vector<expr> &,
                     std::vector<expr::lvalue_proxy_t> &)>
      impl_;
};
