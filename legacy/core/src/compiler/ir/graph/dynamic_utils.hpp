...
struct op_dispatch_tables_t {
  ...
  struct op_func_info {
     ...
     */
    op_func_info(
        const std::shared_ptr<sc_graph_t> &graph,
        const std::shared_ptr<sc_op> &op, const std::string &name_or_postfix,
        const std::shared_ptr<context_t> &ctx,
        const std::shared_ptr<const thread_pool_mode_t> &use_managed_tp,
        bool internal = false)
        : graph_(graph), op_(op), name_or_postfix_(name_or_postfix), ctx_(ctx),
          use_managed_tp_(use_managed_tp), internal_(internal) {}
    /**
...
void initialize_format_table_with_op(const std::shared_ptr<sc_op> &op,
...
void add_dispatch_symbol_to_kernel_table(
    op_dispatch_tables_ptr &tb, const op_dispatch_key_base_t *keys,
...
find_parent_dispatch_node(const std::shared_ptr<graph_tensor> &in);
...
op_layout_link_vec_t get_op_layout_link_relationships(
    ...
    const std::vector<std::shared_ptr<dispatch_key_set_base_t>> &dispatch_keys,
...
void lower_query_function(
    std::vector<bool> &visited, const std::shared_ptr<sc_op> &node,
...
get_graph_inner_dispatch_ops(sc_graph_t &graph, int *total_num_key);
void update_graph_format_by_key(
    const std::shared_ptr<context_t> &ctx,
...
expr call_op_dynamic_query_function(const std::shared_ptr<sc_op> &op,
...
SC_API runtime::dynamic_tensor_t
convert_graph_tensor_to_dynamic_tensor(const std::shared_ptr<graph_tensor> &in,
