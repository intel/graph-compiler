...
bool fusion_partition_t::is_ok_to_add(sc_op *op,
                                      const op_dep_matrix_t &g) const {
  if (merged_to) {
    return get_root()->is_ok_to_add(op, g);
  }
  for (auto &in : op->get_inputs()) {
    // if the input is in the partition, don't need to check
    if (contains(in->producer_owner_)) {
      continue;
    }
    // if "in" depends on an op in the partition
    if (main_tunable_op && g.lookup(main_tunable_op->logical_op_id_,
                                    in->producer_owner_->logical_op_id_) == 1) {
      return false;
    }
...
func_t create_func_decl_for_op(sc_op *op, std::vector<expr> &ins,
  ...
  ins = ins.empty() ? graph::tensor_detail_to_ir_tensor(graph, "__ins_",
                                                        op->get_inputs())
                    : ins;
  outs = outs.empty() ? graph::tensor_detail_to_ir_tensor(graph, "__outs_",
                                                          op->get_outputs())
                      : outs;
...
func_t create_query_func_decl_for_op(
  ...
  if ((op->isa<mixed_fuse_op_t>() && !op->stc_cast<mixed_fuse_op_t>()
    ...
    for (size_t i = 0; i < ori_sz; i++) {
      auto ori_in = graph::tensor_detail_to_ir_tensor(
          graph, std::string("__ori_ins_") + std::to_string(i),
          op->get_inputs()[i]->details_);
      ori_in->attr().set(attr_keys::always_trans, true);
      ori_ins.emplace_back(ori_in);
      ori_in_fmts.emplace_back(
          builder::make_tensor(ori_in.static_as<tensor>()->name_ + "_format",
  ...
  std::transform(
      outs.begin(), outs.end(), out_sizes.begin(), [](const expr &in) {
        return builder::make_tensor(in.static_as<tensor>()->name_ + "_size",
                                    {UINT64_C(1)}, datatypes::index);
      });
  ...
  kernel = builder::make_tensor(
      "func_kernel", {1 + get_num_of_internal_funcs(op->shared_from_this())},
      datatypes::index);
...
tsr_info_t get_or_create_tsr_and_fmt(general_fused_params_t &gp,
                                     const graph_tensor_ptr &in) {
  auto it = gp.ltsr_rtsr.find(in);
  if (it != gp.ltsr_rtsr.end()) {
    return it->second;
  }
  auto &bld = gp.bld;
  auto rtsr =
  ...
  auto out_fmt = builder::make_tensor(
      "format_" + std::to_string(gp.inner_tsr_count), {1UL}, datatypes::index);
  bld.push_var_tensor_def(rtsr);
  bld.push_var_tensor_def(shape_tsr);
  bld.push_evaluate(
      builder::make_write_struct(rtsr, shape_tsr, dyn_tsr_struct_t::name,
                                 dyn_tsr_struct_t::fields::dim_ptr));
  bld.push_evaluate(builder::make_write_struct(
      rtsr,
      builder::make_constant({in->details_.get_plain_dims().size()},
                             datatypes::s32),
      dyn_tsr_struct_t::name, dyn_tsr_struct_t::fields::ndims));
  uint64_t etype =
      in->details_.dtype_.is_etype_pointer()
          ? in->details_.dtype_.get_pointer_element().as_etype_int()
          : in->details_.dtype_.as_etype_int();
  bld.push_evaluate(builder::make_write_struct(
      rtsr, builder::make_constant({etype}, datatypes::u32),
      dyn_tsr_struct_t::name, dyn_tsr_struct_t::fields::dtype));
  auto plain_shapes =
      gp.node->get_owner_graph().dims_to_expr(in->details_.get_plain_dims());
  uint64_t dyn_mask_int = 0;
  for (size_t i = 0; i < plain_shapes.size(); i++) {
    bld.push_assign(builder::make_indexing(shape_tsr, {i}), plain_shapes[i]);
    dyn_mask_int |= (uint64_t(!plain_shapes[i].isa<constant>()) << i);
  }
  bld.push_evaluate(builder::make_write_struct(
      rtsr, builder::make_constant({dyn_mask_int}, datatypes::u8),
...
static bool need_inner_query(general_fused_params_t &gp, const sc_op_ptr &node,
                             int &main_idx) {
  ...
  if (node->isa<binary_elementwise_op_t>()) {
    int bc_idx =
        node->stc_cast<binary_elementwise_op_t>()->get_broadcast_input();
  ...
  for (size_t i = 0; i < outputs.size(); i++) {
    ...
    for (size_t j = 0; j < out->uses_.size(); j++) {
      if (out->uses_[j].second->isa<output_op>()) {
        return true;
      }
    }
  }
...
void declare_dummy_and_combined_tsrs(general_fused_params_t &gp,
                                     int total_key_num, int dispatch_op_num) {
  auto &bld = gp.bld;
  bld.push_scope();
  // create dummy tensor/var for inner query.
  gp.exprs.dummy_kernel = builder::make_var(datatypes::pointer, "dummy_kernel");
  ...
  if (total_key_num) {
    expr combined_keys = builder::make_tensor("combined_keys", {total_key_num},
                                              datatypes::pointer);
...
void set_original_tensor_and_format_for_tunables(
    general_fused_params_t &gp, sc_op *node_before,
    const std::vector<expr> &ori_ins, const std::vector<expr> &ori_in_fmts,
    expr &ori_tsr, expr &ori_fmt) {
  tsr_info_t tsr_info;
...
void create_query_function_by_graph(
    general_fused_params_t &gp, const expr &kernel,
    const std::vector<expr> &ori_ins, const std::vector<expr> &ori_in_fmts,
    std::vector<int> &each_op_num_keys, int total_key_num, int dispatch_op_num,
    const std::vector<size_t> &tunable_inp_indices = std::vector<size_t>()) {
  auto &bld = gp.bld;
  ...
  auto create_internal_query_func = [&](const sc_op_ptr &op) {
    ...
    if (op->isa<ops::matmul_core_op_t>() ||
      ...
      set_original_tensor_and_format_for_tunables(
          gp, node_before_in0, ori_ins, ori_in_fmts, ori_in0, ori_in_fmt0);
      set_original_tensor_and_format_for_tunables(
          gp, node_before_in1, ori_ins, ori_in_fmts, ori_in1, ori_in_fmt1);
      add_global_table_var(gp, table_name, table_ptr, table_var);
      auto internal_kernel =
          op->need_dynamic_internal_query()
      ...
      // set combined tensor
      bld.push_assign(
          builder::make_indexing(combined_keys, {cur_combined_key_idx++}),
          op_ins[0].format_);
      bld.push_assign(
          builder::make_indexing(combined_keys, {cur_combined_key_idx++}),
          op_ins[1].format_);
      ...
      if (need_inner_query(gp, op, main_idx)) {
        ...
        std::vector<expr> args = {table_var,         op_outs[0].tensor_,
                                  ...
                                  op_ins[0].format_, op_outs[0].size_,
                                  dummy_kernel};
        bld.push_evaluate(call_op_dynamic_query_function(op, args));
      } else {
      ...
      if (need_inner_query(gp, op, main_idx)) {
        ...
        std::vector<expr> args = {
            ...
            op_ins[1].tensor_, op_outs[0].format_, op_ins[0].format_,
      ...
      std::vector<expr> args = {
          table_var,         op_outs[0].tensor_, op_ins[0].tensor_,
          op_ins[1].tensor_, op_ins[2].tensor_,  op_outs[0].format_,
          op_ins[0].format_, op_ins[1].format_,  op_ins[2].format_,
          op_outs[0].size_,  dummy_kernel};
      bld.push_evaluate(call_op_dynamic_query_function(op, args));
    } else if (op->isa<shape_of_tensor_op_t>()) {
      // do nothing
    } else {
      COMPILE_ASSERT(false, "Currently dynamic fusbile op only support "
                            "unary/binary.");
    }
  ...
  auto main_table_var = builder::make_var(datatypes::pointer, main_table_name);
  ...
  expr each_op_num_keys_tsr =
      builder::make_tensor("each_op_num_keys", {dispatch_op_num},
  ...
  gp.modu->add_global_var(builder::make_var_tensor_def_unattached(
                              ...
                              .static_as<define>());
  bld.push_evaluate(builtin::call_fused_op_query_combined(
      main_table_var, combined_keys.defined() ? combined_keys : get_ir_null(),
      combined_algs.defined() ? combined_algs : get_ir_null(),
      each_op_num_keys_tsr, dispatch_op_num, kernel));
}

mixed_fuse_op_t::mixed_fuse_op_t(
    const std::string &name, const std::vector<mixed_parti_t::ptr> &parti_list,
...
bool mixed_fuse_op_t::need_dynamic_internal_query_impl() const {
  return !std::all_of(
      sub_graph_.ops_.begin(), sub_graph_.ops_.end(),
      [](const sc_op_ptr &op) { return !op->need_dynamic_internal_query(); });
}
  ...
  if (!use_cache && can_op_be_dispatched(shared_from_this())) {
    ...
    if (need_dynamic_internal_query()) {
      auto internal_func_arg = builder::make_tensor(
          "extra_internal_funcs", {get_num_of_internal_funcs(sub_graph_)},
          datatypes::index);
    ...
    stmt policy_dispatch;
    {
      ...
      max_loop_parallel_modu = lower_graph(ctx, cpy_graph, lower_args, false);
      ...
      max_loop_parallelism_cond =
          cpy_graph.attrs_.get<expr>("temp.fusion_policy_condition");
    ...
    if (max_loop_parallelism_cond->equals(expr(true))) {
      ...
      max_fusion_func->name_ += "_max_fusion_" + std::to_string(logical_op_id_);
    ...
    func->body_ =
        builder::make_stmts_unattached({policy_dispatch, return_stmt});
    modu->add_func({func});
...
void schedule_loop_body(const stmt &body, node_ptr_map *node_remap) {
  ...
  while (true) {
    ...
    if (fused_number / run_threads > 12 ||
        (fused_number >= run_threads && (fused_number % run_threads) == 0))
      break;
    ...
    if (inner_loop.defined() && !inner_loop->num_threads_ &&
        ...
        !inner_loop->attr().get_or_else(stmt_attr_key::no_loop_fuse, false)) {
      node_ptr_map cur_remap;
      outer_most_loop->fuse(inner_loop, node_remap ? &cur_remap : nullptr);
      cur_loop = inner_loop;
      if (node_remap) {
...
static op_traits::may_prefetch_t *find_prefetch_op(const sc_graph_t &graph) {
  ...
  op_visitor_t::dfs_topology_sort(graph.ops_.size())
      .visit_graph(graph, [&found_op](op_visitor_t *vis, const sc_op_ptr &op) {
        if (found_op) {
          return;
        }
        if (op->isa<tunable_op_t>() && op->isa<op_traits::may_prefetch_t>() &&
            op->attrs_.get_or_else(mixed_partition_hint::first_prefetch_op,
                                   false)) {
          for (auto &ins : op->get_inputs()) {
            if (ins->producer_owner_->isa<input_op>()) {
              found_op = op->dyn_cast<op_traits::may_prefetch_t>();
              return;
            }
          }
        }
...
std::vector<int>
mixed_fuse_op_t::query_prefetch(const context_ptr &ctx, bool is_global,
...
void mixed_fuse_op_t::generate_prefetcher_body_for_tensor(
  ...
  if (auto found_op = find_prefetch_op(sub_graph_)) {
    ...
    found_op->generate_prefetcher_body_for_tensor(ctx, func_args, ins,
                                                  indices_in_op);
  }
}
...
mixed_fuse_op_t::get_inner_dispatch_ops(int *total_key_num) {
...
void mixed_fuse_op_t::update_internal_graph_format(
  ...
  int key_idx = 0;
  update_graph_format_by_key(ctx, shared_from_this(), sub_graph_, key, key_idx,
                             0, 0);
...
ir_module_ptr mixed_fuse_op_t::get_dynamic_query_func(const context_ptr &ctx) {
  ...
  for (auto &out : graph_output_ops) {
    ...
    ltsr_rtsr[ltsr] = tsr_info_t(outs[out_idx], expr(), out_fmts[out_idx],
                                 out_sizes[out_idx]);
    out_idx++;
  }
  ...
  for (size_t i = 0; i < query_idx.size(); i++) {
    ...
    ltsr_rtsr[ltsr] = tsr_info_t(ori_ins[i], expr(), ori_in_fmts[i], expr());
  }
  ...
  create_query_function_by_graph(gp, kernel, ori_ins, ori_in_fmts,
                                 each_op_num_keys, total_key_num,
                                 dispatch_op_num);
...
void mixed_fuse_op_t::create_internal_dispatch_funcs(
  ...
  for (auto &op : sub_graph_.ops_) {
    if (op->need_dynamic_internal_query()) {
      COMPILE_ASSERT(
          op->info_.internal_info_ &&
              !op->info_.internal_info_->dispatch_table_name_.empty(),
      ...
      op->get_internal_dispatch_key_set(ctx)->for_each_key_process(std::bind(
          create_dispatch_funcs_by_keys, ctx, std::ref(mod), table_name, op,
          std::placeholders::_1, expr(), std::ref(dyn_idx), use_mtp,
          /*internal*/ true));
    }
  }
}
...
struct inplace_recursion_context_t {
  ...
  const std::vector<kind_t> *call(const graph_tensor_ptr &tsr) {
    ...
    auto &ret = (result_[tsr] = std::vector<kind_t>(tsr_2_in_index_.size()));
    ...
    if (is_binary) {
      bcast_idx =
          producer->stc_cast<binary_elementwise_op_t>()->get_broadcast_input();
    }
    ...
    for (size_t input_idx = 0; input_idx < inputs.size(); input_idx++) {
      ...
      for (size_t i = 0; i < ret.size(); i++) {
        ...
        if ((int64_t)bcast_idx == (int64_t)input_idx && result != UNDEF) {
          result = NO_INPLACE;
        }
...
float mixed_fuse_op_t::get_gflop() { return sub_graph_.get_gflop(); }

sc_op_ptr mixed_fuse_op_t::copy( // NOLINT
    const std::vector<graph_tensor_ptr> &ins,
    const std::vector<graph_tensor_ptr> &outs, sc_graph_t &graph) {
  auto ret =
      graph.make<mixed_fuse_op_t>(op_name_, parti_list_, nullptr,
  ...
  for (auto &out : out_ops) {
    for (auto &outtsr : out->get_inputs()) {
      ...
      for (size_t i = 0; i < rec_ret->size(); i++) {
        if ((*rec_ret)[i] == inplace_recursion_context_t::ZERO_OFFSET_INPLACE) {
          ...
          if (utils::get_sizeof_type(index_2_tsr.at(i)->details_.dtype_) ==
              utils::get_sizeof_type(outtsr->details_.dtype_)) {
            can_inplace.emplace_back(tensor_inplace_info_t{
                static_cast<int>(i), inplace_kind::ZERO_OFFSET});
          }
        } else if ((*rec_ret)[i] == inplace_recursion_context_t::FREE_INPLACE) {
          can_inplace.emplace_back(
              tensor_inplace_info_t{static_cast<int>(i), inplace_kind::FREE});
        }
      }
