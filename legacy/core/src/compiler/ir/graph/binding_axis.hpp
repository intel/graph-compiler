...
using global_binding_axis_map =
    std::unordered_map<uintptr_t, std::unordered_map<uintptr_t, binding_axis>>;
...
void bind_loop_axis(const std::shared_ptr<graph_tensor> &gt,
                    ...
                    const std::vector<int> &axis, bool is_block = false);
