...
struct op_dispatch_key_base_t {
  ...
  virtual void
  set_op_dispatch_key(const std::shared_ptr<sc_op> &node,
...
struct op_dispatch_key_t : public op_dispatch_key_base_t {
  ...
  op_dispatch_key_t(const std::vector<std::vector<sc_dim>> &var_block,
                    const std::vector<sc_data_format_t> &formats,
                    bool impl = false)
...
struct impl_op_dispatch_key_t : public op_dispatch_key_base_t {
  ...
  impl_op_dispatch_key_t(int impl, int repeat) : impl_(impl), repeat_(repeat) {}
  void
  set_op_dispatch_key(const std::shared_ptr<sc_op> &node,
                      const std::shared_ptr<context_t> &ctx) const override;
  std::vector<runtime::dispatch_key>
...
struct dispatch_key_cmper_t {
  bool operator()(const op_dispatch_key_t &key0,
...
struct dispatch_key_set_base_t {
  ...
  virtual void for_each_key_process(
      const std::function<void(const op_dispatch_key_base_t *)> &callback) = 0;
  virtual std::set<op_dispatch_key_t, dispatch_key_cmper_t> &
  get_inner_set() = 0;
...
struct combined_dispatch_key_set_t : public dispatch_key_set_base_t {
  using inner_set_t =
      std::set<combined_op_dispatch_key_t, combined_dispatch_key_cmper_t>;
  ...
  void
  internal_construct(const std::vector<std::shared_ptr<dispatch_key_set_base_t>>
                         &dispatch_sets,
                     const std::vector<std::shared_ptr<sc_op>> &inputs =
...
std::vector<int>
get_dynamic_impl_dispatch_candidates(tunable_op_t *op,
