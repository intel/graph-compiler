...
public:
   ...
   * */
  virtual infer_status_code infer_slice_ranges(const context_ptr &ctx,
                                               fslice_map &fsmap) = 0;
   ...
   * output slice.
   * */
  virtual infer_status_code pre_infer_slice_ranges(const context_ptr &ctx,
                                                   fslice_map &fsmap) = 0;
   ...
   * */
  virtual void
  compute_block(context_ptr ctx, const std::vector<tensor_slice *> &dst,
                const std::vector<const tensor_slice *> &inputs) = 0;
  /**
   ...
   * */
  virtual size_t
  compute_fusible_workload(const context_ptr &ctx,
...
public:
  ...
  sc_dims get_constant_blocking_dims() {
    return sc_data_format_t::get_blocking_shapes(get_constant_plain_dims(),
                                                 get_constant_format());
  }
  bool
  compare_contents(const sc_op *other,
