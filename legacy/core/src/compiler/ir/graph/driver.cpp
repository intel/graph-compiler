...
basic_graph_pass_ptr create_graph_pass(const std::string &name,
                                       ...
                                       pass_type type, sc_opt_level opt_level,
                                       bool enabled) {
  return std::make_shared<basic_graph_pass_t>(func_t, name, required, type,
                                              opt_level, enabled);
}

static std::vector<basic_graph_pass_ptr>
filter_passes_by_opt_level(const std::vector<basic_graph_pass_ptr> &passes,
                           sc_opt_level opt_level) {
  std::vector<basic_graph_pass_ptr> ret;
  for (auto &p : passes) {
    if (p->opt_level_ <= opt_level) {
      ret.emplace_back(p);
    }
  }
  return ret;
...
create_default_graph_flow(const context_ptr &ctx) {
  ...
  pre_tune_passes.push_back(create_graph_pass(
      "eliminate_zero_shaped_tensors", eliminate_zero_shaped_tensors, {},
  ...
  pre_tune_passes.push_back(create_graph_pass(
      "annotate_fusion_break", quantize::annotate_fusion_break, {},
      pass_type::pre_tune, sc_opt_level::lv2, true));
  pre_tune_passes.push_back(
      create_graph_pass("annotate_config", annotate_config, {},
                        pass_type::pre_tune, sc_opt_level::lv2, true));
  ...
  pre_tune_passes.push_back(
      create_graph_pass("constant_optimization", constant_optimization, {},
                        pass_type::pre_tune, sc_opt_level::lv0, true));
  pre_tune_passes.push_back(create_graph_pass(
      "quantized_info_propagation", quantize::quantize_info_propagation, {},
      pass_type::pre_tune, sc_opt_level::lv0, true));
  pre_tune_passes.push_back(create_graph_pass(
      "quantized_graph_reschedule", quantize::graph_reschedule, {},
      pass_type::pre_tune, sc_opt_level::lv0, true));
  ...
  pre_tune_passes.push_back(
      create_graph_pass("rl_conv_weight_transform", rl_conv_weight_transform,
                        {}, pass_type::pre_tune, sc_opt_level::lv2, true));
  pre_tune_passes.push_back(
  ...
  pre_tune_passes.push_back(
      create_graph_pass("dynamic_graph_transform", dynamic_graph_transform, {},
                        pass_type::pre_tune, sc_opt_level::lv2, true));
  pre_tune_passes.push_back(
  ...
  pre_tune_passes.push_back(
      create_graph_pass("permute_propagation", permute_propagation, {},
                        pass_type::pre_tune, sc_opt_level::lv0, true));
  pre_tune_passes.push_back(create_graph_pass(
      "quantize_op_compensation", quantize::calculate_op_compensation, {},
      pass_type::pre_tune, sc_opt_level::lv0, true));
  pre_tune_passes.push_back(
      create_graph_pass("broadcast_transform", broadcast_transform, {},
                        pass_type::pre_tune, sc_opt_level::lv0, true));
  pre_tune_passes.push_back(create_graph_pass(
      "elemwise_dimension_alignment", elemwise_dimension_alignment, {},
      pass_type::pre_tune, sc_opt_level::lv0, true));
  pre_tune_passes.push_back(create_graph_pass(
      "shape_relationship_binding", shape_relationship_binding, {},
      pass_type::pre_tune, sc_opt_level::lv0, true));
  ...
  post_tune_passes.push_back(
      create_graph_pass("const_folding", graph_constant_input_folding, {},
                        pass_type::post_tune, sc_opt_level::lv0, true));
  post_tune_passes.push_back(create_graph_pass("graph_simplify", graph_simplify,
                                               {}, pass_type::post_tune,
                                               sc_opt_level::lv2, true));
  post_tune_passes.push_back(
  ...
  if (ctx->flags_.concat_optimization_) {
    post_tune_passes.push_back(create_graph_pass("merge_concats", merge_concats,
                                                 {}, pass_type::post_tune,
                                                 sc_opt_level::lv2, true));
  }
  post_tune_passes.push_back(
      create_graph_pass("const_folding_and_share",
  ...
  if (ctx->flags_.concat_optimization_) {
    post_tune_passes.push_back(
        create_graph_pass("graph_concat_optimize", graph_concat_memory_planning,
                          {}, pass_type::post_tune, sc_opt_level::lv2, true));
  }
  // filter passes by opt level
  pre_tune_passes =
      filter_passes_by_opt_level(pre_tune_passes, ctx->flags_.opt_level_);
  post_tune_passes =
      filter_passes_by_opt_level(post_tune_passes, ctx->flags_.opt_level_);
    ...
    if (pass->enabled_) {
      auto timer = utils::create_scoped_timer(
          need_time, [&pass](utils::time_duration dur) {
            std::string name = std::string("graph.driver.time.") + pass->name_;
      ...
      if (allow_cache && pass->type_ == pass_type::post_tune &&
        ...
        if (graph.attrs_.has_key("graph_code_cache")) {
          break;
        }
      }
      if (need_result) {
        std::string name = std::string("graph.driver.debug.") + pass->name_;
        if (auto stream = runtime::get_info_logging_stream(name.c_str())) {
...
create_op_map(sc_graph_t &lg, sc_graph_t &rg) {
...
sc_graph_t dynamic_shape_infer_preprocess(const sc_graph_t &graph,
...
void graph_driver(sc_graph_t &graph, const context_ptr &ctx,
                  const graph_config *in_cfg, graph_config *out_cfg,
                  int batch_size, int repeat, int64_t timeout,
                  tuner_creator *tune_creator,
...
void graph_driver(sc_graph_t &graph, int batch_size, int repeat,
                  const context_ptr &ctx) {
