...
namespace graph {
namespace gc {
void initialize_format_table_with_op(const sc_op_ptr &op,
                                     op_dispatch_tables_ptr &tb) {
  ...
  if (dispatch_keys->size() <= 1) {
    return;
  }
  if (op->isa<tunable_op_t>()) {
    auto set_format_by_keys = [&](const op_dispatch_key_base_t *dispatch_key) {
      std::vector<runtime::dispatch_key> all_formats =
          dispatch_key->convert_to_runtime_format_vec();
      std::vector<runtime::dispatch_key> keys(all_formats.begin(),
                                              all_formats.end() - 1);
    ...
    auto set_format_by_key = [&](const op_dispatch_key_base_t *dispatch_key) {
...
void initialize_op_info_with_op(const sc_op_ptr &op,
                                op_dispatch_tables_ptr &tb) {
...
void add_dispatch_symbol_to_kernel_table(
    op_dispatch_tables_ptr &tb, const op_dispatch_key_base_t *key,
...
bool can_op_query_output(const sc_op_ptr &op) {
  if (!is_internal_op(op)) {
    return false;
  }
  if (op->op_name_ == "dynamic_reshape") {
    return false;
  }
  ...
  for (auto &out : op->get_outputs()) {
    for (auto &d : out->details_.get_plain_dims()) {
      if (is_dynamic_dim(d) && set.find(d) == set.end()) {
        return true;
      }
    }
  }
...
std::vector<dispatch_set_ptr>
get_dispatch_set_vec_from_ops(const std::vector<sc_op_ptr> &ops) {
...
sc_op_ptr find_parent_dispatch_node(const graph_tensor_ptr &in) {
  ...
  while (!(cur_op->isa<tunable_op_t>() || cur_op->isa<input_op>() ||
    ...
    int layout_input_idx =
        cur_op->attrs_.get_or_else(op_attr_key::layout_input_index, 0);
  ...
  for (auto &use : in->uses_) {
    ...
    if (op->isa<binary_elementwise_op_t>() || op->isa<select_op_t>()) {
      for (size_t i = 0; i < op->get_inputs().size(); i++) {
        auto parent_node = find_parent_dispatch_node(op->get_inputs()[i]);
...
make_linked_op_output_pair(const std::vector<dispatch_set_ptr> &dispatch_keys,
                           int op_idx) {
  assert(op_idx >= 0 && op_idx < static_cast<int>(dispatch_keys.size()));
  return std::make_pair(op_idx, static_cast<int>(dispatch_keys[op_idx]
                                                     ->get_inner_set()
                                                     .begin()
...
op_layout_link_vec_t get_op_layout_link_relationships(
  ...
  for (size_t i = 0; i < ops.size(); i++) {
    assert(!dispatch_keys[i]->get_inner_set().begin()->in_out_formats_.empty());
    ret[i].resize(
        dispatch_keys[i]->get_inner_set().begin()->in_out_formats_.size(),
  ...
  for (size_t i = 0; i < ops.size(); i++) {
    ...
    if (op->isa<reorder_op_t>()) {
      ...
      if (parent_inp->isa<tunable_op_t>() || parent_inp == modified_inp) {
        ...
        auto tunable_op = find_output_linked_tunable_op(op->get_outputs()[0]);
        op_layout_idx = 1;
        if (tunable_op) {
      ...
      for (size_t j = 0; j < op->get_inputs().size(); j++) {
        auto parent_node = find_parent_dispatch_node(op->get_inputs()[j]);
        if (parent_node->isa<tunable_op_t>() ||
...
bool is_linked_layout(const sc_data_format_t &layout1,
                      const sc_data_format_t &layout2) {
...
convert_graph_tensor_to_dynamic_tensor(const graph_tensor_ptr &in,
                                       void *data_ptr, sc_dim *shape_ptr) {
  ...
  for (int i = 0; i < static_cast<int>(plain_dims.size()); i++) {
    if (is_dynamic_dim(plain_dims[i])) {
      ret.dyn_mask_ |= (1 << i);
    }
  }
  return ret;
...
static bool need_query_next_first(const sc_op_ptr &node) {
  ...
  if (node->isa<mixed_fuse_op_t>()) {
    out_ops = node->stc_cast<mixed_fuse_op_t>()->sub_graph_.get_output_ops();
  }
  for (auto &op : out_ops) {
  ...
  return has_tail_reorder &&
         can_op_be_dispatched(node->get_outputs()[0]->uses_[0].second.lock());
}

void lower_query_function(
    std::vector<bool> &visited, const sc_op_ptr &node,
    const std::function<void(const sc_op_ptr &)> &callback) {
  if (visited[node->logical_op_id_]) {
    return;
  }
...
void visit_fused_graph_by_query_order(
    sc_graph_t &graph, const std::function<void(const sc_op_ptr &)> &callback) {
...
std::vector<sc_op_ptr> get_graph_inner_dispatch_ops(sc_graph_t &graph,
                                                    int *total_key_num) {
  ...
  auto get_inner_ops = [&ret, &total_key_num](const sc_op_ptr &op) {
    if (op->isa<tunable_op_t>() || op->isa<reorder_op_t>()) {
      ret.emplace_back(op);
      if (total_key_num) {
        *total_key_num += static_cast<int>(op->get_inputs().size() +
                                           op->get_outputs().size());
      }
    }
  };
...
static int find_padding_impl(const combined_op_dispatch_key_t &key) {
  for (auto &cur_key : key.keys_) {
    // reorder
    if (cur_key.in_out_formats_.size() == 2) {
      return cur_key.impl_;
    }
  }
...
void update_graph_format_by_key(const context_ptr &ctx,
                                ...
                                int &key_idx, size_t node_input_offset,
                                size_t graph_input_offset,
                                const sc_op_ptr &modified_inp) {
  ...
  auto update_format = [&](const sc_op_ptr &node) {
    if (node->isa<tunable_op_t>() || node->isa<reorder_op_t>()) {
      ...
      for (size_t i = 0; i < cur_key.in_out_formats_.size() - 1; i++) {
        node->get_inputs()[i]->details_.set_format(cur_key.in_out_formats_[i]);
      }
      ...
      if (node->isa<reorder_op_t>() &&
        ...
        auto inp_parent = find_parent_dispatch_node(node->get_inputs()[0]);
      ...
      node->get_outputs()[0]->details_.set_format(
          cur_key.in_out_formats_[cur_key.in_out_formats_.size() - 1]);
      // update impl alg
...
expr call_op_dynamic_query_function(const sc_op_ptr &op,
  ...
  if (op->isa<ops::matmul_core_op_t>()) {
    ...
    return builtin::call_matmul_core_query_format(
        args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
        args[8], args[9], args[10], args[11], args[12],
        args.size() == 13 ? get_ir_null() : args[13]);
  } else if (op->isa<ops::managed_matmul_core_op_t>()) {
    assert(args.size() == 13 || args.size() == 14);
    return builtin::call_managed_matmul_core_query_format(
        args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
        args[8], args[9], args[10], args[11], args[12],
        args.size() == 13 ? get_ir_null() : args[13]);
  } else if (op->isa<ops::conv_fwd_core_op_t>()) {
    assert(args.size() == 13 || args.size() == 14);
    return builtin::call_conv_fwd_core_query_format(
        args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
        args[8], args[9], args[10], args[11], args[12],
        args.size() == 13 ? get_ir_null() : args[13]);
    ...
    assert(args.size() == 9);
    return builtin::call_binary_fusible_op_query_format(
        args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
        args[8]);
    ...
    assert(args.size() == 11);
    return builtin::call_select_op_query_format(
        args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
    ...
    COMPILE_ASSERT(false, "unsupported op query function: " << op->op_name_);
  }
  return expr();
}
...
void create_internal_dispatch_funcs_by_node(
    ...
    const std::string &table_name, const sc_op_ptr &node,
...
void create_dispatch_funcs_by_keys(
    const context_ptr &ctx, ir_module_ptr &ret_mod,
    const std::string &table_name, const sc_op_ptr &node,
    const op_dispatch_key_base_t *key, expr &op_dispatch_kernel, int &dyn_idx,
    const std::shared_ptr<const thread_pool_mode_t> &use_mtp, bool internal) {
  ...
  if (!should_compile_later) {
    ...
    auto mod = internal ? node->get_internal_func(ctx) : node->get_func(ctx);
    auto func = mod->get_entry_func();
    if (internal) {
      func->name_ += "_internal";
    }
    ...
    add_dispatch_symbol_to_kernel_table(
        cur_table, key, op_dispatch_tables_t::op_func_info{func->name_});
  }
  dyn_idx++;
}
...
bool is_dyn_specific_graph(sc_graph_t &graph) {
  if (graph.dyn_info_ && graph.dyn_info_->is_specific_) {
    return true;
  }
  return false;
}
