...
static_fusion_cost_model_t::static_fusion_cost_model_t(mixed_parti_t *parti)
    : fusion_cost_model_base_t(parti), max_scores_(0),
...
float static_fusion_cost_model_t::evaluate() {
  ...
  if (new_scores > max_scores_)
    max_scores_ = new_scores;
  return new_scores;
}

void static_fusion_cost_model_t::append_evaluator(float weight,
                                                  const cost_eval &eval) {
...
bool static_fusion_cost_model_t::make_decision_for_parti(
  ...
  auto buffer_usage = get_buffer_usage(binded_mxp_->ctx_, merged_mem_info.first,
                                       merged_mem_info.second);
  // get threshold
...
static bool need_double_check_standalone_parallel(const sc_op *op,
                                                  const mixed_parti_t *mxp) {
  ...
  if (auto broadcast_op = op->dyn_cast<const op_traits::may_broadcast_t>()) {
    ...
    if (non_bc_index.size() == op->get_inputs().size())
      return false;
    ...
    if (mxp->contain_tunable_op() &&
      ...
      return false;
    if (mxp->can_optimize_outer_loop(true))
      return false;
    return true;
  }
  return false;
...
bool static_fusion_cost_model_t::make_decision_for_op(
  ...
  auto fanchor_loop_parallelism =
      evaluate_loop_parallel_balance(binded_mxp_->get_outer_loops(fanchor));
  ...
  if (need_double_check_standalone_parallel(op, binded_mxp_)) {
    ...
    mixed_parti_t op_parti(binded_mxp_->ctx_, std::const_pointer_cast<sc_op>(
                                                  op->shared_from_this()));
    ...
    if (ret &&
        !evaluate_loop_parallel_balance(binded_mxp_->get_outer_loops(), true)) {
    ...
    } else if (!ret && evaluate_loop_parallel_balance(
...
bool dynamic_fusion_cost_model_t::make_decision_for_parti(
    ...
    parti_merge_kind merge_kind) {
  // query if turn on
  if (!enable_ || policy_ == dynamic_fusion_policy_t::max_fusion)
    return true;
  /* loop_parallelism */
  ...
  if (merge_kind == parti_merge_kind::horizontal) {
    float ths_parallelism =
        evaluate_loop_parallel_balance({ths_outer_loops[0]}, res_cond);
    float other_parallelism =
        evaluate_loop_parallel_balance({other_outer_loops[0]}, dummy_cond);
    bool ret = ths_parallelism != 1.0f && other_parallelism != 1.0f;
    if (!ret) {
      cond_ = cond_ || !(res_cond && dummy_cond);
    }
    return ret;
  }
  /* for verticall merge*/
  ...
  auto merged_outer_loop = std::vector<for_loop>{
      ths_outer_loops.begin(), ths_outer_loops.begin() + merged_loop_size};
...
bool dynamic_fusion_cost_model_t::make_decision_for_op(
  ...
  if (op->isa<tunable_op_t>() &&
      evaluate_loop_parallel_balance(binded_mxp_->get_outer_loops(), thr_cond,
                                     true) == 0.f) {
    SC_MODULE_INFO << "double check standalone parallellism for "
                   << op->op_name_ << op->logical_op_id_;
    mixed_parti_t tunable_parti(
        binded_mxp_->ctx_,
        std::const_pointer_cast<sc_op>(op->shared_from_this()));
    if (evaluate_loop_parallel_balance(tunable_parti.get_outer_loops(),
                                       dummy_cond) > other_parallelism) {
      ret = false;
