...
public:
  ...
  expr visit_impl(var v) override {
    ...
    if (itr != node_remap_.end()) {
      changed_ = true;
      return static_cast<expr_base *>(itr->second.get())->node_ptr_from_this();
    }
    return v;
  }
  ...
  expr visit_impl(tensor v) override {
    ...
    if (v->attr_ && v->attr_->has_key(tensor_shrinker_attrs::should_shrink)) {
      ...
      ir_inplace_visitor_t::dispatch_expr_vector(shrink_info.base_, _new_expr);
      ir_inplace_visitor_t::dispatch_expr_vector(shrink_info.shape_, _new_expr);
    }
...
public:
  ...
  // set inplace hint
  void set_buffer_inplace_hint(const expr &target_buf, const expr &inplace_buf);
  // get inplaced buffer
  expr get_inplaced_buffer(const expr &buf) const;
  // get allocated buffer
  std::tuple<std::vector<expr>, std::vector<expr>> get_buffer(sc_op *op) const;
...
struct mixed_parti_t : fusion_partition_t {
  ...
  fusion_anchor_ptr lookup_anchor_map(sc_op *op,
                                      bool throw_assert = true) const;

  // look up fanchor by stmts
  fusion_anchor_ptr lookup_anchor_map(const stmts &ss) const;

  // look up sub fanchor by parent fanchor
  std::vector<fusion_anchor_ptr>
  lookup_sub_anchor_map(const fusion_anchor_ptr &parent_fanchor) const;

  // get anchor inside given loop
  fusion_anchor_ptr get_anchor_inside_loop(const for_loop &loop,
                                           bool input_anchor = false) const;

  /// get next inner loop including anchor
  for_loop get_next_inner_loop_with_anchor(
  ...
  template <typename T> size_t count_op_with_type() const {
    if (merged_to) {
      return get_root()->count_op_with_type<T>();
    }
    size_t cnt = 0;
    for (auto &op : ops) {
      if (op->isa<T>())
  ...
  template <typename T> bool contain_op_with_type() const {
    if (merged_to) {
      return get_root()->contain_op_with_type<T>();
    }
  ...
  sc_graph_t &get_host_graph() const {
    COMPILE_ASSERT(!committed_ops_.empty(),
                   "No op contained in current partition")
...
inline bool is_optimized_sub_graph(sc_graph_t &g) {
  return g.attrs_.get_or_else(mixed_partition_hint::optimized_sub_graph, false);
}
