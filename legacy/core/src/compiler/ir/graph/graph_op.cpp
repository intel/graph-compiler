...
graph_op_t::remake_logical_tensors(const std::vector<graph_tensor_ptr> &flts) {
  std::vector<graph_tensor_ptr> new_flts(flts.size());
  for (size_t i = 0; i < flts.size(); ++i) {
    new_flts[i] = std::make_shared<graph_tensor>(
        nullptr, flts[i]->details_.get_format(),
...
ir_module_ptr graph_op_t::get_func(context_ptr ctx) {
  ...
  return lower_graph(ctx, *graph, {graph->ops_.back(), graph->ops_[0]}, false);
}
...
graph_tensor_ptr
graph_op_t::cast_input_dtype(graph_tensor_ptr &inp,
  ...
  if (inp->details_.dtype_.is_etype(sc_data_etype::BF16) ||
    ...
    auto cast_input = graph->make(
        "cast", {inp}, {},
        attrs.as_map().empty() ? any_map_t{{"dtype", datatypes::f32}} : attrs);
    input = cast_input->get_outputs()[0];
  }
  return input;
}

std::shared_ptr<sc_op> graph_op_t::cast_output_dtype(
    graph_tensor_ptr &inp, std::shared_ptr<sc_graph_t> &graph,
    std::shared_ptr<sc_op> &last_op, const any_map_t &attrs) {
  std::shared_ptr<sc_op> &cast_output = last_op;
  if (inp->details_.dtype_.is_etype(sc_data_etype::BF16) ||
      inp->details_.dtype_.is_etype(sc_data_etype::F16)) {
    auto target_dtype = inp->details_.dtype_;
    cast_output = graph->make(
        "cast", last_op->get_outputs(), {},
        attrs.as_map().empty() ? any_map_t{{"dtype", target_dtype}} : attrs);
  }
  return cast_output;
}
...
nested_graph_op_t::nested_graph_op_t(const std::string &op_name,
                                     ...
                                     const any_map_t &attrs, sc_graph_t &&graph)
    : configurable_graph_op_t(op_name, ins, outs, attrs),
      graph_(std::move(graph)) {
  if (outs.empty()) {
    for (auto &out_op : graph_.get_output_ops()) {
      info_.outputs_.insert(info_.outputs_.end(), out_op->get_inputs().begin(),
                            out_op->get_inputs().end());
    }
...
sc_op_ptr nested_graph_op_t::copy( // NOLINT
  ...
  auto ret = mgr.make<nested_graph_op_t>(this->op_name_, ins, outs, attrs_,
                                         copy_graph(graph_));
  return ret;
}
} // namespace gc
