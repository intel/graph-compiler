...
bool tunable_op_t::is_valid(const context_ptr &ctx) {
  if (!config_data_ ||
      !create_generator()->is_valid_config(ctx, config_data_.data_.get())) {
    return false;
  }
  return true;
}
...
ir_module_ptr tunable_op_t::get_func(context_ptr ctx) {
  ...
  auto gen_body = [&](const body_generator_ptr &gen_ptr) {
    ...
    bool status = gen_ptr->generate(ctx, config_data_.data_.get(), nullptr, ins,
                                    outs, loops);
    assert(status);
  ...
  if (need_inner_query) {
    ...
    auto inter_ptr = builder::make_reinterpret(
        builder::make_indexing(func->params_.back(), {0}), datatypes::pointer);
...
func_t tunable_op_t::get_func(mixed_parti_t *parti,
  ...
  auto gen_body = [&](const body_generator_ptr &gen_ptr) {
    ...
    bld.push_scope();
    std::vector<for_loop> loops;
    bool status = gen_ptr->generate(parti->ctx_, config_data_.data_.get(),
                                    &fmgr, ins, outs, loops);
    ...
    if (parti->ready_for_op(this)) {
      ...
      for (auto &template_anchor : fmgr.get_fusion_anchor()) {
        template_anchor->attach_parent_anchor(committed_anchor, nullptr, true);
      }
    }
    assert(status);
    auto body = bld.pop_scope();
  ...
  if (need_inner_query) {
    ...
    auto common_args = outs;
    common_args.insert(common_args.end(), ins.begin(), ins.end());
    std::for_each(common_args.begin(), common_args.end(), [](const expr &arg) {
    ...
    if (!parti->dyn_inter_->inter_funcs_param_.defined()) {
      parti->dyn_inter_->inter_funcs_param_ =
          builder::make_tensor("internal_funcs", {1}, datatypes::index);
      parti->dyn_inter_->num_func_ = 1;
      parti->dyn_inter_->inter_funcs_param_->attr().set(attr_keys::no_index2var,
                                                        true);
    } else {
...
void tunable_op_t::commit_into_anchor(fusion_anchor_t *committed_anchor) {
  ...
  std::transform(get_outputs().begin(), get_outputs().end(), outs_slice.begin(),
                 [&committed_anchor](const graph_tensor_ptr &gt) {
                   auto slice_list = committed_anchor->fsmap_.get(gt);
                   COMPILE_ASSERT(slice_list.size() == 1,
                                  "multi-slice is not expected to tunable op");
...
void tunable_op_t::set_config_if_empty(context_ptr ctx,
                                       body_generator_base_t *p) {
  if (!config_data_) {
    set_config(p->get_default_config(std::move(ctx)));
  }
}
...
config_ptr_vec
tunable_op_t::get_dynamic_config_candidates(const context_ptr &ctx) {
...
std::vector<int>
tunable_op_t::get_impl_dispatch_candidates(const context_ptr &ctx) {
