...
static void infer_aligned_shape(const logical_tensor_t &a,
                                const logical_tensor_t &b,
                                const std::vector<int> &plain_bc_axis,
                                sc_dims &aligned_shape,
  ...
  if (b_format.is_any()) {
    b_format = sc_data_format_t::get_plain_by_dims(b.get_plain_dims().size());
  }
  ...
  for (size_t i = 0; i < b.get_plain_dims().size(); ++i) {
    if (b.get_plain_dims()[i] == 1 &&
      ...
      COMPILE_ASSERT(
          is_dynamic_dim(b.get_plain_dims()[i]) ||
              is_dynamic_dim(a.get_plain_dims()[plain_bc_axis[bc_axis_idx]]) ||
              b.get_plain_dims()[i] ==
                  a.get_plain_dims()[plain_bc_axis[bc_axis_idx]],
  ...
  for (const auto &bind : bc_axis_bind) {
    aligned_shape[plain_bc_axis[bind.second]] = b.get_plain_dims()[bind.first];
  }
  ...
  std::vector<int> storage_args(b_format.format_code_.ndims() + dim_difference,
                                -1);
...
void elemwise_dimension_alignment(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (auto may_broadcast_node =
      ...
      COMPILE_ASSERT(
          !may_broadcast_node->get_non_broadcast_input_index(false).empty(),
      ...
      for (size_t i = 0; i < node->get_inputs().size(); ++i) {
        ...
        if (cur_in_lt.get_plain_dims().size() <
          ...
          if (!shape.empty()) {
            // insert tensor view
            auto ret = graph.make("tensor_view", {node->info_.inputs_[i]}, {},
                                  {{"shape", shape},
                                   {"format", format},
      ...
      if (cur_in_lt.get_plain_dims().size() <
        ...
        if (!shape.empty()) {
          // insert tensor view
          auto ret = graph.make("tensor_view", {node->info_.inputs_[0]}, {},
                                {{"shape", shape},
                                 {"format", format},
                                 {"expand_dim", aligned_axis}});
