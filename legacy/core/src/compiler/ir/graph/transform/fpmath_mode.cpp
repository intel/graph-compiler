...
bool down_converted_op_exit(
    const context_ptr &ctx, const sc_op_ptr &node,
  ...
  for (auto &in : node->get_inputs()) {
    if (std::find(onedim_const.begin(), onedim_const.end(),
                  in->producer_owner_->logical_op_id_) != onedim_const.end())
      return false;
  ...
  for (auto &out : node->get_outputs()) {
    if (!utils::is_one_of(out->details_.dtype_, datatypes::f32, target_type))
      return false;
  }
  return true;
}
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->isa<output_op>()) {
      bool downconvert_output =
          dtype_backup.count(
              node->get_inputs()[0]->producer_owner_->logical_op_id_) > 0;
      ...
      if (down_converted_op_exit(ctx, node, datatypes::bf16, onedim_const)) {
        for (auto in_id = 0; in_id < int(node->get_inputs().size()); ++in_id) {
          if (node->get_inputs()[in_id]->details_.dtype_ == datatypes::f32) {
            // add the cast op: F32 -> BF16
            auto cur_owner_op = node->get_inputs()[in_id]->producer_owner_;
            auto is_constant_op = (cur_owner_op->isa<constant_op_t>() ||
                                   cur_owner_op->attrs_.get_or_else(
                                       "constant", const_kind::not_const));
            sc_op_ptr cast_node =
                graph.make("cast", {node->get_inputs()[in_id]}, {},
                           {{"dtype", datatypes::bf16}});
            cast_node->get_outputs()[0]->details_.dtype_ = datatypes::bf16;
            if (is_constant_op) {
              cast_node->attrs_.set("constant", const_kind::local_const);
            }
        ...
        if (!node->get_outputs().empty() && !node->isa<tunable_op_t>()) {
          if (node->get_outputs()[0]->details_.dtype_ == datatypes::f32) {
        ...
        for (auto in_id = 0; in_id < int(node->get_inputs().size()); ++in_id) {
          if (node->get_inputs()[in_id]->details_.dtype_ == datatypes::bf16) {
            // add the cast op: BF16 -> F32
            auto cur_owner_op = node->get_inputs()[in_id]->producer_owner_;
            auto is_constant_op = (cur_owner_op->isa<constant_op_t>() ||
                                   cur_owner_op->attrs_.get_or_else(
            ...
            sc_op_ptr cast_node =
                graph.make("cast", {node->get_inputs()[in_id]}, {},
                           {{"dtype", datatypes::f32}});
            cast_node->get_outputs()[0]->details_.dtype_ = datatypes::f32;
            if (is_constant_op) {
              cast_node->attrs_.set("constant", const_kind::local_const);
            }
            node->replace_input(in_id, cast_node->get_outputs()[0]);
