...
void pre_padding(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  visitor.visit_graph(graph, [&](op_visitor_t *visitor, const sc_op_ptr &node) {
    // if current node is a conv op with paddings,
    // insert a padding op before current node

    if (node->isa<ops::conv_fwd_core_op_t>()) {
      auto data_plain_dims = node->info_.inputs_[0]->details_.get_plain_dims();
      auto weight_plain_dims =
          node->info_.inputs_[1]->details_.get_plain_dims();
      bool is_amx_dtype =
          ops::is_amx_dtype(ctx, node->get_inputs()[0]->details_.dtype_);
      sc_dim groups = node->attrs_.get_or_else("groups", 1);
      bool is_dw_brdgmm = groups > 1 && weight_plain_dims.size() >= 5 &&
                          data_plain_dims.size() >= 5 &&
                          1 == weight_plain_dims[1] && 1 == data_plain_dims[2];
      auto padding_value = node->attrs_.get_or_else("padding_value", 0);
      // TODO(xurui)
      ...
      bool is_weight_constant =
          node->get_inputs()[1]->producer_owner_->isa<constant_op_t>() ||
          node->get_inputs()[1]->producer_owner_->attrs_.get_or_else(
              "constant", const_kind::not_const) ||
          node->get_inputs()[1]->attrs_.get_or_else("constant",
                                                    const_kind::not_const);
      if (!is_weight_constant) {
        return;
      }
      if (node->attrs_.get_or_else("use_rl", ops::rl_kind::NO_LOWERING) ==
      ...
      if (is_zero_paddings(pads_begin)) {
        return;
      }

      auto parent_node = node->get_inputs()[0]->producer_owner_;

      if (parent_node->isa<input_op>()) {
        return;
      }
      ...
      if (node->get_inputs()[0]->details_.get_plain_dims()[0] == 1) {
        padding_node->attrs_.set(op_attr_key::break_post_fuse, true);
      }
