...
static void insert_reorder_op(sc_graph_t &graph, reorder_map_t &reorder_map,
                              ...
                              const format_stride_pair &out_format_stride,
  ...
  if (!is_input_plain && !cur_op->is_dynamic() &&
    ...
    in->details_.set_format_and_stride(out_format_stride.first,
                                       out_format_stride.second);
    return;
  }
  ...
  if (tsr_it != reorder_map.end()) {
    ...
    if (op_it != op_map.end()) {
      ...
      if (is_graph_dynamic && is_key_valid) {
        auto &dynamic_formats = ret->get_dispatch_key_set()->get_inner_set();
        ...
        graph.make("reorder", {in}, {},
                   {{"internal", true},
                    ...
                    {op_attr_key::no_fuse, // work around for conv
                     ...
                     graph.attrs_.get_or_else("reorder_not_to_fuse", false)}});
    // map reorder's in/out
    if (is_graph_dynamic && is_key_valid) {
...
static void update_output_formats(
  ...
  for (size_t i = 0; i < outs.size(); ++i) {
    if (!out_supported_pairs.empty()) {
      ...
      outs[i]->details_.set_format_and_stride(fs_pair.first, fs_pair.second);
...
static void
insert_reorder_for_output_op(reorder_map_t &reorder_map, const sc_op_ptr &node,
                             bool use_channel_last_format, bool is_out_plain,
                             bool is_input_plain, bool is_graph_dynamic,
                             sc_graph_t &graph,
                             reorder_callback_type &insert_reorder_callback) {
  auto given_target_formats =
      node->attrs_.get_or_null<std::vector<sc_data_format_t>>("target_formats");
  auto given_target_strides =
      node->attrs_.get_or_null<std::vector<sc_dims>>("target_strides");
  if (given_target_formats) {
    COMPILE_ASSERT(given_target_formats->size() == node->get_inputs().size(),
                   "Output op's target_formats' size should be equal to "
                   "number of tensors");
  }

  if (given_target_strides) {
    COMPILE_ASSERT(given_target_strides->size() == node->get_inputs().size(),
  ...
  for (size_t i = 0; i < node->get_inputs().size(); ++i) {
    ...
    if (given_target_strides) {
      ...
      if (use_channel_last_format) {
        ...
        target_stride = logical_tensor_t::compute_dense_stride(permuted_dims);
      } else {
        target_stride =
            logical_tensor_t::compute_dense_stride(in_detail.get_plain_dims());
      }
    } else {
      if (given_target_formats) {
        auto dims = logical_tensor_t(target_format, in_detail.get_plain_dims(),
                                     in_detail.dtype_)
                        .get_blocking_dims();
    ...
    if (is_graph_dynamic) {
      ...
      format_stride_pair target_fs_pair(target_format,
                                        std::move(target_stride));
...
static void combine_layout_and_impl_dispatch(const context_ptr &ctx,
                                             sc_graph_t &graph) {
  auto &ops = graph.ops_;
  for (auto &op : ops) {
    if (!op->is_dynamic()) {
    ...
    if (impl_candidates.empty() || !op->is_dynamic()) {
      continue;
    }
...
static bool has_conv_op(sc_graph_t &graph) {
  auto pos = std::find_if(graph.ops_.begin(), graph.ops_.end(),
                          [](const sc_op_ptr &op) {
...
SC_INTERNAL_API void layout_propagation(sc_graph_t &graph,
                                        const context_ptr &ctx) {
  bool use_channel_last_format =
      has_channel_last_input(graph) && has_conv_op(graph) &&
      graph.attrs_.get_or_else(
          sc_graph_t::attr_key_t::allow_channel_last_output, false);
  bool is_input_plain =
      graph.attrs_.get_or_else(sc_graph_t::attr_key_t::is_input_plain, true);
  bool is_graph_dynamic =
      graph.is_dynamic() && graph.attrs_.get_or_else("insert_reorder", true);
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    ...
    if (node->isa<input_op>() || node->isa<constant_op_t>()) {
      ...
      for (auto &in : node->get_outputs()) {
        backup_info.emplace_back(std::make_pair(in->details_.get_format(),
    ...
    if (auto tunable_node = node->dyn_cast<tunable_op_t>()) {
      ...
      if (!has_config) {
        tunable_node->set_config(nullptr);
      }
      if (in_supported_pairs.empty()) {
        num_choices[node->logical_op_id_] = 1;
      } else {
        num_choices[node->logical_op_id_] = in_supported_pairs[0].size();
        if (!graph.is_dynamic() && total_choices <= STATIC_MAX_LAYOUT_TRIES)
          total_choices = total_choices * num_choices[node->logical_op_id_];
      }
  ...
  auto reset_input_fmt = [&]() {
    ...
    for (size_t input_cnt = 0; input_cnt < input_ops.size(); input_cnt++) {
      ...
      for (size_t i = 0; i < outs.size(); i++) {
        outs[i]->details_.set_format_and_stride(fmts[i].first, fmts[i].second);
      }
    }
  };
  reorder_callback_type insert_reorder_callback;
  ...
  reorder_map_t reorder_map;
  bool is_out_plain =
      graph.attrs_.get_or_else(sc_graph_t::attr_key_t::is_output_plain, true);
  auto do_visit = [&](const sc_op_ptr &node) {
    if (node->isa<output_op>()) {
      insert_reorder_for_output_op(
          reorder_map, node, use_channel_last_format, is_out_plain,
          is_input_plain, is_graph_dynamic, graph, insert_reorder_callback);
      ...
      auto &inputs = is_graph_dynamic ? inputs_backup : node->info_.inputs_;
      auto &outputs = node->info_.outputs_;
      check_input_format(inputs);
      size_t cur_layout_choice = cur_choice[node->logical_op_id_];
      if (node->isa<tunable_op_t>() || node->isa<binary_elementwise_op_t>() ||
          node->isa<tensor_view_op_t>() ||
          node->isa<ops::dynamic_reshape_op>() || node->isa<concat_op_t>() ||
          node->isa<select_op_t>() || node->isa<broadcast_op_t>() ||
          node->isa<reshape_op_t>() || node->isa<reduce_op_t>()) {
        std::vector<sc_data_format_t> old_formats;
        std::vector<std::vector<sc_data_format_t>> input_format_candidates;
        ...
        for (size_t i = 0; i < inputs.size(); ++i) {
          ...
          std::set<sc_data_format_t, sc_data_format_cmper_t> format_candidates;
          if (is_graph_dynamic) {
            auto &inp_candidates = inputs[i]->details_.get_format_candidates();
            format_candidates.insert(inp_candidates.begin(),
                                     inp_candidates.end());
          } else {
            format_candidates.insert(inputs[i]->details_.get_format());
          }
          input_format_candidates.emplace_back(format_candidates.begin(),
        ...
        for (size_t c = 0; c < total_format_choice; c++) {
          ...
          if (is_graph_dynamic) {
            for (size_t i = input_format_candidates.size(); i > 0; i--) {
              inputs[i - 1]->details_.set_format(
                  input_format_candidates[i - 1]
          ...
          if (!in_supported_pairs.empty() && !out_supported_pairs.empty()) {
            for (size_t i = 0; i < inputs.size(); i++) {
              auto &target_fs_pair = in_supported_pairs[i][cur_layout_choice];
              format_stride_pair in_fs_pair(inputs[i]->details_.get_format(),
              ...
              if (is_graph_dynamic) {
                if (node->isa<tunable_op_t>()) {
                  ...
                  // config.
                  for (size_t k = in_supported_pairs[i].size(); k > 0; k--) {
                  ...
                  if (format_candidates.find(target_format) ==
                      format_candidates.end()) {
                    insert_reorder_op(graph, reorder_map, inputs[i], i,
                                      target_fs_pair, node, is_input_plain,
                                      insert_reorder_callback);
                  }
                }
              } else if (in_fs_pair != target_fs_pair) {
                if (graph.attrs_.get_or_else("insert_reorder", true)) {
                  // static update
                  insert_reorder_op(graph, reorder_map, inputs[i], i,
                                    target_fs_pair, node, is_input_plain,
                                    insert_reorder_callback);
                  ...
                  inputs[i]->details_.set_format_and_stride(
                      target_fs_pair.first, target_fs_pair.second);
                }
              } else if (!insert_reorder_callback) {
                // if static and no need of reorder
                remove_inserted_reorder_op(graph, reorder_map, inputs[i], node);
              }
              if (is_graph_dynamic) {
            ...
            if (is_graph_dynamic) {
              ...
              if (!node->isa<tunable_op_t>()) {
                ...
                // itself.
                dispatch_format.push_back(out_supported_pairs[0][0].first);
            ...
            COMPILE_ASSERT(
                0, "The op must support query_format: " << node->op_name_);
          }
        }
        // reset old format
        if (is_graph_dynamic) {
        ...
        // query_format func
        if (is_graph_dynamic) {
          auto &format_candidates = inputs[0]->details_.get_format_candidates();
        ...
        COMPILE_ASSERT(0, "Only support fusible op/tunable op/in op/out op "
                          "in "
  ...
  if (total_choices <= STATIC_MAX_LAYOUT_TRIES && total_choices > 1) {
    ...
    for (size_t tr = 0; tr < total_choices; tr++) {
      reset_input_fmt();
      size_t cost = 0;
      insert_reorder_callback = [&cost](const graph_tensor_ptr &in,
  ...
  if (is_graph_dynamic) {
    combine_layout_and_impl_dispatch(ctx, graph);
  }
  graph.reset_op_ids();
}
} // namespace gc
