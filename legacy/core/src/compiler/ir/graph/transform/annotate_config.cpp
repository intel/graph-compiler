...
void annotate_config(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  vis0.visit_graph(graph, [&](op_visitor_t *vis0, const sc_op_ptr &node) {
    ...
    if (node->isa<quantize::dequantize_op_t>()) {
      is_int8 = true;
    }
    if (node->op_name_ == "matmul" || node->op_name_ == "managed_matmul_core") {
      // currently only consider static case
      if (node->is_dynamic()) {
        return;
      }
      size_t use_size = node->get_outputs()[0]->uses_.size();
      auto next_node = node->get_outputs()[0]->uses_.at(0).second.get_shared();
      while (!next_node->isa<tunable_op_t>() && !next_node->isa<output_op>() &&
             next_node->op_name_ != "matmul") {
        if (next_node->isa<reduce_op_t>() ||
            next_node->isa<reduce_mean_op_t>() ||
            next_node->op_name_ == "layernorm") {
          auto data_dims = node->get_inputs()[0]->details_.get_plain_dims();
          auto weight_dims = node->get_inputs()[1]->details_.get_plain_dims();
          auto out_dims_size = std::max(data_dims.size(), weight_dims.size());
          bool transpose_b = node->attrs_.get_or_else("transpose_b", false);
          bool transpose_a = node->attrs_.get_or_else("transpose_a", false);
          if (next_node->attrs_.has_key("rd_axis") &&
              !next_node->attrs_.get_or_else(op_attr_key::break_pre_fuse,
                                             false)) {
            std::vector<int> rd_axis =
                next_node->attrs_.get<std::vector<int>>("rd_axis");
            if ((weight_dims.size() == 2 || data_dims.size() == 2) &&
                rd_axis.size() == 1 &&
                rd_axis.at(0) == static_cast<int>(out_dims_size) - 1) {
              auto K = transpose_b ? weight_dims.at(1) : weight_dims.at(0);
              auto M = transpose_a ? data_dims.at(1)
                                   : math_utils::get_dims_product(data_dims) /
                                         data_dims.back();
              if (((K >= 640 && K < 4096) || M < 12288) &&
                  utils::is_one_of(node->get_inputs()[0]->details_.dtype_,
                                   datatypes::bf16, datatypes::f16) &&
                  !is_int8) {
                next_node->attrs_.set(op_attr_key::break_pre_fuse, true);
                break;
              }
            }
        ...
        if (next_node->attrs_.get_or_else(op_attr_key::break_pre_fuse, false)) {
          break;
        }
        if (next_node->get_outputs()[0]->uses_.size() > 1) {
          break;
        }
        if (next_node->isa<binary_elementwise_op_t>() &&
            !(next_node->get_inputs()[1]->producer_owner_->isa<input_op>()) &&
            node->get_inputs()[0]->details_.dtype_ == datatypes::f32 &&
            !is_int8) {
          // consider residual cases
          break;
        }
        next_node =
            next_node->get_outputs()[0]->uses_.at(0).second.get_shared();
      }
    }
  });
}
