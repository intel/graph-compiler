...
static bool is_tensor_zero_shape(const graph_tensor_ptr &gt) {
  ...
  if (std::all_of(dims.begin(), dims.end(), [](sc_dim d) { return d != 0; })) {
    return false;
  }
  return true;
}
...
SC_INTERNAL_API void eliminate_zero_shaped_tensors(sc_graph_t &graph,
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &op) {
    ...
    if (op->isa<concat_op_t>() || op->isa<output_op>()) {
      ...
      COMPILE_ASSERT(0, "padding and shape op do not support zero-shape tensor "
                        "input.");
    } else {
      vis->update_state_for_visited(op);
