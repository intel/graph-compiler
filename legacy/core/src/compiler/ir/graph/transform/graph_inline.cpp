...
static void do_inline_graph(const context_ptr &ctx, sc_graph_t &full_graph,
  ...
  std::unordered_map<sc_op_ptr, std::vector<sc_op_ptr>> *tunable_op_map =
      full_graph.attrs_
          .get_or_null<std::unordered_map<sc_op_ptr, std::vector<sc_op_ptr>>>(
              "temp.op_map");
  ...
  for (auto &op : sub_graph.ops_) {
    if (op->isa<input_op>()) {
      ...
      if (ctx->flags_.opt_level_ == sc_opt_level::lv1 ||
          cur_node->attrs_.get_or_else(op_attr_key::break_pre_fuse, false) ||
          cur_node->attrs_.get_or_else(op_attr_key::no_fuse, false)) {
          ...
          for (auto &u : cur->uses_) {
            ...
            for (auto &in : user_op->get_inputs()) {
              if (!(in->producer_owner_->isa<input_op>() ||
                    in->producer_owner_->isa<constant_op_t>())) {
                need_break = false;
                break;
              }
            }
            u.second->attrs_.set(op_attr_key::break_pre_fuse, need_break);
          }
        }
      }
      ...
      if (ctx->flags_.opt_level_ == sc_opt_level::lv1 ||
          cur_node->attrs_.get_or_else(op_attr_key::break_post_fuse, false) ||
          cur_node->attrs_.get_or_else(op_attr_key::no_fuse, false)) {
        for (auto &cur : op->get_inputs()) {
          cur->producer_owner_->attrs_.set(op_attr_key::break_post_fuse, true);
        }
      }
    ...
    if (std::all_of(
            graph.ops_.begin(), graph.ops_.end(), [&](const sc_op_ptr &op) {
              return !op->isa<graph_op_t>() ||
                     blocked_list.find(op->op_name_) != blocked_list.end();
            })) {
      break;
    }
  }
