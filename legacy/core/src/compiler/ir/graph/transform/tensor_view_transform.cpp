...
sc_data_format_t try_compress_format(const sc_dims &plain_dims,
...
bool should_transform_reorder(const sc_op_ptr &node) {
  if (can_op_be_dispatched(node)) {
    ...
    return std::all_of(
        key_set.begin(), key_set.end(), [](const op_dispatch_key_t &key) {
  ...
  auto output_plain_shapes = node->get_outputs()[0]->details_.get_plain_dims();
  ...
  // reorder for padding
  if (sc_data_format_t::get_padded_plain_shapes(input_blocking_shapes,
                                                input_format) !=
      sc_data_format_t::get_padded_plain_shapes(output_blocking_shapes,
                                                output_format)) {
    return false;
  }
  ...
  input_blocking_shapes =
      sc_data_format_t::get_blocking_shapes(input_plain_shapes, input_format);
  output_blocking_shapes =
      sc_data_format_t::get_blocking_shapes(output_plain_shapes, output_format);
  ...
  while (inp_idx < sc_data_format_kind_t::MAX_DIMS &&
    ...
    if (inp_code.get(inp_idx) != out_code.get(out_idx)) {
      return false;
    }
    // skip same axis
    ...
    // different axis
    if (orig_inp_axis != orig_out_axis) {
      return false;
    }
    // different block
    ...
    if (inp_block_itr != inp_blocked_axis.end() ||
      ...
      if (inp_block_itr == inp_blocked_axis.end() &&
          out_block_idx[orig_out_axis] < out_block_itr->second.size()) {
        return false;
      }
      // out has no block, but inp has blocks remained.
      if (out_block_itr == out_blocked_axis.end() &&
          inp_block_idx[orig_inp_axis] < inp_block_itr->second.size()) {
        return false;
      }
      ...
      if (inp_block_itr != inp_blocked_axis.end() &&
        ...
        if (inp_block_idx[orig_inp_axis] < inp_block_itr->second.size() &&
          ...
          if (inp_block_itr->second[inp_block_idx[orig_inp_axis]] !=
              out_block_itr->second[out_block_idx[orig_out_axis]]) {
            return false;
          }
...
void convert_to_tensor_view(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->isa<reorder_op_t>() && reorder2tv &&
      ...
      auto view = graph.make(
          "tensor_view", node->get_inputs(), {tensor_view_out},
      ...
      tensor_view_out->producer_owner_ = nullptr;
      auto view =
          graph.make("tensor_view", node->get_inputs(), {tensor_view_out},
                     {{"shape", tensor_view_out->details_.get_blocking_dims()},
                      {"order", node->attrs_.get<std::vector<int>>("order")}});
      view->copy_dispatch_key_set_from_op(node);
      node->replace_uses_with_and_remove(view);
      vis->update_state_for_visited(view);
    }
