...
static op_traits::brgemm_fusion_acceptable_t *
to_brg_fusible(const sc_op_ptr &p) {
...
static brg_broadcast_type get_brgemm_broadcast_type(
  ...
  COMPILE_ASSERT(
      (l_format.format_code_ == r_format.format_code_ &&
...
struct fusion_state_t {
  ...
  virtual std::vector<std::shared_ptr<fusion_state_t>> next_possible_states(
      const std::vector<std::shared_ptr<fusion_state_t>> &all_states) = 0;
...
#define GET_BROADCAST_TYPE()                                                   \
  ...
  auto bct_type = get_brgemm_broadcast_type(l_format, r_format, l_plain_dims,  \
                                            r_plain_dims);
...
struct ab_zp_state_t : public fusion_state_t {
  ...
  bool is_acceptable(const sc_op_ptr &cur_op) override {
    if (cur_op_count_ >= max_op_count_) {
      return false;
    }
    if (!cur_op->isa<sub_op_t>()) {
      return false;
    }
    auto sub_op = cur_op->stc_cast<sub_op_t>();
    if (sub_op->get_broadcast_input() == 0) {
      return false;
    }
    GET_BROADCAST_TYPE();
  ...
  std::vector<std::shared_ptr<fusion_state_t>> next_possible_states(
      const std::vector<std::shared_ptr<fusion_state_t>> &all_states) override {
...
struct cast_state_t : public fusion_state_t {
  ...
  bool is_acceptable(const sc_op_ptr &cur_op) override {
    ...
    if (!cur_op->isa<cast_op_t>()) {
      return false;
    }
    cur_op_count_++;
    cur_op->stc_cast<cast_op_t>()->alg_kind_ = brgemm::alg_kind_t::out_dtype;
    return true;
  }
  std::vector<std::shared_ptr<fusion_state_t>> next_possible_states(
      const std::vector<std::shared_ptr<fusion_state_t>> &all_states) override {
...
struct scale_state_t : public fusion_state_t {
  bool is_acceptable(const sc_op_ptr &cur_op) override {
    ...
    if (!cur_op->isa<mul_op_t>()) {
      return false;
    }
    auto mul_op = cur_op->stc_cast<mul_op_t>();
    if (mul_op->get_broadcast_input() == 0) {
      return false;
    }
    ...
    if (bct_type == brg_broadcast_type::all_broadcast &&
        !mul_op->get_inputs()[1]->producer_owner_->isa<constant_op_t>()) {
      return false;
    }
    // should be f32
    if (!cur_op->get_inputs()[0]->details_.dtype_.is_etype(
  ...
  void do_transform(sc_graph_t &graph, const sc_op_ptr &main_op,
    ...
    if (auto const_op = cur_op->get_inputs()[1]
      ...
      if (const_op->get_constant_plain_dims() == sc_dims{1}) {
        ...
        auto main_format = main_op->get_outputs()[0]->details_.get_format();
        ...
        new_plain_dims.at(channel_axis) = main_plain_shapes.at(channel_axis);
        auto new_format = main_format;
        for (int i = 0; i < new_format.format_code_.norig_dims(); i++) {
          auto blk_idxs = new_format.format_code_.collect_blocking_index(i);
          if (i != channel_axis) {
        ...
        auto new_const_op =
            graph.make("constant", {}, {},
                       {{"values", std::make_shared<static_data_t>(scales)},
                        ...
                        {"format", new_format}});
        auto new_scale_op = graph.make(
            "mul", {cur_op->get_inputs()[0], new_const_op->get_outputs()[0]},
            {}, cur_op->attrs_);
...
struct bias_state_t : public fusion_state_t {
  bool is_acceptable(const sc_op_ptr &cur_op) override {
    ...
    if (!cur_op->isa<add_op_t>()) {
      return false;
    }
    auto add_op = cur_op->stc_cast<add_op_t>();
    if (add_op->get_broadcast_input() == 0) {
      return false;
    }
    GET_BROADCAST_TYPE();
    // support only_channel
    if (bct_type != brg_broadcast_type::only_channel) {
      return false;
    }
  ...
  void do_transform(sc_graph_t &graph, const sc_op_ptr &main_op,
    ...
    if (main_op->get_outputs()[0]->details_.dtype_.is_etype(
            sc_data_etype::S32) &&
        cur_op->get_inputs()[0]->details_.dtype_.is_etype(sc_data_etype::F32)) {
      ...
      auto l_plain_dims = main_op->get_outputs()[0]->details_.get_plain_dims();
      auto r_plain_dims = cur_op->get_inputs()[1]->details_.get_plain_dims();
      auto &r_format = cur_op->get_inputs()[1]->details_.get_format();
      auto &fcode = r_format.format_code_;
      int channel_axis = fcode.get(fcode.ndims() - 1);
      sc_dim channel_size = r_plain_dims.at(channel_axis);
      auto data_scales = main_op->attrs_.get<std::vector<float>>("data_scales");
      auto weight_scales =
          main_op->attrs_.get<std::vector<float>>("weight_scales");
      auto output_scales = math_utils::vector_mul(data_scales, weight_scales);
      ...
      if (output_scales.size() == 1) {
        output_scales = std::vector<float>(channel_size, output_scales[0]);
      }
      sc_dims const_plain_dims = r_plain_dims;
      auto const_scales = graph.make(
          "constant", {}, {},
          {{"values", std::make_shared<static_data_t>(output_scales)},
           {"dtype", datatypes::f32},
           {"plain_dims", const_plain_dims},
           {"format", r_format}});
      auto bias_mul_scales = graph.make(
          "mul", {cur_op->get_inputs()[1], const_scales->get_outputs()[0]}, {},
          {});
      auto castf32 = graph.make("cast", main_op->get_outputs(), {},
                                {{"dtype", datatypes::f32}});
      auto bias_add = graph.make(
          "add", {castf32->get_outputs()[0], bias_mul_scales->get_outputs()[0]},
          {}, cur_op->attrs_);
      ...
      for (auto &use : uses) {
        use.second->replace_input(use.first, pre_op_by_cur->get_outputs()[0]);
      }
      cur_op->remove();
      return;
    }
...
struct binary_elem_state_t : public elem_state_t {
  bool is_acceptable(const sc_op_ptr &cur_op) override {
    ...
    if (!cur_op->isa<binary_elementwise_op_t>()) {
      return false;
    }
    auto bin_op = cur_op->stc_cast<binary_elementwise_op_t>();
    if (bin_op->get_broadcast_input() == 0) {
      return false;
    }
    GET_BROADCAST_TYPE();
...
std::vector<std::shared_ptr<fusion_state_t>>
create_all_possible_states(bool include_zp = false,
                           bool include_binary = false) {
  std::vector<std::shared_ptr<fusion_state_t>> ret;
  if (include_zp) {
    ret.emplace_back(std::make_shared<ab_zp_state_t>());
  }
  ...
  if (include_zp) {
    ret.emplace_back(std::make_shared<c_zp_state_t>());
  }
  return ret;
}
...
void brgemm_fusion_transform(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  if (ctx->machine_.cpu_flags_.fAVX512AMXTILE) {
    return;
  }
  auto ops = graph.ops_;
  for (auto &main_op : ops) {
    if (!main_op->isa<tunable_op_t>()) {
      continue;
    }
    if (!main_op->is_single_output_single_use()) {
      continue;
    }
    sc_op_ptr cur_op = main_op;
    // todo : enable zp
    bool include_zp =
        graph.attrs_.get_or_else("temp.brgemm_fusion_include_zp", false);
    // currently binary injector has slow implement.
    bool include_binary =
        graph.attrs_.get_or_else("temp.brgemm_fusion_include_binary", false);
    ...
    static std::vector<std::shared_ptr<fusion_state_t>>
        all_possible_states_include_zp = create_all_possible_states(true);
    auto all_states =
        include_zp ? all_possible_states_include_zp : all_possible_states;
    if (include_binary) {
