...
struct compare_sc_op_t {
  bool operator()(const sc_op_ptr &v0, const sc_op_ptr &v1) const {
    return v0->op_name_ == v1->op_name_ &&
           v0->info_.outputs_[0]->details_ == v1->info_.outputs_[0]->details_ &&
...
void drop_same_op_on_output(sc_graph_t &graph, const graph_tensor_ptr &output) {
  std::unordered_map<sc_op_ptr, std::vector<int>, hash_sc_op_t, compare_sc_op_t>
      same_op_map;
  for (size_t i = 0; i < output->uses_.size(); i++) {
    auto node = output->uses_[i];
    // do not eliminate redundant ops marked as "temp.not_redundant"
    if (node.second->attrs_.get_or_else(op_attr_key::not_redundant, false)) {
      continue;
    }
  ...
  std::vector<std::pair<int, sc_op_ptr>> next_nodes(output->uses_.begin(),
                                                    output->uses_.end());
    ...
    if (it.second.size() > 1) {
      ...
      for (size_t i = 1; i < it.second.size(); i++) {
        if (it.second[i] >= static_cast<int>(next_nodes.size()) ||
            next_nodes[it.second[i]].second->get_outputs()[0]->uses_.empty()) {
          break;
        }
...
// input and output
void useless_tensor_view_elimination(sc_graph_t &graph,
...
void excess_tensor_view_elimination(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->isa<tensor_view_op_t>() && is_single_use(node)) {
      ...
      while (next_node->isa<tensor_view_op_t>() && is_single_use(next_node)) {
        merge_dispatch_key_sets(node, next_node);
        node_to_remove.push_back(next_node);
        pre_node = next_node;
        next_node = next_node->get_outputs()[0]->uses_[0].second.get_shared();
      }
      ...
      if (pre_node != next_node || pre_node->isa<tensor_view_op_t>()) {
        ...
        node->get_outputs()[0]->details_ = pre_node->get_outputs()[0]->details_;
        std::vector<std::pair<int, sc_op_weak_ptr_t>> uses =
            pre_node->get_outputs()[0]->uses_;
        for (size_t i = 0; i < uses.size(); i++) {
          int pre_idx = uses[i].first;
          uses[i].second->replace_input(pre_idx, node->get_outputs()[0]);
        }
        for (auto &del_node : node_to_remove) {
          del_node->remove();
        }
        if (pre_node->isa<tensor_view_op_t>()) {
          pre_node->remove();
        }
...
static bool can_simplify(const sc_op_ptr &node,
                         const constant_op_t *in_const_op) {
  auto const_dtype = in_const_op->get_constant_dtype();
  if (in_const_op->get_constant_plain_dims() != sc_dims{1}) {
    return false;
  }
  if (const_dtype == datatypes::f32) {
    float constant_val =
        reinterpret_cast<float *>(in_const_op->get_constant_values()->data_)[0];
    if ((constant_val == 0.f &&
    ...
    int constant_val =
        reinterpret_cast<int *>(in_const_op->get_constant_values()->data_)[0];
    if ((constant_val == 0 &&
         (node->isa<add_op_t>() || node->isa<sub_op_t>())) ||
        (constant_val == 1 && (node->isa<mul_op_t>() || node->isa<div_op_t>())))
      return true;
...
void redundant_binary_op_elimination(sc_graph_t &graph,
                                     const context_ptr &ctx) {
  auto vis = op_visitor_t::bfs();
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    vis->update_state_for_visited(node);
    ...
    if (node->isa<add_op_t>() || node->isa<mul_op_t>()) {
      for (size_t i = 0; i < node->get_inputs().size(); i++) {
        if (node->get_inputs()[i]->producer_owner_->isa<constant_op_t>()) {
          auto in_const_op =
              node->get_inputs()[i]->producer_owner_->dyn_cast<constant_op_t>();
          ...
          if (do_simplify) {
            ...
            for (size_t j = 0; j < use_size; j++) {
              sc_op_ptr next_node =
                  node->get_outputs()[0]->uses_.at(use_idx).second.get_shared();
              int idx = node->get_outputs()[0]->uses_.at(use_idx).first;
              if (next_node->isa<output_op>() &&
                  node->get_inputs()[1 - i]->producer_owner_->isa<input_op>()) {
                insert_tensor_view_op(graph, node->get_inputs()[1 - i], idx,
                                      next_node);
              } else {
                next_node->replace_input(idx, node->get_inputs()[1 - i]);
                use_idx--;
              }
              use_idx++;
            }
    ...
    // x - 0 or x / 1
      ...
      if (node->get_inputs()[1]->producer_owner_->isa<constant_op_t>()) {
        auto in_const_op =
            node->get_inputs()[1]->producer_owner_->dyn_cast<constant_op_t>();
        bool do_simplify = can_simplify(node, in_const_op);
        if (do_simplify) {
          size_t use_size = node->get_outputs()[0]->uses_.size();
          int use_idx = 0;
          for (size_t j = 0; j < use_size; j++) {
            ...
            if (next_node->isa<output_op>() &&
                node->get_inputs()[0]->producer_owner_->isa<input_op>()) {
              insert_tensor_view_op(graph, node->get_inputs()[0], idx,
                                    next_node);
            } else {
              next_node->replace_input(idx, node->get_inputs()[0]);
              use_idx--;
            }
            use_idx++;
          }
...
// not, exchange them.
static void exchange_binary_const_ops(sc_graph_t &graph,
                                      const context_ptr &ctx) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->isa<mul_op_t>() || node->isa<add_op_t>()) {
      if (node->get_inputs()[0]->producer_owner_->attrs_.get_or_else(
          ...
          node->get_inputs()[1]->producer_owner_->attrs_.get_or_else(
              "constant", const_kind::not_const) == const_kind::not_const) {
        auto new_node = graph.make(
            node->op_name_, {node->get_inputs()[1], node->get_inputs()[0]}, {},
            node->attrs_);
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->isa<relu_op_t>()) {
      ...
      while (cur_node->is_single_output_single_use() &&
             utils::is_one_of(
                 cur_node->get_outputs()[0]->uses_[0].second->op_name_,
        ...
        if (pre_node == node) {
          cur_node->replace_input(node->get_outputs()[0]->uses_[0].first,
                                  node->get_inputs()[0]);
        }
        pre_node = cur_node;
      }
      if (cur_node != node) {
        ...
        for (auto &use : uses) {
          use.second->replace_input(use.first, node->get_outputs()[0]);
        }
...
static sc_op_ptr same_priority_pattern_fold(sc_graph_t &graph,
  ...
  if (is_basic_binary_calculate_op(node) &&
    ...
    std::string elt_type;
    if (utils::is_one_of(out_tsr->details_.dtype_, datatypes::f32,
                         datatypes::s32) &&
        is_same_priority(node, next_node, out_tsr->details_.dtype_, elt_type)) {
      ...
      if (is_constant_op(cur_inp1) && is_constant_op(next_inp1) &&
          next_inp1 != node.get()) {
        sc_op_ptr cal_const;
        cal_const = graph.make(
            elt_type, {cur_inp1->get_outputs()[0], next_inp1->get_outputs()[0]},
            {}, {});
        cal_const->attrs_.set("constant", const_kind::local_const);
        cal_const->attrs_.set("all_positive", is_all_positive(cur_inp1) &&
                                                  is_all_positive(next_inp1));
        auto new_node =
            graph.make(node->op_name_,
        ...
        for (auto &use : uses) {
          use.second->replace_input(use.first, new_node->get_outputs()[0]);
        }
        next_node->remove();
        return new_node;
      }
...
static sc_op_ptr diff_priority_pattern_fold(sc_graph_t &graph,
                                            const sc_op_ptr &node) {
  if (is_high_priority_op(node) && node->is_single_output_single_use()) {
    ...
    if (is_lower_priority_op(next_node) &&
      ...
      if (is_high_priority_op(nnext_node) &&
        ...
        if (!is_constant_op(nnext_inp1)) {
          return nullptr;
        }
        std::string cur_elt_type = "mul", next_elt_type = nnext_node->op_name_;
        if ((dtype == datatypes::s32 && node->isa<mul_op_t>() &&
             nnext_node->isa<mul_op_t>()) ||
            (dtype == datatypes::f32 &&
             is_same_priority(node, nnext_node, dtype, cur_elt_type))) {
          auto cur_cal_const = graph.make(
              cur_elt_type,
              {cur_inp1->get_outputs()[0], nnext_inp1->get_outputs()[0]}, {},
              {});
          cur_cal_const->attrs_.set("constant", const_kind::local_const);
          cur_cal_const->attrs_.set("all_positive",
                                    is_all_positive(cur_inp1) &&
                                        is_all_positive(nnext_inp1));
          auto new_node = graph.make(
              node->op_name_,
              {node->get_inputs()[0], cur_cal_const->get_outputs()[0]}, {},
              node->attrs_);
          new_node->copy_dispatch_key_set_from_op(node);
          node->replace_uses_with_and_remove(new_node);
          auto next_cal_const = graph.make(
              next_elt_type,
              {next_inp1->get_outputs()[0], nnext_inp1->get_outputs()[0]}, {},
              {});
          next_cal_const->attrs_.set("constant", const_kind::local_const);
          next_cal_const->attrs_.set("all_positive",
                                     is_all_positive(next_inp1) &&
                                         is_all_positive(nnext_inp1));
          auto new_next_node = graph.make(
              next_node->op_name_,
              {next_node->get_inputs()[0], next_cal_const->get_outputs()[0]},
              {}, next_node->attrs_);
...
static void fold_polynomial(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    ...
    for (int i = 0; i < MAX_TRY_TIMES; i++) {
      ...
      if (same_folded) {
        last_folded = same_folded;
      }
      auto diff_folded = diff_priority_pattern_fold(graph, last_folded);
      if (diff_folded) {
        last_folded = diff_folded;
      }
      if (pre_folded == last_folded) {
        break;
      }
