...
static bool use_rl(const context_ptr &ctx, const sc_data_type_t &data_dtype,
  ...
  // handling
  return (
      !is_1x1 && ndims == 4UL + (groups > 1) && is_small_padding &&
      (ic <= (tile_col / 2)) &&
      ((ic % vnni_blk != 0 && kh * ic <= threshold) || (ic % vnni_blk == 0)) &&
      kh <= ih);
}

static void query_accu_info_for_rl(const context_ptr &ctx,
                                   const sc_data_type_t &dtype, const int kh,
                                   const int kw, const int ic,
                                   int &num_brgemm_k, int &brgemm_k,
                                   int &extra_padding, int &kind, int groups) {
...
void rl_conv_weight_transform(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (auto op = node->dyn_cast<ops::conv_fwd_core_op_t>()) {
      auto data_plain_dims = op->info_.inputs_[0]->details_.get_plain_dims();
      auto weight_plain_dims = op->info_.inputs_[1]->details_.get_plain_dims();
      ...
      if (ndims != 4UL + (groups > 1)) {
        return;
      }
      // depthwise convolution
      bool is_dw_brdgmm =
          groups > 1 && 1 == weight_plain_dims[1] && 1 == data_plain_dims[2];
      if (is_dw_brdgmm) {
        return;
      }
      ...
      if (kind == ops::rl_kind::NO_LOWERING) {
        return;
      }
#if 0
      ...
      op->attrs_["origin_wei_plain_dims"] =
          groups > 1 ? sc_dims{groups, oc, ic, kh, kw}
                     : sc_dims{oc, ic, kh, kw};

      auto weight = op->get_inputs()[1];
      auto weight_op = weight->producer_owner_;
      auto is_constant_weight =
          (weight_op->isa<constant_op_t>() ||
           weight_op->attrs_.get_or_else("constant", const_kind::not_const));
      if (kind == ops::rl_kind::FULL_LOWERING) {
        // B-OIHW->reorder->HWIO->view->KN->(padding on "K")
        auto wei_shape_2d = sc_dims{ic * kh * kw, groups * oc};
        auto trans_wei = graph.make(
            "reorder", {weight}, {},
            {{"out_format", sc_data_format_t(groups > 1 ? format_kinds::EDCAB
                                                        : format_kinds::DCBA)},
             {"internal", true}});
        ...
        auto view_wei =
            graph.make("tensor_view", trans_wei->get_outputs(), {},
                       {{"shape", wei_shape_2d},
        ...
        if (extra_padding > 0) {
          auto padded_wei = graph.make("padding", view_wei->get_outputs(), {},
