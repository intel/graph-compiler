...
static bool do_swap(sc_graph_t &mgr, sc_op *bcast, sc_op *other,
  ...
  assert(bcast->get_outputs().size() == 1 && other->get_outputs().size() == 1);
  ...
  auto new_other = mgr.make(other->op_name_, {new_bcast->get_outputs()[0], v1},
                            {}, other->attrs_);
...
static bool check_and_swap(sc_graph_t &mgr, add_op_t *bcast_node,
  ...
  if (auto parent_add = parent->dyn_cast<add_op_t>()) {
    if (parent_add->get_broadcast_input() == -1) {
      return do_swap(mgr, bcast_node, parent_add, bcast_input_idx, visitor);
    }
  }
  return false;
}
