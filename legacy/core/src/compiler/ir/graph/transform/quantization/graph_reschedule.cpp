...
bool reschedule_forbid_op(const sc_op_ptr &node) {
  ...
  return ((node->isa<ops::dynamic_reshape_op>() ||
          ...
          !node->attrs_.get_or_else("allow_quantize_reschedule", false)) ||
    ...
    if (node->isa<dequantize_op_t>() || node->isa<dynamic_dequantize_op_t>()) {
      ...
      if (cld_nodes.size() == 1) {
        ...
        if (cast_op->isa<cast_op_t>() &&
            cast_op->attrs_.get_or_else(attr_keys::mixed_dtype, false)) {
      ...
      for (auto &cld_node : cld_nodes) {
        if ((cld_node.second->isa<op_traits::may_quantize_t>() &&
             cld_node.second->dyn_cast<op_traits::may_quantize_t>()
                 ->should_quantized_ &&
             !reschedule_forbid_op(cld_node.second))) {
          cld_node.second->replace_input(cld_node.first, dequantize_input);
          use_count--;
        }
      }
      if (!use_count) {
  ...
  vis.visit_graph(mgr, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->dyn_cast<op_traits::may_quantize_t>()) {
      if (node->isa<tunable_op_t>()) {
        ...
        // `to_visit` list.
        auto may_quantize_node = node->dyn_cast<op_traits::may_quantize_t>();
        ...
        node->get_outputs()[0]->details_.dtype_.type_code_ = sc_data_etype::S32;
        may_quantize_node->is_quantized_ = true;
        node->op_name_ = "quantized_" + node->op_name_;
        assert((node->attrs_.has_key(attr_keys::data_scales) &&
                node->attrs_.has_key(attr_keys::weight_scales)) ||
               (node->attrs_.has_key(attr_keys::dyn_data_scales) &&
                node->attrs_.has_key(attr_keys::dyn_weight_scales)));
        bool is_dyn_quan = node->attrs_.has_key(attr_keys::dyn_data_scales);
        auto weight_scales = node->attrs_.get_or_else(attr_keys::weight_scales,
                                                      std::vector<float>());
        int output_channel_axis;
        if (node->attrs_.has_key(attr_keys::output_channel_axis)) {
          output_channel_axis =
              node->attrs_.get<int>(attr_keys::output_channel_axis);
        } else if (node->isa<ops::conv_fwd_core_op_t>() &&
                   node->attrs_.has_key("data_format")) {
          // infer output_channel_axis based on op attributes
          auto data_format = node->attrs_.get<std::string>("data_format");
          auto ndims = node->get_outputs()[0]->details_.get_plain_dims().size();
          output_channel_axis = data_format == "NCX" ? 1 : ndims - 1;
        } else {
          output_channel_axis =
              node->attrs_.get<int>(attr_keys::weight_channel_axis);
          if (weight_scales.size() > 1 &&
              node->attrs_.has_key(attr_keys::weight_channel_axis)) {
        ...
        for (auto &child : tunable_op_consumers) {
          ...
          while (cur_child.second->dyn_cast<op_traits::may_quantize_t>() &&
            ...
            cur_child.second->replace_input(cur_child.first,
                                            cur_parent->get_outputs()[0]);
            cur_parent = cur_child.second;
            cur_child = cur_child.second->get_outputs()[0]->uses_[0];
          ...
          auto cur_parent_uses =
              cur_parent == node
                  ? std::vector<std::pair<int, sc_op_weak_ptr_t>>{child}
                  : cur_parent->get_outputs()[0]->uses_;
          // TODO(yifei): overcome the constraints here
          for (const auto &use : cur_parent_uses) {
            COMPILE_ASSERT(
                use.second->isa<concat_op_t>() ||
                    !use.second->dyn_cast<op_traits::may_quantize_t>(),
          ...
          if (is_dyn_quan) {
            assert(node->attrs_.has_key(attr_keys::dyn_data_scales) &&
                   (node->attrs_.has_key(attr_keys::dyn_weight_scales) ||
                    node->attrs_.has_key(attr_keys::weight_scales)));
            auto data_scales_gt =
                node->attrs_.get<graph_tensor_ptr>(attr_keys::dyn_data_scales);
            auto weight_scales_gt = node->attrs_.get_or_else(
                attr_keys::dyn_weight_scales, graph_tensor_ptr());
            sc_op_ptr out_scales;
            if (weight_scales_gt) {
              out_scales =
                  mgr.make("mul", {data_scales_gt, weight_scales_gt}, {}, {});
            } else {
              auto wei_scale_const = mgr.make(
                  "constant", {}, {},
                  {{"values", std::make_shared<static_data_t>(weight_scales)},
                   {"dtype", datatypes::f32},
                   {"plain_dims",
                    sc_dims{static_cast<sc_dim>(weight_scales.size())}},
                   {"format", sc_data_format_t()}});
              auto &wei_details = wei_scale_const->get_outputs()[0]->details_;
              out_scales = mgr.make(
                  "mul", {data_scales_gt, wei_scale_const->get_outputs()[0]},
                  {std::make_shared<graph_tensor>(
                      nullptr, wei_details.get_format(),
                      wei_details.get_plain_dims(), wei_details.dtype_)},
                  {});
            }
            bool is_per_channel =
                out_scales->get_outputs()[0]->details_.get_plain_dims()[0] > 1;
            dequantize_node = mgr.make(
                "dynamic_dequantize",
                {cur_parent->get_outputs()[0], out_scales->get_outputs()[0]},
                {},
                {{attr_keys::quan_dtype, datatypes::f32},
                 {attr_keys::per_channel, is_per_channel},
                 {attr_keys::channel_axis, output_channel_axis}});
          } else {
            auto data_scales =
                node->attrs_.get<std::vector<float>>(attr_keys::data_scales);
            auto output_scales =
                math_utils::vector_mul(data_scales, weight_scales);
            dequantize_node =
                mgr.make("dequantize", cur_parent->get_outputs(),
                         std::vector<graph_tensor_ptr>{},
                         {{attr_keys::quan_dtype, datatypes::f32},
                          {attr_keys::scales, output_scales},
                          {attr_keys::channel_axis, output_channel_axis}});
          }
          if (node->attrs_.get_or_else(attr_keys::mixed_dtype, false)) {
            dequantize_node = mgr.make("cast", dequantize_node->get_outputs(),
                                       std::vector<graph_tensor_ptr>{},
                                       {{"dtype", datatypes::bf16}});
          }
          for (const auto &use : cur_parent_uses) {
            use.second->replace_input(use.first,
        ...
        for (auto &out : node->get_outputs()) {
          if (node->isa<pooling_avg_op_t>()) {
            ...
            out->details_.dtype_ = node->get_inputs()[0]->details_.dtype_;
          }
          // insert dequantize if last op of pattern is output op,
          // for pattern like `qua->deq->reshape->output`
          std::vector<std::pair<int, sc_op_weak_ptr_t>> uses = out->uses_;
          for (auto &use : uses) {
            ...
            if ((!(cld_op->dyn_cast<op_traits::may_quantize_t>() &&
                ...
                ((node->attrs_.has_key(attr_keys::scales) &&
                  node->attrs_.has_key(attr_keys::zero_points)) ||
                 (node->attrs_.has_key(attr_keys::dyn_scales)))) {
              bool is_dyn_quan = node->attrs_.has_key(attr_keys::dyn_scales);
              sc_op_ptr dequantize_node;
              if (is_dyn_quan) {
                auto &scales =
                    node->attrs_.get<graph_tensor_ptr>(attr_keys::dyn_scales);
                auto &zero_points = node->attrs_.get<graph_tensor_ptr>(
                    attr_keys::dyn_zero_points);
                std::vector<graph_tensor_ptr> ins = {out, scales};
                if (zero_points) {
                  ins.emplace_back(zero_points);
                }
                dequantize_node =
                    mgr.make("dynamic_dequantize", ins, {}, node->attrs_);
              } else {
                dequantize_node =
                    mgr.make("dequantize", {out},
                             std::vector<graph_tensor_ptr>{}, node->attrs_);
              }
              if (node->attrs_.get_or_else(attr_keys::mixed_dtype, false)) {
                dequantize_node =
                    mgr.make("cast", dequantize_node->get_outputs(),
                             std::vector<graph_tensor_ptr>{},
                             {{"dtype", datatypes::bf16}});
              }
