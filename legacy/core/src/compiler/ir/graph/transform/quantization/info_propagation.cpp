...
// quantize node.
static std::vector<std::pair<int, sc_op_ptr>>
find_quantize_aware_nodes(const sc_op_ptr &node) {
  if (node == nullptr) {
    return std::vector<std::pair<int, sc_op_ptr>>();
  }
  ...
  for (auto &child_lt : node->get_outputs()) {
    for (const auto &child_op : child_lt->uses_) {
      ...
      if ((child_op.second->dyn_cast<op_traits::may_quantize_t>() &&
           child_op.second->attrs_.get_or_else(sc_graph_t::attr_key_t::quantize,
                                               true)) ||
...
void has_key_and_set(any_map_t &attrs, const std::string &key,
  ...
  if (attrs.has_key(key)) {
    aware_node->attrs_.set(new_key.empty() ? key : new_key, attrs.get<T>(key));
  }
}

static void
propagate_quantize_info(const sc_op_ptr &quantize_node,
                        const std::pair<int, sc_op_ptr> &aware_node) {
  assert((!(aware_node.second->attrs_.has_key(attr_keys::data_scales) &&
            aware_node.second->attrs_.has_key(attr_keys::dyn_data_scales)) ||
          !(aware_node.second->attrs_.has_key(attr_keys::weight_scales) &&
            aware_node.second->attrs_.has_key(attr_keys::dyn_weight_scales))) &&
  ...
  if (data_wei_op_set.find(aware_node.second->op_name_) !=
    ...
    case 1:
      ...
      break;
    default:
      assert(0 && "invalid tensor type!");
      break;
    };
    aware_node.second->attrs_.set(prefix + attr_keys::scales, qinfos.scales_);
    aware_node.second->attrs_.set(prefix + attr_keys::zero_points,
                                  qinfos.zero_points_);
    aware_node.second->attrs_.set(prefix + attr_keys::per_channel,
                                  qinfos.per_channel_);
    aware_node.second->attrs_.set(prefix + attr_keys::channel_axis,
                                  qinfos.channel_axis_);
    // dynamic quantize
    if (quantize_node->isa<dynamic_dequantize_op_t>()) {
      ...
      has_key_and_set<graph_tensor_ptr>(quantize_node->attrs_,
                                        ...
                                        aware_node.second, cur_dyn_scales);
      has_key_and_set<graph_tensor_ptr>(quantize_node->attrs_,
                                        attr_keys::dyn_zero_points,
                                        aware_node.second, cur_dyn_zero_points);
    }
    if (quantize_node->isa<dequantize_op_t>() ||
        quantize_node->isa<dynamic_dequantize_op_t>() ||
        quantize_node->attrs_.get_or_else(attr_keys::may_quantize, false)) {
    ...
    if (quantize_node->isa<dynamic_dequantize_op_t>()) {
      ...
      has_key_and_set<graph_tensor_ptr>(
          quantize_node->attrs_, attr_keys::dyn_scales, aware_node.second);
      has_key_and_set<graph_tensor_ptr>(
          quantize_node->attrs_, attr_keys::dyn_zero_points, aware_node.second);
    }
    // static quantize
    has_key_and_set<std::vector<float>>(quantize_node->attrs_,
                                        attr_keys::scales, aware_node.second);
    has_key_and_set<std::vector<int>>(
        quantize_node->attrs_, attr_keys::zero_points, aware_node.second);
    // common propagation.
    ...
    if (is_transpose &&
      ...
      auto order = aware_node.second->attrs_.get<std::vector<int>>("order");
      for (size_t i = 0; i < order.size(); ++i) {
        axis_map[order[i]] = i;
      }
      aware_node.second->attrs_.set(attr_keys::channel_axis,
                                    axis_map[channel_axis]);
    }
    has_key_and_set<bool>(quantize_node->attrs_, attr_keys::mixed_dtype,
                          aware_node.second);
    has_key_and_set<std::vector<float>>(
        quantize_node->attrs_, attr_keys::data_scales, aware_node.second);
    has_key_and_set<std::vector<float>>(
        quantize_node->attrs_, attr_keys::weight_scales, aware_node.second);
    has_key_and_set<std::vector<int>>(
        quantize_node->attrs_, attr_keys::data_zero_points, aware_node.second);
    has_key_and_set<std::vector<int>>(quantize_node->attrs_,
                                      attr_keys::weight_zero_points,
                                      aware_node.second);
    has_key_and_set<graph_tensor_ptr>(
        quantize_node->attrs_, attr_keys::dyn_data_scales, aware_node.second);
    has_key_and_set<graph_tensor_ptr>(
        quantize_node->attrs_, attr_keys::dyn_weight_scales, aware_node.second);
    has_key_and_set<graph_tensor_ptr>(quantize_node->attrs_,
                                      attr_keys::dyn_data_zero_points,
                                      aware_node.second);
    has_key_and_set<graph_tensor_ptr>(quantize_node->attrs_,
                                      attr_keys::dyn_weight_zero_points,
                                      aware_node.second);
    has_key_and_set<int>(quantize_node->attrs_, attr_keys::weight_channel_axis,
                         aware_node.second);
    if (quantize_node->isa<dequantize_op_t>() ||
        quantize_node->isa<dynamic_dequantize_op_t>() ||
        quantize_node->attrs_.get_or_else(attr_keys::may_quantize, false)) {
  ...
  if (attrs.has_key(attr_keys::data_scales) ||
    ...
    assert((attrs.has_key(attr_keys::weight_scales) ||
           ...
           !(attrs.has_key(attr_keys::scales) ||
             ...
             attrs.has_key(attr_keys::dyn_scales)) {
    assert(attrs.get<sc_data_type_t>(attr_keys::quan_dtype) == datatypes::bf16);
  ...
  vis.visit_graph(mgr, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->isa<dynamic_quantize_op_t>() ||
      ...
      if (node->get_inputs().size() == 3 &&
          node->get_inputs()[2]->details_.dtype_ != datatypes::s32) {
        auto &zp = node->get_inputs()[2];
        auto casts32 = mgr.make("cast", {zp}, {}, {{"dtype", datatypes::s32}});
      ...
      if (node->get_inputs()[0]->details_.dtype_ == datatypes::u8) {
        ...
        if (need_s8) {
          ...
          if (node_before->isa<quantize_op_t>() ||
            ...
            bool dyn_quan_before = node_before->isa<dynamic_quantize_op_t>();
            assert(node_before->attrs_.get_or_else(
                       attr_keys::quan_dtype, datatypes::s8) == datatypes::u8);
            node_before->attrs_.get<sc_data_type_t>(attr_keys::quan_dtype) =
                datatypes::s8;
            node->get_inputs()[0]->details_.dtype_ = datatypes::s8;
            assert(node->attrs_.get_or_else(attr_keys::quan_dtype,
                                            datatypes::s8) == datatypes::f32);
            ...
            if (dyn_quan_before) {
              ...
              int value = 128;
              auto const_128 = mgr.make(
                  "constant", {}, {},
                  {{"dtype", datatypes::s32},
                   {"format", sc_data_format_t()},
                   {"values",
                    std::make_shared<static_data_t>(&value, sizeof(int))},
                   {"plain_dims", sc_dims{1}}});
              auto shift_128 =
                  mgr.make("sub", {zp, const_128->get_outputs()[0]}, {}, {});
              node_before->replace_input(2, shift_128->get_outputs()[0]);
              node->replace_input(2, shift_128->get_outputs()[0]);
            } else {
              assert(
                  node_before->attrs_
                      .get_or_else(attr_keys::zero_points, std::vector<int>())
                      .size() == 1);
              assert(
                  node->attrs_
                      .get_or_else(attr_keys::zero_points, std::vector<int>())
              ...
              node_before->attrs_.get<std::vector<int>>(
                  attr_keys::zero_points)[0] -= 128;
              node->attrs_.get<std::vector<int>>(attr_keys::zero_points)[0] -=
                  128;
            }
          } else {
            auto *node_before = node->get_inputs()[0]->producer_owner_;
            const auto &qinfos = get_quantize_info_from_attrs(node->attrs_);
            auto new_zero_point = qinfos.zero_points_[0] - 128;
            auto casts32 = mgr.make("cast", node_before->get_outputs(), {},
                                    {{"dtype", datatypes::s32}});
            auto const128 = mgr.make(
                "constant", {}, {},
                {{"values",
                  std::make_shared<static_data_t>(std::vector<int>{128})},
                 {attr_keys::quan_dtype, datatypes::s32},
                 {"plain_dims", sc_dims{1}}});
            auto sub128 = mgr.make(
                "sub", {casts32->get_outputs()[0], const128->get_outputs()[0]},
                {}, {});
            ...
            if (dyn_quan_cur) {
              ...
              auto &zp = node->get_inputs()[2];
              auto shift_128 =
                  mgr.make("sub", {zp, const128->get_outputs()[0]}, {}, {});
              ...
              deq = mgr.make(
                  "dequantize", casts8->get_outputs(), {},
                  {{attr_keys::quan_dtype, datatypes::f32},
                   {attr_keys::scales, qinfos.scales_},
                   {attr_keys::zero_points, std::vector<int>{new_zero_point}}});
            }
...
// to calculation op
SC_INTERNAL_API void quantize_info_propagation(sc_graph_t &mgr,
  ...
  vis.visit_graph(mgr, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (node->isa<dequantize_op_t>() || node->isa<dynamic_dequantize_op_t>() ||
        (node->isa<op_traits::may_quantize_t>() && !node->isa<concat_op_t>()) ||
        node->isa<cast_op_t>()) {
      if (node->isa<cast_op_t>()) {
        check_and_set_mixed_dtype(node);
      }
