...
sc_dims get_conv1d_flatten_shape(const sc_data_format_t &format,
                                 const sc_dims &origin_shape,
                                 int merge_bs = 1) {
  COMPILE_ASSERT(origin_shape.size() == 4,
...
int get_minibatch(const int &bs, const int &min_os) {
  ...
  if ((bs / minibatch % num_threads != 0 && bs / minibatch < 4 * num_threads)) {
...
int minimum_spatial_shape(sc_graph_t &graph, bool &is_support) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (auto op = node->dyn_cast<ops::conv_fwd_core_op_t>()) {
      ...
      if (spatial_size < min_spatial) {
        min_spatial = spatial_size;
      }
    }
  });
  return min_spatial;
...
void conv1d_flatten(sc_graph_t &graph, const context_ptr &ctx) {
  ...
  vis.visit_graph(graph, [&](op_visitor_t *vis, const sc_op_ptr &node) {
    if (auto op = node->dyn_cast<ops::conv_fwd_core_op_t>()) {
      auto data_plain_shape = op->get_inputs()[0]->details_.get_plain_dims();
      // do not support dynamic shape
      if (op->get_inputs()[0]->is_dynamic())
        return;
      ...
      auto weight_plain_dims = op->get_inputs()[1]->details_.get_plain_dims();
      assert(weight_plain_dims.size() == ndims && "invalid weight_plain_dims");
      ...
      if (op->use_conv1d(ctx)) {
        { // pre tensor_view(data)
          auto shape = get_conv1d_flatten_shape(
              ...
              sc_data_format_t::get_padded_plain_shapes(data_origin_shape,
                                                        data_format),
              merge_bs);
          auto reorder_op = graph.make(
              "reorder", {op->get_inputs()[0]}, {},
              {{"out_format", sc_data_format_t::NHWC()}, {"internal", true}});
          auto view = graph.make("tensor_view", reorder_op->get_outputs(), {},
                                 {{"shape", shape},
        ...
        { // pre tensor_view(weight)
          auto shape =
              get_conv1d_flatten_shape(weight_format, weight_origin_shape);
          auto reorder_op = graph.make(
              "reorder", {op->get_inputs()[1]}, {},
              {{"out_format", sc_data_format_t::KCRS()}, {"internal", true}});
          auto view = graph.make("tensor_view", reorder_op->get_outputs(), {},
                                 {{"shape", shape},
        ...
        { // post tensor view(output)
          ...
          auto new_conv_out = std::make_shared<graph_tensor>(
              &(*node), sc_data_format_t::NCS(),
              get_conv1d_flatten_shape(
                  sc_data_format_t::NCHW(),
                  op->get_outputs()[0]->details_.get_blocking_dims(), merge_bs),
              dtype);
          op->info_.outputs_[0] = new_conv_out;

          auto reorder_op =
              graph.make("reorder", op->get_outputs(), {},
                         {{"out_format", sc_data_format_t::NSC()}});
          auto view =
              graph.make("tensor_view", reorder_op->get_outputs(), {},
...
void flatten_conv(sc_graph_t &graph, const context_ptr &ctx) {
  if (graph.attrs_.get_or_else("no_conv1d", false)) {
    return;
  }
