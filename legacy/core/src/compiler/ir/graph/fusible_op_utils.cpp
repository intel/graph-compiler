...
static std::vector<const tensor_slice *>
convert_const_t(std::vector<tensor_slice> &src) {
...
static std::vector<tensor_slice>
make_tensor_slice(sc_graph_t &graph, const std::vector<graph_tensor_ptr> &data,
                  const std::string &tensor_name,
                  std::vector<expr> &flattened) {
  std::vector<tensor_slice> expected;
  for (size_t i = 0; i < data.size(); ++i) {
    std::vector<expr> dims = data[i]->details_.get_blocking_dims_expr(graph);
    std::vector<expr> strides = dims_to_dense_stride(dims);
    expr aexpr = builder::make_stensor(tensor_name + std::to_string(i), dims,
                                       strides, data[i]->details_.dtype_);
...
ir_module_ptr fusible_op_get_func(fusible_op_t *op, const context_ptr &ctx) {
  ...
  for (auto &out : op->get_outputs()) {
    outs.emplace_back(std::make_shared<graph_tensor>(nullptr, out->details_));
  }
  g.make_input(ins);
  auto copyable = op->dyn_cast<op_traits::copyable_t>();
  COMPILE_ASSERT(copyable,
                 "The fusible op should be copyable: " << op->op_name_);
  ...
  if (!op->attrs_.get_or_else("temp.no_optimize_op", false) &&
    ...
    parti = std::make_shared<mixed_parti_t>(
        ctx, std::const_pointer_cast<sc_op>(copied->shared_from_this()));
  }
  ...
  if (cur_idx.defined() && upper_bound.defined()) {
    ...
    for (size_t i = 0; i < len; i++) {
      cur_list.emplace_back(builder::make_assign_unattached(
          out[i], make_select_by_mask(out[i], cur_idx, upper_bound, lanes)));
    }
...
expr make_select_by_mask(const expr &lhs_vec, const expr &cur_index,
  ...
  auto cur_step = builder::make_min(
      builder::make_max(builder::make_sub(upper_bound_int, cur_index_int), 0),
      step);
  stmt mask_def;
...
void choose_mask_vartype_init_value(sc_data_type_t &var_dtype,
                                    uint64_t &init_value, int32_t step) {
  switch (step) {
  case 4: {
  ...
  default:
    COMPILE_ASSERT(false, "invalid lanes: " << step);
  }
}

expr calculate_mask_cur_step(const expr &len, const expr &iter_var,
                             const int32_t lanes) {
  ...
  return builder::make_min(
      builder::make_max(builder::make_constant(0), last_axis_offset), lanes);
}
...
expr generate_mask_var_by_step(stmt &mask_def, const expr &cur_step,
  ...
  auto mask =
      builder::make_var(var_dtype, "__mask_" + std::to_string(var_idx++));
  mask_def = builder::make_var_tensor_def_unattached(mask, linkage::local,
                                                     mask_select);
  return mask;
}

expr generate_mask_by_step_directly(const expr &cur_step, int32_t step,
                                    const expr &sup_condition,
                                    bool direct_sup_cond) {
  // notice: cur_step must be s32
  sc_data_type_t var_dtype;
  uint64_t init_value;
  ...
  empty_mask_condition =
      (sup_condition.defined())
          ? (cur_step == 0 ||
             (direct_sup_cond ? sup_condition : !sup_condition))
          : (cur_step == 0);
  return builder::make_select(
      empty_mask_condition, empty_mask,
...
void compute_mask_and_generate_condition(
    ...
    const sc_dims &plain_dims, sc_data_format_t format,
    const std::vector<expr> &iter_vars, int lanes,
...
expr indexing_from_diff_cond(const bool is_lastdim_meet_require,
                             ...
                             std::vector<expr> &input_idx, const int32_t lanes,
                             expr &res_idx, const expr &axis_len,
                             const expr &iter_var, const expr &floor,
                             bool just_tail_part) {
  if (is_lastdim_meet_require) {
    ...
    auto mask = last_dim_generate_mask(iter_var, floor, axis_len, lanes,
                                       just_tail_part);
...
void compute_vectorized_op(const context_ptr &ctx, sc_graph_t &graph,
                           ...
                           const mask_compute_func_t &compute_scalar,
  ...
  bool dense_stride = std::all_of(
      info.inputs_.begin(), info.inputs_.end(),
    ...
    if (loop_size == vx_info.axis + 1 && i == vx_info.axis) {
      if (dense_stride && (!floor.isa<constant>() || floor_int)) {
        ...
        indexing_from_diff_cond(false, false, dst, dst_idx_floor, lanes,
                                indexed_target_floor, slice_len,
                                iter_vars.at(i), floor);
        std::vector<expr> indexed_input_floor;
        expr input_floor_idx;
        for (unsigned j = 0; j < src.size(); j++) {
          indexed_input_floor.emplace_back(indexing_from_diff_cond(
              false, false, *src.at(j), src_indices_floor.at(j), lanes,
              input_floor_idx, slice_len, iter_vars.at(i), floor));
        }
      ...
      if (((!tail.isa<constant>() && !is_blocking_shape) || tail_int) ||
        ...
        for (unsigned j = 0; j < src.size(); j++) {
          indexed_input_tail.emplace_back(
              builder::make_indexing(src.at(j)->tptr_, src_indices_tail.at(j),
        ...
        cur = make_stmt<for_loop_node_t>(
            tail_var, !dense_stride ? expr(0) : floor, slice_len,
            use_scalar ? expr(1) : expr(lanes), bld->pop_scope(), true,
            for_type::NORMAL);
        if (unroll_inner_loop) {
          cur->attr()[stmt_attr_key::unroll_loop] = 0;
        }
      ...
      if (!tcur.empty() && tcur[0].defined()) {
        ...
        indexed_target_floor = builder::make_indexing(dst.tptr_, dst_idx_floor);
        std::vector<expr> indexed_input_floor;
        for (unsigned j = 0; j < src.size(); j++) {
        ...
        bld->emit(cur);
        cur = make_stmt<for_loop_node_t>(
            iter_vars.at(i), expr(0), expand_loop_by->get_shape().at(i),
            expr(1), bld->pop_scope(), true, for_type::NORMAL);
        if (unroll_inner_loop) {
          cur->attr()[stmt_attr_key::unroll_loop] = 0;
        }
      }
...
void set_unknown_input_slice(fusible_op_t *cur,
                             const slice_range_map &known_ranges_map,
                             fslice_map &fsmap) {
...
std::unordered_map<int, binding_axis>
search_known_input_axis(sc_op *cur, binding_axis_map &bdax_map) {
  ...
  COMPILE_ASSERT(!known_axis_map.empty(), "No binded input axis found for "
                                              << cur->op_name_
                                              << cur->logical_op_id_)
...
void call_output_user_axis_binding(sc_op *cur, binding_axis_map &bdax_map) {
  for (auto &out : cur->get_outputs()) {
    for (auto &user : out->uses_) {
      if (auto bd_op =
              user.second->dyn_cast<op_traits::mixed_partition_acceptable>()) {
...
void set_unknown_binding_axis(
    sc_op *cur, const std::unordered_map<int, binding_axis> &known_axis_map,
  ...
  for (auto &op : pre_infer_list) {
    if (auto preop = op->dyn_cast<op_traits::mixed_partition_acceptable>()) {
...
std::vector<int>
transform_axis_plain2blocking(const logical_tensor_t &lt,
                              const std::vector<int> &plain_axis) {
...
std::vector<int>
transform_axis_plain2blocking(const graph_tensor_ptr &gt,
                              const std::vector<int> &plain_axis) {
  return transform_axis_plain2blocking(gt->details_, plain_axis);
}

std::vector<int>
transform_axis_blocking2plain(const logical_tensor_t &lt,
                              const std::vector<int> &blocking_axis) {
  auto fmt = lt.get_format();
  // If format is any, just return.
  if (fmt.is_any()) {
...
cmp_res cmp_slice_range(const slice_range_list &left_slice_range_list,
  ...
  COMPILE_ASSERT(!left_slice_range_list.empty() &&
                     !right_slice_range_list.empty(),
  ...
  if (is_dynamic_slice_range_list(left_slice_range_list) ||
    ...
    for (size_t i = left_slice_range.size(); i > 0; i--) {
      ...
      auto right_shape = do_cast_and_fold(right_slice_range[i - 1].second);
      if (!left_shape->equals(right_shape) &&
          !(left_shape.isa<constant>() && right_shape.isa<constant>() &&
            get_expr_as_int(left_shape) == get_expr_as_int(right_shape))) {
        if (left_shape.isa<constant>() && right_shape.isa<constant>()) {
          if (get_expr_as_int(left_shape) > get_expr_as_int(right_shape)) {
            left_slice_size++;
          } else {
            right_slice_size++;
          }
...
bool is_reshaped_tensor(const expr &tsr) {
  COMPILE_ASSERT(tsr.isa<tensorptr>(),
                 ...
                 "got "
                     << tsr);
  if (tsr.static_as<tensorptr>()->is_slice_)
    return false;
  ...
  for (auto &idx : base.static_as<indexing>()->idx_) {
    if (!idx.isa<constant>() || get_expr_as_int(idx) != 0)
      return false;
  }
...
expr transform_tsr2stsr_with_range(const expr &tsr, const slice_range &range) {
  ...
  return builder::make_stensor(t->name_ + "_strd", new_dims, new_strides,
                               t->elem_dtype_);
}

expr transform_tsl2stsr(const tensor_slice &tsl) {
  return transform_tsr2stsr_with_range(tsl.get_real_tensor(), tsl.get_ranges());
}

expr transform_tsr2tptr_with_range(const expr &tsr, const slice_range &range) {
  auto new_dims = get_slice_shape(range);
  return builder::tensor_ptr(tsr, get_slice_idx(range), get_slice_shape(range),
                             true);
}
...
expr transform_tptr2stsr(const expr &tptr) {
  ...
  COMPILE_ASSERT(tsr.isa<tensor>(), "tensor node is expected, but got " << tsr);
  auto t = tsr.static_as<tensor>();
  return builder::make_stensor(t->name_ + "_strd", tp->shape_, t->strides_,
                               t->elem_dtype_);
}

float evaluate_loop_parallel_balance(const std::vector<for_loop> &loops,
                                     bool check_use_full_threads) {
  expr dummy;
...
float evaluate_loop_parallel_balance(const std::vector<for_loop> &loops,
  ...
  for (size_t i = 0; i < loops.size(); i++) {
    ...
    if (!(loop->iter_begin_.isa<constant_c>() &&
      ...
      COMPILE_ASSERT(end > begin, "loop end is expected to larger than begin")
  ...
  if (stc_loops == 1) {
    return 0.0f + minor_dyn_parallelism;
  }
  ...
  if (stc_loops < run_threads) {
    cal_parallelism += minor_dyn_parallelism;
  }
  ...
  for (auto &ax : axis) {
    ...
    if (!first.isa<constant>()) {
      if (first->node_type_ == sc_expr_type::mul) {
        auto rv = constant_folding::get_operand_from_binary(first).second;
        // {i * block, block} case where `block_size==dims[i]`
        if (rv.isa<constant>() &&
            get_const_as_int(rv.static_as<constant>()) == dim[ax])
          continue;
      }
      return false;
...
bool slice_divisible_on_axis(const sc_dims &dim, const slice_range &ranges,
  ...
  for (auto &ax : axis) {
    ...
    if (second.isa<constant>() &&
        dim[ax] % get_const_as_int(second.checked_as<constant>()) != 0) {
      return false;
    }
  }
  return true;
...
bool slice_larger_than_bound_on_axis(const slice_range &ranges,
                                     const std::vector<int> &axis,
                                     const int factor, const int lower_bound) {
  auto total_len = 1;
  for (auto &ax : axis) {
    auto second = do_cast_and_fold(ranges[ax].second);
    if (second.isa<constant>()) {
  ...
  if (total_len / factor < lower_bound) {
    return false;
  }
  return true;
}
...
bool innermost_slice_with_non_dividable_lanes(const context_ptr &ctx,
                                              const slice_range &slice,
                                              const sc_data_type_t &dtype,
                                              sc_dim &floor, sc_dim &tail) {
  if (is_dynamic_slice_range_list({slice}))
    return false;
  ...
  if (get_dims_product(dims) != (size_t)last_dim)
    return false;
  // get max lanes
...
get_sorted_inputs_by_layout_input(const sc_op_ptr &op) {
  int layout_input_index =
      op->attrs_.get_or_else(op_attr_key::layout_input_index, 0);
  if (layout_input_index == 0) {
    return op->get_inputs();
  }
