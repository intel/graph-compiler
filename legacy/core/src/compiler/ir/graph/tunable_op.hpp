...
public:
  ...
  virtual void set_internal_config_by_key(const impl_op_dispatch_key_t &key,
                                          const context_ptr &ctx) {
    throw std::runtime_error("unimplemented");
  }
  ...
  config_ptr_vec get_dynamic_config_candidates(const context_ptr &ctx) override;
  impl_kind_map
  convert_config_candidates_to_impl_map(const config_ptr_vec &configs) override;
  std::vector<int>
