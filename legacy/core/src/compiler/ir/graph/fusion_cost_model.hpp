...
public:
  ...
  virtual bool make_decision_for_parti(const mixed_parti_t *parti,
                                       size_t merge_loop_size,
                                       parti_merge_kind merge_kind) = 0;
  // make decision for op and fusion anchor
  virtual bool
  make_decision_for_op(const sc_op *op,
                       const std::shared_ptr<fusion_anchor_t> &fanchor) = 0;
...
public:
  ...
  bool make_decision_for_parti(const mixed_parti_t *parti,
                               size_t merge_loop_size,
                               parti_merge_kind merge_kind) override;
  // make decision for op and fusion anchor
...
public:
  dynamic_fusion_cost_model_t(mixed_parti_t *parti,
                              dynamic_fusion_policy_t policy);
  // evaluate current mixed partition by several evaluator
