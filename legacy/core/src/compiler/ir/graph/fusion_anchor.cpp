...
static void inherit_parent_offset(slice_range_list &cur_value,
                                  const slice_range_list &parent_value) {
  // if slice range list size is not equal
  if (cur_value.size() != parent_value.size())
    return;
  for (size_t i = 0; i < cur_value.size(); i++) {
    ...
    for (size_t j = 0; j < cur_slice.size(); j++) {
      // inherit offset
      cur_slice[j].first =
          do_cast_and_fold(parent_slice[j].first + cur_slice[j].first);
    }
  }
}
...
void fusion_anchor_t::attach_parent_anchor(
    ...
    const std::shared_ptr<fusion_anchor_t> &repl_parent, bool inherit_offset) {
  if (!parent)
    return;
  if (inherit_offset) {
...
void fusion_anchor_t::merge(const std::shared_ptr<fusion_anchor_t> &other) {
  fsmap_.datamap_.insert(other->fsmap_.datamap_.begin(),
                         other->fsmap_.datamap_.end());
  blocked_gt_set_.insert(other->blocked_gt_set_.begin(),
                         other->blocked_gt_set_.end());
...
bool fusion_anchor_t::has_view_of(sc_op *op) {
  ...
  // if not found
  if (iter == binded_mxp_->op_anchor_map_.end())
    return true;
  auto op_anchor = iter->second;
  while (true) {
    if (op_anchor.get() == this)
      return true;
  ...
  auto ths_pos = std::find_if(
      ths_root_scope.begin(), ths_root_scope.end(),
      [&ths_anchor_ss](const stmt &s) { return ths_anchor_ss.ptr_same(s); });
  auto op_anchor_loop = op_anchor->get_parent_loop();
  while (true) {
    auto op_anchor_pos =
        std::find_if(ths_root_scope.begin(), ths_root_scope.end(),
                     [&op_anchor_loop](const stmt &s) {
...
bool fusion_anchor_t::check_dep_for_op(const sc_op *op) {
  ...
  for (auto &inp : op->get_inputs()) {
    ...
    for (auto &cur : parti->ops) {
      if (std::any_of(cur->get_outputs().begin(), cur->get_outputs().end(),
...
bool fusion_anchor_t::validate_input_for_op(
  ...
  auto fanchor = this;
  return std::all_of(
      op->get_inputs().begin(), op->get_inputs().end(),
      [&fanchor, &known_gt](const graph_tensor_ptr &gt) {
        ...
        while (cur->parent_) {
          ...
          // should not be empty
          if (!cur->fsmap_.hasvalue(gt) || (!parti->is_parti_inp(dep_inp) &&
          ...
          if (res != cmp_res::l_larger_r) {
            fanchor->borrowed_fanchor_map_[gt] = cur->shared_from_this();
            return true;
          }
        }
        return false;
...
bool fusion_anchor_t::is_small_op_workload(const sc_op *op) {
  if (op->is_dynamic() || !op->isa<fusible_op_t>())
    return false;
  ...
  for (auto &gt : gt_list) {
    ...
    wkld += utils::divide_and_ceil(
        get_dims_product(slice_shape),
        vectorize_step(get_binded_mxp()->ctx_, gt->details_.dtype_.type_code_));
    computed_count++;
  };
...
void fusion_anchor_mgr_t::create_fusion_anchor(const slice_map &fsmap,
                                               const fusion_anchor_ptr &parent,
...
void fusion_anchor_mgr_t::create_fusion_anchor(const expr &iter_var,
                                               const slice_map &fsmap,
                                               const stmt &dispatch_helper,
                                               const fusion_anchor_ptr &parent,
                                               bool is_input_anchor) {
  // create anchor placeholder in IR
  auto bld = builder::get_current_builder();
  ...
  auto iter_anchor = std::make_shared<iter_fusion_anchor_t>(
...
void fusion_anchor_mgr_t::create_fusion_anchor(const graph_tensor_ptr &gt,
                                               ...
                                               const fusion_anchor_ptr &parent,
                                               bool is_input_anchor) {
  create_fusion_anchor(slice_map{{gt.get(), slice_range_list{slice}}}, parent,
                       is_input_anchor);
}
...
void create_fusion_anchor(fusion_anchor_mgr_t *fusion,
                          ...
                          const slice_range &slice) {
  if (fusion) {
    fusion->create_fusion_anchor(gt, slice);
  }
}

fusion_anchor_ptr try_convert_anchor(const context_ptr &ctx,
  ...
  if (typeid(anchor_type) != typeid(fusion_anchor_t))
    return fanchor;
  ...
  if (!std::all_of(
          base_gt->uses_.begin(), base_gt->uses_.end(),
  ...
  // make gourped anchor
  return std::make_shared<grouped_fusion_anchor_t>(
      new_pos, fslice_map(datamap), parent_anchor, fanchor->is_input_anchor());
}
