...
static int get_base_input_idx(fusible_op_t *cur) {
  ...
  if (auto binary_node = cur->dyn_cast<binary_elementwise_op_t>()) {
    ...
    if (!binary_node->get_broadcast_input()) {
      base_idx = 1;
    }
  }
...
void fusible_op_t::create_mixed_partition(mixed_parti_t *parti) {
  ...
  if (auto reo_op = this->dyn_cast<reorder_op_t>()) {
    ...
    if (reo_op->check_padding()) {
      ...
      use_output_mode = !reo_op->attrs_.get_or_else("use_input_loop", false) &&
  ...
  COMPILE_ASSERT(status, "generate outer loops failed for "
                             << op_name_ << "_" << logical_op_id_
                             << ", please check");
  ...
  if (used_anchor != parti->fanchors_.back()) {
    ...
    for (auto iter = parti->fanchors_.begin(); iter < parti->fanchors_.end();) {
      if ((*iter) == used_anchor)
        break;
...
void fusible_op_t::append_mixed_partition(mixed_parti_t *parti) {
  COMPILE_ASSERT(parti->ready_for_op(this), "No suitable anchor found for "
                                                << op_name_ << "_"
                                                << logical_op_id_);
    ...
    if (!this->isa<movement_op_t>()) {
      ...
      if (fsmap.get(base_gt).size() == 1) {
        ...
        if (committed_anchor != parti->lookup_anchor_map(this)) {
          ...
          parti->fanchors_.erase(parti->fanchors_.end() - inner_anchors.size(),
                                 parti->fanchors_.end());
          // clear invoid inner anchor content from parent anchor
          for (auto iter = inner_anchors.rbegin(); iter != inner_anchors.rend();
               iter++) {
...
void fusible_op_t::commit_into_anchor(fusion_anchor_t *committed_anchor) {
  ...
  auto wrap_tsr2tsl_ = [&ths, &committed_anchor](const expr &tsr,
    ...
    if (!range_list.empty()) {
      if (ths->isa<reorder_op_t>() && is_output) {
        auto input_size =
            committed_anchor->fsmap_.get(ths->get_inputs()[0]).size();
        // multi-slice reorder
      ...
      if (is_output) {
        ...
        auto output_size =
            committed_anchor->fsmap_.get(ths->get_outputs()[0]).size();
  ...
  COMPILE_ASSERT(
      std::all_of(inputs.begin(), inputs.end(),
                  [&in_slice_size](const std::vector<tensor_slice> &input_tsl) {
...
void fusible_op_t::query_format(
    context_ptr ctx,
...
size_t
fusible_op_t::compute_workload(const std::vector<shape_dtype_pair> &ins,
                               const std::vector<shape_dtype_pair> &outs) {
  size_t wkld = 0UL;
  auto accumulate_workload = [&wkld](size_t weight, const shape_dtype_pair &v) {
    auto &dtype = v.second;
...
size_t fusible_op_t::compute_fusible_workload(
    const context_ptr &ctx, const std::vector<tensor_slice *> &dst,
  ...
  std::transform(dst.begin(), dst.end(), wkld_outs.begin(),
                 get_shape_dtype_pair);
...
input_op::input_op(const sc_dims &dims, sc_data_type_t dtype) {
  ...
  info_.outputs_.emplace_back(
      std::make_shared<graph_tensor>(this, sc_data_format_t(), dims, dtype));
}
...
static void extract_const_op_data(
    const sc_op *ths,
  ...
  if (!filter || filter(ths, "values")) {
    out_vales =
        ths->attrs_.get_or_null<std::shared_ptr<static_data_t>>("values");
...
bool constant_op_t::compare_contents(
    const sc_op *other,
    const std::function<bool(const sc_op *, const std::string &)> &filter)
    const {
  if (other->op_name_ != op_name_) {
    return false;
  }
  ...
  if (dtype) {
    if (!other_dtype || *dtype != *other_dtype) {
      return false;
    }
  }
  if (fmt) {
    if (!other_fmt || *fmt != *other_fmt) {
      return false;
    }
  }
  if (!values) {
    return other_values == nullptr;
  }
...
void constant_op_t::reset_const_values() {
  if (attrs_.has_key("temp.var") && attrs_.has_key("temp.val/var")) {
    int K =
        static_cast<int>(attrs_.get<std::shared_ptr<VConst>>("temp.var")->var_);
    int base_val = attrs_.get<int>("temp.val/var");
    // update private member
    const_values_ =
        std::make_shared<static_data_t>(std::vector<int>{base_val * K});
    // update attr
