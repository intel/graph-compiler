...
public:
  ...
  static std::vector<graph_tensor_ptr>
  remake_logical_tensors(const std::vector<graph_tensor_ptr> &flts);
  ...
  static std::shared_ptr<sc_op>
  cast_output_dtype(graph_tensor_ptr &inp, std::shared_ptr<sc_graph_t> &graph,
...
public:
  ...
  get_default_config(context_ptr ctx) override;

  config_ptr_vec
  get_dynamic_config_candidates(const context_ptr &ctx) override {
