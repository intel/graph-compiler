...
template <typename valT> struct gt_map_t;
...
struct sc_op_weak_ptr_t : public std::weak_ptr<sc_op> {
  ...
  bool operator==(const sc_op_ptr &other) const { return get() == other.get(); }
...
struct SC_API graph_tensor : public std::enable_shared_from_this<graph_tensor>
  ...
  static graph_tensor_ptr
  make(const sc_dims &shape, const sc_data_format_t &fmt = sc_data_format_t(),
       sc_data_type_t dtype = sc_data_type_t(sc_data_etype::F32, 1),
       const sc_dims &stride = {}) {
    return std::make_shared<graph_tensor>(nullptr, fmt, shape, dtype, stride);
  }
...
public:
  ...
  virtual dispatch_set_ptr
  get_internal_dispatch_key_set(const context_ptr &ctx);
  ...
  template <typename opT> bool share_gt_with_op(const graph_tensor_ptr &gt) {
    auto ths = this;
  ...
  void set_owner_graph(sc_graph_t *owner_graph) { owner_graph_ = owner_graph; }
   ...
   * @param T the subclass
   * */
  template <typename T> T *dyn_cast() const { return dynamic_cast<T *>(this); }

  template <typename T> T *dyn_cast() { return dynamic_cast<T *>(this); }

  /**
   * Static cast the node to a subclass or a trait.
   * @param T the subclass
   * */
  template <typename T> T *stc_cast() {
  ...
  static size_t standard_hash_contents(
      const sc_op *p,
      const std::function<bool(const sc_op *, const std::string &)> &filter);
  ...
  virtual void query_format(
      context_ptr ctx,
      std::vector<std::vector<format_stride_pair>> &supported_ins,
      std::vector<std::vector<format_stride_pair>> &supported_outs) = 0;
  ...
  virtual std::vector<int> get_impl_dispatch_candidates(const context_ptr &ctx);
...
public:
  ...
  make_output(const std::vector<graph_tensor_ptr> &inputs,
              const any_map_t &attrs = any_map_t());
  // input op
  std::shared_ptr<sc_op> make_input(const std::vector<graph_tensor_ptr> &inputs,
                                    const any_map_t &attrs = any_map_t());
  sc_graph_t() = default;
...
template <typename T> struct register_helper_t {
...
#define OP_REGISTER(IDENTIFIER, NAME)                                          \
  volatile bool __help_dummy_##NAME =                                          \
      dnnl::impl::graph::gc::register_helper_t<IDENTIFIER>::op_call(#NAME);
#endif
