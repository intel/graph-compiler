...
inline constexpr bool is_pointer(sc_data_etype type_code) {
  return static_cast<int>(type_code) & static_cast<int>(sc_data_etype::POINTER);
}
...
inline constexpr sc_data_etype get_pointerof(sc_data_etype type_code) {
  return is_pointer(type_code) ? sc_data_etype::POINTER
...
struct SC_API sc_data_type_t {
  ...
  inline static constexpr sc_data_type_t undef(uint16_t lanes = 1,
                                               uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::UNDEF, lanes, rows);
  }
  inline static constexpr sc_data_type_t f16(uint16_t lanes = 1,
                                             uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::F16, lanes, rows);
  }
  inline static constexpr sc_data_type_t bf16(uint16_t lanes = 1,
                                              uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::BF16, lanes, rows);
  }
  inline static constexpr sc_data_type_t u16(uint16_t lanes = 1,
                                             uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::U16, lanes, rows);
  }
  inline static constexpr sc_data_type_t f32(uint16_t lanes = 1,
                                             uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::F32, lanes, rows);
  }
  inline static constexpr sc_data_type_t s32(uint16_t lanes = 1,
                                             uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::S32, lanes, rows);
  }
  inline static constexpr sc_data_type_t u32(uint16_t lanes = 1,
                                             uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::U32, lanes, rows);
  }
  inline static constexpr sc_data_type_t s8(uint16_t lanes = 1,
                                            uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::S8, lanes, rows);
  }
  inline static constexpr sc_data_type_t u8(uint16_t lanes = 1,
                                            uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::U8, lanes, rows);
  }
  inline static constexpr sc_data_type_t index(uint16_t lanes = 1,
                                               uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::INDEX, lanes, rows);
  }
  inline static constexpr sc_data_type_t boolean(uint16_t lanes = 1,
                                                 uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::BOOLEAN, lanes, rows);
  }
  inline static constexpr sc_data_type_t generic(uint16_t lanes = 1,
                                                 uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::GENERIC, lanes, rows);
  }
  // void* type
  inline static constexpr sc_data_type_t pointer(uint16_t lanes = 1,
                                                 uint16_t rows = 0) {
    return sc_data_type_t(sc_data_etype::POINTER, lanes, rows);
  }
  inline static constexpr sc_data_type_t
  pointerof(sc_data_etype etype, uint32_t lanes = 1, uint16_t rows = 0) {
    return sc_data_type_t(etypes::get_pointerof(etype), lanes, rows);
  }
  inline static constexpr sc_data_type_t void_t(uint16_t lanes = 1,
                                                uint16_t rows = 0) {
...
SC_INTERNAL_API std::ostream &operator<<(std::ostream &os,
                                         sc_data_type_t dtype);

namespace utils {
template <> std::string print_vector(const std::vector<sc_data_type_t> &vec);
} // namespace utils
...
namespace std {
template <> struct hash<dnnl::impl::graph::gc::sc_data_type_t> {
  std::size_t operator()(const dnnl::impl::graph::gc::sc_data_type_t &k) const;
};
} // namespace std
