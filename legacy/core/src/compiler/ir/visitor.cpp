...
expr visit_base_binary(ir_visitor_base_impl_t<is_inplace> *vis, T &&v) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return builder::remake_binary(std::move(l), std::move(r),
                                    std::forward<T>(v));
    } else {
      return std::forward<T>(v);
    }
...
func_t ir_visitor_base_impl_t<is_inplace>::dispatch_impl(func_t s) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(
          *s, builder::make_func(s->name_, newparam, body, s->ret_type_));
    }
  }
  return s;
}
...
expr ir_visitor_base_impl_t<is_inplace>::visit_impl(tensor v) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(*v, make_expr<tensor_node>(v->elem_dtype_, v->name_,
                                                  newdims, v->address_space_,
                                                  v->init_value_, newstrides));
    } else {
      return std::move(v);
    }
  }
...
expr ir_visitor_base_impl_t<is_inplace>::visit_impl(select v) {
  ...
  changed_ =
      !cond.ptr_same(v->cond_) || !l.ptr_same(v->l_) || !r.ptr_same(v->r_);
  if (is_inplace) {
    if (changed_) {
      v->cond_ = cond;
      v->l_ = l;
    ...
    if (changed_) {
      return copy_attr(*v, builder::make_select(std::move(cond), std::move(l),
                                                std::move(r)));
    } else {
      return std::move(v);
    }
...
expr ir_visitor_base_impl_t<is_inplace>::visit_impl(indexing v) {
  ...
  if (is_inplace) {
    ...
    if (changed) {
      return copy_attr(
          *v, builder::make_indexing(std::move(ptr), std::move(new_arr),
...
expr ir_visitor_base_impl_t<is_inplace>::visit_impl(tensorptr v) {
  ...
  if (is_inplace) {
    if (changed_) {
      v->base_ = ptr.template checked_as<indexing>();
    }
    return std::move(v);
  } else {
    if (changed_) {
      return copy_attr(
          *v, make_expr<tensorptr_node>(ptr.template checked_as<indexing>(),
...
expr ir_visitor_base_impl_t<is_inplace>::visit_impl(logic_not v) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(*v, make_expr<logic_not_node>(l));
    }
    return std::move(v);
  }
}
...
stmt ir_visitor_base_impl_t<is_inplace>::visit_impl(assign v) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(*v,
                       make_stmt<assign_node_t>(std::move(l), std::move(r)));
    }
    return std::move(v);
  }
}
...
stmt ir_visitor_base_impl_t<is_inplace>::visit_impl(if_else v) {
  ...
  if (v->else_case_.defined())
    elsecase = dispatch_impl(v->else_case_);
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(*v, make_stmt<if_else_node_t>(std::move(cond),
                                                     std::move(thencase),
                                                     std::move(elsecase)));
    }
    return std::move(v);
...
stmt ir_visitor_base_impl_t<is_inplace>::visit_impl(for_loop v) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(*v, make_stmt<for_loop_node_t>(
                               std::move(var), std::move(begin), std::move(end),
...
stmt ir_visitor_base_impl_t<is_inplace>::visit_impl(evaluate v) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(*v, make_stmt<evaluate_node_t>(val));
    }
    return std::move(v);
  }
}
...
stmt ir_visitor_base_impl_t<is_inplace>::visit_impl(define v) {
  ...
  if (is_inplace) {
    ...
    if (changed_) {
      return copy_attr(*v, make_stmt<define_node_t>(std::move(var), v->linkage_,
                                                    std::move(init)));
    }
    return std::move(v);
  }
...
bool ir_visitor_t::dispatch_expr_vector(const std::vector<expr> &arr,
