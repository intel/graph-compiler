...
expr ssa_data_t::get_value_of_var_nothrow() const {
  ...
  auto owner = owner_.lock();
  assert(owner);
  if (owner->node_type_ != sc_stmt_type::define) {
    return expr();
  }
  return static_cast<define_node_t *>(owner.get())->init_;
}
...
stmt_c ssa_visitor_t::dispatch(stmt_c e) {
  ...
  case sc_stmt_type::define:
    process_define_node_after_visit(ret);
    break;
  case sc_stmt_type::stmts:
    break;
  }
...
stmt_c ssa_visitor_t::visit(stmts_c v) {
  ...
  for (auto &s : v->seq_) {
    ...
    for (auto &st : stmts_to_insert_after) {
      if (st.isa<define>()) {
        process_define_node_after_visit(st);
      }
...
define ssa_visitor_t::make_def(const expr_c &v) {
  auto ret = builder::make_var(v->dtype_, std::string("__tmp") +
                                              std::to_string(var_def_idx_++));
...
expr_c ssa_visitor_t::visit(tensor_c v) { return v; }
