...
module {
func.func @main_entry(%a: tensor<128xf32>, %b: tensor<128xf32>) -> tensor<128xf32> attributes { llvm.emit_c_interface } {
...
TEST(ExecutionEngine, JitWrapper) {
  ...
  ASSERT_TRUE(module);
  auto jited = gc::JitModule::create(module.get(), true);
  ...
  void *pargs[] = {&args[0], &args[1], &args[2]};
  jited.get()->call(pargs);
  for (int i = 0; i < 128; i++) {
