...
for thread in 56
do
for mode in bf16_4dx4d
do

for hidden_size in 2048x2048
do
for bs in 2048
do
    ...
    numactl -N 1 --membind=1 perf stat -d -d -d python3 ${PROJECT_DIR}/tools/main.py --type=bench --driver=load_mlir --path=${PROJECT_DIR}/build/mlp.mlir --repeat=$REPEAT
    # elapsed_time=`numactl -N 1 --membind=1 python3 ${PROJECT_DIR}/tools/main.py --type=bench --driver=load_mlir --path=${PROJECT_DIR}/build/mlp.mlir 2>log | tail -n 2 | head -n 1 | sed 's/.*execute_cost(ms)":.//g'`
    # gflops=`python -c "print(int($bs) * (int)('$hidden_size'.split('x')[0]) * (int)('$hidden_size'.split('x')[1]) * 2 / $elapsed_time / 1e6)"`
	correctness=`numactl -N 1 --membind=1 python3 ${PROJECT_DIR}/tools/example/simple_test.py --M=${M_SIZE} --K=${K_SIZE} --N=${N_SIZE} --tile_m=${TILE_M} --tile_n=${TILE_N} --tile_k=${TILE_K} --mode=${PERF_MODE} 2>__useless | tail -n 1`
