...
def generate_single_matmul_mlir(M, N, K):
    ...
    mat_D = numpy.random.rand(M, N)
    mat_C = numpy.add(mat_C, mat_D)
    mat_E = numpy.zeros((M, N))
    mat_C = numpy.max(mat_C, mat_E)
    block_start = "{"
    block_end = "}"
    mlir_code = f'''
func.func @main_entry() attributes {block_start}llvm.emit_c_interface{block_end} {block_start}
  ...
  %dd = arith.constant dense<{numpy.array2string(mat_D, separator=", ", max_line_width=100000)}> : tensor<{M}x{N}xf32>
  %dzero = arith.constant dense<{numpy.array2string(mat_E, separator=", ", max_line_width=100000)}> : tensor<{M}x{N}xf32>
  ...
  %1 = linalg.add ins(%0, %dd : tensor<{M}x{N}xf32>, tensor<{M}x{N}xf32>) outs(%C : tensor<{M}x{N}xf32>) -> tensor<{M}x{N}xf32>
  %2 = linalg.max ins(%1, %dzero : tensor<{M}x{N}xf32>, tensor<{M}x{N}xf32>) outs(%C : tensor<{M}x{N}xf32>) -> tensor<{M}x{N}xf32>
  ...
  check.expect_almost_eq(%result, %2, %threshold): tensor<{M}x{N}xf32>, tensor<{M}x{N}xf32>, f32
  return
{block_end}
    '''
    return mlir_code
...
def generate_mlir_bf16_2dx4d(M, N, K, tile_m = 32, tile_n = 32, tile_k = 32, dtype_size=2):
    ...
    block_end = "}"
    mlir_code = f'''
func.func @main_entry(%arg0: tensor<{M}x{K}xbf16>, %arg1: tensor<{M}x{N}xbf16>, %arg2: tensor<{M}x{N}xbf16>, %cst: tensor<{N_block}x{K_block}x{tile_k // dtype_size}x{tile_n}x{dtype_size}xbf16> ) -> tensor<{M}x{N}xbf16> attributes {block_start}llvm.emit_c_interface{block_end} {block_start}
    ...
    %3 = linalg.add ins(%2, %arg1 : tensor<{M}x{N}xbf16>, tensor<{M}x{N}xbf16>) outs(%1 : tensor<{M}x{N}xbf16>) -> tensor<{M}x{N}xbf16>
    %4 = linalg.max ins(%3, %arg2 : tensor<{M}x{N}xbf16>, tensor<{M}x{N}xbf16>) outs(%1 : tensor<{M}x{N}xbf16>) -> tensor<{M}x{N}xbf16>
    return %4 : tensor<{M}x{N}xbf16>
{block_end}
    '''
    return mlir_code
...
def generate_mlir_bf16_4dx4d(M, N, K, tile_m = 32, tile_n = 32, tile_k = 32, dtype_size=2):
    ...
    block_end = "}"
    mlir_code = f'''
func.func @main_entry(%arg0: tensor<{M_block}x{K_block}x{tile_m}x{tile_k}xbf16>, %arg1: tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>, %arg2: tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>, %cst: tensor<{N_block}x{K_block}x{tile_k // dtype_size}x{tile_n}x{dtype_size}xbf16> ) -> tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16> attributes {block_start}llvm.emit_c_interface{block_end} {block_start}
    %cst_0 = arith.constant 0.000000e+00 : bf16
    ...
    %3 = linalg.add ins(%arg1, %2 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>, tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>) outs(%1 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>) -> tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>
    %4 = linalg.max ins(%arg2, %3 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>, tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>) outs(%1 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>) -> tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>
    
    return %4 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xbf16>
{block_end}
    '''
    return mlir_code
...
def generate_mlir_f32_4dx4d_generic(M, N, K, tile_m = 32, tile_n = 32, tile_k = 32):
    ...
    block_end = "}"
    mlir_code = f'''
func.func @main_entry(%arg0: tensor<{M_block}x{K_block}x{tile_m}x{tile_k}xf32>, %arg1: tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>, %arg2: tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>, %cst: tensor<{N_block}x{K_block}x{tile_k}x{tile_n}xf32> ) -> tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32> attributes {block_start}llvm.emit_c_interface{block_end}
 {block_start}
    %cst_0 = arith.constant 0.000000e+00 : f32
    ...
    %3 = linalg.add ins(%arg1, %2 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>, tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>) outs(%1 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>) -> tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>
    %4 = linalg.max ins(%arg2, %3 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>, tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>) outs(%1 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>) -> tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>
    return %4 : tensor<{M_block}x{N_block}x{tile_m}x{tile_n}xf32>
{block_end}
    '''
    return mlir_code
