...
void SplitMMonN(Operation *op, SmallVector<Value> &outputs,
                SmallVector<Value> &inputs, TensorType &resultTy, bool isTransA,
                bool isTransB, Location &loc, PatternRewriter &rewriter) {
  /*Split on N axis*/
  std::cout << "split on N" << std::endl;
  int64_t M =
      inputs[0].getType().cast<RankedTensorType>().getDimSize(isTransA ? 1 : 0);
  int64_t N =
      inputs[1].getType().cast<RankedTensorType>().getDimSize(isTransB ? 0 : 1);
  int64_t K =
      inputs[0].getType().cast<RankedTensorType>().getDimSize(isTransA ? 0 : 1);
  SmallVector<Value> splited_weights;
  int64_t target_dim = isTransB ? 0 : 1;
  getSplitedTensors(splited_weights, inputs[1], target_dim, rewriter);
  if (splited_weights.size() != NUM_OF_NUMA)
    return;

  for (size_t i = 0; i < splited_weights.size(); ++i) {
    Value weight = splited_weights[i];
    ...
    Operation *newMM;
    if (isTransA)
      newMM = rewriter.create<linalg::MatmulTransposeAOp>(
          /*location=*/loc,
          /*resultTensorTypes=*/
      ...
      newMM = rewriter.create<linalg::MatmulTransposeBOp>(
          /*location=*/loc,
          /*resultTensorTypes=*/
          tensor.getType().cast<RankedTensorType>(),
          /*inputs=*/ValueRange{inputs[0], weight},
      ...
      newMM = rewriter.create<linalg::MatmulOp>(
          /*location=*/loc,
          /*resultTensorTypes=*/
          tensor.getType().cast<RankedTensorType>(),
          /*inputs=*/ValueRange{inputs[0], weight},
...
void SplitMMonK(Operation *op, SmallVector<Value> &outputs,
  ...
  getSplitedTensors(splited_data, inputs[0], /*target_dim*/ isTransA ? 0 : 1,
                    rewriter);
  std::cout << "splited_data size: " << splited_data.size() << std::endl;
  if (splited_data.size() != NUM_OF_NUMA)
    return;
  getSplitedTensors(splited_weights, inputs[1],
                    /*target_dim*/ isTransB ? 1 : 0, rewriter);
  std::cout << "splited_weights size: " << splited_weights.size() << std::endl;
  if (splited_weights.size() != NUM_OF_NUMA)
    return;

  assert(splited_data.size() == splited_weights.size());
  for (size_t i = 0; i < splited_data.size(); ++i) {
    Value data = splited_data[i];
    Value weight = splited_weights[i];
    Value zero = rewriter.create<arith::ConstantOp>(
        loc, rewriter.getZeroAttr(resultTy.getElementType()));
    ...
    if (isTransA)
      ...
      newMM = rewriter.create<linalg::MatmulOp>(
          ...
          /*inputs=*/ValueRange{data, weight},
          /*outputs=*/tensor);
    IntegerAttr intAttr = rewriter.getIntegerAttr(rewriter.getIntegerType(64),
                                                  i); // 64位整数，值为1
    newMM->setAttr("splited", intAttr);
    outputs.push_back(newMM->getResult(0));
  }
...
Value addN(Value &initTensor, SmallVector<Value> &ins, TensorType &resultTy,
  ...
  // Mark the output as the result of the function (for demonstration
  // purposes)
...
LogicalResult splitSingleMM(Operation *op, PatternRewriter &rewriter) {
  ...
  for (Value operand : input_operands) {
    ...
    llvm::outs() << "input tensor: " << operand << "\n";
    input_tensors.push_back(operand);
  }
  bool isTransA = isa<linalg::MatmulTransposeAOp>(op);
  bool isTransB = isa<linalg::MatmulTransposeBOp>(op);
  llvm::outs() << "is trans B\n";
  llvm::outs() << "input_0: ";
  for (auto s :
       input_tensors[0].getType().cast<RankedTensorType>().getShape()) {
    llvm::outs() << s << ", ";
  }
  llvm::outs() << "\n";
  llvm::outs() << "input_1: ";
  for (auto s :
       input_tensors[1].getType().cast<RankedTensorType>().getShape()) {
    llvm::outs() << s << ", ";
  }
  llvm::outs() << "\n";
  int64_t M = input_tensors[0].getType().cast<RankedTensorType>().getDimSize(
      isTransA ? 1 : 0);
  int64_t N = input_tensors[1].getType().cast<RankedTensorType>().getDimSize(
      isTransB ? 0 : 1);
  int64_t K = input_tensors[0].getType().cast<RankedTensorType>().getDimSize(
      isTransA ? 0 : 1);
  ...
  if (target_dim == 1) {
    ...
    SplitMMonK(op, splites_res, input_tensors, resultTy, isTransA, isTransB,
               loc, rewriter);
    if (splites_res.size() != NUM_OF_NUMA)
      return failure();
...
template <typename opType>
struct SplitMatmulRewriterBase : public OpRewritePattern<opType> {
...
using SplitMatmulRewriter = SplitMatmulRewriterBase<linalg::MatmulOp>;
using SplitMatmulTransposeBRewriter =
    SplitMatmulRewriterBase<linalg::MatmulTransposeBOp>;
using SplitMatmulTransposeARewriter =
    SplitMatmulRewriterBase<linalg::MatmulTransposeAOp>;
...
public:
  ...
  void runOnOperation() final {
    ...
    patterns.insert<SplitMatmulTransposeARewriter>(&getContext());
    // RewritePatternSet patterns(&getContext());
    // patterns.add<
    //     // clang-format off
    //     SplitMatmulRewriter,
    //     SplitMatmulTransposeBRewriter,
    //     SplitMatmulTransposeARewriter
    //     // clang-format on
    //     >(&getContext());
    FrozenRewritePatternSet patternSet(std::move(patterns));
    SmallVector<Operation *> ops;
    getOperation()->walk([&](Operation *op) {
      if (isa<linalg::MatmulOp, linalg::MatmulTransposeBOp,
              linalg::MatmulTransposeAOp>(op))
        ops.push_back(op);
    });
