//===-- StructuredOpMatcher.cpp - Utils for structured Op -------*- C++ -*-===//
//
 ...
 * https://github.com/plaidml/tpp-mlir/tree/main/lib/TPP/IR/StructuredOpMatcher.cpp
 */
...
structured_match::StructuredOpMatcher::dim(
    MatchSelector range, SmallVector<mlir::utils::IteratorType> kinds) {
  predicates.push_back([=](linalg::LinalgOp linalgOp) -> bool {
    size_t upperBound = range.getUpperBound();
    size_t lowerBound = range.getLowerBound();
    if (upperBound == std::numeric_limits<size_t>::max())
...
bool structured_match::withOpChainImpl(
    ...
    SmallVectorImpl<structured_match::TypeCheckFunc> &typeChecks) {
